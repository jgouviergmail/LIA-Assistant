...
<p align="center">
  <strong>Version 1.27.7</strong> — <strong>The assistant was answering with its own diagnosis.</strong> When a writing plan exhausts its automatic repairs, LIA refuses to run it and asks the user to decide — a sound guardrail. But the question asked <em>was</em> the internal issue description: a French-speaking account received <code>for_each pattern issue detected</code>, then <code>Fabricated placeholder contact detail: step_2.to='jerome@example.com'</code> — jargon, a code path, and a made-up address you could believe came from your own address book. Four times in thirty days. Fifteen questions now exist in all six languages, refused at boot if one is missing — and refused in the other direction too, which dropped two questions written for names the code never produces: perfectly translated, perfectly unreachable, and reading like coverage (ADR-195). <strong>And a value you gave is never reinvented:</strong> a recipient supplied in clarification is carried over from the previous plan instead of being re-imagined. The repair cannot overwrite a change of mind, and that is what makes it safe — it does not fire because a parameter changed, but because it holds a documentation address nobody ever asks for. <strong>The guard built for that very case had never guarded anything.</strong> A whole subsystem — tool schema registry, extractor, reference validator, planner-injected guide, ~1,900 lines wired from startup to the plan validator — existed precisely to refuse <code>contacts[0].name</code> before execution. It had <strong>never rejected anything since the first commit</strong>: measured 0 rejections over 254 references, and 28 attempts / 0 successes over 30 production days. Repairing it measured worse than deleting it — the repaired manifest branch rejects <strong>63 perfectly legitimate paths out of 112</strong>. A path's truth is now checked <strong>before merge</strong>, in CI, against the tool's real output (ADR-194): coverage went from 6 to 36 of the 59 tools that publish paths, and thirteen manifests were caught lying. <strong>And commitments can now be corrected</strong>, not only closed — the ledger fills itself from conversation, so it is sometimes wrong: the wording and the deadline are editable, the counterparty and the meaning are not, because changing those would describe a different commitment rather than correct this one. <strong>17,803 backend</strong> + <strong>4,487 frontend</strong> tests, <strong>149</strong> hermetic browser journeys, all six languages. — 2 August 2026.
...
|                           |                                         |                             |                                                                         |
| ------------------------- | --------------------------------------- | --------------------------- | ----------------------------------------------------------------------- |
| **36** functional domains | **505,000** lines of code (excl. tests) | **22,200+** automated tests | **194** ADRs                                                           |
| **194** versions shipped  | **6 languages**, parity enforced in CI  | **466** Prometheus metrics  | [**8.3/10** technical audit, 24 normalized areas](docs/audit/README.md) |
