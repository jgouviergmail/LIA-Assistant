...
<p align="center">
  <strong>Version 1.20.13</strong> — Responses no longer fail or degrade with <strong>Gemini 3.x</strong> models. Gemini 3.x (e.g. <code>gemini-3.5-flash</code>, <code>gemini-3-pro-preview</code>) returns <code>AIMessage.content</code> as a <strong>list of content blocks</strong> (<code>[{"type": "text", "text": "…"}]</code>) instead of a plain string like earlier Gemini and other providers. Several paths assumed <code>str</code>: SSE token streaming aborted with a <code>ChatStreamChunk</code> ValidationError (surfacing as <em>"Un problème est survenu lors de la génération de la réponse"</em> in both Pipeline and ReAct modes), ReAct crashed in <code>format_conversation_history</code> (<code>'list' object has no attribute 'strip'</code>) — even in mixed-provider setups where another model produced the response — plus silent degradations (garbled TTS, distorted token counts, dropped tokens after a HITL resumption, skipped psyche/registry/HTML post-processing). <strong>Fix:</strong> a single defensive primitive <code>coerce_content_to_text()</code> (<code>infrastructure/llm/message_text.py</code>) normalizes any <code>.content</code> shape to text, mirroring LangChain <code>BaseMessage.text</code>; applied at the shared chokepoints (SSE token formatter, <code>format_conversation_history</code>, <code>structured_output</code> fallback) so all callers are protected, and consolidating four pre-existing bespoke implementations (DRY). Non-destructive — messages stored in graph state are never mutated. <strong>Tests:</strong> new helper + regression suites reproducing both crashes; Ruff / Black / MyPy clean; 4447 unit tests green. No DB migration, no new env var, no endpoint change. Docs: <code>docs/technical/LLM_PROVIDER_CONSTRAINTS.md</code>. — 22 May 2026.
</p>
