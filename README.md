...
<p align="center">
  <strong>Version 1.23.11</strong> — <strong>The structural-decoupling release: the identity domain becomes a leaf, the users domain assumes its hub role (ADR-126).</strong> Audit cycle 3's coupling metrics had exposed a textbook Stable Dependencies violation: <code>auth</code>, the most depended-upon domain of the backend (26 afferent domains), itself depended on 14 domains and sat in 11 of the 31 bidirectional import cycles. Three behavior-preserving lots later the boundary is clean — auth = identity &amp; sessions (2 outgoing edges, 0 incoming, 0 cycles), users = the User aggregate, profile and account lifecycle. The <code>User</code> ORM model moved to the users domain <em>byte-identically</em> (proven by blob comparison against HEAD, ~84 import sites migrated, no DB change of any kind); account provisioning joined the deletion cascade in a dedicated <code>AccountProvisioningService</code> (both historical transaction topologies preserved exactly); the borrowed haversine helper and the provider-key probe were promoted to <code>core</code>; and the measurement itself is now a committed instrument (<code>scripts/audit/measure_coupling.py</code>, exact cycle-3 reproduction plus a runtime/typing split — hub cycles relocate to users and are now honestly measured, a trade-off fully documented in the ADR). Zero behavior change by construction: no endpoint, wire contract, transaction topology or log event touched. <strong>Verified:</strong> full backend suites green at every lot boundary (~9,265 unit + 140 integration + 969 agents), MyPy strict 0 errors on 870 files, Docker boot after each lot, existing auth tests passed unmodified. — 11 July 2026.
</p>
