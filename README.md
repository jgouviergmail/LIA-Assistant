...
<p align="center">
  <strong>Version 1.25.33</strong> — <strong>The send button claimed one thing and did another; now the whole composer tells the truth.</strong> With an empty input, the button was accessibly labelled <em>"hold to speak"</em> while showing a send icon — and on a device with no usable microphone it stayed clickable and did <strong>nothing</strong>. The button now shows exactly what a press will do: microphone when it would talk, send when it would send, disabled when there is nothing to do. Around that truth, the composer grew the workflow it was missing: a screenshot pastes straight into the conversation (<kbd>Ctrl</kbd>+<kbd>V</kbd>) through the exact pipeline the paperclip already used — a mixed text-plus-image paste keeps <em>both</em> — a visible landing zone replaces the bare focus ring during drag-and-drop, and every answer gains a share/export menu: the platform share sheet where the browser offers one (<em>feature</em> detection — desktop Windows exposes <code>navigator.share</code> too), a dated Markdown download everywhere. The four-act hero animation became a shared timeline engine now driving the landing <em>and</em> <code>/demo</code> interactively — scene pastilles, pause/replay, a frozen resolution frame per selected scene — with the decorative <code>role="img"</code> kept free of controls. Measurement fixed two lies on the way: the oversized-PDF toast claimed the <strong>image</strong> limit (10 MB) against a 20 MB document cap, and an animation that never fires (hidden tab, motion preference flipped mid-flight) could pin the send icon in microphone territory forever. <strong>16,535 backend</strong> + <strong>3,818 frontend</strong> tests, ratchets lowered (hooks 34→33, complexity 51→50). — 28 July 2026.
