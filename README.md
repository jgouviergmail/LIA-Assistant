...
<p align="center">
  <strong>Version 1.20.21</strong> — <strong>Security and maintenance release: 43 dependency vulnerabilities resolved (open Dependabot alerts 43 → 0) and accurate Prometheus metrics across all workers.</strong> The frontend (<code>next</code> 16.2.7, <code>mermaid</code> 11.15, <code>postcss</code>, plus <code>protobufjs</code>/<code>uuid</code> <code>pnpm</code> overrides) and backend (<code>langchain-core</code> 1.4.0, <code>python-multipart</code> 0.0.30, shipped as the coherent LangChain / LangGraph / OpenAI / Anthropic / FastAPI ecosystem upgrade — PR #145) were patched; real exposure was assessed LOW (SCA hygiene and hardening) and validated by 8431 backend unit tests, a backward-compatible LangGraph checkpoint format, and a clean real-prod smoke. Under <code>uvicorn --workers 4</code> the Prometheus metrics were served by a single worker (the others failed to bind 9091), so real-time dashboards under-counted ~4×; metrics are now aggregated via <code>prometheus_client</code> multiprocess mode (<code>PROMETHEUS_MULTIPROC_DIR</code>, gated on <code>--workers</code>, RAM <code>/dev/shm</code>, non-fatal; <code>mark_process_dead</code> on shutdown), the <strong>45 Gauges</strong> each given an explicit <code>multiprocess_mode</code> alongside five instrumentation fixes — single-worker dev is unchanged. <strong>No schema change, no migration.</strong> <strong>Tests:</strong> Ruff / Black / MyPy strict clean; multiprocess + 2-worker end-to-end validated (0 <code>pid</code>-labelled series across 1874). See <a href="./docs/architecture/ADR-089-Prometheus-Multiprocess-Metrics.md">ADR-089</a>. — 3 June 2026.
</p>
