...
<p align="center">
  <strong>Version 1.21.13</strong> — <strong>Math &amp; LaTeX rendering in chat responses.</strong> A rendering-fix release — frontend only, prompt-nudge, no DB migration. <strong>Formulas now render:</strong> the mathematical and scientific formulas LIA writes (<code>$…$</code> inline, <code>$$…$$</code> display) now render as real KaTeX equations instead of raw <code>$$ P(G) = \frac{…} $$</code> text. <strong>Root cause:</strong> the assistant emits its entire answer as HTML, and <code>remark-math</code> operates on markdown and treats raw HTML blocks as opaque — running <em>before</em> <code>rehype-raw</code> expands them — so math buried in HTML (100% of real answers) was never tokenized. A new hast-level plugin (<code>rehypeMathInText</code>) renders <code>$…$</code>/<code>$$…$$</code> after the HTML is expanded, covering both HTML-wrapped and pure-markdown paths uniformly. <strong>Latent fix &amp; cleanup:</strong> currency amounts (<code>9$</code>) no longer surface a stray backslash inside HTML cards, and the fragile string-level currency-escaping is removed (net −90 lines). <strong>Prompt nudge:</strong> the response directive now guides inline <code>$…$</code> vs standalone <code>$$…$$</code>. <strong>Security:</strong> the new stage sits inside the existing sanitize boundary — it reads only already-sanitized text and emits fixed-class spans, so the XSS posture is unchanged (<a href="docs/architecture/ADR-093-Security-Hardening-Proxy-XSS.md">ADR-093</a> updated). <strong>Tests:</strong> ESLint + <code>tsc</code> strict clean; full frontend suite green (163 tests) including a verbatim production-message fixture and dedicated tokenizer edge-case coverage; i18n key parity across all 6 locales. — 7 July 2026.
</p>
