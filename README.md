...
<p align="center">
  <strong>Version 1.27.6</strong> — <strong>A question about one person left carrying another person's name.</strong> In production, a 360° recap on someone, then a second person's card opened and launched, sent the FIRST one's sentence: four identical rows in <code>conversation_messages</code> and three semantic-pivot cache HITS, which only happen when the very same string arrives. The cause is not in application code — the App Router <strong>restores the search params of the entry it already holds</strong> for a route, so a <code>router.push('/dashboard/chat?draft=…Paul…')</code> came back as <code>?intent=…Marie…</code>, a URL the application cannot build. Three candidate causes were ruled out by experiment rather than reasoning: static prerendering (route forced dynamic, defect unchanged), our own one-shot URL cleanup (a first visit carrying no query, hence no cleanup, still poisons the next link), and the i18n rewrite of the default locale (identical in <code>en</code>, where no rewrite applies). A chat deep link is now a <strong>real navigation</strong>: the browser is the single authority on the URL again (ADR-192). The prefill case is the dangerous one — a <code>?draft=</code> link returning as a stale <code>?intent=</code> does not merely show the wrong text, it <strong>executes</strong> a request the user never made on that click. <strong>And a domain that can only write will push to write.</strong> <em>"When did I last call my wife?"</em> was planned in two steps: find the contact, then <strong>phone her to ask</strong>. Only a failed reference stopped it. Not a whim of the model but the only way to obey: the prompt announces <code>Primary domain: telephony</code>, and that domain's catalogue held <strong>exactly one capability — writing</strong>. Three read capabilities now exist, each in the domain that lacked one — calls, open commitments, relayed messages — and NOT on the contact agent, because measurement showed that making them reachable from <em>contact</em> evicted six <em>mutation</em> tools from the crowded catalogues. A read capability must not cost a write one (ADR-193). <strong>A read must never produce a plan that writes:</strong> a deterministic rule, before any model call, refuses a writing plan when the analyzer read no intent to write — out of reach of the exemption that skipped review for exactly the offending shape. <strong>A published path is a path that exists:</strong> <code>contacts[0].name</code> was advertised and produced by none of the tool's three modes, while the reference validator approved it <em>on the manifest's word</em>. <strong>And identity is folded by the system, corrected by the user:</strong> name folding cannot know that a raw phone number and a name are one relationship, so merging two relationships is manual, never proposed, always reversible, and shown on the card with its own undo. It never touches the peer directory — a display decision must not redirect a message to another account. <strong>17,622 backend</strong> + <strong>4,474 frontend</strong> tests, <strong>149</strong> hermetic browser journeys, all six languages. — 2 August 2026.
...
|                           |                                         |                             |                                                                         |
| ------------------------- | --------------------------------------- | --------------------------- | ----------------------------------------------------------------------- |
| **36** functional domains | **496,000** lines of code (excl. tests) | **22,000+** automated tests | **192** ADRs                                                           |
...
| [GUIDE_TESTING](./docs/guides/GUIDE_TESTING.md)               | Testing strategy (~17,622 backend tests across 947 files) |
| [GUIDE_DEBUGGING](./docs/guides/GUIDE_DEBUGGING.md)           | LangGraph and log debugging                               |

### Architecture Decision Records (ADR)

192 ADR files (ADR-001 through ADR-193 — ADR-008 has no separate file) documenting major architectural decisions:
