...
<p align="center">
  <strong>Version 1.30.3</strong> — <strong>The production build no longer depends on Google's weather.</strong> Two candidate image builds failed the previous day on <code>fonts.gstatic.com</code> 404s — Google rotated its hosted font files mid-build (28 Turbopack errors each, with a rerun as the only palliative). All seven type families are now <strong>self-hosted</strong>: the exact woff2 files (latin subset, same weights, same CSS variables — rendering unchanged) are committed to the repository and served via <code>next/font/local</code>, with a drift guard forbidding any return of <code>next/font/google</code> and verifying every declared file exists. <strong>The qualified-release pipeline reaches its first end-to-end pass</strong>: the disposable self-host installer qualification — never green since its creation — was repaired defect by defect, each measured on a real run (pre-created scaffold mistaken for a git clone, prebuilt legs silently degrading to local mode, a runtime harness swallowed by <code>|| true</code>, a cleanup trap tearing down the freshly verified stack before the probes, a test email the login rejects, the dependency catalogue living outside the wizard's embedded tree, and qualification evidence unconsumable by construction). This version ships the repaired end-to-end chain: digest-addressed candidate → hermetic multi-arch qualification → qualified publication. — 17 August 2026.
