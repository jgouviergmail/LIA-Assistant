...
<p align="center">
  <strong>Version 1.23.13</strong> — <strong>Companion presence & embodiment: LIA follows you across the dashboard.</strong> A floating mini-avatar now lives on every dashboard page except chat (where the full avatar already is): it rests as the current mood emoji, shows a "thinking" bubble when a background run is active (ADR-117 <code>runs/active</code> poll), and carries an unread-notification badge fed by a layout-level SSE — click it to jump to chat, minimize it to a discreet dot. Alongside it, three grounded micro-interactions: the empty-chat greeting becomes time-aware (☕ morning · 👋 day · 🌛 evening · 😴 deep night, with a truthful "LIA consolidates its memories at night" note), a proactive notification makes the avatar wobble once on live arrival (never on history), and a briefing card lights up a "just updated ✨" badge on refresh. The mood-colored bubble glow from v1.23.12 was <em>retired</em> after UAT. Ships with a documentation realignment (236 broken doc links → 0) and a committed <code>doc_audit.py</code> drift instrument. Pure frontend for the features, zero new dependency, hard fallbacks, reduced-motion covered — and a real SSE reconnect-storm bug caught by driving the app end-to-end. <strong>Verified:</strong> 523 frontend tests green (+13), ESLint + <code>tsc --noEmit</code> clean, i18n parity across the 6 locales, runtime-verified in the dev container. — 11 July 2026.
