...
<p align="center">
  <strong>Version 1.21.0</strong> — <strong>Feature release: latency campaign, semantic-layer governance, ReAct turn isolation (ADR-090, ADR-091).</strong> Rich HTML answers start 3–4s earlier (the LLM no longer emits a ~550-token inline <code>&lt;style&gt;</code> block — the <code>.lia-response</code> rules ship once in the app stylesheet), the response node's user-context injections (memory, RAG, journal, portrait, psyche) are prefetched from the initiative node in parallel with its LLM evaluation (both pipeline and ReAct, identical inline fallback), LLM client instances are reused across calls (httpx pool keep-alive), a runtime negative cache eliminates the silent second LLM call on buffered reasoning-stream paths, and SSE tokens are batched per animation frame on the frontend. The semantic layer is overhauled: type↔tool links are now the union of the editorial ontology and the live <code>semantic_type</code> manifest annotations (12 phantom tool names from the v3.2 rename fixed), guarded by 5 test-enforced integrity locks, and feed the planner, the initiative's new <code>&lt;SemanticBridges&gt;</code> connection candidates and the ReAct prompt (PRECISION rule). ReAct fixes: previous turn's data no longer re-displayed (per-turn registry purge) and routes now use the contact's exact address instead of a memory approximation. Plus: shared Redis rate limiter with <code>NOSCRIPT</code> retry (no more permanent fail-open after a Redis restart), skill badge visible for all users, 5 fire-and-forget call-sites made GC-safe. <strong>No schema change, no migration; 4 new optional env vars.</strong> <strong>Tests:</strong> Ruff / Black / MyPy strict clean; full backend suite 8469 unit tests green (+~60 new); verified live in the running container. — 2 July 2026.
</p>
