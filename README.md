...
<p align="center">
  <strong>Version 1.25.23</strong> — <strong>Ten test files had switched themselves off; 219 tests had never run once.</strong> A module-level skip on a missing provider key silenced the HITL classifier, the approval-resume path, the draft executor and the graph builder — and a skipped test is green, so nothing ever reported it. Switched back on, 142 came back red against an API that had moved on. Repairing them surfaced defects nobody had seen: the voice <strong>split every number in two</strong> — "it's three." then, in a separate audio clip, "point five degrees" — because both sentence splitters treated any period as a full stop, prices, version numbers and URLs included. A reminder was <strong>lost for good</strong> when the user hit their usage budget in the minute it fired: the row kept the lease it had taken and the next pass only ever looks at pending ones. A Chinese user received every reminder title in English, from a table keyed on the wrong spelling for the third time in this codebase. And the value someone had just corrected in a confirmation <strong>vanished from the conversation</strong> for every tool but one, because the message that replaces their own turn was wired to a single tool name. Deleting an email was announced to the classifier as <em>sending</em> one, so the prompt's own delete-safety rule could never fire. Server-side error details now reach the user instead of a generic sentence, in the twenty-one places that read the wrong response shape. The agents CI job goes from 978 to <strong>1,158 tests actually executed, none skipped</strong>; the guard's exemption list is empty. <strong>15,877 backend</strong> + <strong>2,832 frontend</strong> tests, coverage floor 61 → <strong>62 %</strong>, MyPy strict clean, all ratchets hold. — 26 July 2026.
...
| [GUIDE_TESTING](./docs/guides/GUIDE_TESTING.md)               | Testing strategy (~15,877 backend tests across 837 files) |
