...
<p align="center">
  <strong>Version 1.23.8</strong> — <strong>The unified error taxonomy reaches the whole backend (ADR-124).</strong> The 33 remaining raw <code>raise HTTPException</code> sites in routers, services and validation modules — untyped, invisible to Prometheus, unlogged by the central mechanism — are migrated to the centralized raisers and <code>BaseAPIException</code> subclasses of <code>src/core/exceptions.py</code>, closing the phase ADR-114 had deferred: review rule #18 now holds repo-wide, <code>grep "raise HTTPException" src/</code> returns <em>zero hits, no exemptions</em>. The external API contract is proven byte-identical the same way ADR-114 did it — 47 contract tests pinned every site's status/detail/headers on the PRE-migration code and carried over green by inheritance, plus TestClient edge parity for the five new exception classes (structured 422, 413, 502, 410, dict-capable 429/409) and an end-to-end run through the real SSE generator for the HITL rate-limit path. One deliberate, user-approved fix ships with it: inconsistent heartbeat settings now return a clear 422 in the user's language (6 locales) instead of a generic 500. Every migrated error path now feeds structured logging and Prometheus error metrics automatically, a CI grep guard prevents recurrence, and the file-size ratchet forced an honest arbitration — <code>core/exceptions.py</code> split behind a façade with zero consumer import changes, caps lowered 928 → 848. <strong>Verified:</strong> 9,143 unit + 138 integration tests, MyPy strict on 867 files, fresh Docker boot to healthy. — 10 July 2026.
</p>
