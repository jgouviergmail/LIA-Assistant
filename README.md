...
<p align="center">
  <strong>Version 1.21.9</strong> — <strong>HITL internationalization &amp; domain-vocabulary fixes.</strong> The Human-in-the-Loop layer was structurally French: for the five other languages, the approval / edit / refusal messages are now localized and the response classifier — its few-shot examples externalized to a versioned English prompt — is no longer biased by French-only examples (<a href="docs/architecture/ADR-103-HITL-Backend-i18n.md">ADR-103</a>). <strong>Cross-domain reference queries bypass the LLM planner again</strong> ("the restaurant of this meeting", "directions to that address"): a plural-vs-singular vocabulary mismatch had silently disabled the LLM-free shortcut (~800 ms/query), plus ordinal reference resolution ("show me the 2nd one") and <code>$context.file</code> references (<a href="docs/architecture/ADR-102-Domain-Vocabulary-Single-Source.md">ADR-102</a>). <strong>Two permanent parity guards</strong>: a backend i18n guard (found a missing <code>zh-CN</code> string) and a domain-vocabulary guard, both axis-aware. <strong>No DB migration, no schema change</strong>; one new <code>.env</code> behaviour toggle (<code>PLANNER_CROSS_DOMAIN_BYPASS_ENABLED</code>). <strong>Tests:</strong> Ruff / Black / MyPy strict clean; parity guards + behavioural red-first + an end-to-end "no French for a German/Chinese user" guard; full unit suite green; i18n key parity across all 6 locales; Docker startup verified healthy. — 5 July 2026.
</p>
