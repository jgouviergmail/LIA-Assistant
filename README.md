...
<p align="center">
  <strong>Version 1.47.3</strong> — <strong>Each person chooses the rhythm of their exchanges.</strong> Frequent exchanges bind every tool so the provider's cache reads each ReAct turn back from the previous one, occasional ones keep a steady cost; a turn's history no longer shifts mid-turn, and the logs no longer carry the person's words — 24 September 2026.
</p>
...
|                           |                                         |                             |                                                                         |
| ------------------------- | --------------------------------------- | --------------------------- | ----------------------------------------------------------------------- |
| **51** functional domains | **720,000+** source lines (excl. tests) | **41,000+** automated tests | **310** ADRs                                                            |
| **268** versions shipped  | **6 languages**, parity enforced in CI  | **587** Prometheus metrics  | [**8.3/10** technical audit, 24 normalized areas](docs/audit/README.md) |
...
| Metric                  | Value                                                                                                                                            |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| Backend tests           | 32,106 collected over `tests/` (`pytest --collect-only -q`, 1,937 files, 2026-09-24)                                                             |
| Frontend tests (vitest) | 9,071 passed across 729 files (`task test:frontend:coverage`, 2026-09-24), plus hermetic Playwright journeys with axe, dark-mode and zoom checks |
...
310 ADR files (ADR-001 through ADR-311 — ADR-008 has no separate file) record every major architectural decision with its context, the alternatives and, increasingly, the production measurement that motivated it. Three to start with, and [the full index](docs/architecture/ADR_INDEX.md):
