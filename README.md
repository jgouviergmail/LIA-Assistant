...
<p align="center">
  <strong>Version 1.20.15</strong> — <strong>Observability hardening.</strong> A panel-by-panel audit of every Grafana query against the <strong>production</strong> Prometheus/Loki repaired the dashboards. Sub-agent execution metrics (<code>subagent_spawned/active/duration/tokens/errors</code>) are now emitted from <code>ReactSubAgentRunner</code> — the <em>Sub-agents &amp; Skills</em> dashboard was previously dark because <code>metrics_subagent.py</code> was defined but never imported or incremented — and an active-count gauge leak (missing <code>finally</code> around setup) was fixed. Dozens of panels stuck on <em>No data</em> from metric-name / label / label-value mismatches were corrected: the LLM error taxonomy repointed to the dedicated <code>llm_api_errors_total</code> / <code>llm_rate_limit_hit_total</code> / <code>llm_context_length_exceeded_total</code> / <code>llm_content_filter_violations_total</code> family, Loki event-name filters (<code>router_*_confidence</code>, <code>response_node_exception</code>), embedding <code>model</code>/<code>currency</code> filters (Gemini, USD), a Histogram queried as a Counter, and a wrong HITL resumption grouping label. Nine never-emitted / deprecated metric definitions were removed (including a per-<code>user_id</code> PII label and a cardinality-risk counter). <strong>Tests:</strong> sub-agent instrumentation + gauge-balance regression; Ruff / Black / MyPy clean; 334 observability + runner unit tests green; every changed query validated live against prod. No DB migration, no new env var, no endpoint change. — 29 May 2026.
</p>
