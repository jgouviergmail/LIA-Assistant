...
<p align="center">
  <strong>Version 1.21.12</strong> — <strong>HITL contract coherence.</strong> A confirmation-flow release that unifies Human-in-the-Loop across both execution modes (<a href="docs/architecture/ADR-106-HITL-Contract-Coherence.md">ADR-106</a>) — prompt-free, no DB migration. <strong>Correct batch wording:</strong> a confirmation grouping several sends/creates/updates no longer inherits the deletion wording ("this action is irreversible / confirm this deletion?"); the wording is derived from the ADR-085 draft display registry (<code>verb_past_key</code>), deletes keeping their strong irreversible warning. <strong>Recipient visible:</strong> send-type batch rows now render <code>📧 Email to &lt;recipient&gt; : &lt;subject&gt;</code> — the recipient being the discriminating field the frontend has no card for. <strong>ReAct HITL unified:</strong> the autonomous-mode mutation gate now raises the shared <code>tool_confirmation</code> interrupt (rendered + Redis-persisted) instead of an unwired <code>react_tool_approval</code> value — deleting an email/event or running a user MCP mutation tool in ReAct mode no longer silently hangs, and the pipeline <code>tool_confirmation</code> resume path is repaired in the same branch. <strong>Enforced <code>hitl_required</code> invariant:</strong> four draft-based delete/cancel tools carrying a stale <code>hitl_required=True</code> (which routed them to the broken ReAct gate) were corrected to <code>False</code>, and a boot/CI invariant test now locks the whole tool catalogue. <strong>Tests:</strong> Ruff / Black / MyPy strict clean; new HITL batch-wording, recipient, tool_confirmation-resume and <code>hitl_required</code>-consistency suites; full unit suite green; i18n key parity across all 6 locales. — 6 July 2026.
</p>
