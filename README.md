...
<p align="center">
  <strong>Version 1.25.24</strong> — <strong>The product could be used, but not always found.</strong> On a phone, below 768 px, the navigation bar simply vanished and nothing replaced it: from the chat, settings and help were reachable only by typing a URL. The getting-started checklist pointed six of its seven items at a bare settings page — "choose a personality" landed you at the top of thirty collapsed accordions. Seven of the nine home cards <strong>render nothing at all</strong> when their source is not configured, so a fresh account met two empty cards and seven invisible holes. Usage quotas gave no warning: nothing, nothing, nothing, then a wall, mid-task. Generated images were deleted on a timer and never said so. A failed turn was a dead end — you had to find your question and retype it. And in German at 768 px the logout button sat <strong>235 px outside the viewport</strong>, clipped by an <code>overflow-x: hidden</code> that made every existing reflow guard report zero. This release is that pass: the logo becomes the way to every page on a phone, seventeen settings sections become deep-linkable, the unconfigured cards are named with a link each, the quota speaks at 80 %, the images announce their deadline, a failure offers to replay <em>exactly</em> what was asked, and the 375 written FAQ examples become one tap away from the composer, in all six languages. Nine native <code>confirm()</code> dialogs — the most irreversible actions in the product — become real in-app confirmations. On the telephony side, a vendor refusal used to answer HTTP 200 with <code>success: false</code>: the call never happened, the row stayed <em>dialing</em>, and it blocked every further call for fifteen minutes. <strong>15,954 backend</strong> + <strong>3,460 frontend</strong> tests, <strong>97/97</strong> hermetic browser journeys, MyPy strict clean, every ratchet held and the backend complexity ceiling lowered. — 27 July 2026.
...
| [GUIDE_TESTING](./docs/guides/GUIDE_TESTING.md)               | Testing strategy (~15,954 backend tests across 842 files) |
