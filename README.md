...
<p align="center">
  <strong>Version 1.21.7</strong> — <strong>Regression recovery &amp; robustness.</strong> Fixes the collateral damage of the wave-3 strict CSP and a five-defect incident where a diagram request hung two minutes then rendered an unrelated map. <strong>CSP widget airlock</strong> (<a href="docs/architecture/ADR-098-CSP-Widget-Airlock.md">ADR-098</a>): the strict CSP had blocked three things — voice input (an <code>AudioWorklet</code> loads from a <code>blob:</code> URL, governed by <code>script-src</code> not <code>worker-src</code> → <code>blob:</code> restored), the interactive-map skill (no <code>frame-src</code> → fell back to <code>default-src 'self'</code> → explicit allowlist), and every third-party MCP App widget (Excalidraw &amp; co. load their runtime from external CDNs, which a <code>srcDoc</code> iframe inheriting the parent CSP forbids). Widgets now boot through a same-origin <strong>airlock shell</strong> (<code>public/widget-frame.html</code>) served with its own permissive CSP while the app policy stays strict; isolation still comes from the iframe <code>sandbox</code> (opaque origin), the shell is hardened by five anti-abuse locks, and both policies live in <code>src/lib/csp.ts</code> with every feature-bearing directive pinned by tests. <strong>Structured-output robustness</strong> (<a href="docs/architecture/ADR-100-Structured-Output-Prompt-Conflict-Guard.md">ADR-100</a>): the semantic plan validator had silently failed open (100%) because its prompt said "Output JSON only" while the code forced a tool call, so the reasoning model answered in unparseable text → a runtime <strong>rescue net</strong> now salvages structured output from any model that answers in JSON text, and a prompt-convention sweep cleaned four native-structured prompts (five manual-JSON prompts left untouched). MCP iterative <code>*_task</code> steps get a dedicated timeout family (300 s floor / 600 s ceiling) so complex diagrams stop being killed mid-generation; the planner drops a <code>skill_name</code> that contradicts the request, and the response node no longer dresses a failed plan up as a successful skill answer. Dead, misleading nginx config removed (<a href="docs/architecture/ADR-099-Remove-Dead-Nginx-Config.md">ADR-099</a>). <strong>No DB migration, no schema change</strong>; new <code>.env</code> keys are timeout-tuning knobs. <strong>Tests:</strong> Ruff / Black / MyPy strict clean; new non-regression suites for CSP policies, structured-output rescue, MCP timeout family, plan-skill coherence and the failed-plan guard; frontend + backend green; i18n key parity across all 6 locales; Docker startup verified healthy. — 4 July 2026.
</p>
