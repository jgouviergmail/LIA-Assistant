...
<p align="center">
  <strong>Version 1.23.4</strong> — <strong>Every paid tool now carries a technical ceiling.</strong> A full inventory of the 47 tool modules found that the image generation tools — among the most expensive per-call paid APIs in the catalogue — were the only family with neither <code>@track_tool_metrics</code> nor <code>@rate_limit</code>, and no client-layer limiter either; their only guard was the usage-limits cost cap, per billing cycle and Redis-cached, which a runaway loop could overshoot before it bites. <code>generate_image</code> and <code>edit_image</code> now carry the full standard decorator stack (per-user sliding window, default 10 calls / 5 min, blocked <em>before</em> the paid API call), and the DevOps Claude CLI tool — a paid Claude run plus real SSH actions on a server — gains its own per-user ceiling (5 calls / 10 min). Every threshold is settings-driven, tunable in production without rebuild. <strong>Verified:</strong> 11 new unit tests on the rate-limit contract, registry smoke green (96 tools), full suite 9,082 passed. — 10 July 2026.
</p>
