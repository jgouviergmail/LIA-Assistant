...
<p align="center">
  <strong>Version 1.27.5</strong> — <strong>A tool the catalogue advertises and the executor cannot find.</strong> In production, asking for a 360° overview of a connected peer answered <em>"I could not retrieve their recent interactions"</em>. The logs say exactly what happened: <code>"tool": "get_person_overview_tool"</code>, then <code>"error": "Tool 'get_person_overview_tool' not found"</code>. The planner was right — the catalogue advertised the tool. It was the module loader that never imported <code>person_tools</code>, nor <code>documents_tools</code>, nor <code>automation_tools</code>: <strong>three families advertised, zero registered</strong>. A manifest without a tool is a promise the executor cannot keep, and it is the user who receives the failure. A CI guard now compares what the catalogue advertises against what the registry actually holds, <strong>after import</strong>, exactly as boot does (ADR-190). <strong>And even found, the tool would have answered badly:</strong> it searched mail by the person's NAME and the calendar by a text query — a mail search is matched against MIME headers, and <code>list_events(query=)</code> has no cross-provider parity nor any notion of "this person is an attendee". It now delegates to the services the Relations page uses, which resolve ADDRESSES from your own address book: same identity, same cache, so a 360° asked right after opening a card costs no provider call at all. <strong>The scope is chosen, and that choice is applied.</strong> The request leaves the page as a chat link — prose, and nothing else — so letting the planner infer the scope from a sentence would have made it a suggestion; it is saved before the chat opens and the tool reads it back. <strong>The best-scoring tool in the catalogue was eliminated before its score was ever read.</strong> A 360° on a connected person came back missing its open commitments, its calls and its relayed messages — and not by any whim of the model. The 360° tool lives in the <em>contact</em> domain; catalogue filtering drops out-of-domain tools <strong>before</strong> consulting any relevance; and the analyzer's own instructions send every question about a connected user to the <em>peer</em> domain, stating that the address book cannot answer it. Reproduced exactly: the tool scored <strong>0.853</strong>, the best of the whole catalogue against generic tools at 0.000, and it never reached the planner. When it worked, it was because the model had stepped outside its instructions. A tool can now <strong>declare the domains it is reachable from</strong>, and a single implementation answers that question for all filtering (ADR-191). <strong>And a click is not a sentence.</strong> You press a button, on a named card, with boxes ticked: the system holds that certainty before any model is consulted — then it turned it into French prose and spent three AI stages trying to recover it. The request now carries the <strong>invoked capability</strong>, as data, alongside the sentence you read. The browser names a capability, never a tool; the server decides which read-only tool implements it. Every other tool call that <em>adds</em> something is <strong>kept</strong>; what the capability already answers is dropped, because an unrelated answer placed next to a stated gap contradicts it — an event of your own calendar was being presented as the peer's. <strong>And a connected person absent from your address book finally has their mail and meetings</strong>: exchanges are matched by ADDRESS, and someone you never saved had none. If they turned on address sharing, LIA uses it to search <em>your</em> mail and <em>your</em> calendar — their setting stays the only thing that decides. <strong>And the contact card stops being an extract</strong>, carrying all thirteen blocks the address book holds. <strong>17,415 backend</strong> + <strong>4,447 frontend</strong> tests, all six languages. — 1 August 2026.
...
|                           |                                         |                             |                                                                         |
| ------------------------- | --------------------------------------- | --------------------------- | ----------------------------------------------------------------------- |
| **36** functional domains | **493,000** lines of code (excl. tests) | **21,000+** automated tests | **189** ADRs                                                           |
...
| [GUIDE_TESTING](./docs/guides/GUIDE_TESTING.md)               | Testing strategy (~17,415 backend tests across 933 files) |
| [GUIDE_DEBUGGING](./docs/guides/GUIDE_DEBUGGING.md)           | LangGraph and log debugging                               |

### Architecture Decision Records (ADR)

190 ADR files (ADR-001 through ADR-191 — ADR-008 has no separate file) documenting major architectural decisions:
