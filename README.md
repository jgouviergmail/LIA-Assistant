...
<p align="center">
  <strong>Version 1.23.9</strong> — <strong>The complexity-reduction series reaches its second target (ADR-125): the draft preview renderer leaves the models module.</strong> The cycle-3 audit re-ranked decomposition candidates by cyclomatic complexity and pointed at <code>Draft.get_detailed_preview</code> — a CC ≈ 93, 14-branch <code>elif</code> cascade rendering the HITL confirmation previews from inside a <em>models</em> module. The renderer now lives in <code>drafts/preview_renderer.py</code> as a dispatch table of small per-type functions (max CC 9), extracted under a byte-identical golden characterization net — 61 golden cases across all 16 draft types, mixed modified/preserved cases, 6 languages, timezone conversion — that passed <em>unmodified</em> after the cut (the ADR-122 method, replayed). A boot-time completeness gate (ADR-085 pattern) refuses to start the app if a draft type lacks a renderer. Three long-standing preview defects ship fixed with it, each proven surgical by an old-vs-new golden diff: the hardcoded French "(sans objet)" shown to every non-French user on subject-less email deletions is now localized in all 6 languages (the placeholder was baked in at five layers — all now store the raw truth, localization happens at render time), a forward without an added message no longer renders the literal string "None", and an empty reminder deletion no longer produces a blank preview. The per-function complexity instrument is now committed as <code>scripts/audit/measure_cc.py</code>. <strong>Verified:</strong> 9,242 unit tests, MyPy strict/Ruff/Black clean, file-size ratchet green (<code>models.py</code> 803 → 584 logical SLOC, off the frozen list), fresh Docker boot to healthy, all 16 draft types rendered in-container matching 61/61 goldens. — 11 July 2026.
</p>
