...
<p align="center">
  <strong>Version 1.34.0</strong> — <strong>LIA can now read its own telemetry</strong>. The assistant emitted hundreds of metrics, structured logs and a core of alerts — and read none of it: fully instrumented, entirely blind to itself. This release closes the loop (ADR-247): a deterministic self-check keeps a health snapshot with exact measured values, alerts and critical verdicts converge into one incident memory with runbook-grounded, budget-capped LLM diagnoses, and administrators get it all in-app — a "Platform health" settings panel, proactive notifications, and four read-only chat tools behind "LIA, diagnose yourself". The same knowledge now shapes answers: a known outage is routed around at planning time instead of being rediscovered by timeout, and a failed step is explained honestly from typed error codes — never from raw logs, never invented. Everything is bounded by construction: no free-form query language ever leaves the model, and with the flag off the subsystem does not exist. — 28 August 2026.
