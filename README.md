...
<p align="center">
  <strong>Version 1.21.6</strong> — <strong>Correctness under concurrency, GDPR &amp; sandbox: full-codebase audit (wave 4).</strong> The audit's most insidious class: defects that surface only under <em>concurrency</em> or in <em>account-lifecycle</em> scenarios — invisible to unit tests and static analysis. Reinforced protocol: on top of red-first, <em>every item carries a concurrency or integration test that reproduces the defect before the fix</em> (<a href="docs/architecture/ADR-097-Concurrency-GDPR-Sandbox-Wave4-Audit.md">ADR-097</a>). Concurrency: a shared SQLAlchemy <code>AsyncSession</code> was raced under <code>asyncio.gather</code> in the health overview and the heartbeat aggregator (8 of 10 fetchers), silently dropping context sources via <code>return_exceptions</code> → sequential loop + <strong>one <code>get_db_context()</code> session per fetcher</strong> (7 lost heartbeat sources → 0, proven on real Postgres). Cross-user isolation: singletons held per-request state (planner <code>journal_context</code>, tool <code>runtime</code>, catalogue metrics) that could leak between simultaneous users → explicit parameter + task-local <code>ContextVar</code>s, proven by an interleaved two-user test. Privacy (GDPR): account deletion now purges <code>health_samples</code> / <code>health_metric_tokens</code> and scrubs the last known location (the soft-deleted account means FK <code>CASCADE</code> never fired), and a deleted owner's device can no longer ingest (owner-state token auth). Agents: <code>{current_datetime}</code> was frozen at build time → per-invocation middleware; token accounting was quadratic → index-diff; a dormant <code>ContextEditingMiddleware</code> fell back to an edit object with no <code>.apply()</code> and <em>crashed every agent model call</em> → fixed. Tool history filtering could orphan a <code>ToolMessage</code> from its <code>AIMessage</code> (provider 400) → atomic tool-call units. Security: user skill scripts ran as <strong>root</strong> with the Docker socket visible → each subprocess is <strong>dropped to an unprivileged uid</strong> (socket unreachable) under <strong>CPU/RAM/process/file-size rlimits</strong>; the admin DevOps CLI is unaffected. Tooling: mypy now type-checks against the Linux deployment target. <strong>No DB migration, no schema change</strong>; new <code>.env</code> keys are opt-outable sandbox knobs. <strong>Tests:</strong> Ruff / Black / MyPy strict clean (0 errors / 865 files); backend <strong>10 211 unit tests green</strong> + wave-4 concurrency/GDPR/sandbox integration green; i18n key parity across all 6 locales; Docker startup verified healthy. — 4 July 2026.
</p>
