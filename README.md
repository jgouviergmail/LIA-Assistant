...
<p align="center">
  <strong>Version 1.30.9</strong> — <strong>Settings go master-detail — the rail is the map, the pane is the territory</strong> ([ADR-227](docs/architecture/ADR-227-Reglages-Master-Detail.md)). The settings page used to stack <strong>51 collapsed accordions</strong> across tabs: nothing was scannable, no description was visible before opening, and navigating meant scrolling. It becomes the master-detail shell Linear, Slack, Notion and Stripe all converged on for rich settings: a <strong>permanent rail</strong> lists every visible section — grouped, iconed, one activation away at all times — beside a <strong>pane</strong> showing the chosen section fully open; with no selection, an <strong>overview</strong> presents every section as a card with its description, finally visible. On phones the rail is the landing screen and a pick swaps it for the pane with a back control — the drill-down a phone reader expects. Quick search moves to the top of the rail and now covers <strong>administration</strong> (phase 2 of ADR-172 delivered: the 15 admin sections become searchable and deep-linkable, for superusers only); <code>?section=</code> becomes selection state, so a reload or a shared URL lands on the same pane. The honesty doctrine survives, improved: a legitimately absent section shows an explicit inline empty state and <em>keeps watching</em> — data answering late replaces the message. Under the hood the page hand-lists nothing any more — it renders from the existing tables plus two compiler-complete registries proven against each section's source, ~330 duplicated layout lines disappear, and <strong>one section mounts at a time</strong>: its requests fire on selection instead of twenty sections fetching on tab load. Also in this release: generated documents open <strong>in-app</strong> (CSV as a real table, markdown through the sanitized chat pipeline, native formats as an honest download panel — ADR-226 amendment), and the per-user document-generation opt-in folds into the instance-level capability switch. — 18 August 2026.
