...
<p align="center">
  <strong>Version 1.25.11</strong> — <strong>A widget that fails now says why — and the interactive map finally works everywhere.</strong> Asked from an iPhone, the map was a blank rectangle: the app-wide <code>COEP: require-corp</code> (needed once for the voice wake-word's <code>SharedArrayBuffer</code>) is only lifted by the Chromium-only <code>credentialless</code> attribute — WebKit cancels the embed with no event, no message, no log. Behind that one symptom, measurement found a family: widget payloads lived only in the browser's React state (a reload showed grey "unavailable" boxes), the LLM imitated host-owned widget markup from its own history (duplicate and phantom widgets, proven byte-for-byte in production data), the skill sub-agent had no notion of the user's position (<em>"montre-moi où je suis"</em> sent the literal words <em>"ma position"</em> — then <em>"France"</em> — as Google Maps searches), and the analyzer's skill detection hijacked MCP-domain turns three times in a row (a diagram request drew the water cycle <strong>on a Google Maps embed</strong>, then spawned a parasite skill). The header moves to <code>credentialless</code>, widgets travel with their message (frame privileges recomputed per user at read time), the skill runner receives the resolved location, an MCP-domain guard ends the hijacks, and the airlock shell relays boot failures in plain text — a phone report now names the culprit without a Mac. <strong>Verified:</strong> a Playwright bench replayed the real 450 kB production payload through the real shell on both engines, light and dark, refuting five hypotheses on the way — and destroying one design assumption before it shipped (<code>document.open()</code> erases the Window's listeners). Also in this wave: <strong>16 code-scanning alerts and 7 advisories</strong> remediated at source with <code>pnpm audit</code> blocking again, 109 npm updates, the six founding ADRs reconstituted, and a 215-file documentation realignment. <strong>12,434 backend</strong> + <strong>2,219 frontend</strong> tests, tsc/ESLint/Prettier clean, all ratchets at baseline. — 21 July 2026.
