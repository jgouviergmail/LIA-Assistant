...
<p align="center">
  <strong>Version 1.34.1</strong> — <strong>Two silences, closed</strong>. Long-term memory has a category for the standing instructions you give about how the assistant should work ("answer me more briefly") — the screen shipped, but nothing could ever fill it: the parser rejected that exact category and logged the rejection at debug level, so a whole family of memories was dropped without a trace. The vocabulary now has one authority, checked at startup against every place that restates it. The second silence was the platform's own: hours after the self-diagnostics of v1.34.0 reached production, the host stopped routing traffic for its containers and every outbound call failed while the site stayed up and healthy. LIA reported the symptoms in six minutes and named none of the cause — so it now carries an opt-in outbound-connectivity probe, publishes the unit of every health figure instead of letting the screen guess it, and refuses to boot with a check no probe backs. — 28 August 2026.
