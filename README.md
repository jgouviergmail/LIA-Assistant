...
<p align="center">
  <strong>Version 1.30.2</strong> — <strong>The LangChain/LangGraph ecosystem catches up on five months of upstream fixes — on evidence, not on faith.</strong> langchain 1.3.9→1.3.15, langgraph 1.2.4→1.2.11, langchain-core 1.4.6→1.5.5, all four provider integrations, and the openai 2.54.0 / anthropic 0.122.0 SDKs (openai 3.x is deliberately excluded: it switches to HTTPX2 and stops installing httpx, which LIA pins everywhere). Every hypothesis was validated <em>before</em> touching the repo: dry-run dependency resolution, a real install in a throwaway environment, 8,489 tests executed under the target versions, offline simulations of the four private integration points, and an A/B control against the current pins. Direct wins on components LIA runs in production: tool retries no longer replay definitive failures (paid calls saved), HITL interrupts propagate through retry layers, history summarization no longer loses the conversation when the summary call fails, the nested-subgraph checkpoint-namespace regression (present in the embedded version) is fixed, and an abandoned SSE stream now cancels its subgraphs instead of burning tokens. Anthropic reasoning tokens enter the ADR-220 accounting contract. <strong>~1,750 lines of dead HITL code deleted</strong> (ADR-222): a second, never-wired implementation of HITL resumption — kept green by ~50 dedicated tests — is gone with its Protocol; one implementation of resume remains, so drift between paths becomes structurally impossible. <strong>Context windows now read the admin catalogue</strong> (the hand-maintained table — which already disagreed with it — becomes the safety net), fixing on the way a latent bug that silently degraded every native model profile to 8,192 tokens. <strong>Paid tools get a per-run call ceiling</strong>: image generation, Perplexity and Brave are bounded per request (six tools by default, boot-validated — a malformed value refuses to start rather than silently dropping a cost protection). — 16 August 2026.
...
|                           |                                         |                             |                                                                         |
| ------------------------- | --------------------------------------- | --------------------------- | ----------------------------------------------------------------------- |
| **40** functional domains | **520,000** lines of code (excl. tests) | **23,700+** automated tests | **221** ADRs                                                           |
| **206** versions shipped  | **6 languages**, parity enforced in CI  | **473** Prometheus metrics  | [**8.3/10** technical audit, 24 normalized areas](docs/audit/README.md) |
