...
<p align="center">
  <strong>Version 1.21.5</strong> — <strong>Performance &amp; hardening: full-codebase audit (wave 3).</strong> Three boundaries the earlier waves did not touch — the async/event-loop boundary, the network-exposure boundary, and the XSS trust boundary — plus four localized performance/correctness fixes, each <em>measured before/after</em> (latency or call count) and driven red-first (<a href="docs/architecture/ADR-096-Performance-Boundary-Hardening-Wave3-Audit.md">ADR-096</a>). Event loop: <code>firebase_admin.messaging.send</code>, Pillow resize and two sync embedding calls ran inline on the loop, freezing every concurrent coroutine (SSE included) for 251–496 ms; moved off via <code>asyncio.to_thread</code> / native async embeddings (down to 11–12 ms), guarded by an event-loop stall test. Parallelism: a hot-path domain scan (<code>631→63 ms</code>) and Gmail's N+1 metadata fetch (<code>331→62 ms</code>) now use bounded <code>asyncio.gather</code>. Caching: user display preferences move from one DB query <em>per tool call</em> to a per-worker TTL cache; admin broadcast translations are persisted in a new JSONB column (0 LLM calls on re-read instead of a re-translation on every view). Security: the LLM dashboard greeting is now auto-escaped React children (a booby-trapped event title is inert) behind a new strict <strong>Content-Security-Policy</strong>; in production the 13 internal service ports (Postgres, Grafana, Prometheus, Loki, Tempo, exporters) are loopback-bound so only the web app is LAN-exposed (Docker bypasses ufw — documented). Correctness: invalid display locales (<code>en-EN</code>, <code>zh-ZH</code> — the latter broke Chinese dates) fixed via a <code>LANGUAGE_TO_LOCALE</code> map; the personality translator is now a registered, admin-configurable LLM slot; the Gmail reply body is built before MIME construction. <strong>One DB migration + two <code>.env</code> cache keys</strong> (both opt-outable). <strong>Tests:</strong> Ruff / Black / MyPy strict clean; backend <strong>8687 unit tests green</strong> + agents green; frontend <strong>123 tests green</strong> (+3 XSS); i18n key parity across all 6 locales; Docker startup verified healthy (migration applied, CSP active). — 3 July 2026.
</p>
