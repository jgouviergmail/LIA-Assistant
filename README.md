...
<p align="center">
  <strong>Version 1.25.25</strong> — <strong>Six times in a row, "create a realistic image of a cat" produced no image.</strong> Four independent defects were stacked, each enough on its own, and none left a readable trace: the analyzer wrote the <em>string</em> <code>"null"</code> where it meant "no skill", an empty plan discarded the <code>generate_image</code> tool the router had just picked with a score of <strong>1.0</strong>, high quality needed <strong>138.3 s</strong> measured under a generic 120 s ceiling — unreachable at any setting — and the skills volume had been owned by <code>root</code> since 13 March, which broke <strong>every</strong> import, from the chat and from settings alike. Meanwhile the same answer rendered on desktop and stayed frozen on "Generating the response…" on a phone, indefinitely, while the server had finished and saved it: a suspended tab kills the connection without an end or an error, and the guard meant to protect a live stream was blocking the very resume that would have rescued it. And the fourteen bundled skills all showed an empty thumbnail — the code worked, the images had never been produced. This release closes all of it, each defect sealed by a guard that was first made to fail on purpose. <strong>16,057 backend</strong> + <strong>3,471 frontend</strong> tests, MyPy strict clean, every ratchet held and the analyzer's output contract extracted so its size cap could come <em>down</em>. — 27 July 2026.
...
| [GUIDE_TESTING](./docs/guides/GUIDE_TESTING.md)               | Testing strategy (~16,057 backend tests across 849 files) |
