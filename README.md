...
<p align="center">
  <strong>Version 1.30.4</strong> — <strong>The assistant never again confirms an action its own tools refused.</strong> A production incident, measured request by request: a relayed message three times over the length limit was refused by the tool — yet the answer displayed "Message delivered — full content, unmodified" with a success card. The plan executor was overwriting every unified-format tool verdict with a hardcoded success; the verdict now propagates as-is (failure, message, error code), which re-arms the existing honesty layer end to end — a failed step no longer counts as "executed", the blocker becomes sayable with its cause, and the tool's refusal reaches the answer. <strong>An oversized relayed message is now refused with its numbers</strong>: the enforced limit (2,000 characters, instance-tunable) becomes the first clarification asked, exact and localized in all six languages, with an offer to shorten or summarize — and the catalogue publishes the bound from the setting itself, so configuration and contract can no longer drift apart. <strong>Bulk-operation confirmations state the measured count</strong>, restated after pre-execution (never the theoretical cap), with correct singular agreement in all six languages. — 17 August 2026.
