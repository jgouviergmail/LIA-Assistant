...
<p align="center">
  <strong>Version 1.20.5</strong> — Sub-agent delivery quality: dedicated step timeout + tools whitelist + recursion bump + rewritten prompt + response-node verbatim override. <strong>Why:</strong> after the ADR-083 Phase 2 rewrite of <code>delegate_to_sub_agent_tool</code> onto <code>ReactSubAgentRunner</code>, real "as a senior analyst, use a specialized sub-agent" runs surfaced four compounding failure modes that nullified the value of delegation. (1) <code>subagent_default_max_iterations</code> defaulted to <code>5</code> — LangGraph spent that on ~2 <code>call_model</code> + 2 <code>execute_tools</code> + 1 supersteps, so a sub-agent batching 4-5 parallel searches in pass 1 had no budget left for synthesis and exited with a near-empty <code>final_message</code> (one observed run: 9 iterations, 31 <code>brave_search</code> calls, <strong>47 chars</strong> returned, then <code>GraphRecursionError</code>). Default bumped to <code>10</code> (range 1–30); the ReAct loop now has headroom for ~3–4 tool rounds + synthesis. (2) <code>resolve_tools_for_subagent(allowed_tools=[], ...)</code> fell back to "everything minus <code>SUBAGENT_DEFAULT_BLOCKED_TOOLS</code>" — ~83 read-only tools — and the sub-agent's ReAct loop burned its recursion budget exploring options instead of converging. New <code>SUBAGENT_RESEARCH_TOOLS_WHITELIST</code> env (default <code>brave_search_tool,fetch_web_page_tool</code>, snake_case CSV with a <code>field_validator</code> that rejects dashes / semicolons / uppercase / leading digits at config-load time) switches the resolver to allow-list mode. (3) The generic <code>MAX_TOOL_TIMEOUT_SECONDS = 120</code> killed slow-reasoning sub-agent steps mid-synthesis — pair of dedicated env vars added (<code>SUBAGENT_TOOL_TIMEOUT_SECONDS</code> floor, default 180, range 30–600 ; <code>SUBAGENT_TOOL_MAX_TIMEOUT_SECONDS</code> ceiling, default 300, range 60–900), wired through <code>parallel_executor._execute_step_with_timeout</code> via a <code>tool_name == "delegate_to_sub_agent_tool"</code> branch so other tools keep the global 120 s (unchanged). (4) <code>response_node</code> compressed 26 KB of expert analysis to 2 KB and overlaid the principal's conversational voice on top — the <code>&lt;ResponseGuidelines&gt;</code> rule <em>"do not list/detail results (handled by cards)"</em> is correct for record lists but defeats delegation when the payload is a multi-section expert analysis. New conditional <code>&lt;SubAgentDeliveryOverride&gt;</code> block in <code>response_system_prompt_base.txt</code> detects substantial textual analyses (markdown sections + expert voice + cited sources + several thousand chars) and switches to verbatim restitution. <code>subagent_react_prompt.txt</code> rewritten from ~17 to ~65 lines: explicit value contract ("if a knowledgeable assistant could write the same thing without consulting you, you have failed"), task calibration lexicon (analyse / synthèse / résumé / comparaison → adapted output forms), epistemic rigor (cite URLs, mark inferences, name gaps, no fabrication), execution discipline (1–3 tool rounds preferred, STOP-and-write soft signal), negative voice anchor (no sarcasm / no "tour de contrôle" rhetoric — sub-agent is a written expert document), <strong>NO markdown tables</strong> directive, 6-question pre-emit self-check, 7 explicit anti-patterns. Tests: <code>tests/unit/test_subagent_settings.py</code> 21 cases; <code>tests/unit/domains/sub_agents/test_subagent_prompt.py</code> 8 cases. — 14 May 2026
</p>

<details>
<summary><strong>Version 1.20.4</strong> — Browser agent reliability + Anthropic model-id normalization + LLM-config robustness on model/provider change — 12 May 2026</summary>
<p>
  <strong>Browser agent:</strong> <code>browser_task_tool</code> runs a full nested ReAct loop but was absent from <code>_HIGH_LATENCY_TOOLS</code> in the parallel executor, so the planner's 30/60/120 s <code>timeout_seconds</code> cancelled it mid-task (observed step durations of exactly 30 009 / 60 020 / 120 009 ms, then <code>asyncio.CancelledError</code> inside <code>browser_task_tool</code>) — it now has a dedicated floor <code>BROWSER_TOOL_TIMEOUT_SECONDS = 300 s</code> and ceiling <code>MAX_BROWSER_TOOL_TIMEOUT_SECONDS = 600 s</code>; <code>BROWSER_AX_TREE_MAX_TOKENS</code> default raised 5000 → 15000 (form controls were past the truncation cutoff); new <code>BROWSER_REACT_MAX_ITERATIONS</code> env (default 15, range 1–50) caps the loop's <code>recursion_limit</code> — mirrors <code>REACT_AGENT_MAX_ITERATIONS</code> / <code>MCP_REACT_MAX_ITERATIONS</code>; <code>browser_agent_prompt</code> rewritten from web-research best practices (read-vs-transactional task modes, no fabricated query-param URLs, observe→act→verify with a mandatory snapshot after state-changing actions, stale-<code>[ref]</code> handling, autocomplete → click-the-suggestion not Enter, <code>fill</code> only on real input controls, stuck-loop detection, stop-before-payment) and added to the <code>PromptName</code> Literal. <strong>Anthropic model ids:</strong> <code>claude-opus-4.6</code> / <code>claude-opus-4.5</code> / <code>claude-sonnet-4.6</code> / <code>claude-haiku-4.5</code> (dotted, as stored in the catalogue / <code>llm_config_overrides</code>) → the dashed API form <code>claude-opus-4-6</code> …; the dotted ids returned <code>404 not_found_error</code> from the Anthropic API, so any LLM type pointed at one (e.g. <code>browser_agent</code> after the admin picked "Claude Opus 4.6") crashed at instantiation — renamed everywhere + new Alembic migration <code>rename_anthropic_model_ids_001</code> (idempotent, reversible; DEV migrated, PROD via the equivalent in-transaction SQL). <strong><code>reasoning_effort</code> ↔ model coherence:</strong> switching a model/provider could leave a <code>reasoning_effort</code> whose shape no longer matched the new model's <code>reasoning_widget</code> (e.g. a DeepSeek <code>{"effort":"off"}</code> enum value on a config whose effective model became the Qwen <code>toggle_budget</code> default) and crash the typed reasoning builder at <code>get_llm()</code> time — three layers now prevent it: the admin UI normalizes <code>reasoning_effort</code> to the new model on change (<code>coerceReasoningEffortForModel</code>), <code>LLMConfigService.update_config</code> validates against the effective model (422), and <code>merge_config</code> → <code>_reconcile_reasoning_effort</code> drops any still-incompatible value at merge time (falling back to the model default, logging <code>llm_config_reasoning_effort_dropped</code>) — shared non-raising predicate <code>reasoning_effort_matches_widget</code>. New tests: <code>TestReasoningEffortMatchesWidget</code>, <code>TestEffectiveConfigReasoningReconciliation</code>, <code>reasoningHelpers.test.ts</code> (10 vitest).
</p>
</details>
