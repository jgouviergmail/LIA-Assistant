...
<p align="center">
  <strong>Version 1.23.3</strong> — <strong>Faster on every request, and a semantic layer that finally keeps its promises (<a href="docs/architecture/ADR-120-Semantic-Evidence-Expansion-And-Param-Guard.md">ADR-120</a>, <a href="docs/architecture/ADR-121-Semantic-Annotation-Backfill.md">ADR-121</a>).</strong> The router's semantic pivot now runs in parallel with memory resolution — <strong>−1.1s p50 on the first token of every graph turn</strong> (measured, n=5), with the response context prefetched at router entry and a new per-stage duration histogram to keep it measurable in production. The recurring "route to my brother" failure — a person's <em>name</em> geocoded to a random location, wrong route cached — is closed at three levels: deterministic person-reference evidence, a manifest-driven runtime guard in both execution modes (a person name can no longer reach an address/email-typed parameter), and an unresolvable-destination error in the routes tool. The audit behind it found <strong>73 of 99 ontology types consumed by no tool manifest</strong>: ~130 annotations later (params 14%→53%, outputs 22%→40%), cross-domain chains like "email the attendees of this meeting" or "reply to the sender" are finally expressible — each pinned by tests on the real manifests. Background runs harden against hard kills: safety TTL on every stream from the first chunk, atomic listener counters (Lua), and orphaned streams now end with a clear localized error instead of an infinite wait. <strong>Verified:</strong> full suite 9,093 passed, TTFT protocol and per-stage baselines committed (<code>scripts/perf/measure_ttft.py</code>, <code>docs/optim/LATENCY_PLAN.md</code>). — 10 July 2026.
</p>
