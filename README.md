...
<p align="center">
  <strong>Version 1.45.2</strong> — <strong>A knowledge document joins a message, a mail attachment is read, a Drive folder is a tree.</strong> The composer's « + » offers every document of your knowledge spaces, a paused space included, as a copy for that message; the assistant reads the attachments of your e-mails — a document as text, a picture or a scan through the vision model — and treats them as what a stranger sent; a linked Drive folder is synced with its sub-folders and, past a threshold, states the exact number of files before anything starts; long web syntheses fold behind « see more », code stays inside its card, every read of a space reaches your register, and iPhone photos are decoded on the server — 18 September 2026.
</p>
...
|                           |                                         |                             |                                                                         |
| ------------------------- | --------------------------------------- | --------------------------- | ----------------------------------------------------------------------- |
| **49** functional domains | **660,000** lines of code (excl. tests) | **38,000+** automated tests | **296** ADRs                                                           |
| **263** versions shipped  | **6 languages**, parity enforced in CI  | **565** Prometheus metrics  | [**8.3/10** technical audit, 24 normalized areas](docs/audit/README.md) |
...
- **Prometheus**: 565 custom metrics (agents, LLM, infrastructure). A metric nobody can see is a metric nobody acts on: every one must be wired to a Grafana panel, a recording rule or an alert, and a shrink-only ratchet fails the build on a newly blind metric.
...
296 ADR files (ADR-001 through ADR-297 — ADR-008 has no separate file) record every major architectural decision with its context, the alternatives and, increasingly, the production measurement that motivated it. Three to start with, and [the full index](docs/architecture/ADR_INDEX.md):
