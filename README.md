...
<p align="center">
  <strong>Version 1.20.18</strong> — <strong>Rich HTML display stays consistent across multi-turn conversations + exact token/cost accounting on the reasoning path.</strong> In the <em>HTML enriched</em> display mode, the response LLM's conversational history treated prior turns asymmetrically (HTML answers reduced to a placeholder, Markdown answers kept verbatim), so over a long conversation the visible history looked uniformly plain and the model drifted back to Markdown, overriding the HTML directive. Fixed by <strong>symmetric style neutralization</strong> of prior assistant turns — scoped to the HTML mode only (<code>filter_for_llm_context(neutralize_formatting=…)</code>): the formatting is stripped <em>structurally</em> (content preserved) so no style precedent survives, the current turn's content to render is untouched, and the <code>cards</code>/<code>markdown</code> modes stay byte-for-byte unchanged. Separately, a proactive (<em>initiative</em>) LLM call could be recorded <strong>twice</strong> — once correctly attributed, once as <code>unknown</code> — because the reasoning-streaming path (<code>astream_events</code> over a tool-bound model) flattens the inherited <code>CallbackManager</code> into a list and re-roots the run, firing <code>on_llm_end</code> twice. Fixed at the source with <code>enrich_config_preserving_callbacks</code> (keeps a single firing) and locked by a <strong>symmetric idempotency guard</strong> on both the DB (<code>TokenTrackingCallback</code>) and Prometheus (<code>MetricsCallbackHandler</code>) handlers, so the Debug Panel total, usage logs and user statistics count each call once. <strong>Tests:</strong> Ruff / Black / MyPy strict clean; message-filter, token-dedup and a real-Pregel double-fire reproduction suites green; no DB migration, no new env var, no endpoint change. — 2 June 2026.
</p>
