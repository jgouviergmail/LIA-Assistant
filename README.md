...
<p align="center">
  <strong>Version 1.35.0</strong> — <strong>A search that gets cut short says so</strong>. Asked for the transfer times on a flight, the assistant answered "give me a minute, I'll get you those numbers" — and the turn ended there. It had not refused to act: it had run six reasoning steps, called a tool at every one, and been cut off by its own budget, leaving the user a promise instead of an answer. Three things changed. A message that still carries pending tool calls can no longer be served as a reply — the answer is rebuilt from what the tools actually returned, states in one sentence that the search was interrupted, and offers to continue, never announcing work it will not do. The iteration budget is now earned with results: the assistant keeps going while it keeps finding things, instead of stopping on a count that measured how wide a question was rather than how deep its answer lay. And the autonomous mode finally reasons with your long-term memory — the standing instructions you have recorded about how it should work now reach the decisions, not just the wording. — 28 August 2026.
