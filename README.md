...
<p align="center">
  <strong>Version 1.21.26</strong> — <strong>Frontend test foundation (ADR-116).</strong> The product's most critical state machine — chat SSE streaming, HITL approvals, voice mode — was protected only by <code>tsc</code>/<code>eslint</code>, and the coverage tooling was fictional: the provider was never installed and the CI flag was silently swallowed by pnpm, so no frontend coverage report had ever existed. The foundation lands in three layers: pure logic at <strong>100% on all four metrics</strong> (chat reducer with deep-frozen states proving immutability, all 19 SSE handlers, both zustand stores), hooks driven over scripted I/O (the full HITL interrupt→resume cycle through the real pipeline; the entire voice machine — wake word, VAD, transcription, TTS — with zero real audio), and ratcheted coverage thresholds that now gate CI (locked at 100 where earned, never lowered). Writing the tests surfaced and fixed four user-facing bugs: an infinite voice "processing" spinner on WebSocket drop (stale closure), an unhandled rejection after every wake-word recording, a zustand <code>setError</code> that corrupted the voice state key, and chat error bubbles hardcoded in French around English text — the shipped-but-never-wired <code>ChatStreamError.i18nKey</code> mechanism now localizes them in all 6 languages. The SSE wire contract was purged of 8 never-emitted types and is locked by an executable frontend↔backend symmetry test that re-parses the backend Literal. <strong>Verified:</strong> 434 vitest tests green under active thresholds, threshold blocking demonstrated both ways, 62 pytest SSE tests green, i18n parity across 6 locales, both apps boot healthy. — 9 July 2026.
</p>
