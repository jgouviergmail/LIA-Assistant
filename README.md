...
<p align="center">
  <strong>Version 1.23.6</strong> — <strong>The core-agent decomposition reaches the streaming monolith.</strong> The largest function of the backend — the 1,135-logical-SLOC SSE generator driving every chat turn — sheds its densest coupling: the full voice/TTS state machine (progressive chat TTS, parallel agent-mode voice, three finalization paths, cost backfills, teardowns) moves verbatim behind an explicit typed seam, <code>VoiceStreamCoordinator</code> (ADR-122). The cut follows the Feathers characterization-first method proven at v1.21.15: a golden net of 11 scenarios pinning the exact ordered SSE sequences (HITL interrupt/resume, all three voice paths, error exits) was written first and re-passed <em>identical and unmodified</em> after the extraction — <code>service.py</code> drops 35% (1,585 → 1,031 logical SLOC). Two new quality guards make regressions structural rather than disciplinary: a <strong>file-size ratchet</strong> (no logical file may grow; 57 legacy files frozen at their audited size, caps only go down) and a local <strong>infra-denylist guard</strong> (pytest + pre-commit) keeping private infrastructure values out of tracked files. Logwatch mail settings now render from env at deploy time, and an intermittent xdist flake (leaked ContextTypeRegistry) is fixed at the root. <strong>Verified:</strong> golden net green unchanged before/after, 969 agents + 9,097 unit tests, 53% coverage gate, clean Docker boot. — 10 July 2026.
</p>
