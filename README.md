...
<p align="center">
  <strong>Version 1.20.16</strong> — <strong>Voice no longer reads formatting aloud.</strong> With text-to-speech on, conversational replies in the rich-HTML display mode were being spoken with their markup (tags and inline CSS) instead of the answer. The rich-HTML response directive is now scoped to tool/data turns only — keyed on the router's <code>route_to == "planner"</code>, the exact signal the voice path uses, so the display gate can never desync from it — and conversational replies stay plain Markdown (rendered identically by the frontend via <code>ReactMarkdown</code> + <code>rehypeRaw</code> in every display mode). A defense-in-depth pass also strips HTML to speakable text at the synchronous TTS entry points (reference turns, post-LLM data cards, sync fallbacks), but <em>only</em> when real markup is present — so prose containing bare angle brackets (<code>x &lt; 5 and y &gt; 3</code>) is left intact. The <code>route_to</code> lookup reuses the canonical <code>get_qi_attr</code> helper (object- and dict-form safe). <strong>Tests:</strong> Ruff / Black / MyPy clean; 20 unit tests (directive gating + TTS sanitization, incl. the angle-bracket guard); Docker startup verified healthy. No DB migration, no new env var, no endpoint change. — 30 May 2026.
</p>
