...
<p align="center">
  <strong>Version 1.21.2</strong> — <strong>Security & robustness hardening (ADR-093): trusted proxy chain, XSS sanitization boundary, reliable math rendering.</strong> Two coupled security posture fixes: production API ports are now <strong>loopback-bound</strong> (cloudflared is the single public entry — LAN clients can no longer bypass Cloudflare) with uvicorn <code>--proxy-headers</code>, so <code>request.client.host</code> finally carries the real visitor IP — per-IP rate limiting becomes effective per visitor (it was one shared global bucket via the Docker gateway), GeoIP dashboards populate, and the spoofable raw <code>X-Forwarded-For</code> read is removed from the auth path. And the chat markdown pipeline gets its missing <strong>XSS boundary</strong>: <code>rehype-sanitize</code> with a schema audited against every legitimate card/rich-HTML producer (<code>script</code>/<code>iframe</code>/<code>form</code>/handlers dropped; MCP App iframes stay outside markdown via sandboxed widgets). Robustness riders: LangGraph state schema migrations are finally <strong>wired into checkpoint loading</strong> (new 1.3 step formalizes the ADR-092 keys), response-node state writes persist by contract instead of shared-reference side effects, the 4 custom middlewares are rewritten as <strong>pure ASGI</strong> (no more per-request task-group + per-SSE-chunk re-wrapping), and the HITL token guard is language-agnostic. Math rendering is fixed end to end: a pre-existing crash (curly braces from LaTeX/MCP history breaking the prompt template — conversations stuck on "Generating response...") is closed, and formulas now render whatever notation the model picks (<code>$…$</code>, <code>$$…$$</code>, <code>```latex</code> fences, <code>\[…\]</code>, <code>\(…\)</code>) while dollar amounts stay literal (MathJax delimiter rules). <strong>No DB schema change, no migration; no new env vars (the loopback + forwarded-allow-ips pair is a deliberate coupled invariant).</strong> <strong>Tests:</strong> Ruff / Black / MyPy strict clean; backend 8518 unit tests green (+17 new), frontend 118 green (+18 new); middleware stack verified live (headers, request-id, SSE). — 3 July 2026.
</p>
