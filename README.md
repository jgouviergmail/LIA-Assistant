...
<p align="center">
  <strong>Version 1.25.12</strong> — <strong>Six quick wins move the daily surfaces from consultative to actionable.</strong> The ⚙ execution trace used to die with the page: a server-side capture now mirrors the live accumulator step for step (i18n keys only — the PII guard is structural, free text has no slot to land in) and a reloaded conversation shows the same "N steps · X s" backstage the user watched live. The amber "Reconnect" banner used to fire only on the run that broke a connector — resolution-time detection now brings it back on every later run, through two paths because the audit found the dominant one <em>returns</em> its error instead of raising it. History search stops lying by omission: accent-insensitive on both ends, a match counter, in-bubble highlighting through a post-sanitize rehype plugin (the XSS boundary stays intact), and a whole-history panel whose dated results jump into place with a keyset cursor — plus an honest "back to present" path, and the search finally exists on mobile. The briefing cards become real buttons that open the chat prefilled with a contextual intent (<em>"Résume-moi le mail … et propose une réponse"</em>); the compiled portrait — one of the product's most singular artifacts, buried three levels deep — gains a settings shortcut, a dashboard hint and an onboarding mention; the chat header goes quiet (the permanently pulsing "online" pill is gone, totals fold into the context pill's tooltip); and ordinary responses finally get the 👍/👎 the proactive notifications always had — persisted cross-device, feeding the journals' evidence/contradiction counters on the first verdict only, with a one-line "what went wrong?" on 👎 and never an automatic regeneration (<code>response_feedback_total</code> is the first direct satisfaction metric on ordinary answers). Two ratchets moved the right way: the streaming hotspot shrank back under its frozen cap, and the frontend complexity baseline was <em>lowered</em> (max 74 → 60). <strong>Verified end-to-end on the dev stack:</strong> the archived trace reread and expanded in a real browser, a forged <code>ERROR</code> connector emitting the reconnect chunk in a live SSE stream, and a verdict flipped in-browser landing in the database and on <code>/metrics</code>. <strong>12,468 backend</strong> + <strong>2,282 frontend</strong> tests, tsc/ESLint/Prettier clean, i18n parity ×6, domain cycles at baseline. — 22 July 2026.
