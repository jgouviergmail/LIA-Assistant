...
<p align="center">
  <strong>Version 1.21.3</strong> — <strong>Reliability: full-codebase audit (wave 1) + hardening.</strong> Fourteen confirmed, surgical, single-function fixes, each with a regression test written red-first. Connected services: Apple Calendar events now honor the requested timezone (a 10:00 Europe/Paris event no longer lands at 10:00 UTC), Microsoft Calendar search tolerates apostrophes (<code>l'anniversaire</code> no longer breaks the OData filter), the Google Tasks "list task lists" tool works again (a missing <code>()</code> raised on every call), and static route maps render correctly (a polyline-encoder precedence bug corrupted multi-segment geometry). Assistant behavior: ambiguous Gmail labels now ask which one instead of a generic error, and stopping an autonomous ReAct exploration (closing the conversation) actually cancels it (<code>CancelledError</code> was swallowed as a tool error). Connector "not activated" messages are localized in all <strong>6 languages</strong> (no more inline French in LLM-facing scaffolding). Admin & security: <code>/users/search/by-email</code> is superuser-only (account-enumeration surface closed), the planner budget/steps <code>.env</code> overrides are finally honored (one path always served a hardcoded 50$/50), and two previously-hardcoded values (metrics sync interval, MCP ReAct step timeout) become <code>.env</code>-configurable. Reliability riders: registry LRU eviction no longer crashes on mixed timestamp forms in long conversations, a <code>QueryIntelligence</code> field silently dropped on every HITL resume is restored (guarded by a full round-trip test), the LLM capability check is unified, the web client stops leaking an abort timer per request, and a dead never-wired per-node message-windowing subsystem is removed (<a href="docs/architecture/ADR-094-Remove-Dead-Per-Node-Windowing-Helpers.md">ADR-094</a>). <strong>Test infrastructure:</strong> the long-silently-rotted <code>tests/agents</code> suite (83 stale failures, never gated) is rehabilitated to green and wired into CI; a value-blind-test scanner (<code>scripts/analyze_value_blind_tests.py</code>) closes the class of gap the polyline bug exposed (structure-only tests hiding a broken computation) — zero fully value-blind files remain. <strong>No DB schema change, no migration.</strong> <strong>Tests:</strong> Ruff / Black / MyPy strict clean; backend <strong>8572 unit tests green</strong> + <strong>1069 agents tests green</strong> (rehabilitated + CI-gated), frontend <strong>120 green</strong>. — 3 July 2026.
</p>
