...
<p align="center">
  <strong>Version 1.20.19</strong> — <strong>The assistant's personal journal grows up: restraint-first writing, level-routed injection, and cross-mode coherence.</strong> Real usage after the stratified-journal refactor (ADR-079) surfaced entries that were often useless or harmful — over-generalising a stable rule from a single short message, or enshrining a false "I have no access to your data" limitation that then made the assistant refuse to even try. Root cause: a <strong>systemic production bias</strong> in both write prompts with no grounding bar. The extraction prompt is rewritten <strong>restraint-first</strong> (default <code>[]</code>): an operational directive (L1) must be <em>grounded in an explicit user signal you could quote</em> AND <em>safe to obey blindly</em>; generic prohibitions forbid asserting a limit on the assistant's own capabilities/tools and generalising from a single surface feature; <code>L0</code> becomes a capped, private "release valve". The consolidation prompt is de-pressured (L2 synthesis only on genuine convergence). Levels now carry an <strong>operational meaning at read time</strong>: <code>build_journal_context</code> injects only <strong>L1/L2</strong> directives — <code>L0</code> (private feedstock) and <code>L3</code> (carried by the portrait) are excluded via <code>JOURNAL_OPERATIONAL_INJECTION_EXCLUDE_LEVELS</code>; extraction/consolidation still see every level. The <strong>ReAct reasoning loop</strong> now also receives the L1/L2 directives (count-capped by <code>JOURNAL_REACT_CONTEXT_MAX_ENTRIES</code>, no truncation), closing a cross-mode gap. <strong>No schema change, no migration.</strong> <strong>Tests:</strong> Ruff / Black / MyPy strict clean; 223 journal + heartbeat unit tests green. See <a href="./docs/architecture/ADR-088-Journal-Restraint-And-Level-Routed-Injection.md">ADR-088</a>. — 2 June 2026.
</p>
