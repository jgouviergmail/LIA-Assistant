...
<p align="center">
  <strong>Version 1.30.1</strong> — <strong>Token accounting becomes a contract — it used to rest on the provider's politeness</strong> (ADR-220). A counter-audit run against production data refuted an internal audit's worst-case claim (“streamed slots count zero tokens”): 510 out of 510 calls WERE counted over 30 days. The reason was worse than the alleged defect: the request never asked for the count — DeepSeek volunteered it, a behaviour nothing requested, nothing tested, nothing watched. Every chat provider now declares its accounting mode in a boot-verified registry (the app refuses to start on an undeclared provider), clients ask for the count explicitly, and a paid call completing without usage becomes a counter, a warning and a zero-threshold alert instead of an invisible debug log. <strong>The timeout setting the administrator sees is finally the one the system applies</strong> (ADR-221): the per-slot field existed from the admin screen down to the database and then nobody read it — it now reaches every provider client as the per-attempt transport bound, and <strong>no default was applied unmeasured</strong>: six values were raised after confrontation with 30 days of production latencies, two of which already exceeded the old value. <strong>The dashboard said “0 successful actions” since forever — the thermometer was broken</strong>: result classification compared the router intention against a value no router ever emitted, so every completed action was counted as a plain answer; the vocabulary is aligned, contract-pinned on both sides, and a migration reclassifies the history from the archived intention. <strong>Fifteen false memory alerts</strong> traced to a division by zero (a container without a memory limit reported “limit = 0”, the ratio read +∞ and passed the filter) — the guard moved to the denominator and the seven unlimited demonstrator containers received measured ceilings. <strong>Configuration failures finally name themselves</strong>: a bad API key, an exhausted provider credit or a nonexistent model used to get the vaguest message — or a lying “service saturated, retry”; classification now reads the HTTP status first, with three new message families in all six languages pointing at the exact screen where the repair lives. <strong>18,369 backend</strong> + <strong>5,476 frontend</strong> tests, coverage floor raised to 66%. — 16 August 2026.
...
|                           |                                         |                             |                                                                         |
| ------------------------- | --------------------------------------- | --------------------------- | ----------------------------------------------------------------------- |
| **40** functional domains | **520,000** lines of code (excl. tests) | **23,800+** automated tests | **220** ADRs                                                           |
| **205** versions shipped  | **6 languages**, parity enforced in CI  | **473** Prometheus metrics  | [**8.3/10** technical audit, 24 normalized areas](docs/audit/README.md) |
