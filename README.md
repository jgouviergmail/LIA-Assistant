...
<p align="center">
  <strong>Version 1.47.2</strong> — <strong>What LIA sets out to obtain ends obtained or said, and every call is valued at the price the provider bills.</strong> A ReAct turn is judged on its result and a tool failure is stated, never guessed; images are served by OpenAI or Qwen through one declared offer; keyless connectors belong to the instance, and a Google sign-in can no longer activate or unblock an account — 24 September 2026.
</p>
...
|                           |                                         |                             |                                                                         |
| ------------------------- | --------------------------------------- | --------------------------- | ----------------------------------------------------------------------- |
| **51** functional domains | **720,000+** source lines (excl. tests) | **40,000+** automated tests | **309** ADRs                                                            |
| **267** versions shipped  | **6 languages**, parity enforced in CI  | **587** Prometheus metrics  | [**8.3/10** technical audit, 24 normalized areas](docs/audit/README.md) |
...
- **Prometheus**: 587 custom metrics (agents, LLM, infrastructure). A metric nobody can see is a metric nobody acts on: every one must be wired to a Grafana panel, a recording rule or an alert, and a shrink-only ratchet fails the build on a newly blind metric.
...
309 ADR files (ADR-001 through ADR-310 — ADR-008 has no separate file) record every major architectural decision with its context, the alternatives and, increasingly, the production measurement that motivated it. Three to start with, and [the full index](docs/architecture/ADR_INDEX.md):
