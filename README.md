...
<p align="center">
  <strong>Version 1.30.7</strong> — <strong>LIA speaks the Agent Plugins open standard — portable plugins from the whole ecosystem install in one step</strong> ([ADR-225](docs/architecture/ADR-225-Standard-Agent-Plugins-v1.md)). The <a href="https://agent-plugins.org">Agent Plugins v1.0.0</a> standard (TSC: AWS, Cursor, Microsoft, OpenAI, Vercel) defines one portable package format — a manifest, agentskills.io skills, MCP servers — adopted at launch by ChatGPT, Codex, Cursor, GitHub Copilot, Kiro and VS Code. LIA becomes a conformant client under the spec's incremental profile: a plugin published for any of those tools installs into LIA unchanged, from a zip upload or an SSRF-hardened https URL, and brings its skills and streamable-http MCP servers in one step — each behaving exactly like its manually-created counterpart afterwards (activation, HITL, sandbox, quotas). Every install returns an <strong>exhaustive per-component report</strong> — installed, updated, skipped with a translated reason, removed — never a silent partial success: an stdio server (never launched on a multi-user server, by design) or a non-loopback HTTP endpoint is <em>said</em> to be skipped, not pretended installed. Updates are re-imports that preserve configured OAuth credentials; uninstalling removes the plugin and all its components as a group — and that group removal is the only exit: deleting an individual plugin-owned skill or server is refused server-side and guided in the UI toward the plugin. New Plugins settings section (import, listing with component badges, import report, uninstall), provenance badge on plugin-owned MCP servers, six languages. Proven end-to-end at runtime against the real database: install → report → update (renamed-directory edge included) → collision refusal → group uninstall, plus a from-scratch migration replay. — 17 August 2026.
