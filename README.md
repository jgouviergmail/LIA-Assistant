...
<p align="center">
  <strong>Version 1.30.13</strong> — <strong>The self-improvement loops close</strong> ([ADR-232](docs/architecture/ADR-232-cognitive-loops-closure.md), [ADR-233](docs/architecture/ADR-233-ontology-simplification.md)). A production counter-audit of LIA's four cognitive loops — personal journal, learned habits, proactive notifications, cross-domain ontology — verified every hypothesis against live data (PostgreSQL, Prometheus, Loki) and then by <strong>executed simulation</strong>: offline replay of the rhythm detector on the real activity ledger, journal-extraction replay with the production LLM, Monte-Carlo of the quota scheduler. Three loops were running empty, each for a different reason. The proactive <strong>“don't interrupt an active user” guard had never fired once</strong> — it read a phantom attribute, fell back to a nonexistent class and swallowed the error into success; it is rebuilt as an injected probe that genuinely checks the user's latest message, with failures surfacing instead of dissolving. The journal's <strong>deferred self-evaluation loop is unblocked end to end</strong>: delta-driven consolidation (the absolute floor starved every pruned journal and froze portraits for months), verified previous-turn IDs joining the hallucination filter, and an epistemic clamp that makes “high confidence requires evidence” true in code rather than in a docstring. <strong>Thresholds become adaptive per user</strong>: a generic bounded controller (hard bounds 0.55–0.70, one step per 24h, kill switch, 90-day sliding TTL) tunes journal injection to each user's own score distribution — convergence simulated on the real production distribution with zero oscillation, then proven by a real end-to-end chat turn. And the habit detector's <strong>enforced detection bar is now published</strong> where the question arises (Settings), including the calendar-driven weekend asymmetry nobody could see (real bar 0.572 weekdays vs 0.699 weekends behind a displayed 0.55). The semantic ontology sheds its consumer-less reasoning machinery (Wu &amp; Palmer, transitive subsumption, SKOS graph), the domain graph gains identity-bridged adjacencies under a new bidirectional integrity test, and two hypotheses were <strong>refuted with evidence before any fix</strong> — the rhythm detector's “zero habits” verdict is statistically correct, and conversation resets do not starve the evaluation window. — 19 August 2026.
</p>
...
|                           |                                         |                             |                                                                         |
| ------------------------- | --------------------------------------- | --------------------------- | ----------------------------------------------------------------------- |
| **41** functional domains | **540,000** lines of code (excl. tests) | **25,700+** automated tests | **232** ADRs                                                           |
