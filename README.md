...
<p align="center">
  <strong>Version 1.25.29</strong> — <strong>Blocking a subject stopped it coming back, and one turn can no longer erase a profile.</strong> The interest prompt ordered what the user complained about: its first rule declared that any request for information <em>was</em> an interest, which neutralised the rule meant to demand an authentic sign. Production said the rest — <strong>7 of the 10 interests created in July were blocked by the user themselves</strong>, against 0 on the 25 before. Replaying 45 real conversation windows, the previous prompt wrote on 16 of them, and one ordinary turn proposed <strong>19 deletions — the entire active profile</strong>. Three structural defects lived beside it: deduplication only saw <em>active</em> interests, so a subject blocked at 19:14 was re-created at 19:39 under a neighbouring label (0.9821 similarity, far above the merge threshold); dormant rows were invisible the same way, making their revival branch unreachable; and the scan window held 20 rows for 19 active interests. A creation now requires a <strong>named ground</strong> among four and a quote of the words that carry it, deduplication reads every status, and a destructive-action cap drops the batch that proposes nineteen. Measured on two models: noise on negatives <strong>0.50 → 0.00</strong>, recall <strong>0.75 → 1.00</strong>, held-out battery included. Heartbeat notifications finally have feedback buttons — 914 of them had none — and the audit column that stayed empty on 989 rows is wired. <strong>16,388 backend</strong> + <strong>3,491 frontend</strong> tests, MyPy strict clean, every ratchet held. — 27 July 2026.
