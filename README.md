...
<p align="center">
  <strong>Version 1.21.8</strong> — <strong>Calendar-search hardening.</strong> Pipeline-mode calendar searches that worked in ReAct but failed randomly in Pipeline ("my next medical appointments", "the hotel particulier meeting") now succeed (<a href="docs/architecture/ADR-101-Calendar-Search-Hardening.md">ADR-101</a>). <strong>Free-text is no longer sent to the weak Google Calendar <code>q</code></strong> (a "contains" match that never finds a medical appointment titled "Dentiste" and trips on accents): the tool keeps only a person resolved to an attendee email and otherwise lists your events over the window and filters the topic when answering — the list-and-filter model the tasks tool already uses and the behaviour that made ReAct succeed; Gmail, Tasks, Contacts and Drive are untouched (their search is reliable, analysed per-API). <strong>Open/relative queries no longer miss later events</strong>: a new reliable analyzer signal <code>has_temporal_reference</code> (validated 14/14 across the 6 languages) lets the validator drop a planner-hallucinated narrow end date (declared per-parameter via <code>search_role</code>) while preserving explicit dates. <strong>Volumetry cap centralized</strong>: the global per-request ceiling was silently bypassed by five clients (Google Calendar, the three Apple clients, Microsoft Calendar) — all item search/list calls now route through <code>apply_max_items_limit</code>, guarded by a new AST test, and the ceiling is raised 10 → 25 (global + calendar). <strong>No DB migration, no schema change</strong>; new <code>.env</code> keys are a behaviour toggle and two cap knobs. <strong>Tests:</strong> Ruff / Black / MyPy strict clean; +21 tests (date reset, free-text resolution, cap guard, round-trip); full unit suite green; i18n key parity across all 6 locales; Docker startup verified healthy. — 4 July 2026.
</p>
