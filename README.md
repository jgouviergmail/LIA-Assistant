...
<p align="center">
  <strong>Version 1.21.11</strong> — <strong>Latent-debt hardening.</strong> A maintenance release that pays down four independent, isolated debts surfaced by the codebase audit — no user-facing feature change. <strong>Bounded psyche history:</strong> the per-message emotional-evolution snapshots are now purged on write beyond a configurable rolling window (<code>PSYCHE_HISTORY_RETENTION_DAYS</code>, 90 days default), bounding <code>psyche_history</code> growth on constrained hosts. <strong>Leaner query analysis:</strong> the pre-planner memory resolver no longer fires a redundant LLM resolution call when a message carries no anaphoric reference — one model call saved per turn on that path. <strong>Single-source timezone default:</strong> ~30 hard-coded <code>"Europe/Paris"</code> defaults across 19 files now resolve through the central <code>DEFAULT_USER_DISPLAY_TIMEZONE</code>, enforced by a new AST CI guard. <strong>One JSON-LLM parser:</strong> the strip-fences-and-trailing-commas logic reimplemented across six extractors now delegates to the instrumented central parser, made strict-first then tolerant (valid JSON — URLs included — untouched; malformed input repaired incrementally). <strong>Prompt-free, no DB migration.</strong> <strong>Tests:</strong> Ruff / Black / MyPy strict clean; new retention, timezone-guard and JSON-tolerance suites; full unit suite green (8856 passed); i18n key parity across all 6 locales. — 6 July 2026.
</p>
