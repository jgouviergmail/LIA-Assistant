...
<p align="center">
  <strong>Version 1.25.1</strong> — <strong>Security hardening: audit Lot&nbsp;1 remediation, shipped transparent and zero-regression.</strong> Following an independent security audit of 13&nbsp;July&nbsp;2026, this release closes the first wave of fixes — each a pure reinforcement that removes no feature, each shipped with an anti-recurrence guard. <strong>Credentials never reach the logs</strong>: the central structured-log filter now fingerprints OAuth <code>state</code> (correlatable, non-reversible), strips single-use tokens from any logged verification/reset/OAuth URL, and redacts PKCE/<code>client_secret</code> fields — systemically, at every call site. <strong>A real vulnerability-disclosure channel</strong>: an RFC&nbsp;9116 <code>/.well-known/security.txt</code> plus a rewritten <code>SECURITY.md</code> that routes reports to GitHub Security Advisories, corrects the supported versions and drops every unverified claim. <strong>Stronger transport &amp; secrets</strong>: HSTS on the web frontend (production, progressive rollout), and the production <code>.env</code> hardened to <code>0600</code> inside a <code>0700</code> directory at deploy time. A RAG-upload route-escape is closed at the edge (strict UUID validation), and the release-vs-API-contract version drift is removed. <strong>Verified:</strong> Ruff + Black + MyPy strict + tsc + ESLint clean, <strong>10,188 backend unit + 1,278 vitest + 27/27 hermetic deploy tests</strong> green, and the disclosure endpoint served live. — 16 July 2026.
