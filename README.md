...
<p align="center">
  <strong>Version 1.27.11</strong> — <strong>The gap between a label and its field existed in the code, never on screen.</strong> The label primitive kept the browser’s <code>inline</code> rendering, and vertical margins on an inline element are <strong>computed but never rendered</strong>: three successive spacing recalibrations changed the code without moving a pixel. Fixed at the primitive — a label is a block — and the canonical gap was arbitrated <strong>on real screenshots</strong> in a driven browser, then measured back at the pixel (ADR-206). <strong>A row exposes its actions ONE way now</strong> (ADR-208): three category screens had three grammars — hover-revealed buttons a keyboard user <strong>tabbed onto without seeing</strong>, a tap-anywhere full-screen dialog duplicated per screen, and always-on icons. Today: visible icons from tablet up with delete carrying its red at rest, and on phones a single named “⋮” menu — one tap fewer, ~250 duplicated lines gone, and <strong>Export is back on phones</strong> (it was simply removed below 1024 px). <strong>An action has an altitude</strong> (ADR-207): section CTAs are solid and themed, mass deletion is solid red at its neighbours’ size, outline means exactly one thing — a true secondary. <strong>Settings read like an index:</strong> peer connections fold into five badged, icon-carrying zones with mirrored share views; journals fold their whole configuration and move their toolbar below the categories; a routine says <strong>“Weekdays at 08:00”</strong> instead of naming five days, its card down from ~8 always-visible lines to 3. TOTP now enrolls from a <strong>switch</strong> whose thumb only moves on server confirmation; push notifications follow the global opt-in automatically — the per-feature duplicate that could silently mute a feature is gone. Two rules engraved on the way: <strong>a title always carries a theme-coloured icon</strong>, and <strong>grey badges are reserved for the inactive</strong>. <strong>18,016 backend</strong> + <strong>4,925 frontend</strong> tests, <strong>172</strong> hermetic browser journeys replayed against a production build, <strong>23</strong> accessibility audits, all six languages. — 4 August 2026.
