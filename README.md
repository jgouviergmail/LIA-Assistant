...
<p align="center">
  <strong>Version 1.20.12</strong> — Dormant interests are no longer invisible: an interest that decays into the <code>dormant</code> status (low effective weight, no recent mention) now appears in a dedicated, visually-distinct section of Settings with full edit/delete control and an explicit <strong>Reactivate</strong> action (resets the Bayesian signal counters to a fresh state so the nightly cleanup won't immediately re-dormant it), and the counter is split into active / dormant / blocked. <strong>Long-term memory</strong> now surfaces a read-only <strong>purge-risk</strong> indicator: each memory is classified <code>protected</code> / <code>safe</code> / <code>at_risk</code> / <code>imminent</code> from its retention score, so memories approaching the automatic purge are flagged in the list — prompting you to pin them in time. <strong>Why:</strong> both domains auto-decay items in the background (interests → dormant after ~15 days, deleted at 90; memories → hard-deleted below a retention threshold), but the user had lost visibility and control over what was about to disappear. The retention scoring (<code>calculate_retention_score</code> / <code>should_purge</code>) was extracted verbatim from the cleanup scheduler into a pure domain module <code>domains/memories/retention.py</code> (now also exposing <code>classify_purge_risk</code> + a <code>PurgeRiskLevel</code> enum), reused by both the nightly job and the API — no behaviour change to the purge job. Strictly additive: no DB migration, existing endpoints unchanged. <strong>Tests:</strong> new repository / router / retention suites; Ruff / Black / MyPy clean; 8286 unit tests green. Docs: knowledge FAQ (interests lifecycle, memory purge-risk). — 21 May 2026.
</p>
