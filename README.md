...
<p align="center">
  <strong>Version 1.20.20</strong> — <strong>Your custom (user) MCP servers now work in ReAct mode exactly like the pipeline, plus a strict-server reliability fix.</strong> ReAct built its tool palette from the per-request manifests but resolved tool <em>instances</em> only through the process-global registry — so <strong>user</strong> MCP tools, whose instances live only in the per-request <code>user_mcp_tools_ctx</code> ContextVar, were silently dropped (<code>manifest_without_registered_tool</code>) and the loop fell back to generic web search. A shared <code>tool_resolution.py</code> resolver (global registry → hallucinated-suffix strip → user MCP ContextVar) is now the <strong>single source of truth</strong> used by ReAct (<code>ReactToolSelector</code>, <code>_rebuild_wrapped_tools</code>), the pipeline executor (three inline fallbacks unified) and display-metadata lookup; <strong>HITL is read from the in-hand manifest</strong> so user MCP mutation tools keep their approval gate. Because the ReAct loop is <em>itself</em> iterative, <strong>iterative user MCP servers are expanded to their individual tools</strong> (so the model picks them by description) instead of the opaque per-server task tool — <em>except</em> MCP App servers (e.g. Excalidraw), which keep their dedicated agent — gated by the new <code>REACT_MCP_EXPAND_ITERATIVE_ENABLED</code> (default <code>true</code>, instant rollback). A cross-mode fix accompanies it: optional MCP parameters left unset were forwarded as <code>null</code> and rejected by strictly-typed (Go) servers — both adapters now drop <code>None</code>-valued arguments before the call. <strong>No schema change, no migration.</strong> <strong>Tests:</strong> Ruff / Black / MyPy strict clean; 1186+ unit tests green. See <a href="./docs/architecture/ADR-070-ReAct-Execution-Mode.md">ADR-070</a> (amendment 2026-06-02). — 2 June 2026.
</p>
