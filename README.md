...
<p align="center">
  <strong>Version 1.46.0</strong> — <strong>The sandbox becomes a toolbox: a script reaches the web through one door, with keys it never reads, and a question it cannot skip.</strong> In the autonomous mode, the short Python script the assistant writes for a computation may now call the services you connected — through a single egress proxy that swaps a per-run token for your own API key on that host alone, so the key never enters the container; a host nobody permitted is asked for with three answers — with this turn's data, without it, or not at all — and the question is settled inside the loop, so the rest of your request goes on; your answers are kept as permissions you review and revoke from Settings; four roles (compute, diagnose, fill a gap, transform), twenty-two declared libraries verified in the built image, a budget per turn, and every network run claimed in your register before it starts — 18 September 2026.
</p>
...
|                           |                                         |                             |                                                                         |
| ------------------------- | --------------------------------------- | --------------------------- | ----------------------------------------------------------------------- |
| **49** functional domains | **660,000** lines of code (excl. tests) | **38,000+** automated tests | **297** ADRs                                                           |
| **264** versions shipped  | **6 languages**, parity enforced in CI  | **568** Prometheus metrics  | [**8.3/10** technical audit, 24 normalized areas](docs/audit/README.md) |
...
- **Prometheus**: 568 custom metrics (agents, LLM, infrastructure). A metric nobody can see is a metric nobody acts on: every one must be wired to a Grafana panel, a recording rule or an alert, and a shrink-only ratchet fails the build on a newly blind metric.
...
| Metric                  | Value                                                                                                 |
| ----------------------- | ----------------------------------------------------------------------------------------------------- |
| Backend tests           | 29,840 collected over `tests/` (`pytest --collect-only -q`, 1,767 files, 2026-09-18)                  |
| Frontend tests (vitest) | 8,467 across 665 files, plus hermetic Playwright journeys with axe, dark-mode and zoom checks          |
...
297 ADR files (ADR-001 through ADR-298 — ADR-008 has no separate file) record every major architectural decision with its context, the alternatives and, increasingly, the production measurement that motivated it. Three to start with, and [the full index](docs/architecture/ADR_INDEX.md):
