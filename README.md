...
<p align="center">
  <strong>Version 1.30.8</strong> — <strong>LIA writes real documents — CSV, Excel, Word, PowerPoint, PDF, on demand</strong> ([ADR-226](docs/architecture/ADR-226-Document-Generation-Agent.md)). "Research the latest LLM models and formalize the result in a CSV", "make a detailed presentation about Alsace": the answer is now a <strong>finished file</strong>, delivered as a downloadable card below the response — meaningful filename, size, explicit expiry deadline — and chainable on a web research done in the same request. The architecture mirrors image generation (virtual catalogue agent, both execution modes), but the "external generator" is a <strong>dedicated LLM slot</strong> (<code>document_generation</code>, admin-configurable like every other slot) called with structured output typed per format family — tabular (csv/xlsx), sectioned (docx/pdf/md/txt), slides (pptx, speaker notes included) — then a <strong>pure local renderer</strong> builds the exact bytes with the libraries already shipped for RAG extraction (openpyxl, python-docx, python-pptx, PyMuPDF): zero new dependency, zero third-party document service. Spreadsheets are safe by construction (proven formula-injection neutralization with signed-number exemption, Excel BOM, sanitized sheet titles), PDFs open straight in a browser tab, files live in the existing attachments store with the same TTL purge as generated images, opt-out is per user and the capability instance-switchable. Full honesty doctrine: a render failing after the paid LLM call returns an explicit error — never a phantom card; a source-data truncation is reported, never silent. The review also closed a whole trap class for every tool: the catalogue now publishes <code>enum</code> constraints to the planner, exactly as ADR-184 mandated for numeric bounds. — 17 August 2026.
