...
<p align="center">
  <strong>Version 1.20.10</strong> — ReAct mode: the agent's internal reasoning no longer leaks into the final reply, and the answer is no longer shown twice. <strong>Why:</strong> two regressions hit the ReAct answer. (1) The reply reproduced the agent's scaffolding (<em>PLAN … OBSERVATION … CROSS-CHECK … RÉPONSE FINALE</em>) or the model impersonated the agent (<em>"I'll search the address… I need your starting point…"</em>) instead of just answering. Root cause was twofold and converged on <code>response_node</code>: <code>react_setup_node</code> accumulates the full <code>react_agent_prompt</code> — including its <code>&lt;Workflow&gt;</code> PLAN/ACT/OBSERVE/CROSS-CHECK loop and the "you operate in ReAct mode, call tools" role — as <code>SystemMessage</code>s in <code>state["messages"]</code>, and <code>filter_for_llm_context()</code> kept every non-<code>__</code> SystemMessage, so this internal scaffolding reached the response LLM (which mimicked the reasoning structure or impersonated the agent); meanwhile the ReAct loop's clean final answer is handed to the synthesizer via <code>agent_results[…]["data"]["react_synthesis"]</code>, but that entry carries no <code>status</code> key, so <code>_format_status_messages()</code> fell through to <code>"❓ react_agent: Statut inconnu"</code> and the answer was <strong>silently dropped</strong> — leaving the model to reconstruct one from the polluted context. A controlled A/B (deepseek-v4-flash, temp 1.0) confirmed the trigger: with the scaffolding in history the leak/role-confusion appeared in ~20 % of runs; with it removed, 0 %. (2) LangGraph <code>stream_mode="messages"</code> emits <strong>both</strong> the LLM token deltas (<code>AIMessageChunk</code>) and the complete post-processed <code>AIMessage</code> the node returns to the <code>messages</code> channel, so the whole reply was streamed a second time before a post-loop <code>content_replacement</code> chunk collapsed it (streamed length ≈ <code>len(raw) + len(cleaned)</code>); the previous guard could never fire for the current turn (its flag is set only after the node's own tokens already streamed). The fix reuses existing structures: <code>filter_for_llm_context()</code> now <strong>allowlists only the compaction-summary</strong> <code>SystemMessage</code> (matched via a new <code>COMPACTION_SUMMARY_MARKER</code> constant in <code>core/constants.py</code>, reused by <code>compaction_node</code>/<code>compaction_service</code> in place of three duplicated literals) and drops every other internal scaffolding SystemMessage — the compaction summary is preserved because it is the response LLM's sole carrier of compacted history; <code>_format_status_messages()</code> surfaces <code>data["react_synthesis"]</code> (keyed by a new shared <code>FIELD_REACT_SYNTHESIS</code> constant, eliminating the writer/reader literal drift) as the authoritative answer; <code>StreamingService._process_messages_chunk()</code> streams token deltas only (<code>AIMessageChunk</code>) and skips the duplicate complete <code>AIMessage</code> once deltas have been seen, with a non-streaming fallback that still emits the complete message when no delta occurred. Confirmed in dev logs (<code>response_duplicate_message_skipped{message_type="AIMessage"}</code>). <strong>i18n:</strong> no new user-facing strings — the reply is generated by the existing response LLM in the user's language; the fix only stops internal English scaffolding from leaking into it. <strong>Tests:</strong> new <code>test_agent_results_react_synthesis.py</code> + filter cases (compaction summary kept, ReAct scaffolding dropped, list-content guarded) + streaming cases (delta vs complete-message dedup); 270+ unit tests green. Doc: <a href="docs/technical/REACT_EXECUTION_MODE.md"><code>docs/technical/REACT_EXECUTION_MODE.md</code></a>. — 21 May 2026.
</p>
