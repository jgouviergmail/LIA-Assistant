...
<p align="center">
  <strong>Version 1.22.0</strong> — <strong>Background chat runs (ADR-117): answers survive you.</strong> Chat generation is now detached from the HTTP connection: closing the tab, navigating away or losing the network no longer kills an in-progress answer — a detached producer finishes it server-side, publishing every chunk to a per-run Redis Stream that the SSE endpoint merely subscribes to. Come back mid-generation and the answer <strong>resumes live</strong> (full replay then live tail, across tabs too), guarded by a per-conversation concurrency lock (HTTP 409 → auto-reattach). A <strong>stop button</strong> cancels a run in ~1s cross-worker: the partial text is kept and badged "interrupted", already-consumed tokens stay billed, and a turn-start checkpoint repair strips the dangling <code>tool_calls</code> a cancellation can leave behind (the exact poisoning proven by POC before a line was written). Voice synthesis is skipped when nobody is listening. The whole feature is flag-gated (<code>BACKGROUND_RUNS_ENABLED</code>) with instant rollback, and it closes two long-standing leaks: user messages are now archived <em>before</em> execution, and interrupted runs commit their token records (billing honesty on every exit path). <strong>Verified:</strong> 8,960 backend + 453 frontend tests green, four de-risking POCs, live E2E proofs for disconnect-survival, 322-chunk replay, cancellation and billing. — 9 July 2026.
</p>
