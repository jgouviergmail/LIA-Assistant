...
<p align="center">
  <strong>Version 1.23.10</strong> — <strong>The living-interface release: LIA's psyche becomes visible, and the chat gains a coherent system of micro-interactions.</strong> The mood avatar is now a real animated face on the current reply (Google's Noto Animated Emoji, self-hosted, CC BY 4.0) while history keeps its static mood snapshots — waking up on hover; the ring pulses once when LIA's mood shifts between replies, and relationship milestones (Orientation → Exploratory → Affective → Stable) are celebrated with a one-shot toast. Personality emojis come alive in the header (current one always, menu items on hover, codepoints derived at runtime from the DB-managed glyphs with silent static fallback). The pipeline's execution steps now breathe — past steps dim, the current one slides in and pulses — on a new typed <code>streaming.phase</code> field in the chat reducer FSM; a caret blinks while the answer streams; the typing indicator picks one of six shapes at random per response; the tab title alternates with "✦ LIA is writing…" while a background run streams in a hidden tab. Every animation degrades to the previous behavior on asset failure, hidden psyche visuals, and <code>prefers-reduced-motion</code> (animated assets are then not even fetched). Pure frontend: no backend change, no migration, no new dependency. <strong>Verified:</strong> 498 frontend tests green (+45 new across 9 suites), ESLint + <code>tsc --noEmit</code> clean, i18n parity across the 6 locales. — 11 July 2026.
</p>
