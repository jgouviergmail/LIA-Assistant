...
<p align="center">
  <strong>Version 1.30.15</strong> — <strong>Production logs become a contract</strong>. A systemic seven-day audit of production telemetry (Loki, container logs, database and metrics cross-checked) catalogued and fixed twelve findings — each verified against real data before any code changed, several plausible hypotheses refuted along the way. Real-time streaming can no longer exhaust the infrastructure: notification SSE streams are <strong>capped per user, newest wins</strong> (an evicted stream is told so with a <code>superseded</code> event and closes itself; a visible tab reclaims its slot instantly, a hidden one waits to be looked at — and the connection is never refused, because EventSource cannot read an HTTP status), while the <code>RedisConnectionPoolExhaustion</code> alert is reborn on the direct failure signal — the previous rule lived in a file Prometheus never loaded, with a threshold a bounded ratio could never cross. A human approval is <strong>no longer logged as an error</strong>: HITL interrupts are recognized as control flow across every metrics wrapper — no more tracebacks carrying draft content and recipient names at ERROR level, no more falsified error counters. Vendor limits are <strong>repaired before the call</strong>: over-long Brave queries are word-trimmed to the API's published bounds (and the bounds are now published to the planner), an OpenWeatherMap 404 becomes a clean “location not found” verdict that re-enables the Google fallback, and Perplexity-backed web research gets its own timeout family instead of being killed at the generic 30 s. The showroom funnel finally tells the truth: the event column was narrower than its own vocabulary (every per-mission insert had silently failed for six weeks — now widened, with a derived guard making the bug class impossible), and clicks toward the free live instance become a measured event, so “zero signups” and “nobody clicked” can never be confused again. — 20 August 2026.
</p>
...
|                           |                                         |                             |                                                                         |
| ------------------------- | --------------------------------------- | --------------------------- | ----------------------------------------------------------------------- |
| **42** functional domains | **540,000** lines of code (excl. tests) | **25,200+** automated tests | **238** ADRs                                                           |
