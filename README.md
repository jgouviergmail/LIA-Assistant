...
<p align="center">
  <strong>Version 1.21.14</strong> — <strong>Latent-debt hardening (CA-1, CA-4, CA-5).</strong> A maintenance release — no user-facing feature change, prompt-free, no DB migration — closing three residual debts from the 2026-07 audit, each fixed test-first. <strong>Privacy:</strong> the INFO-level log-redaction net now covers 13 more field names, so raw user content (uploaded file names, HITL replies, calendar/task titles, reminder and edited-message bodies, bulk-operation exclusion text) can no longer leak into INFO logs — contents stay at DEBUG or redacted, counters/IDs remain at INFO. <strong>Performance:</strong> seven blocking file reads/writes on hot async paths (generated-image PNG writes, attachment &amp; RAG uploads, document text extraction, skill endpoints, LLM vision image reads) are offloaded via <code>asyncio.to_thread</code>, so a multi-MB write no longer freezes concurrent requests (SSE included) during an image generation or a RAG upload. <strong>Security:</strong> JSON-LD SEO scripts now escape every <code>&lt;</code> to its <code>U+003C</code> JSON form through a shared helper (every sink + the blog page), neutralizing <code>&lt;/script&gt;</code> breakout from app-compiled dynamic fields, with a systemic CI guard against any future raw <code>JSON.stringify</code> in a script. <strong>Tests:</strong> red-first for each item; Ruff / Black / MyPy strict clean; full backend and frontend suites green; i18n parity across all 6 locales; <code>PII_LOGGING_SECURITY.md</code> updated. — 7 July 2026.
</p>
