...
<p align="center">
  <strong>Version 1.21.10</strong> — <strong>Psyche made perceptible.</strong> Production telemetry showed the Psyche Engine was both <strong>saturated</strong> (pride dominated 61 % of turns, only 4 of 14 moods were ever reached, arousal/dominance ratcheted permanently positive) and <strong>invisible</strong> (a rich inner state was computed every turn but never surfaced in the wording). Two coordinated fixes. <strong>De-saturated dynamics</strong> (<a href="docs/architecture/ADR-104-Psyche-De-Saturation.md">ADR-104</a>): baseline damping pulls each personality off its extreme resting point, an asymmetric anti-ratchet relaxation lowers only the arousal/dominance axes above baseline, emotion decay is faster and at most 4 emotions stay active at once — the mood now traverses far more of the 14-label space. <strong>Embodied expression (A–E)</strong> (<a href="docs/architecture/ADR-105-Psyche-Embodied-Expression.md">ADR-105</a>): a first-person inner-voice injection gives each mood a concrete voice grammar, lets it reshape the reply's form, and reconciles personality × mood — so the variety is <em>felt</em> in the tone of every answer and every proactive message (reminders, voice, heartbeat, interests). The state is shown, never announced unless you ask. <strong>Prompt-only:</strong> no DB migration, no schema change; five default-on <code>.env</code> tuning knobs. <strong>Tests:</strong> Ruff / Black / MyPy strict clean; new de-saturation + embodied-template suites, 173 psyche deterministic tests, full unit suite green; i18n key parity across all 6 locales. — 5 July 2026.
</p>
