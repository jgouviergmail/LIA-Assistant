...
<p align="center">
  <strong>Version 1.30.0</strong> — <strong>On the move, LIA finally answers from where you are — never from your home address by default</strong> (ADR-219). The last-known position existed since April (encrypted, opt-in, never historized) but only proactive notifications read it: chat, scheduled actions, the briefing and skills fell back to the home address the moment the browser position died — which a PWA does after a few hours of inactivity. The cascade is now <strong>one and everywhere</strong>: live browser position, else the fresh consented last-known position, else home — and scheduled actions, which never have a browser, inherit it without a line of code. <strong>A dated position announces itself as dated</strong>: "where am I?" without a live GPS used to answer "enable geolocation"; it now answers from the remembered position <strong>with its age stated</strong> — the age travels in the data contract (<code>as_of</code>), through the tool, the skill context and the versioned prompt, so the model cannot claim "you are at…" on a two-hour-old point. And "at home" is never resolved from a position captured on the road. <strong>The PWA wakes up instead of waiting to be shaken</strong>: on return to foreground the permission is re-checked — still granted → the position refreshes silently and ships to the server; dropped (what iOS does after inactivity) → a banner shows on chat open, <strong>before you type anything</strong>, its button supplying the user gesture the native sheet requires. The periodic position sync no longer depends on a settings page being open. <strong>Four production failures diagnosed from the logs, four root causes fixed</strong>: ReAct mode died with a 400 on Gemini whenever an MCP tool declared an array (the schema lost its item type — one tool poisoned the whole request); phone calls failed with a lying "try again later" after ElevenLabs hardened its key format (the new <code>auth_failed</code> status states the real repair, in all six languages); LLM administration returned 422 loops when the dialog sent the previous model's reasoning shape; and the deploy script wrote <code>DOCKER_GID</code> into a file the atomic switchover then discarded — skill widgets lost the Docker socket on every production rollout. <strong>Cold multi-worker boot no longer fights itself</strong>: a PostgreSQL advisory lock — polled, because the blocking form deadlocks against <code>CREATE INDEX CONCURRENTLY</code>, measured live — serializes the LangGraph schema setup: 68-second cold boot, zero startup failures, where a fresh database used to respawn-loop for 25 minutes. <strong>18,254 backend</strong> + <strong>5,475 frontend</strong> tests, all six languages. — 16 August 2026.
...
|                           |                                         |                             |                                                                         |
| ------------------------- | --------------------------------------- | --------------------------- | ----------------------------------------------------------------------- |
| **40** functional domains | **520,000** lines of code (excl. tests) | **23,700+** automated tests | **218** ADRs                                                           |
| **204** versions shipped  | **6 languages**, parity enforced in CI  | **471** Prometheus metrics  | [**8.3/10** technical audit, 24 normalized areas](docs/audit/README.md) |
...
| Metric                  | Value                                                                                         |
| ----------------------- | --------------------------------------------------------------------------------------------- |
| Total backend tests     | 18,254 collected across 990 files (`pytest tests/unit tests/agents --collect-only`)           |
| Backend breakdown       | unit 17,060 (923 files) · agents 1,194 (67 files) · integration 704 (72 files)                 |
| Frontend tests (vitest) | 5,475 across 441 files (+ 52 hermetic Playwright E2E specs incl. axe/dark/zoom)                |
