...
<p align="center">
  <strong>Version 1.47.4</strong> — <strong>Exact answers become tools, and LIA reads its own records.</strong> Calculations, dates and currencies are computed rather than guessed, memory and journals are looked up on purpose, an e-mail to yourself needs no confirmation, a generated image is shared with a connection as a copy, and the server's logs keep facts, never words — 25 September 2026.
</p>
...
|                           |                                         |                             |                                                                         |
| ------------------------- | --------------------------------------- | --------------------------- | ----------------------------------------------------------------------- |
| **51** functional domains | **720,000+** source lines (excl. tests) | **41,000+** automated tests | **317** ADRs                                                            |
| **269** versions shipped  | **6 languages**, parity enforced in CI  | **589** Prometheus metrics  | [**8.3/10** technical audit, 24 normalized areas](docs/audit/README.md) |
...
- **Prometheus**: 589 custom metrics (agents, LLM, infrastructure). A metric nobody can see is a metric nobody acts on: every one must be wired to a Grafana panel, a recording rule or an alert, and a shrink-only ratchet fails the build on a newly blind metric.
- **Grafana**: 30 dashboards, including a product-value cockpit · **Loki**: structured JSON logs that keep facts, never people's words ([ADR-317](docs/architecture/ADR-317-A-Log-Line-Carries-Facts-Never-The-Words.md)) · **Tempo**: distributed tracing · **Langfuse**: LLM tracing with prompt versions.
...
- **ReAct** (⚡) — the model reasons iteratively, calling tools one by one and adapting to each result. More autonomous, more expensive; ideal for exploratory, research or ambiguous queries. Its iteration budget is extended while the loop keeps producing results, a turn that stops mid-flight closes its own books, the tools it binds are chosen by relevance to the question rather than cut by a cap — or all of them, when the person chose frequent exchanges so the provider's cache reads each turn back from the previous one — and a turn is judged on its result: a gap it declares buys a bounded recovery pass.
...
| **Adaptive re-planner**    | A rule-based analyser classifies an execution failure and picks a recovery; in _panic mode_ the catalogue expands to every tool for one retry                                                                                                                        |
...
| Qwen       | Thinking, tools and vision through Alibaba Cloud DashScope; explicit prompt cache where the family needs it; Qwen Image 3.0; regional endpoint via `QWEN_BASE_URL` |
...
- **The context window belongs to the configured slot** ([ADR-278](docs/architecture/ADR-278-Per-Slot-Context-Window.md)) — a frugal router and a generous responder can share one model with different windows; it is the number LIA _asks_ for and the number it _counts_ with.
...
| Metric                  | Value                                                                                                                                            |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| Backend tests           | 32,725 collected over `tests/` (`pytest --collect-only -q`, 1,974 files, 2026-09-25)                                                             |
| Frontend tests (vitest) | 9,160 passed across 738 files (`task test:frontend:coverage`, 2026-09-25), plus hermetic Playwright journeys with axe, dark-mode and zoom checks |
| Coverage floor          | 74% enforced in CI on the backend — a shrink-only ratchet, never lowered; frontend thresholds per glob                                           |
...
| -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [GDPR / RGPD](https://eur-lex.europa.eu/eli/reg/2016/679)                              | PII filtering, data minimisation, full-account export and account deletion that also removes linked OAuth grants and transparency records ([privacy guide](docs/knowledge/15_privacy.md))                                                                                                                                                                                                     |
...
317 ADR files (ADR-001 through ADR-318 — ADR-008 has no separate file) record every major architectural decision with its context, the alternatives and, increasingly, the production measurement that motivated it. Three to start with, and [the full index](docs/architecture/ADR_INDEX.md):
...
- **Rules that are not stylistic**: read the _Systemic Rules_ in [CLAUDE.md](CLAUDE.md) — each one closes a bug class measured in production, and a guard enforces most of them
