...
<p align="center">
  <strong>Version 1.18.0</strong> — Today Briefing : la home du dashboard devient un rituel quotidien. Greeting LLM contextuel + synthèse 2-3 phrases au-dessus d'une grille de 6 cards opérationnelles (météo enrichie avec prévisions 5 jours, agenda, mails non lus, anniversaires à venir, rappels actifs, métriques santé). Architecture pure : aucun graphe LangGraph, `asyncio.gather` parallèle, cache Redis par section avec TTL adapté à la fréquence de changement (météo 1h, agenda 10min, mails 5min…). Endpoints split (`/briefing/cards` rapide + `/briefing/synthesis` LLM-bound) pour un rendu progressif non-bloquant. Tokens et coût EUR de chaque appel LLM exposés dans l'UI à côté du timestamp (badge discret, tooltip détaillé) et trackés dans `user_statistics` via `track_proactive_tokens` (avec subtraction des cached tokens pour éviter le double comptage). Carte santé alignée verticalement (séparateurs aujourd'hui/moyenne en colonne fixe via `display:contents`). Prévisions météo localisées en 6 langues côté client via `Intl.DateTimeFormat`. — April 2026
</p>
