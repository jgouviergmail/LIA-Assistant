...
<p align="center">
  <strong>Version 1.21.18</strong> — <strong>Automated PostgreSQL backups &amp; tested restore (ADR-109).</strong> An infrastructure release closing the top operational risk from the 2026-07 360° audit: no backup tooling was versioned and the RPO was undefined. <strong>Added:</strong> a <code>postgres-backup</code> sidecar (pg_dump 16, ARM64-verified) in both compose files with daily/weekly/monthly rotation, every parameter <code>.env</code>-driven (new section <code>[80]</code>, safe defaults); dumps land in a dedicated chmod-700 host directory prepared by <code>deploy.sh</code>; <code>task backup:now</code> / <code>task backup:verify</code>; a verification script that restores the latest dump into a throwaway pgvector container and compares schema revision and row counts against the live source; a full backup/restore runbook covering the systemic aftermath (automatic migrations on API restart, Redis flush, health check). <strong>Verified:</strong> real backup restored with 0 SQL errors, <code>alembic_version</code> identical across repo/source/restore, identical row counts on 3 reference tables; RPO now ≤ 24 h (tunable). Known phase-2 gaps documented in ADR-109 (off-site encrypted copy, attachments volume, failure alerting). — 8 July 2026.
</p>
