...
<p align="center">
  <strong>Version 1.21.15</strong> — <strong>Core-agent decomposition (B1) &amp; i18n/prompt debt hardening.</strong> A maintenance release — no user-facing feature change, no behavior change (verbatim extractions, LLM prompt input byte-preserved), no DB migration. <strong>Decomposition:</strong> the two agent-core functions still above the ~300-line threshold — <code>response_node</code> (1516 → 293 code lines, −81 %) and <code>stream_sse_chunks</code> (403 → ~250) — are split into 27 + 2 single-responsibility helpers under a characterization-test safety net (Feathers); <code>parallel_executor</code> and <code>_handle_hitl_interrupt</code> already complied and were left untouched. <strong>New module:</strong> the 730-line <code>_add_debug_metrics_sections()</code> becomes <code>DebugMetricsBuilder</code> (one method per debug-panel section, per-section error isolation, assembly order preserved). <strong>Debt hardening:</strong> the inline <code>&lt;InitiativeSuggestion&gt;</code> / <code>&lt;ProactiveFindings&gt;</code> prompt scaffolding moves to versioned <code>prompts/v1/</code> files (byte-exact) and the inline French data-filtering error marker becomes an English constant (LLM-facing, per the "prompts are not translated" convention). <strong>Fixed:</strong> the new production module was silently matched by the <code>debug_*.py</code> gitignore pattern (would have crashed the admin debug panel on a fresh checkout) — a targeted negation now tracks it. <strong>Tests:</strong> ~90 characterization / unit tests pin the observable contract (exact <code>state_update</code> keys, streaming chunks, every builder section) before any extraction and stayed green identically throughout — catching one self-introduced bug; Ruff / Black / MyPy strict clean; backend &amp; frontend suites green; i18n parity across all 6 locales; coverage up (<code>response_node</code> 42→69 %, streaming 38→59 %). — 7 July 2026.
</p>
