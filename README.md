...
<p align="center">
  <strong>Version 1.20.8</strong> — Conversation-history compaction v2 (ADR-086): hardening, observability, and user-visible truncation. <strong>Why:</strong> on 2026-05-16 a production user could no longer send any message — every turn hit the compaction node, the LLM responded once after ~32 s, then went silent for ~93 s until the Cloudflare tunnel cut the SSE stream at 125 s with <code>Erreur de connexion: network error</code>. Forensic audit surfaced seven defects: <code>await llm.ainvoke()</code> had no timeout in <code>_summarize_chunk</code>; the router-level <code>last_heartbeat</code> only pulsed <em>between</em> received chunks (never during a silent <code>await</code>); prior <code>"compaction #N"</code> SystemMessages accumulated linearly; an <code>except Exception</code> branch silently produced <code>strategy="descriptive_fallback"</code> with a stub <code>"[Previous conversation compacted — 48 messages…]"</code> and was counted as a success; no retry on transient errors, no global budget, no SSE signal during the wait; the frontend had no <code>'compacting'</code> status. The v2 rebuild ships across five layers. <strong>(1) Resilience.</strong> Every LLM call is wrapped in <code>asyncio.wait_for(per_chunk_timeout_seconds=35)</code> with a <code>tenacity.AsyncRetrying</code> exponential backoff (<code>stop_after_attempt(3)</code>, <code>wait_exponential(multiplier=1.0, min=1.0, max=30)</code> — <code>min</code> aligned with the multiplier so the first retry actually waits one base unit instead of clamping to 0 per tenacity issue #175). The whole <code>compact()</code> is then wrapped in a global <code>asyncio.wait_for(120s)</code>. On exhaustion or unrecoverable error, <code>_truncation_fallback</code> produces an explicit <code>SystemMessage</code> (<code>"[Older conversation truncated — N messages removed because the automatic summary could not complete (global_timeout). Key identifiers preserved: …]"</code>) that the user can read — the silent <code>descriptive_fallback</code> stub is gone; <code>strategy="truncation"</code> is the explicit substitute. <code>should_compact()</code> was reordered so token-heavy-but-message-light conversations (13 messages but one 6 K-token <code>AIMessage</code> with embedded tool data) still trigger compaction — the token count is the real signal, the <code>compaction_min_messages</code> floor is now only a fast-path for sub-threshold runs. <strong>(2) Consolidation.</strong> Prior <code>"compaction #N"</code> SystemMessages are prepended to the merge step (opt-in <code>compaction_include_previous_summaries=true</code>) and only removed from state when the merge actually consolidated them (<code>consolidated_previous_summaries=True</code>) — truncation fallback preserves them so the conversation does not lose information v1 had retained. New strategy label <code>single_chunk_with_merge</code> distinguishes a one-chunk run that still ran a merge step from a true multi-chunk LLM workload. <code>_format_messages_for_summary</code> now serialises content blocks via <code>json.dumps</code> instead of <code>BaseMessage.text</code>, which on <code>ToolMessage</code> with <code>content: list[ContentBlock]</code> was leaking Python repr / JSON wrappers into the prompt. <strong>(3) SSE custom stream-mode signal.</strong> The compaction node emits <code>compaction_start</code> and <code>compaction_done</code> via <code>langgraph.config.get_stream_writer()</code> through a new <code>stream_mode="custom"</code> (LangGraph 1.x). <code>StreamingService._process_custom_chunk</code> translates these payloads into <code>ChatStreamChunk(type="execution_step", metadata={step_type: "compaction", step_label: …})</code>. The node falls back to a no-op writer when the LangGraph symbol is missing or raises, logs at WARNING <strong>and</strong> increments a new Counter <code>compaction_writer_unavailable_total{reason}</code> so silent degradation is now visible in Grafana. <strong>(4) Concurrent SSE keepalive.</strong> New module <code>apps/api/src/domains/agents/api/sse_keepalive.py</code> exposes <code>iter_with_keepalive(source, keepalive_interval_seconds)</code> which spawns one consumer task that drains the upstream into an <code>asyncio.Queue</code> and yields a <code>KeepalivePulse</code> sentinel on every <code>queue.get()</code> timeout (defaults to 15 s). The single-consumer-task design preserves <code>ContextVar.set()/.reset()</code> stability — the earlier prototype spawned a fresh <code>create_task(__anext__())</code> per iteration, each running in a brand-new Context where the tokens minted by the upstream's <code>__aenter__</code> could no longer be <code>.reset()</code>. The router serialises pulses as the SSE comment line <code>": heartbeat\n\n"</code>; the legacy <code>last_heartbeat</code> check is removed. <strong>(5) User-visible UX.</strong> New reducer transitions <code>STREAM_COMPACTION_START</code> / <code>STREAM_COMPACTION_DONE</code> set <code>status === 'compacting'</code> (folded into <code>useChat.isTyping</code> so the existing <code>disabled={isTyping || isUsageBlocked}</code> wire on <code>ChatInput</code> locks the textarea). <code>handleCompactionStep</code> drives a morphing <strong>sonner toast</strong> via a stable id: <code>toast.loading(t('chat.compaction.in_progress'))</code> on start, then <code>toast.success(t('chat.compaction.completed', {tokens}))</code> or <code>toast.warning(t('chat.compaction.truncated'))</code> on done. A first attempt rendered a <code>&lt;CompactionBanner&gt;</code> inside <code>ChatMessageList</code> (sticky-top variant tested) but the dual <code>overflow-y: auto</code> boundary made the banner invisible at the default post-send scroll position — the pivot to a position-fixed sonner toast (decoupled from the chat scroll layout) is documented in the <em>Update</em> section of ADR-086. A new <strong>context-usage pill</strong> in the chat header shows the live <code>tokens / threshold</code> ratio (16-px SVG ring, 4 colour tiers, badge clamped at 100 % so we never display nonsensical inline values like "150%", real ratio kept in the tooltip with the explicit <code>tooltip_overflow</code> copy when <code>ratio > 1</code>). The pill hydrates from server state via <code>useChat.hydrateContextUsage(tokens, threshold)</code> fed by two new optional fields on <code>ConversationTotalsResponse</code> populated by <code>ConversationService._compute_checkpoint_context_usage()</code> which uses <code>checkpointer.aget_tuple(config)</code> (canonical <code>CheckpointTuple</code> API) so a future LangGraph bump cannot silently break it. <strong>Observability:</strong> five new Prometheus metrics (<code>compaction_chunk_timeouts_total</code>, <code>compaction_global_timeouts_total</code>, <code>compaction_total_duration_seconds</code>, <code>compaction_writer_unavailable_total{reason}</code>, plus the existing <code>compaction_executions_total{strategy}</code> with the new labels) and a new Grafana dashboard <code>infrastructure/observability/grafana/dashboards/14-compaction.json</code> (7 panels). <strong>Operator runbook:</strong> new transactional admin script <code>scripts/admin/reset_user_checkpoints.sql</code> that wipes a single user's LangGraph checkpoints inside a transaction with before/after counts. <strong>Dependencies:</strong> <code>tenacity==9.1.4</code> pinned explicitly in <code>requirements.txt</code> (was transitive via <code>langchain-core</code>); DeepSeek V4 family added to <code>MODEL_CONTEXT_WINDOWS</code> (1 M tokens). <strong>i18n:</strong> <code>chat.compaction.{in_progress, completed, truncated}</code> and <code>chat.context_usage.{tooltip, tooltip_compact, tooltip_overflow}</code> in all 6 languages (fr / en / de / es / it / zh). <strong>Tests:</strong> 25 new backend tests + 25 new frontend tests; also a 2026-05-20 baseline sweep cleared 19 pre-existing integration-test failures unrelated to compaction (4 stale <code>get_redis_session</code> mocks, 1 hardcoded provider list, 14 stale <code>get_global_registry</code> mocks + singular/plural alignment). Architecture: <a href="docs/architecture/ADR-086-Conversation-History-Compaction-v2.md"><code>docs/architecture/ADR-086-Conversation-History-Compaction-v2.md</code></a>. Runbook: <a href="docs/technical/COMPACTION_v2.md"><code>docs/technical/COMPACTION_v2.md</code></a>. — 20 May 2026.
</p>
