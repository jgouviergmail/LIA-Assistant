...
<p align="center">
  <strong>Version 1.27.14</strong> — <strong>A deployment does not disturb the stack that is serving</strong> (ADR-211, ADR-212, ADR-213). Seven days of production logs, read line by line, and the most serious finding was one nobody was looking for: the deployment rebuilt its directory in place, and a bind mount resolves to an <strong>inode</strong> — so every release quietly destroyed the files under the containers that were still answering users. Measured live mid-deployment: the running API saw its Firebase credentials, its knowledge base and its skills as <strong>empty directories</strong> for the ten minutes of the build. The same deletion swept the <strong>PostgreSQL backups</strong>, which sat inside the replaced tree: the sidecar had been writing dumps no restore could ever have used. Releases now stage into a separate directory and swap by <strong>renaming</strong>, which preserves the inode, so live containers keep valid mounts until they are recreated on purpose; backups live outside the deployment entirely. Uvicorn's shutdown is bounded too — an open SSE stream used to block it forever and Docker killed the container after ninety seconds, taking every in-flight conversation with it: <strong>thirty-one of forty-two stops</strong> ended that way, and this release ended <strong>zero</strong>. The audit also closed a <strong>rate-limit bypass</strong> (the per-caller budget keyed on an address the caller could write), retired <strong>dead push tokens</strong> that had been retried for three days because a verdict was classified by message text rather than by type, gave <strong>accounts awaiting activation</strong> the page that explains the wait instead of an application that looks broken, and made the log platform survive being queried: high-cardinality fields no longer become Loki labels, and the structured payload is no longer overwritten in transit. Backend i18n catalogues, absent from the production image since the beginning, now ship. <strong>18,128 backend</strong> + <strong>5,018 frontend</strong> tests, all six languages. — 5 August 2026.
