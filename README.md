...
<p align="center">
  <strong>Version 1.25.22</strong> — <strong>The controls the previous wave announced, verified — three of them did not exist.</strong> The rate-limit alert had been added to a rule file Prometheus does not load <em>and</em> that the config pipeline regenerates, so it could never fire and would have vanished at the next deploy; it now lives in the vital core (13 → <strong>14 alerts</strong>) with a per-environment threshold, a linked runbook and two promtool cases — one proving it fires, one proving a single Redis hiccup does not. The deploy's permission hardening ended in an <code>echo</code> that ran whether or not a single <code>chmod</code> succeeded; it now reads the resulting modes back with <code>stat</code> and <strong>refuses to start the platform</strong> when the production <code>.env</code> is not 0600 inside a 0700 directory. And the global rate limit was bypassable by prefixing any request with <code>/health</code> — exemptions are now exact. The wave also closed defects the audit never listed: an <strong>IDOR on push-token revocation</strong> (any signed-in account could switch off another device's notifications), an HSTS header the API emitted with <code>includeSubDomains; preload</code> against the project's own written policy — both surfaces now read <strong>one ladder</strong> — an email address that escaped the PII filter in free log text, and the frontend published on <strong>every network interface</strong> while the other fifteen ports were loopback-only. Signing out now revokes the device's push token before closing the session, and the last known position and geolocation consent no longer survive into the next account. Every fix was <strong>falsified</strong> — the old behaviour restored, the test verified to fail: the previous <code>RemoteDir</code> rule accepted 8 of 14 hostile values, including <code>..</code>, which expands to <code>sudo rm -rf ~/../*</code>. Three <strong>executable guards</strong> close the classes: a rendered alert file must match its template, no container may take the full secret set, every published port must bind to loopback. <strong>15,331 backend</strong> + <strong>2,729 frontend</strong> tests, MyPy strict clean, all ratchets hold. — 25 July 2026.
...
| [GUIDE_TESTING](./docs/guides/GUIDE_TESTING.md)               | Testing strategy (~15,331 backend tests across 824 files) |
