...
<p align="center">
  <strong>Version 1.20.11</strong> — ReAct mode is now proactive: it gains the <strong>Initiative phase</strong> it previously had only in Pipeline mode, so after answering it can run read-only cross-domain lookups and weave the result into the reply (e.g. the weather and the drive time before your next appointment). <strong>Why:</strong> Initiative only ran on the Pipeline path; in ReAct it was reachable merely <em>incidentally</em> on the draft-confirmation path, while the nominal <code>react_finalize → response</code> path bypassed it. ReAct now routes through a gated conditional edge <code>route_from_react_finalize → initiative</code> (new <code>INITIATIVE_REACT_ENABLED</code> flag, default off; the draft path stays untouched, so default-off is byte-identical to before). The answer is preserved alongside the findings via an idempotent merge in <code>response_node</code> (replacing an <code>if not agent_results</code> guard that would otherwise have dropped it), and <code>_format_execution_summary</code> now reads registry items in both their pipeline (<code>dict</code>) and ReAct (Pydantic <code>RegistryItem</code>) shapes. This release also makes the <strong>Today Briefing widget limits configurable</strong> (<code>BriefingSettings</code>; agenda raised 3 → 10) and the home greeting now reflects your real agenda (right day, named event) — plus fixes: currency amounts (<code>$</code>) no longer render as broken LaTeX in chat/notifications, the Frankfurter exchange-rate endpoint is corrected (<code>.dev/v1</code>), and a noisy scheduler leader-election log is demoted to info. <strong>Tests:</strong> new routing / merge / execution-summary / briefing-settings / greeting suites; Ruff / Black / MyPy clean. Docs: <a href="docs/architecture/ADR-070-ReAct-Execution-Mode.md">ADR-070</a> + <a href="docs/technical/REACT_EXECUTION_MODE.md"><code>REACT_EXECUTION_MODE.md</code></a>. — 21 May 2026.
</p>
