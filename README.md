...
<p align="center">
  <strong>Version 1.25.28</strong> — <strong>Entire conversations never made it into memory, and nothing could say so.</strong> Two questions — do plain exchanges feed memory, interests and journals, and can a skill be modified — opened the same blind spot: every extraction decision was logged at <code>debug</code> level with nothing aggregating it. Four defects lived there. A conversation held on Telegram <strong>never</strong> fed a journal nor the psyche whatever the user had enabled — the handler did not forward the two preferences, the signature defaults (<code>False</code>) applied, and the database columns say <code>true</code>. An exchange leading to a confirmed action extracted <strong>nothing at all</strong>: the rich turn stops on the interrupt before the response node, and the confirmation turn returned from a fast path placed before scheduling — <em>send Marie a mail saying I'm moving to Lyon</em> left no trace. Conversely a refusal had four extractors analyse a message <strong>the system had fabricated</strong>, with the matching risk of writing it into memory. And a contact named Fine, Cool or Bien lost every associated memory, silently: the heuristic that discards <em>ok</em> and <em>thanks</em> also ran on person names, and those words are real surnames. On the skills side the write engine had existed since ADR-118 but stayed unreachable — the assistant could not read a manifest, a rewrite erased the gallery thumbnail, and its own prompt ordered it to rename rather than update. Editing is now a full regeneration gated by a content-derived token: a plain flag would have been a convention the model can skip, whereas a digest can only have been received. <strong>16,293 backend</strong> + <strong>3,473 frontend</strong> tests, MyPy strict clean, every ratchet held, and reviewing the fix found four more defects in the fix itself. — 27 July 2026.
