...
<p align="center">
  <strong>Version 1.21.4</strong> — <strong>Reliability &amp; privacy: full-codebase audit (wave 2).</strong> Closes seven <em>systemic</em> classes of near-zero-risk defect — not just their occurrences — each with a permanent guard (a CI test, a Prometheus metric, or a written convention) so the class cannot silently recur (<a href="docs/architecture/ADR-095-Systemic-Guards-Wave2-Audit.md">ADR-095</a>). Persistence: in-place JSONB mutations were silently dropped by SQLAlchemy (four sites — an API-key "last used" timestamp, connector error diagnostics, two psyche-state writes); fixed by new-object reassignment, guarded by an AST test that fails CI on any in-place JSONB mutation anywhere in <code>src/</code>. Billing statistics: three cycle-rollover paths each hand-reset a different subset of counters, so voice/image/Google/token counters leaked across the boundary depending on which event crossed it; all three now delegate to one <code>reset_cycle()</code> that zeroes every counter by introspection, guarded by a multi-silo test. Privacy: home address &amp; GPS coordinates, resolved contact names/emails, email recipients/subjects and memory previews were logged at <code>INFO</code>; removed at the call sites and backed by a systemic net that redacts content fields at <code>INFO</code> and above (GDPR data-minimization). Tool registry: a broken tool module could vanish silently from the catalogue — imports now fail loudly outside production (Prometheus counter in prod), guarded by a three-layer smoke test that imports and invokes all ~95 tools (it caught two latent <code>runtime</code> bugs, now fixed). i18n: Chinese-language users now get Chinese labels (two modules never reached the <code>zh-CN</code> tables), the last-resort fallback message is localized in all <strong>6 languages</strong> (was hardcoded accent-less French), and the language-normalization chokepoint is unified into a single implementation. Plus five docstrings corrected to match the code. <strong>No DB schema change, no migration, no new <code>.env</code> key</strong> (one new Prometheus metric). <strong>Tests:</strong> Ruff / Black / MyPy strict clean; backend <strong>8641 unit tests green</strong> (+66) + <strong>1069 agents tests green</strong>; i18n key parity across all 6 locales; Docker startup verified healthy. — 3 July 2026.
</p>
