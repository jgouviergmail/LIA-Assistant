...
<p align="center">
  <strong>Version 1.25.27</strong> — <strong>Giving the cache a volume was not enough: on the first boot onto that fresh volume, two workers died.</strong> v1.25.26 made the tool-embeddings cache survive a deploy; the very next boot showed what the volume alone does not settle. The volume was empty, so <strong>all four</strong> uvicorn workers missed and all four embedded <strong>the same 713 catalogue texts at once</strong> — 2,852 contents. The provider answered a <code>429</code> of a different kind from the previous one — <em>Resource exhausted. Please try again later</em>, a Vertex AI link, so capacity rather than a named quota — and because the tool selector has no retry and its failure escapes the lifespan, <code>Application startup failed. Exiting.</code> landed twice. They did not degrade, they <strong>exited</strong>: uvicorn respawned them and their replacements read the cache the two survivors had just written — <strong>~48 s running on two workers out of four</strong>. That recovery works by accident; it requires at least one worker to succeed. Now exactly one computes, claiming the right with <code>O_CREAT | O_EXCL</code> so the filesystem picks the winner — no Redis, no database — while the others wait for its result. A dead holder cannot block a boot, a failing holder hands the claim over so attempts serialise at 713 texts instead of parallelising at 2,852, and losing the coordination is never fatal. The review of this very fix found four defects in it, two of which would have made it worse than the bug — including a fresh volume waiting 90 s and then embedding anyway. <strong>16,159 backend</strong> + <strong>3,473 frontend</strong> tests, MyPy strict clean, every ratchet held, and the tool selector down from 515 to 462 logical lines. — 27 July 2026.
