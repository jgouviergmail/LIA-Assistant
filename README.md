...
<p align="center">
  <strong>Version 1.25.13</strong> — <strong>The assistant learns to connect its domains.</strong> Six delivered lots from one full-codebase product analysis (ADR-139/140/141). LIA now keeps a <strong>commitments ledger</strong>: "je dois rappeler le plombier" quietly becomes an open loop — extracted in the background after the answer, nudged by the heartbeat with an anti-nag cooldown, closed the day you say "c'est fait". "Fais-moi une revue de presse tous les matins" finally works <em>from the chat</em>: a confirmable draft creates the recurring automation, and a deterministic detector notices "same ask, same hour, three distinct days" and offers the automation before you think of it. Your documents become an <strong>active domain</strong> the planner can chain ("compare the PDF quote with Paul's email"), and one call builds a <strong>person-360 overview</strong> — contact card, recent mails, shared events, memories — honestly partial when a connector is missing. The Today briefing grows to <strong>nine cards</strong>: "For you" (open loops + automations digest with the precise next run time), strictly pending/overdue <strong>Tasks</strong> (a task due today is never "overdue" — date-only local semantics), and latest-modified Drive <strong>Documents</strong> (summarize in chat, or open in Drive). The heartbeat gains birthdays with a proposed action chain, a traffic-aware <strong>departure alert</strong> ("pars vers 13:30 — 30 min de trajet"), locally-anchored interests ("expo à Lyon ce week-end"), a meeting-prep nudge, and an anti-repetition window that finally sees reminders, automations and call reports; a completed phone call now proposes the calendar entry it agreed on. Four successive counter-reviews caught <strong>15 real defects before release</strong> — a Redis budget key built on process-randomized <code>hash()</code>, six leaked httpx transports (three pre-existing in the heartbeat, one per source per cycle), debug caches whose eviction ran only on a never-executed read path. New proactive surfaces ship <strong>flag-OFF by default</strong>. <strong>12,688 backend</strong> + <strong>2,305 frontend</strong> tests, mypy strict clean, tsc/ESLint/Prettier clean, i18n parity ×6, all ratchets hold. — 22 July 2026.
