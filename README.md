...
<p align="center">
  <strong>Version 1.23.12</strong> — <strong>The wow batch: six targeted effects that push the living interface one notch further.</strong> The psyche gains its strongest embodiment yet — the bubble being streamed takes a thin border and soft halo <em>in LIA's current mood color</em>, and a whitelisted strong emotion (intensity ≥ 80 %) releases a one-shot burst of three particles from the avatar (✨ ❤️ ⚡ 💢 — rarity is the point: change-only, never on mount, never on history rows). The moment LIA starts answering becomes cinematic: the execution steps cross-fade into the first tokens (one keyed remount per response, none per token) instead of being sharply replaced. The Today Briefing weather card comes alive — falling drops on rain, flakes on snow, a slow-rotating halo on clear skies, defensively mapped from <code>condition_code</code>. The landing hero mockup is upgraded to product level (live breathing steps, transient streaming caret), and the relationship-milestone toast gets the animated ✨. Pure frontend, zero new dependency, hard fallbacks everywhere, reduced-motion covered — including two cascade-level traps caught in self-review (CSS animation shorthands never share an element; fill-mode animations are neutralized by duration, not <code>animation: none</code>). <strong>Verified:</strong> 509 frontend tests green (+11), ESLint + <code>tsc --noEmit</code> clean, i18n parity across the 6 locales. — 11 July 2026.
