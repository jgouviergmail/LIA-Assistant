...
<p align="center">
  <strong>Version 1.23.5</strong> — <strong>Production maintenance stops handling credentials.</strong> The post-deploy reindex runbook used to instruct the operator to paste a superuser session <em>cookie</em> into a shell command on the prod box — a replayable admin bearer token landing in <code>~/.bash_history</code> (and its backups), defeating the very <code>HttpOnly</code> flag the cookie is set with, when the same script already had root-level <code>docker exec</code> access three steps above. RAG Spaces reindexation now runs fully in-container through a new <code>reindex_rag_spaces.py</code> CLI (<code>start_reindexation</code> gained a <code>run_in_background</code> flag so the process awaits completion), consistent with the sibling embedding reindex — no HTTP call, no cookie. In the same pass, the real production SSH coordinates (host, port, username) were scrubbed from five tracked files and aligned to the repo's existing <code>&lt;user&gt;@&lt;prod-host&gt;</code> placeholder convention. <strong>Verified:</strong> reindex module + HTTP endpoint import-clean in the dev container, DevOps SSH test fixture moved to RFC-5737 documentation values. — 10 July 2026.
</p>
