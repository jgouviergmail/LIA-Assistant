...
<p align="center">
  <strong>Version 1.23.7</strong> — <strong>The decomposition program reaches the backend's second monolith: application startup.</strong> The <code>main.py</code> lifespan — ~780 logical SLOC across 23 startup and 20 shutdown steps, with a boot order that is critical but was entirely implicit — is decomposed into a new <code>src/infrastructure/startup/</code> package (ADR-123): seven subsystem modules (registries, caches, agents, integrations, schedulers, observability, shutdown), one typed function per contiguous boot segment, an explicit <code>StartupHandles</code> dataclass for the cross-<code>yield</code> state. The lifespan stays the single orchestration point and becomes a readable ~25-call sequence in the exact historical order, headed by a comment documenting the eight ordering constraints (why the checkpointer precedes the registry, why MCP precedes the semantic selector, why Redis closes last…). Strict-identity extraction, proven rather than assumed: a line-accounting check against the previous revision (every code line survives verbatim, per-block order checks 14/14) and a fresh Docker boot producing <em>1,067 structlog events in a strictly identical sequence</em> — same events, same order, same 16-job scheduler summary. <code>main.py</code> drops from 982 to 181 logical SLOC and leaves the ratchet's frozen list; the wiring guides (APScheduler, MCP, LLM config caches) now point at the new registration sites. <strong>Verified:</strong> empty boot-log diff, 9,111 unit + 969 agents tests, MyPy strict clean, <code>/health</code>/<code>/ready</code> 200. — 10 July 2026.
</p>
