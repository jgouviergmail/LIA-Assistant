...
<p align="center">
  <strong>Version 1.33.0</strong> — <strong>LIA now has native Android and iOS apps — one published app per store, a client for any self-hosted server</strong>. The WebView loads <em>your</em> server's remote origin, whose URL you type at first launch: the UI is never duplicated, web releases arrive without a store update, and the httpOnly-cookie session contract that keeps the PWA safe is exactly what makes the shells possible. Every platform claim was <em>measured, not assumed</em> — a dedicated bench drives the real debug app on an emulator through ten scenes, from first-run setup to forgetting a mistyped server, and found three live defects before its first green run. Sign-in works in-app the only way Google permits: the flow leaves for the system browser and returns through a <code>lia://</code> link redeemed against a verifier only the app holds — and wiring that path closed a pre-existing hole, where signing in with Google skipped an account's TOTP entirely. Notifications are native on both platforms and deliberately asymmetric: Android initialises Firebase at runtime with options <em>your</em> server publishes, so a self-hoster's pushes never leave their own project; iOS cannot work that way — APNs answers only to the Apple team owning the bundle id — so the published app is woken through a stateless relay that stores nothing (the handle <em>is</em> the sealed device token), says one fixed sentence in six languages, and never learns who was woken or why. Doubt never deletes: only "handle unreadable" and "device gone" may drop a token, never an outage of ours. Every OAuth departure — twelve flows: connectors, MCP servers, sign-in — now comes home to the surface that opened it, decided once at the single door all of them already shared. — 26 August 2026.
