...
<p align="center">
  <strong>Version 1.20.17</strong> — <strong>Live agent reasoning + native OpenAI client + Anthropic per-model thinking.</strong> Agents now stream their chain-of-thought to the progress UI wherever the provider API supports it (OpenAI Responses summary, DeepSeek <code>reasoning_content</code>, the ReAct loop, and structured nodes via an auto-tool path), enabled <em>only</em> by the per-model "Configuration LLM" matrix — never injected, so a reasoning block appears solely for agents the admin turned it on for. The ~1800-line custom <code>ResponsesLLM</code> is replaced by native <code>ChatOpenAI</code> (<code>use_responses_api=True</code>); only a ~1-method <code>ChatOpenAICached</code> remains, to preserve LIA's static-prefix <code>prompt_cache_key</code> routing. Anthropic extended thinking is now configured per model (opus-4-6/sonnet-4-6 adaptive; opus-4-5/haiku-4-5 manual budget) with temperature/top-p locked while thinking is on — and a long-standing 400 (<code>"Thinking may not be enabled when tool_choice forces tool use"</code>) that broke structured nodes on a thinking-enabled Claude is fixed via the auto-tool path. Decision recorded in <a href="docs/architecture/ADR-087-Native-ChatOpenAI-And-Per-Provider-Reasoning.md">ADR-087</a>. <strong>Tests:</strong> Ruff / Black / MyPy strict clean; reasoning/structured/migration unit suites green; 2 Alembic migrations (single head). — 1 June 2026.
</p>
