...
<p align="center">
  <strong>Version 1.27.13</strong> — <strong>A clicked action never replays itself</strong> (ADR-210). The fourth failure mode of the chat deep links, measured in production: “Prepare a reply to the email…”, launched from a dashboard card, executed correctly — then <strong>re-executed identically</strong> every time the user returned to the chat page. The cause lives where the application cannot reach: since deep links became real navigations (ADR-192), the URL carrying the order is a <strong>first-class visit in the browser's history</strong> — omnibox autocomplete, most-visited tiles, session restore — and cleaning the URL only rewrites the session entry, never the visit database. Three successive hardenings had policed the carrier; each was bypassed by a new resurrection path. Idempotence now sits at the <strong>point of consumption</strong>: every click mints a one-shot id, recorded in a bounded local ledger the instant the URL is cleaned — a resurrected URL sends nothing and <strong>drops its text into the composer as a visible draft</strong>, while backend-emitted “Run it now” links deliberately keep their click-is-consent contract. Proven on the production bundle by a new hermetic journey (resurrection, client-side round trip over the poisoned router entry, reload — one single execution). The release also repairs the <strong>rich-HTML mode</strong>'s key-value lists (a long label crushed the value column to 0 px; the grid now caps the key column by content and past answers self-heal on display), and puts <strong>the essentials on phones</strong>: the context-usage gauge joins the centre of the chat header at every width, the dashboard's Consumption opens expanded, and <strong>Export is a first-class button</strong> on Long-term memory, Interests and Personal journals — the section toolbar gained a pinning contract, and its row wraps cleanly at 360 px. Frontend coverage floors raised again (70/67/67/71). <strong>18,041 backend</strong> + <strong>5,014 frontend</strong> tests, all six languages. — 5 August 2026.
