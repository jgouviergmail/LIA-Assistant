...
<p align="center">
  <strong>Version 1.21.23</strong> — <strong>Connector client domain error contract (ADR-114).</strong> The 2026-07 audit flagged a layering defect: the connector <em>client</em> layer — a domain layer — raised raw transport <code>HTTPException</code> at 28 sites across the OAuth bases (Google, Microsoft) and the Google-direct clients (Places, Routes, Geocoding, To&nbsp;Do). <strong>Changed:</strong> all 28 sites now raise typed <code>BaseAPIException</code> subclasses — <code>AuthenticationError</code>, <code>ExternalServiceError</code>, <code>RateLimitError</code>, and the new <code>ConnectorAPIError</code> forwarding upstream status codes as-is — gaining automatic structured logging and Prometheus metrics, while the external API contract stays <em>byte-identical by construction</em>: <code>BaseAPIException</code> IS-A <code>HTTPException</code>, so status codes, <code>{"detail"}</code> payloads and headers render unchanged through FastAPI's native handler, and tool-path error classification is untouched. <strong>Added:</strong> <code>headers</code> support on the exception taxonomy (<code>Retry-After</code>, <code>X-Requires-Reconnect</code> now expressible by typed exceptions) and a 34-test contract suite pinning every migrated site byte-for-byte, including typed-vs-raw FastAPI render parity. <strong>Hardened:</strong> Microsoft's 401 message aligned with Google's via the central i18n catalog (provider parity — it now tells users to reactivate the connector), upstream response bodies truncated to 200 chars in error details, GPS coordinates demoted to DEBUG in geocoding logs (no-PII rule). <strong>Verified:</strong> 8,904 unit tests green, 501 connector tests green, MyPy strict, Docker boot healthy with live OAuth token refresh through the migrated code. — 8 July 2026.
</p>
