...
<p align="center">
  <strong>Version 1.32.0</strong> — <strong>Asking a model to think had four stored shapes, seven builders and three authorities that all had to agree. One remains</strong>. The same instruction was stored as <code>{"effort": "off"}</code> on twenty-one slots and <code>{"effort": "none"}</code> on six others; its <em>shape</em> was dispatched on a catalogue column and its meaning re-derived by an <code>isinstance</code> inside each provider builder, so a disagreement raised on the hot path for a configuration the admin UI had accepted. A single <code>ReasoningIntent</code> replaces them, on an ordinal ladder that is the same for every provider, and what a given model accepts is <em>derived</em> from (provider, model) — the catalogue may narrow that ladder, never widen it. Coercion is a safety contract rather than a convenience: ties break <em>upward</em>, <code>none</code> is never a coercion target, and <code>can_disable</code> — not ladder membership — governs whether reasoning can be switched off, a distinction whose absence silently <em>enabled</em> reasoning on an explicit <code>none</code>. Every coercion is counted and logged. The admin dropdown is now built from the same resolved profile the API validates against, so it can no longer offer a depth the provider refuses. Two catalogue columns that discriminated the old shapes are dropped rather than left editable, and the work of the two vendored public registries finally becomes visible: every model row states whether its capabilities were registry-corroborated, human-verified, or never curated at all. Behaviour is unchanged where it counts — the golden harness replays the pre-change kwargs for the live configuration, 54 of 54 identical. The screens follow the same rule: the accepted depths are now <em>ticked</em> rather than typed — the form renders the ladder that model's family actually offers and you untick what it refuses — and the Excel workbook writes those same two columns instead of copying another model's shape, a shortcut that silently removed depths whenever the two models belonged to different families. — 24 August 2026.
