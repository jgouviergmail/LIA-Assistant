...
<p align="center">
  <strong>Version 1.20.22</strong> — <strong>Maintenance release: scheduled actions no longer feed long-term memory, interests, journal or psyche — only direct user inputs do.</strong> The "automated source" guard in <code>response_node</code> had been reading the scheduled-action marker from <code>config["metadata"]["session_id"]</code>, which the Langfuse instrumentation rebuilds (and silently drops); the signal is now an explicit <code>is_automated_source</code> flag threaded through <code>configurable</code> — which survives enrichment by construction — and a single computed flag gates all four post-response extractions. Direct paths (web chat, Telegram, voice) default to <code>False</code> and are unchanged; proactive notifications never ran through <code>response_node</code> and were never affected. Also fixes the dev Docker build: <code>apps/api/Dockerfile.dev</code> hard-coded a stale GeoIP month (<code>DBIP_DATE=2026-03</code>) that db-ip.com no longer serves — the <code>geoip-downloader</code> stage now resolves the latest available month dynamically with <code>curl -fsSL</code> (mirroring the production Dockerfile). <strong>No schema change, no migration, no new env var.</strong> <strong>Tests:</strong> Ruff / Black / MyPy strict clean; 34 unit tests green across the three affected files (instrumentation, response_node, scheduled-action executor); dev API boots clean; GeoIP dev stage build verified. — 19 June 2026.
</p>
