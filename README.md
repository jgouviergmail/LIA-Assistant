...
<p align="center">
  <strong>Version 1.30.14</strong> — <strong>The assistant's invisible work becomes visible — and it learns how to work for you</strong> ([ADR-234](docs/architecture/ADR-234-activity-timeline.md) → [ADR-239](docs/architecture/ADR-239-evolution-program-closure.md)). A new <strong>Activity</strong> page gathers everything LIA did on its own initiative — proactive notifications, interest discoveries, automatic journal notes, detected habits, tracked and closed commitments, executed routines — as one chronological feed with exact per-kind totals, stated caps and stated partial failures (never silently completed). Memory gains a <strong>supersession trail</strong>: automated corrections no longer overwrite or delete a fact — the old one leaves the active set pointing at its successor, every read serves live facts only, and each injected memory now carries its date so the assistant can naturally say “last week you told me…”. A seventh memory category, <strong>procedural</strong>, turns explicit standing instructions (“answer shorter”, “stop suggesting X”) into durable rules injected ahead of every conversation — readable, editable and deletable by their owner — while a repair directive makes a correction acknowledged once, without over-apology, and applied in the same answer. The voice <strong>breathes with the mood</strong> (bounded ElevenLabs prosody driven by the psyche engine) and the morning briefing gains a <strong>listen button</strong> whose failures are real HTTP errors, never a silent empty stream. Bounded autonomy grows: a <strong>Proposals inbox</strong> surfaces undecided missed-routine offers (accepting prefills the chat — nothing auto-sends; declining feeds the habit's Bayesian signals), the ReAct iteration budget becomes <strong>adaptive</strong> to the query's domain span (ceiling preserved, opt-in), the repeated-call guard demands a one-sentence Reflexion diagnosis before the next action, and calibration evidence for future adaptive perimeters starts accumulating. The closure ADR records, with evidence, what was verified as already satisfied (event-driven ambience IS the conditional-actions engine) and what stays reserved for explicit product arbitration — each with its written trigger. — 20 August 2026.
</p>
...
|                           |                                         |                             |                                                                         |
| ------------------------- | --------------------------------------- | --------------------------- | ----------------------------------------------------------------------- |
| **42** functional domains | **540,000** lines of code (excl. tests) | **25,100+** automated tests | **238** ADRs                                                           |
