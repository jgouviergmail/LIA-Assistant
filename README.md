...
<p align="center">
  <strong>Version 1.47.0</strong> — <strong>Live mode: talk with LIA in real time on your own key — two intelligences, one seam — and one policy per mode, on the phone as in the browser.</strong> A speech-to-speech session opens from the voice icon on a live model you connect yourself (Gemini Live, GPT-Live or your ElevenLabs agent); the voice holds the conversation and hands every request to the chat engine, which runs it as an ordinary turn while you speak — confirmations, registers and quotas included — while what the provider bills on your key is shown, never recorded; a direct session lets the voice read your data itself and act on nothing; the same two modes, Live and Live direct, now govern the calls LIA makes to you, with one closing for both lines; and every euro the platform pays for a person — a map lookup during a call, the briefing's weather, a photo shown — reaches their ledger whatever the path — 20 September 2026.
</p>
...
|                           |                                         |                             |                                                                         |
| ------------------------- | --------------------------------------- | --------------------------- | ----------------------------------------------------------------------- |
| **51** functional domains | **660,000** lines of code (excl. tests) | **39,000+** automated tests | **300** ADRs                                                           |
| **265** versions shipped  | **6 languages**, parity enforced in CI  | **581** Prometheus metrics  | [**8.3/10** technical audit, 24 normalized areas](docs/audit/README.md) |
...
- **Prometheus**: 581 custom metrics (agents, LLM, infrastructure). A metric nobody can see is a metric nobody acts on: every one must be wired to a Grafana panel, a recording rule or an alert, and a shrink-only ratchet fails the build on a newly blind metric.
...
| Metric                  | Value                                                                                                 |
| ----------------------- | ----------------------------------------------------------------------------------------------------- |
| Backend tests           | 30,704 collected over `tests/` (`pytest --collect-only -q`, 1,842 files, 2026-09-20)                  |
| Frontend tests (vitest) | 8,801 across 697 files, plus hermetic Playwright journeys with axe, dark-mode and zoom checks          |
...
300 ADR files (ADR-001 through ADR-301 — ADR-008 has no separate file) record every major architectural decision with its context, the alternatives and, increasingly, the production measurement that motivated it. Three to start with, and [the full index](docs/architecture/ADR_INDEX.md):
