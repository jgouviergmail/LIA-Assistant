...
<p align="center">
  <strong>Version 1.21.1</strong> — <strong>Fix release: replay-safe HITL loops (ADR-092), trustworthy draft editing, bounded HTTP metrics.</strong> LangGraph resume semantics re-execute the whole interrupted node, so the two HITL flows that looped around <code>interrupt()</code> in-node replayed their side effects on every user decision: draft editing re-ran every past LLM modification (the email sent could differ from the last version shown) and FOR_EACH bulk confirmation re-fetched providers and re-ran past item filters. Both now follow the normative ADR-092 pattern — one <code>interrupt()</code> per node execution, loop state checkpointed through state returns, conditional self-loop edges (new dedicated <code>for_each_confirm</code> node) — proven by compiled replay harnesses: <strong>what you last saw is exactly what executes</strong>. UX fixes ride along: each re-presented draft gets its own chat bubble (per-interrupt SSE <code>message_id</code> from the LangGraph interrupt id), "sign with my name" resolves the user's real first name (shared <code>resolve_user_display_name</code> flowing into the state and content-generation prompts — no more placeholders or invented names), and the clarify follow-up question is finally displayed. Robustness: <code>CancelledError</code> propagates at both executor stages (a closed stream actually cancels bulk work), <code>on_item_error="stop"</code> forces sequential execution so "stop" can stop, HTTP metric labels are cardinality-bounded (route templates + <code>unmatched</code>), cards/emails/voice timestamps use the user's timezone, and prod boot is quiet (Telegram <code>set_webhook</code> flood retry, "system" embedding skip). <strong>No schema change, no migration; 1 new optional env var.</strong> <strong>Tests:</strong> Ruff / Black / MyPy strict clean (863 files); full backend suite 8501 unit tests green (+32 new incl. two compiled replay harnesses); verified live in the running container. — 2 July 2026.
</p>
