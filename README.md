...
<p align="center">
  <strong>Version 1.26.3</strong> — <strong>The product cockpit now tells the truth, immediately.</strong> A production audit of the new product dashboard surfaced three real defects, all fixed: the read-only Grafana role had never received its view permissions (created before the views existed, and the maintenance script could not even run — it was missing from the deployment transfer whitelist, now shipped); the hourly rollup <strong>starved</strong> (an interval-only job never fires when the API restarts more often than the interval — measured: four boots, zero ticks, every gauge empty), so the first run is now pinned <strong>two minutes after boot</strong>, proven in-process; and stale placeholder panels claimed to await phases that had already shipped. Six panels now read <strong>exact SQL</strong> over three new read-only views (routines health, signup→first-value percentiles, daily outcome quality), settings-search and Web Vitals are live Prometheus, and every remaining waiting note states the true current reason. <strong>16,626 backend</strong> + <strong>4,083 frontend</strong> tests, all six languages. — 29 July 2026.
