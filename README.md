...
<p align="center">
  <strong>Version 1.25.30</strong> — <strong>Text written by a stranger reached the model unlabelled, and an autonomous loop billed its budget for the time it spent waiting on a human.</strong> Part of what LIA puts in its prompts is not written by its user: an email body, an invitation description authored by its organiser, a web page, a place's editorial summary, an MCP server's result. That text must reach the model tagged <em>data to analyse</em>, never <em>instruction to follow</em>. The mechanism that existed was applied tool by tool, and an exhaustive caller search invalidated it as a strategy — <code>perplexity_tools</code>, <code>brave_tools</code>, <code>mcp_react_tools</code> and <code>emails_tools</code> were not covered, the last one announcing in its own docstring that it returns <em>"FULL email content (body, headers, attachments)"</em>. Above all, content reaches the model through two surfaces, and <strong>neither is a tool</strong>: one of them runs on <em>every</em> turn that produces data, in <em>both</em> execution modes. Provenance is now carried by the <strong>data</strong> — the registry's 24 types are classified once, an unknown type resolves to <em>external</em>, and boot refuses an incomplete table. Seven injection families are detected across the six languages and the content still reaches the model <strong>unchanged</strong>: sanitising would rewrite an email the user may want to read as-is, for a guarantee the next bypass would deny. Four proven defects lived beside it: the ReAct prompt was re-emitted every turn (<strong>840 tokens × N</strong>, prefix cache destroyed), the loop's deadline ran on wall clock rather than compute (<strong>2.01 s wall for 0.0102 s of compute</strong>, measured on a real graph), nothing detected an agent calling the same tool forever, and a hybrid memory search advertised in the UI <strong>had not existed since v1.14.0</strong>. <strong>16,535 backend</strong> + <strong>3,491 frontend</strong> tests, MyPy strict clean, every ratchet held. — 28 July 2026.
