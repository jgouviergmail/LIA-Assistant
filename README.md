...
<p align="center">
  <strong>Version 1.25.26</strong> — <strong>Every startup paid four times for the same work, and the trace blamed the database.</strong> The API runs four uvicorn workers and nothing coordinated them at boot: each re-embedded the <strong>713 catalogue tool texts</strong> and then the <strong>269 FAQ entries</strong> — <strong>3,928 contents in 72 requests</strong> per startup, for a payload whose fingerprint had not changed in four days. The quota rejection that followed landed <strong>18 s into the boot on 11 of 11</strong> traced startups, and was filed under the database layer: <strong>69 quota rejections logged as <code>database_session_error</code></strong> with a full traceback, sending anyone triaging them into the wrong subsystem. Meanwhile the four interleaved reindexations left <strong>807 chunks for 269 distinct contents</strong> — three copies of every answer — and since retrieval sorts by score then truncates at five, the FAQ LIA consults about itself was returning <strong>two distinct answers out of five</strong>. No test, no metric and no log said so. The space row is now claimed with <code>FOR UPDATE SKIP LOCKED</code>, embeddings are computed <em>before</em> the first destructive statement, and staleness is judged on the stored corpus rather than on a hash alone — so the next boot repairs the duplication instead of freezing it. <strong>269 contents in 10 requests</strong> per startup, a bounded retry classified on the status code rather than on message text, and an alert for the day the knowledge base stops being updated. <strong>16,146 backend</strong> + <strong>3,473 frontend</strong> tests, MyPy strict clean, every ratchet held, and three landing figures corrected <em>downward</em> where they over-claimed. — 27 July 2026.
...
| [GUIDE_TESTING](./docs/guides/GUIDE_TESTING.md)               | Testing strategy (~16,146 backend tests across 853 files) |
