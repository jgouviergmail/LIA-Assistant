...
<p align="center">
  <strong>Version 1.45.1</strong> — <strong>What you keep is searched, your portrait says where it comes from, and the assistant binds the tools a question needs.</strong> A kept answer is indexed in a knowledge space of your own and cited weeks later; the user portrait is compiled from memories, interests, learned habits and relationship debriefs and shows its sources; the ReAct loop binds tools by relevance instead of a blind cap and sees the same knowledge the pipeline sees; the rich HTML mode lays every data-bearing answer out as a composed page, on conversational turns too; a document that cannot be indexed says why in your language, a scanned PDF with its remedy; and the face never plays the same thing twice — 17 September 2026.
</p>
...
|                           |                                         |                             |                                                                         |
| ------------------------- | --------------------------------------- | --------------------------- | ----------------------------------------------------------------------- |
| **49** functional domains | **660,000** lines of code (excl. tests) | **38,000+** automated tests | **293** ADRs                                                           |
| **262** versions shipped  | **6 languages**, parity enforced in CI  | **564** Prometheus metrics  | [**8.3/10** technical audit, 24 normalized areas](docs/audit/README.md) |
...
- **Prometheus**: 564 custom metrics (agents, LLM, infrastructure). A metric nobody can see is a metric nobody acts on: every one must be wired to a Grafana panel, a recording rule or an alert, and a shrink-only ratchet fails the build on a newly blind metric.
...
| Metric                  | Value                                                                                                 |
| ----------------------- | ----------------------------------------------------------------------------------------------------- |
| Backend tests           | 29,725 collected over `tests/` (`pytest --collect-only -q`, 1,758 files, 2026-09-17)                  |
| Frontend tests (vitest) | 8,423 across 660 files, plus hermetic Playwright journeys with axe, dark-mode and zoom checks          |
...
293 ADR files (ADR-001 through ADR-294 — ADR-008 has no separate file) record every major architectural decision with its context, the alternatives and, increasingly, the production measurement that motivated it. Three to start with, and [the full index](docs/architecture/ADR_INDEX.md):
