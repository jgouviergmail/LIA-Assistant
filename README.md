...
<p align="center">
  <strong>Version 1.30.12</strong> — <strong>The run context becomes typed — and a sub-assistant can no longer lose it in silence</strong> ([ADR-231](docs/architecture/ADR-231-Contexte-Runtime-Type.md)). The "who, where, with which preferences" of every conversation travelled in an untyped 17-key bag, written in one place and read across 43 files — four keys were private and unpublished, and the same identity circulated under <strong>two names and two types</strong> (a UUID from one writer, a string from another, plus a duplicate justified by an integration that was never installed). It is now a frozen dataclass: built once, injected at four points, <strong>validated at the graph's entrance</strong> — a missing or malformed context fails loudly instead of degrading silently, including on the resume of a pending confirmation, where the absence used to pass unnoticed (measured, not assumed). The ReAct sub-agent used to re-project that context by hand, keeping <strong>6 keys out of 17</strong> and silently dropping consented geolocation, the original message and the sender identity; it now <em>derives</em> the whole context from its parent, so the next field added is carried without touching a line. Two new CI guards close the holes that made this migration risky in the first place: every one of the <strong>105 registered tools</strong> must convert to the schema the LLM actually sees, with no injected argument leaking to the model — nothing in CI checked that surface before, and the guard was proven able to fail by injecting three mutations. The study behind the release also settled a strategic question with evidence: LangChain's Agent Server v0.13 was evaluated in depth and <strong>refused on five independent blockers</strong> — the typed-context half of the idea never needed it, and ships here on the MIT package already installed. Backend coverage floor raised to <strong>67%</strong> (measured 69.04%), 25,600+ automated tests. — 19 August 2026.
</p>
...
|                           |                                         |                             |                                                                         |
| ------------------------- | --------------------------------------- | --------------------------- | ----------------------------------------------------------------------- |
| **41** functional domains | **540,000** lines of code (excl. tests) | **25,600+** automated tests | **230** ADRs                                                           |
