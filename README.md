...
<p align="center">
  <strong>Version 1.27.12</strong> — <strong>The debug panel now reads like the run itself</strong> (ADR-209). It had grown by iterations and lied about chronology: the query displayed after the routing decision, <em>planned</em> waves after the <em>actual</em> timeline, and a hand-maintained node list that filed every unknown node <strong>after the response</strong> — in ReAct mode, the model loop appeared to run after the answer it produced. Every LLM call (and image generation) now carries a <strong>run-anchored start offset</strong> shared by the pipeline and its background tasks, so the trace orders itself by what actually happened — and a <strong>waterfall</strong> renders serialization vs parallelism at a glance. The panel reads in <strong>7 numbered phases</strong> in execution order, folds idle sections behind a per-phase disclosure, and finally shows the stages that existed nowhere: the semantic validator's verdict (<strong>informative</strong> — a rejected plan still executes, ADR-184 stated in place), the ReAct loop against its <strong>published</strong> iteration bound, human-in-the-loop gates, context compaction, image-generation and TTS spend, and the open-loop extraction's LLM cost. One presentation grammar replaces three mixed languages and a dark-only palette: design-system tokens for every status (contrast-guarded across the five themes), <strong>bi-theme identity families</strong> for nodes, a single score bar with its <strong>decision threshold drawn on the bar</strong>, neutral empty states — an absent stage is not a failure — and an <strong>anomaly channel</strong> (failed steps, panic planning, budget zones, schema drift detected by Zod without ever hiding a section) surfaced on a scannable per-request summary strip: route, engine, duration, tokens, cost. Frontend coverage floors raised on all four axes (70/66/66/71), complexity ratchet tightened (50 → 48). <strong>18,041 backend</strong> + <strong>4,987 frontend</strong> tests, all six languages. — 5 August 2026.
