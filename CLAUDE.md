...
**Five ReAct invariants (ADR-248, ADR-286), each paid for by a production defect:**
...
- **Tests**: pytest with `asyncio_mode = "auto"`. Coverage threshold: 73% (ratchet: never lowered — raise the floor after coverage-improving work to lock the gains, keeping ≥2 pts margin vs measured; see GUIDE_TESTING.md). Markers: `e2e`, `integration`, `slow`, `benchmark`, `multiprocess`.
...
- **Every euro the platform pays for a person is traced, displayed, attributed
  ...
  the model column alone and priced the Maps lookups at nothing.
...
- **Observability**: 583 Prometheus metrics defined in `src/infrastructure/observability/`. Langfuse for LLM tracing.
...
- ADR index (301 ADR files, ADR-302 latest — ADR-008 has no separate file, so the highest number runs one above the file count): `docs/architecture/ADR_INDEX.md`
