...
task test:backend:unit:coverage    # The CI command verbatim, including the 63% floor
...
- **Tests**: pytest with `asyncio_mode = "auto"`. Coverage threshold: 63% (ratchet: never lowered — raise the floor after coverage-improving work to lock the gains, keeping ≥2 pts margin vs measured; see GUIDE_TESTING.md). Markers: `e2e`, `integration`, `slow`, `benchmark`, `multiprocess`.
...
- ADR index (181 architectural decisions, ADR-181 latest): `docs/architecture/ADR_INDEX.md`
