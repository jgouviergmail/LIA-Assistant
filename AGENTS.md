...
- **What a model accepts is a derived `ReasoningProfile`**, not a catalogue column:
  ...
  (`core/reasoning_profiles.py`, re-exported by `llm/reasoning/profiles.py` —
  `core` reads it, so it cannot live in `infrastructure`). A new provider is one rule entry plus
...
User Message → Router Node → (react mode → ReAct Setup → ReAct Call Model ↔ ReAct Execute Tools → ReAct Finalize → Response)
                                                                 ↕ ReAct Recovery (a declared gap, bounded passes — ADR-310)
```
...
**Six ReAct invariants (ADR-248, ADR-286, ADR-310), each paid for by a defect measured on a real turn:**
...
3. **The loop knows what the pipeline knows.** `react_setup_node` injects the
   ...
   The same holds for the person's KNOWLEDGE SPACES (2026-09-17, ADR-248
   amendment): `build_knowledge_block` mounts the pipeline's own
   `<UserDocuments>` — read from the bundle the router prefetched, never
   consumed (`peek_response_context`) — and the tool cap keeps every family
   reachable (ADR-293), so the knowledge-space search is never the tool that
   falls off. Measured on dev: a kept answer holding the fact reached the
   response prompt while the loop, blind to it, spent its iterations elsewhere.
...
5. **A tool result is projected item by item under a token budget, and the
   cut is stated** (ADR-286). `render_data_block` (`tools/react_tool_wrapper.py`)
   pages the heaviest item list of `structured_data` at ITEM boundaries (the
   first item always passes WHOLE — a cut item is unusable, an oversized one
   merely costs) under
   `min(react_tool_result_max_tokens, window × react_tool_result_window_fraction)`
   — the window being the slot's own (ADR-278) — and `compose_tool_message`
   appends `tool_result_budget_note` AFTER the `</external_content>` tag when it
   cut, and counts it (`react_tool_result_truncated_total`). Before: a JSON dump
   cut at 8 000 CHARACTERS, spent on the first e-mail's SMTP headers and base64
   (measured 2026-09-15: zero readable field of any e-mail, eleven iterations,
   ~74 k tokens, a wrong answer; base64 tokenises at 1.45 characters per token).
   Two corollaries: the provider's raw tree never leaves the builder
   (`build_emails_output` pops `payload` once its two readers ran), and a tool
   whose data only the pipeline executor injects declares
   `execution_modes={pipeline}` (`local_query_engine_tool`), the mirror of the
   ADR-249 rule. **And the block fails CLOSED**: `mark_untrusted_data` wraps it
   as external unless every registry item it carries is INTERNAL — a
   `structured_data` with no registry is wrapped, because `get_context_list`
   and `resolve_reference` re-serve e-mail bodies exactly that way.
6. **A turn is judged on its RESULT: a gap it declares buys a bounded recovery
   pass** (ADR-310). The loop used to end the moment the model called no tool —
   measured 2026-09-23: a forecast came back for the wrong day (the tool read
   « demain » as today and said nothing), the loop saw it and answered « I could
   not get it » after 4 iterations of the 70 its budget allowed, a web search
   never tried. Every fact the loop SET OUT to obtain — the person's, and each
   cross-check it started — now ends obtained or declared in an `<unresolved>`
   block with the rungs tried, never dropped in silence: measured on the
   incident's replay, letting the loop judge what its answer « cannot do
   without » dropped the wrong-day forecast without a word in 4 runs of 9. A detail
   nobody asked for and nobody looked up is no gap, nor is a fact obtained from
   another source (it carries that source) — the looser « facts the answer
   needs » made two model families of three declare wind and rain on a clean
   turn. `should_recover` CALLS `react_exit_reason` and routes a declared gap to
   `react_recovery` while a pass is left (`REACT_RECOVERY_PASSES_MAX`, `0` =
   off); the node calls no model, removes the draft AT the pass (every exit
   leaves one answer, the HITL hand-off included), and every later call is
   SHOWN the draft and a directive after its anchor, never written to
   `messages` — a persisted directive would be read by later turns as the
   person's words. The final message after a pass REPLACES the draft, so it
   restates every finding: told to « conclude again », a model wrote about its
   gaps alone and the list it held never reached the answer — and a pass that
   ends with NO usable answer (an empty reply, a budget cut) hands the draft
   back rather than trade it for nothing. A result that
   answers ANOTHER question is an obstacle exactly like an error; the ladder is
   the loop's own call corrected, then another source marked `(fallback)`, then
   the declaration; relative dates become ISO 8601 before any tool parameter. And
   **a tool never replaces a value it cannot read**: it refuses it
   (`INVALID_INPUT`, the accepted format, today's date), under a contract
   published by ONE constant read by the manifest AND the `@tool` schema — the
   loop binds the schema, never the manifest.
...
- `src/domains/agents/nodes/react_*.py` — ReAct mode 5-node loop (setup, call model, execute tools, recovery, finalize)
...
Four traps when touching state:
...
- **Nothing that must hold for a whole turn is derived from the LENGTH of `messages`.** The reducer trims the head by tokens on every update, so a long turn's own tool results shorten the list between two of its calls. Anchor on `current_turn_id`, or count from the END. The ReAct history's blocks were aligned on the list's length: they moved in the middle of a turn, and the provider re-billed three calls of one routine run after the tools (production 2026-09-24, ADR-309 amendment).
...
Prompts are versioned files in `src/domains/agents/prompts/v1/` (`.txt` files). Loaded via `load_prompt()` from `src/domains/agents/prompts/prompt_loader.py` — there is no fallback loader: a prompt has one text, the versioned file (ADR-284). Versions are configurable via environment variables (e.g., `RESPONSE_PROMPT_VERSION=v1`), and every `*_prompt_version` setting must have a reader (guarded).
...
- **Tests**: pytest with `asyncio_mode = "auto"`. Coverage threshold: 74% (ratchet: never lowered — raise the floor after coverage-improving work to lock the gains, keeping ≥2 pts margin vs measured; see GUIDE_TESTING.md). Markers: `e2e`, `integration`, `slow`, `benchmark`, `multiprocess`.
...
- **A session lives no longer than the queries it serves — never « around » a
  wait.** `get_db_context()` and `get_db` commit at exit, the first statement
  opens the transaction, and the pooled connection returns only at
  commit/rollback (measured: `checked out` 1 → 0). A `yield` dependency lives
  as long as the response, so a route that STREAMS authenticates through
  `get_current_active_session_for_stream` (a session of its own, closed before
  the route; guard `test_streaming_routes_hold_no_session_guard.py`) — measured
  2026-09-20: one PostgreSQL backend in `idle in transaction` PER OPEN TAB, for
  the tab's life. A runner drives a model turn with NO session open (the
  workboard runner; the voice runners since ADR-301: 8.9 s held before, none
  after). Measure with `pg_stat_activity` (`state`, `xact_start`) during the
  act, never by reading the code.
  **ADR-304 extends it to every network call** (measured 2026-09-22: the push
  wake sweep pinned 863 s of `idle in transaction` and served nothing for 14
  minutes; the census found 11-330 s sessions in the briefing, heartbeat,
  moments, telephony context and relations lens). Three shapes hold it. **A
  connector client is never built on `ConnectorService(db)`** — its token
  refreshes would land on `db`, so `db` had to stay open while the provider
  answered: hand it a `DetachedConnectorService` or open it through
  `connectors/active_client.open_active_client` (one short session for
  provider, credentials and the owner's default container NAME; the client
  closed on every path). Guard: `test_connector_client_on_session_guard.py`,
  which names the constructor argument, no allowlist. **A job that keeps its
  session commits its reads before every wait** (a provider, a model, an
  embedding batch) — in a session it owns, never a borrowed one. **The chat
  turn's connector operations each end their transaction**
  (`ConcurrencySafeConnectorService`, `__getattr__` coroutines included) and a
  client's own writes run on a short session of their own. Two traps the pass
  found: a credential READ wrote `last_used_at` on its caller's session — a
  row lock for the whole turn, and a read-modify-write of the whole JSONB that
  lost concurrent keys — now one atomic `||` merge apart under a bounded
  `lock_timeout` (`connectors/api_key_use.py`); and a claim that only
  `flush`es is no claim — `get_and_lock_pending_reminders` held up to 100 row
  locks and one transaction through every model call of its batch while its
  comment said the lock was released. Reminders are claimed ONE at a time,
  committed, settled conditioned on the claim, stale claims released on
  `updated_at`. And a bot-global resource (the Telegram webhook) is set by
  the scheduler LEADER alone, never deleted by one stopping worker.
...
- **A worker rebuilds a cached table from the DATABASE at startup, never from a shared blob.** It keeps its in-memory copy for its whole life, and a blob written before a deploy's migration still carries the old rows — measured 2026-09-23: the dev API restarted after the DeepSeek weekday migration and billed Saturday peak hours at double from a 47-minute-old pricing blob. Only a cross-worker invalidation adopts the blob, because the writer republished it just before notifying (`load_published_pricing_cache`, ADR-063). And **every writer of such a table publishes that invalidation, after its commit** — rebuild, then notify, never after a failed rebuild (`refresh_and_publish_pricing_cache`): the tariff writers used to rebuild their OWN worker alone, so under `WEB_CONCURRENCY=4` an edited price reached one worker in four and the three others billed the old one until they restarted; and the USD→EUR rate the cache converts every cost with — a table of its OWN, written by the daily sync — reached no worker at all. A cached value has as many writers as the tables it is built from: find them all.
...
- **A status vocabulary is a contract: every member is PRODUCED somewhere and READ somewhere else** (ADR-303). When one side writes a value the other has never heard of, nothing fails — the reader takes its fallback and the feature dies in silence. Measured across twenty ADRs without a line moving: `mappers.py` published `failed`, the response formatter knew only `success`/`error`/`connector_disabled`, so every failed plan reached the model as « ❓ plan_executor : Statut inconnu (failed) » **with the error text dropped** — and the two branches that restituted `error` were themselves dead, because `error` and `connector_disabled` had no producer at all. `AgentResultStatus` is now binary (`SUCCESS`/`ERROR`), a partial failure is a `SUCCESS` carrying `AgentResult.failed_steps` — **the field states the partial, never the status** — and `test_agent_status_vocabulary_guard.py` holds three rules: the `Literal` IS the enum, no reader compares a status to a retired value (allowlist entries carry a reason, and one that no longer applies fails), every member lives in at least two modules. The same trap in the other direction: a READER that looks for a key no writer writes (`step["status"] == "error"`) returns empty for the life of the feature while its own test freezes the fictional shape — writer and reader share the field CONSTANT (`FIELD_SUCCESS`, `FIELD_ERROR`, `FIELD_ERROR_CODE`), never a convention.
- **A graph config's run id has ONE reader** (`core/run_config.run_id_of`). ADR-231 moved it to `metadata`; eleven readers kept reading `configurable`, a key no writer writes, and their tests built that fictional shape. The effect register was one of them: every direct action was filed under the THREAD id, the "actions performed" card never appeared, and the pipeline step key `{thread}:step:step_1` collided across turns — production, 2026-09-22, a reminder SERVED an earlier turn's recorded result instead of being created. Guard: `test_run_id_single_reader_guard.py`. Corollary (ADR-263 §23): a ReAct answer reaches the response model as PROSE, and it never sees a tool result — the register's succeeded acts are stated in a directive of their OWN, as the model's own acts (`response_directive_performed_actions.txt`), never as data lines beside the answer: with nothing stated, 6/6 answers denied an image that existed; as a data line, 3/100 invented a second image and the person still met a denial; as the directive, 0/100 and 0/100.
...
- **A prompt states what the code enforces, and nothing else** (ADR-284, prompt audit
  2026-09-12 — every finding proved by assembling the REAL prompt, then measured on
  Docker dev). **Every `{placeholder}` of a file has a producer** in the module that
  loads it, and a file containing `{{` is rendered by `.format()`, never `.replace()`
  (`test_prompt_placeholders_are_produced.py`; a renderer reached through a constant
  is declared in `RENDERED_BY`, a third party's `{{var}}` syntax in
  `PROVIDER_TEMPLATE_SYNTAX`): the HITL question generator read
  `Tool name: {tool_name}` from v1.0.0 to v1.44.4 while every guard was green, because
  the cache guard scans the static prefix only. **One instruction per context, in the
  file, emitted only when the content exists**: `response_context_sections.txt`
  declares `key|Tag|description` in injection order and `get_response_prompt`
  neither wraps nor escapes anything itself — two wrappers with two instructions
  (« always cite » / « always synthesize ») reached the model together, six empty
  wrappers cost 246 uncached tokens per bare turn, and escaping a value here AND the
  whole prompt in the response node sent `r={x}` as `r={{x}}`. **The bound a prompt
  publishes is the bound the tool enforces, from ONE constant** (Brave published
  `maximum=20/50` to the planner and clamped at 10 in silence — ADR-184 pointing the
  other way). **A prompt promises only what the turn can run**: `<Computation>` is
  rendered iff `run_python_tool` is bound AND the capability is on, with the enforced
  budget. **Prose never lives in a `.py`, and neither does a fallback**: one-line
  scaffolds go in a `key|template` lines file read by `parse_prompt_sections`
  (`core/prompt_store.py`, ONE parser), `load_prompt_with_fallback` is gone (its
  inline copy had already lost a rule of the file), tool payload messages are
  technical English (ADR-256). A model is told a language's NAME through
  `core.i18n.get_language_name`, which normalises first (`"zh"` used to come back as
  `"zh"`). Every `*_prompt_version` setting has a reader
  (`test_prompt_version_settings_have_readers.py`).
...
- **A boot-time completeness assert must actually stop the boot.** `init_agent_registry`
  ...
  resilience (ADR-263).
- Error payloads returned to the LLM must not embed raw tracebacks or raw user params (token waste + PII in prompts). Classify with the `ToolErrorCode` taxonomy — never by string-matching on exception messages. Guarded by the shrink-only ratchet `test_no_message_substring_classification_guard.py`, which found a live defect: `browser_tools` tested `"Max concurrent" in str(e)` while `pool.py` writes « Maximum concurrent », so a saturated pool was reported as a CONFIGURATION error instead of a limit to wait out. **The scan covers the whole of `src/` and follows the alias a message is read through** (`error_str = str(e)` then `"rate" in error_str`) — scoped to two directories and reading `str(e)` inline only, it could see ONE of the three defects its own docstring names, and the six sites of `openai_tts_client`, where « generate » contains « rate », were invisible.
- **A tool fails by RETURNING, and what it returns is READ — by one predicate** (`core/tool_outcome.explicit_success`, ADR-303). `ToolErrorModel.to_response()` and `UnifiedToolOutput.failure()` never raise, so a reader that only watches for exceptions counts every failure as a success: measured in production, four HTTP 403s wrote four `failed` rows in the consultation register while Prometheus saw four successes, and `business:tool_success_rate:5m_by_tool` was 1.0 by construction — its two alerts, one of them PagerDuty, could not fire. Three readers now share the predicate (register, metrics decorator, ReAct loop); a failure log carries the error CODE, never the message (a code is bounded, a message carries the URL, the name, the mailbox). **And what a failure means reaches the model**: pipeline step failures through the runtime failures directive (which reads the shape the executor writes, names the tool and publishes the EXACT total), ReAct failures through a STRUCTURAL marker on the message itself (`ToolMessage.status="error"`, measured as surviving the checkpoint) — because the ReAct body carries the tool's PROSE, so nothing in it can be parsed. Told nothing, a model invents: measured, 7 of 21 answers on failed turns carried a fabricated configuration diagnosis against **0 of 142** on clean ones, and a scheduled briefing announced every other morning that three `ACTIVE` connectors were « not configured ». A declared failure also buys NO iteration (ADR-256's rule, which `bool(pydantic_model)` silently defeated).
...
- **Logging is configured before the application is imported, and a logger is bound where it logs.** Whatever logs, or binds a logger, before `configure_logging()` keeps structlog's defaults — no level filter, no PII filter, console rendering Promtail cannot parse. `main.py` imported every route first while the tool singletons bound their logger in `__init__`: DEBUG tool lines with their parameters reached production logs at `LOG_LEVEL=INFO` (measured 2026-09-23). The configuration is `main.py`'s first application import (`observability/logging_bootstrap.py`, guard `test_logging_configured_first_guard.py`) and the tool bases expose `logger` as a property (`_ToolLoggerMixin`).
...
- **A capability that is not a TOOL records no consultation, by construction.**
  ...
  **A cache hit is not a consultation** (Redis answered; the mailbox was never
  ...
  refused reads `failed` — never a silent success. **And a row recorded outside
  a published collector is DROPPED**: the sink keeps only what a run gathers,
  and no act of a knowledge space ever published one — every `space_read` since
  ADR-262 wrote nothing (zero `space:*` rows on dev, found 2026-09-17). A
  surface that runs alone opens its own run (`consultation_collector`) when
  `collector_is_active()` says nobody did, and joins the turn otherwise.
...
- **Every euro the platform pays for a person is traced, displayed, attributed
  and counted for that person — whatever the modality or the path** (owner
  rule 2026-09-19, ADR-272 amendment 2026-09-20). Not the model's tokens
  alone: every INSTANCE family of `cost_bearers` — and Google Maps Platform had
  holes in every direction. **A persistence predicate covers EVERY family the
  tracker holds**: both doors of `TrackingContext` decided on the model's
  records alone, so a phone-call or live-session lookup on Places or Routes
  (no model call) wrote nothing to any of the four ledgers; `pending_families()`
  is the ONE predicate and a guard refuses a `_*_records` bucket it does not
  read. **A counter never fails open**: `track_google_api_call` « did nothing »
  without an ambient tracker — 3 020 usage rows on dev, none outside a chat
  turn, while the heartbeat (Routes), the briefing (Google Weather: 8 billed
  calls per refresh), the meetings (Geocoding) and the image proxies paid; it
  now counts `google_api_calls_unaccounted_total` (alert
  `GoogleApiCallsUnaccounted`, expected 0). **An out-of-turn surface opens its
  own accounting around the WHOLE act** (runner, briefing, meeting enrichment,
  profile geocoding) and **the road is declared** in
  `domains/google_api/spend_roads.py`, the sibling of `LLM_SPEND_ROADS`, guarded
  over the import graph with the doors checked as AST calls. **A billed image
  is counted where Google bills it** — at the proxy, on the 200, through ONE
  helper — never pre-counted when the URL is built (a fetch that may never
  happen, a re-fetch never counted), and joined to its turn by a run id SIGNED
  over (run id, account): a bare id in a URL lets a caller create another
  account's summary row. **One figure for what a run cost**:
  `MessageTokenSummary.billed_cost_eur` — the phone bill and the live card read
  the model column alone and priced the Maps lookups at nothing. **A Maps call is
  filed at the SKU its REQUEST triggers, and a price is the vendor's, read**
  (price audit 2026-09-23, migration `d5f8b2a6c9e3`): Places by field mask
  (`:lite`), Routes by request features (`routes_sku_suffix` — traffic-aware →
  `:pro`, tolls or two-wheeler → `:enterprise`, which the default drive route
  is: measured, a Paris→Lyon route billed $15 per 1000 had been filed at $5),
  and a Route Matrix per ELEMENT returned (`units`, persisted as
  `request_count`). The same audit found eleven token tariffs, nine image prices
  and Static Street View different from the vendors' pages; a correction
  migration replaces only a value LIA shipped, never an administrator's.
...
- **Connector abstraction**: Google, Apple, Microsoft APIs share a common connector pattern with provider resolver (`src/domains/connectors/`). Only one provider active per functional category (email, calendar, contacts, tasks). When touching a provider client, check the **other providers' behavior for the same operation** (cache invalidation, reply-all semantics, multi-valued field updates) — provider asymmetries are a recurring bug source; parity is the rule. **An e-mail speaks ONE vocabulary at the client boundary** (`EmailMessage`, ADR-287): clean text body, quoted history and signature removed, no fabricated Gmail tree — the three normalisers are measured against it on real payloads. **A digest is computed once per message and cached** (`EmailDigestService`, family `email:digest`); `get_emails_tool` chooses what it serves through `detail` (`metadata` | `summary` | `full`), and a tool's model spend travels on the runtime's `RunnableConfig` — handed no config, the structured door builds a fresh one and the turn's tracker never sees the call (measured 2026-09-15: six digests, zero `token_usage_recorded`).
- **HITL (Human-in-the-Loop)**: 6 approval levels (plan approval, clarification, draft critique, destructive confirm, FOR_EACH confirm, modifier review). Classified in `src/domains/agents/services/hitl_classifier.py`. **Several drafts in one turn are reviewed one at a time** (`nodes/draft_sequence.py`, ADR-288): only the members of a FOR_EACH the person approved this turn are confirmed as one lot. **A draft card and its result are described once and drawn per surface** (`drafts/card_spec.py`, `drafts/card_html.py`, ADR-289): the chat gets a `lia-card`, a ticket the Markdown. Note: the plan-approval level is currently auto-approved (`approval_gate_node` is a pass-through — tool-level HITL supersedes it); do not build on plan-level interrupts without re-wiring the gate.
...
- **Observability**: 587 Prometheus metrics defined in `src/infrastructure/observability/`. Langfuse for LLM tracing.
...
- Review all plan and code on substance and form:
  ...
  16. Prompts into versioned files in `src/domains/agents/prompts/v1/`, loaded via `load_prompt()` (no inline fallback — ADR-284)
  ...
  8. **Prompt files** — New prompts placed in `src/domains/agents/prompts/v1/`, loaded via `load_prompt()`, prompt name added to `PromptName` Literal; every `{placeholder}` produced by the loader (guarded), one-line scaffolds in a `key|template` lines file read by `parse_prompt_sections`
...
- A prompt states what the code enforces, and nothing else (ADR-284, amending ADR-184, ADR-025, ADR-245, ADR-248, ADR-263, ADR-272): `docs/architecture/ADR-284-Prompt-States-What-The-Code-Enforces.md` — the 2026-09-12 prompt audit's six proven defects and their guards; measured before fixing: the HITL placeholder defect had NO effect on 24 real questions, the cite/synthesize contradiction NONE on 12 real answers (a third instruction in the RAG header decided), so both were treated as hygiene and cost, never as quality
- A short answer is asked for without reasoning, and a model family is declared once (ADR-285, amending ADR-245, ADR-267, ADR-244, ADR-275, ADR-272, ADR-223): `docs/architecture/ADR-285-Short-Answer-Without-Reasoning-And-One-Family-Declaration.md` — production moved the `response` slot to `deepseek-flash` (DeepSeek's CURRENT name for V4.1-Flash; `deepseek-v4-flash` is a retired alias the API still accepts) and every reminder arrived as « 🔔 » alone: the generator asked for 150 tokens, the model thinks by default and bills the thinking INSIDE `max_tokens`, so 150 tokens of hidden chain of thought and an empty answer (measured 3/3 in production; OpenAI, Anthropic and Gemini count it inside the cap too). **A caller that needs two sentences declares `none` through the seam and keeps its small budget only where the resolved profile `can_disable`** (`short_answer_config`, `core/llm_config_helper.py`; `REMINDER_MESSAGE_MAX_TOKENS`) — a cap that includes the thinking is not a cap on the answer. **An empty or truncated answer is a refusal** (`is_output_truncated`, ADR-275): the written fallback goes out, the spend that happened is kept. **The DeepSeek thinking family is declared ONCE** (`DEEPSEEK_THINKING_PREFIXES` in `core/reasoning_profiles.py`, read through `is_deepseek_thinking_model` by the adapter and the structured-output detour; an AST guard refuses a private `startswith("deepseek-v4")`): three private copies had all missed the current name, so the admin UI offered no ladder, an explicit `none` sent no `thinking: disabled`, and the V3 output cap of 8 192 applied to a model that emits 384 K. The ladder is the vendor's documented `none/low/high/max`; the output cap is the catalogue row (`providers/deepseek_limits.py`); the current name travels by migration `e9b5d7f3a2c4` AND by seed, priced with its UTC windows, provenance `verified` like the row production curated by hand (a guard holds migration and seed equal); `model_name_of_response` / `reasoning_tokens_of` join `usage_metadata.py` — the ninth local reader billed every reminder to an unnamed model at zero.
- A kept answer is indexed like a document of a knowledge space (ADR-291, amending ADR-282, ADR-258, ADR-262, ADR-279/280, ADR-272): `docs/technical/BOOKMARKS.md` § « The knowledge space » — a bookmark is PROJECTED into a per-account space found by ROLE (`rag_spaces.kind = 'bookmarks'`, `BOOKMARKS_SPACE_KIND`), never a system space (no owner); the meetings shape (ADR-258), not a synced source (ADR-262): the bookmark is the RECORD (`rag_document_id` `SET NULL`, `index_state` = why there is NO projection, `indexed_at`), the Markdown document its projection (dated title, quoted request or the notification line, the answer — an HTML `lia-response` converted by `html_to_markdown`, the ONE reading of « how HTML becomes text »: `markdownify(strip=["script","style"])` used to KEEP the tags' content). **The document is the authority while it exists**: the exposed state is DERIVED (`bookmarks/projection.py`), the cost reported once READY only. **A projection is CLAIMED first** (`claim_for_projection`, one conditional UPDATE; a stale `pending` is taken over); the gates leave honest states — both capabilities read AT CALL TIME → `disabled`, `spend_blocked` → `deferred`, a failure → `error` (dead-lettered, never retried by the sweep; keeping again re-projects). The backfill rides the reaper's tick (`infrastructure/scheduler/rag_maintenance.py`, composed there because `rag_spaces → bookmarks` would close a cycle; the reaper runs FIRST), serves the batch least recently ATTEMPTED first (a refused attempt goes to the back of the queue — by creation, one account under quota starved everybody else) and selects nothing while the capabilities are off; a bookmark deleted between its document's commit and the link's takes the document back (the link's row count says the row is gone). The space and document caps do not apply (the bookmarks cap is the bound); a kept answer's document is neither movable nor deletable from the space (409 `document_managed_by_bookmarks`, ONE table `MANAGED_DOCUMENT_CODES` — which also made a label's document unmovable, as ADR-262 had stated), a space managed by role is not deletable by hand (403 `space_managed_by_domain`, meetings included), `kind` is on the space's wire shape. Every display name somebody else wrote goes through `rag_spaces/document_names.py`; `process_document` logs no display name at INFO.
- The user portrait reads four sources, and says which (ADR-292, amending ADR-079, ADR-088, ADR-184, ADR-269, ADR-280, ADR-270): `docs/technical/JOURNALS.md` § « The four portrait sources » — memories, interests, learned habits and relationship debriefs enter the consolidation prompt as MATERIAL for the synthesis. **`journals` imports no source domain**: `interests` and `relations` already import `journals`, so the sources are OFFERED through `domains/shared/portrait_sources.py` (closed vocabulary `PORTRAIT_SOURCE_KEYS`, one `portrait_source.py` per domain, the boot INSTALLS explicitly and refuses an incomplete registry — an already-imported module wires nothing). **A reader answers `used | empty | disabled | unavailable` with an EXACT total**, under its three gates read at call time (ceiling, operator switch, the person's preference), and never raises: a blind source is NAMED, never read as empty. The assembler (`journals/portrait_sources.py`) reads ONE source at a time under `JOURNAL_CONSOLIDATION_*_MAX` and drops a section WHOLE over `JOURNAL_CONSOLIDATION_SOURCES_MAX_CHARS`. The prompt renders a section only when it exists; STEP 7 forbids re-listing facts, quoting a line verbatim and mentioning the sources — « a source absent from SECTION 1 is UNKNOWN, not empty » — and the portrait budgets are `{portrait_full_tokens}` / `{portrait_brief_tokens}` from settings (a stated budget, not a clamp: measured overshoot about a third). **The provenance is persisted WITH the portrait** (`users.journal_portrait_sources`, same UPDATE, untouched when no portrait came back; read through `provenance_of`, lenient) and drawn under it; **a source that moved reopens the eligibility** (one `EXISTS` per installed `FreshnessProbe` on `Base.metadata`, entries OUTER-joined). The usage and health scaffolds moved into `journal_portrait_source_lines.txt` (ADR-284), and `emotional_label` lives in `memories/emotional_state.py`.
- Several drafts in one turn are reviewed ONE AT A TIME, a pre-approved FOR_EACH lot stays grouped (ADR-288, amending ADR-153, ADR-276, ADR-263, ADR-184, ADR-274): `docs/architecture/ADR-288-Independent-Drafts-Reviewed-One-At-A-Time.md` — measured 2026-09-16: two independent e-mails, ONE question on the first, « Valider » sent both, « Modifier » rewrote only the first; the queue carried a FOR_EACH lot semantics applied to every multi-draft turn. The PRODUCER declares the mode (`pending_drafts_grouped`, true only for `is_pre_approved_lot`: items of a FOR_EACH approved THIS turn, one type), a sequence banks each decision in `confirmed_drafts` and presents the next draft in its own node execution, the last answer `settle`s everything into one ordered `confirm_batch` whose entries carry their own decision and type, a terminal cancel REPORTS what was still queued, the executor skips cancelled entries and counts them, the position (« Draft 2 of 3 ») is a server line in six languages, and a NEW turn starts with no draft under review (`draft_turn_reset()`, spread by the router like the ReAct counters — the interrupt record expires, the checkpoint never did)
- A draft and its result are DESCRIBED once and drawn per surface (ADR-289, amending ADR-276 lots 13-14, ADR-274, ADR-288, ADR-185): `docs/architecture/ADR-289-Draft-Surfaces-Drawn-From-One-Description.md` — the per-type renderers describe the card (`drafts/card_spec.py`: `Row`, `Note`, `Block`), `to_markdown_lines` draws the lot-13 Markdown byte for byte (the golden net is the proof), `card_html.to_html_card` draws the chat's `lia-card` from the data cards' own classes; the SURFACE is decided by the run (`card_surface()`: an out-of-turn origin, a channel's `plain_surface_ctx`, or the `markdown` display mode render plain), never guessed by a caller; `filter_for_llm_context` flattens a `lia-draft*` card to its words (what was sent, to whom) where a data card is reduced to its prose; the first interrupt of a sequence carries `sequence_drafts` and its question opens on « N drafts to review » with one row per draft in its own type's wording; the execution result is described too (`describe_execution_result`) and every batch entry carries the registry's `detail_fields` minus what the row already says, the first text field as a bounded excerpt (`DRAFT_RESULT_EXCERPT_MAX_CHARS`)
- The phone as a channel (ADR-290, amending ADR-127, ADR-263, ADR-276, ADR-179, ADR-184, ADR-284): `docs/architecture/ADR-290-Phone-As-A-Channel-Owner-Calls.md` — LIA calls the PERSON on a number they declared and VERIFIED by a spoken code (never a name match; `TelephonyIdentityService.verified_number` is the ONE seam the no-card exception rests on, and the third-party tool refuses that number), through the SAME vendor agent under a per-call `conversation_config_override` rendered server-side (`telephony/mandates.py`, one mandate per `CallKind`, boot-asserted; the override permission is part of the agent fingerprint and its sync is MANDATORY for an override); `call_me_tool` is `reversible` with a written reason, so a routine may plan it; the call carries the chat's context under a token budget (`self_call_context.py`, sections in a declared order, cuts stated, reads filed on the `phone_call` surface); the transcript comes back as the person's OWN turn (`infrastructure/scheduler/phone_relay_runner.py`, `StreamRequest.spoken_by_person`, a VISIBLE `RunOrigin`), claimed as `RELAYING` before the turn and settled by conditional update, stale rows swept by the notification reaper, ten `RelayOutcome`s counted and drawn — « nobody picked up » and « the line failed » told apart from « someone else answered » (production 2026-09-16, a call that died at pickup was reported as a stranger on the line); live read-only lookups during the call are FLAGGED (`TELEPHONY_LIVE_TOOLS_ENABLED`, `agents/telephony/live_tools.py`: an allowlist boot-asserted read-only, vendor webhook tools provisioned per connector by fingerprint and attached to the AGENT for the owner's call only — the vendor REFUSES `tool_ids` inside a per-call override, measured on a real call — detached when it ends and again before any stranger is dialled (`live_tools_attached` on the connector), a session-less call-back that answers only for an ACTIVE owner call with a token DERIVED from the webhook secret, arguments validated by the tool's own schema, bounded under the vendor timeout, projected like ReAct). **What the voice agent SOUNDS like is the portal's, never a setting** (owner decision 2026-09-16): its LLM and reasoning effort, language, voice, audio format and duration cap are administered on the ElevenLabs portal without a restart — the vendor MERGES a PATCH with the stored config and validates the pair, so a pinned model collided with the portal's reasoning effort and refused every owner and verification call; LIA sends only what is its own (prompt, greeting, tools, context, data contract) and the agent fingerprint covers exactly that. Measured on production: the verification call succeeds, and one owner call ran end to end with four live lookups. **Lot 8 — the phone reads everything the chat reads** (owner decision 2026-09-16, read-only by construction): the tool set is a RULE over the catalogue (`derive_live_tool_specs`: search/explicit-read, not `system`, a phone domain of `shared/phone_domains.PHONE_DOMAINS`, speakable required parameters — 55 tools over 22 domains, plus the native `recall_memories`), every result is reduced to what a VOICE can say before the ADR-286 paging (`voice_projection.py` — measured: four weekend events returned, one heard), every euro of a call lands under ONE run id (`telephony/spend.phone_call_run_id`: live lookups under a `TrackingContext` whose callback rides the synthetic runtime's config, the synthesis, the relayed turn — so the chat meter's own summary row is the call's bill, carried by `GET /telephony/calls` as `usage`), and the person switches domains off for their own calls (`users.phone_disabled_domains`, the DISABLED set, read from their row at every call-back). **Lot 9**: the personality the person configured for LIA (the chat's own `PersonalityService` door) reaches both mandates — rendered into the owner prompt, a dynamic variable for the baked third-party agent — colouring how the voice speaks, never what it may do. Not measured yet: an owner call under the derived set.
- A face that never plays the same thing twice (ADR-294, amending ADR-264, ADR-252, ADR-240): `docs/architecture/ADR-294-A-Face-That-Never-Plays-The-Same-Twice.md` — every finding MEASURED on the running widget before it was touched (a recorder around `CSSStyleDeclaration.setProperty`, five minutes on the chat page of a throwaway account). **The scenes played and nobody saw them**: the wait was drawn once at rig construction on the rig's own clock, so every page navigation restarted it and every draw that fell on a thought was lost — a `SketchClock` the HOST keeps for the document now counts RESTING time only (first scene at 20-40 s of rest, later 45-120 s, never one of the last three), and a scene that looks aside turns the head. **A presence channel is an interface element switching on** (27 flashes in five minutes): `browA`/`mouthA` are gone, both organs drawn whole. **The brow sits on the visible edge of the eye** (pinned to the box it floated a third of an eye above a joy dome), has a derived weight (`browS`), knits (`browX`), leads on its own group, and a one-sided beat is flipped to either side (`flipTapes`). **Speech is generated** (`rig/speech.ts`: syllables, words, phrase pauses, a shape per word, stressed words the brows mark) and a pattern reaching its cycle is RESOLVED AGAIN, never rewound. **No two performances alike**: `warpTapes` (a pace, then per-CHANNEL time and size — one factor per channel so a hold and its release stay joined), `withRelease` (each mimic's own spring home), phrases instead of a cadence, the face answering the eyes (`answerIn`). **The pose's pull ramps up when a beat hands a channel back** (`SPRING_BLEND_MS`) — never at an attack, never between two beats, never on a new expression (a startle blended out of a sad face is a dulled startle); and the mask blink holds shut PAST the swap (`maskBlinkTapes`). Found on the way: `tapeSpringIn` imposed a tape's spring before its first key. Rejected: a replay button, a crescent mouth, blending every spring change, a bimodal eye cadence.
- The ReAct loop binds tools by RELEVANCE, the planner keeps its catalogue (ADR-293, amending ADR-070, ADR-248, ADR-256, ADR-184): `docs/technical/REACT_EXECUTION_MODE.md` § « Tool System » — the cap used to cut in REGISTRATION order, so on any account with more tools than the cap the same families fell off on every turn whatever the question (the user's own MCP tools among them), and eighty schemas were 95 % of the first call's prompt, uncounted. **One embedding, two readers**: the router's domain scores for the planner are unchanged, and from the same vector `rank_tools` orders EVERY available manifest into `tool_selection_result["global_ranking"]` (`router_tool_scoring.score_tools_for_turn`, the one seam; `None` = no scores, as before). **The loop binds by composition and no tool name is written anywhere**: the detected domains' tools, then every other family's `CATALOGUE_DOMAIN_COVERAGE_TOP_N` best-ranked tools (the catalogue's own coverage constant, shared), then the first `REACT_TOOL_SEMANTIC_TOP_K` of the ranking; the cap is a safety net by stable sort on tiers; no ranking or K = 0 means the previous behaviour. **A declared « core » list was written and rejected** — it encoded one account's usage in a multi-user product; family coverage says it structurally. The planner catalogue is unchanged (its exposure is domain under-detection, which the ranking does not fix). **A binding unit is bound whole or not at all** (amendment 2026-09-20): tools that are ONE affordance declare `ToolManifest.binding_unit` (the four skills tools, `SKILLS_BINDING_UNIT`, guarded) and the selector ranks the unit as its best member, seats it once, and never cuts inside it — measured on dev, the ranking bound `activate_skill_tool` and dropped `run_skill_script`, so the loop activated a skill it could not run and answered in prose (the production motif on both skills). `react_tools_bound` and `react_bound_tool_tokens` measure the prefix the delivered-context histogram never counted; `task react:selection:measure` replays an operator's own turns and holds no one's.
- A knowledge-space document attaches to a message as a COPY, paused space or not (ADR-295, amending ADR-279, ADR-280, ADR-286, ADR-185): `docs/technical/ATTACHMENTS_INTEGRATION.md` § `from-knowledge-document` — the chat's retrieval reads the ACTIVE spaces alone, so the « + » lists the `ready` documents of EVERY space of the account (`GET /rag-spaces/documents`, mounted BEFORE `/{space_id}`, page and exact total, LIKE escaped) and copies the chosen one through the attachments' own mechanism (`attachments/knowledge_copy.py`, the one `attachments → rag_spaces` edge): the stored file under a fresh UUID name, the text through `rag_spaces.processing.extract_text` (fifteen formats, not the upload's image/PDF allowlist) under `attachments_max_pdf_text_chars`, the row in `origin = upload` so the reset removes it and the space is never touched; only a `ready` document (409 `document_not_ready`), both capability guards on the route (the route-wiring guard names the two doors that put a file in). **The « + » is a menu iff it has a second entry** (browser: one-click picker until the spaces are on; Android: camera; knowledge last). **A cut is stated to the model** for every document attachment (`[truncated: the first N characters …]`), and the hint labels come from `APIMessages.attachment_hint_labels` through `normalize_language` — keyed `zh`, they served English to every Chinese reader. The ReAct LOOP still sees the hint alone (as for uploads); the text reaches the synthesis in both modes (measured on dev).
- A mail attachment is read — its text when it has one, the vision slot otherwise (ADR-296, amending ADR-286, ADR-287, ADR-272, ADR-275, ADR-184): `docs/technical/REACT_EXECUTION_MODE.md` § « Reading a mail attachment » — `EmailClientProtocol.download_attachment` on the three clients (parity contract; Gmail in `gmail_attachments_mixin.py`, extracted so the frozen client shrank; IMAP publishes the part INDEX as its handle), ONE selection (`clients/email_attachments.select_attachment`: handle, else name, an ambiguous name refused with its candidates). **A Gmail handle EXPIRES between two reads of a message** (measured 2026-09-17 on a real mailbox): a stale handle resolves by the name, then by uniqueness, else answers `DISAMBIGUATION_REQUIRED` with the CURRENT handles — the manifest tells the model to prefer the file name and pass both. `agents/emails/attachment_content.py` reads BYTES (magic bytes decide the route; `extract_text` in a thread; an image or a scanned PDF rendered to bounded, downscaled pages for `vision_analysis` through `invoke_with_instrumentation` with the runtime's config — `skipped_quota` under a ceiling, a truncated answer a refusal); `tools/email_attachment_tools.py` serves the text in PARTS under `emails_body_part_tokens`, wrapped as EXTERNAL content, every refusal named; `email_attachment_reads_total{route,outcome}` on dashboard 10. `vision_analysis` joined the `LLMType` Literal (the slot vocabulary guard's gap shrank). Known limit shared with uploads: HEIC needs a Pillow plugin the image does not ship.
- A linked Drive folder is a TREE, and a sync past the threshold states its exact count (ADR-297, amending ADR-261, ADR-262, ADR-185, ADR-184, ADR-263, superseding ADR-056's non-recursive rule): ONE bounded breadth-first walk (`rag_spaces/drive_walk.py`: `RAG_DRIVE_MAX_FILES_PER_SYNC` and `RAG_DRIVE_MAX_FOLDERS_PER_WALK`, `truncated` stated, shortcuts never followed, a folder walked once, an unreadable sub-folder counted) shared by the synchronisation and `GET …/drive-sources/{id}/preflight`, which classifies every file with the ingest's own predicates (`is_supported_drive_file`, `is_unchanged` — extracted from `ingest_drive_file`) under the space's document cap and publishes `to_index`, the threshold (`rag_drive_sync_confirm_threshold`, 10) and the bounds; the frontend asks it AT THE CLICK and, past the threshold, an alert dialog states the exact figures (« at least » when cut) before posting the sync. **The walked folder set travels with the source** (`rag_drive_sources.folder_ids`, JSONB, migration `b8d4f6c0e2a3`, reassigned as a NEW list): the push routes on it and follows the feed (a folder created under the tree joins, a trashed one leaves); a NESTED link is refused (409 `drive_folder_nested`, ancestors both ways under `RAG_DRIVE_MAX_ANCESTOR_DEPTH`) because two sources would steal each other's documents. The background sync now records ITS consultation (`space_read`), which it never did. `FakeDriveClient` (`tests/unit/domains/rag_spaces/drive_fakes.py`) honours one level per call, the filter and the paging — the real API's contract.
- The sandbox reaches the network through ONE proxy, with tokens it cannot read and a question it cannot skip (ADR-298, amending ADR-249, ADR-263, ADR-280, ADR-284, ADR-288, SEC-001): `docs/architecture/ADR-298-Sandbox-Egress-Toolbox.md` and `docs/technical/REACT_EXECUTION_MODE.md` § « Network runs » — iron-proxy 0.49.0 as a sibling container, the ONLY routed member of the `internal` network `lia-sandbox` the sandbox joins alone (an env-variable interception was rejected: a raw socket walks around it), its state minted by its own entrypoint into tmpfs volumes (an init container was measured wrong — Docker released the tmpfs at its exit); every declared host has ONE of four statuses (`connector` DERIVED from the client classes, `operator`, `grant`, `unknown`); a credential is a per-run token read from `os.environ["LIA_KEY_<CONNECTOR>"]` and SWAPPED by the proxy for the person's own key on that host alone — never OAuth, never an instance key, never inside the container; the ruleset is rendered from a Redis registry of live runs (owner-token claim, `redis_claim.py`) and a refused reload is a run that never starts. **An unknown host is ASKED with three answers** (with the turn's data / without / refuse — `confirm_without_data`, the action the web normaliser used to drop), remembered as a grant under a published cap — and **settled IN the loop** (`nodes/react_egress_question.py`): a permission is not an action, so the node raises the interrupt itself and re-invokes the SAME call under the answer (`tool_path.approved_for_call`), because a dispatched draft answers from its result and drops every later step (measured 2026-09-18: the mails never counted); a question costs none of the turn's runs; the script never leaves the loop; and `interrupt()` is a bubble-up the node's tool-error net must re-raise, not swallow. **A network run is claimed before the container starts** (`in_turn_effects.py`, `python_sandbox_network`). **The prompt is measured before it is trusted**: told « `LIA_KEY_X` in the header » the model sent the variable's NAME as the value (422, twice, no self-correction) — the line spells the read; the libraries a prompt promises live in ONE table pinned DIRECTLY (six were transitive) and are imported in the built image (`task sandbox:libraries:check`); `<Computation>` costs 439/692 tokens (no network / four hosts). **A list in state is an accumulator too**: `react_scripts` grew for the life of the thread until it joined `react_turn_reset()`.
- The live voice mode — two intelligences, one seam (ADR-299, amending ADR-070, ADR-245, ADR-258, ADR-263, ADR-272, ADR-280, ADR-290): `docs/architecture/ADR-299-Live-Voice-Mode-Two-Intelligences-One-Seam.md` and `docs/technical/LIVE_MODE.md` — a speech-to-speech session on the PERSON's own provider key (connector category `live`, `GEMINI_LIVE` today, another provider is one transport + one `LiveProvider` + one CSP host away); nothing of the provider's is counted, stored or shown. **The voice model owns the conversation and DELEGATES every data or action request to the chat engine through ONE `NON_BLOCKING` function, `send_to_lia`**, which the browser turns into an ordinary `POST /chat/stream` with the person's own cookie — the delegated turn runs in the graph (HITL, registers, quotas, archive, learning), draws itself in the thread stamped `live_session_id` + `spoken_text`, and the bridge hands the voice a bounded, flattened answer or LIA's pending question, which IS the answer; the voice never holds a tool, a credential or a row of its own, and — owner decision — the mode is NOT bridled (ADR-290's read-only rule is about a stranger's line). Four facts MEASURED before the design was frozen (`task live:probe`, spec § 9), each a rule in the code: a credential's constraint does NOT lock the system instruction (the API renders the `setup`, the browser replays it verbatim); a used single-use credential CANNOT reconnect (every reconnection mints a fresh one for the SAME record, `POST /live/sessions/{id}/credential`, riding the resumption handle); the provider silently ACCEPTS an unknown voice name (the voice is validated against the vendored, dated, sourced list — never by the provider); a raw browser WebSocket is accepted ONLY by the `BidiGenerateContentConstrained` method with the token in `access_token`. One session per account (owner-token claim `live:session`, USER_RUNTIME; `live:active` GLOBAL caps the instance; a mint rate limit; the record outlives the cap by `LIVE_SESSION_RECORD_GRACE_SECONDS` so a session that ran to it still closes its books). A voice-only exchange is archived at `turnComplete` as two visible `live_turn` rows (injected into the next written turn like the proactive rows, `out_of_graph.py`); the session closes on ONE `live_session_summary` card whose spend is the SUM of the delegated runs' own `message_token_summary` rows — LIA's, in the meter's vocabulary, displayed at the end of the mode (owner decision); a session is ONE decision row (`live_session`), each delegated turn its own run. The thinking level is offered ONLY on the model's ADR-245 ladder (`gemini_live` family) and refused off it on the write path (`thinking_level_unknown`). Web: `LiveSessionController` (a plain class, tested without React) behind the thin `useLiveSession`; `useLiveHoldsMicrophone` extends ADR-258's one-owner rule to the wake-word loop; `effectiveVoiceState` feeds the eyes without a new input; the entry button is `aria-disabled`, never `disabled`; the page's complexity FELL (27 → 23, `composerLocks`); CSP allowlists ONE exact host (`LIVE_PROVIDER_CONNECT_SRC`), and the mobile probe measures `external_websocket`. E2E: `chat-live-session.spec.ts` plays the provider through `page.routeWebSocket` on Chromium's fake microphone.
- A second live provider — one seam, two wires (ADR-300, amending ADR-299, ADR-245, ADR-184, ADR-185, ADR-263, ADR-272, ADR-260): `docs/architecture/ADR-300-A-Second-Live-Provider-One-Seam-Two-Wires.md` and `docs/technical/LIVE_MODE.md` § « Two wires, one seam » — GPT-Live joins Gemini Live without a line of the bridge, the banner, the registers or the closing card moving. **A provider DECLARES its wire, the seam never branches on its name**: `connection` (`token` — the browser opens the socket with the provider's credential; `offer` — the API mints its OWN single-use nonce, kept on the session record and consumed by `POST /live/sessions/{id}/offer`, which exchanges the browser's SDP on the person's key: « one credential opens one connection » on both wires, a second exchange 409 `credential_invalid`, an extension re-mints on `token` only because a WebRTC reconnection is a NEW provider session) and `delegation_wire` (`tool` — `send_to_lia`; `native` — `session.delegation.created` carries an id and an `offset_ms`, NO text: the request is COMPOSED in the browser from the input transcript, `lib/live/request-composer.ts`, after a short grace). The mandate is rendered with ONE verb per wire (`delegate_by_call` / `delegate_native`) and a delegation block per wire and capability. **The transport derives what the seam needs from the provider's own signals** (`transports/openai-webrtc.ts`): the assistant's transcript IS its speech (its quiet ends the utterance and the turn), the person's words meanwhile are an interruption, a result is split under the documented 500-token append bound, the session is ready on the MEASURED `session.started` (bounded), `close()` waits for `session.closed`, `session.usage.updated` is read by nobody. **A voice the speech endpoint does not serve is sampled by the live model itself** (`providers/openai_live_socket.py`: silence streamed at real-time pace, the sentence through an instruction append — `delegation_id: null` and `content` are REQUIRED, measured — the continuous output trimmed by `infrastructure/media/pcm.py`); the same socket is the activation probe, which judges model AND voice (the provider refuses a wrong voice BEFORE any start, unlike Gemini). **The `live` category is ADDITIVE** (`CONNECTOR_ADDITIVE_CATEGORIES`): both keys active, the sessions on the account's choice (`users.live_preferences.provider`; saving a connector makes it the one), the settings listing the UNION of the models grouped by brand — choosing a model IS choosing its provider — and the reflexes a provider decides itself (`configurable_vad: false`) not offered. The wave also brought: ONE header voice icon (a toggle, or a menu « Comments / Live »), no TTS comment on a delegated turn, opaque bands, a voice sample on every voice change, Extended Thinking activating on a probe of the REAL setup, the newest request winning (a correction is a SECOND `toolCall`, measured), idle 10 s / cap 10 min with unlimited EXPLICIT extensions (an open Gemini socket dies at the token's expiry, so an extension re-mints and reconnects at once). Measured before trusted, in a real Chromium against dev with a microphone playing a spoken question: connected in 480 ms, the model delegating BY ITSELF 6 ms after the last transcript fragment, our commentary spoken back and acknowledged. A WebRTC wire needs no CSP host; the mobile shells have not measured one yet. **Amended 2026-09-19 (five owner adjustments)**: a connector remembers EVERY model it was set up on (`live/model_settings.py` — a model switched to comes back with its own voice, level and durations); the silence timeout and the cap are the MODEL's, `0` = no limit under a billing warning (a provider may bill the waiting), bounds shared with the instance settings and published, the gap under the minimum refused; an unlimited cap ROLLS by extension slices renewed in silence and never counted as the person's extensions (`unlimited_cap`, `kind=rolling`); the header entry is « Live session (<brand>) »; no talk mode; LIA's inner state reaches the mandate (the psyche engine's own block) and each answer's register (ADR-253) reaches the voice as a DELIVERY NOTE (`live_tone_lines.txt`, a `tone` field on Gemini, a silent `thinking.append` on GPT-Live). **Gemini's `enableAffectiveDialog` is measured OUT** — a `setupComplete` then 1007 at the first thing the model must answer, spoken input included; a negative test pins it. **The PCM player is ONE AudioWorklet reading a queue, never one `AudioBufferSourceNode` per chunk**: Chrome 152.0.7977.83 (the owner's build, measured by the community on AI Studio too) sometimes renders a scheduled chunk's start as one render block repeated ~100 times — the « biiiip » of every session; the raw provider audio is clean. The two voices are EXCLUSIVE (a live start switches the spoken replies off, the checkbox is refused during a session, the toast rides the session's own `live` status), and a provider close reaches the API log AND the banner's toast with its code and reason (`LiveEndRequest.detail`). **Wave 3 (2026-09-19)**: the session's cost is SHOWN, never recorded — the provider's own usage reports (Gemini `usageMetadata` per turn by modality, the whole context re-billed; GPT-Live `session.usage.updated` seconds) are folded in the browser (`lib/live/meter.ts`) and priced by the tariff the start publishes (`LiveRates`, from the pricing cache), drawn under the last caption behind a lateral bar, in the chat meter's vocabulary with the context size; a cost needing an undeclared rate is unavailable, never partial. **A live model is offered and started only when the tariff table declares it** (`domains/live/pricing.py`, `model_unpriced`; the listing NAMES the undeclared): the table gained the audio pair (`audio_input_unit_price` / `audio_output_unit_price`, both or neither, `per_1m_tokens` only; migration `f1a3c5e7b9d2` + seed + workbook v3, guarded equal), and the captions are not an option (billed inside the minute or at the text rate, needed by both wires). An optional per-session spend ceiling is the CONNECTOR's (`session_budget_eur` ≤ `LIVE_SESSION_BUDGET_EUR_MAX`, outcome `budget_reached`). The Android shell produces a WebRTC offer and loads a `blob:` AudioWorklet under the production CSP (measured); iOS not yet. **Wave 4 (2026-09-19, owner decision « faire simple »)**: a DIRECT session is the phone's line in the browser — the third entry of the voice menu where `LiveModelCapabilities.direct_tools` is true (never a provider id), the voice model holding LIA's READ-ONLY tools itself (the phone's derived set minus the person's switches, `direct_mandate.py`) and never delegating; the mode is a column of the session RECORD (`LiveSessionRecord.mode`, `POST /live/sessions {mode}`, `mode_unsupported` elsewhere); ONE tool door `POST /live/sessions/{id}/tools` (`tool_door.py`) on the phone's `run_live_tool` generalised to a `VoiceToolHost` (surface `live_session`, the session's run id, node `live_session_tool`), a per-session lookup budget (`LIVE_DIRECT_TOOL_CALLS_MAX`), every refusal a sentence the voice says, `live_tool_calls_total` on dashboard 30; the exchanges were RECORDED NOWHERE (owner request 2026-09-19: no `live_turn` row, 409 `direct_not_archived`, no learning pass — SUPERSEDED the next day by ADR-301: the exchanges are KEPT in the session record and relayed at the end as the person's own turn, the 409 is gone; what stays true is that no `live_turn` row is ever archived for a direct session, so nothing reaches the next written turn's extractors through `out_of_graph.py`); the header's voice menu follows the Live settings through `stores/revisionStore.ts` (a writer bumps `live_connectors`, the reader's query depends on it). **The owner's premise measured on a real session**: 55 declarations are 35 864 of the setup's 40 312 characters and the first turn bills 10 983 prompt tokens against 1 227 delegated — a direct session buys latency, never economy. **Found by the first proof**: `from_json` did not read `mode` (every lookup « not a direct session ») — a round-trip equality test over EVERY field now pins the record and the setup-inputs pair (`setup_inputs_to_dict` / `setup_inputs_from_dict`). **ElevenLabs Agents is the third provider**, on the first wire: the person's AGENT stands where a model stands (`label` = its name), everything but the prompt and LIA's tools stays on the portal (`portal_voice`: no voice, no sample, the sentinel `agent` held equal across the stack), the credential a signed URL on the `convai` subprotocol, the delegation a blocking CLIENT tool. Three provider-neutral seams grew: `LiveProvider.billing` (`tariff` — the platform's table prices the model, the start publishes `LiveRates`; `vendor` — the session runs on the PERSON's own key under the vendor's own grid and the platform prices NOTHING of it: `rates` null, the clock alone on the meter, no ceiling, `LiveModelCapabilities.vendor_billed`, offered whatever the table holds, never refused as « unpriced ». **ElevenLabs has TWO price grids and the tariff table knows ONE** — owner rule 2026-09-20: the SIMPLE API (STT/TTS, per character, what the deployment's own slots run — `eleven_v3_conversational` is a `tts` row at 0.05 USD per 1 000 characters, migration `a4c8e1f7b3d5` + seed, guarded equal) and the AGENTS API (telephony and the live mode, per conversation minute plus the LLM, on the person's key only — priced by nobody here). Two guesses were written and retired first: the flat row `elevenlabs-agents` at 0.10 USD/min (`b7d1e3f5a9c2`, kept inactive and OUT of the catalogue, `RETIRED_CATALOGUE`), then « the agent's voice model's minute as a floor » — a per-character figure read as a per-minute one. The agent's `voice_model` and `llm` are still read at the listing, for information), `VendorBilling` (the end reads the vendor's own bill under the conversation id the wire named — `cost_fiat`, the LLM/call charges in credits, the models charged for, the duration — SHOWN once by the banner, recorded nowhere: nothing of the person's own key reaches a ledger) and `AgentSyncing` (before the mint: the prompt-override permission MERGED into the agent's own overrides block, LIA's client tools created once per fingerprint, `tool_ids` = the person's own + THIS session's set, kept on the connector's metadata as a NEW dict); the agents listing lives in `infrastructure/llm/providers/elevenlabs_live_listing.py` so `connectors` imports neither `live` nor `telephony` (the cycle ratchet caught the first draft). **Measured on the owner's agent the same evening** (the first draft was read from the official `@elevenlabs/client` 1.25.0 source): `source_info.source` is validated against the vendor's SDK names and any other word closes 1008 AFTER the metadata — the frame carries none, and the probe waits `ELEVENLABS_LIVE_PROBE_SETTLE_SECONDS` past the metadata, which is NOT the verdict; a client tool's array items need a `description` (422, the phone's own measurement, now in `function_declaration`); a refused creation CANCELS its siblings before the rollback (`TaskGroup`, never `gather` — 99 orphan tools measured in the workspace and removed by hand).
- Voice sessions: one policy per mode, whichever the line (ADR-301, amending ADR-290, ADR-299, ADR-300, ADR-272, ADR-263, ADR-185, ADR-184): `docs/architecture/ADR-301-Voice-Sessions-One-Policy-Per-Mode.md`, `docs/technical/TELEPHONY.md` § « Live mode » and `docs/technical/LIVE_MODE.md` § « The voice sessions' bounded context » — the phone (ADR-290) relayed the conversation at its END while the browser's live mode delegated every request DURING; the two lines now share a MODE (`delegated` « Live » — every request a chat turn of the person as it is said, with HITL, registers, quotas, archive; `direct` « Live direct » — the voice reads, acts on nothing, and the words are relayed at the end as the person's own turn) and the policy follows the mode alone. **ONE closing** (`infrastructure/scheduler/voice_session_closing.close_voice_session`), called by `POST /live/sessions/{id}/end` AND the phone's post-call webhook: delegated archives the voice-only exchanges, draws the card with EXACT figures, files the decision and learns; direct synthesises and relays in a task the closing owns and REWRITES the card at the settle with its cost re-read (the relayed turn spent under the session's run after the card was written — ADR-185). **What both share moved OUT of both**: `domains/voice_sessions/` (the `VoiceSession` value with its carrier-neutral KEY — the phone's is `phone_call_<hex>`, minted there so `telephony` reads it and never the reverse —, `VoiceTranscript` with the delegated exchanges marked WHOLE as the browser flags a buffer, the projection pinned to `lib/live/delegation.ts` by ONE corpus run by pytest and vitest, the summary queries, the delegation block of every mandate) and the runners in `infrastructure/scheduler/` (`voice_relay` — the former `self_call_relay`, moved byte for byte under a golden file that now pins the contract, the context's `SESSION:` line included —, `voice_delegation`); the HTML flattener the projection needs is HANDED IN (`agents` reads the package, so a runtime import back closed a cycle the ratchet caught). **The phone's Live mode is the browser's delegation, server-side**: one asynchronous vendor webhook tool `send_to_lia` (the browser's own name, description and schema; `telephony/delegation_tool.py`), no context and no lookup for the voice (strict symmetry, owner D5), and `voice_delegation.delegate` mirrors the TS bridge rule for rule — an empty request costs nothing; **the newest request wins** through a Redis marker the running bridge polls (two vendor webhooks may land on two workers; proven on real Redis with two tasks); **a question LIA asked IS the result** and the next request RESUMES the run that asked (`original_run_id`, reused for the accounting, the chat router's own resumption); the turn is the PERSON's (`spoken_by_person`, no out-of-turn origin — the gate ASKS); a wait past the vendor's bound leaves the turn running and the voice says so; nothing reaches the voice as an exception. **Measured on the vendor's real engine before it was trusted** (no phone — owner constraint: the vendor played in HTTP against the dev API, then a text-only conversation on the production mandate and tool through a tunnel, `task telephony:simulate:live` / `task telephony:probe:live`): `execution_mode: async` + `pre_tool_speech: force` let the voice announce and keep talking; past `response_timeout_secs` the late answer is LOST, so the bridge answers `timed_out` the inner margin before (`TELEPHONY_DELEGATION_TIMEOUT_SECONDS`); the transcript writes an async call as an EMPTY agent entry with `tool_calls`, its acknowledgement as another, then a second pair and the restitution. **The choice is the person's, the effective mode is derived and published** (`users.phone_call_mode`, Live by default; `phone_calls.call_mode`, the mode a call RAN — written at the dial, read at the closing): Live is available only where the callback base's host is PUBLIC (`telephony/callback.py`, `TELEPHONY_CALLBACK_BASE_URL` for a dev tunnel), `GET /telephony/identity` publishes `live_available` / the reason / `call_mode_effective`, the settings list is disabled with the reason, and a delegation tool the vendor refused degrades the call to DIRECT and says so (ADR-184). A DIRECT browser session keeps its turns in its record (a bounded Redis list, `POST …/turns` answers no id, the list living as long as the record — an extension moves both) and the 409 `direct_not_archived` is gone. **`POST …/end` calls no model** (review 2026-09-20): the synthesis AND the relayed turn run in the task the closing owns, `end` answers `scheduled` or `empty` at once (measured 0,026 s) and releases the claim, the settle writes the fate WHATEVER raised (a fresh session for the rewrite; a turn that ran is never reported failed by a rewrite that failed after it), every `RelayOutcome` has its sentence (guarded, both sides), the words that could not become a turn keep their recap on the card (`live_summary.relay_summary` — the phone's fallback push carries the same; the card's figures live under their OWN key, since the origin stamp of a relayed browser turn writes under `live_session` and would have replaced them), and an authenticated session is the account holder's by construction — the synthesis's own `owner_confirmed` (a model output) is overwritten, since it could answer `not_owner` for the person's own session. On the ElevenLabs agent, at most ONE client-tool set per KIND survives a fingerprint change (`elevenlabs_live_tool_kinds`): the old set is taken off the agent then deleted, the phone's own rule on drift. The domain switches (`phone_disabled_domains`) govern every direct voice surface, the browser's included, so the settings draw them whatever mode the phone runs in. Found on the way: `html_to_text` glued « IntituléSenterre » where the browser read « Intitulé : … » (the corpus now pins both); `ConversationRepository.get_by_id` is the CONVERSATION's — a card read through it was never rewritten (`get_message`, a PostgreSQL test). **A call nobody answered has no mode** (review 2026-09-20): no answer, a voicemail or a failed line is told « nobody answered » / « the line failed » whatever the person chose, decided BEFORE the mode — a Live call used to close with nothing to deliver; and a Live call whose delegation tool cannot be provisioned runs DIRECT with what direct needs (the mode is settled before the lookups and the context are built). **The lookup admission is ONE sequence** (`agents/telephony/voice_lookup.serve_voice_lookup`): offered under the door's own gate, then the host's budget (a refused tool spends nothing), then the run under the `VoiceToolHost` — the phone's call-back and the browser's session door hand it their offered set, their budget and their way of saying a refusal; a spoken HITL on a mutable tool (not built) slots in between the budget and the run, never in a door. The relay synthesis is told which line carried the session (`voice_relay_lines.txt`, `SESSION:`): a browser session is authenticated by construction, a phone call is confirmed on the transcript. **A voice runner holds no database session across the turn** (the workboard runner's rule): `run_voice_relay` probes on a session of its own, the settle and the phone's `run_relay` open theirs after the turn — measured 8.9 s of `idle in transaction` before, none after, and a commit hands the pooled connection back (`checked out` 1 → 0). Known and written: a pending question of a previous session takes the first request of the next call for its answer — the chat's own rule (ADR-276 lot 7).
- A tool failure is SAID, never guessed (ADR-303, amending ADR-184, ADR-182, ADR-185, ADR-248, ADR-254, ADR-263, ADR-286): `docs/architecture/ADR-303-Tool-Failure-Restitution.md` — one status vocabulary whose every member is produced and read, an aggregate that is binary with `failed_steps` carrying the partial, ONE fact ONE channel (the honesty directive, no longer gated on `DIAGNOSTICS_ENABLED`), a STRUCTURAL verdict in ReAct, one success predicate for the register, the metrics and the loop, and a shrink-only ratchet against classifying an error by the words of its message
- No transaction open across a network call (ADR-304, amending ADR-261, ADR-301, ADR-263, ADR-119, ADR-283): `docs/architecture/ADR-304-No-Transaction-Across-A-Network-Call.md` — a connector client never built on a session-bound service (guarded), one door to a category's active client, jobs commit their reads before every wait, the chat turn's connector operations end their transactions, the API-key stamp is an atomic merge apart, the Drive push drain is bounded and resumable with a compare-and-set token and a truncation breaker, wakes served one at a time under a ceiling (`PushWakeSweepStalled` watches queued-but-unserved), reminders claimed one at a time, the Telegram webhook set by the leader alone, WebSockets on uvicorn's sans-I/O implementation, and the web container's V8 heap bounded below its limit
- An image model declares its offer; one provider client serves it (ADR-305, amending ADR-184, ADR-272, ADR-279, ADR-063): `docs/architecture/ADR-305-Image-Model-Declares-Its-Offer.md` and `docs/technical/IMAGE_GENERATION.md` — a FAMILY (`image_generation/families.py`, ordered provider + prefix rules like the reasoning profiles) declares what a model accepts (qualities, a fixed size list or an area envelope, billing tiers, whether an edit's reference image is billed per image, the source box and byte limit) and every surface reads that one answer: a model no family declares cannot be priced (the admin write refuses the row), offered (`ImageOptionsCache` keeps servable models only), selected (the image slot's list comes from the image domain alone, the write refuses `image_model_not_served`) or run. One client per provider under `providers/`, the registry checked at import and by `run_failfast_validations()`. **The model that runs is the model that is priced**: the OpenAI edit is `images.edit` on the configured model — the Responses detour ran the tool's default `gpt-image-1` and an unrecorded text model. Qwen answers with a URL valid 24 h, downloaded at once under a vendor-host allowlist, no redirect, bounded, PNG signature checked, the vendor's billed tier compared with ours. **A preference is an intent**: validated for its shape, mapped at run time by ONE resolver (`preferences.py`: an unoffered quality → the cheapest offered, a size → same orientation and nearest area, an edit → the source's proportions within the preferred tier), published as `effective_*` by `GET /image-generation/options`; the person's output format (PNG, JPEG, WebP) is applied once, for every vendor, by converting the client's PNG before storage (`encoding.py`). The cost counts the reference image (`cost_per_input_image_usd`, required or refused by the family) in the pricing cache's one exchange rate. Qwen's rows travel by seed AND migration `a9d3f1c7e5b2`, guarded equal; a same-named parameter bound in a select list and a comparison is two types under asyncpg — bind each once, typed. **A client's failure carries its FACTS, never its wording**: `ImageGenerationError` holds the vendor's status, its code and a timeout flag (the OpenAI-wire translation lives once, in `providers/openai_sdk.py`), and the tool classifies them through `http_status_to_error_code` (ADR-303) — a 429 reads « retry later », not the `TOOL_ERROR` every failure used to read.
- The Claude request surface is declared once; the prompt cache is shaped, and billed, the way the vendor measures it (ADR-306, amending ADR-087, ADR-244, ADR-245, ADR-263 lot 7, ADR-272, ADR-284): `docs/architecture/ADR-306-Claude-Request-Surface-And-Billed-Prompt-Cache.md` — what each Claude generation accepts was MEASURED on the API and lives in ONE table, `core/claude_surface.py` (sampling refused from Opus 4.7 on, a forced `tool_choice` refused by Fable 5.1 and Opus 5.5, six thinking shapes, the depth an empty request runs at, thinking blocks BOUND to their conversation on Fable 5.1/Opus 5.5 — a 400 once the history before them changes, which LIA's per-turn system prompt always does); the reasoning rules, the adapter (`providers/anthropic_kwargs.py`), the structured-output door and the admin temperature lock read it, never a model name. `anthropic_adaptive_display` renders a level as adaptive thinking + `display` + an effort carried in `output_config` (what `ChatAnthropic` publishes, so the Article-12 row finally holds the depth); `ReasoningProfile.implicit_level` makes the thinking-budget guard read an empty level on an always-on or default-on generation as the depth it runs at. **The payload is shaped for the cache** (`providers/anthropic_payload.py`, ONE policy on the factory hook): the static prefix is marked at `DYNAMIC CONTEXT` whatever the system's shape (a list-shaped system used to put the breakpoint on the turn's data: 0 read, 5,222 rewritten at 1.25x), the rolling breakpoint only in a tool loop, a previous turn's thinking never replayed, the 5-minute TTL only (guarded). **A cache write is billed at its price on every path**: `UsageTokens.cache_write` is the fourth count of the one reader — read on the generic `cache_creation` (the only key the path LIA runs reports, measured) and on the TTL breakdown — and the TARIFF says what it costs (`CachedModelPrice.cache_write_multiplier`: 1.25 for every Anthropic and OpenAI tariff, 1.0 elsewhere); every pricing door takes the writes explicitly (`test_cache_write_reaches_every_price`, AST, no allowlist — `0` where the priced thing has no prompt cache). `TokenCaptureHandler` counts through the one reader, which ended a double billing of cached tokens on four surfaces (every provider). The eight new rows travel by seed AND migration `c3e7a1f5d9b2`, `verified`, guarded against the surface. Measured through the real chain on dev: 5,076 written tokens billed 0.012884 USD, the vendor's exact arithmetic (0.010346 before). **The OpenAI half (section 7)**: GPT-5.6 and GPT-6 cache by breakpoint and bill a write at 1.25x, and OpenAI's default breakpoint ended after the turn's data, so every call REWROTE its whole prompt (0 read, 2,873 rewritten) — `providers/openai_payload.py`, applied by `ChatOpenAICached`, marks the static prefix at `DYNAMIC CONTEXT` (2,832 read at 0.1x, 41 written) and keeps the IMPLICIT mode a tool loop reads through; the `prompt_cache_key` now stops at the FIRST marker as well — a later marker-less system message (a ReAct turn's context blocks, a response's agent results) was hashed whole, gave every turn its own key and, from GPT-5.6 on, its own CACHE (0 read before, 2,831 after); the field is sent to the DECLARED families alone (`gpt-6`, `gpt-5.6`), because ten earlier models refuse it with a 400 on every call, and a guard holds the catalogue's matches equal to the measured list. The price follows what the vendor REPORTS (the provider-wide `openai` line: only those two generations report a write), the shape what the model ACCEPTS.
- A connector that asks nothing of the person belongs to the instance (ADR-307, amending ADR-126, ADR-134, ADR-184): `docs/architecture/ADR-307-Keyless-Connectors-Belong-To-The-Instance.md` — Wikipedia, page browsing, Google Places, Google Weather and Google Environment have NO per-account row and « My connectors » neither lists nor offers them (owner decision 2026-09-23, no explanatory line). ONE predicate, `connectors/keyless.py` (the administrator's global switch, the platform key, `BROWSER_ENABLED`), is what `ConnectorService.is_connector_active` answers for every keyless type; migration `b2e6d0f4a8c1` deleted the rows, which brought back every account that had switched one off, never received one (created before 2026-09-12) or kept a `REVOKED` one after an administrator re-enabled the type — the stored values are enum NAMES, measured. In the « weather » category the person's own provider wins and Google Weather is the instance's DEFAULT (`provider_resolver._instance_default`): with a row each, bulk reactivation would have silently taken OpenWeatherMap from those who chose it. `APIKeyActivationRequest` refuses a keyless type (activation and rotation), a backend test refuses one in the frontend's activation lists, a withheld keyless service answers `CONFIGURATION_ERROR` « not available on this instance » (never « go to Settings ») and a 401/403 on the platform key raises no « Reconnect » banner.
- One flag makes a ReAct turn's prefix the previous turn's (ADR-308, amending ADR-293, ADR-169, ADR-306): `docs/architecture/ADR-308-ReAct-Cross-Turn-Prompt-Cache.md` — `REACT_CROSS_TURN_CACHE_ENABLED`, off by default (off, nothing changes), one behaviour for every model (owner arbitration): `ReactToolSelector.select` binds EVERY available tool in registration order and `react_call_model_node` places the turn's context AFTER the question (`nodes/react_turn_layout.py`), the leading system message being the static prompt ending on its `DYNAMIC CONTEXT` line — where the shapers cut and the OpenAI cache key stops. **Both, or neither**: measured 2026-09-23 on 396 real turns weighted by production's gaps between ReAct turns, the pair saves 18 % (Claude Sonnet 5) to 42 % (deepseek-flash) per turn, while every tool WITHOUT the move costs 44 % MORE on DeepSeek, whose cached prefix puts the system before the tools; the history window slides every turn, so the history is never the lever. « After the question » has one shape per provider, DECLARED (`CONTEXT_PLACEMENT`, boot-checked): a system message for OpenAI, DeepSeek, Qwen (measured); the context appended to the question's own text for Anthropic (a non-first system message is refused), Gemini (its client folds a later one back into the system instruction), Ollama, Perplexity. Every tool that cannot be bound falls back to the relevance selection, COUNTED (`react_cross_turn_cache_fallback_total{reason}`): `cap` (the bound rose to 400 — production's main account already holds 212 tools; 400 tools weigh ~140K tokens of schemas, meant for a 1M-window model) or `window` (`REACT_CROSS_TURN_CACHE_MAX_WINDOW_FRACTION` of the slot's window). A model with no prompt cache pays more (+18 % measured on one); every slot is configurable, so the flag is judged on its AVERAGE over the cache mechanisms, never on one model.
- One prompt layout for every cache mechanism (ADR-309, amending ADR-306, ADR-308, ADR-284, ADR-064, ADR-147): `docs/architecture/ADR-309-One-Prompt-Layout-For-Every-Cache-Mechanism.md` — **owner rule: never reason on one provider or model, every slot is configurable; aim at the best AVERAGE.** Stable before volatile, and no cache write nobody reads; each provider adapter translates the ONE boundary (`core/prompt_layout.split_at_marker`, the end of the `DYNAMIC_CONTEXT_MARKER` line) into its own mechanism, and a model without a cache pays what it paid. A single call sends its static part as the system message (`single_call_messages` — as ONE user message it was never marked: on gpt-6-luna every extraction read 0 % and wrote all at 1.25x, to the cent), AST-guarded on the CALL — the journal extraction included, its rules and analyst persona now above its marker (`{analyst_persona}`, refused when missing); a request no tool loop can continue writes its declared breakpoints only (GPT-5.6/6 explicit mode when no tool is bound); **the response sends the conversation ONCE**, as its messages, whole — the `<History>` text repeated the same window cut at 500 characters, dropped the compaction summary and escaped the plan-rejection filter — and its sections keep their attention order, because every one of them changes per turn (the « profile » is a memory retrieval for the query: a stable-first order was written, measured on the owner's turns and reverted); the initiative's catalogue opens its dynamic part; under `REACT_CROSS_TURN_CACHE_ENABLED` the ReAct LOOP's history drops by blocks (`REACT_CROSS_TURN_HISTORY_BLOCK_FRACTION`), counted by the TURN COUNTER from the end in whole turns (`get_block_windowed_messages` — aligned on the thread's length, every head trim of the reducer moved them mid-turn: amended 2026-09-24), while the response slides — its conversation follows the turn's context, which no cache reads; Qwen 3.5/3.6 (no implicit cache in any region) are marked explicitly by `ChatQwenCached`, whose write (`cache_creation_input_tokens`, unmapped by langchain) reaches the one usage reader at 1.25x. Gemini's 0 % was the provider, not LIA: measured, gemini-3.7-flash reads nothing below ~10K-token requests whatever its documented 4,096 minimum.
- A ReAct turn is judged on its result (ADR-310, amending ADR-248, ADR-303, ADR-298, ADR-284, ADR-184): `docs/architecture/ADR-310-ReAct-Turn-Judged-On-Its-Result.md` — invariant 6 above, in full; `task react:recovery:measure` runs seven deterministic obstacles and a clean control through the loop's own functions on real models (Anthropic refused) and compares a doctrine with its predecessor (`--baseline-prompt`).
- The exchange rhythm is the person's choice (ADR-311, amending ADR-308, ADR-309, ADR-293): `docs/architecture/ADR-311-Exchange-Rhythm-Is-The-Persons-Choice.md` — `users.exchange_rhythm` (`frequent` | `occasional`; NULL = never chosen, which follows `REACT_CROSS_TURN_CACHE_ENABLED`, now the operator's DEFAULT, so no deployment changes at upgrade) decides the three cross-turn effects TOGETHER — every tool bound, the context after the question, the history by blocks — because, measured, the context move pays only when the tools before it stay the same and the blocks only when the history is read again. **Read once per turn**: the stream service reads the row, the single runtime-context builder resolves the effective rhythm (`core/exchange_rhythm`, the ONE reader of the setting — guarded), the router publishes it into the state (`exchange_rhythm`), the setup and every call read the state (`react_turn_layout.frequent_exchanges`); a HITL resume re-enters the node, never the router, so a turn keeps the rhythm it started with. Written through the generic profile update (`PATCH /users/{id}`), strict on write, forgiving on read; the profile publishes the EFFECTIVE rhythm (Settings › Preferences › Personalization › « Usage preferences »).
- ADR index (310 ADR files, ADR-311 latest — ADR-008 has no separate file, so the highest number runs one above the file count): `docs/architecture/ADR_INDEX.md`
