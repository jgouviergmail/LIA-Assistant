...
30. [Sichtbare Arbeit, gesteuertes Lernen](#30-sichtbare-arbeit-gesteuertes-lernen)
...
| Vollständige Transparenz | 589 Prometheus-Metriken, eingebettetes Debug-Panel, Token-für-Token-Tracking |
| Produktionszuverlässigkeit | 317 ADRs, ~32.725 von pytest gesammelte Tests in 1.974 Dateien, native Observability, HITL auf 6 Ebenen |
...
| Metrik | Wert |
|----------|--------|
| Tests | 32.725 von pytest über 1.974 Testdateien gesammelt + 9.160 vitest-Tests im Frontend (Abdeckungsschwellen fixiert, ADR-116) |
| pytest-Fixtures | 1.082, davon 48 über conftest geteilt |
| Dokumentationsdokumente | 703 |
| ADRs (Architecture Decision Records) | 317 |
| Prometheus-Metriken | 589 Definitionen |
...
|----------|---------|-------------|
| OpenAI | GPT-6 (astra, sol, luna), GPT-5.6 (terra, sol, luna), GPT-5.5, GPT-5.4 (+ mini), GPT-5.x (+ mini/nano), GPT-4.1, GPT-4o, o3/o4-mini | Natives Prompt Caching, Responses API, reasoning_effort |
...
| DeepSeek | deepseek-flash (V4.1), deepseek-v4-pro (V4), deepseek-chat (V3), deepseek-reasoner (R1) | Reduzierte Kosten, natives Reasoning |
...
**Architektur**: 5 eigene Knoten im übergeordneten LangGraph-Graphen (kein Subgraph):
...
Die Werkzeuge, die die Schleife erhält, werden **nach Relevanz gebunden, nie nach Registrierungsreihenfolge** (ADR-293). Der Router berechnet ohnehin ein Embedding der Frage, um die Domänen zu bewerten; aus demselben Vektor ordnet er jedes verfügbare Manifest — native Werkzeuge wie die MCP-Server der Person — zu einer globalen Rangfolge, die die Schleife liest. Die Auswahl setzt sich zusammen, ohne dass irgendwo ein Werkzeugname geschrieben steht: die Werkzeuge der erkannten Domänen, dann die besten jeder anderen Familie, damit jede erreichbar bleibt, dann die Spitze der Rangfolge bis zu einem veröffentlichten Budget; die Obergrenze ist nur noch ein Sicherheitsnetz per stabiler Sortierung. Eine von Hand deklarierte „Kern“-Liste wurde geschrieben und verworfen, weil sie die Nutzung eines Kontos in ein Mehrbenutzerprodukt einschrieb — die Abdeckung pro Familie sagt dasselbe strukturell. Die Schleife montiert außerdem den Wissensraum-Block, den die Pipeline injiziert, gelesen aus dem Bündel, das der Router vorgeladen hat, ohne es zu verbrauchen: Was die Pipeline weiß, weiß die Schleife. Die Zahl der gebundenen Werkzeuge und die Tokens ihrer Schemata werden gemessen, denn dieses Präfix wog den Großteil des ersten Aufrufs, ohne gezählt zu werden.
...
Die Bildgenerierung folgt derselben Regel des erklärten Angebots (ADR-305). Eine **Familie** – OpenAI GPT Image, Qwen Image 3.0 – erklärt, was ein Modell akzeptiert: seine Qualitäten, seine Größen (eine feste Liste oder eine Flächenhülle), seine Abrechnungsstufen und ob das Referenzbild einer Bearbeitung pro Bild berechnet wird. Jede Oberfläche liest diese eine Antwort: Ein Modell, das keine Familie erklärt, wird weder bepreist noch angeboten, ausgewählt oder ausgeführt. Ein Client pro Anbieter führt das konfigurierte Modell aus – die OpenAI-Bearbeitung läuft über `images.edit` auf diesem Modell, wo ein Umweg über die Responses-API das Standardmodell des SDK und ein Textmodell ausführte, das niemand zählte –, ein als URL geliefertes Ergebnis wird sofort unter einer Host-Allowlist heruntergeladen, begrenzt und geprüft, und die Vorliebe der Person ist eine Absicht, die zur Laufzeit in das Angebot des Modells aufgelöst wird (die günstigste angebotene Qualität, die Größe gleicher Ausrichtung mit der nächstliegenden Fläche) und dann als wirksamer Wert veröffentlicht wird. Das gewählte Format wird einmal für alle Anbieter angewendet, indem das Bild vor dem Speichern umgewandelt wird.

Auf Wunsch der Person kann die Beschreibung eines **neuen** Bildes vor dem Aufruf von einem eigenen Slot umgeschrieben werden, nach den Empfehlungen, die jeder Anbieter veröffentlicht (ADR-315). Das ist eine Verbesserung, nie ein Tor: Ein in Anführungszeichen zitierter Text muss unverändert zurückkommen, und eine leere, zu lange oder gescheiterte Umschreibung lässt die ursprüngliche Beschreibung hinausgehen; eine Bearbeitung wird nie umgeschrieben; der Aufruf wird abgerechnet und protokolliert wie jeder andere, und die Galerie behält die Worte der Person. Der Betreiber kann die Option für die ganze Instanz zurückziehen, und der Optionsbildschirm veröffentlicht diesen Zustand, statt eine wirkungslose Einstellung anzubieten.
...
Das Lesen und Löschen eigener Dateien überlebt die Upload-Obergrenze: der Router `attachments` wird unabhängig von `ATTACHMENTS_ENABLED` eingehängt, die Fähigkeitswache sitzt allein auf dem Upload. Da jedes erzeugte Dokument über diese Leseroute ausgeliefert wird, macht eine Instanz, die erzeugt, ohne Uploads anzunehmen — der Demonstrator —, das von ihr Erzeugte öffenbar, und der Ablaufdurchlauf läuft für die vier Produzenten der Tabelle, nie nur für den Upload.

Eine aufbewahrte Antwort wird außerdem **als Dokument eines Wissensraums indexiert** (ADR-291): ein Raum pro Konto, über seine Rolle gefunden und nie über seinen Namen, beim ersten Lesezeichen angelegt, standardmäßig aktiv, außerhalb der Raum- und Dokumentgrenzen. Das Lesezeichen bleibt der Datensatz und das Dokument seine Projektion — ein datierter Titel, die zitierte Anfrage, die Antwort in Markdown umgewandelt, wenn sie eine HTML-Seite war —, sodass der angezeigte Zustand (indexiert, ausstehend, unter einer Ausgabengrenze zurückgestellt, deaktiviert, fehlerhaft) vom Dokument abgeleitet wird, solange es existiert, und die Kosten erst gemeldet werden, wenn es bereit ist. Eine Projektion wird beansprucht, bevor sie berechnet wird, die Fähigkeits- und Ausgabentore werden beim Akt gelesen, und ein Nachholen reitet auf dem Wartungstakt der Räume und bedient zuerst das am längsten nicht versuchte Lesezeichen — ein Konto unter Quote lässt die anderen nicht mehr verhungern. Das Löschen des Lesezeichens entfernt das Dokument; das Dokument wird aus dem Raum weder verschoben noch gelöscht, und ein über seine Rolle verwalteter Raum wird nicht von Hand gelöscht.

Was erzeugt wurde, lässt sich auch **durch Fragen** wiederfinden (ADR-318): Ein Lesewerkzeug durchsucht die lebende Galerie nach Wörtern des Titels, Familie und Zeitraum und zeigt die gefundenen Dateien als die eigenen Karten des Chats, mit der exakten Gesamtzahl. Ein erzeugtes Bild lässt sich schließlich als Kopie **mit einer Verbindung teilen** (ADR-316): Der Klick ist die Bestätigung — kein Entwurf, kein Modell; zwei Tageskontingente werden je Absender unter einer transaktionsgebundenen Sperre serialisiert; die Dateikopie, die Galeriezeile der Empfängerseite und das Freigaberegister werden in einer einzigen Transaktion geschrieben; danach wird der Chat der Empfängerseite nach bestem Bemühen erreicht, auf einer eigenen Sitzung. Der Kommentar eines Dritten wird dort **wörtlich** zitiert: neutralisiert durch numerische Zeichenreferenzen, weil der Chat Markdown mit rohem HTML und `https:`-Bildern rendert und ein Backslash-Escape als Formel dargestellt würde.
...
**Was nichts zu bestätigen hat.** Eine Aktion fragt nichts, wenn nichts sie umleiten kann. Die E-Mail an sich selbst (ADR-314) hat keinen Empfänger als Parameter: Die Adresse ist die, die der Anbieter des verbundenen Postfachs nennt, oder, ohne Postfach, die bestätigte Adresse des Kontos über LIAs Relais — kein fremder Text kann sie also umleiten. Das Werkzeug erklärt sich `reversible` mit schriftlicher Begründung, sodass eine Routine ihr Ergebnis senden kann, wo ein Entwurf abgelehnt wird. Eine eingetippte Adresse, die zufällig die eigene ist, geht dagegen über den Entwurf: Ein Empfänger, der die Bestätigung manchmal überspränge, wäre ein Ziel für Injektionen.
...
**Eine einzige Suchtür** (ADR-313). Eine Anfrage wird als *Schlüssel* eingebettet — ohne die Trivialitätsmuster des Gesprächs, die mit echten Namen und kurzen Fragen kollidieren —, bevor eine Sitzung geöffnet wird; dann werden die lebenden Erinnerungen der Person oberhalb der Relevanzschwelle geliefert, auf Wunsch auf Kategorien eingegrenzt. Alle Aufrufer gehen durch sie: die Gedächtnisfakten des Planers, der Initiative und der Referenzauflösung, der 360°-Rückblick auf eine Person und die Nachschlagevorgänge am Telefon, die „was weiß ich über X“ mit X beantworten statt mit den neuesten Erinnerungen.

**Und gezielt nachgeschlagen.** Eine routbare Domäne trägt ein Lesewerkzeug, sodass die ReAct-Schleife und der Planer ein Thema nachschlagen können, das die Nachricht nicht nannte — den Absender einer gerade gelesenen E-Mail, einen in einem Dokument genannten Ort. Seine Grenzen werden aus den Quellen veröffentlicht, die es durchsetzt (das Kategorievokabular *ist* das gespeicherte, die Ergebnisobergrenze *ist* die Einstellung), die Präferenz der Person lehnt es ab wie die Einspeisung, eine sensible Erinnerung kommt mit ihrer Nutzungsnuance markiert zurück, und ein Fehler wird klassifiziert, nie „nichts erinnert“. Die hybride Suche BM25 + Embeddings lebt dort, wo sie dient: in den RAG Spaces (Abschnitt 17).
...
Das Porträt setzt sich aus **vier Quellen zusätzlich zu den Tagebüchern** zusammen (ADR-292): Erinnerungen, Interessen, gelernte Gewohnheiten und Beziehungs-Debriefs gehen als Material für die Synthese in den Konsolidierungsprompt ein. Die Tagebuch-Domäne importiert keine dieser Domänen — zwei von ihnen importieren sie bereits —, also bietet sich jede Quelle über ein geteiltes Register mit geschlossenem Vokabular an, das der Start explizit installiert und unvollständig ablehnt. Ein Leser antwortet `used | empty | disabled | unavailable` mit einer exakten Gesamtzahl, unter seinen drei beim Akt gelesenen Toren (Obergrenze, Schalter des Betreibers, Präferenz der Person), und wirft nie: Eine blinde Quelle wird benannt, nie als leer gelesen. Der Assembler liest eine Quelle nach der anderen unter einer Grenze pro Quelle und lässt einen ganzen Abschnitt jenseits des Gesamtbudgets fallen; der Prompt rendert einen Abschnitt nur, wenn er existiert, und verbietet, eine Zeile erneut aufzuzählen oder seine Quellen zu nennen. Die Herkunft wird mit dem Porträt persistiert, im selben Schreibvorgang, und darunter gezeichnet; eine Quelle, die sich bewegt hat, öffnet die Berechtigung zur nächsten Konsolidierung erneut.

**Die Tagebücher lassen sich auch gezielt durchsuchen** (ADR-318). Über die Einspeisung hinaus durchsucht ein Lesewerkzeug — in beiden Modi — die Tagebücher der Person nach einem unterwegs gefundenen Thema, über dieselben Vektoren, an der *konfigurierten* Ähnlichkeitsschwelle statt der gelernten, und liefert nur Direktiven, Muster und Facetten: Eine rohe, noch nicht konsolidierte `L0`-Beobachtung kommt nie heraus. Die Präferenz der Person lehnt es ab wie die Einspeisung.
...
**Eine Modellfamilie wird einmal deklariert, und eine kurze Antwort wird ohne Nachdenken angefordert** (ADR-285). Die *Familie* eines Modells ist die Form seiner Reasoning-API — ein Schalter und eine Aufwandsleiter bei DeepSeek, ein Budget bei Gemini 2.5, eine Stufe bei OpenAI — und sie leitet sich aus einer einzigen Deklaration in `reasoning/profiles.py` ab, die Adapter, Validator und der Umweg der strukturierten Ausgabe alle lesen: drei private Präfixtests ließen einmal das umbenannte Spitzenmodell eines Anbieters durch alle drei zugleich, ohne angebotene Leiter und ohne gesendeten Ausschalter. Dieselbe Naht dient den Aufrufern, die zwei Sätze brauchen: bei einem Modell, das standardmäßig nachdenkt, rechnet der Anbieter das Nachdenken *innerhalb* von `max_tokens` ab (OpenAI, Anthropic, Gemini und DeepSeek tun das alle), sodass ein kleines Budget verborgenes Nachdenken und keine Antwort kauft. `short_answer_config` deklariert daher *kein Nachdenken*, wo das aufgelöste Profil es abschalten kann, und behält sein kleines Budget nur dort; sonst bleibt das Budget des Slots, denn eine Grenze, die das Nachdenken einschließt, ist keine Grenze für die Antwort. Die Ausgabegrenze einer Familie wird ebenso aus der Katalogzeile gelesen statt aus einem Literal.
...
Die Konvention ist zu **einem Aufbau für alle Cache-Mechanismen** geworden (ADR-309): Stabiles vor Wechselndem, eine einzige Lesart der Grenze (`core/prompt_layout.split_at_marker`) und kein Cache-Schreibvorgang, den niemand wieder liest. Ein Einzelaufruf – Analysator, Initiative, Extraktion von Erinnerungen, Interessen und Journal – schickt seinen festen Teil als Systemnachricht, wo jeder Adapter ihn markieren kann; Anbieter mit Haltepunkten erhalten ihren am Ende des statischen Präfixes, egal welche Form das System hat (ein listenförmiges System setzte den Haltepunkt auf die Daten des Durchgangs: nichts gelesen, alles zum 1,25-Fachen neu geschrieben); Anbieter mit nur explizitem Cache werden explizit markiert; eine Anfrage, die keine Werkzeugschleife fortsetzen wird, schreibt nur ihre erklärten Haltepunkte; und die Antwort sendet das Gespräch ein einziges Mal, als vollständige Nachrichten. Was jede Modellgeneration akzeptiert – Sampling, erzwungene Werkzeugwahl, sechs Formen des Nachdenkens, implizite Tiefe, an ihr Gespräch gebundene Denkblöcke –, wird an der API gemessen und in einer einzigen Tabelle erklärt (`core/claude_surface.py` für Claude), die Denkregeln, Adapter, strukturierte Ausgabe und Administration lesen. Ein Modell ohne Cache zahlt genau, was es bezahlt hat: Die Optimierung zielt auf den Durchschnitt der Mechanismen, nie auf ein Modell.
...
Bei Google und Microsoft wird die Einwilligung pro Anbieterkonto gebündelt statt für jeden Dienst wiederholt. Eine Freigabe verwaltet die genehmigten Berechtigungen und die Erneuerung; Gmail, Kalender, Drive und die Microsoft-Gegenstücke bleiben getrennte Fähigkeiten, die sich aktivieren oder trennen lassen. Der OAuth-Rücksprung prüft erwartete Identität und Aussteller, State und PKCE, ohne still ein anderes Konto einzusetzen.

**Keine Transaktion bleibt über einen Netzwerkaufruf hinweg offen** (ADR-304). Ein Konnektor-Client, der auf der Sitzung seines Aufrufers gebaut wurde, zwang diese Sitzung – und ihre Transaktion –, offen zu bleiben, während der Anbieter antwortete: in der Produktion gemessen bis zu 863 Sekunden `idle in transaction` beim Durchlauf der Push-Weckrufe, 11 bis 330 Sekunden im Briefing, im Heartbeat oder in der Beziehungsansicht. Ein Client wird daher nie auf `ConnectorService(db)` gebaut – ein Wächter benennt das Konstruktorargument, ohne Ausnahmeliste –; eine einzige Tür öffnet den aktiven Client einer Kategorie in einer kurzen Sitzung und schließt ihn auf jedem Weg; ein Job, der seine Sitzung behält, committet seine Lesevorgänge vor jedem Warten; jede Konnektoroperation des Chat-Durchgangs beendet ihre Transaktion, und der Nutzungsstempel eines API-Schlüssels ist ein eigener atomarer JSONB-Merge unter einer begrenzten Sperrwartezeit. Hintergrundarbeit folgt derselben Regel: Eine Erinnerung wird einzeln beansprucht, committet und dann ohne offene Transaktion zugestellt; der Drive-Push-Feed rückt per Compare-and-Set vor, in Seiten, die nach Anzahl und Zeit begrenzt sind; und nur der Leader-Worker setzt den Telegram-Webhook.

### 13.4. Zwei Wege zur Authentifizierung

Nicht jeder Konnektor verlangt ein Konto. Ein **OAuth-Konnektor** hält die persönlichen Zugangsdaten der Nutzerin: Gmail, Kalender, Kontakte, Drive. Ein **Dienst mit Plattformschlüssel** hält keine nutzerbezogenen Daten — es gibt nichts einzuschalten, und der Schlüssel gehört der Installation: Routen, Orte, Wetter, Umwelt. `ConnectorType.uses_global_api_key` trägt diese Unterscheidung, und die Werkzeugbasis wählt den Zugangsdatenpfad anhand des **aufgelösten** Typs. Eine funktionale Kategorie kann daher beides mischen: Wetter akzeptiert einen Anbieter mit persönlichem Schlüssel ebenso wie einen Plattformdienst, ohne dass der Aufrufer weiß, wer geantwortet hat.
...
Die Unterscheidung entscheidet auch, was überhaupt zu einem **Konto** gehört. Fünf Konnektoren verlangen nichts von der Person — Wikipedia, das Seiten-Browsing, Orte, Wetter, Umwelt — und gehören daher der Instanz: keine Zeile pro Konto, nichts in den Einstellungen, und ein einziges Prädikat (`connectors/keyless.py`) entscheidet, ob sie allen dienen — ein vom Administrator abgeschalteter Typ, ein fehlender Plattformschlüssel oder deaktiviertes Seiten-Browsing hält ihn für alle zurück. In der Wetterkategorie gewinnt der Anbieter der Person, der von Google ist der Standard; ein Test hält die fünf aus der Aktivierungsliste der Einstellungen heraus (ADR-307).
...
**Eine Regel je Modus, gleich auf welcher Leitung (ADR-301).** Das Telefon übertrug das Gespräch am *Ende*; das Live des Browsers reichte jede Bitte *währenddessen* an den Chat — zwei Wahrheiten für ein Wort. Eine Sprachsitzung hat nun einen Modus, und nur ihm folgt die Regel: in **Live** (Standard des Telefons, gewählt unter Einstellungen › Telefonie · Meine Identität) hält die Stimme weder Kontext noch Werkzeug und übergibt jede Bitte an LIA durch eine einzige Funktion — dieselbe, die das Live des Browsers seinem Anbieter erklärt —, die der Server in einen Chat-Zug der Person verwandelt, während sie spricht: LIAs Fragen (eine Frage IST das Ergebnis, die nächste Bitte setzt den fragenden Zug fort), die Bestätigungen, die Register, die Kontingente und die Spur in der Unterhaltung kommen mit; in **Live direkt** liest die Stimme für die Person, handelt in nichts, und ihre Worte werden am Ende als Nachricht von ihr übertragen — auf der Leitung wie im Browser, dessen direkte Sitzung nicht mehr „nichts aufgezeichnet“ heißt. Ein Abschluss führt die Bücher beider Leitungen: die rein gesprochenen Austausche archiviert, die Karte mit genauen Zahlen, die Entscheidung, das Lernen — oder die Zusammenfassung, der übertragene Zug und die beim Abschluss neu geschriebene Karte mit neu gelesenen Kosten. Live wird nur dort angeboten, wo der Telefonanbieter die Instanz zurückrufen kann; sonst sagen es die Einstellungen und der Anruf läuft direkt. Alles wurde auf der echten Engine des Anbieters gemessen, ohne Telefon: die Stimme kündigt den Aufruf an und führt das Gespräch weiter, eine späte Antwort ginge nach dem Zeitlimit des Anbieters verloren — die Brücke antwortet also davor —, und ein Austausch mit einer Delegation gehört ganz dem Graphen.
...
**Phase 1 — Entscheidung** (der Slot `heartbeat_decision`, ein sparsames Modell aus dem Admin-Katalog):
...
### 16.3. Geplante Aktionen und Erinnerungen

APScheduler mit Redis Leader Election (SETNX, TTL 120s, Recheck 5s) für die Schleife, `FOR UPDATE SKIP LOCKED` für die Isolation, automatisch genehmigte Pläne, Abschaltung nach 5 aufeinanderfolgenden Fehlschlägen.

**Eine Wiederholung ist ein Produkt**: welche Kalendertage × welche Tageszeiten. Eine einzige Maschine (`src/core/recurrence`) antwortet für Routinen wie für Erinnerungen — `dateutil.rrule` zählt die Tage auf, jeder Moment wird mit `fold=0` lokalisiert, und die Reihe wird geordnet und nach Zeitpunkt entdoppelt, denn bei der Umstellung auf die Winterzeit fallen zwei Wanduhrzeiten auf denselben Zeitpunkt. Die Tage werden nie einem Cron überlassen: ein `IntervalTrigger` überspringt den ganzen Tag, wenn sich der Versatz um Mitternacht ändert — 142 Ausführungen pro Jahr in 73 Zeitzonen, ohne Spur in den Protokollen.

**Die Erinnerung, die nur einmal klingelt, ist keine Ausnahme, sondern der Regelfall**: gefragt, was als Nächstes zu setzen ist, antwortet `rearm_after` bei einer verbrauchten Einzelgelegenheit `None`, und `None` bedeutet „löschen“. Nirgends fragt ein Zweig „wiederholt sich das?“. Eine Folge für die Typisierung: `trigger_at` einer Erinnerung bleibt NOT NULL — eine Erinnerung ohne Zukunft wird gelöscht —, während `next_trigger_at` einer Routine nullable ist, denn dort ist die Konfiguration das Wertvolle.

Die Obergrenzen werden **vom Verbraucher injiziert**, nie im Modell gelesen: 12 Auslösungen pro Tag für eine Routine, 48 für eine Erinnerung. Jedes Gesprächswerkzeug veröffentlicht die Grenzen, die es durchsetzt (ADR-184), sodass eine Zahl außerhalb der Grenzen vom Clamp des Planers repariert und nicht als Fehler gemeldet wird.

Jeder Tick endet mit einer Zeile in einer **Ausführungshistorie**, beim Ergebnis in der Markierungstransaktion geschrieben — fünf Ausgänge, einer je Executor-Ausgang, in einem Savepoint und nie blockierend. Die laufende Woche wird serverseitig mit derselben Maschine berechnet: eine Zelle nimmt die letzte Ausführung, deren bedienter Zeitpunkt dem des Slots **gleicht**, nie ein Toleranzfenster — so setzt eine Planänderung das Raster konstruktionsbedingt zurück, und der Browser liest den Plan nie erneut, er zeichnet nur.

Der Satz der Erinnerung folgt derselben Regel wie jede kurze Antwort: Er wird **ohne Nachdenken** angefordert, wo das Modell anhalten kann, mit einem kleinen Antwortbudget — und eine Antwort, die leer zurückkommt oder die der Anbieter als an seinem Budget abgeschnitten meldet, ist eine Verweigerung und keine Nachricht. Der geschriebene Ersatzsatz geht an ihrer Stelle hinaus, die entstandene Ausgabe bleibt verbucht, und die Warnung nennt das Modell und seine Nachdenk-Tokens. Eine Benachrichtigung ist die einzige Handlung aus LIAs eigener Initiative, die eine Person wirklich erlebt; sie kommt nie als Glocke und sonst nichts an.
...
### 16.5. Antizipierte Momente und minutengenau bediente Wachen (ADR-281)
...
Dasselbe Aufwecken bedient die **Wachen**: Eine Bedingungsroutine „Mail von diesem Absender“ wartete bis zu zwei Stunden auf den Durchlauf des Executors; das Aufwecken, das das Gmail-Delta ohnehin hält, bedient sie vor dem Verdikt des Vorfilters — der Vorfilter antwortet „ist das ein Aufwecken wert“, eine Wache antwortet „ist das, worauf ich warte“. Es führt die Routine nie aus: Es zieht ihre Fälligkeit vor, der Executor bleibt der einzige Richter, und es scharft jenseits der veröffentlichten TTL des Such-Caches, weil ein Cache, der vor Ankunft der Mail gefüllt wurde, „nicht erfüllt“ antworten würde und dieses Verdikt die Scharfschaltung verbraucht. Eine beendete Routine **schließt** (`is_enabled = false`, `status = completed`) — ihr Ende hat eine einzige Autorität, das bereits gespeicherte `SeriesEnd`. Und der „Beobachten“-Chip des Briefings schreibt diese Routine, mit dem Absender als Schlüssel und nie dem Betreff, nachdem er gelesen hat, was das Konto bereits hält.
...
**Eine föderierte Identität verleiht kein Recht.** Das Verknüpfen einer Google-Identität ändert nie den Status eines Kontos: Eine Registrierung, die auf Freigabe wartet, bleibt wartend, ein gesperrtes Konto bleibt gesperrt. Die angegebene Adresse wird nur geglaubt, wenn Google für sie bürgt; war sie nie bewiesen, beweist die Verknüpfung sie, widerruft Passwort und Sitzungen der anmeldenden Person und benachrichtigt die Administration wie bei einer E-Mail-Bestätigung; ein gelöschtes Konto wird nie wiederbelebt, und eine Ablehnung trägt einen begrenzten Grund. „Wer ruft an?“ hat eine einzige Antwort, `resolve_client_ip`, die Administrationsaudits, Sitzungsadresse und Ratenbegrenzungen lesen – ein AST-Wächter lehnt jede andere Lesart des Headers ab, den der Besucher schreibt, und der Webserver gibt die Adresse weiter, die Cloudflare bestätigt. Schließlich ist die Logging-Konfiguration der erste Import der Anwendung, und ein Werkzeug bindet seinen Logger in dem Moment, in dem es loggt: Ein früher gebundener Logger behielt die Standardwerte von structlog, ohne Level- und ohne Datenschutzfilter.
...
| Technologie | Rolle |
|-------------|------|
| Prometheus | 589 benutzerdefinierte Metriken (RED Pattern) |
...
Versionierte `.txt`-Dateien in `src/domains/agents/prompts/v1/` (eine pro Prompt, das `PromptName`-Literal durch einen Test synchron gehalten), geladen über `load_prompt()` mit einem LRU-Cache, der größer als der gesamte Korpus ist. Versionen konfigurierbar über Umgebungsvariablen.

**Ein Prompt sagt, was der Code erzwingt, und sonst nichts** (ADR-284). Jeder `{placeholder}` einer Datei hat einen Erzeuger in dem Modul, das sie lädt, und eine Datei, die `{{` enthält, wird mit `.format()` gerendert — ein AST-Wächter liest beides pro Datei. Eine Anweisung pro Kontext lebt in der Datei und wird nur ausgegeben, wenn der Inhalt existiert (`response_context_sections.txt`), sodass ein nackter Zug keine leere Hülle und keine Anweisung zu einem fehlenden Dokument trägt. Die Grenze, die ein Prompt veröffentlicht, ist die Grenze, die das Werkzeug erzwingt, aus einer Konstante; eine Zahl kommt aus einer Einstellung und eine Tatsache aus dem Code, der sie anwendet; eine Fähigkeit wird nur versprochen, wenn der Zug sie ausführen kann. Prosa lebt nie in einer `.py`, ein Ersatztext auch nicht: einzeilige Gerüste liegen in `Schlüssel|Vorlage`-Zeilendateien, die ein einziger Parser liest (`parse_prompt_sections`), und eine fehlende Datei ist ein kaputtes Deployment, das laut scheitert, statt einer stillen zweiten Formulierung.
...
- **Lese-Invariante & reifes Eingabefeld** — eine streamende Antwort reißt einen nach oben gescrollten Leser nie mehr weg: Die Follow-Entscheidung misst die Geometrie live im Entscheidungsmoment (wachstumskompensiert), ein expliziter Sende-Tick ersetzt Daten-Diff-Heuristiken (zwei davon feuerten am echten Engine falsch), und ein schwebender Button mit Badge der Offscreen-Antworten bringt den Leser zurück. Das Eingabefeld trägt einen persistenten Entwurf pro Nutzer (debounced, beim Logout gelöscht), ein ↑/↓-Blättern über die letzten 10 Sendungen, `/`-Slash-Befehle (WAI-ARIA-Combobox auf der nativen Textarea, akzent-insensitives lokalisiertes Filtern) und eine In-Flow-Aktionszeile unter jeder Antwort (Kopieren, Behalten, Teilen, Herunterladen, Feedback, Ausführungs-Trace — Teilen öffnet das Teilen-Menü des Systems und dann die Verbindungen, deren Auswahl das Eingabefeld vorausfüllt, statt zu senden; Herunterladen schreibt eine datierte Markdown-Datei).
...
317 ADRs im MADR-Format dokumentieren die wichtigsten Architekturentscheidungen. Einige repräsentative Beispiele:
...
Die Obergrenze hält nur, wenn das Buch vollständig ist, und die Regel gilt nicht nur dem Modell: **Jeder Euro, den die Plattform für eine Person zahlt, wird für diese Person verfolgt, angezeigt, zugeordnet und gezählt – welche Modalität oder welcher Pfad auch immer** (ADR-272, Ergänzung). Google Maps Platform hatte Löcher in jede Richtung. Die beiden Persistenztüren des Trackers entschieden allein auf den Datensätzen des Modells, sodass eine Places- oder Routes-Abfrage während eines Telefonats oder einer direkten Live-Sitzung – kein Modellaufruf – keines der vier Bücher erreichte; `pending_families()` ist jetzt das eine Prädikat, und eine Wache verweigert einen Datensatzbehälter, den es nicht liest. Der Google-Aufrufzähler tat ohne Umgebungs-Tracker nichts, während der Heartbeat, das Briefing (acht berechnete Wetteraufrufe je Aktualisierung), die Besprechungen und die Bildproxys zahlten: Er schlägt jetzt geschlossen in `google_api_calls_unaccounted_total` fehl, von einem Alarm auf null gehalten. Jede Oberfläche außerhalb eines Zuges öffnet ihre eigene Buchführung um den ganzen Akt über eine einzige Tür, die Straße jedes Moduls ist in einem Register – dem Geschwister von `LLM_SPEND_ROADS` – deklariert und über den Importgraphen geprüft, ein berechnetes Bild wird dort gezählt, wo Google es berechnet – am Proxy, auf dem 200, über eine über (Lauf-Id, Konto) signierte Lauf-Id an seinen Zug gebunden –, und eine einzige Spalte, `billed_cost_eur`, ersetzt vier handgeschriebene Summen mit drei verschiedenen Teilmengen.
...
Der Konsum ist bewusst zurückhaltend: Umgebungskontext für Antworten und Briefing, höchstens ein Hinweis auf eine verpasste Routine pro Tag, nach zwei ignorierten Hinweisen stummgeschaltet, bis die Routine wieder vorkommt, und ein Benachrichtigungs-Timing, das gelernte Fenster bevorzugt, ohne je die vom Nutzer gesetzten Grenzen zu erweitern — eine Anti-Aushungerungs-Regel garantiert, dass eine leere Schnittmenge nichts ändert. Jeder angewandte Schwellenwert wird im Panel publiziert: Eine angezeigte Gewohnheit ist bewiesen, oder sie existiert nicht.

Die zweite Hälfte — die **wiederkehrenden Anfragen** — liest die Entscheidung des Routers in geschlossenem Vokabular: genau den Wert, aus dem das Produkt-Dashboard ableitet, sodass eine als Aktion gezählte Runde eine Runde ist, die das Ledger aufzeichnet. Die Signatur bleibt allein die Primärdomäne. Vier Formen sind benannt: täglich, Werktage, wöchentlich und **intermittierend** — „mehrmals pro Woche gegen 9 Uhr“ — entschieden durch die Dichte verschiedener Tage über die zulässige Spanne, nie durch die Sperre: Ein 3×/Woche wird nicht mehr als „jeden Tag“ versprochen. Die Schwellen werden auf einem dauerhaften Prüfstand gemessen (`task habits:calibration:measure`, 300 Versuche pro Zelle, dichte und moderate Populationen jeweils mit einer verstreuten Kontrolle bei gleichem Volumen): Ein Tagesrhythmus wird an T+14 erkannt, ein 3×/Woche zwischen T+21 und T+35, ein zu 90 % eingehaltenes wöchentliches Ritual an T+42 — über fünf Slots, damit eine verpasste Woche die Sperre nicht mehr tötet — mit 0-0,3 % falschen Sperren bei strukturloser Nutzung; die vom Prüfstand abgelehnten Lockerungen bleiben in seinen Tabellen. Und Stille ist ein Alarm: `RecurrenceLedgerSilent` vergleicht handlungsfähige menschliche Runden mit gelandeten Schreibvorgängen — ein Ledger, das nichts schreibt, während man mit ihm spricht, ist kein diskretes Ledger, sondern ein kaputtes.

**Was gelernt wurde, wird über EIN Prädikat konsumiert — und per Ausführung auditiert.** Das Gewohnheiten-Panel bearbeitet Spiegelzeilen — ein Fenster lässt sich pausieren oder sperren —, während der Heartbeat-Block, das Tick-Scoring und der Umgebungsblock das Profil lasen: Ein gesperrtes Fenster steuerte die Ticks weiter. `habits/consumption.py::load_consumable_profile` bedient jetzt alle drei, behält nur Fenster mit aktiver Zeile, und der Fensterschlüssel hat einen einzigen Erzeuger. Eine gelernte Gewohnheit hat einen Lebenszyklus, den der nächtliche Job besitzt (`recurrence_sync`: befördern, auffrischen, behalten, zurückstufen — nur aktive Zeilen, nie im Zweifel, und auch für ein stilles Konto, das genau der Fall der Rückstufung ist), nie der Chat-Vorschlag. Das Lern-Gate (`learning_gate.read_learning_gate`, fail-closed) schließt jede Tür — den Ledger-Schreibvorgang, den Initiative-Detektor, das Seeding — und folgt `HABITS_ENABLED` plus dem Fähigkeitsschalter `habits`, gelesen bei der Handlung: `habits` und `heartbeat` haben die routergeschützte Familie von ADR-280 zugunsten der service-durchgesetzten verlassen, weil die Routen das Archiv sind. Die Entscheidung **deklariert** ihr Angebot (`HeartbeatDecision.habit_offered`, Regel 24), statt dass das Angebotsbudget an einem vom Modell gewählten Quellen-Label hängt, und das Angebot hat einen Gegenstand: Der `immediate_intent` des Analysators ist ein begrenztes Histogramm in der Ledger-Nutzlast — nie Teil des Schlüssels, was jedes Ledger fragmentieren würde —, sodass `usual_intent` die Zeile, den Prompt (« usually asks for a 'search' on 'email' ») und das Panel über das geschlossene Vokabular `IMMEDIATE_INTENTS` erreicht. Das Tick-Scoring lebt in `habits/tick_scoring.py` mit einer `TickSurface` je Durchlauf, an dessen Eligibility-Checker gepinnt, und der Interessen-Durchlauf wendet beide Gates an — die Meeting-Sperre und den gelernten Rhythmus; ein Aufwecken umgeht den Rhythmus, tritt aber vor einem Meeting beiseite, wobei das Kalenderurteil `next_start` trägt, damit der Rhythmus nie einen Termin verdeckt. Die Präsenz ist standardmäßig an, und „zuletzt gesehen“ zählt einen Präsenz-Ping. Und das Register der Nachschlagevorgänge wird um den ganzen Durchlauf herum gespeist, Eligibility eingeschlossen — der Aggregator las in `select_target`, während der Kollektor in `generate_content` öffnete: 824 Durchläufe, null Zeilen.
...
Der öffentliche Demonstrator wendet diese Grenzen auf sich selbst an und macht sie lesbar. Jede Fähigkeit des Registers ist in seiner Umgebungsvorlage schriftlich entschieden — ein- oder ausgeschaltet, mit Begründung — und zwei Wachen verweigern eine dem Code-Standard überlassene Obergrenze oder eine Abweichung zwischen Entwicklungs- und Produktionsvorlage; die Zählung der erreichbaren Routen hängt die Router unter genau diesen Obergrenzen ein, sodass eine abgeschaltete Familie nicht erscheint und eine eingeschaltete Route für Route geschrieben steht. Schließlich veröffentlicht jede Instanz in ihrer öffentlichen Konfiguration den wirksamen Zustand aller Fähigkeiten — Obergrenze und Schalter verknüpft — und die Instanz, die einen Demonstrator ankündigt, liest diese Konfiguration serverseitig und zeigt sie neben dem Link: die Liste dessen, was ein Besucher vorfinden wird und was nicht, folgt der Umgebungsdatei der Demonstration, niemand pflegt sie von Hand.

**Eine Ankündigung sagt, an wen sie gerichtet war** (ADR-312). Das Publikum einer Ankündigung der Administration wird aufgelöst, bevor ihre Zeile geschrieben wird, und ihre Empfänger werden mit ihr gespeichert: Eine gezielte Ankündigung wird nur ihnen ausgeliefert — auch bei ihrer nächsten Anmeldung —, und eine Auswahl, die kein aktives Konto trifft, wird abgelehnt, statt an niemanden oder an alle zu gehen. Der Verlauf liefert eine Seite und ihre exakte Gesamtzahl, eine begrenzte Stichprobe der Empfänger mit ihrer exakten Anzahl, den Ablaufzeitpunkt und die gewählte Frist — in vier Abfragen, wie groß die Seite auch ist.
...
## 30. Sichtbare Arbeit, gesteuertes Lernen

Die Aktivitätsseite ist ein **reines Read-Model**: Parallele Fetcher (eine Session pro Quelle, da `AsyncSession` nicht nebenläufig ist) aggregieren die vorhandenen Audit-Tabellen, die Summen sind exakte `COUNT(*)` über das ganze Fenster, Obergrenzen sind deklariert, und eine ausgefallene Quelle wird aufgeführt statt stillschweigend ergänzt — ehrliches Zählen (ADR-185), durchgängig angewandt. Das Gedächtnis führt eine **Supersession-Spur**: Eine automatische Korrektur erzeugt einen Nachfolger und archiviert die alte Tatsache (`superseded_by_id`), jeder Lesevorgang filtert die aktive Menge über ein zentrales Prädikat, und die Spur wird nach der Aufbewahrungsfrist bereinigt; die manuelle Bearbeitung behält ihre Überschreibungshoheit. Gelernte Regeln sind eine **eigenständige Gedächtniskategorie**, an den Anfang des Prompts eingefügt, unter denselben Schutzmechanismen (Anheften, Aufbewahrung, DSGVO). Die Sprachprosodie ist eine **begrenzte Modulation** (Totband, harte Grenzen, Flag) der administrierten Einstellungen — nie ein Ersatz. Und die Autonomie bleibt gedeckelt: Das ReAct-Iterationsbudget passt sich der Domänenbreite der Anfrage an, ohne je die konfigurierte Obergrenze zu überschreiten, und eine unbekannte Komplexität erhält die volle Obergrenze — gespart wird nur beim nachweislich Einfachen.
...
All diese Observability zu emittieren und nichts davon zu lesen heißt: überall instrumentiert und sich selbst gegenüber blind. Das Selbstdiagnose-Subsystem (ADR-247) schließt die Schleife mit einer Designregel pro Säule.
...
**Alltägliche Rechnungen warten nicht auf ein Skript: Sie sind Werkzeuge** (ADR-318). In beiden Modi wird eine Rechnung in einen Baum zerlegt und in exakter Dezimalarithmetik ausgewertet — nie ausgeführt —, die Unendlichkeiten, die die Dezimalbibliothek ohne Fehler zurückgibt (`0 ** -1`, `ln(0)`), werden abgelehnt, und ein Ergebnis, das gerundet werden musste, sagt es. Eine Dauer wird zwischen zwei absoluten Zeitpunkten gezählt, weil zwei Zeitstempel derselben Zone auf ihrer Wanduhrzeit subtrahiert werden und für die 25 Stunden einer Zeitumstellung 24 ergäben. Eine Umrechnung liest den Referenzkurs der EZB und nennt sein Datum. Jede durchgesetzte Grenze ist eine Einstellung, formuliert in dem Wortlaut, den das Manifest des Planers und das ReAct-Schema gleichermaßen lesen. Das folgende flüchtige Skript bleibt für das, was kein Werkzeug abdeckt.
...
**Die Assistentin liest ihre eigenen Register** (ADR-318). Auf „was hast du diese Woche für mich getan?“ antwortet ein Lesewerkzeug aus den beiden Registern, mit exakten Summen und demselben Urheberfilter wie die Reiter — was die Person verlangt hat, was LIA von sich aus unternommen hat — statt aus der Erinnerung an ein Gespräch. Ein Register dient so zwei Lesern ohne zwei Implementierungen: dem Bildschirm, der es zeichnet, und der Antwort, die es zusammenfasst.
...
## 41. Das Ticket-Board: eine Zeile, zwei Seiten und eine Assistentin, die fragt

**Eine Arbeitseinheit braucht einen Lebenszyklus, einen Inhaber und ein Ergebnis** — und keine der vorhandenen Flächen hatte alle drei. Die Aufgabenliste eines Anbieters hat keinen Lebenszyklus; eine Erinnerung ist ein Anstoß in einem Augenblick, und nach dem Klingeln bleibt nichts; eine Routine ist eine wiederholte Anweisung, die nie fertig wird. Ein Ticket trägt Titel, Beschreibung, Priorität, Daten, Unter-Tickets, Kommentare und eine Historie durch sieben Spalten, und sein Inhaber kann der Eigentümer sein, eine verbundene Person oder die Assistentin selbst.

**Eine Zeile, von beiden Seiten gelesen.** Ein Board ist „ich besitze es ODER ich halte es“, einmal als Repository-Prädikat geschrieben, das jede Lesung wiederverwendet — zwei Abfragen wären zwei Autoritäten darüber, was ein geteiltes Ticket sagt. Ein Ticket, das die aufrufende Seite nicht sieht, antwortet genau wie eines, das nicht existiert: Keine Kennung lässt sich abtasten. Die Seite und ihre Zählungen je Spalte stammen aus derselben gefilterten Anweisung: Eine Überschrift, die sieben nennt, über einer Spalte, die drei zeigt, wäre schlimmer als gar keine Überschrift.

**Was die Datenbank tragen kann und was nicht.** Die Inhaberspalte steht auf `SET NULL`, weil vier Wege eine `users`-Zeile hart löschen und nur einer davon die Kontobereinigung ausführt — eine Kaskade würde das Ticket des Eigentümers zerstören, wenn dessen Gegenüber geht. Der Invariant „eine verbundene Person hält ein Ticket nur unter einer angenommenen Verbindung“ ist bewusst KEINE Einschränkung: Ein `CHECK` über zwei Spalten, die eine Fremdschlüsselaktion berühren kann, ist in jeder Kaskadenrichtung verletzbar, keine Reihenfolge ist garantiert, und PostgreSQL kennt kein aufschiebbares `CHECK`. Er lebt daher im Service, der die Verbindung bei jedem Schreiben erneut prüft — was ohnehin die richtige Regel ist: Eine Freigabe wird zur Ausführungszeit geprüft und nie aus einer gespeicherten Zeile gelesen.

**Ein Durchlauf beansprucht ein Ticket und schließt aus einem Ergebnis ab.** Jede Minute gibt ein Job mit Jitter zuerst die Ansprüche frei, die ein toter Worker noch hält, und beansprucht dann ein einziges Ticket unter `FOR UPDATE SKIP LOCKED` mit einem bedingten `UPDATE`, das festgeschrieben wird, bevor irgendeine Arbeit beginnt — wer fünf genommen hätte, ließe fünf zurück. Abgeschlossen wird aus einem ausdrücklichen Ergebnis, nie aus dem Ausbleiben einer Ausnahme, und der Durchlauf benennt, was er getan hat: Drei Ablehnungen können ein beanspruchtes Ticket stoppen, und keine davon ist ein Fehlschlag. Ein inaktives Konto ist endgültig; eine Kontingentgrenze und ein laufendes Gespräch geben den Anspruch zurück, erstatten den Durchlauf dem Budget des Tickets und werden als *übersprungen* protokolliert.

**Außerhalb eines Zuges fragt sie, statt abzulehnen.** Das Ausführungstor lehnte außerhalb eines Gesprächs sowohl eine Bestätigung als auch einen Entwurf ab, weil es niemanden zu fragen gab. Ein Ticket-Durchlauf hat jemanden: Das Tor lässt nun eine Herkunft, die einen Entwurf tragen kann, genau die Karte bauen, die der Chat gezeigt hätte; das Ticket landet in „zu bestätigen“ und hält sie, und der nächste Kommentar des Eigentümers klassifiziert die Antwort **ganz ohne Modellaufruf** — ein gefaltetes Lexikon in sechs Sprachen, in dem ein bloßes Ja freigibt, ein bloßes Nein schließt und alles andere zur Anweisung wird. Die Wiederholung läuft dann im Graphen unter einer Prüfsummensperre: Der Durchlauf veröffentlicht die Identität dessen, was er gezeigt hat, und der vom Werkzeug neu gebaute Entwurf passiert nur unter genau dieser Identität und unter keiner weiteren.

**Was ein Durchlauf schreibt und wohin.** Ein Durchlauf archiviert sein Briefing und seine Frage als Zeilen DES DURCHLAUFS — aus dem Chat herausgehalten, vom Entscheidungsregister referenziert, von der Aufbewahrung nach dem Abschluss des Tickets entfernt —, während die Benachrichtigung, die die Person lesen soll, eine gewöhnliche Nachricht ist. Der Unterschied ist eine ABGELEITETE Spalte statt eines Stempels in den Metadaten, berechnet an der einen Stelle, die die Zeile baut; und jede archivierte Zug-Metadatenstruktur wird von einem benannten Erbauer gebaut, unter einer Wache, die ein an der Aufrufstelle geschriebenes Wörterbuch ablehnt — genau so hatte einst eine Löschvorschau den Chat erreicht.

## 42. Die Anatomie eines Prozesses: was er lädt, ist deklariert, gemessen und begrenzt

Ein API-Server besteht aus mehreren Prozessen: einem Supervisor, der keine Anfrage bedient, und Workern, die alles bedienen. Jeder hält aus anderen Gründen Speicher, und „der Container belegt fünf Gigabyte“ sagt nichts Nützliches, solange unklar ist, welcher Prozess was hält. LIA wendet hier eine einzige Regel an: **Was ein Prozess lädt, ist deklariert, pro Prozess gemessen und dort begrenzt, wo es sich vervielfacht.** Jeder Posten wurde auf dem Zielhost in einem Wegwerfprozess gemessen, bevor er angefasst wurde — und drei Messungen widersprachen der ersten Lesart, was genau der Grund ist, zu messen.

Der Supervisor lädt die Anwendung nicht. Die uvicorn-Kommandozeile importiert die Anwendung im Elternprozess, um einen frühen Importfehler zu erzeugen, und verwirft sie dann — ein Elternprozess, der nur seine Worker beobachtet, hielt so mehrere hundert Megabyte für die gesamte Lebensdauer des Containers. `python -m src.serve` fährt dieselbe Kette `Config → Server → Multiprocess` mit denselben Optionen, ohne diesen Import; eine AST-Wache verbietet dem Modul jeden Import aus `src`, und der frühe Fehler wird vom Health-Check gemeldet, den das Deployment ohnehin liest. Im selben Geist wird eine schwere Bibliothek — PDF-Rendering, Tabellenrechnung, Audio-Engines — dort importiert, wo sie gebraucht wird, und nie beim Start: eine Liste deklariert die gemessenen Kosten jeder einzelnen, und eine Wache verweigert jeden Import auf Modulebene unter `src/`, auch einen in einem `try` auf Modulebene versteckten.

Was ein Singleton lädt, multipliziert sich mit der Zahl der Worker. Die Sprachtranskription hielt pro jemals angefragter Sprache ein residentes Modell, in einem Singleton pro Prozess; der Cache ist nun begrenzt (`VOICE_STT_MAX_RECOGNIZERS`, standardmäßig eins), das am längsten Unbenutzte fliegt heraus, beim Erzeugen wird nichts geladen, eine Verdrängung gibt Speicher an das System zurück — gemessen — und eine laufende Dekodierung behält ihre eigene Referenz. Und jeder Worker veröffentlicht, was er hält: `lia_worker_memory_bytes{kind}` liest jeder Worker aus seinem eigenen `/proc/self/status`, eine Serie pro lebendem Worker, die mit ihm verschwindet, auf zwei Panels gezeichnet und vom Alarm `ApiWorkerMemoryHigh` überwacht, der den Prozess benennt. Das antwortet auf eine strukturelle Grenze: cAdvisor sieht den Container, und der Multiprozess-Modus von prometheus_client exportiert keine Prozessmetrik.

Das Verbindungsbudget hat einen Boden, nicht nur eine Decke. Audit F004 begrenzte die Spitze — was die Pools öffnen dürfen; jede persistente Verbindung ist zugleich ein untätiges Postgres-Backend mit sieben Megabyte, neben den `shared_buffers`. `ConnectionBudget.persistent_total` benennt diesen Boden, und eine Wache liest die Compose-Datei (Speicherlimit, `shared_buffers`) zusammen mit den beiden ausgelieferten `.env`-Profilen — dem vollständigen und dem minimalen des Selbsthosters, fehlende Schlüssel lösen wie beim Start zu den Code-Standards auf: der Boden muss unter der Hälfte des Limits bleiben, damit die Datenbank die andere Hälfte für ihren Cache behält. Ausgeliefertes Profil: fünf persistente Verbindungen und fünfzehn im Überlauf pro Worker, ein Limit von zwei Gibibyte. Was das Los offen lässt, steht geschrieben: das Wachstum eines Workers ohne Transkription über mehrere Tage — jetzt messbar — und die Aufbewahrung der LangGraph-Checkpoints.

## 43. Der Live-Modus: zwei Intelligenzen, eine Naht – und eine Regel pro Modus

Eine Sitzung von Sprache zu Sprache läuft auf einem Live-Modell, das die Person mit **ihrem eigenen Schlüssel** verbindet – eine eigene Konnektor-Kategorie, `live`, additiv: Mehrere Schlüssel können aktiv sein, die Sitzungen öffnen sich auf dem gewählten, und ein Modell zu wählen *heißt*, seinen Anbieter zu wählen. Nichts, was der Anbieter abrechnet, wird von der Plattform gezählt, gespeichert oder gezeigt. Die API prägt einen einmal nutzbaren Berechtigungsnachweis und rendert das Setup; der Browser spielt es wörtlich ab und öffnet die Verbindung selbst, sodass **der Ton nie die API passiert**. Vier Tatsachen wurden am echten Anbieter gemessen, bevor das Design eingefroren wurde, und jede wurde zur Regel: Die Einschränkung eines Berechtigungsnachweises sperrt die Systemanweisung nicht; ein verbrauchter Einmal-Nachweis kann sich nicht neu verbinden (jede Neuverbindung prägt einen frischen für denselben Datensatz); der Anbieter akzeptiert stillschweigend einen unbekannten Stimmennamen (die Stimme wird gegen eine mitgelieferte, datierte Liste geprüft); ein roher Browser-WebSocket wird nur von einer einzigen Methode angenommen.
...
Eine **direkte Sitzung** ist die Telefonleitung im Browser: Die Stimme hält LIAs Lesewerkzeuge selbst – die abgeleitete Menge des Telefons abzüglich der eigenen Schalter der Person – hinter einer einzigen Werkzeugtür unter einem Budget pro Sitzung, und delegiert nie. Die Prämisse des Eigentümers wurde an einer echten Sitzung gemessen: Fünfundfünfzig Deklarationen wiegen neunzig Prozent des Setups, und der erste Zug kostet das Neunfache des delegierten – eine direkte Sitzung kauft Latenz, nie Ersparnis. Die Nutzungsberichte des Anbieters werden im Browser gefaltet und mit dem Tarif bepreist, den der Start veröffentlicht, unter dem letzten Untertitel gezeichnet; Kosten, die einen nicht hinterlegten Tarif bräuchten, sind nicht verfügbar, nie teilweise; ein Live-Modell wird nur angeboten, wenn die Tariftabelle es deklariert. Zwei gemessene Fallen leben im Player: Geminis affektiver Dialog schließt den Socket beim ersten, was das Modell beantworten muss, und Chrome 152 wiederholt den ersten Renderblock eines geplanten Chunks – der PCM-Player ist ein einziges AudioWorklet, das eine Warteschlange liest, nie ein Quellknoten pro Chunk.

Dieselben zwei Modi erreichten dann das Telefon (ADR-301). Der Anruf des Inhabers gab das Gespräch an seinem *Ende* weiter, während der Browser jede Bitte *währenddessen* delegierte – zwei Wahrheiten für ein Wort. Eine Sprachsitzung hat jetzt einen **Modus** und einen **Träger**, und die Regel folgt allein dem Modus: `delegated` („Live“), jede Bitte ein Chat-Zug der Person, während sie gesagt wird; `direct` („Live direkt“), die Stimme liest, handelt nicht, und die Worte werden am Ende als eigener Zug der Person weitergegeben. **Ein Abschluss** bedient beide Leitungen, aufgerufen von der End-Route des Browsers und dem Post-Call-Webhook des Telefons; was die Leitungen teilten, zog aus beiden nach `domains/voice_sessions/`, und der Live-Modus des Telefons ist die Delegation des Browsers serverseitig – ein asynchrones Anbieterwerkzeug mit dem Namen und Schema des Browsers, eine Brücke, die die TypeScript-Brücke Regel für Regel spiegelt (die neueste Bitte gewinnt über eine Redis-Markierung, die die laufende Brücke abfragt; eine Frage, die LIA stellte, ist das Ergebnis, und die nächste Bitte setzt den Lauf fort, der sie stellte). Am echten Motor des Anbieters ohne Telefon gemessen: Die Stimme kündigt den Aufruf an und spricht weiter, eine späte Antwort jenseits der Anbietergrenze geht verloren, also antwortet die Brücke vorher. Der effektive Modus wird abgeleitet und veröffentlicht – Live gibt es nur dort, wo der Rückruf-Host öffentlich ist – und ein Runner hält über den Zug hinweg keine Datenbanksitzung: 8,9 s `idle in transaction` vorher, danach keine.
...
Die 317 ADRs dokumentieren nicht nur die getroffenen Entscheidungen, sondern auch die verworfenen Alternativen und die akzeptierten Kompromisse. Die ~32.725 Tests in 1.974 Dateien, die vollständige CI/CD-Pipeline und der strikte MyPy-Modus sind keine Eitelkeitsmetriken — sie sind die Mechanismen, die es ermöglichen, ein System dieser Komplexität ohne Regressionen weiterzuentwickeln.
...
*Dokument verfasst auf Grundlage der Analyse des Quellcodes (`apps/api/src/`, `apps/web/src/`), der technischen Dokumentation (680+ Dokumente), der 317 ADRs und des Changelogs (v1.0 bis v1.47.4). Alle genannten Metriken, Versionen und Patterns sind in der Codebase verifizierbar.*
