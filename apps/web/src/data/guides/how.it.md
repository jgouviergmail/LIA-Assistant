...
8. [Routing semantico ed embeddings semantici](#8-routing-semantico-ed-embeddings-semantici)
...
37. [Tre meccanismi per una convergenza: smorzare una raffica che un tetto non vede](#37-tre-meccanismi-per-una-convergenza-smorzare-una-raffica-che-un-tetto-non-vede)
...
| Trasparenza totale | 589 metriche Prometheus, debug panel integrato, tracciamento token per token |
| Affidabilità in produzione | 317 ADRs, ~32.725 test raccolti da pytest in 1.974 file, osservabilità nativa, HITL a 6 livelli |
...
| Metrica | Valore |
|---------|--------|
| Test | 32.725 raccolti da pytest su 1.974 file di test + 9.160 test vitest sul frontend (soglie di copertura bloccate, ADR-116) |
| Fixture pytest | 1.082, di cui 48 condivise tramite conftest |
| Documenti di documentazione | 703 |
| ADR (Architecture Decision Record) | 317 |
| Metriche Prometheus | 589 definizioni |
...
|----------|---------|-------------|
| OpenAI | GPT-6 (astra, sol, luna), GPT-5.6 (terra, sol, luna), GPT-5.5, GPT-5.4 (+ mini), GPT-5.x (+ mini/nano), GPT-4.1, GPT-4o, o3/o4-mini | Prompt caching nativo, Responses API, reasoning_effort |
...
| Google | Gemini 3.8/3.7/3.6/3.5 Flash, Gemini 3.1 Pro, Gemini 2.5 Pro/Flash | Multimodale, embedding duali |
| DeepSeek | deepseek-flash (V4.1), deepseek-v4-pro (V4), deepseek-chat (V3), deepseek-reasoner (R1) | Costo ridotto, reasoning nativo |
...
| Ollama | Qualsiasi modello locale (capacità lette dal server) | Client nativo: ragionamento controllato, finestra di contesto, JSON vincolato. Zero costo API, auto-ospitato |

**Perché 7 provider?** La scelta non è la collezione fine a sé stessa. È una strategia di resilienza: ogni nodo della pipeline può essere assegnato a un provider diverso. Se OpenAI aumenta le tariffe, il router passa a DeepSeek. Se Anthropic ha un'interruzione, la risposta viene dirottata su Gemini. L'astrazione LLM (`src/infrastructure/llm/factory.py`) utilizza il pattern Factory con `init_chat_model()`, sovrascritto da adattatori specifici (`ResponsesLLM` per l'API Responses di OpenAI, eleggibilità tramite regex `^(gpt-4\.1|gpt-5|gpt-6|o[1-9])`).

**Il caso particolare dei modelli locali.** Sei fornitori parlano un'API remota; il settimo gira da te, e LIA gli parla tramite la sua **API nativa** (`langchain-ollama`) invece che con il suo strato di compatibilità OpenAI. La distinzione non è cosmetica: lo strato di compatibilità non sa esprimere `think` (spegnere il ragionamento di un modello che ragiona, o sceglierne la profondità), né `num_ctx` (la finestra di contesto, che un server locale sceglie altrimenti in base alla memoria video e che taglia in silenzio l'inizio di un prompt troppo lungo), né separare la traccia di ragionamento dalla risposta. E poiché il nome di un modello non dice nulla delle sue capacità, queste sono **lette dal server** (`/api/show`: strumenti, visione, ragionamento, lunghezza del contesto) e alimentano sia l'esecuzione sia la schermata di amministrazione — così una profondità di ragionamento non raggiunge mai un modello che non ragiona, cosa che il server rifiuterebbe.
...
│   │   ├── nodes/                # Nodi del grafo: pipeline, ReAct, HITL, iniziativa
...
│   │   ├── prompts/v1/           # file .txt di prompt versionati
│   │   ├── graphs/               # Un builder di agente per dominio
...
│   ├── sub_agents/               # Sub-agenti effimeri, delegati e in sola lettura
...
└── infrastructure/               # Livello trasversale
    ...
    └── observability/            # Metriche Prometheus per sottosistema, tracing OTel
```

### 3.2. Catena di priorità della configurazione

Un invariante fondamentale attraversa l'intero backend. È applicato ovunque, poiché una divergenza tra una costante e la configurazione reale di produzione provoca un bug silente:
...
LIA offre due modalità di esecuzione (commutabili per utente tramite un toggle nell'intestazione della chat): **Pipeline** (predefinita, deterministica ed economica in token) e **ReAct** (autonoma e iterativa). Il Router classifica prima la richiesta (conversazione diretta o azionabile) e poi la inoltra alla modalità attiva.
...
**Architettura**: 5 nodi dedicati nel grafo LangGraph principale (non un sottografo):
...
La spesa di un turno obbedisce a una legge di conservazione (ADR-256): il tempo di ragionamento e il tempo degli strumenti sono due budget con un nome, mai uno solo — una delega (sotto-agente, task MCP iterativo, browser) apre il proprio ciclo LLM dietro una singola chiamata di strumento e, al suo limite di pipeline, varrebbe da sola il 100 % del budget di ragionamento. Ogni chiamata di strumento porta un limite individuale tratto dalle stesse famiglie di timeout della modalità Pipeline, scelto per non essere mai più severo di quello che lo strato sottostante applica già, e un timeout sollevato dallo strumento stesso è attribuito allo strumento, non al nostro taglio. La condizione di arresto resta un unico predicato con due lettori: il router lo applica per decidere, il nodo di finalizzazione per spiegare.
...
Ciò che uno strumento restituisce viene **proiettato nel contesto del ciclo elemento per elemento, sotto un budget di token** (ADR-286). L'elenco di elementi più pesante di un risultato è paginato ai confini degli elementi — un elemento ammesso è completo, il blocco resta JSON valido, gli scalari viaggiano interi e il primo elemento passa sempre — sotto `min(REACT_TOOL_RESULT_MAX_TOKENS, finestra × REACT_TOOL_RESULT_WINDOW_FRACTION)`, la finestra essendo quella dello slot ReAct. Un taglio viene **annunciato** al modello dopo il tag di contenuto esterno (quanti su quanti, e la via verso il resto) e contato per strumento, così che una casella di messaggi interi non raggiunge mai il modello come un dump tagliato in mezzo alle intestazioni del primo. L'albero grezzo del fornitore non lascia mai il builder delle e-mail, e uno strumento i cui dati sono iniettati solo dall'esecutore del pipeline si dichiara riservato al pipeline invece di essere offerto a un ciclo che non può eseguirlo.
...
Gli strumenti che il ciclo riceve sono **legati per rilevanza, mai per ordine di registrazione** (ADR-293). Il router calcola già un embedding della domanda per valutare i domini; dallo stesso vettore ordina ogni manifesto disponibile — strumenti nativi e server MCP della persona insieme — in una classifica globale che il ciclo legge. La selezione si compone senza che alcun nome di strumento sia scritto da nessuna parte: gli strumenti dei domini rilevati, poi i migliori di ogni altra famiglia perché ciascuna resti raggiungibile, poi la testa della classifica fino a un budget pubblicato; il tetto non è più che una rete di sicurezza per ordinamento stabile. Una lista «di base» dichiarata a mano è stata scritta e rifiutata, perché codificava l'uso di un account in un prodotto multiutente — la copertura per famiglia dice la stessa cosa strutturalmente. Il ciclo monta anche il blocco degli spazi di conoscenza che la pipeline inietta, letto dal pacchetto che il router ha precaricato senza consumarlo: ciò che la pipeline sa, il ciclo lo sa. Il numero di strumenti legati e i token dei loro schemi sono misurati, perché quel prefisso pesava la maggior parte della prima chiamata senza essere contato.

Un ciclo è **giudicato sul risultato** (ADR-310). Si fermava appena il modello non chiamava più strumenti, così la perseveranza dipendeva da come un modello configurato leggeva una frase. Ogni dato che il ciclo si è proposto di ottenere — la richiesta della persona e ogni verifica incrociata che ha avviato — finisce ora ottenuto o dichiarato in un blocco `<unresolved>` che chiude la bozza, con i gradini tentati: la chiamata corretta, poi un'altra fonte indicata come `(fallback)`, poi la dichiarazione. Un solo predicato, `should_recover`, legge la dichiarazione e chiama quello che decide l'arresto (`react_exit_reason`) invece di ricopiarlo; instrada verso `react_recovery`, un quinto nodo che non chiama alcun modello: toglie la bozza dal thread nel momento stesso del passaggio, e ogni chiamata successiva del turno riceve, in modo transitorio — mai scritto in `messages` —, la bozza e un'istruzione collocate subito dopo. I passaggi sono limitati (`REACT_RECOVERY_PASSES_MAX`), contati per esito, e un passaggio che finirebbe senza una risposta utilizzabile restituisce la bozza invece di scambiarla con nulla. A monte, uno strumento non sostituisce mai un valore che non sa leggere: il meteo rifiuta una data illeggibile con il formato atteso e la data di oggi, sotto un contratto pubblicato da un'unica costante letta sia dal manifesto sia dallo schema dello strumento — il ciclo collega lo schema, mai il manifesto.

Un errore di uno strumento vi si legge in modo **strutturale** (ADR-303): il corpo di un `ToolMessage` porta la prosa dello strumento, quindi l'unico verdetto onesto è un marcatore che il messaggio porta da sé — `status="error"`, misurato come persistente dopo il checkpoint. Un unico predicato di successo, `core/tool_outcome.explicit_success`, serve il ciclo, il registro delle consultazioni e le metriche: un errore dichiarato, come un risultato vuoto, non compra più alcuna iterazione. E quando una persona sceglie gli scambi frequenti nelle impostazioni (ADR-311; `REACT_CROSS_TURN_CACHE_ENABLED` fissa solo il valore predefinito di un account che non ha scelto, ADR-308), il ciclo collega tutti gli strumenti nel loro ordine di registrazione e colloca il contesto del turno dopo la domanda — nella forma che ogni fornitore accetta, dichiarata e verificata all'avvio —, perché il prefisso di un turno sia quello del successivo: misurato su 396 turni reali, dal 18 al 42 % di risparmio per turno secondo il meccanismo di cache, un sovraccosto su un modello senza cache e un ripiego contato sulla selezione per pertinenza quando il catalogo supera il tetto o la finestra. Anche la cronologia scende allora a blocchi, ancorati al contatore dei turni, perché ogni turno prolunghi il precedente: i tre vanno insieme perché, misurato, nessuno conviene da solo, e gli scambi occasionali mantengono la selezione per pertinenza, il cui costo non dipende dall'intervallo tra due turni.
...
- **Spegnimento pulito** — allo shutdown, il lifespan drena i produttori in corso prima di cedere il controllo; un run ucciso archivia il suo parziale con il flag `interrupted`, e una riparazione a inizio turno ripulisce i `tool_calls` orfani che un checkpoint interrotto lascerebbe (i provider rigorosi li rifiutano al turno successivo).

L'insieme è governato da un feature flag e una dozzina di impostazioni configurabili via env (TTL, heartbeat, drain, polling) validate al boot — un periodo di heartbeat incompatibile con il TTL del lock rifiuta di avviarsi.
...
### 5.5. Artefatti generati: dalla richiesta al file scaricabile (ADR-226)
...
La generazione di immagini segue la stessa regola dell'offerta dichiarata (ADR-305). Una **famiglia** — OpenAI GPT Image, Qwen Image 3.0 — dichiara ciò che un modello accetta: le sue qualità, le sue dimensioni (un elenco fisso o un inviluppo di area), i suoi livelli di fatturazione e se l'immagine di riferimento di un ritocco si fattura per immagine. Ogni superficie legge quest'unica risposta: un modello che nessuna famiglia dichiara non è tariffato, né proposto, né scelto, né eseguito. Un client per fornitore esegue il modello configurato — il ritocco OpenAI passa da `images.edit` su quel modello, dove una deviazione tramite l'API Responses eseguiva il modello predefinito dell'SDK e un modello di testo che nessuno contava —, un risultato consegnato come URL viene scaricato subito sotto un elenco di host consentiti, limitato e verificato, e la preferenza della persona è un'intenzione risolta in esecuzione verso l'offerta del modello (la qualità più economica offerta, la dimensione con lo stesso orientamento e l'area più vicina) poi pubblicata come valore effettivo. Il formato scelto viene applicato una volta per tutti i fornitori, convertendo l'immagine prima di salvarla.

Se la persona lo sceglie, la descrizione di una **nuova** immagine può essere riscritta prima della chiamata da uno slot dedicato, secondo i consigli che ogni fornitore pubblica (ADR-315). È un miglioramento, mai una porta: un testo citato tra virgolette deve tornare intatto, e una riscrittura vuota, troppo lunga o fallita lascia partire la descrizione originale; un ritocco non viene mai riscritto; la chiamata è fatturata e registrata come le altre, e la galleria conserva le parole della persona. L'operatore può ritirare l'opzione per l'intera istanza, e la schermata delle opzioni pubblica questo stato invece di proporre un'impostazione senza effetto.

### 5.6. Ciò che la persona conserva: file prodotti e risposte salvate (ADR-279, ADR-282)

Un artefatto e una risposta seguono due cicli di vita diversi una volta che esistono, ed entrambi appartengono alla persona e non alla conversazione. Un file prodotto — un'immagine, un documento, una schermata del browser — è un allegato marcato con la sua **origine**, elencato in una galleria propria con un totale esatto e una scadenza di conservazione visibile; svuotare una conversazione toglie ciò che la persona ha caricato e mai ciò che LIA ha prodotto, perché una pulizia toglie ciò che la sua famiglia dichiara, non ciò che le somiglia.

Una risposta che la persona vuole conservare è un altro oggetto: è prosa, deve essere resa esattamente come la bolla e deve sopravvivere a una conversazione che si ripristina spesso. Un **segnalibro è quindi una copia, mai un puntatore**. Al clic, la risposta (markdown o un documento HTML arricchito, tale e quale), la richiesta che l'ha prodotta e la data della risposta vengono scritte in una tabella propria. I due riferimenti verso la conversazione sono `SET NULL` alla cancellazione: servono solo all'interruttore sulla bolla finché la conversazione vive. La richiesta è risolta **lato server**, sotto lo stesso predicato di visibilità della chat — l'ultimo messaggio visibile della persona prima della risposta, mai la domanda sintetica di un'esecuzione fuori turno — e una notifica inviata di iniziativa di LIA non conserva alcuna richiesta piuttosto che una sbagliata. Un indice univoco parziale rende l'interruttore idempotente per costruzione, il limite dell'account è pubblicato perché è imposto, pagina e totale vengono dallo stesso `WHERE`, e l'interruttore dell'operatore protegge solo l'atto di conservare: ciò che è già stato conservato resta leggibile, esportabile ed eliminabile.

Leggere e cancellare i propri file sopravvive al tetto dei caricamenti: il router `attachments` è montato qualunque cosa dica `ATTACHMENTS_ENABLED`, la guardia di capacità posandosi sul solo caricamento. Poiché ogni documento generato è servito da quella lettura, un'istanza che genera senza accettare caricamenti — il dimostratore — rende apribile ciò che produce, e la scansione di scadenza gira per i quattro produttori della tabella, mai per il solo caricamento.

Una risposta conservata è anche **indicizzata come documento di uno spazio di conoscenza** (ADR-291): uno spazio per account, trovato dal suo ruolo e mai dal suo nome, creato al primo segnalibro, attivo per impostazione predefinita, fuori dai tetti di spazi e documenti. Il segnalibro resta il record e il documento la sua proiezione — un titolo datato, la richiesta citata, la risposta convertita in Markdown quando era una pagina HTML —, così che lo stato esposto (indicizzato, in attesa, rinviato sotto un tetto di spesa, disattivato, in errore) è derivato dal documento finché esiste, e il costo è riportato solo una volta pronto. Una proiezione è rivendicata prima di essere calcolata, le porte di capacità e di spesa sono lette all'atto, e un recupero cavalca il tick di manutenzione degli spazi servendo per primo il segnalibro tentato meno di recente — un account sotto quota non affama più gli altri. Eliminare il segnalibro rimuove il documento; il documento non si sposta né si elimina dallo spazio, e uno spazio gestito dal suo ruolo non si elimina a mano.

Ciò che è stato prodotto si ritrova anche **chiedendolo** (ADR-318): uno strumento di lettura cerca nella galleria viva per parole del titolo, famiglia e periodo, e mostra i file trovati come le schede stesse della chat, con il totale esatto. Un'immagine generata si può infine **condividere con una connessione** come copia (ADR-316): il clic vale come conferma — niente bozza né modello; due quote giornaliere sono serializzate per mittente sotto un lock di transazione; la copia del file, la riga di galleria del destinatario e il registro delle condivisioni sono scritti in un'unica transazione; poi la chat del destinatario è raggiunta al meglio, su una sessione propria. Il commento di un terzo vi è citato **alla lettera**: neutralizzato con riferimenti numerici di carattere, perché la chat rende il Markdown con HTML grezzo e immagini `https:`, e un escape con barra rovesciata verrebbe reso come una formula.
...
### 6.7. Capacità invocata: quando la richiesta non è una frase

Un piano nasce da un testo. Ma quando la richiesta viene da un **pulsante** — una scheda con un nome, caselle spuntate — il sistema possiede quella certezza **prima** che sia consultato un solo modello. Trasformarla in prosa e poi spendere tre fasi stocastiche (analizzatore, pianificatore, validatore) per ricostruirla distrugge informazione e paga per ritrovarla. Misurato: lo strumento atteso otteneva **0,853**, il miglior punteggio del catalogo, e il piano ne chiamava uno generico.

La richiesta porta quindi, accanto alla frase mostrata, la **capacità invocata**: una coppia `{capability, subject}`. `capability` è un `Literal` **chiuso**, respinto da Pydantic al confine HTTP — il browser nomina una capacità, **mai** uno strumento, e il server decide quale strumento in sola lettura la realizza. Questa porta non conduce ad alcuno strumento di mutazione. Il trasporto fino al pianificatore è una `ContextVar` di richiesta, posta nello stesso punto e con la stessa disciplina delle preferenze degli skill.

Viene applicata **prima della validazione**, esattamente come il riporto dei parametri entro i limiti: ciò che è meccanicamente riparabile viene riparato, mai segnalato come difetto. Il piano è **arricchito, non sostituito** — tutto ciò che il pianificatore ha previsto e che aggiunge qualcosa resta; ciò che ha previsto e che la capacità già copre viene rimosso, perché una risposta senza relazione posta accanto a una lacuna dichiarata la contraddice. Due garanzie: un passo che un altro sta ancora leggendo — per dipendenza dichiarata **o** per riferimento `$steps` — viene conservato, e un piano senza passi (chiarimento in attesa, esecuzione delegata a uno skill) non viene mai trasformato in un'esecuzione. Una garanzia che scavalca una domanda non è una garanzia.
...
### 7.5. Chiusura del catalogo

Il filtraggio semantico valuta gli strumenti rispetto a una **parafrasi inglese della richiesta, rigenerata a ogni turno da un modello**: la stessa domanda può quindi produrre due cataloghi diversi. Se gli strumenti selezionati richiedono un dato che nessuno di loro sa produrre — l'identificativo di un messaggio per potervi rispondere —, lo spazio dei piani validi è vuoto **prima ancora** che il modello inizi. Non gli resta che inventare un nome di strumento.

La chiusura applica una regola che non guarda mai la richiesta: *ogni tipo di dato richiesto da uno strumento del catalogo deve essere prodotto da un altro strumento del catalogo*. È un linker che risolve riferimenti mancanti, non una ricerca che tira a indovinare. Due condizioni la rendono corretta e non soltanto plausibile: uno strumento non soddisfa mai se stesso («rispondere a una mail» produce anch'esso un identificativo di messaggio — quello appena inviato), e solo uno strumento di **lettura** vale come fonte (non si scatena un invio per scoprire un identificativo). Crescita misurata del catalogo: **+1 strumento**.

---

### 7.6. Raggiungibilità tra domini

Chiudere il catalogo definisce ciò che un piano può **concatenare**. Prima viene un'altra domanda: quali strumenti vi **entrano**. Il filtraggio scarta ogni strumento il cui dominio non figura tra quelli rilevati — **prima** di consultare qualunque punteggio semantico. Uno strumento davvero trasversale è quindi invisibile a ogni richiesta classificata altrove, per quanto bene sia valutato.
...
Non era un capriccio del modello ma l'unico modo di obbedire. Il prompt annuncia `Primary domain: telephony`, una regola verifica che il piano copra quel dominio, e il catalogo di `telephony` conteneva **esattamente una capacità: effettuare una chiamata**. Coprire il proprio dominio primario significava quindi agire.

Sono state aggiunte tre capacità di lettura, **ciascuna nel dominio che ne era privo** — chiamate, impegni aperti, messaggi inoltrati. L'alternativa, tramite i domini aggiuntivi della sezione precedente, è stata misurata e scartata: renderle raggiungibili da `contact` espelleva **sei strumenti di mutazione** dai cataloghi più affollati, essendo il limite fisso. Una capacità di lettura non deve costare una capacità di scrittura.

Una regola **deterministica** completa il dispositivo, prima di qualsiasi chiamata al modello: intento non mutante rilevato + piano che chiama uno strumento di mutazione → piano non valido. Viene eseguita con le altre regole pre-LLM, quindi fuori dalla portata dell'esenzione che dispensava dalla verifica ogni piano ben concatenato che terminasse con una mutazione — cioè esattamente la forma difettosa. Più il piano era ben formato, meno veniva verificato.
...
### 8.1. Perché embeddings semantici? (ADR-049)

Il routing puramente LLM presentava due problemi: costo (ogni richiesta = una chiamata LLM) e precisione (il LLM si sbagliava sui domini nel ~20% dei casi multi-dominio). Gli embeddings semantici risolvono entrambi:
...
**Ciò che non ha nulla da confermare.** Un'azione non chiede nulla quando nulla può deviarla. L'e-mail a sé stessi (ADR-314) non ha un destinatario come parametro: l'indirizzo è quello dichiarato dal fornitore della casella collegata o, senza casella, l'indirizzo verificato dell'account tramite il relay di LIA — nessun testo di terzi può quindi reindirizzarla. Lo strumento si dichiara `reversible` con la sua ragione scritta, così una routine può inviare il proprio risultato dove una bozza è rifiutata. Un indirizzo digitato che risulta essere il proprio passa invece dalla bozza: un destinatario che a volte saltasse la conferma sarebbe un bersaglio di iniezione.

### 9.3. Critica della bozza: una descrizione, una domanda per bozza

Una bozza da confermare e il resoconto della sua esecuzione sono **descritti una volta** — una specifica di scheda fatta di righe etichettate, note e blocchi, costruita da un renderer per tipo di bozza dal registro di visualizzazione — e **disegnati per superficie**: nella chat, lo stesso markup `lia-card` di un'e-mail o di un evento (intestazione, illustrazione, campi con le loro icone, corpo in blocco); su un ticket, su un canale esterno o nella modalità di visualizzazione `markdown`, il Markdown che il commento del ticket sa appiattire. La superficie è decisa dal run, mai indovinata da un chiamante. Un resoconto dice cosa è stato fatto, a chi e con cosa: ogni riga di un lotto porta i campi chiave che il suo tipo dichiara e un estratto limitato del testo inviato, citato con le virgolette della lingua. Il confronto prima/dopo degli aggiornamenti, gli avvisi di irreversibilità, le etichette i18n e i link cliccabili fanno parte di quella descrizione.

Un turno che prepara **più bozze indipendenti** — due e-mail, un evento e un'attività, più chiamate a strumenti di una stessa iterazione ReAct — le sottopone **una bozza per interruzione**: la sequenza si apre con l'elenco di tutto ciò che il turno ha preparato, ogni bozza ha la sua scheda, la sua domanda e la sua modifica, la posizione è indicata («Bozza 2 di 3»), e niente viene eseguito prima dell'ultima risposta; le decisioni accumulate vengono poi eseguite insieme, ciascuna sotto il proprio tipo, e una bozza annullata viene riportata anziché persa. Solo un lotto che la persona ha **pre-approvato come elenco** — gli elementi di un passo FOR_EACH approvato in quel turno, di un solo tipo — conserva la sua conferma raggruppata. Un ticket segue la stessa identità (la bozza sullo schermo), e un nuovo turno inizia sempre senza bozza in revisione.
...
### 9.5. Cicli di revisione replay-safe (ADR-092)

La semantica di ripresa di LangGraph ri-esegue il nodo interrotto **per intero**: gli `interrupt()` passati restituiscono i valori memorizzati, ma tutto il resto viene rieseguito dal vivo. Qualsiasi ciclo scritto attorno a `interrupt()` dentro un nodo ripete quindi i suoi effetti collaterali (chiamate LLM, API) a ogni decisione dell'utente. I due cicli di revisione — modifica iterativa delle bozze e conferma delle operazioni massive (nodo dedicato `for_each_confirm`) — seguono un pattern normativo: **un solo `interrupt()` per esecuzione di nodo**, lo stato del ciclo passa dallo state checkpointed e l'iterazione avviene tramite un self-loop condizionale. Garanzia dimostrata con harness di replay compilati: ogni modifica LLM viene eseguita esattamente una volta e il contenuto confermato è esattamente l'ultimo mostrato.
...
```
PostgreSQL + pgvector
├── memories                                        → Memoria a lungo termine: fatti, preferenze, profilo psicologico
├── rag_documents + rag_chunks                      → Spazi di conoscenza, ricerca ibrida
└── AsyncPostgresStore (user_id, "context", domain) → Contesto strumenti (Data Registry)
```
...
**Una sola porta di ricerca** (ADR-313). Una query viene incorporata come *chiave* — senza i pattern di banalità conversazionale, che collidono con nomi reali e domande brevi — prima che si apra una sessione; poi vengono serviti i ricordi vivi della persona sopra la soglia di pertinenza, eventualmente ristretti a categorie. Tutti i chiamanti passano da essa: i fatti di memoria del pianificatore, dell'iniziativa e della risoluzione dei riferimenti, il richiamo 360° di una persona e le ricerche del telefono, che rispondono a «cosa so di X» con X invece che con gli ultimi ricordi.

**E consultata di proposito.** Un dominio instradabile porta uno strumento di lettura, così il ciclo ReAct e il pianificatore possono cercare un argomento che il messaggio non nominava — il mittente di un'e-mail appena letta, un luogo citato in un documento. I suoi limiti sono pubblicati dalle fonti che applica (il vocabolario delle categorie *è* quello memorizzato, il tetto dei risultati *è* l'impostazione), la preferenza della persona lo rifiuta come rifiuta l'iniezione, un ricordo sensibile torna marcato con la sua sfumatura d'uso, e un fallimento è classificato, mai «nulla in memoria». La ricerca ibrida BM25 + embedding vive dove serve: i RAG Spaces (sezione 17).
...
**Disciplina di dedup**: nessun guard write-time. Al consolidamento, `STEP 1` esegue una scansione a coppie esplicita che fonde i duplicati semantici, e `STEP 5` raggruppa attivamente le L1 convergenti in pattern L2.
...
Il ritratto si compone di **quattro fonti oltre ai diari** (ADR-292): memorie, interessi, abitudini apprese e debrief di relazione entrano nel prompt di consolidamento come materia di sintesi. Il dominio dei diari non importa nessuno di quei domini — due di essi lo importano già —, quindi ogni fonte si offre attraverso un registro condiviso dal vocabolario chiuso, che l'avvio installa esplicitamente e rifiuta se incompleto. Un lettore risponde `used | empty | disabled | unavailable` con un totale esatto, sotto le sue tre porte lette all'atto (tetto, interruttore dell'operatore, preferenza della persona), e non solleva mai eccezioni: una fonte cieca è nominata, mai letta come vuota. L'assemblatore legge una fonte alla volta sotto un tetto per fonte e scarta una sezione intera oltre il budget globale; il prompt rende una sezione solo se esiste e vieta di rielencare una riga o di nominare le sue fonti. La provenienza è persistita con il ritratto, nella stessa scrittura, e disegnata sotto di esso; una fonte che si è mossa riapre l'idoneità al consolidamento successivo.

**I diari si consultano anche di proposito** (ADR-318). Oltre all'iniezione, uno strumento di lettura — in entrambe le modalità — cerca nei diari della persona un argomento trovato strada facendo, sugli stessi vettori, alla soglia di similarità *configurata* anziché appresa, e serve solo direttive, schemi e sfaccettature: un'osservazione `L0` grezza, non ancora consolidata, non esce mai. La preferenza della persona lo rifiuta come rifiuta l'iniezione.

### 11.6. Sistema di interessi

Rilevamento tramite analisi delle richieste con evoluzione bayesiana dei pesi (decay configurabile). Gli interessi vengono raggruppati in **argomenti** tramite clustering LLM batch (dato derivato, auto-riparante), e la selezione delle notifiche estrae con **rarità a due livelli** (cooldown per argomento + priorità agli argomenti e interessi meno serviti) — una passione non monopolizza mai le notifiche. Contenuto multi-sorgente (Perplexity, Brave, Wikipedia, riflessione LLM) con **link alle fonti cliccabili** aggiunti in modo deterministico. Il feedback utente (thumbs up/down/block) regola i pesi; fusione notturna dei quasi-duplicati.
...
Il conteggio stesso è **contrattuale, non accidentale**: un fornitore compatibile OpenAI emette l'oggetto `usage` su una risposta in streaming solo se la richiesta lo domanda. Ogni fornitore di chat dichiara quindi la sua modalità di contabilità in un registro — richiesta esplicita `stream_usage`, conteggio nativo dell'SDK, o esclusione deliberata (modelli locali gratuiti, chiavi dell'utente finale) — la cui completezza è verificata all'avvio: l'applicazione rifiuta di avviarsi con un fornitore non dichiarato (ADR-220, dottrina ADR-085). Una chiamata a pagamento che termina senza conteggio incrementa un contatore dedicato, registra un avviso e fa scattare un allarme a soglia zero: l'intera classe dei buchi di fatturazione silenziosi diventa un segnale. La stessa dottrina vale per i timeout: il `timeout_seconds` per postazione, amministrabile, è trasmesso al client di ogni fornitore come limite di trasporto per tentativo — le barriere `asyncio.wait_for` dei nodi restano il limite di esperienza utente — e nessun valore predefinito è stato applicato senza confronto con le latenze reali di produzione (ADR-221).
...
Una tariffa vale solo se è **quella che il fornitore fattura**. Una scrittura nella cache del prompt è fatturata al suo prezzo — 1,25 volte l'input presso Anthropic e presso OpenAI per le generazioni che la segnalano —, contata come quarta grandezza dell'unico lettore d'uso, e una guardia AST rifiuta qualsiasi porta di tariffazione che la dimentichi (ADR-306). I prezzi stessi sono riletti sulle pagine dei fornitori, e una migrazione di correzione sostituisce solo un valore che LIA aveva distribuito, mai uno inserito da un amministratore; una chiamata Google Maps è classificata nello SKU che la sua richiesta attiva — un percorso con traffico o pedaggi non è un percorso semplice, una matrice si fattura per elemento. E la cache dei prezzi è una tabella **condivisa da tutti i worker**: ciascuno la ricostruisce dal database all'avvio — mai da un blob anteriore all'ultima migrazione — e ogni scrittore, tariffa o tasso di cambio, pubblica l'invalidazione dopo il suo commit, così un prezzo modificato raggiunge subito ogni worker invece di uno su quattro (ADR-063).

### 12.4. Catalogo admin DB-source-of-truth

La tabella `llm_models` porta il catalogo completo: provider, capacità funzionali classiche (`supports_tools`, `supports_structured_output`, `supports_strict_mode`, `supports_streaming`, `supports_vision`), e — aggiunte strutturanti — la **matrice sampling per modello** (`supports_temperature`, `supports_top_p`, `supports_frequency_penalty`, `supports_presence_penalty`) nonché la **scala di ragionamento accettata** (`reasoning_enum_values`, lista JSONB) e la chiave i18n del suo aiuto (`reasoning_doc_i18n_key`). Questa dichiarazione per modello sostituisce la regex frontend che indovinava prima quali slider nascondere: la finestra Configurazione LLM legge direttamente i flag DB ed espone solo i parametri che l'API del modello accetta realmente. Quella scala non **dichiara** più ciò che un modello accetta, lo **restringe**: ciò che accetta è derivato dalla sua coppia (fornitore, modello), e le due colonne che un tempo descrivevano la *forma* del ragionamento sono rimosse — ora la forma è una sola.

La schermata Tariffazione LLM scrive quella scala **direttamente**: mostra le profondità che la famiglia del modello offre — risolte in tempo reale da `GET /admin/llm/reasoning-family`, con la stessa funzione del traduttore e del validatore — e si **deseleziona** ciò che questo modello rifiuta. Selezionare tutto non memorizza nulla: si applica la scala della famiglia così com'è. La cartella Excel (ADR-228) porta le stesse due colonne, e la sua importazione rifiuta una profondità fuori famiglia **nominando quelle che sarebbero state accettate**: un foglio di calcolo non può mostrare caselle, quindi la garanzia si sposta all'importazione. Qui stava un meccanismo di template — «copia la forma di quel modello esistente» — che raggruppava i modelli per la loro scala *memorizzata* e non per famiglia, così copiarne uno tra famiglie rimuoveva profondità in silenzio. Vedi `docs/technical/LLM_REASONING_IDENTITY.md`.

**Una famiglia di modelli si dichiara una volta, e una risposta breve si chiede senza ragionamento** (ADR-285). La *famiglia* di un modello è la forma della sua API di ragionamento — un interruttore e una scala di sforzo per DeepSeek, un budget per Gemini 2.5, un livello per OpenAI — e deriva da un'unica dichiarazione in `reasoning/profiles.py` che l'adattatore, il validatore e la deviazione dell'output strutturato leggono tutti: tre test di prefisso privati hanno lasciato passare una volta il modello di punta rinominato di un fornitore attraverso tutti e tre insieme, senza scala offerta né interruttore inviato. La stessa cucitura serve i chiamanti che hanno bisogno di due frasi: su un modello che ragiona per impostazione predefinita, il fornitore fattura il ragionamento *dentro* `max_tokens` (OpenAI, Anthropic, Gemini e DeepSeek lo fanno tutti), così un piccolo budget compra ragionamento nascosto e nessuna risposta. `short_answer_config` dichiara quindi *nessun ragionamento* ovunque il profilo risolto possa spegnerlo e solo lì mantiene il suo piccolo budget; altrove resta il budget dello slot, perché un tetto che include il ragionamento non è un tetto sulla risposta. Il tetto di uscita di una famiglia, allo stesso modo, si legge nella riga del catalogo e non in un letterale.

### 12.5. Prompt caching indipendente dal provider

Ogni provider fattura meno (e risponde più velocemente) quando l'inizio del prompt è byte-identico tra le richieste — ma ciascuno con il proprio meccanismo: i blocchi `cache_control` di Anthropic, il routing `prompt_cache_key` di OpenAI, le cache implicite di prefisso su DeepSeek/Qwen/Gemini. LIA separa le responsabilità: ogni prompt di sistema versionato colloca il contenuto statico (ruolo, regole, esempi, formato di output) all'inizio, poi un marcatore canonico `--- DYNAMIC CONTEXT ---`, quindi tutto il contenuto per-richiesta (data, query, contesto, catalogo degli strumenti). I template restano neutrali rispetto al modello; il livello infrastruttura traduce il marcatore nel dialetto di ciascun provider — lo split `cache_control` per Anthropic, la chiave di routing della cache per OpenAI, nulla per le cache implicite, che sfruttano il prefisso stabile così com'è. Il prompt del planner — il più costoso della pipeline — espone così un prefisso cacheabile byte-stabile di ~77 % tra due richieste qualsiasi. Guardie CI shrink-only blindano la convenzione: ogni prompt dinamico deve portare il marcatore, nessun placeholder può precederlo senza un'eccezione giustificata, e la stabilità byte del prefisso del planner è verificata a ogni build.

La convenzione è diventata **un unico schema per tutti i meccanismi di cache** (ADR-309): lo stabile prima del volatile, un'unica lettura del confine (`core/prompt_layout.split_at_marker`) e nessuna scrittura in cache che nessuno rileggerà. Una chiamata singola — l'analizzatore, l'iniziativa, le estrazioni di memoria, interessi e diario — invia la sua parte fissa come messaggio di sistema, dove ogni adattatore può marcarla; i fornitori con punti di interruzione ricevono il loro alla fine del prefisso statico qualunque sia la forma del sistema (un sistema a lista metteva il punto di interruzione sui dati del turno: niente letto, tutto riscritto a 1,25); quelli con la sola cache esplicita vengono marcati esplicitamente; una richiesta che nessun ciclo di strumenti prolungherà scrive solo i suoi punti di interruzione dichiarati; e la risposta invia la conversazione una sola volta, in messaggi interi. Ciò che ogni generazione di modelli accetta — campionamento, scelta forzata dello strumento, sei forme di ragionamento, profondità implicita, blocchi di ragionamento legati alla loro conversazione — è misurato sull'API e dichiarato in un'unica tabella (`core/claude_surface.py` per Claude) letta dalle regole di ragionamento, dall'adattatore, dall'output strutturato e dall'amministrazione. Un modello senza cache paga esattamente ciò che pagava: l'ottimizzazione punta alla media dei meccanismi, mai a un modello.
...
Per l'e-mail quel modello unificato è misurato, non presunto: i tre normalizer producono lo stesso vocabolario `EmailMessage` — un corpo in testo, mai in HTML, da cui la citazione dello storico e la firma vengono rimosse al confine del client (sei lingue, misurato su un corpus di 48 corpi) — e nessuno fabbrica più il formato di un altro provider. `get_emails_tool` sceglie poi cosa servire in base alla domanda, con il suo costo dichiarato: `metadata` elenca senza corpo, `full` legge un corpo pulito paginato per paragrafo, `summary` ragiona su un **condensato per messaggio** (l'essenziale, i punti chiave, le azioni, la categoria, l'importanza) calcolato una sola volta da un piccolo modello senza ragionamento, messo in cache per trenta giorni e rifiutato dai tetti di spesa — mai inventato quando il modello fallisce. È ciò che permette «riassumi i miei non letti» o «una sintesi delle newsletter della settimana» su venti messaggi senza caricarli tutti, per Gmail, Outlook e Apple allo stesso modo (ADR-287).
...
**Nessuna transazione resta aperta durante una chiamata di rete** (ADR-304). Un client di connettore costruito sulla sessione del suo chiamante costringeva quella sessione — e la sua transazione — a restare aperta mentre il fornitore rispondeva: misurato in produzione, fino a 863 secondi `idle in transaction` nella scansione dei risvegli push, da 11 a 330 secondi nel briefing, nell'heartbeat o nella vista delle relazioni. Un client non viene quindi mai costruito su `ConnectorService(db)` — una guardia nomina l'argomento del costruttore, senza elenco di eccezioni —; un'unica porta apre il client attivo di una categoria in una sessione breve e lo chiude su ogni percorso; un job che conserva la sua sessione conferma le sue letture prima di ogni attesa; ogni operazione di connettore del turno di chat chiude la sua transazione, e il timbro d'uso di una chiave API è un merge JSONB atomico a parte, con un tempo di attesa del lock limitato. Il lavoro in background segue la stessa regola: un promemoria viene rivendicato da solo, confermato, poi notificato senza transazioni aperte; il feed push di Drive avanza per confronta-e-scambia, a pagine limitate per numero e per tempo; e solo il worker leader imposta il webhook di Telegram.

### 13.4. Due percorsi di autenticazione

Non tutti i connettori richiedono un account. Un **connettore OAuth** conserva le credenziali personali dell'utente: Gmail, Calendario, Contatti, Drive. Un **servizio a chiave di piattaforma** non conserva alcun dato per utente — non c'è niente da attivare, e la chiave appartiene all'installazione: Percorsi, Luoghi, Meteo, Ambiente. `ConnectorType.uses_global_api_key` porta questa distinzione, e la base degli strumenti sceglie il percorso delle credenziali in base al tipo **risolto**. Una stessa categoria funzionale può quindi mescolare i due: il meteo accetta un fornitore a chiave personale così come un servizio di piattaforma, senza che il chiamante sappia chi ha risposto.

Esiste un terzo caso: un client che **prende in prestito il token di un connettore vicino**. Fogli di calcolo e documenti leggono e scrivono con il token di Drive; le impostazioni Gmail con quello di Gmail. Nelle impostazioni non compare alcun connettore aggiuntivo, ed è voluto — l'utente ha autorizzato uno spazio di lavoro, non un'API. La conseguenza è stata misurata: la cache dei client era indicizzata su utente e tipo di connettore, così due classi che condividono un token si servivano a vicenda. Ora la chiave porta anche il nome della classe.

La distinzione decide anche che cosa appartiene a un **account**. Cinque connettori non chiedono nulla alla persona — Wikipedia, la navigazione di pagine, Luoghi, Meteo, Ambiente — e appartengono quindi all'istanza: nessuna riga per account, niente nelle impostazioni, e un solo predicato (`connectors/keyless.py`) decide se servono tutti — un tipo spento dall'amministratore, una chiave di piattaforma che l'istanza non ha o la navigazione disattivata lo trattiene per tutti. Nella categoria meteo vince il fornitore della persona e quello di Google è il predefinito; un test tiene i cinque fuori dall'elenco di attivazione delle impostazioni (ADR-307).

### 13.5. Telefonia agentica (ADR-127)

LIA può effettuare una chiamata in uscita per conto dell'utente, condurre una conversazione orientata all'obiettivo e reiniettare un riepilogo scritto nella chat. A differenza dei connettori di lettura/scrittura sopra, il connettore di telefonia guida un **agente vocale di terze parti** (ElevenLabs Agents) sulla rete telefonica, configurato per utente (credenziali personali) — LIA non effettua alcuna misurazione dei costi per proprio conto.

**Protezione dei dati per capacità, non per prompt.** L'agente di chiamata dispone di un unico strumento di disponibilità in sola lettura che risolve solo le fasce libero/occupato; non può mai leggere titoli, partecipanti, luoghi o contenuti degli eventi. La garanzia è strutturale — lo strumento semplicemente non espone quei dati — e non un'istruzione di prompt da cui il modello potrebbe essere distolto.

**Percorso di ritorno.** La chiamata non viene mai registrata e la trascrizione non viene mai conservata. Al termine della chiamata, un webhook firmato HMAC specifico per ogni utente avvia una sintesi LLM senza strumenti che produce un riepilogo breve ed effimero, reiniettato in modo asincrono nella conversazione (lo stesso canale di esecuzione distaccata dell'ADR-117) con una bozza di follow-up opzionale in un tocco. Ogni chiamata richiede una conferma HITL prima di comporre il numero e l'intero sottosistema è protetto da un feature flag.

**Il telefono come canale (ADR-290).** La scheda di conferma protegge un terzo che non ha chiesto nulla; quando LIA chiama *la persona stessa*, chi confermerebbe è chi risponde — purché il numero sia provato suo. L'identità è quindi un numero dichiarato nelle impostazioni, mostrato per intero, poi verificato da una chiamata in cui LIA legge un codice che la persona digita (tentativi limitati, un codice effimero legato al numero, un confronto a tempo costante, lo stesso tetto orario di ogni chiamata a pagamento); mai un riconoscimento per nome, e lo strumento per le chiamate a terzi rifiuta quel numero. Lo **stesso** agente vocale serve tre mandati — quello dei terzi cotto nell'agente, quello del titolare e quello della verifica inviati come override per chiamata reso lato server — perché nulla del contesto della persona sia mai cotto in un agente che chiama anche sconosciuti; e come l'agente *suona* (modello, lingua, voce, formato audio, tetto di durata) appartiene al portale del fornitore, mai a un'impostazione, perché il fornitore fonde un PATCH con ciò che conserva e un modello fissato collideva a ogni sincronizzazione con lo sforzo di ragionamento del portale. `call_me` non porta scheda per costruzione, il che permette anche a una routine di pianificarlo. La chiamata porta il contesto della chat sotto un budget di token (ricordi, agenda, promemoria, fili aperti, ultimi scambi — un interruttore lo spegne) e la personalità configurata per l'assistente, ogni lettura annotata nel registro delle consultazioni. Dietro una seconda bandiera, l'agente **legge LIA in diretta** durante la chiamata, e non agisce su nulla: l'insieme degli strumenti è una regola sul catalogo e non un elenco — ogni strumento che si limita a leggere, di un ambito che il telefono offre, i cui parametri obbligatori una voce può dire (55 strumenti su 22 ambiti, più un richiamo nativo della memoria) — collegato all'agente solo per la chiamata del titolare, poiché il fornitore rifiuta identificativi di strumenti in un override per chiamata, e ogni risultato è ridotto a ciò che una voce può dire prima della paginazione elemento per elemento (un identificativo, un link, una struttura annidata non raggiungono mai la voce; quattro eventi di un fine settimana stanno dove prima ne stava uno grezzo). Ogni ambito ha un interruttore proprio della persona, riletto sulla sua riga a ogni richiamo. Quando la chiamata finisce, ciò che è stato detto torna come **il messaggio della persona stessa**: la trascrizione è sintetizzata e poi riproposta dal motore fuori turno come un turno *parlato dalla persona* — le estrazioni di memoria, diario e psiche girano come nella chat, il messaggio porta un badge telefono, una bozza attende conferma nella chat. Il riporto è reclamato prima del turno e chiuso con un aggiornamento condizionale, un crash a metà riporto torna a una notifica che dice perché, e dieci verdetti di riporto sono contati e disegnati nell'elenco delle chiamate — «nessuno ha risposto» e «la linea è caduta» distinti da «ha risposto qualcun altro». Ogni euro di una chiamata cade sotto un unico run id — le ricerche in diretta, la sintesi, il turno riportato — così che il riepilogo per run che il contatore della chat già legge è la fattura della chiamata, sulla risposta riportata e nell'elenco delle chiamate; ciò che gira sulla chiave personale del fornitore è fatturato lì e mai contato qui.

**Una politica per modalità, qualunque sia la linea (ADR-301).** Il telefono riportava la conversazione alla *fine*; il Live del browser affidava ogni richiesta alla chat *durante* — due verità per una stessa parola. Una sessione vocale ha ora una modalità, e la politica segue solo quella: in **Live** (il valore predefinito del telefono, scelto in Impostazioni › Telefonia · La mia identità) la voce non tiene né contesto né strumento e consegna ogni richiesta a LIA attraverso una sola funzione — la stessa che il Live del browser dichiara al suo fornitore — che il server trasforma in un turno di chat della persona mentre parla: le domande di LIA (una domanda È il risultato, la richiesta successiva riprende il turno che l'ha posta), le conferme, i registri, le quote e la traccia nella conversazione arrivano con essa; in **Live diretto** la voce legge per la persona, non agisce su nulla, e le sue parole sono riportate alla fine come un messaggio suo — sulla linea come nel browser, la cui sessione diretta non è più «nulla registrato». Una sola chiusura tiene i conti di entrambe le linee: gli scambi solo vocali archiviati, la scheda con cifre esatte, la decisione, l'apprendimento — oppure la sintesi, il turno riportato e la scheda riscritta quando si chiude, con la spesa riletta. Live è offerto solo dove il fornitore telefonico può richiamare l'istanza; altrimenti le impostazioni lo dicono e la chiamata gira in diretta. Tutto è stato misurato sul motore reale del fornitore, senza telefono: la voce annuncia la chiamata e continua la conversazione, una risposta tardiva andrebbe persa oltre il limite del fornitore — perciò il ponte risponde prima —, e uno scambio che contiene una delega appartiene per intero al grafo.

### 13.6. Leggere un allegato: il suo testo quando ne ha uno, la visione altrimenti

Elencare gli allegati di un messaggio è una lettura di metadati; aprirne uno è un download, e i tre client lo espongono dietro UN metodo del protocollo e-mail — stesso nome, stessa regola di selezione, stessi errori — perché l'asimmetria tra fornitori è l'origine dei bug dei connettori. La selezione è un helper condiviso: per identificatore, altrimenti per nome, un nome ambiguo rifiutato con i suoi candidati. Deve essere così, perché un identificatore di allegato Gmail non è stabile: misurato su una vera casella, cambia tra due letture dello stesso messaggio, così l'identificatore servito da un elenco può essere già sparito quando lo strumento rilegge il messaggio. Un identificatore scaduto non è quindi una bugia: il nome decide quando è dato, una parte unica è inequivocabile, e ogni altro caso risponde con gli identificatori ATTUALI perché il modello riprovi.
...
LIA segue la revisione corrente del protocollo — **2026-07-28** — su entrambe le metà di ciò che un server espone: come gli si parla e che cosa dichiara. Leggere una dichiarazione è un problema a sé, e ha esattamente una implementazione (ADR-255). La specifica definisce lo schema di ingresso di uno strumento come JSON Schema 2020-12 e vi ammette *qualsiasi* parola chiave: un parametro opzionale può scriversi `"type": ["string", "null"]` così come un `anyOf`, una struttura annidata dichiararsi con un `$ref` verso `$defs`, un insieme chiuso con `enum` o `const`. Due consumatori leggono quella stessa dichiarazione — l'adattatore, che ne fa la firma offerta al modello, e il catalogo, che ne fa una voce di piano — e due letture della stessa cosa finiscono sempre per divergere. `json_schema.py` è dunque l'autorità unica, e un test di parità confronta ciò che ciascuno ne ricava. Ogni sua funzione è totale: risponde per qualunque dichiarazione un server possa inviare, perché sollevare un'eccezione lì costa uno strumento, e uno strumento perduto è una capacità che l'utente non ha più senza esserne avvertito. Ciò che resta indecidibile — un `not`, un riferimento irraggiungibile — viene tipizzato in modo permissivo anziché scartato, e ciò che il server applica davvero (insiemi chiusi, limiti, dimensioni) viene pubblicato al pianificatore nello stesso vocabolario di vincoli che gli strumenti nativi usano da sempre.
...
Le annotazioni di comportamento di uno strumento (`readOnlyHint`, `destructiveHint`) sono lette in **una sola direzione**. Qui la specifica è normativa: un client deve trattarle come non affidabili finché non provengono da un server fidato. A una mutazione dichiarata si crede dunque — al peggio un server bugiardo si compra una conferma superflua — mentre a una pretesa di sola lettura non si crede mai, perché una categoria dichiarata prevale sull'euristica dei nomi e toglierebbe lo strumento dalla rete di sicurezza contro le mutazioni non valide e dal rilevamento di portata HITL. In modalità iterativa, dove tutti gli strumenti di un server condividono un'unica impostazione di conferma, uno strumento che il server stesso dichiara distruttivo la chiede in ogni caso.
...
Due fatti viaggiano con ciascun server come dati, mai come prosa di prompt. Un server utente la cui credenziale è quella dell'utente stesso (OAuth, bearer o chiave personale) pubblica la propria portata di account ovunque il modello legga le sue capacità — descrizione dello strumento di delega, manifesto del planner, contesto del sotto-agente e descrizione di dominio generata —, così che «i miei repository» si risolva sull'account collegato invece di finire in una domanda; un server senza autenticazione non pubblica nulla, perché affermare il contrario sarebbe una capacità inventata. E il sotto-agente restituisce i suoi dati, non un conteggio: il suo output porta lo stesso contratto messaggio + blocco dati limitato di ogni strumento ReAct, con i contenuti di terzi contrassegnati come esterni.
...
La frase del promemoria segue la stessa regola di ogni risposta breve: si chiede **senza ragionamento** dove il modello può fermarsi, con un piccolo budget di risposta — e una risposta tornata vuota, o che il fornitore dichiara tagliata al suo budget, è un rifiuto e non un messaggio. La frase di riserva scritta parte al suo posto, la spesa avvenuta viene conservata, e l'avviso nomina il modello e i suoi token di ragionamento. Una notifica è l'unico atto di iniziativa propria di LIA che una persona sperimenta davvero; non arriva mai come una campanella e nient'altro.
...
Un documento che non ha potuto essere indicizzato **dice perché**: un codice chiuso è memorizzato sulla riga (file scomparso, estrazione impossibile, nessun testo, nessun passaggio, troppi passaggi, tentativi esauriti) e tradotto alla visualizzazione in sei lingue, sotto la riga del documento. Il caso più frequente — un PDF scansionato le cui pagine portano immagini senza livello di testo — è distinto dal file davvero vuoto e mostrato con il suo rimedio: aprire il file in Google Docs, che riconosce il testo, poi aggiungerlo allo spazio tramite la sua sorgente Drive. Nessun riconoscimento dei caratteri è promesso che il codice non fornisca.
...
### 17.4. Una cartella Drive è un albero, e il conto è esatto (ADR-297)

Google Drive elenca un livello per chiamata, così una cartella collegata viene percorsa in ampiezza, limitata due volte — file e cartelle — e il taglio è dichiarato: una scorciatoia non è né elencata né seguita, una cartella raggiunta due volte è percorsa una sola volta, una sottocartella illeggibile è contata e saltata, e solo una radice illeggibile è un errore. L'insieme delle cartelle percorse viaggia con la sorgente, così il percorso push instrada un cambiamento sull'intero albero e lascia che l'insieme segua il flusso — una cartella creata sotto l'albero vi si unisce prima che i file creati al suo interno arrivino nello stesso flusso. Una cartella dentro un albero già collegato, o sopra di esso, viene rifiutata: due sorgenti che deduplicano per identificatore di file si ruberebbero i documenti a ogni sincronizzazione.

Il conto mostrato prima di una grande sincronizzazione è calcolato dal codice che indicizza: lo stesso percorso, gli stessi predicati per « supportato » e « invariato », il tetto di documenti dello spazio. Classifica ogni file — nuovo, modificato, invariato, non supportato, oltre la capacità — e il pulsante chiede alla persona solo oltre una soglia fissata dall'istanza; un percorso tagliato da un limite dice « almeno ». Ogni sincronizzazione, manuale o in background, deposita una consultazione per l'atto — e una lettura fuori da ogni esecuzione pubblica la propria, perché il registro conserva solo ciò che un collettore raccoglie.

### 17.5. Un documento di uno spazio entra in un messaggio come copia (ADR-295)

La ricerca della chat legge solo gli spazi attivi, così un documento indicizzato in uno spazio in pausa era irraggiungibile da una domanda senza riattivare l'intero spazio. Il « + » del compositore elenca quindi i documenti pronti di ogni spazio che la persona possiede, uno spazio in pausa contrassegnato invece che nascosto, con una pagina e un totale esatto, e il documento scelto viene COPIATO tramite il meccanismo stesso degli allegati: il file archiviato sotto un nome nuovo, il testo tramite l'estrazione degli spazi di conoscenza — quindici formati, non la lista immagine-e-PDF del caricamento —, la riga un caricamento, così un ripristino della conversazione rimuove la copia e lo spazio, il suo indice e il suo file non vengono mai toccati. Solo un documento pronto viene proposto, i due interruttori di capacità devono essere aperti, e il modello è avvisato quando il testo è al tetto: un taglio è dichiarato, mai applicato in silenzio.
...
**Autenticazione forte (ADR-143/144).** Oltre alla password e a OAuth di Google, l'account può essere protetto da **passkey WebAuthn** (credenziali discoverable, conditional UI sul campo e-mail, sfide Redis monouso, rilevamento dei cloni tramite contatori di firma, zero enumerazione sul percorso anonimo) e un **secondo fattore TOTP** (accesso in due passaggi tramite un token effimero, anti-replay per timestep esplicito, 10 codici di backup monouso con hash). Le azioni sensibili — gestione delle credenziali, esportazione, revoca dei dispositivi, disattivazione della password — passano per una **riautenticazione step-up**: una finestra di 5 minuti aperta da qualsiasi accesso completo (semantica sudo), con un contratto **403 tipizzato** (`step_up_required`, mai un semplice 401 che reindirizzerebbe a /login). **I miei dispositivi** elenca ogni sessione BFF sotto un `display_id` opaco con metadati deliberatamente limitati (famiglie UA/OS, IP troncato a /24), revoca un dispositivo o tutti gli altri, e taglia il flusso SSE di una sessione revocata entro un tick di keepalive; una notifica push segnala qualsiasi accesso da un dispositivo non attestato da un token FCM valido.

**Un'identità federata non concede alcun diritto.** Collegare un'identità Google non cambia mai lo stato di un account: una registrazione in attesa di approvazione resta in attesa, un account bloccato resta bloccato. L'indirizzo dichiarato viene creduto solo se Google ne garantisce la verifica; quando non era mai stato provato, il collegamento lo prova, revoca password e sessioni di chi l'aveva registrato e avvisa gli amministratori come farebbe una verifica dell'e-mail; un account eliminato non viene mai riattivato, e un rifiuto porta un motivo limitato. «Chi sta chiamando?» ha una sola risposta, `resolve_client_ip`, letta dagli audit di amministrazione, dall'indirizzo delle sessioni e dai limiti di frequenza — una guardia AST rifiuta qualsiasi altra lettura dell'intestazione scritta dal visitatore, e il server web trasmette l'indirizzo che Cloudflare attesta. Infine la configurazione dei log è il primo import dell'applicazione, e uno strumento collega il suo logger nel momento in cui registra: un logger collegato prima manteneva i valori predefiniti di structlog, senza filtro di livello né di dati personali.
...
| Tecnologia | Ruolo |
|------------|-------|
| Prometheus | 589 metriche custom (RED pattern) |
...
| Primo evento SSE (richiesta confermata) | 380 ms | < 500 ms |
| Router Latency | 800 ms | < 2 s |
| Planner Latency | 2.5 s | < 5 s |
| Embedding semantico | ~100 ms | < 200 ms |
| Checkpoint save | < 50 ms | P95 |
| Redis session lookup | < 5 ms | P95 |

> Queste latenze misurano l'infrastruttura. Il tempo di risposta completo percepito dipende dalla cascata di chiamate LLM (da pochi secondi a diverse decine secondo la complessità della richiesta e l'hardware) — è il principale cantiere di ottimizzazione in corso, misurato in produzione e tracciato nella roadmap.
...
Dati infra/personali              Code Hygiene (i18n, docs, cicli, parità CI)
Ruff + Black + MyPy               Secret scan (Gitleaks)
Unit tests rapidi                 Test unitari + agenti, soglia di copertura
Rilevamento pattern critici       Test di integrazione (PostgreSQL + Redis)
Sync chiavi i18n                  Replay delle migrazioni (da zero)
...
Tutti i tool restituiscono `ToolResponse` (successo) o `ToolErrorModel` (fallimento) con un enum `ToolErrorCode` (18+ tipi: INVALID_INPUT, RATE_LIMIT_EXCEEDED, TEMPLATE_EVALUATION_FAILED...) e un flag `recoverability`. Lato API, raiser di eccezioni centralizzati (`raise_user_not_found`, `raise_permission_denied`...) sostituiscono ovunque le HTTPException grezze — zero `raise HTTPException` nel codice, mantenuto da una guardia CI e da una rete di test di contratto che prova risposte byte-identiche — garantendo contratti di errore coerenti, registrati e misurati (Prometheus) su ogni percorso di errore.

### 23.4. Sistema di Prompt

File `.txt` versionati in `src/domains/agents/prompts/v1/` (uno per prompt, il literal `PromptName` tenuto allineato da un test), caricati tramite `load_prompt()` con una cache LRU dimensionata oltre l'intero corpus. Versioni configurabili tramite variabili d'ambiente.

**Un prompt enuncia ciò che il codice impone, e nient'altro** (ADR-284). Ogni `{placeholder}` di un file ha un produttore nel modulo che lo carica, e un file che contiene `{{` è reso con `.format()` — una guardia AST legge entrambe le cose per file. Un'istruzione per contesto vive nel file ed è emessa solo quando il contenuto esiste (`response_context_sections.txt`), così un turno nudo non porta alcun involucro vuoto né istruzioni su un documento assente. Il limite che un prompt pubblica è il limite che lo strumento impone, da una costante; un numero viene da un'impostazione e un fatto dal codice che lo applica; una capacità è promessa solo se il turno può eseguirla. La prosa non abita mai un `.py`, e nemmeno un testo di riserva: le impalcature di una riga vivono in file di righe `chiave|modello` letti da un unico parser (`parse_prompt_sections`), e un file mancante è un deployment rotto che fallisce rumorosamente invece di una seconda formulazione silenziosa.
...
L'osservabilità poggia su tre pilastri: **emissione difensiva** sul percorso critico, **dashboard Grafana** pre-cablate (28 dashboard / 719 pannelli che coprono l'app, l'infra e ogni sotto-sistema di business) e **gauge DB-backed** mantenute da un updater periodico.

Un 26° dashboard trasforma questa telemetria in un cockpit prodotto (ADR-178): i risultati sono validati E1 (conferma esplicita dell'utente) o E2 (un'azione rimasta senza correzioni per un'intera finestra comportamentale), il conteggio esatto e deduplicato vive in PostgreSQL — stati mutabili non si derivano mai da contatori Prometheus — e Grafana lo legge tramite un ruolo in sola lettura ristretto alle viste aggregate con statement timeout fissato.
...
**La posizione sopravvive al ciclo di vita mobile** (ADR-219). Una PWA congelata dall'OS non rimonta mai il suo stato: la posizione scadeva in silenzio e ogni richiesta ripartiva dall'indirizzo di casa. Ogni risoluzione di posizione passa ora da un'unica cascata — posizione viva del browser, altrimenti ultima posizione memorizzata (opt-in, cifrata, fresca sotto le 24 h), altrimenti domicilio — che le azioni pianificate, senza browser, ereditano senza codice dedicato. Due regole di onestà la delimitano: una posizione memorizzata viaggia con la sua età e il modello la enuncia («in base alla tua ultima posizione nota alle 9:30»), mai come attuale; e «a casa» non viene mai risolto da una posizione catturata per strada. Al ritorno in primo piano il permesso viene ricontrollato: ancora concesso, la posizione si aggiorna in silenzio; decaduto — iOS lo fa dopo l'inattività — un banner fornisce, all'apertura della chat, il gesto utente che il foglio permessi nativo richiede.
...
La stessa dottrina risponde a un guasto più silenzioso: una superficie che smette di descrivere il prodotto senza che nessuno se ne accorga (ADR-229). La mappa delle capacità — la pagina che risponde «che cosa sa fare il mio assistente per me?» — pubblicava tredici nodi congelati mentre il prodotto rilasciava la generazione di immagini, i documenti, i plugin, le abitudini apprese, i server MCP utente e la telefonia: proprio lo schermo il cui unico mestiere è essere aggiornato era diventato il meno aggiornato dell'applicazione. Una convenzione scritta aveva già fallito esattamente lì; la correzione è quindi strutturale e non un promemoria. Due tabelle dichiarate partizionano ora l'enumerazione delle capacità di piattaforma fra «disegna un nodo» e «deliberatamente fuori mappa, per questa ragione scritta», e un assert gira all'IMPORT: una capacità aggiunta senza deciderne la sorte fa fallire l'avvio invece di partire invisibile. Una guardia gemella legge le tre superfici client che l'assert non vede — i posti del grafico, i collegamenti «passo successivo», le sei lingue — perché una guardia limitata a Python avrebbe lasciato passare la metà TypeScript della deriva. La stessa aggregazione alimenta poi la panoramica delle impostazioni: una richiesta dice che cosa contiene ogni sezione, con le parole stesse dell'elenco delle capacità, e non dice nulla finché la risposta è in volo, quando è fallita, o per una sezione di cui non sa nulla.

Una regola CSS governa le spaziature del design system: i margini verticali di un elemento `inline` sono **calcolati ma mai disegnati**. Un `<label>` è inline per impostazione del browser, quindi la primitiva di etichetta è dichiarata `block` — altrimenti ogni `space-y-*` sopra un controllo produce lo stesso stacco di tre pixel qualunque sia il valore, e una ricalibrazione cambia il codice senza spostare un pixel. Da qui il riflesso: quando una regolazione visiva non ha effetto, misurare il `display` e la geometria DOM in un browser reale prima di sospettare della consegna. Una guardia lo blocca.

## 24. Architettura delle decisioni (ADR)

317 ADRs in formato MADR documentano le decisioni architetturali principali. Alcuni esempi rappresentativi:
...
| 049 | Embeddings semantici | Routing LLM solo impreciso | +48% di precisione via embeddings semantici |
...
**Frontend**: Avatar emozionale con anello colorato per messaggio, dashboard a 4 grafici (umore/emozioni/relazione/motivazioni), guida educativa interattiva con 7 sezioni, espressività e stabilità personalizzabili.

**Il quadro di riposo, ricentrato su misura**: la proiezione di Mehrabian faceva riposare **tutte e 14** le personalità del catalogo a D > 0, così i cinque stati d'animo che richiedono dominanza negativa restavano irraggiungibili a riposo — lo smorzamento è un'omotetia e non può correggerlo. Due impostazioni sono consegnate **inerti** perché la loro attivazione sia una decisione misurata. La misurazione di agosto 2026 (769 istantanee, 3 utenti, 90 giorni) ha confermato la diagnosi più nettamente della simulazione: quota di dominanza negativa **0,0 %**, e l'impulso che incoronava la gioia emozione dominante nel **31 %** dei turni, a prescindere dalla valutazione realmente riportata. Entrambe sono ora i valori predefiniti del codice: a 0,20 il catalogo abbraccia lo zero (7/14 riposano al di sotto, ordine esattamente preservato) e la valutazione riportata torna a governare il canale emotivo. Ciò che la stessa misurazione ha **confutato** è annotato anch'esso: anche l'attivazione è bloccata, ma dal flusso di valutazione e non dalla geometria dei punti di riposo — questo cambiamento non la corregge.

Il tetto regge solo se il libro è completo, e la regola non è solo quella del modello: **ogni euro che la piattaforma paga per una persona è tracciato, mostrato, attribuito e contato per quella persona, qualunque sia la modalità o il percorso** (ADR-272, emendamento). Google Maps Platform aveva buchi in ogni direzione. Le due porte di persistenza del tracciatore decidevano solo sui record del modello, così una consultazione Places o Routes durante una telefonata o una sessione live diretta — nessuna chiamata al modello — non raggiungeva nessuno dei quattro registri; `pending_families()` è ora l'unico predicato, e una guardia rifiuta un secchio di record che non legge. Il contatore delle chiamate Google non faceva nulla senza tracciatore ambientale mentre l'heartbeat, il briefing (otto chiamate meteo fatturate per aggiornamento), le riunioni e i proxy delle immagini pagavano: ora fallisce chiuso in `google_api_calls_unaccounted_total`, tenuto a zero da un allarme. Ogni superficie fuori turno apre la propria contabilità attorno all'intero atto attraverso un'unica porta, la strada che prende ogni modulo è dichiarata in un registro gemello di `LLM_SPEND_ROADS` e verificata sul grafo degli import, un'immagine fatturata è contata dove Google la fattura — al proxy, sul 200, unita al suo turno da un id di esecuzione firmato su (id, account) — e una sola colonna, `billed_cost_eur`, sostituisce quattro somme scritte a mano con tre sottoinsiemi diversi.
...
Il consumo è deliberatamente contenuto: contesto ambientale per risposte e briefing, al massimo un'offerta di routine mancata al giorno, silenziata dopo due offerte ignorate finché la routine non si ripresenta, e uno scoring del momento delle notifiche che preferisce le finestre apprese senza mai allargare i limiti configurati dall'utente — una regola anti-inedia garantisce che un'intersezione vuota non cambi nulla. Ogni soglia applicata è pubblicata nel pannello: un'abitudine mostrata è provata, o non esiste.
...
**Ciò che è appreso viene consumato attraverso UN predicato, e verificato per esecuzione.** Il pannello delle abitudini modifica righe specchio — una fascia si mette in pausa o si blocca — mentre il blocco del heartbeat, lo scoring dei tick e il blocco ambientale leggevano il profilo: una fascia bloccata continuava a guidare i tick. `habits/consumption.py::load_consumable_profile` serve ora tutti e tre, conservando solo le fasce la cui riga è attiva, e la chiave di una fascia ha un solo produttore. Un'abitudine appresa ha un ciclo di vita posseduto dal lavoro notturno (`recurrence_sync`: promuovere, aggiornare, conservare, retrocedere — solo righe attive, mai nel dubbio, e anche per un account silenzioso, che è precisamente il caso della retrocessione), mai dal suggerimento della chat. La porta di apprendimento (`learning_gate.read_learning_gate`, chiusa per impostazione predefinita) chiude tutte le porte — la scrittura del ledger, il rilevatore di iniziativa, il seeding — e segue `HABITS_ENABLED` più l'interruttore di capacità `habits` letto all'atto: `habits` e `heartbeat` hanno lasciato la famiglia protetta per rotta di ADR-280 per la famiglia imposta nel servizio, perché le rotte sono l'archivio. La decisione **dichiara** la sua offerta (`HeartbeatDecision.habit_offered`, regola 24) invece che il budget delle offerte penda da un'etichetta di fonte scelta dal modello, e l'offerta ha un oggetto: l'`immediate_intent` dell'analizzatore è un istogramma limitato dentro il payload del ledger — mai parte della chiave, che frammenterebbe ogni ledger — così che `usual_intent` raggiunge la riga, il prompt (« usually asks for a 'search' on 'email' ») e il pannello attraverso il vocabolario chiuso `IMMEDIATE_INTENTS`. Lo scoring dei tick vive in `habits/tick_scoring.py` con una `TickSurface` per passaggio fissata al suo verificatore di idoneità, e il passaggio degli interessi applica entrambe le porte — la guardia di riunione e il ritmo appreso; un risveglio aggira il ritmo ma si fa da parte davanti a una riunione, e il verdetto di agenda porta `next_start` perché il ritmo non nasconda mai un appuntamento. La presenza è attiva per impostazione predefinita e l'ultima attività conta un ping di presenza. E il registro delle consultazioni è alimentato attorno all'intero passaggio, idoneità compresa — l'aggregatore leggeva in `select_target` mentre il collettore si apriva in `generate_content`: 824 passaggi, zero righe.
...
Il dimostratore pubblico applica questi limiti a se stesso, e li rende leggibili. Ogni capacità del registro vi è decisa per iscritto nel modello di ambiente — attiva o spenta, con la sua ragione — e due guardie rifiutano un tetto lasciato al valore predefinito del codice o una divergenza tra i modelli di sviluppo e di produzione; il censimento delle rotte raggiungibili monta i router sotto quegli stessi tetti, così una famiglia spenta non vi compare e una attiva vi è scritta rotta per rotta. Infine, ogni istanza pubblica nella sua configurazione pubblica lo stato effettivo di tutte le sue capacità — tetto e interruttore composti — e l'istanza che annuncia un dimostratore legge quella configurazione lato server e la mostra accanto al link: l'elenco di ciò che un visitatore troverà, e di ciò che non troverà, segue il file di ambiente della dimostrazione, nessuno lo tiene a mano.

**Un annuncio dice a chi era rivolto** (ADR-312). Il pubblico di un annuncio dell'amministrazione è risolto prima che la sua riga venga scritta, e i suoi destinatari sono registrati con esso: un annuncio mirato è servito solo a loro — anche al loro accesso successivo —, e una selezione che non raggiunge alcun account attivo è rifiutata invece di partire verso nessuno, o verso tutti. La cronologia serve una pagina e il suo totale esatto, un campione limitato dei destinatari con il loro conteggio esatto, l'istante di scadenza e il termine scelto, in quattro query qualunque sia la dimensione della pagina.

## 29. Amministrare per file: la cartella è il modulo

Il catalogo dei modelli LLM conta centoventiquattro voci; ognuna porta ventiquattro caratteristiche e una tariffa a quattro dimensioni. Si amministrava con una finestra di dialogo per modello — adeguata per correggere un prezzo, assurda per accogliere l'intera griglia che un fornitore rivede due o tre volte l'anno. La risposta non è stata una schermata in più, ma una **base dichiarativa**: `WorkbookSpec` / `SheetSpec` / `ColumnSpec` descrivono una cartella di lavoro, e da lì si derivano i due sensi — lo scrittore produce il file, il lettore lo rilegge. La base non importa alcun dominio; il dominio fornisce solo una dichiarazione di colonne e un applicatore che passa dal proprio servizio. Declinare il meccanismo a un'altra schermata di amministrazione significa scrivere una dichiarazione, non codice di formato.

### 29.1. Tre proprietà che separano un'esportazione da un'amministrazione

**Le colonne si risolvono per chiave tecnica, mai per posizione.** La prima riga porta le chiavi invarianti e resta nascosta; la seconda porta le etichette tradotte, colorate per blocco, e i dati iniziano dalla terza. Riordinare una colonna, nasconderla, aggiungerne una, o esportare in una lingua per reimportare in un'altra: nessun effetto sulla lettura.

**Nulla viene cancellato implicitamente.** Una riga assente dal file non cancella mai nulla — un filtro rimasto attivo in Excel non può svuotare un catalogo. La rimozione passa da una colonna di stato esplicita, e rimetterla a vero riattiva, il che colma per inciso una disattivazione che non aveva alcun inverso nell'applicazione.

**L'anteprima impegna.** L'importazione avviene in due tempi: il primo non scrive nulla e restituisce il piano campo per campo; il secondo **ri-deriva** quel piano e rifiuta se differisce da quello letto. Un lucchetto ottimistico **per riga** — un'impronta trasportata in una colonna nascosta — rifiuta solo le righe cambiate nel frattempo: chi tocca un modello estraneo non fa respingere l'intero file. E ciò che non è cambiato non viene scritto: senza questa regola, reimportare centoventiquattro righe lascerebbe dietro di sé centoventiquattro versioni di tariffa inutili.

### 29.2. Il file dice ciò che è, non ciò che si suppone

Tre colonne derivate, in sola lettura, esistono perché il dato grezzo inganna. Un modello senza tariffa attiva viene fatturato a zero in silenzio: il file lo dice a lettere. Una tariffa a fasce orarie — le ore di morbida di un fornitore — si leggeva come una tariffa piatta, perché le fasce vivevano su un foglio che nessuno aveva motivo di aprire: ora compare sulla riga che porta il prezzo. E la modalità esportata vale sempre lo stato reale, mai l'istruzione «eredita», che è una consegna di scrittura e non uno stato.

La completezza, dal canto suo, è **sorvegliata** anziché ricordata. Una prima versione della cartella esportava sedici colonne contro uno schema che ne conteneva molte di più, e il test di fedeltà non poteva vederlo: confrontava un'estrazione con sé stessa. L'oracolo è ora lo schema del database — ogni colonna di business è esportata, oppure esclusa con una ragione scritta — e una colonna aggiunta domani fa diventare rossa l'integrazione continua. È la dottrina delle asserzioni di completezza di registro (ADR-085), applicata a un formato di file.

### 29.3. Ciò che la richiesta ha rivelato prima della prima riga di codice

Progettare l'esportazione richiedeva di rispondere a una domanda semplice: qual è la tariffa di un modello? Non c'era risposta. Nessun vincolo imponeva una sola tariffa attiva, e quattro percorsi di lettura selezionavano senza ordine deterministico — due di essi potevano restituire prezzi diversi per lo stesso modello, nello stesso istante, sullo stesso database. Inoltre una cache riempita per nome grezzo e letta per nome normalizzato fatturava un modello datato al prezzo del suo modello di base. Questi difetti non sono danni collaterali: senza di essi l'esportazione non ha oggetto, perché non saprebbe quale riga mostrare.

Il riordino ha prodotto una regola che vale oltre questo dominio: **una migrazione non inventa mai un dato di business.** La regola intuitiva — tenere la riga più recente — è stata confrontata con i casi divergenti reali e si è rivelata falsa ogni volta: la riga giusta era quella vecchia, e su due modelli era l'*unità* di fatturazione stessa a essere cambiata. La migrazione fonde quindi soltanto i duplicati rigorosamente identici e si ferma **nominando** i divergenti. L'arbitrato resta umano.

### 29.4. Il formato non è un dettaglio
...
**Nessun linguaggio di query libero parte mai da un LLM.** Un catalogo di query nominate (assert di completezza al boot) è l'unico produttore di PromQL; un builder vincolato l'unico produttore di LogQL — enum di servizi chiuso, pattern di evento rigoroso, tetti di intervallo e volume come costanti. L'iniezione non può essere scritta, e Loki (con una storia di OOM sul Pi) è protetto per costruzione.

**L'autocontrollo funziona anche da cieco.** Il ciclo leader valuta i segnali aurei via Prometheus *e* sonde in-process (PostgreSQL, Redis, interruttori, la propria vitalità): con Prometheus giù i controlli interessati passano a `unknown` ma il ciclo continua — e `unknown` limita il verdetto globale a `degraded`, perché essere ciechi non è essere sani, e la cecità non è un guasto.

**Un guasto, un incidente.** Il webhook di Alertmanager (Bearer, frammenti versionati, matrice riprodotta in CI) e i verdetti critici convergono in un unico incidente per chiave di correlazione — indice unico parziale, upsert atomico sotto la concorrenza webhook-contro-leader. La diagnosi LLM è fondata sul runbook dell'avviso, limitata da un budget giornaliero atomico, e le sue raccomandazioni restano proposte: nulla viene eseguito da testo di modello.
...
**Il diagnostico legge le sue prove nel momento in cui diagnostica.** Un valore e la sua soglia sono un verdetto, non una diagnosi, e quattro incidenti di fila lo avevano mostrato: il modello rispondeva «prove insufficienti» mentre la ripartizione era in Prometheus e il percorso guasto in Loki. Ogni incidente dichiara ora una *ricetta* — le query del catalogo e gli eventi di log che vale la pena leggere per la sua chiave di correlazione, completezza verificata all'avvio — e la pompa di diagnosi raccoglie il fascicolo una volta per incidente, dopo il cancello di budget: serie ripartite, un estratto di log delimitato filtrato da una lista bianca chiusa e dal pulitore di dati personali, la versione in esecuzione e il suo tempo di attività. Ogni fonte degrada da sola: un archivio di log irraggiungibile costa alla diagnosi il suo estratto, mai la diagnosi, ed è nominato non disponibile invece di essere letto come silenzio. Ciò che il modello ha visto è conservato con ciò che ha scritto e disegnato sotto il verdetto per l'amministratore.
...
**I calcoli di tutti i giorni non aspettano uno script: sono strumenti** (ADR-318). In entrambe le modalità, un calcolo viene analizzato ad albero e valutato in decimale esatto — mai eseguito —, gli infiniti che la libreria decimale restituisce senza sollevare errori (`0 ** -1`, `ln(0)`) sono rifiutati, e un risultato che ha dovuto essere arrotondato lo dice. Una durata si conta tra due istanti assoluti, perché due date dello stesso fuso si sottraggono sull'ora dell'orologio e darebbero 24 ore per le 25 di un cambio d'ora. Una conversione legge il tasso di riferimento della BCE e ne indica la data. Ogni limite applicato è un'impostazione enunciata nella formulazione che leggono sia il manifesto del pianificatore sia lo schema ReAct. Lo script effimero che segue resta per ciò che nessuno strumento copre.
...
## 37. Tre meccanismi per una convergenza: smorzare una raffica che un tetto non vede

**Un trigger a intervallo conta dall'avvio dello scheduler, quindi periodi con un divisore comune si allineano per sempre.** È aritmetica, non carico: attività da 5, 15, 30 e 60 minuti finiscono per partire nello stesso secondo e ci restano per tutta la vita del processo. Misurato in produzione: sei attività in un secondo ogni ora, ognuna con un agente, ogni agente a vettorializzare — 11 errori su 24 chiamate, mentre un ritmo costante di quattro al minuto passava senza un solo errore.

**Tre meccanismi, tre ruoli, e distinguerli è il progetto.** Lo **sfalsamento** tratta la causa: 15 % del periodo, soglia minima di 5 s, sempre strettamente sotto il periodo perché due esecuzioni non possano né sovrapporsi né invertirsi. La **regolazione** non avrebbe cambiato nulla in questo incidente — sei chiamate non superano alcun tetto al minuto — è l'assicurazione per la crescita, da cui una finestra volutamente **breve**: un tetto al minuto non può vedere una raffica. Il **ritentativo** raccoglie il residuo.

**Nessuno dei tre è un cancello.** La regolazione compone il limitatore esistente invece di aggiungerne un secondo — una finestra scorrevole, uno script Lua, un insieme di metriche — e scade **aperta**: l'attesa è limitata, e chi ha atteso la propria parte passa comunque. La nostra stessa regolazione non deve mai essere il motivo per cui una risposta perde la memoria. Con Redis irraggiungibile la risposta è «nessun posto», subito, e la chiamata parte.

**Un ritentativo richiede una fabbrica, non un awaitable.** Una coroutine si attende esattamente una volta: una giuntura che tiene `client.aembed_query(...)` non può rilanciarla — la seconda attesa solleva invece di chiamare. E la classificazione è **strutturale**: il codice di stato letto risalendo la catena delle cause, mai il testo del messaggio, che una riformulazione del fornitore renderebbe falso in silenzio. Corollario: uno strato che esaurisce il proprio ritentativo deve **concatenare la causa**, altrimenti lo strato superiore legge un involucro muto e dichiara definitivo ciò che non lo era.
...
## 38. Verbali di riunione: la riga è il job, il modello è il contratto

**Una riunione sono ore di audio prodotte su un dispositivo che può addormentarsi, perdere la rete o essere chiuso per errore — la cattura è quindi progettata per il guasto, non per il caso nominale.** Il browser registra in brevi segmenti e carica ciascuno come richiesta a corpo grezzo appena esiste; un segmento è un file sul server, scritto con un nome temporaneo e rinominato atomicamente, così quattro worker dell'API possono ricevere fuori ordine senza mai intrecciare byte, un caricamento duplicato è una sovrascrittura innocua e rilevare una lacuna è elencare una cartella. Due sorgenti stanno dietro un'unica interfaccia: Opus tramite `MediaRecorder` dove il motore è affidabile, PCM grezzo a 16 kHz tramite l'AudioWorklet che l'input vocale usa già ovunque — l'unica via sui dispositivi Apple, il cui registratore produceva un contenitore che il backend rifiuta e si comportava male nelle app della schermata iniziale. Il formato è una funzione pura dell'ambiente, decisa una volta prima del primo byte. I caricamenti partono in ordine, ritentano con un arretramento che non rinuncia mai finché dura la registrazione e aspettano — non falliscono — mentre il browser è offline; uno stop dichiara il conteggio proprio della coda, e le sequenze che il server non ha mai ricevuto sono rifiutate per nome.

**La riga della riunione è il job durevole.** Il suo stato porta tutto il ciclo di vita — registrazione, interrotta, fermata, in elaborazione, pronta o fallita — e ogni transizione è un singolo `UPDATE` condizionale. La rivendicazione dell'elaborazione prende un lease, i battiti lo rinnovano pubblicando la fase che la pagina mostra (normalizzazione, trascrizione, sintesi, indicizzazione), un battito perduto interrompe ogni effetto successivo, e i mietitori rilanciano gli orfani fermati e i lease scaduti entro un budget di tentativi limitato. I rifiuti che non sono guasti della pipeline — un limite d'uso, nessun motore disponibile — rilasciano il job senza consumare un tentativo. E poiché un aggiornamento massivo lascia obsoleta l'identity map di SQLAlchemy, ogni lettura che lo segue fa prima scadere la sessione: la prova a runtime ha misurato uno stop che rispondeva `status: recording` prima che quella regola esistesse.

**I motori formano una catena, e la catena viene percorsa due volte.** Una prima del primo secondo, dalla sola cache delle chiavi dei fornitori: lo slot vocale dell'amministratore, poi ElevenLabs Scribe o OpenAI `gpt-4o-transcribe-diarize` — file intero, voci separate —, poi il Whisper locale. Il percorso locale decodificava una finestra di trenta secondi e restituiva in silenzio i primi trenta secondi di tutto ciò che durava di più; un comando vocale non incontrava mai il difetto, una riunione l'avrebbe incontrato sempre. L'audio lungo è ora tagliato da Silero VAD in finestre di parlato di al massimo venti secondi, ciascuna decodificata da sola, con finestre fisse quando il modello manca — mai troncamento. La catena viene ripercorsa all'elaborazione: un guasto permanente di un fornitore (chiave rifiutata, file troppo grande) passa al motore successivo entro la preferenza dell'utente; solo il silenzio, che dice qualcosa dell'audio, o un guasto transitorio, che appartiene al budget dei tentativi, ferma il percorso. L'istanza di sviluppo conservava un identificativo di chiave al posto di una chiave; senza il secondo percorso ogni riunione sarebbe fallita con una chiave OpenAI un gradino più in basso.

**Il modello è il contratto, e al modello linguistico non si crede byte per byte.** Le sezioni dell'utente — chiave, etichetta, istruzione, tipo — sono rese in un'unica chiamata a output strutturato; quando la trascrizione supera la finestra del modello, viene prima condensata per parti, così un modello da 128k produce ancora verbali fedeli per tre ore di riunione. Il modello risponde in una forma permissiva, e un passo di riparazione la ripiega nel rapporto rigoroso: le sezioni saltate tornano vuote, quelle inventate sono scartate, un contenuto nella forma sbagliata per il suo tipo è convertito, i partecipanti sono limitati alle etichette che hanno davvero parlato. Il rapporto generato è immutabile; ciò che l'utente modifica è una copia, «ripristina» ricopia, «ricostruisci» riesegue la sintesi sulla trascrizione conservata con il modello corrente. Un unico serializzatore produce il Markdown che lo spazio di conoscenza «Riunioni» indicizza (trovato per ruolo, un documento per riunione riscritto sul posto ed eliminato con essa), il contenuto a sezioni che il renderer PDF trasforma in file e l'HTML che l'e-mail trasporta — così i tre non possono mai contraddirsi.
...
## 39. Tre registri, e quello che nessuno aveva chiesto

**Un assistente che agisce deve poter dire che cosa ha fatto, che cosa ha letto e in quale turno.** Tre registri rispondono a questo e non si confondono mai: il primo porta una riga per **azione**, rivendicata prima che avvenga e chiusa solo su un esito esplicito; il secondo una riga per **consultazione**, nominata come capacità e mai come contenuto cercato; il terzo una riga per **turno**, la spina dorsale a cui gli altri due si agganciano. I loro totali non si sommano, ed è voluto: un turno può consultare cinque fonti e non cambiare nulla.

**La garanzia era strutturale, ma la porta era unica.** La registrazione è installata sulla capacità nel momento in cui viene dichiarata — un nuovo strumento non può quindi dimenticarla — salvo che una capacità raggiunta tramite il suo *client* anziché tramite la porta degli strumenti resta invisibile. Misurato in produzione su quattordici giorni: le superfici conversazionali risultavano registrate 24 volte su 24, e il lavoro fuori turno **0 volte su 228**. Nulla era guasto: i registri seguono il grafo, e quelle superfici chiamano il modello direttamente. L'elenco di ciò che l'assistente intraprende di sua iniziativa si leggeva dunque vuoto qualunque cosa facesse — la versione più netta del buco, dato che una notifica proattiva è l'unico atto di iniziativa propria che una persona sperimenta davvero.

**La risposta non è un quarto rilevatore, è una dichiarazione.** Ogni superficie che legge senza passare da uno strumento — il briefing, il debriefing di relazione, la passata periodica, gli interessi, gli spazi di conoscenza — dichiara il proprio vocabolario in un'unica tabella, e l'asserzione di avvio verifica ora le trentuno capacità là dove guardava solo gli strumenti. I ventidue moduli che importano un client di connettore sono elencati uno per uno: registratore, non-lettore con la ragione scritta, oppure debito — una tabella di debito vuota, che può solo restringersi. Il registro si **offre** finalmente anziché farsi cercare: il dominio degli agenti importa già quello delle relazioni, quindi un import di ritorno chiuderebbe un ciclo che un import locale si limiterebbe a nascondere.

**Dove va un euro è dichiarato, mai dedotto.** La contabilità è ambientale — un nodo spende attraverso un contesto pubblicato da un antenato — sicché rispondere «è contato?» leggendo i file ha prodotto nove conclusioni false in una sola sessione, in entrambi i sensi. Una tabella nomina dunque i quarantasei siti di chiamata e il registro che ciascuno raggiunge, e una guardia che legge le chiamate tramite AST ne ha corretti sei. Contare è però solo metà di un tetto: l'altra metà è **chiedere prima**, e cinque di quei siti non erano limitati da nulla. Tre forme li avevano prodotti — un registro che nominava un involucro anziché la porta, una guardia che usciva prima di chiedere, e uno stesso rifiuto reso in due modi. Un rifiuto segue ora il proprio trasporto e mai il proprio verdetto: una richiesta solleva, un compito in background rinuncia e si registra come **saltato**, mai come *fallito*.

**Due punti di ingresso che servono uno stesso schermo fanno un solo atto di lettura.** Il cruscotto recupera schede e sintesi in parallelo, e ciascuno costruiva il lotto delle nove sezioni per conto proprio: su sette giorni, 151 costruzioni di cui 44 duplicate — il 39 % dei caricamenti, e 44 su 44 concorrenti. Ogni connettore veniva aperto due volte e due lotti di consultazioni depositati per un solo atto di lettura. La causa era una domanda, non una riga di codice: la sintesi chiedeva «questo lotto contiene qualcosa di interessante?» invece di «questo lotto è stato costruito?», e la prima non distingue una cache fredda da una giornata tranquilla. Chi chiede per primo costruisce e gli altri ricevono lo stesso oggetto — nel processo e tra i quattro worker di un'istanza di produzione, tramite una rivendicazione rilasciata solo dal suo proprietario.

**Un'estrazione è completa, oppure non è un'estrazione.** I cinque record scaricabili portavano un tetto di righe misurato e non arbitrario: sul Raspberry Pi di destinazione, cinque fonti a cinquemila righe toccavano un picco di 33,9 MB. Il vincolo era reale — l'intero documento veniva assemblato in memoria — ma applicato alla variabile sbagliata. **Scarsa era la memoria; limitata era la verità**: 49 195 righe reali contro mille per fonte, ossia il 97,9 % del record d'inferenza assente, sotto un'intestazione che diceva il vero annunciando «troncato». Un cursore lato server limita ora il buffer; il conteggio è esatto — un aggregato sulla stessa statement che il corpo percorre — e pubblicato prima della prima riga, sicché un'estrazione senza limite superiore ne riceve uno: l'istante della sua generazione, nominato nell'intestazione anziché fissato in silenzio.

**L'assistente legge i propri registri** (ADR-318). A «cosa hai fatto per me questa settimana?», uno strumento di lettura risponde dai due registri, con totali esatti e lo stesso filtro di autore delle schede — ciò che la persona ha chiesto, ciò che LIA ha intrapreso di propria iniziativa — invece che dal ricordo di una conversazione. Un registro serve così due lettori senza due implementazioni: la schermata che lo disegna e la risposta che lo riassume.

## 40. Un debriefing per relazione: ciò che dieci sezioni non dicono

**Una scheda impila dieci sezioni, e nessuno legge dieci sezioni.** Ciò che un lettore cerca per primo — a che punto sono con questa persona, e che cosa affrontare — è una sintesi che nessun aggregato produce. Viene quindi scritta dal modello in cima alla scheda: ciò che resta aperto, ciò che fare dopo, ciò che vale la pena ricordare. La sua fabbricazione è **pigra**: nasce all'apertura della scheda, al massimo una volta per giorno locale del lettore, mai per opera di uno scheduler — il numero di relazioni non è limitato — e mai durante un turno di chat. Solo tre ricostruzioni sono legittime: la lingua, l'ampiezza richiesta e una richiesta esplicita.

**Due assemblaggi avrebbero creato due autorità su chi è quella persona.** L'assemblaggio delle prove esisteva già, dentro lo strumento che risponde in chat; è stato dunque estratto in un modulo proprio, di cui lo strumento è diventato il primo consumatore. Un refactor su un percorso in produzione non si verifica leggendo: un file dorato è stato catturato **sul codice precedente**, diciotto casi di ampiezza, payload e messaggio confrontati byte per byte. La metà del fornitore non viene più letta per intero per essere poi scartata — fino a undici chiamate esterne venivano fatturate contro una selezione che il lettore aveva già fatto — e il restringimento ha richiesto un terzo stato: «non ho guardato, di proposito» non è né «non ho trovato nulla» né «non ho potuto guardare».

**Nulla è inventato, e un fallimento non distrugge nulla.** Nessuna prova conclude *vuoto* senza chiamare il modello, e un aggiornamento fallito **conserva il testo precedente** sotto una riga che lo segnala: sostituire una sintesi utilizzabile con un pannello vuoto trasforma «non ho potuto aggiornare» in «non c'è nulla». La rivendicazione è una sola istruzione SQL, e ogni chiusura scrive soltanto le proprie colonne — pronta, vuota, fallita — perché una chiusura unica sovrascriveva il corpo al primo fallimento. Ciò che la stesura è costata è memorizzato con essa e mostrato sotto; essendo un costo nullo un'affermazione, una riga muta non mostra nulla anziché «0,00 €».
...
## 42. L'anatomia di un processo: ciò che carica è dichiarato, misurato e limitato

Un server API è più processi: un supervisore che non serve alcuna richiesta e worker che servono tutto. Ciascuno trattiene memoria per ragioni diverse, e «il container consuma cinque gigabyte» non dice nulla di utile finché non si sa quale processo trattiene cosa. LIA applica qui una regola sola: **ciò che un processo carica è dichiarato, misurato per processo e limitato là dove si moltiplica**. Ogni voce è stata misurata sull'host di destinazione, in un processo usa e getta, prima di essere toccata — e tre misure hanno contraddetto la prima lettura, che è esattamente il motivo per misurare.

Il supervisore non carica l'applicazione. La riga di comando di uvicorn importa l'applicazione nel processo padre per produrre un errore di import precoce, poi la scarta — un padre che si limita a sorvegliare i suoi worker tratteneva così diverse centinaia di megabyte per tutta la vita del container. `python -m src.serve` guida la stessa catena `Config → Server → Multiprocess` con le stesse opzioni, senza quell'import; una guardia AST vieta al modulo qualsiasi import da `src`, e l'errore precoce è reso dal controllo di salute che il deploy già legge. Nello stesso spirito, una libreria pesante — rendering PDF, calcolo tabellare, motori audio — è importata dove serve e mai all'avvio: un elenco dichiara il costo misurato di ciascuna, e una guardia rifiuta qualsiasi import di primo livello sotto `src/`, anche nascosto in un `try` di modulo.

Ciò che un singleton carica si moltiplica per il numero di worker. La trascrizione vocale teneva un modello residente per ogni lingua mai richiesta, in un singleton per processo; la cache è ora limitata (`VOICE_STT_MAX_RECOGNIZERS`, uno per impostazione predefinita), si espelle il meno usato di recente, nulla è caricato alla costruzione, un'espulsione restituisce la memoria al sistema — misurato — e una decodifica in corso conserva il proprio riferimento. E ogni worker pubblica ciò che trattiene: `lia_worker_memory_bytes{kind}` è letta da ogni worker nel proprio `/proc/self/status`, una serie per worker vivo che scompare con lui, tracciata su due pannelli e sorvegliata dall'allarme `ApiWorkerMemoryHigh`, che nomina il processo. È la risposta a un limite strutturale: cAdvisor vede il container, e la modalità multiprocesso di prometheus_client non esporta alcuna metrica di processo.

Il budget di connessioni ha un pavimento, non solo un tetto. L'audit F004 limitava il picco — ciò che i pool possono aprire; ogni connessione persistente è anche un backend Postgres inattivo che trattiene sette megabyte, accanto agli `shared_buffers`. `ConnectionBudget.persistent_total` nomina quel pavimento, e una guardia legge insieme il compose (limite di memoria, `shared_buffers`) e i due profili `.env` consegnati — quello completo e quello minimo dell'auto-hoster, con le chiavi assenti risolte ai valori predefiniti del codice come all'avvio: il pavimento deve restare sotto la metà del limite, perché il database tenga l'altra metà per la sua cache. Profilo consegnato: cinque connessioni persistenti e quindici in overflow per worker, limite di due gibibyte. Ciò che il lotto lascia aperto è scritto: la crescita su più giorni di un worker senza trascrizione — ora misurabile — e la conservazione dei checkpoint LangGraph.

## 43. La modalità Live: due intelligenze, una cucitura — e una politica per modalità

Una sessione da voce a voce gira su un modello live che la persona collega con **la propria chiave** — una categoria di connettore a sé, `live`, additiva: più chiavi possono essere attive, le sessioni si aprono su quella scelta, e scegliere un modello *è* scegliere il suo fornitore. Nulla di ciò che il fornitore fattura è contato, memorizzato o mostrato dalla piattaforma. L'API conia una credenziale monouso e genera la configurazione; il browser la ripete tale e quale e apre da solo la connessione, così che **l'audio non transita mai dall'API**. Quattro fatti sono stati misurati sul fornitore reale prima di congelare il design, e ciascuno è diventato una regola: il vincolo di una credenziale non blocca l'istruzione di sistema; una credenziale monouso consumata non può riconnettersi (ogni riconnessione ne conia una fresca per lo stesso record); il fornitore accetta in silenzio un nome di voce sconosciuto (la voce è validata contro una lista incorporata e datata); un WebSocket grezzo del browser è accettato da un solo metodo.
...
Una **sessione diretta** è la linea del telefono nel browser: la voce tiene da sola gli strumenti di lettura di LIA — l'insieme derivato del telefono meno gli interruttori della persona — dietro un'unica porta degli strumenti sotto un budget per sessione, e non delega mai. La premessa del proprietario è stata misurata su una sessione reale: cinquantacinque dichiarazioni pesano il novanta per cento della configurazione e il primo turno fattura nove volte quello delegato — una sessione diretta compra latenza, mai risparmio. I rapporti d'uso del fornitore sono ripiegati nel browser e tariffati con la tariffa che l'avvio pubblica, disegnati sotto l'ultimo sottotitolo; un costo che richiederebbe una tariffa non dichiarata è non disponibile, mai parziale; un modello live è offerto solo se la tabella tariffaria lo dichiara. Due trappole misurate vivono nel lettore: il dialogo affettivo di Gemini chiude il socket alla prima cosa a cui il modello deve rispondere, e Chrome 152 ripete il primo blocco di rendering di un frammento pianificato — il lettore PCM è un solo AudioWorklet che legge una coda, mai un nodo sorgente per frammento.

Le stesse due modalità hanno poi raggiunto il telefono (ADR-301). La chiamata del titolare riportava la conversazione alla sua *fine* mentre il browser delegava ogni richiesta *durante* — due verità per una stessa parola. Una sessione vocale ha ora una **modalità** e un **portatore**, e la politica segue solo la modalità: `delegated` («Live»), ogni richiesta un turno di chat della persona nel momento in cui la dice; `direct` («Live diretto»), la voce legge, non agisce su nulla, e le parole sono riportate alla fine come turno della persona. **Una sola chiusura** serve entrambe le linee, chiamata dalla rotta di fine del browser e dal webhook post-chiamata del telefono; ciò che le linee condividevano è uscito da entrambe in `domains/voice_sessions/`, e la modalità Live del telefono è la delega del browser lato server — uno strumento asincrono del fornitore con il nome e lo schema del browser, un ponte che rispecchia quello TypeScript regola per regola (la richiesta più recente vince tramite un marcatore Redis che il ponte in corso sonda; una domanda che LIA ha posto è il risultato e la richiesta successiva riprende l'esecuzione che l'ha posta). Misurato sul vero motore del fornitore, senza telefono: la voce annuncia la chiamata e continua a parlare, una risposta tardiva oltre il limite del fornitore è persa, quindi il ponte risponde prima. La modalità effettiva è derivata e pubblicata — Live esiste solo dove l'host di richiamo è pubblico — e un runner non tiene alcuna sessione di database durante il turno: 8,9 s di `idle in transaction` prima, nessuna dopo.
...
I 317 ADRs documentano non solo le decisioni prese, ma anche le alternative scartate e i compromessi accettati. I ~32.725 test in 1.974 file, la CI/CD completa e il MyPy strict non sono metriche di vanità — sono i meccanismi che permettono di far evolvere un sistema di questa complessità senza regressioni.
...
*Documento redatto sulla base dell'analisi del codice sorgente (`apps/api/src/`, `apps/web/src/`), della documentazione tecnica (680+ documenti), dei 317 ADRs e del changelog (da v1.0 a v1.47.4). Tutte le metriche, versioni e pattern citati sono verificabili nel codebase.*
