...
30. [Travail visible, apprentissage gouverné](#30-travail-visible-apprentissage-gouverné)
...
39. [Trois registres, et celui que personne n'avait demandé](#39-trois-registres-et-celui-que-personne-navait-demandé)
40. [Un débrief par relation : ce qu'une fiche ne dit pas en dix sections](#40-un-débrief-par-relation--ce-quune-fiche-ne-dit-pas-en-dix-sections)
...
| Transparence totale | 589 métriques Prometheus, debug panel embarqué, suivi token par token |
| Fiabilité en production | 317 ADRs, ~32 725 tests collectés par pytest sur 1 974 fichiers, observabilité native, HITL à 6 niveaux |
...
| Métrique | Valeur |
|----------|--------|
| Tests | 32 725 collectés par pytest sur 1 974 fichiers de test + 9 160 tests vitest côté frontend (seuils de couverture verrouillés, ADR-116) |
| Fixtures pytest | 1 082, dont 48 partagées via conftest |
| Documents de documentation | 703 |
| ADRs (Architecture Decision Records) | 317 |
| Métriques Prometheus | 589 définitions |
...
|----------|---------|-------------|
| OpenAI | GPT-6 (astra, sol, luna), GPT-5.6 (terra, sol, luna), GPT-5.5, GPT-5.4 (+ mini), GPT-5.x (+ mini/nano), GPT-4.1, GPT-4o, o3/o4-mini | Prompt caching natif, Responses API, reasoning_effort |
...
| Google | Gemini 3.8/3.7/3.6/3.5 Flash, Gemini 3.1 Pro, Gemini 2.5 Pro/Flash | Multimodal, embeddings duaux |
| DeepSeek | deepseek-flash (V4.1), deepseek-v4-pro (V4), deepseek-chat (V3), deepseek-reasoner (R1) | Coût réduit, reasoning natif |
...
│   │   ├── nodes/                # Nœuds du graphe : pipeline, ReAct, HITL, initiative
...
│   │   ├── prompts/v1/           # fichiers .txt de prompts versionnés
│   │   ├── graphs/               # Un builder d'agent par domaine
...
│   ├── sub_agents/               # Sous-agents éphémères, délégués et en lecture seule
...
└── infrastructure/               # Couche transversale
    ...
    └── observability/            # Métriques Prometheus par sous-système, tracing OTel
```

### 3.2. Chaîne de priorité de configuration

Un invariant fondamental traverse tout le backend. Il est appliqué partout, parce qu'une divergence entre une constante et la configuration réelle de production produit un bug silencieux :
...
**Architecture** : 5 nodes custom dans le graph LangGraph parent (pas un sous-graphe) :
...
La dépense d'un tour obéit à une loi de conservation (ADR-256) : le temps de raisonnement et le temps des outils sont deux budgets nommés, jamais un seul — une délégation (sous-agent, tâche MCP itérative, navigateur) ouvre sa propre boucle LLM derrière un unique appel d'outil, et à sa borne pipeline elle vaudrait à elle seule 100 % du budget de raisonnement. Chaque appel d'outil porte une borne individuelle issue des mêmes familles de timeout que le mode Pipeline, choisie pour ne jamais être plus stricte que celle que la couche inférieure applique déjà, et un timeout levé par l'outil lui-même est attribué à l'outil, pas à notre propre coupure. La condition d'arrêt reste un prédicat unique à deux lecteurs : le routeur l'applique pour décider, le nœud de finalisation pour expliquer.
...
Ce qu'un outil renvoie est **projeté dans le contexte de la boucle item par item, sous un budget de tokens** (ADR-286). La liste d'items la plus lourde d'un résultat est paginée aux frontières d'items — un item admis est complet, le bloc reste du JSON valide, les scalaires voyagent entiers et le premier item passe toujours — sous `min(REACT_TOOL_RESULT_MAX_TOKENS, fenêtre × REACT_TOOL_RESULT_WINDOW_FRACTION)`, la fenêtre étant celle du slot ReAct. Une coupe est **dite** au modèle après la balise de contenu externe (combien sur combien, et la voie vers le reste) et comptée par outil, de sorte qu'une boîte de messages entiers n'atteint jamais le modèle sous la forme d'un vidage coupé au milieu des en-têtes du premier. L'arbre brut du fournisseur ne quitte jamais le builder d'e-mails, et un outil dont seul l'exécuteur du pipeline injecte les données se déclare réservé au pipeline plutôt que d'être offert à une boucle qui ne peut pas l'exécuter.
...
Une boucle est **jugée sur son résultat** (ADR-310). Elle s'arrêtait dès que le modèle n'appelait plus d'outil, si bien que la persévérance dépendait de la façon dont un modèle configuré lisait une phrase. Tout fait que la boucle a entrepris d'obtenir — la demande de la personne et chaque vérification croisée qu'elle a lancée — finit désormais obtenu ou déclaré dans un bloc `<unresolved>` qui clôt le brouillon, avec les échelons tentés : l'appel corrigé, puis une autre source nommée `(fallback)`, puis la déclaration. Un seul prédicat, `should_recover`, lit la déclaration et appelle celui qui décide de l'arrêt (`react_exit_reason`) au lieu de le recopier ; il route vers `react_recovery`, un cinquième nœud qui n'appelle aucun modèle : il retire le brouillon du fil au moment même de la passe, et chaque appel suivant du tour reçoit, de façon transitoire — jamais écrite dans `messages` —, le brouillon et une consigne placés juste après lui. Les passes sont bornées (`REACT_RECOVERY_PASSES_MAX`), comptées par issue, et une passe qui finirait sans réponse utilisable rend le brouillon plutôt que de l'échanger contre rien. En amont, un outil ne remplace jamais une valeur qu'il ne sait pas lire : la météo refuse une date illisible avec le format attendu et la date du jour, sous un contrat publié par une seule constante que lisent le manifeste et le schéma de l'outil — la boucle lie le schéma, jamais le manifeste.

Un échec d'outil s'y lit **structurellement** (ADR-303) : le corps d'un `ToolMessage` porte la prose de l'outil, donc le seul verdict honnête est un marqueur que le message porte lui-même — `status="error"`, mesuré comme survivant au checkpoint. Un prédicat de succès unique, `core/tool_outcome.explicit_success`, sert la boucle, le registre des consultations et les métriques : un échec déclaré, comme un résultat vide, n'achète plus d'itération. Et quand une personne choisit les échanges fréquents dans ses réglages (ADR-311 ; `REACT_CROSS_TURN_CACHE_ENABLED` ne fixe que le défaut d'un compte qui n'a pas choisi, ADR-308), la boucle lie tous les outils dans leur ordre d'enregistrement et place le contexte du tour après la question — dans la forme que chaque fournisseur accepte, déclarée et vérifiée au démarrage —, pour que le préfixe d'un tour soit celui du suivant : mesuré sur 396 tours réels, de 18 à 42 % d'économie par tour selon le mécanisme de cache, un surcoût sur un modèle sans cache, et un repli sur la sélection par pertinence compté quand le catalogue dépasse le plafond ou la fenêtre. L'historique tombe alors lui aussi par blocs, ancrés sur le compteur de tours, pour que chaque tour prolonge le précédent : les trois vont ensemble parce que, mesuré, aucun ne paie seul, et les échanges ponctuels gardent la sélection par pertinence, dont le coût ne dépend pas de l'écart entre deux tours.
...
**La facture appartient au renderer, le sens au modèle (ADR-274).** Le schéma que le slot rédacteur remplit est *sémantique* : il nomme ce qu'une chose est — un titre, une séquence ordonnée, une citation, un encadré, une ouverture de partie, une comparaison à deux colonnes, un tableau légendé — et jamais comment la dessiner. La décision de mise en page appartient au renderer, qui l'exprime avec les mécanismes natifs du format plutôt qu'en les imitant : styles nommés, champs `PAGE`/`NUMPAGES` et définition de numérotation multiniveau côté Word, donc un sommaire et des numéros que Word recalcule lui-même ; mises en page et espaces réservés du gabarit côté PowerPoint, sur une scène 16:9 ; tableau nommé sur colonnes typées côté Excel, donc filtre et tri gratuits ; signets, liens et numéros de page exacts côté PDF, obtenus en paginant le corps une fois puis en concaténant le liminaire devant lui. Ce partage évite le catalogue de gabarits que le modèle aurait à choisir — une décision de dessin qu'il ne sait pas juger — et laisse le rendu s'améliorer sans toucher au schéma.

**Le texte est mesuré avant d'être posé.** PowerPoint ne calcule aucun ajustement automatique à l'ouverture d'un fichier, et l'ajustement offert par la bibliothèque se trompe d'un facteur deux : le renderer mesure donc lui-même, avec un estimateur calibré contre PowerPoint — un glyphe pleine chasse valant un em entier, un glyphe latin une fraction mesurée. Ce qui ne tient pas est d'abord réduit jusqu'à un plancher de lisibilité, puis scindé en diapositives « Titre (2/3) » ; une puce trop longue est coupée à une fin de phrase. Rien n'est jamais rogné, parce qu'un texte coupé à l'écran est une information perdue sans avertissement. Le même principe gouverne l'appel au modèle : une réponse que le fournisseur déclare tronquée est refusée avec son budget nommé (ADR-275), jamais refermée pour ressembler à un document complet.

La génération d'images suit la même règle d'offre déclarée (ADR-305). Une **famille** — OpenAI GPT Image, Qwen Image 3.0 — déclare ce qu'un modèle accepte : ses qualités, ses tailles (une liste fixe ou une enveloppe d'aire), ses paliers de facturation, et si l'image de référence d'une retouche se facture à l'image. Chaque surface lit cette seule réponse : un modèle qu'aucune famille ne déclare n'est ni tarifé, ni proposé, ni choisi, ni exécuté. Un client par fournisseur exécute le modèle configuré — la retouche OpenAI passe par `images.edit` sur ce modèle, là où un détour par l'API Responses exécutait le modèle par défaut du SDK et un modèle texte que personne ne comptait —, un résultat livré par URL est téléchargé aussitôt sous une liste d'hôtes, borné et vérifié, et la préférence de la personne est une intention résolue à l'exécution vers l'offre du modèle (la qualité la moins chère offerte, la taille de même orientation et d'aire la plus proche) puis publiée comme valeur effective. Le format choisi est appliqué une fois pour tous les fournisseurs, en convertissant l'image avant son stockage.

Sur option de la personne, la description d'une **nouvelle** image peut être réécrite avant l'appel par un slot dédié, selon les conseils que publie chaque fournisseur (ADR-315). C'est une amélioration, jamais une porte : un texte cité entre guillemets doit revenir intact, et une réécriture vide, trop longue ou en échec laisse partir la description d'origine ; une retouche n'est jamais réécrite ; l'appel est facturé et audité comme les autres, et la galerie garde les mots de la personne. L'exploitant peut retirer l'option pour toute l'instance, et l'écran des options publie cet état plutôt que de proposer un réglage sans effet.

### 5.6. Ce que la personne garde : fichiers produits et réponses conservées (ADR-279, ADR-282)

Un artefact et une réponse suivent deux cycles de vie différents une fois qu'ils existent, et les deux appartiennent à la personne plutôt qu'à la conversation. Un fichier produit — une image, un document, une capture de navigateur — est une pièce jointe tamponnée de son **origine**, listée dans une galerie qui lui est propre avec un total exact et une échéance de conservation visible ; effacer une conversation retire ce que la personne a envoyé et jamais ce que LIA a produit, parce qu'une purge retire ce que sa famille déclare, pas ce qui lui ressemble.
...
Ce qui a été produit se retrouve aussi **en le demandant** (ADR-318) : un outil de lecture cherche dans la galerie vivante par mots du titre, famille et période, et montre les fichiers trouvés comme les cartes mêmes du chat, avec le total exact. Une image générée se **partage enfin avec une connexion** comme une copie (ADR-316) : le clic vaut confirmation, sans brouillon ni modèle ; deux plafonds quotidiens sont sérialisés par expéditeur sous un verrou transactionnel ; la copie du fichier, la ligne de galerie du destinataire et le registre des partages s'écrivent dans une même transaction ; puis le chat du destinataire est atteint au mieux, sur une session à part. Le commentaire d'un tiers y est cité **à la lettre** : neutralisé par références numériques de caractères, parce que le chat rend le Markdown avec HTML brut et images `https:`, et qu'un échappement par antislash serait rendu comme une formule.
...
**Ce qui n'a rien à confirmer.** Une action ne demande rien quand rien ne peut la détourner. L'e-mail à soi-même (ADR-314) n'a pas de destinataire en paramètre : l'adresse est celle que le fournisseur de la boîte connectée déclare, ou, sans boîte, l'adresse vérifiée du compte par le relais de LIA — aucun texte tiers ne peut donc la rediriger. L'outil déclare `reversible` avec sa raison écrite, si bien qu'une routine peut envoyer son résultat là où un brouillon est refusé. Une adresse tapée qui se trouve être la sienne passe, elle, par le brouillon : un destinataire qui sauterait parfois la confirmation serait une cible d'injection.

### 9.3. Critique de brouillon : une description, une question par brouillon

Un brouillon à confirmer et le compte rendu de son exécution sont **décrits une fois** — une spécification de carte faite de lignes libellées, de notes et de blocs, construite par un renderer par type de brouillon depuis le registre d'affichage — et **dessinés par surface** : dans le chat, le même balisage `lia-card` qu'un e-mail ou un événement (en-tête, illustration, champs avec leurs icônes, corps en bloc) ; sur un ticket, sur un canal externe ou sous l'affichage `markdown`, le Markdown que le commentaire de ticket sait aplatir. La surface est décidée par le run, jamais devinée par un appelant. Un compte rendu dit ce qui a été fait, à qui et avec quoi : chaque ligne d'un lot porte les champs clés que son type déclare et un extrait borné du texte envoyé, cité avec les guillemets de la langue. La comparaison avant/après des mises à jour, les avertissements d'irréversibilité, les libellés i18n et les liens cliquables font partie de cette description.

Un tour qui prépare **plusieurs brouillons indépendants** — deux e-mails, un événement et une tâche, plusieurs appels d'outil d'une même itération ReAct — les soumet **un brouillon par interruption** : la suite s'ouvre sur la liste de tout ce que le tour a préparé, chaque brouillon a sa carte, sa question et sa modification, la position est dite (« Brouillon 2 sur 3 »), et rien ne s'exécute avant la dernière réponse ; les décisions banquées sont ensuite exécutées ensemble, chacune sous son propre type, et un brouillon annulé est rapporté plutôt que perdu. Seul un lot que la personne a **pré-approuvé comme liste** — les items d'une étape FOR_EACH approuvée dans ce tour, d'un seul type — garde sa confirmation groupée. Un ticket suit la même identité (le brouillon à l'écran), et un nouveau tour commence toujours sans brouillon en relecture.
...
```
PostgreSQL + pgvector
├── memories                                        → Mémoire long terme : faits, préférences, profil psychologique
├── rag_documents + rag_chunks                      → Espaces de connaissances, recherche hybride
└── AsyncPostgresStore (user_id, "context", domain) → Contexte des outils (Data Registry)
```
...
**Une seule porte de recherche** (ADR-313). Une requête est plongée comme une *clé* — sans les motifs de trivialité conversationnelle, qui entrent en collision avec des noms réels et des questions courtes — avant qu'une session ne s'ouvre, puis les souvenirs vivants de la personne au-dessus du plancher de pertinence sont servis, éventuellement restreints à des catégories. Tous les appelants passent par elle : les faits mémoire du planificateur, de l'initiative et de la résolution de références, le rappel 360° d'une personne, et les recherches du téléphone, qui répondent à « que sais-je de X » avec X plutôt qu'avec les derniers souvenirs.

**Et consultée exprès.** Un domaine routable porte un outil de lecture, si bien que la boucle ReAct et le planificateur peuvent chercher un sujet que le message ne nommait pas — l'expéditeur d'un e-mail qu'ils viennent de lire, un lieu cité dans un document. Ses bornes sont publiées depuis les sources qu'il applique (le vocabulaire des catégories *est* celui qui est stocké, le plafond de résultats *est* le réglage), la préférence de la personne le refuse comme elle refuse l'injection, un souvenir sensible revient marqué de sa nuance d'usage, et un échec est classé, jamais « rien en mémoire ». La recherche hybride BM25 + embeddings vit là où elle sert : les RAG Spaces (section 17).
...
**Discipline de dédoublonnage** : pas de garde-fou write-time. À la consolidation, `STEP 1` impose un scan pairwise explicite qui fusionne les doublons sémantiques, et `STEP 5` regroupe activement les L1 convergentes en patterns L2.
...
**Les carnets se consultent aussi exprès** (ADR-318). Au-delà de l'injection, un outil de lecture — dans les deux modes — cherche dans les carnets de la personne un sujet trouvé en chemin, sur les mêmes vecteurs, au plancher de similarité *configuré* plutôt qu'appris, et n'en sert que les directives, motifs et facettes : l'observation `L0` brute, pas encore consolidée, n'en sort jamais. La préférence de la personne le refuse comme elle refuse l'injection.
...
**Une famille de modèles se déclare une fois, et une réponse courte se demande sans raisonnement** (ADR-285). La *famille* d'un modèle est la forme de son API de raisonnement — une bascule et une échelle d'effort chez DeepSeek, un budget chez Gemini 2.5, un niveau chez OpenAI — et elle se dérive d'une seule déclaration dans `reasoning/profiles.py` que l'adaptateur, le validateur et le détour de sortie structurée lisent tous : trois tests de préfixe privés ont un jour laissé passer le modèle phare renommé d'un vendeur dans les trois à la fois, sans échelle proposée ni interrupteur envoyé. La même couture sert les appelants qui ont besoin de deux phrases : sur un modèle qui réfléchit par défaut, le fournisseur facture la réflexion *dans* `max_tokens` (OpenAI, Anthropic, Gemini et DeepSeek le font tous), si bien qu'un petit budget achète du raisonnement caché et aucune réponse. `short_answer_config` déclare donc *aucun raisonnement* partout où le profil résolu peut le couper et n'y garde son petit budget que là ; ailleurs le budget du slot reste, car un plafond qui inclut la réflexion n'est pas un plafond sur la réponse. Le plafond de sortie d'une famille, de même, se lit dans la ligne de catalogue et non dans un littéral.
...
La convention est devenue **une mise en page pour tous les mécanismes de cache** (ADR-309) : le stable avant le volatil, une seule lecture de la frontière (`core/prompt_layout.split_at_marker`), et aucune écriture de cache que personne ne relira. Un appel unique — analyseur, initiative, extractions de mémoire, d'intérêts et du journal — envoie sa partie fixe en message système, là où chaque adaptateur peut la marquer ; les fournisseurs à points d'arrêt reçoivent le leur à la fin du préfixe statique quelle que soit la forme du système (un système en liste plaçait le point d'arrêt sur les données du tour : zéro lu, tout réécrit à 1,25) ; ceux qui n'ont qu'un cache explicite sont marqués explicitement ; une requête qu'aucune boucle d'outils ne prolongera n'écrit que ses points d'arrêt déclarés ; et la réponse envoie la conversation une seule fois, en messages entiers. Ce que chaque génération de modèle accepte — échantillonnage, choix d'outil forcé, six formes de réflexion, profondeur implicite, blocs de réflexion liés à leur conversation — est mesuré sur l'API et déclaré dans une table unique (`core/claude_surface.py` pour Claude) que lisent les règles de raisonnement, l'adaptateur, la sortie structurée et l'administration. Un modèle sans cache paie exactement ce qu'il payait : l'optimisation vise la moyenne des mécanismes, jamais un modèle.
...
Pour l'e-mail, ce modèle unifié est mesuré, pas supposé : les trois normalizers produisent le même vocabulaire `EmailMessage` — un corps en texte, jamais en HTML, dont la citation de l'historique et la signature sont retirées au bord du client (en six langues, mesuré sur un corpus de 48 corps) — et aucun ne fabrique plus le format d'un autre. L'outil `get_emails_tool` choisit ensuite ce qu'il sert selon la question, avec son coût annoncé : `metadata` liste sans corps, `full` lit un corps propre paginé au paragraphe, `summary` raisonne sur un **condensé par message** (l'essentiel, les points clés, les actions, la catégorie, l'importance) calculé une fois par un petit modèle sans raisonnement, mis en cache trente jours et refusé par les plafonds de dépense — jamais inventé quand le modèle échoue. C'est ce qui permet « résume mes non lus » ou « une synthèse des newsletters de la semaine » sur vingt messages sans les charger tous, pour Gmail, Outlook et Apple pareillement (ADR-287).
...
Pour Google et Microsoft, le consentement se regroupe par compte fournisseur au lieu de se répéter pour chaque service. Un même droit détient les permissions accordées et le cycle de renouvellement ; Gmail, Agenda, Drive et leurs équivalents Microsoft restent des capacités distinctes que la personne peut activer ou déconnecter. Le retour OAuth vérifie l'identité et l'émetteur attendus, l'état et PKCE, sans substituer silencieusement un autre compte.

**Aucune transaction ne reste ouverte pendant un appel réseau** (ADR-304). Un client connecteur construit sur la session de son appelant obligeait cette session — et sa transaction — à rester ouverte pendant que le fournisseur répondait : mesuré en production, jusqu'à 863 secondes `idle in transaction` sur le balayage des réveils poussés, de 11 à 330 secondes dans le briefing, le heartbeat ou la vue relations. Un client n'est donc jamais construit sur `ConnectorService(db)` — une garde nomme l'argument du constructeur, sans liste d'exceptions — ; une porte unique ouvre le client actif d'une catégorie dans une session courte et le ferme sur tous les chemins ; un job qui garde sa session committe ses lectures avant chaque attente ; chaque opération de connecteur du tour de chat termine sa transaction, et le tampon d'usage d'une clé API est une fusion JSONB atomique à part, sous un délai de verrou borné. Les travaux de fond suivent la même règle : un rappel est réclamé seul, committé, puis notifié sans transaction ; le flux Drive poussé avance par comparer-et-échanger, par pages bornées en nombre et en temps ; et seul le worker leader pose le webhook Telegram.

### 13.4. Deux chemins d'authentification

Tous les connecteurs ne demandent pas un compte. Un **connecteur OAuth** détient les identifiants personnels de l'utilisateur : Gmail, Agenda, Contacts, Drive. Un **service à clé de plateforme** n'a aucune donnée par utilisateur — l'utilisateur n'a rien à activer, et la clé appartient à l'installation : Itinéraires, Lieux, Météo, Environnement. `ConnectorType.uses_global_api_key` porte cette distinction, et la base des outils choisit le chemin d'identifiants d'après le type **résolu**. Une même catégorie fonctionnelle peut donc mélanger les deux : la météo accepte un fournisseur à clé personnelle comme un service de plateforme, sans que l'appelant sache lequel répond.
...
La distinction décide aussi de ce qui appartient à un **compte**. Cinq connecteurs ne demandent rien à la personne — Wikipédia, la navigation de pages, Lieux, Météo, Environnement — ils appartiennent donc à l'instance : aucune ligne par compte, rien dans les réglages, et un seul prédicat (`connectors/keyless.py`) décide s'ils servent tout le monde — un type que l'administrateur a coupé, une clé de plateforme absente ou la navigation désactivée le retient pour tous. Dans la catégorie météo, le fournisseur de la personne gagne et celui de Google est le défaut ; un test tient les cinq hors de la liste d'activation des réglages (ADR-307).
...
**Le téléphone comme canal (ADR-290).** La carte de confirmation protège un tiers qui n'a rien demandé ; quand LIA appelle *la personne elle-même*, celle qui confirmerait est celle qui décroche — à condition que le numéro soit prouvé le sien. L'identité est donc un numéro déclaré dans les réglages, affiché en entier, puis vérifié par un appel où LIA lit un code que la personne retape (essais bornés, code éphémère lié au numéro, comparaison en temps constant, le plafond horaire de tout appel payant) ; jamais une reconnaissance par nom, et l'outil d'appel tiers refuse ce numéro. Le **même** agent vocal sert trois mandats — celui du tiers cuit dans l'agent, celui du titulaire et celui de la vérification envoyés par un override par appel rendu côté serveur — pour que rien du contexte de la personne ne soit jamais cuit dans un agent qui appelle aussi des inconnus ; et ce que l'agent *a dans la voix* (modèle, langue, voix, format audio, plafond de durée) appartient au portail du fournisseur, jamais à un réglage, parce que le fournisseur fusionne un PATCH avec ce qu'il stocke et qu'un modèle épinglé heurtait l'effort de raisonnement du portail à chaque synchronisation. `call_me` n'a pas de carte par construction, ce qui permet aussi à une routine de le planifier. L'appel emporte le contexte du chat sous un budget de jetons (mémoires, agenda, rappels, boucles ouvertes, derniers échanges — un commutateur le coupe) et la personnalité configurée pour l'assistante, chaque lecture consignée au registre des consultations. Derrière un second drapeau, l'agent **lit LIA en direct** pendant l'appel, et n'agit sur rien : l'ensemble d'outils est une règle sur le catalogue plutôt qu'une liste — tout outil qui ne fait que lire, d'un domaine que le téléphone offre, dont les paramètres obligatoires se disent à la voix (55 outils sur 22 domaines, plus un rappel natif de la mémoire) — rattaché à l'agent le temps de l'appel du titulaire, puisque le fournisseur refuse des identifiants d'outils dans un override par appel, et chaque résultat est réduit à ce qu'une voix peut dire avant la pagination item par item (un identifiant, un lien, une structure imbriquée n'atteignent jamais la voix ; quatre événements de week-end tiennent là où un seul passait brut). Chaque domaine a un interrupteur propre à la personne, relu sur sa ligne à chaque rappel. À la fin de l'appel, ce qui a été dit revient comme **le propre message de la personne** : la transcription est synthétisée puis rejouée par le moteur hors tour comme un tour *parlé par la personne* — les extractions de mémoire, de journal et de psyché tournent comme dans le chat, le message porte un badge téléphone, un brouillon attend sa confirmation dans le chat. Le relais est réclamé avant le tour et réglé par mise à jour conditionnelle, un plantage en plein relais est rendu à une notification qui dit pourquoi, et dix verdicts de relais sont comptés et dessinés sur la liste des appels — « personne n'a décroché » et « la ligne a échoué » distingués de « quelqu'un d'autre a répondu ». Chaque euro d'un appel tombe sous un run id unique — les recherches en direct, la synthèse, le tour relayé — si bien que le résumé par run que le compteur du chat lit déjà est la facture de l'appel, sur la réponse relayée et sur la liste des appels ; ce qui tourne sur la clé personnelle du fournisseur y est facturé et n'est jamais compté ici.

**Une politique par mode, quelle que soit la ligne (ADR-301).** Le téléphone relayait la conversation à la *fin* ; le Live du navigateur confiait chaque demande au chat *pendant* — deux vérités pour un même mot. Une session vocale a désormais un mode, et la politique ne suit que lui : en **Live** (le défaut du téléphone, choisi dans Réglages › Téléphonie · Mon identité), la voix ne tient ni contexte ni outil et remet chaque demande à LIA par une seule fonction — celle-là même que le Live du navigateur déclare à son fournisseur — que le serveur transforme en un tour de chat de la personne pendant qu'elle parle : les questions de LIA (une question EST le résultat, la réponse suivante reprend le tour qui l'a posée), les confirmations, les registres, les quotas et la trace dans la conversation viennent avec ; en **Live direct**, la voix lit pour la personne, n'agit sur rien, et ses mots sont relayés à la fin comme un message d'elle — sur la ligne comme dans le navigateur, dont la session directe n'est plus « rien d'enregistré ». Une seule clôture ferme les livres des deux lignes : les échanges purement vocaux archivés, la carte aux chiffres exacts, la décision, l'apprentissage — ou la synthèse, le tour relayé et la carte réécrite quand il se règle, sa dépense relue. Live n'est offert que là où le fournisseur téléphonique peut rappeler l'instance ; sinon les réglages le disent et l'appel part en direct. Tout a été mesuré sur le moteur réel du fournisseur, sans téléphone : la voix annonce l'appel et poursuit la conversation, une réponse tardive serait perdue passé le délai du fournisseur — le pont répond donc avant —, et un échange qui contient une délégation appartient au graphe en entier.

### 13.6. Lire une pièce jointe : le texte quand il y en a, la vision sinon

Lister les pièces jointes d'un message est une lecture de métadonnées ; en ouvrir une est un téléchargement, et les trois clients l'exposent derrière UNE méthode du protocole e-mail — même nom, même règle de sélection, mêmes erreurs — parce que l'asymétrie entre fournisseurs est l'origine des bugs de connecteurs. La sélection est un helper partagé : par identifiant, sinon par nom, un nom ambigu refusé avec ses candidats. Il le faut, parce qu'un identifiant de pièce jointe Gmail n'est pas stable : mesuré sur une vraie boîte, il change entre deux lectures du même message, si bien que l'identifiant servi par un listage peut déjà avoir disparu quand l'outil relit le message. Un identifiant périmé n'est donc pas un mensonge : le nom décide quand il est donné, une partie unique lève l'ambiguïté, et tout autre cas répond avec les identifiants COURANTS pour que le modèle réessaie.
...
Deux faits voyagent avec chaque serveur comme des données, jamais comme de la prose de prompt. Un serveur utilisateur dont le credential est celui de l'utilisateur (OAuth, bearer ou clé personnelle) publie sa portée de compte partout où le modèle lit ses capacités — description de l'outil de délégation, manifeste du planificateur, contexte du sous-agent et description de domaine générée — si bien que « mes dépôts » se résout sur le compte connecté au lieu de finir en question ; un serveur sans authentification ne publie rien, car prétendre l'inverse serait une capacité inventée. Et le sous-agent restitue ses données, pas un décompte : sa sortie porte le même contrat message + bloc de données borné que tout outil ReAct, les contenus tiers étant marqués comme externes.
...
**Phase 1 — Décision** (le poste `heartbeat_decision`, un modèle économe choisi dans le catalogue d'administration) :
...
La phrase du rappel suit la règle de toute réponse courte : elle se demande **sans raisonnement** là où le modèle peut s'arrêter, sur un petit budget de réponse — et une réponse revenue vide, ou que le fournisseur déclare coupée à son budget, est un refus et non un message. La phrase de repli écrite part à sa place, la dépense qui a eu lieu est conservée, et l'avertissement nomme le modèle et ses tokens de réflexion. Une notification est le seul acte de la propre initiative de LIA qu'une personne éprouve vraiment ; elle n'arrive jamais sous la forme d'une cloche et rien d'autre.
...
### 16.5. Moments anticipés et veilles servies à la minute (ADR-281)
...
### 17.4. Un dossier Drive est un arbre, et le compte est exact (ADR-297)

Google Drive liste un niveau par appel, si bien qu'un dossier lié est parcouru en largeur, borné deux fois — fichiers et dossiers — et la coupe est énoncée : un raccourci n'est ni listé ni suivi, un dossier atteint deux fois est parcouru une fois, un sous-dossier illisible est compté et sauté, et seule une racine illisible est une erreur. L'ensemble des dossiers parcourus voyage avec la source, si bien que le chemin push route un changement sur l'arbre entier et laisse l'ensemble suivre le flux — un dossier créé sous l'arbre le rejoint avant que les fichiers créés dedans n'arrivent dans le même flux. Un dossier situé dans un arbre déjà lié, ou au-dessus, est refusé : deux sources dédupliquant par identifiant de fichier se voleraient leurs documents à chaque synchronisation.

Le compte affiché avant une grosse synchronisation est calculé par le code qui indexe : le même parcours, les mêmes prédicats pour « pris en charge » et « inchangé », le plafond de documents de l'espace. Il classe chaque fichier — nouveau, modifié, inchangé, non pris en charge, au-delà de la capacité — et le bouton ne demande à la personne qu'au-delà d'un seuil que l'instance fixe ; un parcours coupé par une borne dit « au moins ». Chaque synchronisation, manuelle ou de fond, dépose une consultation pour l'acte — et une lecture hors de tout run publie le sien, parce que le registre ne garde que ce qu'un collecteur ramasse.

### 17.5. Un document d'un espace rejoint un message comme copie (ADR-295)

La recherche du chat ne lit que les espaces actifs, si bien qu'un document indexé dans un espace en pause était inaccessible depuis une question sans réactiver tout l'espace. Le « + » du composeur liste donc les documents prêts de chaque espace que la personne possède, un espace en pause badgé plutôt que caché, avec une page et un total exact, et le document choisi est COPIÉ par le mécanisme même des pièces jointes : le fichier stocké sous un nom neuf, le texte par l'extraction des espaces de connaissances — quinze formats, pas la liste image-et-PDF de l'envoi —, la ligne un envoi, si bien qu'une réinitialisation de conversation retire la copie et que l'espace, son index et son fichier ne sont jamais touchés. Seul un document prêt est proposé, les deux interrupteurs de capacité doivent être ouverts, et le modèle est prévenu quand le texte est au plafond : une coupe est énoncée, jamais appliquée en silence.
...
**Une identité fédérée ne donne aucun droit.** Rattacher une identité Google ne change jamais le statut d'un compte : une inscription en attente d'approbation le reste, un compte bloqué aussi. L'adresse annoncée n'est crue que si Google s'en porte garant ; quand elle n'avait jamais été prouvée, la liaison la prouve, révoque le mot de passe et les sessions du déclarant et notifie les administrateurs comme une vérification d'e-mail ; un compte supprimé n'est jamais ranimé, et un refus porte un motif borné. « Qui appelle ? » n'a qu'une réponse, `resolve_client_ip`, que lisent les audits d'administration, l'adresse des sessions et les limites de débit — une garde AST refuse toute autre lecture de l'en-tête que le visiteur écrit, et le serveur web transmet l'adresse que Cloudflare atteste. Enfin la configuration des journaux est le premier import de l'application, et un outil lie son logger au moment où il journalise : un logger lié plus tôt gardait les défauts de structlog, sans filtre de niveau ni de données personnelles.
...
| Technologie | Rôle |
|-------------|------|
| Prometheus | 589 métriques custom (RED pattern) |
...
Données infra/perso (denylist)    Code Hygiene (i18n, docs, cycles, parité CI)
Ruff + Black + MyPy               Secret scan (Gitleaks)
Unit tests rapides                Tests unitaires + agents, plancher de couverture
Détection patterns critiques      Tests d'intégration (PostgreSQL + Redis)
Sync clés i18n                    Rejeu des migrations (depuis zéro)
...
Fichiers `.txt` versionnés dans `src/domains/agents/prompts/v1/` (un par prompt, le littéral `PromptName` tenu en phase par un test), chargés via `load_prompt()` avec un cache LRU dimensionné au-dessus du corpus entier. Versions configurables par variables d'environnement.

**Un prompt énonce ce que le code impose, et rien d'autre** (ADR-284). Tout `{placeholder}` d'un fichier a un producteur dans le module qui le charge, et un fichier qui contient `{{` est rendu par `.format()` — un garde AST lit les deux par fichier. Une consigne par contexte vit dans le fichier et n'est émise que quand le contenu existe (`response_context_sections.txt`), si bien qu'un tour nu ne porte aucune enveloppe vide ni consigne sur un document absent. La borne qu'un prompt publie est la borne que l'outil impose, depuis une constante ; un nombre vient d'un réglage et un fait du code qui l'applique ; une capacité n'est promise que si le tour peut l'exécuter. La prose n'habite jamais un `.py`, un repli non plus : les échafaudages d'une ligne vivent dans des fichiers de lignes `clé|gabarit` lus par un parseur unique (`parse_prompt_sections`), et un fichier manquant est un déploiement cassé qui échoue fort plutôt qu'une seconde formulation silencieuse.
...
L'observabilité repose sur trois piliers : **émission défensive** sur le chemin critique, **dashboards Grafana** pré-câblés (28 dashboards / 719 panels couvrant l'application, l'infra et chaque sous-système métier), et **gauges DB-backed** entretenues par un updater périodique.
...
Une règle CSS gouverne les espacements du design system : les marges verticales d’un élément `inline` sont **calculées mais jamais rendues**. Un `<label>` est inline par défaut du navigateur, la primitive d’étiquette est donc déclarée `block` — sans quoi tout `space-y-*` au-dessus d’un contrôle produit le même écart de trois pixels quelle que soit sa valeur, et un recalibrage modifie le code sans déplacer un pixel. Le réflexe en découle : quand un réglage visuel n’a aucun effet, mesurer le `display` et la géométrie DOM dans un vrai navigateur avant de soupçonner la livraison. Une garde le verrouille.

## 24. Architecture des décisions (ADR)

317 ADRs au format MADR documentent les décisions architecturales majeures. Quelques exemples représentatifs :
...
Le plafond ne tient que si le grand livre est complet, et la règle n'est pas celle du seul modèle : **chaque euro que la plateforme paie pour une personne est tracé, affiché, attribué et compté pour elle, quelle que soit la modalité ou le chemin** (ADR-272, amendement). Google Maps Platform avait des trous dans toutes les directions. Les deux portes de persistance du traceur décidaient sur les enregistrements du modèle seuls, si bien qu'une consultation Places ou Routes d'un appel téléphonique ou d'une session live directe — aucun appel de modèle — n'atteignait aucun des quatre grands livres ; `pending_families()` est désormais le prédicat unique, et une garde refuse un seau d'enregistrements qu'il ne lit pas. Le compteur d'appels Google ne faisait rien sans traceur ambiant pendant que le heartbeat, le briefing (huit appels météo facturés par rafraîchissement), les réunions et les proxys d'images payaient : il échoue désormais fermé dans `google_api_calls_unaccounted_total`, tenu à zéro par une alerte. Chaque surface hors tour ouvre sa propre comptabilité autour de l'acte entier par une porte unique, la route que prend chaque module est déclarée dans un registre sœur de `LLM_SPEND_ROADS` et vérifiée sur le graphe d'imports, une image facturée est comptée là où Google la facture — au proxy, sur le 200, jointe à son tour par un run id signé sur (run id, compte) — et une seule colonne, `billed_cost_eur`, remplace quatre sommes écrites à la main avec trois sous-ensembles différents.
...
La consommation est volontairement retenue : contexte ambiant pour les réponses et le briefing, au plus une offre de routine manquée par jour, mise en sourdine après deux offres ignorées jusqu'à l'occurrence suivante, et un scoring du minutage des notifications qui préfère les fenêtres apprises sans jamais élargir les bornes configurées par l'utilisateur — une règle anti-famine garantit qu'une intersection vide ne change rien. Chaque seuil appliqué par les détecteurs est publié dans le panneau : une habitude affichée est prouvée, ou elle n'existe pas.

La seconde moitié — les **demandes récurrentes** — lit la décision du routeur, en vocabulaire fermé : la valeur même dont le tableau de bord produit dérive, si bien qu'un tour compté comme une action est un tour que le ledger enregistre. La signature reste le domaine primaire seul. Quatre formes sont nommées : quotidienne, jours ouvrés, hebdomadaire et **intermittente** — « plusieurs fois par semaine vers 9 h » — décidée par la densité de jours distincts sur l'empan éligible, jamais par le verrou : un 3×/semaine n'est plus promis « tous les jours ». Les seuils sont mesurés sur un harnais durable (`task habits:calibration:measure`, 300 essais par case, populations denses et modérées avec leur contrôle dispersé à volume égal) : un quotidien est reconnu à J+14, un 3×/semaine entre J+21 et J+35, un rituel hebdomadaire fiable à 90 % à J+42 — sur cinq créneaux, pour qu'une semaine manquée ne tue plus le verrou — avec 0-0,3 % de faux verrous sur un usage sans structure ; les relaxations que le harnais a refusées restent dans ses tables. Et le silence est une alerte : `RecurrenceLedgerSilent` compare les tours actionnables humains aux écritures atterries — un ledger qui n'écrit rien pendant qu'on lui parle n'est pas un ledger discret, c'est un ledger cassé.

**Ce qui est appris est consommé par UN prédicat, et audité à l'exécution.** Le panneau des habitudes modifie des lignes miroir — une fenêtre se met en pause ou se bloque — tandis que le bloc du battement, le scoring des ticks et le bloc ambiant lisaient le profil : une fenêtre bloquée continuait de guider les ticks. `habits/consumption.py::load_consumable_profile` sert désormais les trois, ne gardant que les fenêtres dont la ligne est active, et la clé d'une fenêtre n'a qu'un producteur. Une habitude apprise a un cycle de vie tenu par le job nocturne (`recurrence_sync` : promouvoir, rafraîchir, garder, rétrograder — lignes actives seules, jamais dans le doute, et pour un compte silencieux aussi, qui est précisément le cas de la rétrogradation), jamais par la suggestion du chat. La porte d'apprentissage (`learning_gate.read_learning_gate`, fermée par défaut) ferme toutes les portes — l'écriture du ledger, le détecteur d'initiative, l'amorçage — et suit `HABITS_ENABLED` plus le commutateur de capacité `habits` lu à l'acte : `habits` et `heartbeat` ont quitté la famille gardée par route d'ADR-280 pour la famille imposée au service, parce que les routes sont l'archive. La décision **déclare** son offre (`HeartbeatDecision.habit_offered`, règle 24) au lieu que le budget d'offres pende à une étiquette de source choisie par le modèle, et l'offre a un objet : l'`immediate_intent` de l'analyseur est un histogramme borné dans la charge du ledger — jamais une partie de la clé, ce qui fragmenterait chaque ledger — si bien que `usual_intent` atteint la ligne, le prompt (« usually asks for a 'search' on 'email' ») et le panneau par le vocabulaire fermé `IMMEDIATE_INTENTS`. Le scoring des ticks vit dans `habits/tick_scoring.py` avec une `TickSurface` par balayage épinglée sur son vérificateur d'éligibilité, et le balayage d'intérêts applique les deux portes — la garde « en réunion » et le rythme appris ; un réveil contourne le rythme mais s'efface devant une réunion, le verdict d'agenda portant `next_start` pour que le rythme ne cache jamais un rendez-vous. La présence est activée par défaut et la dernière activité compte un ping de présence. Et le registre des consultations est alimenté autour de la passe entière, éligibilité comprise — l'agrégateur lisait dans `select_target` pendant que le collecteur s'ouvrait dans `generate_content` : 824 passes, zéro ligne.
...
Le démonstrateur public applique ces bornes à lui-même, et les rend lisibles. Chaque capacité du registre y est décidée par écrit dans le gabarit d'environnement — activée ou coupée, avec sa raison — et deux gardes refusent un plafond laissé au défaut du code ou un écart entre les gabarits de développement et de production ; le recensement des routes atteignables monte les routeurs sous ces mêmes plafonds, si bien qu'une famille coupée n'y figure pas et qu'une famille activée y est écrite route par route. Enfin, chaque instance publie dans sa configuration publique l'état effectif de toutes ses capacités — plafond et commutateur composés — et l'instance qui annonce un démonstrateur lit cette configuration côté serveur et l'affiche à côté du lien : la liste de ce qu'un visiteur trouvera, et de ce qu'il ne trouvera pas, suit le fichier d'environnement de la démonstration, personne ne la tient à la main.

**Une annonce dit à qui elle s'adresse** (ADR-312). L'audience d'une annonce d'administrateur est résolue avant que la ligne ne soit écrite, et ses destinataires sont enregistrés avec elle : une annonce ciblée n'est servie qu'à eux — y compris à leur connexion suivante —, et une sélection qui ne vise aucun compte actif est refusée plutôt que de partir vers personne, ou vers tous. L'historique sert une page et son total exact, un échantillon borné des destinataires avec leur compte exact, l'instant d'expiration et le délai choisi, en quatre requêtes quelle que soit la taille de la page.
...
## 30. Travail visible, apprentissage gouverné

La page Activité est un **read-model pur** : des fetchers parallèles (une session par source, l'`AsyncSession` n'étant pas concurrente) agrègent les tables d'audit existantes, les totaux sont des `COUNT(*)` exacts sur toute la fenêtre, les plafonds sont déclarés et une source en panne est listée plutôt que complétée en silence — le comptage honnête (ADR-185) appliqué de bout en bout. La mémoire suit une **piste de supersession** : une correction automatique crée un successeur et archive l'ancien fait (`superseded_by_id`), chaque lecture filtre le set actif via un prédicat central, et la piste se purge après rétention ; l'édition manuelle, elle, garde son autorité d'écrasement. Les règles apprises sont une **catégorie de mémoire à part entière**, injectée en tête de prompt, sous les mêmes protections (épinglage, rétention, RGPD). La prosodie vocale est une **modulation bornée** (bande morte, bornes dures, drapeau) des réglages administrés — jamais un remplacement. Et l'autonomie reste plafonnée : le budget d'itérations ReAct s'adapte à l'étendue en domaines de la requête sans jamais dépasser le plafond configuré, et une complexité inconnue reçoit le plafond entier — l'économie ne s'applique qu'au prouvé-simple.
...
Émettre toute cette observabilité sans jamais la lire, c'est être instrumentée de partout et aveugle sur soi-même. Le sous-système d'auto-diagnostic (ADR-247) ferme la boucle avec une règle de conception par pilier.
...
**Les calculs courants n'attendent pas un script : ce sont des outils** (ADR-318). Dans les deux modes, un calcul est analysé en arbre puis évalué en décimal exact — jamais exécuté —, les infinis que la bibliothèque décimale rend sans lever d'erreur (`0 ** -1`, `ln(0)`) sont refusés, et un résultat qui a dû être arrondi le dit. Une durée se compte entre deux instants absolus, parce que deux dates d'un même fuseau se soustraient sur leur heure murale et donneraient 24 heures pour les 25 d'un changement d'heure. Une conversion lit le taux de référence de la BCE et en donne la date. Chaque borne appliquée est un réglage énoncé dans la formulation que lisent à la fois le manifeste du planificateur et le schéma ReAct. Le script éphémère qui suit reste pour ce qu'aucun outil ne couvre.
...
## 37. Trois mécanismes pour une convergence : lisser une rafale qu'un plafond ne voit pas

**Un déclencheur d'intervalle compte depuis le démarrage, donc des périodes qui partagent un diviseur s'alignent pour toujours.** C'est de l'arithmétique, pas de la charge : des tâches de 5, 15, 30 et 60 minutes finissent par partir dans la même seconde, et y restent pour la vie du processus. Mesuré en production : six tâches dans la même seconde chaque heure, chacune faisant tourner un agent, chaque agent vectorisant — 11 échecs sur 24 appels, quand un rythme régulier de quatre par minute passait sans une seule erreur.

**Trois mécanismes, trois rôles, et les distinguer est le fond de la conception.** Le **jitter** traite la cause : 15 % de la période, plancher de 5 s, toujours strictement sous la période pour que deux exécutions ne puissent ni se chevaucher ni s'inverser. Le **lisseur** n'aurait rien changé à cet incident — six appels ne franchissent aucun plafond par minute — il est l'assurance à l'échelle, d'où une fenêtre volontairement **courte** : un plafond par minute ne peut pas voir une rafale. Le **réessai** ramasse le résidu.

**Aucun des trois n'est une porte.** Le lisseur compose le limiteur de débit existant plutôt que d'en ajouter un second — une fenêtre glissante, un script Lua, un jeu de métriques — et il expire **ouvert** : l'attente est bornée, et un appelant qui a patienté sa part passe quand même. Notre propre régulation ne doit jamais être la raison pour laquelle une réponse perd sa mémoire. Redis injoignable, la réponse est « pas de créneau », immédiatement, et l'appel part.

**Un réessai exige une fabrique, pas un awaitable.** Une coroutine ne s'attend qu'une fois : un point de couture qui tient `client.aembed_query(...)` ne peut pas la relancer, la seconde attente lève au lieu d'appeler. Et la classification est **structurelle** — le code de statut lu en remontant la chaîne des causes, jamais le texte du message, qu'une reformulation du fournisseur suffirait à rendre faux. Corollaire : une couche qui épuise son réessai doit **chaîner sa cause**, sinon la couche au-dessus lit une enveloppe muette et déclare définitif ce qui ne l'était pas.
...
## 39. Trois registres, et celui que personne n'avait demandé

**Un assistant qui agit doit pouvoir dire ce qu'il a fait, ce qu'il a lu, et sur quel tour.** Trois registres y répondent et ne se confondent jamais : le premier porte une ligne par **action**, réclamée avant qu'elle ait lieu et close sur un résultat explicite ; le deuxième une ligne par **consultation**, nommée sous la forme de la capacité et jamais de ce qui a été cherché ; le troisième une ligne par **tour**, la colonne vertébrale à laquelle les deux autres se rattachent. Leurs totaux ne s'additionnent pas, et c'est voulu : un tour peut consulter cinq sources et ne rien changer du tout.

**La garantie était structurelle, mais la porte était unique.** L'enregistrement est installé sur la capacité au moment où elle est déclarée — un nouvel outil ne peut donc pas l'oublier — sauf qu'une capacité atteinte par son *client* plutôt que par la porte des outils reste invisible. Mesuré en production sur quatorze jours : les surfaces conversationnelles étaient enregistrées 24 fois sur 24, et le travail hors tour **0 fois sur 228**. Rien n'était en panne : les registres suivent le graphe, et ces surfaces appellent le modèle directement. La liste de ce que l'assistante entreprend d'elle-même se lisait donc vide quoi qu'elle fasse — la version la plus nette du trou, puisqu'une notification proactive est le seul acte d'initiative qu'une personne éprouve réellement.

**La réponse n'est pas un quatrième détecteur, c'est une déclaration.** Chaque surface qui lit sans passer par un outil — le briefing, le débrief de relation, le balayage périodique, les centres d'intérêt, les espaces de connaissances — déclare son vocabulaire dans une table unique, et l'assertion de démarrage vérifie les trente et une capacités là où elle ne regardait que les outils. Les vingt-deux modules qui importent un client de connecteur sont énumérés un par un : enregistreur, non-lecteur avec la raison écrite, ou dette — table de dette vide, et qui ne peut que rétrécir. Le registre s'**offre** enfin plutôt que de se laisser chercher : le domaine des agents importe déjà celui des relations, donc un import en retour refermerait un cycle qu'un import local ne ferait que cacher.

**Où va un euro est déclaré, jamais déduit.** La comptabilité est ambiante — un nœud dépense par un contexte qu'un ancêtre a publié — si bien que répondre « est-ce compté ? » en lisant les fichiers a produit neuf conclusions fausses en une seule session, dans les deux sens. Une table nomme donc les quarante-six sites d'appel et le registre que chacun atteint, et une garde qui lit des appels par AST y a repris six classements. Compter n'est d'ailleurs que la moitié d'un plafond : l'autre est de **demander d'abord**, et cinq de ces sites n'étaient bornés par rien. Trois formes les avaient produits — un registre qui nommait une enveloppe au lieu de la porte, une garde qui sortait avant d'interroger, un même refus rendu de deux façons. Un refus suit désormais son transport et jamais son verdict : une requête lève, une tâche de fond renonce et se journalise **sautée**, jamais *échouée*.

**Deux points d'entrée qui servent un même écran font un seul acte de lecture.** Le tableau de bord tire ses cartes et sa synthèse en parallèle, et chacun construisait le lot des neuf sections pour son compte : sur sept jours, 151 constructions dont 44 doublons — 39 % des chargements, et 44 sur 44 concurrentes. Chaque connecteur était ouvert deux fois et deux lots de consultations déposés pour un seul acte de lecture. La cause était une question, pas une ligne de code : la synthèse demandait « ce lot contient-il quelque chose d'intéressant ? » au lieu de « ce lot a-t-il été construit ? », et la première ne distingue pas un cache froid d'une journée calme. Le premier qui demande construit, les autres reçoivent le même objet — dans le processus, et entre les quatre workers d'une instance de production, par une revendication relâchée par son seul propriétaire.

**Une extraction est complète, ou ce n'est pas une extraction.** Les cinq enregistrements téléchargeables portaient un plafond de lignes, mesuré et non arbitraire : sur le Raspberry Pi visé, cinq sources à cinq mille lignes culminaient à 33,9 Mo. La contrainte était réelle — le document entier était assemblé en mémoire — mais appliquée à la mauvaise variable. **Ce qui était rare, c'était la mémoire ; ce qui était borné, c'était la vérité** : 49 195 lignes réelles contre mille par source, soit 97,9 % de l'enregistrement d'inférence absent, sous un en-tête qui disait vrai en annonçant « tronqué ». Un curseur côté serveur borne désormais le tampon ; le compte est exact — un agrégat sur la statement même que le corps parcourt — et publié avant la première ligne, si bien qu'une extraction sans borne supérieure en reçoit une : l'instant de sa génération, nommé dans l'en-tête plutôt que fixé en silence.

**L'assistante lit ses propres registres** (ADR-318). À « qu'as-tu fait pour moi cette semaine ? », un outil de lecture répond depuis les deux registres, avec des totaux exacts et le même filtre d'auteur que les onglets — ce que la personne a demandé, ce que LIA a entrepris d'elle-même — plutôt que depuis le souvenir d'une conversation. Un registre sert ainsi deux lecteurs sans deux implémentations : l'écran qui le dessine et la réponse qui le résume.

## 40. Un débrief par relation : ce qu'une fiche ne dit pas en dix sections

**Une fiche empile dix sections, et personne ne lit dix sections.** Ce qu'un lecteur cherche d'abord — où j'en suis avec cette personne, qu'est-ce qu'il faut aborder — est une synthèse qu'aucun agrégat ne produit. Elle est donc écrite par le modèle en tête de la fiche : ce qui reste ouvert, ce qu'il faut faire ensuite, ce qui est bon à retenir. Sa fabrication est **paresseuse** : elle naît à l'ouverture de la carte, au plus une fois par jour local, jamais par un ordonnanceur — le nombre de relations n'est pas borné — et jamais pendant un tour de chat. Trois reconstructions seulement sont légitimes : la langue, l'étendue demandée, et une demande explicite.

**Deux assemblages auraient créé deux autorités sur « qui est cette personne ».** L'assemblage des preuves existait déjà, à l'intérieur de l'outil qui répond au chat ; il a donc été extrait dans son propre module, dont l'outil est devenu le premier consommateur. Un refactor sur un chemin en production ne se vérifie pas à la lecture : un fichier doré a été capturé **sur le code d'avant**, dix-huit cas d'étendue, charge utile et message comparés à l'octet. La moitié fournisseur n'est plus lue en entier pour être jetée ensuite — jusqu'à onze appels externes étaient facturés contre une sélection que le lecteur avait déjà faite — et le rétrécissement a exigé un troisième statut : « je n'ai pas regardé, exprès » n'est ni « je n'ai rien trouvé » ni « je n'ai pas pu regarder ».

**Rien n'est inventé, et un échec ne détruit rien.** Aucune preuve ne conclut « rien à dire » sans appeler le modèle, et une actualisation ratée **garde le texte précédent** sous une ligne qui le signale : remplacer une synthèse utilisable par un panneau vide transforme « je n'ai pas pu actualiser » en « il n'y a rien ». La revendication est une seule instruction SQL, et chaque clôture n'écrit que ses propres colonnes — prête, vide, échouée — parce qu'une clôture unique écrasait le corps au premier échec. Ce que la rédaction a coûté est stocké avec elle et montré dessous ; un coût nul étant une affirmation, une ligne muette n'affiche rien plutôt que « 0,00 € ».

**Dans le chat, le débrief rejoint le bloc pair — avec la directive inverse.** Le bloc pair affirme des faits exacts parce qu'il les lit dans le tour lui-même ; la même phrase au-dessus d'une synthèse datée serait une machine à affirmations fausses. Le gabarit dit donc qu'elle est datée, donne son **âge** et non seulement sa date, et renvoie aux outils tout chiffre, tout compte et tout statut. Une correspondance de nom ambiguë n'injecte **rien** : l'annuaire contient toutes les relations jamais ouvertes, noms d'entreprise et numéros compris, et un faux positif remettrait le dossier d'une personne sur une question qui en visait une autre.

## 41. Le tableau de tickets : une ligne, deux côtés, et une assistante qui demande

**Une unité de travail a besoin d'un cycle de vie, d'un porteur et d'un résultat** — et aucune des surfaces existantes ne réunissait les trois. Les tâches d'un fournisseur n'ont pas de cycle de vie ; un rappel est une poussée à un instant, dont rien ne subsiste après la sonnerie ; une routine est une instruction répétée qui ne se termine jamais. Un ticket porte un titre, une description, une priorité, des dates, des sous-tickets, des commentaires et une histoire à travers sept colonnes, et son porteur peut être le propriétaire, un pair connecté, ou l'assistante elle-même.

**Une seule ligne, lue des deux côtés.** Un tableau, c'est « je le possède OU je le détiens », écrit une seule fois comme prédicat de repository que toutes les lectures réutilisent — deux requêtes seraient deux autorités sur ce que dit un ticket partagé. Un ticket que l'appelant ne voit pas répond exactement comme un ticket qui n'existe pas : personne ne peut sonder un identifiant. La page et ses compteurs par colonne sortent du même énoncé filtré : un en-tête annonçant sept au-dessus d'une colonne qui en montre trois serait pire que pas d'en-tête du tout.

**Ce que la base peut porter, et ce qu'elle ne peut pas.** La colonne du porteur est en `SET NULL`, parce que quatre chemins suppriment durement une ligne `users` et qu'un seul déclenche la purge de compte — une cascade détruirait le ticket du propriétaire au départ de son pair. L'invariant « un pair ne détient un ticket que sous une connexion acceptée » n'est délibérément PAS une contrainte : un `CHECK` croisant deux colonnes qu'une action de clé étrangère peut modifier est violable quel que soit le sens des cascades, aucun ordre n'est garanti, et PostgreSQL n'a pas de `CHECK` différable. Il vit donc dans le service, qui revérifie la connexion à chaque écriture — ce qui est aussi la bonne règle : un partage se revérifie à l'exécution et ne se lit jamais depuis une ligne stockée.

**Un balayage revendique un ticket, et solde depuis un résultat.** Chaque minute, un job à gigue libère d'abord les revendications qu'un worker mort tient encore, puis en revendique un seul sous `FOR UPDATE SKIP LOCKED`, par un `UPDATE` conditionnel validé avant que le moindre travail commence — un worker qui en aurait pris cinq en abandonnerait cinq. Il solde depuis une issue explicite, jamais depuis l'absence d'exception, et il nomme ce qu'il a fait : trois refus peuvent arrêter un ticket revendiqué, et aucun n'est un échec. Un compte inactif est permanent ; un plafond de quota et une conversation occupée rendent la revendication, restituent le run au budget du ticket et se journalisent « sauté ».

**Hors tour, elle demande au lieu de refuser.** La porte d'exécution refusait aussi bien une confirmation qu'un brouillon en dehors d'une conversation, faute d'interlocuteur. Un run de ticket en a un : la porte laisse désormais une origine capable de porter un brouillon construire la carte même que le chat aurait montrée, le ticket arrive en « à confirmer » en la tenant, et le commentaire suivant du propriétaire classe la réponse **sans aucun appel modèle** — un lexique replié en six langues, où un oui nu approuve, un non nu clôt, et tout le reste devient une consigne. Le rejeu tourne alors dans le graphe sous verrou d'empreinte : le run publie l'identité de ce qu'il a montré, et le brouillon reconstruit par l'outil ne passe que sur cette identité exacte, et sur rien de plus large.

**Ce qu'un run écrit, et où.** Un run archive son brief et sa question comme des lignes DU RUN — tenues hors du chat, pointées par le registre de décisions, retirées par la rétention après la clôture du ticket — tandis que la notification que la personne doit lire est un message ordinaire. La distinction est une colonne DÉRIVÉE plutôt qu'un tampon dans les métadonnées, calculée au seul endroit qui construit la ligne ; et toute métadonnée de tour archivée est bâtie par un constructeur nommé, sous une garde qui refuse un dictionnaire écrit sur place — c'est exactement ainsi qu'un aperçu de suppression avait atteint le chat.

## 42. L'anatomie d'un processus : ce qu'il charge est déclaré, mesuré et borné

Un serveur API est plusieurs processus : un superviseur qui ne sert aucune requête, et des workers qui servent tout. Chacun tient de la mémoire pour des raisons différentes, et « le conteneur consomme cinq gigaoctets » ne dit rien d'utile tant que l'on ignore quel processus tient quoi. LIA applique ici une règle unique : **ce qu'un processus charge est déclaré, mesuré par processus, et borné là où il se multiplie**. Chaque poste a été mesuré sur l'hôte cible, dans un processus jetable, avant d'être touché — et trois mesures ont contredit la première lecture, ce qui est précisément la raison de mesurer.

Le superviseur ne charge pas l'application. La ligne de commande d'uvicorn importe l'application dans le processus parent pour produire une erreur d'import précoce, puis la jette — un parent qui ne fait que surveiller ses workers tenait ainsi plusieurs centaines de mégaoctets pour toute la vie du conteneur. `python -m src.serve` pilote la même chaîne `Config → Server → Multiprocess` avec les mêmes options, sans cet import ; un garde AST refuse que le module importe quoi que ce soit de `src`, et l'erreur précoce est rendue par le contrôle de santé que le déploiement lit déjà. Dans le même esprit, une bibliothèque lourde — rendu PDF, calcul tabulaire, moteurs audio — est importée là où elle sert et jamais au démarrage : une liste déclare le coût mesuré de chacune, et un garde refuse tout import de premier niveau sous `src/`, y compris caché dans un `try` de module.

Ce qu'un singleton charge est multiplié par le nombre de workers. La transcription vocale gardait un modèle résident par langue jamais demandée, dans un singleton par processus ; le cache est désormais borné (`VOICE_STT_MAX_RECOGNIZERS`, un par défaut), évincé au moins récemment utilisé, rien n'est chargé à la construction, une éviction rend la mémoire au système — mesuré — et un décodage en cours garde sa propre référence. Et chaque worker publie ce qu'il tient : `lia_worker_memory_bytes{kind}` est lue par chaque worker dans son propre `/proc/self/status`, une série par worker vivant qui disparaît avec lui, tracée sur deux panneaux et surveillée par l'alerte `ApiWorkerMemoryHigh`, qui nomme le processus. C'est la réponse à une limite structurelle : cAdvisor voit le conteneur, et le mode multiprocess de prometheus_client n'exporte aucune métrique de processus.

Le budget de connexions a un plancher, pas seulement un plafond. L'audit F004 bornait la pointe — ce que les pools peuvent ouvrir ; chaque connexion persistante est aussi un backend Postgres inactif tenant sept mégaoctets, à côté des `shared_buffers`. `ConnectionBudget.persistent_total` nomme ce plancher, et un garde lit ensemble le compose (limite mémoire, `shared_buffers`) et les deux profils `.env` livrés — le complet et le minimal de l'auto-hébergeur, les clés absentes prenant les défauts du code comme au démarrage : le plancher doit rester sous la moitié de la limite, pour que la base garde l'autre moitié à son cache. Profil livré : cinq connexions persistantes et quinze en débordement par worker, limite de deux gibioctets. Ce que le lot laisse ouvert est écrit : la croissance sur plusieurs jours d'un worker sans transcription — mesurable désormais — et la rétention des points de reprise LangGraph.

## 43. Le mode Live : deux intelligences, une couture — et une politique par mode

Une session de parole à parole tourne sur un modèle live que la personne connecte avec **sa propre clé** — une catégorie de connecteur à part, `live`, additive : plusieurs clés peuvent être actives, les sessions s'ouvrent sur celle qui est choisie, et choisir un modèle *est* choisir son fournisseur. Rien de ce que le fournisseur facture n'est compté, stocké ni montré par la plateforme. L'API frappe une clé à usage unique et rend le setup ; le navigateur le rejoue tel quel et ouvre lui-même la connexion, si bien que **l'audio ne transite jamais par l'API**. Quatre faits ont été mesurés sur le vrai fournisseur avant de figer le design, et chacun est devenu une règle : la contrainte d'une clé ne verrouille pas l'instruction système ; une clé à usage unique consommée ne peut pas se reconnecter (chaque reconnexion en frappe une fraîche pour le même record) ; le fournisseur accepte en silence un nom de voix inconnu (la voix est validée contre une liste vendored et datée) ; un WebSocket brut du navigateur n'est accepté que par une seule méthode.
...
Une **session directe** est la ligne du téléphone dans le navigateur : la voix tient elle-même les outils de lecture de LIA — le jeu dérivé du téléphone moins les interrupteurs de la personne — derrière une porte d'outils unique sous un budget par session, et ne délègue jamais. La prémisse du propriétaire a été mesurée sur une vraie session : cinquante-cinq déclarations pèsent quatre-vingt-dix pour cent du setup et le premier tour facture neuf fois le tour délégué — une session directe achète de la latence, jamais de l'économie. Les relevés d'usage du fournisseur sont repliés dans le navigateur et tarifés par le tarif que le démarrage publie, dessinés sous le dernier sous-titre ; un coût qui aurait besoin d'un tarif non déclaré est indisponible, jamais partiel ; un modèle live n'est offert que si la table tarifaire le déclare. Deux pièges mesurés vivent dans le lecteur : le dialogue affectif de Gemini ferme le socket à la première chose que le modèle doit répondre, et Chrome 152 répète le premier bloc de rendu d'un morceau planifié — le lecteur PCM est un seul AudioWorklet qui lit une file, jamais un nœud source par morceau.

Les deux mêmes modes ont ensuite atteint le téléphone (ADR-301). L'appel du titulaire relayait la conversation à sa *fin* quand le navigateur déléguait chaque demande *pendant* — deux vérités pour un même mot. Une session vocale a désormais un **mode** et un **porteur**, et la politique ne suit que le mode : `delegated` (« Live »), chaque demande un tour de chat de la personne au moment où elle la dit ; `direct` (« Live direct »), la voix lit, n'agit sur rien, et les mots sont relayés à la fin comme le propre tour de la personne. **Une seule clôture** sert les deux lignes, appelée par la route de fin du navigateur et le webhook post-appel du téléphone ; ce que les lignes partageaient est sorti des deux dans `domains/voice_sessions/`, et le mode Live du téléphone est la délégation du navigateur côté serveur — un outil vendeur asynchrone au nom et au schéma du navigateur, un pont qui reflète celui en TypeScript règle pour règle (la demande la plus récente gagne par un marqueur Redis que le pont en cours sonde ; une question que LIA a posée est le résultat et la demande suivante reprend le run qui l'a posée). Mesuré sur le vrai moteur du fournisseur, sans téléphone : la voix annonce l'appel et continue de parler, une réponse tardive au-delà de la borne du fournisseur est perdue, le pont répond donc avant. Le mode effectif est dérivé et publié — Live n'existe que là où l'hôte de rappel est public — et un runner ne tient aucune session de base pendant le tour : 8,9 s d'`idle in transaction` avant, aucune après.
...
*Document rédigé sur la base de l'analyse du code source (`apps/api/src/`, `apps/web/src/`), de la documentation technique (680+ documents), des 317 ADRs, et du changelog (v1.0 à v1.47.4). Toutes les métriques, versions et patterns cités sont vérifiables dans le codebase.*
