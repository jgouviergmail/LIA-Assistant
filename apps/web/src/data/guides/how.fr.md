...
| Transparence totale | 425 métriques Prometheus, debug panel embarqué, suivi token par token |
| Fiabilité en production | 120+ ADRs, ~12 300 tests collectés par pytest sur 702 fichiers, observabilité native, HITL à 6 niveaux |
...
| Métrique | Valeur |
|----------|--------|
| Tests | ~12 300 (collectés par pytest sur 702 fichiers de test) + 2 147 tests vitest côté frontend (seuils de couverture verrouillés, ADR-116) |
| Fixtures réutilisables | 170+ |
| Documents de documentation | 280+ |
| ADRs (Architecture Decision Records) | 120+ |
| Métriques Prometheus | 425 définitions |
...
Factory **catalogue-driven** (ADR-081) : `factory.get_tts_client()` lit l'override actif `voice_tts` (provider + model + voice + tuning, stockés dans `llm_config_overrides.voice_tts.provider_config` JSONB) et instancie le client correspondant. Trois providers livrés : Edge (gratuit, défaut), OpenAI (`tts-1` / `tts-1-hd`) et ElevenLabs (`eleven_multilingual_v2`, `eleven_turbo_v2_5`, `eleven_flash_v2_5`). Si la clé d'un provider payant manque, repli automatique sur Edge (warning loggé). Streaming progressif phrase-par-phrase via `ProgressiveSentenceStreamer` (ADR-082) pour minimiser la latence — la première phrase est synthétisée pendant que le LLM en génère encore d'autres.
...
FAQ intégrée (200+ Q/A, 24 sections) indexée depuis `docs/knowledge/`. Détection `is_app_help_query` par QueryAnalyzer, Rule 0 override dans RoutingDecider, App Identity Prompt (~200 tokens, lazy loading). SHA-256 staleness detection, auto-indexation au démarrage.
...
| Technologie | Rôle |
|-------------|------|
| Prometheus | 425 métriques custom (RED pattern) |
...
| Alertmanager | Noyau de 13 alertes vitales notifiées par e-mail (runbooks liés, seuils par environnement) |
...
| Coverage | 45 % minimum | Imposé en CI |
...
L'observabilité repose sur trois piliers : **émission défensive** sur le chemin critique, **dashboards Grafana** pré-câblés (25 dashboards / 595 panels couvrant l'application, l'infra et chaque sous-système métier), et **gauges DB-backed** entretenues par un updater périodique.
...
120+ ADRs au format MADR documentent les décisions architecturales majeures. Quelques exemples représentatifs :
...
*Document rédigé sur la base de l'analyse du code source (`apps/api/src/`, `apps/web/src/`), de la documentation technique (280+ documents), des 120+ ADRs, et du changelog (v1.0 à v1.25.9). Toutes les métriques, versions et patterns cités sont vérifiables dans le codebase.*
