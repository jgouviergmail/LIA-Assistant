...
| 完全透明 | 581 Prometheus 指标、内嵌调试面板、逐 token 追踪 |
| 生产可靠性 | 300 篇 ADR、由 pytest 在 1 842 个文件中收集的 ~30 704 个测试、原生可观测性、6 层 HITL |
...
| 指标 | 数值 |
|------|------|
| 测试 | 30,704 个（由 pytest 在 1,842 个测试文件中收集）+ 前端 8,801 个 vitest 测试（覆盖率阈值已锁定，ADR-116） |
...
**没有任何仪表板能展示的指标，就是没有人会去处理的指标。** 代码发出的内容与操作者实际能看到的内容之间的差距是被测量的，而非假设的：`scripts/audit/measure_metric_coverage.py` 用 AST 解析每一处指标定义（而不是正则——正则会把 `ZoneInfo("UTC")` 当成一个 `Info` 指标），并把每个名称与所有仪表板面板、recording rule 和告警表达式逐一比对。已定义 581 项；其中 44 项明确列入一份**只能缩短**的基线：新出现的盲点指标会让构建变红，而已接入的指标必须从清单中移除——否则下一个盲点会悄悄顶替它的位置。没有这套机制的代价：一个开路失效的心跳来源曾让健康信号在一周内丢失 46.5% 的 tick，却没有任何指标能察觉（ADR-148）。这道守卫从构造上关掉了两个陷阱——带标签且从未触发过的计数器**根本不会暴露任何序列**，因此监视罕见故障的面板必须写 `or vector(0)`，否则会在操作者期待绿色零值的地方显示「No data」；覆盖率只从面板与规则的**表达式**中读取，因为注释里提到的指标并没有接入。
...
300 篇 MADR 格式的 ADR 记录了主要的架构决策。以下是一些代表性示例：
...
这张脸在两次回答之间活着，而这份生命是一个绑定系统（rig），不是一张样式表（ADR-252、ADR-264）。一个 TypeScript 运行时逐帧计算每个通道——解析弹簧、叠加循环、关键帧带——并将结果发布为样式表只读取的 `--rig-*` 自定义属性：样式表从不声明其中任何一个，也从不给每秒变化六十次的值加过渡。眉毛有弧度，休息时隐约在场；一次呼吸以同一周期带动身体、眉毛和嘴宽；目光抬起眉毛，眨眼让眉毛落下，二者在绑定系统中以**绝对贡献而非增量**耦合，因为空闲快速路径只重写循环所驱动的通道——那里的增量会在整个会话中漂移，一个测试通过把两万个小步与一步相比较来钉住这一点。说话有语句（穿过闭合的包络，开口限定在单位区间内）以及按不规则模式停顿的眉毛。嘴巴有自己的生命——由绑定系统调度、随机节奏的相对小表情——以及安静脸上的十段短小戏，任何表情变化都会中断它，脸精确停在原处，并与一个孪生绑定系统对照验证。随机性来自一个单独播种的流，从不使用 `Math.random`，以便部件测试保持确定；活着的静止在测试中是一个像素预算——休息时可见，低于两像素，专注的脸上恰好为零。同一个部件也在公开首页迎接访客：无需账户，每个界面各有位置，那里强制使用胶囊外观而聊天保留用户自己的样式——样式选择器的预览则关闭这份生命，因为预览是用来比较轮廓的。
...
300 篇 ADR 不仅记录了做出的决策，还记录了被否决的替代方案和接受的权衡。1 842 个文件里的 ~30 704 个测试、完整的 CI/CD 和严格的 MyPy 并非虚荣指标 — 它们是让这种复杂度的系统能够无回归演进的机制。
...
*本文档基于源代码（`apps/api/src/`、`apps/web/src/`）、技术文档（490+ 份文档）、300 篇 ADR 及变更日志（v1.0 至 v1.47.0）的分析编写。文中引用的所有指标、版本和模式均可在代码库中验证。*
