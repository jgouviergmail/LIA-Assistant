...
| 生产可靠性 | 179 篇 ADR、由 pytest 在 881 个文件中收集的 ~16,584 个测试、原生可观测性、6 层 HITL |
...
179 篇 ADR 不仅记录了做出的决策，还记录了被否决的替代方案和接受的权衡。881 个文件里的 ~16,584 个测试、完整的 CI/CD 和严格的 MyPy 并非虚荣指标 — 它们是让这种复杂度的系统能够无回归演进的机制。
...
*本文档基于源代码（`apps/api/src/`、`apps/web/src/`）、技术文档（400+ 份文档）、179 篇 ADR 及变更日志（v1.0 至 v1.27.1）的分析编写。文中引用的所有指标、版本和模式均可在代码库中验证。*
