...
| 完全透明 | 490 Prometheus 指标、内嵌调试面板、逐 token 追踪 |
| 生产可靠性 | 252 篇 ADR、由 pytest 在 1 296 个文件中收集的 ~21 584 个测试、原生可观测性、6 层 HITL |
...
**没有任何仪表板能展示的指标，就是没有人会去处理的指标。** 代码发出的内容与操作者实际能看到的内容之间的差距是被测量的，而非假设的：`scripts/audit/measure_metric_coverage.py` 用 AST 解析每一处指标定义（而不是正则——正则会把 `ZoneInfo("UTC")` 当成一个 `Info` 指标），并把每个名称与所有仪表板面板、recording rule 和告警表达式逐一比对。已定义 490 项，已接入 433 项；其余 57 项明确列入一份**只能缩短**的基线：新出现的盲点指标会让构建变红，而已接入的指标必须从清单中移除——否则下一个盲点会悄悄顶替它的位置。没有这套机制的代价：一个开路失效的心跳来源曾让健康信号在一周内丢失 46.5% 的 tick，却没有任何指标能察觉（ADR-148）。这道守卫从构造上关掉了两个陷阱——带标签且从未触发过的计数器**根本不会暴露任何序列**，因此监视罕见故障的面板必须写 `or vector(0)`，否则会在操作者期待绿色零值的地方显示「No data」；覆盖率只从面板与规则的**表达式**中读取，因为注释里提到的指标并没有接入。
...
252 篇 MADR 格式的 ADR 记录了主要的架构决策。以下是一些代表性示例：
...
252 篇 ADR 不仅记录了做出的决策，还记录了被否决的替代方案和接受的权衡。1 296 个文件里的 ~21 584 个测试、完整的 CI/CD 和严格的 MyPy 并非虚荣指标 — 它们是让这种复杂度的系统能够无回归演进的机制。
...
*本文档基于源代码（`apps/api/src/`、`apps/web/src/`）、技术文档（490+ 份文档）、252 篇 ADR 及变更日志（v1.0 至 v1.38.2）的分析编写。文中引用的所有指标、版本和模式均可在代码库中验证。*
