...
工具失败在这里以**结构化方式**读取（ADR-303）：`ToolMessage` 的正文是工具的文字说明，因此唯一诚实的判定是消息自身携带的标记——`status="error"`，经实测可在检查点之后保留。唯一的成功谓词 `core/tool_outcome.explicit_success` 同时服务于循环、查询登记簿和指标：声明的失败与空结果一样，不再换来额外的迭代。在 `REACT_CROSS_TURN_CACHE_ENABLED`（ADR-308，默认关闭）下，循环按注册顺序绑定全部工具，并把本回合的上下文放在问题之后——采用各服务商接受的形态，在启动时声明并校验——让一个回合的前缀成为下一个回合的前缀：在 396 个真实回合上测得，根据缓存机制不同，每回合节省 18% 至 42%；没有缓存的模型会多花钱；当目录超出上限或上下文窗口时，会回退到按相关性选择，并计入统计。
...
309 篇 MADR 格式的 ADR 记录了主要的架构决策。以下是一些代表性示例：
...
*本文档基于源代码（`apps/api/src/`、`apps/web/src/`）、技术文档（680+ 份文档）、309 篇 ADR 及变更日志（v1.0 至 v1.47.2）的分析编写。文中引用的所有指标、版本和模式均可在代码库中验证。*
