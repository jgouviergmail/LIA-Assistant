...
| 约束 | 架构影响 |
|------|---------|
| ARM64 自托管 | Docker 多架构、本地 E5 嵌入（不依赖 API）、Playwright chromium 跨平台 |
...
| 成本可控 | Smart Services（节省 89% token）、本地嵌入、prompt 缓存、目录过滤 |
...
| sentence-transformers | 5.0+ | 本地嵌入 | E5-small 多语言（384d）、零 API 成本、ARM64 上约 50 ms |
...
### 8.1. 为什么使用本地嵌入？（ADR-049）

纯 LLM 路由有两个问题：成本（每个请求 = 一次 LLM 调用）和精度（LLM 在约 20% 的多领域场景中判断错误）。本地嵌入同时解决了这两个问题：

| 属性 | 值 |
|------|------|
| 模型 | multilingual-e5-small |
| 维度 | 384 |
| 延迟 | 约 50 ms（ARM64 Pi 5） |
| API 成本 | 零 |
| 语言 | 100+ |
...
助手以四个主题（自我反思、用户观察、想法/分析、学习）撰写内省反思。两个触发器：对话后提取 + 定期整合（4 小时）。OpenAI 1536d 嵌入配合 `search_hints`（用户词汇中的 LLM 关键词）。注入到 **Response Node 和 Planner Node** 的提示中 — 后者使用 `intelligence.original_query` 作为语义查询。
...
| 本地嵌入 | 零 API 成本 | 约 470 MB 内存、初始加载 9 秒 |
...
`@connector_tool` 用于凭证注入，`@auto_save_context` 用于 Data Registry 持久化。

### 23.2. Prompt 系统

`src/domains/agents/prompts/v1/` 中的 57 个版本化 `.txt` 文件，通过 `load_prompt()` 加载并配合 LRU 缓存（32 条目）。版本可通过环境变量配置。

### 23.3. Centralized Component Activation（ADR-061）

3 层系统，解决重复问题：在 ADR-061 之前，启用/禁用组件的过滤分散在 7+ 个位置。现在：

| 层 | 机制 |
|----|------|
| Layer 1 | 领域门控：验证 LLM 输出的领域是否在 `available_domains` 中 |
| Layer 2 | `request_tool_manifests_ctx`：每个请求构建一次的 ContextVar |
| Layer 3 | MCP 代理端点的 API guard 403 |
...
| 049 | 本地 E5 嵌入 | 嵌入 API 成本 + 网络延迟 | 零成本、本地 50 ms |
...
*本文档基于源代码（`apps/api/src/`、`apps/web/src/`）、技术文档（190+ 份文档）、63 篇 ADR 及变更日志（v1.0 至 v1.11.1）的分析编写。文中引用的所有指标、版本和模式均可在代码库中验证。*
