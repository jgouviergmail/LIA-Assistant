...
| 完全透明 | 519 Prometheus 指标、内嵌调试面板、逐 token 追踪 |
| 生产可靠性 | 261 篇 ADR、由 pytest 在 1 349 个文件中收集的 ~22 775 个测试、原生可观测性、6 层 HITL |
...
自 v1.30.12 起，state 由一个**类型化的运行上下文**补充（`LiaRuntimeContext`，ADR-231）：一个声明为图 `context_schema` 的冻结 dataclass，承载运行的身份、偏好与活动依赖（SSE 队列、工具容器）。与 state 不同，该上下文从不写入检查点也从不复制 — 对象身份从节点到子图再到工具全程保持 — 图入口处的断言会拒绝任何缺失上下文的运行，包括恢复 HITL 中断时；此前这种缺失只会静默降级。
...
**没有任何仪表板能展示的指标，就是没有人会去处理的指标。** 代码发出的内容与操作者实际能看到的内容之间的差距是被测量的，而非假设的：`scripts/audit/measure_metric_coverage.py` 用 AST 解析每一处指标定义（而不是正则——正则会把 `ZoneInfo("UTC")` 当成一个 `Info` 指标），并把每个名称与所有仪表板面板、recording rule 和告警表达式逐一比对。已定义 519 项；其中 57 项明确列入一份**只能缩短**的基线：新出现的盲点指标会让构建变红，而已接入的指标必须从清单中移除——否则下一个盲点会悄悄顶替它的位置。没有这套机制的代价：一个开路失效的心跳来源曾让健康信号在一周内丢失 46.5% 的 tick，却没有任何指标能察觉（ADR-148）。这道守卫从构造上关掉了两个陷阱——带标签且从未触发过的计数器**根本不会暴露任何序列**，因此监视罕见故障的面板必须写 `or vector(0)`，否则会在操作者期待绿色零值的地方显示「No data」；覆盖率只从面板与规则的**表达式**中读取，因为注释里提到的指标并没有接入。
...
261 篇 MADR 格式的 ADR 记录了主要的架构决策。以下是一些代表性示例：
...
**静息坐标系，依测量重新居中**：Mehrabian 映射让**全部 14 种**人格都静息在 D > 0 一侧，因此五种需要负支配度的情绪在静息状态下始终无法抵达——阻尼是一个位似变换，无法修正这一点。两项开关在 v1.25.14 以**惰性**方式交付，正是为了让启用成为一个经过测量的决定。2026 年 8 月的生产测量（769 份快照、3 位用户、90 天）比模拟更有力地印证了诊断：负支配度占比 **0.0%**，而喜悦脉冲在 **31%** 的对话轮次中把喜悦推举为主导情绪，与实际报告的评估无关。两者现已成为代码默认值：在 0.20 处，人格目录跨越了零点（14 种中有 7 种静息于零以下，且人格次序完全保留），报告出的评估重新掌握情绪通道。同一次测量**推翻**的结论也一并记录：唤醒度同样被锁住，但原因是评估流本身，而非静息点的几何——本次变更并不修正它。
...
261 篇 ADR 不仅记录了做出的决策，还记录了被否决的替代方案和接受的权衡。1 349 个文件里的 ~22 775 个测试、完整的 CI/CD 和严格的 MyPy 并非虚荣指标 — 它们是让这种复杂度的系统能够无回归演进的机制。
...
*本文档基于源代码（`apps/api/src/`、`apps/web/src/`）、技术文档（490+ 份文档）、261 篇 ADR 及变更日志（v1.0 至 v1.40.0）的分析编写。文中引用的所有指标、版本和模式均可在代码库中验证。*
