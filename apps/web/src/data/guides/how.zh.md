...
### 23.7. 聊天 UX 改进（v1.16.9）

多项跨领域改进，共享同一理念：**即时反馈，不必要时零服务器成本**。

- **对话历史搜索** — `GET /conversations/me/messages` 新增 `?search=` 查询参数（PostgreSQL `ILIKE`，不区分大小写，MVP 区分重音）。前端通过 `useMemo` 对已加载消息即时过滤。
- **LaTeX 渲染** — `MarkdownContent.tsx` 接入 `remark-math` + `rehype-katex`。语法 `$inline$` / `$$block$$`。插件顺序 `rehypeRaw → rehypeKatex`，KaTeX 生成自身已消毒的 HTML。
- **语法高亮** — `react-syntax-highlighter`（PrismAsyncLight）懒加载，25 种语言按需加载，主题 `one-dark` / `one-light` 由 `next-themes` 驱动。
- **主动反馈持久化修复** — 👍/👎/🚫 按钮现在跨会话/跨设备持久消失。反馈持久化到 `conversation_messages.message_metadata` JSONB（`jsonb_set`，通过 `conversations.user_id` 子查询按用户隔离）。元数据键集中在 `src/core/field_names.py`。
- **工具 i18n 一致性** — Weather 和 Hue 工具现在正确传播用户语言（`if user_lang: language = user_lang`）。Hue 通过 `ConnectorTool._fetch_language()` / `_language_from_result()` 和 `_LANGUAGE_RESULT_KEY` 常量重构为线程安全的 Option C。6 个 `.po`/`.mo` 文件中添加了 126 条 gettext 条目。
