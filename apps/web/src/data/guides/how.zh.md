...
30. [演进计划：可见的工作，受治理的学习](#30-演进计划可见的工作受治理的学习)
...
| 完全透明 | 541 Prometheus 指标、内嵌调试面板、逐 token 追踪 |
| 生产可靠性 | 274 篇 ADR、由 pytest 在 1 548 个文件中收集的 ~25 732 个测试、原生可观测性、6 层 HITL |
...
| 指标 | 数值 |
|------|------|
| 测试 | 25,732 个（由 pytest 在 1,548 个测试文件中收集）+ 前端 7,642 个 vitest 测试（覆盖率阈值已锁定，ADR-116） |
...
长期记忆拥有自己的 PostgreSQL 模型；上文描述的检索正是它所使用的。检索路径随之迁移，混合路径却没有：截至 2026-07-27，它**已经没有任何调用方**，覆盖率 21 %，127 行中有 100 行从未被执行 —— 而调试面板却仍在向用户宣传这个选项。模块、配置、指标与界面显示已被一并移除（[ADR-168](https://github.com/jgouviergmail/LIA-Assistant/blob/main/docs/architecture/ADR-168-Removal-Of-Dead-Hybrid-Memory-Search.md)）。混合检索依然真实存在，只是在它真正被使用的地方：RAG Spaces（第 17 节）。
...
**没有任何仪表板能展示的指标，就是没有人会去处理的指标。** 代码发出的内容与操作者实际能看到的内容之间的差距是被测量的，而非假设的：`scripts/audit/measure_metric_coverage.py` 用 AST 解析每一处指标定义（而不是正则——正则会把 `ZoneInfo("UTC")` 当成一个 `Info` 指标），并把每个名称与所有仪表板面板、recording rule 和告警表达式逐一比对。已定义 541 项；其中 57 项明确列入一份**只能缩短**的基线：新出现的盲点指标会让构建变红，而已接入的指标必须从清单中移除——否则下一个盲点会悄悄顶替它的位置。没有这套机制的代价：一个开路失效的心跳来源曾让健康信号在一周内丢失 46.5% 的 tick，却没有任何指标能察觉（ADR-148）。这道守卫从构造上关掉了两个陷阱——带标签且从未触发过的计数器**根本不会暴露任何序列**，因此监视罕见故障的面板必须写 `or vector(0)`，否则会在操作者期待绿色零值的地方显示「No data」；覆盖率只从面板与规则的**表达式**中读取，因为注释里提到的指标并没有接入。
...
274 篇 MADR 格式的 ADR 记录了主要的架构决策。以下是一些代表性示例：
...
274 篇 ADR 不仅记录了做出的决策，还记录了被否决的替代方案和接受的权衡。1 548 个文件里的 ~25 732 个测试、完整的 CI/CD 和严格的 MyPy 并非虚荣指标 — 它们是让这种复杂度的系统能够无回归演进的机制。
...
## 30. 演进计划：可见的工作，受治理的学习

活动页面是一个**纯读模型**：并行抓取器（每个数据源一个会话——AsyncSession 不支持并发）聚合七张既有审计表，总数是整个窗口上的精确 `COUNT(*)`，上限会被声明（`truncated`），故障的数据源会被列出而不是被静默补全——诚实计数（ADR-185）贯穿始终。记忆遵循**取代轨迹**（ADR-235）：自动纠正会创建后继并归档旧事实（`superseded_by_id`），每次读取都通过一个中央谓词过滤活跃集合，轨迹在保留期后清除；手动编辑则保留其覆盖权限。已学规则是**第七类记忆**，注入提示词头部，享有同样的保护（固定、保留、GDPR）。语音韵律是对管理设置的**有界调制**（死区、硬边界、开关）——绝非替换。自主性始终有上限：ReAct 迭代预算随请求的领域跨度自适应，但绝不超过配置的上限，未知复杂度获得完整上限——只在可证明简单的场景省钱。
...
在 ADR-247 之前，LIA 发出所有这些可观测性数据却从不读取：处处埋点，对自己却一无所知。自我诊断子系统以「每个支柱一条设计规则」闭合了这个环路。
...
*本文档基于源代码（`apps/api/src/`、`apps/web/src/`）、技术文档（490+ 份文档）、274 篇 ADR 及变更日志（v1.0 至 v1.43.2）的分析编写。文中引用的所有指标、版本和模式均可在代码库中验证。*
