...
9. [Human-in-the-Loop：6 层架构](#9-human-in-the-loop6层架构)
...
29. [按文件管理：工作簿即表单](#29-按文件管理工作簿即表单)
30. [可见的工作，受治理的学习](#30-可见的工作受治理的学习)
31. [表情眼睛：由信号驱动的角色](#31-表情眼睛由信号驱动的角色)
32. [原生应用：一个外壳，您的服务器](#32-原生应用一个外壳您的服务器)
33. [自我诊断：会读取自身遥测的助手](#33-自我诊断会读取自身遥测的助手)
34. [算出来而不是猜出来：在早已存在的沙箱里跑一段临时脚本](#34-算出来而不是猜出来在早已存在的沙箱里跑一段临时脚本)
35. [在交付之前先测量颜色：设置页的调色板](#35-在交付之前先测量颜色设置页的调色板)
36. [特质不是反应：由回答自己声明的语气](#36-特质不是反应由回答自己声明的语气)
37. [三种机制应对一次汇聚：抚平上限看不见的突发](#37-三种机制应对一次汇聚抚平上限看不见的突发)
38. [会议纪要：这一行就是任务，模板就是契约](#38-会议纪要这一行就是任务，模板就是契约)
39. [三本登记簿，以及没有人要求过的那一本](#39-三本登记簿以及没有人要求过的那一本)
40. [每段关系一份小结：十个板块说不出的东西](#40-每段关系一份小结十个板块说不出的东西)
41. [工单板：一行记录、两个视角，以及一个会提问的助手](#41-工单板一行记录两个视角以及一个会提问的助手)
42. [进程的解剖：它加载的内容被声明、测量并加以限制](#42-进程的解剖它加载的内容被声明测量并加以限制)
43. [Live 模式：两个智能、一道接缝——以及按模式的一套规则](#43-live-模式两个智能一道接缝以及按模式的一套规则)
44. [结论](#44-结论)
---
...
| 完全透明 | 583 Prometheus 指标、内嵌调试面板、逐 token 追踪 |
| 生产可靠性 | 301 篇 ADR、由 pytest 在 1 852 个文件中收集的 ~30 855 个测试、原生可观测性、6 层 HITL |
...
| Ollama | 所有本地模型（能力读自服务器） | 原生客户端：可控思考、上下文窗口、受约束 JSON。零 API 成本、自托管 |
...
**本地模型的特殊情况。** 六家提供方使用远程 API；第七个跑在你自己的机器上，LIA 通过它的**原生 API**（`langchain-ollama`）与之对话，而不是它的 OpenAI 兼容层。这个区别并非表面功夫：兼容层无法表达 `think`（关闭会思考模型的思考，或选择其深度），也无法表达 `num_ctx`（上下文窗口——否则本地服务器会按显存大小自行选择，并悄悄截掉过长提示的开头），更无法把思考轨迹与回答分开。而且模型的名字说明不了它的能力，因此能力是**读自服务器**的（`/api/show`：工具、视觉、思考、上下文长度），同时供给运行时与管理界面——这样思考深度绝不会发给不会思考的模型，那本来会被服务器拒绝。
...
一个基本不变量贯穿整个后端。它在各处强制执行，因为常量与实际生产配置之间的偏差会导致静默 bug：
...
回合的开销遵循一条守恒定律（ADR-256）：推理时间与工具时间是两个各自命名的预算，绝不合而为一——一次委派（子代理、迭代式 MCP 任务、浏览器运行）在单个工具调用背后打开自己的 LLM 循环，按其流水线上限计算，仅此一项就等于全部推理预算。每个工具调用都带有独立上限，取自与 Pipeline 模式相同的超时家族，并确保永远不比下层已应用的上限更严格；工具自身抛出的超时归因于工具，而不是我们自己的截断。停止条件仍是唯一谓词、两个读者：路由器用它来决策，收尾节点用它来解释。

两种模式共享相同的工具注册表、HITL 系统、响应节点和可观测性基础设施。用户可通过聊天头部的切换开关在两种模式间自由切换。

工具返回的内容会**在 token 预算内逐项投射到循环的上下文中**（ADR-286）。结果中最重的条目列表在条目边界处分页——被接纳的条目是完整的，区块保持为有效的 JSON，标量整体传递，第一项总会通过——预算为 `min(REACT_TOOL_RESULT_MAX_TOKENS, 窗口 × REACT_TOOL_RESULT_WINDOW_FRACTION)`，窗口即 ReAct 槽位自己的上下文窗口。截断会在外部内容标签之后**明确告知**模型（多少项中的多少项，以及获取其余部分的途径），并按工具计数，因此一整箱完整邮件绝不会以一段在第一封的邮件头中间被截断的转储到达模型。提供商的原始树结构从不离开邮件构建器，而数据只由流水线执行器注入的工具会声明自己仅限流水线使用，而不是被提供给一个无法运行它的循环。

一个回合还欠自己一份能活得比它更久的工作记忆。状态受消息窗口限制，而 ReAct 回合每次迭代新增两条消息——足够长的回合因此会把**自己的问题**挤出这个窗口，此后用于分隔历史与当前循环的窗口化逻辑再也找不到任何切分点。因此，当截断驱逐了回合的问题时，归约器会在它的两条分支上重新固定这个问题；迭代预算与窗口大小之间的耦合也有了名字，而不再是一条心照不宣的算术关系。一个回合实际交付给模型的内容同样被度量——每次迭代的提示词体积及其占模型上下文窗口的比例——因为一个只度量迭代次数和时长的循环，度量了除真正增长之物以外的一切。

循环拿到的工具**按相关性绑定，而不是按注册顺序**（ADR-293）。路由器本来就会为问题计算一个嵌入向量来给领域打分；同一个向量再把每个可用的清单——原生工具与个人的 MCP 服务器一视同仁——排成一份循环读取的全局排名。选择过程由组合得出，任何地方都不写死工具名：先是检测到的领域的工具，再是其他每个家族里排名最高的几个，使每个家族都可触达，最后是排名的前列直至一个公开的预算；上限只剩下按稳定排序的安全网。一份手工声明的「核心」列表曾被写出又被否决，因为它把某一个账户的用法编进了多用户产品——按家族覆盖在结构上说的是同一件事。循环还挂载管道注入的知识空间块，从路由器预取的包里读取而不消耗它：管道知道的，循环也知道。绑定的工具数与其模式的 token 数都被测量，因为这个前缀占了首次调用的大部分却从未被计数。
...
### 5.5. 生成的产物：从请求到可下载的文件（ADR-226）

流水线的终点可以是一个文件，而不仅仅是文字。`generate_document` 工具沿用图像生成的架构 — 目录中的虚拟代理，无专用图节点 — 但它的「生成器」是一个专用 LLM 槽位（`document_generation`，与其他槽位一样可由管理员配置），以**按格式族类型化的结构化输出**调用：CSV/Excel 用表格内容，Word/PDF/Markdown/文本用章节树，PowerPoint 用幻灯片列表。模式在调用*之前*选定，因此每个响应都经过严格模式校验；然后由**纯本地渲染引擎**构建精确的字节 — openpyxl、python-docx、python-pptx、PyMuPDF：这些为 RAG 提取而内置的库，如今从读取转为写入，全程不依赖任何第三方文档服务。

三个设计决策支撑该功能。其一，产物的诚实性：表格单元格针对公式注入做了中和处理（探针证明 openpyxl 会把 `=1+2` 存为活公式），而合法的负数保持原样；付费 LLM 调用之后的任何失败都返回显式错误 — 绝无幻影卡片。其二，链式衔接：规划器可以把网络调研步骤的结果注入文档步骤（`source_data`），因此「调研后整理成 CSV」只需一次请求。其三，生命周期：文件落入既有的附件存储，与生成图像共用同一 TTL 清理；其卡片 — 由 SSE done chunk 实时送达、经由同一个共享序列化器持久化到消息元数据 — 显示确切的过期时间。

**质感属于渲染器，含义属于模型（ADR-274）。** 写作槽位所填的模式是*语义*的：它只说明一件东西是什么——标题、有序序列、引文、提示框、章节开场、双栏对比、带说明的表格——而从不规定如何绘制。版面决策归渲染器，并且用格式的原生机制表达，而不是模仿它们：Word 使用命名样式、`PAGE`/`NUMPAGES` 域与多级编号定义，因此目录与页码由 Word 自己重新计算；PowerPoint 在 16:9 舞台上使用模板自带的版面与占位符；Excel 用命名表格覆盖类型化的列，筛选与排序因此白得；PDF 的书签、链接与准确页码，来自把正文分页一次、再把前置页面拼接在前。这样的分工避免了让模型从模板目录中挑选——那是它无法判断的绘制决策——也让渲染可以在不改动模式的前提下持续改进。

**文字在排布之前先被度量。** PowerPoint 在打开文件时不会计算任何自动缩放，而库自带的适配会差出一倍：因此渲染器自己度量，估算器对照 PowerPoint 校准——全角字形算一个整 em，拉丁字形算一个实测的分数。放不下的内容先缩小到可读下限，再拆分为「标题（2/3）」两页；过长的要点在句末断开。绝不裁切，因为屏幕上被截断的文字是无声消失的信息。同样的原则约束对模型的调用：供应商声明被截断的回答会被拒绝并说明其预算（ADR-275），绝不补全成看似完整的文档。

### 5.6. 属于个人的东西：生成的文件与保存的回答（ADR-279、ADR-282）

产物和回答一旦存在，就遵循两种不同的生命周期，而两者都属于个人，而非对话。生成的文件——图片、文档、浏览器截图——是一份标注了**来源**的附件，列在专属图库中，附带精确总数和可见的保留期限；清空对话只移除个人上传的内容，绝不会移除 LIA 生成的内容，因为清理只移除其家族所声明的，而非看起来相似的。

个人想要保存的回答是另一种对象：它是文字，必须与气泡中呈现得完全一致，并且必须在经常被重置的对话之外留存。因此**书签是一份副本，绝不是一个指针**。点击时，回答（Markdown 或富 HTML 文档，原样保存）、产生它的请求以及回答的日期被写入一张专属的表。指向对话的两个引用在删除时 `SET NULL`：它们只在对话存续期间服务于气泡上的切换。请求在**服务端**解析，遵循与聊天相同的可见性谓词——回答之前个人最后一条可见消息，绝不是非轮次运行的合成问题——而 LIA 主动发送的通知则不保留任何请求，而不是保留一个错误的请求。部分唯一索引让切换在构造上幂等，账户上限因被强制执行而被公布，分页和总数来自同一个 `WHERE`，而运维者的开关只守护「保存」这一动作：已保存的内容仍可阅读、导出和删除。

读取和删除自己的文件不受上传上限影响：无论 `ATTACHMENTS_ENABLED` 如何，`attachments` 路由器都会挂载，能力守卫只落在上传上。由于每个生成的文档都经由这条读取路由提供，一个生成文档却不接受上传的实例——演示实例——能让自己生成的东西被打开，而过期清理会为该表的四个生产者运行，绝不只为上传而运行。

收藏的回答还会**作为知识空间的一份文档被索引**（ADR-291）：每个账户一个空间，按角色找到而不是按名字，在第一次收藏时创建，默认启用，不计入空间与文档的上限。收藏仍是记录，文档是它的投影——带日期的标题、引用的请求、回答本身（若是 HTML 页面则转为 Markdown）——因此对外暴露的状态（已索引、待处理、因支出上限而推迟、已停用、出错）在文档存在期间都由文档推导，费用只在就绪后才报告。投影先认领再计算，能力与支出的门在动作发生时读取，补录搭在空间的维护节拍上，优先服务最久未尝试的收藏——一个超配额的账户不再让其他账户挨饿。删除收藏会移除文档；文档不能从空间里移动或删除，按角色管理的空间也不能手动删除。
...
审阅者还有一个必须在设计上加以防范的盲区：被要求给出裁决的判定者能可靠地检查计划**包含**什么，却会漏掉它**缺少**什么。因此，在形成任何裁决之前，检查以一次覆盖检验开场——先列举请求所确立的各项要求，再逐项核对是否都有步骤覆盖。两道护栏防止它对健康的计划误报：由计划已获取数据的「分析」所满足的要求属于回答 LLM，天然已被覆盖；而任何能力都无法服务的要求同样不算缺失步骤。被判定为缺失的步骤，走的是与其他可修复缺陷完全相同的静默重新规划路径。
...
### 9.3. 草稿评审：一份描述，每份草稿一个问题

待确认的草稿及其执行报告**只描述一次**——一份由带标签的行、备注和区块组成的卡片规范，由每种草稿类型各自的渲染器根据显示注册表构建——然后**按界面分别绘制**：在聊天中，使用与邮件或日程相同的 `lia-card` 标记（标题栏、插图、带图标的字段、成块的正文）；在工单上、外部渠道上或 `markdown` 显示模式下，则是工单评论能够压平的 Markdown。界面由运行本身决定，绝不由调用方猜测。报告会说明做了什么、对谁、用了什么：一批中的每一行都带有其类型声明的关键字段，以及所发送文本的有界摘录，用该语言自己的引号引用。更新时的前后对比、不可逆性警告、i18n 标签和可点击链接都是这份描述的一部分。

一轮准备了**多份相互独立的草稿**——两封邮件、一个日程加一项任务、同一次 ReAct 迭代中的多次工具调用——会**每次中断提交一份草稿**：序列先列出这一轮准备好的全部内容，每份草稿都有自己的卡片、自己的问题和自己的修改，并说明进度（“第 2 份草稿，共 3 份”），在最后一次回答之前什么都不会执行；随后累积的决定一起执行，各归其类型，被取消的草稿会被报告而不是丢失。只有这个人**以列表形式预先批准**的一批——本轮中已批准的 FOR_EACH 步骤的条目，且为同一类型——才保留其成组确认。工单遵循同样的身份（屏幕上的那份草稿），而新的一轮总是从没有待审草稿的状态开始。
...
state 由一个**类型化的运行上下文**补充（`LiaRuntimeContext`，ADR-231）：一个声明为图 `context_schema` 的冻结 dataclass，承载运行的身份、偏好与活动依赖（SSE 队列、工具容器）。与 state 不同，该上下文从不写入检查点也从不复制 — 对象身份从节点到子图再到工具全程保持 — 图入口处的断言会拒绝任何缺失上下文的运行，包括恢复 HITL 中断时；此前这种缺失只会静默降级。
...
**来源信息在摘要中得以存活。** 由携带第三方文本的消息生成的摘要会继承一条来源横幅，摘要提示词也会把第三方的主张放进专门的小节并注明来源——参见 §19.6。
...
长期记忆拥有自己的 PostgreSQL 模型；上文描述的检索正是它所使用的。在它旁边还存在过第二条「混合」路径：**没有任何调用方**，覆盖率极低 —— 而调试面板却仍在向用户宣传这个选项。模块、配置、指标与界面显示已被一并移除（[ADR-168](https://github.com/jgouviergmail/LIA-Assistant/blob/main/docs/architecture/ADR-168-Removal-Of-Dead-Hybrid-Memory-Search.md)）。混合检索依然真实存在，只是在它真正被使用的地方：RAG Spaces（第 17 节）。
...
**去重纪律**：无写入时守卫。在巩固时，`STEP 1` 执行明确的成对扫描以融合语义重复，`STEP 5` 主动将收敛的 L1 聚类成 L2 模式。
...
画像由**日志之外的四个来源**组成（ADR-292）：记忆、兴趣、习得的习惯与关系回顾作为综合的素材进入整合提示词。日志领域不导入这些领域中的任何一个——其中两个已经导入了它——所以每个来源都通过一个词汇封闭的共享注册表提供，启动时显式安装，不完整则拒绝启动。读取器以精确的总数回答 `used | empty | disabled | unavailable`，受其在动作时读取的三道门约束（上限、运营者的开关、个人的偏好），并且从不抛出异常：失明的来源被点名，绝不被当作空。组装器在每来源上限下一次读取一个来源，超出总预算时整节丢弃；提示词只在某节存在时才渲染它，并禁止重复罗列某一行或提及来源。出处与画像一起持久化，在同一次写入中，并绘制在画像之下；有变动的来源会为下一次整合重新打开资格。
...
**日志的自我评估闭环与自适应阈值。**注入到回复中的指令会在下一轮根据用户的反应重新评估：LLM 只负责发出 `evidence` 或 `contradiction` 信号，计数器由系统持有，服务端钳制确保操作型指令在没有证据时无法获得「高」置信度 — L2/L3 不受此限，它们的证据是条目间的收敛。整理的触发条件由增量驱动（存在待办工作：从未整理过，或自上次整理后有条目被修改），而非绝对条数。最后，决定注入与否的相似度阈值不再全局统一：一个有界（0.55–0.70）、带滞回（每 24 小时最多一步 0.01）、可关停的控制器根据每位用户真实的分数分布逐一学习 — 状态仅作参考（Redis，滑动过期），读取失败即回退到静态默认值。
...
**模型家族只声明一次，而短回答在不推理的前提下请求**（ADR-285）。一个模型的*家族*是其推理 API 的形态——DeepSeek 是开关加努力等级，Gemini 2.5 是预算，OpenAI 是级别——它从 `reasoning/profiles.py` 中的唯一一处声明推导而来，适配器、校验器和结构化输出的绕行路径都读取这一处：三个各自私有的前缀判断曾让某供应商改名后的旗舰模型同时从三处漏过，既没有提供等级，也没有发送关闭开关。同一条接缝也服务于只需要两句话的调用方：在默认会思考的模型上，供应商把思考*计入* `max_tokens`（OpenAI、Anthropic、Gemini 和 DeepSeek 都是如此），于是一个小预算买到的是隐藏的推理而不是回答。因此 `short_answer_config` 在解析出的配置能关闭推理的地方声明*不推理*，并只在那里保留小预算；其他地方沿用槽位自身的预算，因为一个包含思考的上限并不是对回答的上限。同样，一个家族的输出上限读取自目录行，而不是字面量。
...
对于电子邮件，这个统一模型是经过测量的，而不是假定的：三个规范化器产出同一套 `EmailMessage` 词汇——正文是纯文本而非 HTML，引用的历史记录和签名在客户端边界即被移除（六种语言，在 48 封邮件正文的语料上测量）——没有任何一个再去伪造另一个提供商的格式。随后 `get_emails_tool` 根据问题选择提供什么，并说明其代价：`metadata` 只列出不含正文的清单，`full` 读取按段落分页的干净正文，`summary` 则基于**每封邮件一份的要点摘要**（要点、关键信息、待办、类别、重要程度）进行推理——由一个不做推理的小模型只计算一次，缓存三十天，并受支出上限约束——模型失败时绝不凭空编造。正是这一点让「总结我的未读邮件」或「本周新闻简报的综述」可以在二十封邮件上完成而无需全部加载，对 Gmail、Outlook 和 Apple 一视同仁（ADR-287）。
...
Google 和 Microsoft 的授权按提供商账户集中进行，而不必为每项服务重复同意。一个授权记录管理已批准的权限和令牌续期；Gmail、日历、云端硬盘及对应的 Microsoft 服务仍是可分别启用或断开的功能。OAuth 回调验证预期身份与签发方、state 和 PKCE，不会悄悄换成另一个账户。

### 13.4. 两条认证路径

并非每个连接器都需要账号。**OAuth 连接器**保存用户的个人凭据：Gmail、日历、联系人、Drive。**平台密钥服务**不保存任何用户数据——用户只需开启，密钥属于这套部署：路线、地点、天气、环境。`ConnectorType.uses_global_api_key` 承载这一区分，工具基类根据**解析后**的类型选择凭据路径。因此同一个功能类别可以混用两者：天气既接受使用个人密钥的服务商，也接受平台服务，而调用方并不知道是谁作出了回应。

还有第三种情况：**借用相邻连接器令牌**的客户端。电子表格和文档使用 Drive 的令牌读写；Gmail 设置使用 Gmail 的令牌。设置页面不会因此多出一个连接器，这是有意为之——用户授权的是一个工作空间，而不是一个 API。其后果曾被实际测到：客户端缓存以用户和连接器类型为键，于是两个共享令牌的类会互相取到对方。现在这个键还带上了类名。

这一区分也决定了**新账户**的起点。五个连接器不需要个人提供任何东西——维基百科、网页浏览、地点、天气、环境——它们在注册时依据后端声明的列表（`ConnectorType.get_keyless_types()`）启用，前端镜像同一列表，一项测试固定了两者的一致性。配置步骤绝不会阻塞注册，并以书面方式拒绝：被管理员关闭的类型、实例缺少平台密钥、网页浏览被禁用。已有账户绝不会被触及。
...
**电话作为一个渠道（ADR-290）。** 确认卡片保护的是从未提出任何请求的第三方；当 LIA 打给*本人*时，该确认的人正是接电话的人——前提是号码已证明属于她。因此身份是在设置中登记的号码，完整显示，再由一次通话验证：LIA 读出验证码，本人输入确认（尝试次数有限、验证码短期有效并绑定号码、恒定时间比较、与所有付费通话共用的每小时上限）；从不按姓名匹配，第三方通话工具也拒绝该号码。**同一个**语音助手服务于三种授权——第三方授权固化在助手中，本人授权和验证授权作为服务端渲染的按次覆盖发送——因此本人的上下文绝不会固化到一个也会拨打陌生人的助手里；而助手*听起来的样子*（模型、语言、声音、音频格式、时长上限）属于服务商门户，从不是设置项，因为服务商会把 PATCH 与已存储的配置合并，固定的模型在每次同步时都与门户的推理设置冲突。`call_me` 在构造上不带卡片，这也让例程得以安排它。通话在令牌预算内带上聊天的上下文（记忆、日程、提醒、未了事项、最近的交流——一个开关可关闭）以及为助手设置的个性，每次读取都记入查阅登记簿。在第二个开关之后，助手可以在通话中**实时读取 LIA**，且不执行任何操作：工具集是对目录的一条规则而非清单——所有只读工具，属于电话提供的领域，且必填参数可以口述（22 个领域 55 个工具，外加原生的记忆回忆）——仅在本人通话期间挂到助手上，因为服务商拒绝在按次覆盖中传入工具标识；每个结果在逐项分页之前先被压缩为语音能说出的内容（标识符、链接、嵌套结构永远不会到达语音；过去原始数据只装得下一个，如今四个周末事件都能容纳）。每个领域都有本人自己的开关，每次回调时从她的记录中读取。通话结束后，所说的话会作为**本人自己的消息**回来：转写被摘要后由回合外引擎作为*本人所说*的一轮回放——记忆、日志与心理画像的提取像在聊天中一样运行，消息带有电话标记，草稿在聊天中等待确认。回放在回合前被认领并以条件更新结算，回放中途崩溃会退回为一条说明原因的通知，十种回放结论被统计并绘制在通话列表上——“无人接听”和“线路故障”与“他人接听”区分开来。一次通话的每一分钱都归于同一个运行标识——实时查询、摘要、回放的回合——因此聊天计量器已在读取的按运行汇总就是这次通话的账单，显示在回放的回复和通话列表上；使用本人服务商密钥运行的部分在那里计费，这里从不统计。

**每种模式一条规则，无论哪条线路（ADR-301）。** 电话在*结束时*回放对话；浏览器的 Live 在*通话中*把每个请求交给聊天——同一个词，两种真相。语音会话现在有了模式，规则只跟随模式：在 **Live**（电话的默认值，在设置 › 电话 · 我的身份中选择）下，语音既不持有上下文也不持有工具，通过一个函数把每个请求交给 LIA——正是浏览器 Live 向其服务商声明的那个函数——服务器在本人说话时把它变成本人的一轮聊天：LIA 的提问（提问就是结果，下一个请求会续接提问的那一轮）、确认、登记簿、配额以及对话中的记录都随之而来；在 **Live 直连**下，语音替本人读取、不执行任何操作，结束时她的话作为她的一条消息回放——在线路上如同在浏览器中，浏览器的直接会话也不再是“什么都不记录”。一个统一的收尾为两条线路记账：纯语音交流归档、精确数字的卡片、决策、学习——或者摘要、回放的回合以及结算时重写的卡片，费用重新读取。只有电话服务商能回调实例时才提供 Live；否则设置会说明，通话以直连进行。一切都在服务商的真实引擎上测量过，没有用电话：语音宣布调用并继续对话，超过服务商的超时后迟到的答复会丢失——因此桥在此之前回复——而包含一次委托的交流整体属于图。

### 13.6. 读取附件：有文本读文本，否则交给视觉模型

列出一封邮件的附件是元数据读取；打开一个附件是下载，三个客户端都把它放在邮件协议的同一个方法之后——同名、同一选择规则、同一组错误——因为供应商之间的不对称正是连接器缺陷的来源。选择逻辑是一个共享的辅助函数：按标识符，否则按名称，名称有歧义时连同候选项一起拒绝。这是必要的，因为 Gmail 的附件标识符并不稳定：在真实邮箱上测量，同一封邮件两次读取之间它会改变，所以列表给出的标识符在工具重新读取邮件时可能已经失效。因此过期的标识符不是谎言：给了名称就按名称决定，只有一个部件时没有歧义，其他情况一律返回当前有效的标识符让模型重试。

字节由一个不接触任何客户端的模块读取。MIME 类型来自魔数字节，发件人的头部只是备选；文档在工作线程中经由知识空间自己的提取管线处理（十五种格式）；图片，或页面只有图像而没有文字层的 PDF，被渲染为数量和尺寸都受限的 PNG 页面交给视觉模型——栅格化本身也受限，边长 200 英寸的页面不得在任何缩放之前分配数 GB 内存——被截断的页面集合会明确告知模型，而不是当作全部读取。视觉调用计入本轮开销，由运行时自身的配置携带，使账户和实例的两个上限都能看到它；配额拒绝记为「跳过」而非「失败」，被截断的回答视为拒绝。大小上限一直传递到客户端，列表已标明过大的部件在一个字节流动之前就被拒绝。返回的内容像邮件正文一样分段送达，并作为外部内容包裹，文件名在标签内转义——附件是陌生人发来的东西，它的名字也是。
...
LIA 在服务器暴露内容的两个层面上都跟进协议的当前修订版——**2026-07-28**：如何与之对话，以及它声明了什么。读懂一份声明本身就是一个独立的问题，并且只有一处实现（ADR-255）。规范将工具的输入模式定义为 JSON Schema 2020-12，并允许其中出现*任何*关键字：一个可选参数既可以写成 `"type": ["string", "null"]`，也可以写成 `anyOf`；嵌套结构可以通过指向 `$defs` 的 `$ref` 声明；封闭集合可以用 `enum` 或 `const` 表达。同一份声明有两个读者——适配器把它变成提供给模型的签名，目录把它变成计划条目——而对同一件事的两种读法最终必然分歧。因此 `json_schema.py` 是唯一权威，并由一个一致性测试比对两者各自的推导结果。其中每个函数都是全函数：无论服务器发来怎样的声明都会给出答案，因为在这里抛出异常的代价是丢掉一个工具，而丢掉的工具意味着用户失去了一项能力却无人告知。仍然无法判定的部分——一个 `not`、一个找不到的引用——会被宽松地定型而不是丢弃；而服务器真正强制执行的内容（封闭集合、取值边界、数量上限）会以原生工具一直使用的那套约束词汇发布给规划器。

客户端是**双代兼容的**（MCP SDK v2，ADR-224）：它既支持 2026-07-28 无状态协议修订版，又能对更早的服务器自动回退到旧的 `initialize` 握手——每台已配置的服务器都原样继续工作，同时新一代服务器变得可以接入。LIA 在握手中标识自己（`clientInfo`）；当某台服务器拒绝 LIA 支持的所有修订版时，用户会得到可操作的诊断信息，而不是埋在嵌套 `ExceptionGroup` 里的原始传输错误。
...
工具的行为注解（`readOnlyHint`、`destructiveHint`）只按**单一方向**解读。规范在此处是强制性的：只要不是来自受信任的服务器，客户端就必须将其视为不可信。因此，被声明的变更操作会被采信——最坏的情况不过是一个说谎的服务器为自己换来一次多余的确认——而「只读」的声称则永远不被采信，因为已声明的类别会压过基于名称的启发式判断，从而把该工具移出无效变更的安全网，也移出 HITL 作用域检测。在迭代模式下，一台服务器的所有工具共用同一个确认设置；凡是服务器自己声明为破坏性的工具，都会照常请求确认。
...
有两个事实作为数据随每台服务器一起传递，绝不写进提示词的文字里。凡是凭据属于用户本人的服务器（OAuth、个人 bearer 或 API 密钥），都会在模型读取其能力的每个位置声明其账号归属——委派工具的描述、规划器清单、子代理的上下文以及生成的领域描述——因此“我的仓库”会解析到已连接的账号，而不是以反问收场；未经身份验证的服务器则什么也不声明，因为声称相反就是编造能力。此外，子代理返回的是数据而非计数：其输出遵循与所有 ReAct 工具相同的“消息 + 有界数据块”契约，第三方内容被标记为外部内容。
...
唤醒词（"OK Guy"）通过浏览器中的 Sherpa-onnx WASM 实现（零外部传输）。后端通过 ThreadPoolExecutor 使用 Whisper Small 转录（99+ 语言，离线）。按用户设置 STT 语言；每个工作进程一个有上限的 `OfflineRecognizer` LRU 缓存（`VOICE_STT_MAX_RECOGNIZERS`，默认为 1）——构造时不加载任何东西，第一次转写为其语言付出加载开销，一次淘汰把内存归还给系统，常驻数量按工作进程公布（`voice_stt_recognizers_loaded`）。
...
**阶段 1 — 决策**（`heartbeat_decision` 槽位，在管理目录中选定的一个经济型模型）：
...
2. `ContextAggregator`：通过 `asyncio.gather` 并行获取 14 个源：Calendar、Weather（变化检测）、Tasks、Emails、Interests、活动记录、近期 heartbeat/兴趣通知、其他主动表面（已触发的提醒、自动化结果、通话报告——扩展的防冗余窗口）、Health、即将到来的生日开放事项（承诺登记簿，ADR-139）、习惯（惯常时段已过的学到的例行动作，连同它通常请求的内容）以及工作看板。随后**第二遍**从聚合上下文推导动态语义查询来挑选日志与记忆（ADR-135 对称性），并计算考虑实时路况的出发建议（Routes ETA，受开关控制）。兴趣以**多样化样本**形式进入（`pick_varied_sample`：每个主题一个兴趣，最久未服务的主题优先）——模型只能提及展示给它的内容，因此轮换是机械保证的

   **连接与被打断是两个决定**（ADR-197）。其中十三个来源各自带有开关，并在**获取之前**生效：被拒绝的来源不再参与决策，*同时*不再消耗一次 API 调用，而无需断开服务连接 — 因此不会失去提问所用的工具。存储保存的是**拒绝**，而非许可：`NULL` 表示「从未表态」，因此既有账户保持原有行为，后续新增的来源在无人拒绝前保持开启。不属于来源的内容 — 活动记录、防冗余窗口 — 按设计不在注册表中：关闭它们只会让助手重复自己，而不是减少打断。依赖关系则被**声明并公开**：出发建议读取第一遍获取的日历，拒绝日历会让它沉默；面板会明确说明，而不是留下一个亮着却无效的开关。
...
**活动守卫是一个注入式探针，账户挑选是公平的。**「不打扰活跃用户」这条规则通过一个端口（`ActivityProbe`）执行：每个调度器将其接入真实的活动来源 — 最后一条人类消息，排除自动化行，且限定在冷却时间范围内。通用检查器不了解任何领域模型：它只接收探针，读取失败会计入运行器的失败统计，而不是悄悄变成放行。在上游，候选账户的挑选把启用标志下推到 SQL 并随机排序（`ORDER BY random()`）：超出批次大小时，没有任何账户会被系统性地排在最后。SQL 时间窗预过滤经评估后被否决 — 一条损坏的时区数据就会让整个批次失败，而收益只有微秒级。
...
循环使用 APScheduler 配合 Redis 领导者选举（SETNX、TTL 120 秒、每 5 秒复检），以 `FOR UPDATE SKIP LOCKED` 实现隔离，计划自动批准，连续 5 次失败后自动停用。

**重复是一个乘积**：哪些日历日 × 一天中的哪些时刻。同一个引擎（`src/core/recurrence`）同时服务于例程与提醒——`dateutil.rrule` 逐日枚举，每个时刻以 `fold=0` 本地化，序列按时刻排序并去重，因为在冬令时切换时两个墙上时间会落在同一时刻。日期绝不交给 cron：当时差在本地午夜变化时，`IntervalTrigger` 会跳过整整一天——每年在 73 个时区丢失 142 次执行，且日志中毫无痕迹。

**只响一次的提醒不是例外，而是通例**：被问及下一次该设定何时，`rearm_after` 对已消费的单次场合回答 `None`，而 `None` 意味着「删除」。代码中没有任何分支会去问「这会重复吗」。类型上的推论：提醒的 `trigger_at` 保持 NOT NULL——没有下一次的提醒即被删除——而例程的 `next_trigger_at` 可为空，因为那里有价值的是配置本身。

上限由**消费方注入**，绝不写死在模型里：例程每天 12 次，提醒每天 48 次。每个对话工具都会公布它所执行的边界（ADR-184），因此超出范围的数字由规划器自身的钳制修复，而不是被报告为缺陷。

每次 tick 都以**执行历史**中的一行结束，在标记事务内于结果处写入——五种结局，对应执行器的五个出口，位于保存点中且绝不阻塞。当前周由服务端用同一个引擎计算：单元格取服务时刻与槽位**相等**的最后一次执行，绝不使用容差窗口，因此计划变更会天然重置网格，浏览器也从不重新解读计划——它只负责绘制。

提醒的那句话遵循与任何短回答相同的规则：在模型能够停止推理的地方以**不推理**方式请求，并使用一个小的回答预算——而一条回来是空的、或被供应商报告为在预算处截断的回答，是拒绝而不是消息。预先写好的备用句子会代替它发出，已发生的花费予以保留，警告中会注明模型及其推理 token 数。通知是 LIA 主动行为中人们真正能感受到的唯一一种；它绝不会只剩一个铃铛送达。

### 16.4. 让推送通知真正导向一次决策

Google 的推送通道原本是活的，却在功能上空转：唯一的消费者只是让缓存失效，于是一条通知为一张没人打开的卡片买来了新鲜度。现在，一条被处理的通知会**把用户排入队列**——按 (用户, 提供方) 使用 `SET NX`，因此一阵密集到达只算一次唤醒，时间以第一条为准——而 webhook 仍然只回 200，不作任何决定。一个短促的、经领导者选举的扫描在**完整**的资格校验下服务这个队列：时段、配额、冷却、来源偏好。只有「保底最小值」的平滑处理被跳过，因为唤醒是在回应一个事件。

在唤醒被真正服务之前，邮件增量只是**预读、绝不消费**：被拒绝的唤醒会把消息留给下一次例行处理——两个 Gmail 锚点刻意保持独立（通道的锚点是最后*看到*的事件，heartbeat 的锚点是最后*消费*的邮件）。预过滤是确定性的，并以设置形式公开：必需标签、排除的类别、排除邮件列表；以及在前瞻期内开始、由他人改动或仍等待你答复的事件。决策也知道自己为何被唤醒：一行 FRESH 开启其上下文，审计记录保存 `trigger = push | tick`，历史界面据此显示。

Drive 的变更根本不是一次决策：它从通道的令牌开始抽取变更流，并精确地重新索引位于已关联文件夹下的文件，走的是完整同步如今共用的按文件摄取路径。

### 16.5. 预判时刻，与按分钟服务的守候（ADR-281）

心跳是**周期性**的：它无法回到某一个时刻。一场会议 15:00 结束，下一轮扫描落在 15:22，而且落在一个也许不含该账户的批次上；上下文的日历窗口又从 `now` 开始向前看——一场已经结束的会议对决策是不可见的。因此有一张 `proactive_moments` 表来保存这些时刻：每个账户、每个来源一行，以 `(账户, 类型, 来源)` 唯一，使一场会议永远不会产生两个时刻；由检测器写入，由单一的扫描服务——带抖动，处于 TTL 与间隔绑定的调度器锁之下。认领是同一事务内的 `FOR UPDATE SKIP LOCKED` 加条件 `UPDATE`，带所有者令牌；无人结算的认领在租约到期后被**收回**，绝不永久遗留。在被服务之前，时刻会被**重新校验**：会议已取消、已拒绝，或本人已经写过——则放弃。

时刻在**完整**的资格检查器下运行，只绕过概率平滑和学习到的节律——与推送唤醒完全一致，理由也相同：一个时刻不能被推迟。**提出的是问题，绝不是评价**：一个开放式问题，最多两个事实，不作评判，绝不叠加两个时刻，也绝不重复同一个问题。「会议中」守卫在 Redis 判定缓存之后读取日历，命中时不记录任何查阅；读取失败则声明为 `failed`。控制随能力一同交付：每种类型一个开关、能力自身的开关、按类型与结果计数的计数器和心跳看板上的「到期 → 已通知」延迟，以及 `task moments:preflight`——它只说明某个账户此刻会收到什么提议，不写入任何东西。

同一个唤醒还服务**守候**：一条「来自此发件人的邮件」条件例程过去要等执行器的下一轮，最多两小时；已握有 Gmail 增量的唤醒会在预过滤器给出判定之前先服务它——预过滤器回答「这值得一次唤醒吗」，守候回答「这是我在等的吗」。它绝不执行例程：只把到期时间提前，执行器仍是唯一的裁判；并且它把下一次读取安排在已公布的搜索缓存 TTL 之后，因为在邮件到达前填满的缓存会回答「未满足」，而这个判定会消耗掉这次布置。结束的例程会**关闭**（`is_enabled = false`、`status = completed`）——它的终点只有一个权威：已经存储的 `SeriesEnd`。简报中的「守候」按钮写入的就是这条例程，以发件人为键、绝不以主题为键，并且先读取账户已持有的内容再写。
...
无法索引的文档会**说明原因**：一个封闭的代码存于该行（文件丢失、无法提取、没有文本、没有段落、段落过多、重试耗尽），显示时翻译成六种语言，出现在文档行之下。最常见的情形——页面只有图像而没有文本层的扫描 PDF——与真正空的文件区分开来，并附上解决办法：在 Google 文档中打开该文件（它会识别文本），再通过 Drive 来源把它加入空间。代码不提供的字符识别，也不会被承诺。

### 17.2. System RAG Spaces（ADR-058）

内置 FAQ，从 `docs/knowledge/` 索引。QueryAnalyzer 的 `is_app_help_query` 检测，RoutingDecider 中的 Rule 0 覆盖，App Identity Prompt（约 200 token，懒加载）。过期判定同时依据源文件的 SHA-256 **与**已入库语料库本身（每条解析条目一个 chunk、恰好一个文档）：签名相符但行数不对，意味着需要修复，而不是无需处理。自动索引在每个 uvicorn worker 中都会执行，因此空间所在行以 `FOR UPDATE SKIP LOCKED` 方式被占用 —— 只有一个写入者，其余直接跳过而不排队 —— 并且每个向量都在第一条删除语句**之前**算好：供应商的拒绝不会删除任何内容，此前的语料库继续提供服务（ADR-162）。

### 17.3. 邮件来源：由用户指定的范围（ADR-262）

一个空间可以关注某个 **Gmail 标签**。只有带该标签的会话才会被渲染——一个会话对应一个
Markdown 文档：主题作标题、消息按时间排列、附件只保留*名称*——随后像其他文档一样被索引。
在 Gmail 中移除标签，下一轮处理即删除对应文档：删除动作正是用户早已熟悉的那一个。

索引整个邮箱经过衡量后被否决：它会把一份私人档案复制到第二处存储，把嵌入开销花在以通知和
邮件列表为主的语料上，并让检索被谁都没打算保留的文本稀释。标签是用户在每天都会打开的工具里
维护的一种指定。完整同步会在列出会话**之前**先锚定 Gmail 历史，因此列举期间到达的消息会
在下一次增量处理中补上，而不会落进空档；该处理搭载第 16 节的推送唤醒，且不受任何通知闸门
约束——索引不是决策。

### 17.4. Drive 文件夹是一棵树，计数是精确的（ADR-297）

Google Drive 每次调用只列出一层，因此已链接的文件夹按广度优先遍历，受文件数和文件夹数双重限制，并明确声明截断：快捷方式既不列出也不跟随，被两次到达的文件夹只遍历一次，无法读取的子文件夹被计数并跳过，只有根目录无法读取才算错误。遍历过的文件夹集合随数据源一起保存，因此推送路径会把变更路由到整棵树，并让集合跟随变更流——在树下新建的文件夹会先加入集合，之后同一变更流中在其内部新建的文件才到达。位于已链接树内部（或其上层）的文件夹会被拒绝：两个按文件标识符去重的数据源会在每次同步时互相抢走对方的文档。

大规模同步之前显示的计数由负责索引的代码计算：同一遍历、同一套「支持」与「未变化」判定、空间自身的文档上限。它对每个文件分类——新增、已修改、未变化、不支持、超出容量——按钮只在超过实例设定的阈值时才询问用户；被上限截断的遍历会说「至少」。每次同步，无论手动还是后台，都会为该动作记录一条查阅记录——在任何运行之外的读取会发布自己的运行，因为登记簿只保留收集器收集到的内容。

### 17.5. 空间中的文档以副本形式加入消息（ADR-295）

聊天的检索只读取启用的空间，因此索引在已暂停空间中的文档无法通过提问触及，除非重新启用整个空间。所以输入框的「+」会列出用户拥有的每个空间中已就绪的文档，已暂停的空间被标注而不是隐藏，附带分页和精确总数，被选中的文档通过附件自身的机制被复制：存储文件换上新名字，文本经由知识空间的提取管线——十五种格式，而不是上传只允许的图片和 PDF——记录被视为一次上传，因此重置对话会移除副本，而空间、其索引和文件永远不受影响。只提供已就绪的文档，两个能力开关都必须打开，当文本达到上限时会告知模型：截断必须声明，绝不默默执行。
...
**技能脚本运行在一次性容器中。** 没有 Docker 套接字、没有网络（助手为计算而写的脚本是唯一的例外，而它也只能经由一道门离开——§34 的出口代理）、只读根文件系统外加一小块可写 tmpfs、非特权 uid、丢弃全部 capability，并对内存、进程数、CPU 和文件大小设限。关键在于子进程*继承*了什么：生产环境中 API 属于 `docker` 组，而组是会被继承的——仅仅切换 uid 仍会让套接字可达。脚本的**源码**以参数形式传入而非挂载，因为 API 本身就是一个容器，挂载会解析到宿主机；这一选择同时让 stdin 保持空闲，用于承载契约所依赖的 JSON 负载。当守护进程不可达时，执行会被拒绝而不是降级——一个会自我关闭的沙箱什么也保护不了。
...
**而无法在摘要中存活的标记，就是会过期的标记。** 压缩会重读整段对话并以 `SystemMessage` 重新发出——这是权限最高的通道，且在其后的每个回合都被保留，因为它*就是*这段对话的压缩记忆。一个被要求保留标识符、决定与结果、却没有被告知任何来源信息的摘要器，会忠实地把发件人的要求提升为既定事实。因此标记随文本一同流动：由携带第三方文本的消息生成的摘要会继承一条来源横幅，该横幅在写入时计算，而非从模型选择复述的内容中反推；摘要提示词也会把第三方的主张放进专门的小节，并注明其来源。横幅位于那个供四处读取方识别该消息的标记之后——改动前缀会让这段对话的压缩记忆悄然消失。确定性回退遵循同一规则：从被标记区域内部采集到的标识符会被列为不可信，而非列为对话的关键标识符；未闭合的标记则会污染整条消息。
---
...
| 技术 | 角色 |
|------|------|
| Prometheus | 583 自定义指标（RED 模式） |
| Grafana | 30 个生产就绪仪表板 |
| Loki | JSON 结构化日志聚合 |
| Tempo | 跨服务分布式追踪（OTLP gRPC） |
| Langfuse | LLM 专用追踪（prompt 版本、token 用量） |
| Alertmanager | 14 项关键告警核心，邮件通知（附带处置指南，按环境设置阈值） + 指向 LIA 的 Webhook：每条警报都成为产品内的事件（ADR-247） |
| structlog | 结构化日志，带 PII 过滤 |

**没有任何仪表板能展示的指标，就是没有人会去处理的指标。** 代码发出的内容与操作者实际能看到的内容之间的差距是被测量的，而非假设的：`scripts/audit/measure_metric_coverage.py` 用 AST 解析每一处指标定义（而不是正则——正则会把 `ZoneInfo("UTC")` 当成一个 `Info` 指标），并把每个名称与所有仪表板面板、recording rule 和告警表达式逐一比对。已定义 583 项；其中 44 项明确列入一份**只能缩短**的基线：新出现的盲点指标会让构建变红，而已接入的指标必须从清单中移除——否则下一个盲点会悄悄顶替它的位置。没有这套机制的代价：一个开路失效的心跳来源曾让健康信号在一周内丢失 46.5% 的 tick，却没有任何指标能察觉（ADR-148）。这道守卫从构造上关掉了两个陷阱——带标签且从未触发过的计数器**根本不会暴露任何序列**，因此监视罕见故障的面板必须写 `or vector(0)`，否则会在操作者期待绿色零值的地方显示「No data」；覆盖率只从面板与规则的**表达式**中读取，因为注释里提到的指标并没有接入。
...
`src/domains/agents/prompts/v1/` 中的版本化 `.txt` 文件（每个提示一个，`PromptName` 字面量由测试保持同步），通过 `load_prompt()` 加载，LRU 缓存容量大于整个语料库。版本可通过环境变量配置。

**提示词只陈述代码所强制的内容，别无其他**（ADR-284）。文件中的每个 `{placeholder}` 都在加载它的模块里有生产者，而包含 `{{` 的文件由 `.format()` 渲染——一个 AST 守卫逐文件读取这两点。每个上下文只有一条指令，写在文件里，且只在内容存在时才输出（`response_context_sections.txt`），因此一个空轮次不会带任何空包裹，也不会带关于并不存在的文档的指令。提示词公布的上限就是工具强制的上限，来自同一个常量；数字来自设置，事实来自应用它的代码；一项能力只有在本轮能够运行时才会被承诺。散文从不存放在 `.py` 里，备用文本也不：单行脚手架存放在由唯一解析器（`parse_prompt_sections`）读取的 `键|模板` 行文件中，而文件缺失意味着部署损坏，应当大声失败，而不是悄悄换一种措辞。
...
- **富HTML模式：组件词汇表** — 当用户选择富HTML显示模式时，提示词指令会提供七种由设计系统样式化的组件（带标题的提示框、带图标的徽章、原生 `details` 折叠区、键值列表、响应式分栏、编号步骤、关键数字卡片），以及行内强调 `mark`/`kbd`/`abbr`，并遵循明确的排版规则——凡是承载数据的回答都是一个经过编排的页面（一句导语、每个侧面各占一节并使用相应组件、一个收尾提示框），形式取决于该回答自身的数据，绝不沿用先前回答的形式。这一增强纯属声明式（提示词 + CSS + 净化白名单：新增六个惰性标签，插件顺序不变），且当指令宣告的类未被样式表覆盖时，CI 守卫会失败。复制、分享和 `.md` 导出会将 HTML 压平为可读文本（双格式剪贴板 `text/html` + `text/plain`），即后端 `html_to_text` 语义的客户端镜像；图标连字被排除在搜索高亮之外。
...
可观测性建立在三大支柱上：关键路径上的**防御性发射**、预连线的 **Grafana 仪表盘**（28 个仪表盘 / 719 面板，覆盖应用、基础设施及每个业务子系统），以及由定期更新器维护的 **DB-backed gauges**。

第 28 个仪表盘把这些遥测变成产品驾驶舱（ADR-178）：结果按 E1（用户明确确认）或 E2（在完整行为窗口内未被更正的操作）验证，精确去重的计数存放在 PostgreSQL 中 — 可变状态绝不能从 Prometheus 计数器推导 — Grafana 通过仅限聚合视图、固定 statement timeout 的只读角色读取。
...
**手写的类名会绕过对比度校验。** 设计系统的校验会验证组件实际产生的每一对配色，覆盖五套主题的浅色、深色与纯黑三态。写在别处的东西不在其中。
...
三份 ADR（206 至 208）把视觉一致性从评审纪律变成了有工具保障的契约。状态不再自选颜色：它**命名一个色调**，由唯一的表格裁决（`status-tone.ts`），并被对比度守卫覆盖五套主题的浅色、深色与纯黑三态。操作不再自选形态：由它的**高度**决定 — 创建用实心、批量删除用实心红、行内删除静止即红、真正的次要操作用描边。列表行只以**一种方式**暴露操作，由共享组件支撑。

纯黑模式（ADR-243）是对这一契约的延伸，而非放宽。把它做成第三套主题本是最自然的做法，但那会从页面上移除 `dark` 类，连带把九处内部判断切换到浅色分支——黑色页面上的浅色语法高亮、白色图表——并让整个公开站点退回浅色变体。因此纯黑是深色的**精细化**，由一个独立属性承载，其选择器无论文件顺序如何都胜过五套强调色。变动的是六个中性表面，没有任何强调色：边框甚至保留其深色取值，它在纯黑上比在原本的灰底上更清晰。这些表面是以已发布的深色模式为基准校准的，而非以零为基准，因此没有任何元素比过去更难分辨。

设置界面本身如今也遵循同样的「结构优先于纪律」的原则（ADR-227）。页面以主从式外壳渲染——常驻的分区侧栏旁是恰好挂载一个分区的面板，未选择时则显示带说明的卡片总览——并且不再手工罗列任何内容：侧栏顺序、分组和挂载的组件全部派生自深层链接表以及两个由编译器保证完整性的注册表，每个注册表都有测试对照分区源码加以证明。其结果是架构性的而非表面的：一个分区存在于页面上，当且仅当表声明了它；旧外壳所需的约 330 行重复布局消失了；并且只有被选中的分区才发起请求——不再有二十个分区在打开标签页时同时拉取数据。缺席依然诚实：一个正当地不渲染任何内容的分区（无 MFA 的实例、从未拨打过电话）会给出明确的内联空状态并持续探测，迟到的数据会替换掉这条消息。

同样的原则也回应了一种更隐蔽的失效：某个界面悄悄地不再描述真实的产品（ADR-229）。能力地图——回答「我的助手能为我做什么」的那个页面——一直只发布十三个固定节点，而产品早已交付了图像生成、文档、插件、习惯学习、用户 MCP 服务器和电话呼叫：偏偏是这块唯一职责就是保持时效的屏幕，成了整个应用中最过时的。写在文档里的约定恰恰在这里失效过；因此修复是结构性的，而不是又一条提醒。现在有两张声明式的表把平台能力枚举划分为「绘制一个节点」和「有意不在地图上，理由如下」，并且有一个断言在导入时执行：新增能力却没有决定它在地图上的归属，会让启动失败，而不是以隐形的方式上线。配套的守卫读取断言看不到的三个客户端界面——图表中的位置、「下一步」链接、六种语言——因为只盯着 Python 的守卫，会漏掉这次漂移中属于 TypeScript 的那一半。随后，同一份聚合还供给设置总览：一次请求就说明每个分区当前有什么，用的正是能力列表的措辞；而在答案尚在途中、请求失败、或对某个它一无所知的分区时，它什么都不说。

设计系统的间距由一条 CSS 规则支配：`inline` 元素的垂直外边距**被计算却从不绘制**。`<label>` 在浏览器中默认即为内联，因此标签原语被声明为 `block`——否则控件上方的任何 `space-y-*`，无论取值多少，都只会产生同样的三像素间隙，而一次重校会改动代码却移动不了一个像素。由此形成的反射是：当视觉调整毫无效果时，先在真实浏览器中测量元素的 `display` 与 DOM 几何，再去怀疑交付链。一道守卫将其固定下来。

## 24. 架构决策记录（ADR）

301 篇 MADR 格式的 ADR 记录了主要的架构决策。以下是一些代表性示例：
...
**静息坐标系，依测量重新居中**：Mehrabian 映射让**全部 14 种**人格都静息在 D > 0 一侧，因此五种需要负支配度的情绪在静息状态下始终无法抵达——阻尼是一个位似变换，无法修正这一点。两项开关以**惰性**方式交付，正是为了让启用成为一个经过测量的决定。2026 年 8 月的生产测量（769 份快照、3 位用户、90 天）比模拟更有力地印证了诊断：负支配度占比 **0.0%**，而喜悦脉冲在 **31%** 的对话轮次中把喜悦推举为主导情绪，与实际报告的评估无关。两者现已成为代码默认值：在 0.20 处，人格目录跨越了零点（14 种中有 7 种静息于零以下，且人格次序完全保留），报告出的评估重新掌握情绪通道。同一次测量**推翻**的结论也一并记录：唤醒度同样被锁住，但原因是评估流本身，而非静息点的几何——本次变更并不修正它。

只有账簿完整，上限才成立，而且这条规则不只针对模型：**平台为某个人支付的每一欧元都要为该人追踪、显示、归属并计数，无论何种方式或路径**（ADR-272 修订）。Google Maps Platform 在各个方向都有漏洞。追踪器的两道持久化门只根据模型的记录做决定，于是电话或直接 Live 会话中的 Places 或 Routes 查询——没有模型调用——进不了四本账簿中的任何一本；如今 `pending_families()` 是唯一的判定，守卫会拒绝它不读取的记录桶。Google 调用计数器在没有环境追踪器时什么也不做，而心跳、简报（每次刷新八次计费的天气调用）、会议和图片代理都在付费：现在它以关闭方式失败，计入 `google_api_calls_unaccounted_total`，并由告警保持为零。每个回合之外的界面都通过单一入口围绕整个动作打开自己的记账，每个模块所走的路径在 `LLM_SPEND_ROADS` 的姊妹登记表中声明并在导入图上校验，计费图片在 Google 计费处计数——在代理上、在 200 响应时，通过按（运行 id、账户）签名的运行 id 关联到其回合——而单一列 `billed_cost_eur` 取代了四处手写的、子集各不相同的求和。
...
使用端刻意克制：为回复和简报提供环境上下文；每天最多一次错过例行安排的提醒，被忽略两次后静音，直到该例行动作再次出现；通知时机的评分会优先选择学到的时间窗，但绝不超出用户设定的边界——一条防饥饿规则保证交集为空时行为完全不变。检测器应用的每个阈值都公开在面板中：显示的习惯必然经过证明，否则就不存在。

另一半——**重复出现的请求**——读取路由器的决策，使用封闭词汇：正是产品看板据以推导的那个值，因此被计为一次行动的回合，就是账本记录的回合。签名仍只取主域。命名了四种形态：每日、工作日、每周，以及**间歇型**——「每周几次，约在 9 点」——由合格跨度内不同天数的密度决定，绝不由锁定决定：每周三次不再被许诺成「每天」。阈值在一个持久的测试台上测得（`task habits:calibration:measure`，每格 300 次试验，密集与适度人群各配一个等量的分散对照）：每日节律在第 14 天被识别，每周三次在第 21 到 35 天之间，一个 90 % 可靠的每周仪式在第 42 天——跨五个时段，使漏掉一周不再杀死锁定——在无结构使用上误锁率为 0-0.3 %；被测试台拒绝的放宽方案保留在它的表格里。而沉默就是告警：`RecurrenceLedgerSilent` 把人类可执行回合与已落地的写入相比较——一个在被交谈时什么也不写的账本，不是一个低调的账本，而是一个坏掉的账本。

**学到的内容通过唯一一个谓词被消费，并以执行来审计。** 习惯面板编辑的是镜像行——一个时间窗可以暂停或屏蔽——而心跳块、tick 评分和环境块读取的却是档案：被屏蔽的时间窗仍在引导 tick。现在 `habits/consumption.py::load_consumable_profile` 同时服务这三者，只保留行处于活跃状态的时间窗，时间窗的键也只有一个生产者。学到的习惯有一个由夜间任务掌管的生命周期（`recurrence_sync`：升级、刷新、保留、降级——只处理活跃行，拿不准时绝不降级，对沉默账户同样执行，因为那正是降级的场景），而不是由聊天建议掌管。学习门（`learning_gate.read_learning_gate`，默认关闭）关上每一扇门——账本写入、主动性检测器、预置——并遵循 `HABITS_ENABLED` 加上在动作时读取的 `habits` 能力开关：`habits` 和 `heartbeat` 离开了 ADR-280 的路由守卫家族，转入服务强制家族，因为路由就是档案。决定**声明**它的提议（`HeartbeatDecision.habit_offered`，规则 24），而不是让提议额度挂在模型挑选的来源标签上；提议也有了对象：分析器的 `immediate_intent` 是账本负载内部的一个有界直方图——绝不是键的一部分，否则每个账本都会碎裂——于是 `usual_intent` 通过封闭词汇表 `IMMEDIATE_INTENTS` 到达习惯行、提示词（« usually asks for a 'search' on 'email' »）和面板。tick 评分位于 `habits/tick_scoring.py`，每个扫描一个 `TickSurface`，固定在它的资格检查器上，兴趣扫描也应用两道门——会议守卫和学到的节奏；唤醒绕过节奏但在会议前退让，日程裁决携带 `next_start`，节奏永远不会遮住约会。在线状态默认开启，最近活动计入在线 ping。查阅登记簿也围绕整个运行进行记录，包括资格检查——聚合器在 `select_target` 中读取，而收集器却在 `generate_content` 中打开：824 次运行，零条记录。
...
公开演示实例把这些边界用在自己身上，并让它们可读。注册表中的每一项能力都在其环境模板中以书面形式决定——开启或关闭，附带理由——两道守卫拒绝任何留给代码默认值的上限，也拒绝开发模板与生产模板之间的分歧；可达路由的普查在同样的上限下挂载路由器，因此被关闭的功能族不会出现，被开启的则逐条路由写明。最后，每个实例都在其公开配置中公布全部能力的有效状态——上限与开关合成——而宣传演示实例的那个实例会在服务端读取这份配置并显示在链接旁：访问者会找到什么、找不到什么，这份清单跟随演示实例的环境文件，无需任何人手动维护。

## 29. 按文件管理：工作簿即表单

LLM 模型目录共有一百二十四条记录，每条承载二十四项特征和一份四维度的价目。此前的管理方式是每个模型一个对话框 —— 改一个价格尚可，接收供应商一年修订两三次的整份价目表则荒谬。答案不是再加一块屏幕，而是一套**声明式底座**：`WorkbookSpec` / `SheetSpec` / `ColumnSpec` 描述一本工作簿，两个方向都由它派生 —— 写入器产出文件，读取器把它读回。底座不引入任何领域；领域只提供一份列声明和一个经由自身服务落库的应用器。把这套机制移植到另一块管理屏幕，写的是一份声明，而不是格式代码。

### 29.1. 把「导出」与「管理」区分开的三条性质

**列按技术键解析，绝不按位置。** 工作簿第一行承载不变的技术键并保持隐藏；第二行承载按区块着色的翻译标签；数据从第三行开始。重排某列、隐藏某列、新增某列，或者用一种语言导出、用另一种语言回传：对读取毫无影响。

**没有任何东西被隐式删除。** 文件中缺失的行永远不会删除任何东西 —— Excel 里忘记取消的筛选无法清空目录。下架要经过一列显式的状态列，把它重新置为真则重新启用 —— 这也顺带补上了应用内一直没有反向操作的停用。

**预览是有约束力的。** 导入分两步：第一步什么都不写，逐字段返回变更计划；第二步**重新推导**该计划，若与已被审阅的那份不同则拒绝执行。**按行**的乐观锁 —— 一枚承载在隐藏列中的指纹 —— 只拒绝期间被改动过的行：同事改了一个无关模型，并不会让整个文件被驳回。而未发生变化的内容不会被写入：没有这条规则，重新导入一百二十四行就会留下一百二十四个无用的价目版本。

### 29.2. 文件陈述事实，而非假设

三列只读的派生列之所以存在，是因为原始数据会误导人。没有生效价目的模型会被静默按零计费：文件把这件事写成文字。带时间窗的价目 —— 某供应商的低谷时段 —— 此前读起来像一份平价目，因为时间窗住在一个没人有理由打开的工作表上；现在它出现在承载价格的那一行上。导出的模式始终等于真实状态，绝不会是「继承」这条指令 —— 那是一条写入指示，而不是一种状态。

至于完整性，它是被**守护**的，而不是被记住的。工作簿的早期版本导出十六列，而真实模式远不止于此，保真度测试却看不见：它拿一份抽取与自己作比较。如今的判据是数据库模式本身 —— 每一列业务字段要么被导出，要么带着书面理由被排除 —— 明天新增一列就会让持续集成变红。这正是注册表完整性断言的教义（ADR-085），被应用到了一种文件格式上。

### 29.3. 需求在写下第一行代码之前所揭示的东西

设计导出需要回答一个简单的问题：一个模型的价目是多少？当时没有答案。没有任何约束保证只有一份生效价目，四条读取路径都在没有确定性排序的情况下选取 —— 其中两条可能在同一时刻、同一数据库上，为同一模型返回不同的价格。此外，一个按原始名称写入、按归一化名称读取的缓存，让带日期的模型按其基础模型的价格计费。这些缺陷不是附带损害：不修它们，导出就失去了对象，因为它不知道该展示哪一行。

这轮整顿产出了一条超出本领域的规则：**迁移绝不臆造业务数据。** 直觉规则 —— 保留最新的那一行 —— 与真实的分歧个案对照后，每一次都被证伪：正确的是较旧那行，而在两个模型上改变的是计费**单位**本身。因此迁移只合并严格相同的重复项，遇到分歧则**点名**并停止。裁决权仍在人手中。

### 29.4. 格式并非细节

`.xlsx` 本身就是一个压缩包：防 zip 炸弹的护栏沿用插件导入器的那一套，是共享而非重写；读取按块设限 —— 超出模板的文件在被整份驻留内存之前就已被拒。其余部分取决于 OOXML 一处会反噬的特性：工作表保护的布尔值为真时意味着「禁止」，于是为了锁住五列计算列而保护工作表，反而**禁止了新增模型**；而那个看似启用下拉列表的属性，实际上是把箭头隐藏起来。这两种行为都由针对生成 XML 的断言钉死，因为对其中任何一个出于好意的「修正」，都会悄无声息地拿掉这份文件一半的易用性。

## 30. 可见的工作，受治理的学习

活动页面是一个**纯读模型**：并行的取数器（每个来源一个会话，因为 `AsyncSession` 不能并发使用）聚合已有的审计表，总数是覆盖整个窗口的精确 `COUNT(*)`，上限被明确声明，出故障的来源会被如实列出而不是被悄悄补齐——这是端到端落实的诚实计数（ADR-185）。记忆遵循一条**取代轨迹**：自动更正会创建一个后继并归档旧的事实（`superseded_by_id`），每次读取都通过一个中心谓词过滤出活跃集合，轨迹在保留期后被清理；而手工编辑保留其覆盖权。学到的规则是**独立的一类记忆**，注入到提示词的开头，受同样的保护（置顶、保留期、GDPR）。语音韵律是对管理员设定的一种**有界调制**（死区、硬边界、开关），而绝非替代。自主性同样有上限：ReAct 的迭代预算随请求涉及的领域跨度调整，但绝不会超过配置的上限；复杂度未知时给予完整上限——节省只适用于已被证明简单的情形。

## 31. 表情眼睛：由信号驱动的角色

聊天页的眼睛小部件（ADR-240）建立在一个原则之上：**不新增信号，不新增成本**。一个纯粹的引擎——注入 RNG 与时钟的决策表——从优先级链（错误 > HITL 提问 > 语音 > 交互 > 回合反应 > 通知 > 输入 > 闲置 > 心情 × 时段）推导出二十种表情之一，其输入完全来自既有机制：聊天状态机、SSE 执行步骤（思考 vs 工具检索）、HITL 卡片、语音状态机与心理引擎。对每条回复的反应读取模型附在自己回合上的情绪自评，并配有严格语言中立的启发式回退（标点、表情符号、结构——含中文全角）。渲染是声明式的——一个表情属性、CSS 变量，以及一张动画表，其中眼睑是**纯几何形变**（锚定的竖向压缩、逐眼旋转、圆角塑形）：任何地方都没有裁剪，每个中间状态都是平滑曲线。事件之间的生命——眨眼、目光扫视、按心情加权的小动作、走神微场景、罕见的滑稽戏——活在自持定时器的调度器里，标签页隐藏或部件最小化时暂停，在 `prefers-reduced-motion` 下静止。六种可选样式共享这副骨架：一个通用注册表，新增一种眼神只需一个 id、一段作用域 CSS 和六条语言条目——完整性由测试保证，而非约定。

这张脸在两次回答之间活着，而这份生命是一个绑定系统（rig），不是一张样式表（ADR-252、ADR-264）。一个 TypeScript 运行时逐帧计算每个通道——解析弹簧、叠加循环、关键帧带——并将结果发布为样式表只读取的 `--rig-*` 自定义属性：样式表从不声明其中任何一个，也从不给每秒变化六十次的值加过渡。眉毛有弧度，休息时隐约在场；一次呼吸以同一周期带动身体、眉毛和嘴宽；目光抬起眉毛，眨眼让眉毛落下，二者在绑定系统中以**绝对贡献而非增量**耦合，因为空闲快速路径只重写循环所驱动的通道——那里的增量会在整个会话中漂移，一个测试通过把两万个小步与一步相比较来钉住这一点。说话有语句（穿过闭合的包络，开口限定在单位区间内）以及按不规则模式停顿的眉毛。嘴巴有自己的生命——由绑定系统调度、随机节奏的相对小表情——以及安静脸上的十段短小戏，任何表情变化都会中断它，脸精确停在原处，并与一个孪生绑定系统对照验证。随机性来自一个单独播种的流，从不使用 `Math.random`，以便部件测试保持确定；活着的静止在测试中是一个像素预算——休息时可见，低于两像素，专注的脸上恰好为零。同一个部件也在公开首页迎接访客：无需账户，每个界面各有位置，那里固定使用 Smiley 外观而聊天保留用户自己的样式——样式选择器的预览则关闭这份生命，因为预览是用来比较轮廓的。

随后，绑定骨架按通道逐一对照页面真实渲染的测量重新打磨（ADR-294）。眉毛落在眼睛上并且有分量：它有质量、会皱起，并**引领**每一个表情而不是跟随。说话不再是循环，而是生成的句子——音节、停顿、重音——没有任何重复；表情的节拍在每次抽取时都被扭曲，因此两次眨眼、两次微笑、两次注视从不相同。小品等待真正的休息时间，用一只页面在导航之间保留的时钟来计时，因为每次挂载就归零的时钟从不让场景到来。这一切体现在六十一个公开的通道里，逐帧测量。

---

## 32. 原生应用：一个外壳，您的服务器

Android 与 iOS 应用（ADR-246）是每个商店只发布一次的 **WebView 外壳**，可作为任何自托管服务器的客户端：WebView 加载服务器的**远程源**，用户在首次启动时输入其 URL。界面从不重复实现 —— 让 PWA 安全的 httpOnly Cookie 会话契约，正是让外壳成为可能的根基 —— 且每一条平台断言都经过**实测，而非假设**（`scripts/mobile-probe/`）。

**登录走 Google 唯一允许的路径**：OAuth 流程跳转到系统浏览器，再经 `lia://auth-callback` 链接返回，凭借一次性代码兑换会话，该代码绑定到只有 WebView 持有的验证器 —— 被截获的链接毫无价值，这正是自定义 scheme 可被接受的原因（App Links 在构建时固定域名，而一个应用要服务所有服务器，这不可能）。接通这条路径的同时，还关闭了联合登录上一个既有的 TOTP 绕过漏洞。

**推送为原生实现，且有意不对称。** Android 在**运行时**用服务器发布的选项初始化 Firebase：自托管者的通知永不离开自己的项目。iOS 做不到 —— APNs 只听命于拥有 bundle id 的 Apple 团队 —— 因此已发布的应用由一个**无状态中继**唤醒：句柄*就是*密封的设备令牌（Fernet 专用密钥），通知是六种语言的一句固定话语，中继永远不知道谁被唤醒、为何被唤醒。疑虑永不删除设备：只有“句柄不可读”和“设备已消失”才能丢弃令牌。

**十二条 OAuth 出口全部回到应用** —— “跳转系统浏览器”的决定只做一次，就在所有流程本已共享的关口，返回时从 OAuth 状态中读取来源界面，而该状态由代码库中唯一构建它的函数写入。一个**专用测试台**通过 WebView devtools 套接字在模拟器上驱动真实的调试应用 —— 十个场景、零服务器，向 `.invalid` 源导航的失败即是判据 —— 并在首次全绿之前找出了三个真实缺陷。

## 33. 自我诊断：会读取自身遥测的助手

发出所有这些可观测性数据却从不读取，等于处处埋点而对自己一无所知。自我诊断子系统（ADR-247）以「每个支柱一条设计规则」闭合了这个环路。

**读取永不抛出异常。** Prometheus/Loki/Alertmanager 客户端（`infrastructure/telemetry/`）把每种故障模式——超时、5xx、畸形 JSON、断路器打开、来源被禁用——都化为类型化的 `unavailable` 结果。没有可观测性栈的安装保持原样运行：URL 为空即禁用该来源。

**任何自由查询语言都不会出自 LLM。** 命名查询目录（启动时完整性断言）是 PromQL 的唯一生产者；受约束的构建器是 LogQL 的唯一生产者——封闭的服务枚举、严格的事件模式、以常量封顶的时间范围与数据量。注入无从拼写，而 Loki（在树莓派上有 OOM 历史）从构造上就被保护。

**自检在失明时也能工作。** 主节点循环通过 Prometheus *和* 进程内探针（PostgreSQL、Redis、断路器、自身活跃度）评估黄金指标：Prometheus 宕机时相关检查变为 `unknown`，但循环继续——且 `unknown` 把总体判定封顶在 `degraded`，因为失明不等于健康，失明也不等于故障。

**一次故障，一个事件。** Alertmanager Webhook（Bearer 认证、已提交的片段、CI 中重放的矩阵）与严重判定按关联键收敛为唯一事件——部分唯一索引，在 Webhook 与主节点并发下保持原子 upsert。LLM 诊断以该警报的运维手册为依据，受原子化每日预算限制，其建议始终只是建议：任何操作都不会由模型文本触发。

**对故障的了解塑造回答。** 平台健康时零成本的 advisor 把降级能力注入规划（「Brave 故障 → Perplexity」），合成阶段以类型化形式接收本次运行的失败——错误码与消息开头，绝非原始日志——并附带诚实指令：说明哪些成功、哪些失败及原因，绝不杜撰诊断。

**诊断器在诊断的那一刻读取证据。** 一个数值和它的阈值只是判定，不是诊断，连续四起事件已经证明了这一点：模型回答「证据不足」，而细分数据就在 Prometheus 里，出错路径就在 Loki 里。现在每类事件声明一份*配方*——为其关联键值得读取的目录查询和日志事件，启动时校验完整——诊断泵在预算门之后为每个事件收集一次证据包：细分序列、经封闭白名单和个人数据清洗器过滤的有界日志摘录、正在运行的版本及其运行时长。每个来源各自退化：无法访问的日志存储只让诊断失去摘录，而不是失去诊断，并被标注为不可用，而非当作沉默。模型看到的内容与它写下的结论一同保存，并在判定下方呈现给管理员。

## 34. 算出来而不是猜出来：在早已存在的沙箱里跑一段临时脚本

问一个语言模型「这几段中转加起来有多久」「哪些名字同时出现在两份名单里」「这一列数字按时区折算后是多少」，它会回答——听起来合理、流畅，而回答本身不会告诉你它错了。这不是提示词的缺陷：预测下一个 token 本来就不是做算术。而五行 Python 是。

**没有新建任何沙箱。** 技能沙箱（SEC-001）本就存在，而且早已加固：一次性容器、不挂载 Docker socket、`--network none`、根文件系统只读、uid 65534、丢弃全部 capability。`execute_source` 只是把一段源码而不是一个文件路径交给它，两条路径共用同一个执行内核——只有一套隔离参数，因此不可能加固了其中一条却漏掉另一条。

**这个决定来自测量，而不是直觉。** 在生产用的树莓派上：冷启动 279 毫秒，标准库 357 毫秒，带 numpy 459 毫秒——不到 30 秒预算的 2 %。API 镜像有 3.76 GB，所以加上 pandas 只多约 1.5 %，而它的硬依赖本就都已安装。最初的直觉——「pandas 会让一切变重」——错了一个数量级，是测量说出了这一点。

**只在自主模式，而且强制两次。** 工具清单声明 `execution_modes={"react"}`，并且目录的*每一个*读取方都会应用这个过滤，因此确定性规划器根本看不到这个工具：一个能看到它的规划器会安排一个执行阶段随后拒绝的步骤，那就是为用户凭空造出的死胡同。工具随后在调用时再从类型化的运行时上下文里核对一次模式。只强制一次会是陷阱；两次才构成契约。

**凡是强制的，都要公开。** 清单写明一次运行能访问什么——不声明主机就什么都访问不了——、没有数据库、除 `/tmp` 外没有任何可写文件系统，写明确切的库清单、体积与时间预算——并明确说明什么时候*不要*动用它。没有后面这句话，一个能干的工具就会变成锤子；没有前面这些，模型就得烧掉一次迭代去撞出边界。

**数据走 stdin，预算存在图状态里。** 把本轮的数据行抄进源码，会为这些 token 付两次钱，而且恰好会截断那些正当使用该功能的大型场景。每轮预算也刻意不放在上下文变量里：在一个 asyncio 任务中设置的值，在兄弟任务中是看不见的，而图执行器完全可以把每个节点放进各自的任务——状态是预算能跨迭代存活的唯一位置。

**输出不可信，代码可审计。** 脚本打印出来的东西，是模型写的代码在第三方数据上运行的结果，因此被标记为不可信内容，和一封邮件的正文完全一样。代码本身连同声明的目的和输出，管理员可以在调试面板中看到：把它藏起来买不到任何安全性——代码是模型写的，本来就在它的上下文里——却会赔上全部可验证性。

**只经一道门访问网络，而这道门就是拓扑本身。** 脚本可以访问网络——但只有声明了主机的脚本可以，而且只能经由一个专用的出口代理。一次联网运行会加入一个内部 Docker 网络，其唯一有路由的成员就是这个代理：裸套接字无处可去，而这正是环境变量式代理留下的绕行口（`HTTPS_PROXY` 只是建议；只有一个出口的网络才是事实）。代理用启动时自铸的证书颁发机构终止 TLS，只放行 API 为当前存活的运行渲染出的主机，在建立连接时就拒绝私有网段、服务器本身和云元数据，并限制转发的请求体大小。它的状态存放在由自身入口脚本写入的 tmpfs 卷里——曾试过用初始化容器，测量证明是错的，因为 Docker 在它退出时就释放了 tmpfs。

**密钥以令牌的形式传递，而令牌离开这次运行便一文不值。** 每个主机带有四种状态之一——从用户自己的 API 密钥连接器派生、由运营者允许、由用户授予，或未知。对于连接器的主机，脚本读取 `os.environ["LIA_KEY_<CONNECTOR>"]`，这是一个每次运行专属的令牌，代理只在该主机上把它换成用户的真实密钥：密钥从不进入容器，OAuth 令牌和实例自身的密钥更是从不提供。这些状态是从客户端类派生的——基础 URL、请求头、前缀——绝不在第二张表里再敲一遍，因此代理的规则和客户端的真实调用不可能出现分歧。

**未知主机会在循环内先问，有三种回答。** 卡片写明主机和模型声明的目的，附上本轮数据的计数——绝不是数据本身——用户可以允许并带上数据、允许但不带数据（此时 stdin 为空）或者拒绝；回答会作为许可被记住，受一个公开的上限约束。这个问题在哪里被处理，和它问的是什么同样重要：节点自己抛出中断，就像变更类工具的确认一样，恢复时在回答之下重新调用同一个调用，因此请求的其余部分得以继续。若把卡片交给草稿分发流程，它会被当作一个动作执行并直接以其结果作答，丢掉后面的每一步——许可不是动作。

**提示词先经过测量，然后才被信任。** 被告知「把 `LIA_KEY_X` 放进请求头」时，模型会把变量的名字当作值发出去；这一行现在把读取方式写清楚了。提示词承诺的库存放在一张直接固定版本的表里，并在每次 CI 运行时于构建好的镜像内导入；四种角色——计算、诊断、填补空缺、转换——连同代码强制执行的边界一起声明，这些边界从设置读取，而不是写在散文里。

## 35. 在交付之前先测量颜色：设置页的调色板

设置页列出五十三个板块。它们全都在同一个颜色的小块上画着同一个颜色的图标，其中十六个还借用了别的板块的图形。眼睛能用来定位的，只剩下一个反复出现的形状。

**颜色修不好一个重复的形状。** 两个插头就算上了两种颜色，依旧是两个插头：这是两个不同的缺陷，因此分开修复——先给每个板块一个属于它自己的图形，再给每个**分组**一种色调。按分组而绝不按条目：十二种颜色是一张看一遍就能记住的地图，五十三种则是需要辨认的噪声。

**用 token，而不是工具类。** 这套调色板是固定的，不跟随用户选的强调色——这是产品的第二个例外，第一个是 skill 的青色徽标。如果写成字面类名，它就会落在对比度守卫的视野之外，因为那个守卫读的是 token 对；写成 `--color-settings-*`，它就天然在内。你强制的约束，必须能被校验它的东西读到。

**sRGB 色域不是一个圆柱体。** 最初的直觉是：十二种均匀分布的色相，单一彩度，每种模式一个明度。测量给出了否定的答案——在 55 % 明度下，紫色能承载 0.25 的彩度，而青绿色到 0.09 就到顶了；二十四种色调里有六种落在色域之外，被浏览器静静地截断，既不呈现写下的色相，也不呈现写下的彩度。现在每种色相都有自己的上限，并留出余量。

**均匀的间距不等于感知上均匀。** 彩度一旦跟随色域，就有两对颜色相距只有 0.116，低于守卫自己设定的区分度下限。因此这十二个角度是**搜索**得到的，而且取两种模式中更差的那一个：两个明度切出的是色域的不同切片，只针对浅色主题优化的一组颜色，在深色下仍有一对只差 0.113。现在最接近的一对是 0.199。

**颜色从不表达状态。** 当前打开的板块靠背景、字重和强调色来区分，而不是变成第十三种色调；某项能力是否启用，仍由实心或空心的圆点表示。这是认真对待 WCAG 1.4.1：看不到这十二种色调的人，不会损失任何信息。图标属于非文本图形对象，因此它的下限是 3:1，并且是在它真正所处的两种底色上测量的——卡片上的小块，以及没有小块的侧栏，含悬停状态。

**两份列表只有一条规则。** 概览卡片与侧栏行读的是同一个函数：它们不可能在某个板块上分歧；表外的调用方会回退到强调色，而不是回退到空。

## 36. 特质不是反应：由回答自己声明的语气

伙伴的表情此前从心理引擎的主导情绪中选取回合结束时的神情。测量给出了结论：连续十四个回合中，有十三个回合的主导情绪完全相同，波动仅 0.02。

**这不是心理引擎的缺陷，而是它的定义。** 心理引擎建模的是内在生活：它是一种**特质**，变化缓慢，而这正是我们对它的要求。缺陷在于让它为一个**瞬时事件**作答——对一个近乎恒定的向量取 argmax，结果就是一个常量。

**唯一知道一段回答语气的，是写下它的模型。** 其他任何来源读到的都只是表层。因此由回答自己声明语气，使用一套只属于动画、不属于其他任何模块的词汇：十二种语气，遵循一条唯一约束——*两种表情会被完全一样地演出来的语气，其实是同一种语气的两个名字。* 这就是列表不再更长的原因。

**放在数据流内，因为两项要求交叉。** 信号必须来自写下回答的模型，**并且**要在回答抵达的那一刻抵达——表情在回答完成时作出反应，而后台处理只会在那一刻之后到来。置于生成末尾的标记同时满足两者：不增加任何模型调用，且随最后一个 token 抵达。这个模式并非为此新造：它就是心理引擎的自我评估机制，已在生产中经受检验。片段会从数据流中滤除，屏幕上不会有任何闪现，完整标记也会从留存文本中移除。

**强度是表演提示，不是置信度。** 渲染会**放大**它而非照搬：把 0.8 演成 0.8 的卡通看起来像视频通话。而且是**语气**限定强度能买到什么——一条以最高强度声明的平实回答，仍然是一张带着笃定感的中性面孔，绝不会变成庆祝。强度说明语气有多强，从不说明那是哪一种语气。

**被强制执行的，必须被公布。** 指令提供而代码拒绝的语气，会静默地产生一个没有表情的回合；代码接受而指令从不提及的语气，则是一张永远不会出现的表情。一个测试把两份清单绑在一起，浏览器侧的副本也以服务端副本为准。

**标记并非每回合都会出现，回退机制为此作了准备。** 真实条件下的首次测量：语气标记与心理标记在十六个回合中恰好落在同样的两个回合上——这个比例是回答模型的属性，而非该功能的属性。八回合才反应一次的表情是坏掉的表情，因此回退不能再返回空：它读取回答的**形态**——长度、代码块、标点密度、表情符号，从不读词，因而六种语言表现一致——并使用与标记**相同的词汇**。一张语气表、一条幅度曲线、一条路径。

**而心理引擎保留它真正擅长的**：静息时的情绪族——呼吸节奏、眨眼频率、闲置动作的权重。特质应当为静息行为着色，而绝不为一次反应着色。

## 37. 三种机制应对一次汇聚：抚平上限看不见的突发

**间隔触发器从调度器启动那一刻开始计时，因此共享公约数的周期会永远对齐。** 这是算术，不是负载：5、15、30、60 分钟的任务终将落在同一秒，并在进程的整个生命周期里保持如此。生产环境实测：每小时六个任务挤在同一秒，每个都要运行一个智能体，每个智能体都要做向量化——24 次调用中失败 11 次；而稳定的每分钟四次调用毫无差错。

**三种机制，三种角色，区分它们正是设计本身。** **错峰**针对成因：取周期的 15 %，下限 5 秒，且始终严格小于周期，使两次运行既不重叠也不倒序。**限流**对这次事件毫无作用——六次调用越不过任何按分钟计的上限——它是规模增长时的保险，因此窗口刻意取**短**：按分钟计的上限看不见突发。**重试**收拾余下的部分。

**三者都不是闸门。** 限流复用既有的限流器而非新增第二个——一个滑动窗口、一段 Lua 脚本、一套指标——并且**超时放行**：等待有上限，等够了自己那一份的调用照样通过。我们自己的调节，绝不能成为一个回答失去记忆的原因。Redis 不可达时，答案立刻是「没有名额」，调用照常发出。

**重试需要的是工厂，而不是可等待对象。** 协程只能被等待一次：持有 `client.aembed_query(...)` 的接缝无法重跑它——第二次等待会抛错而不是重新调用。分类则是**结构化**的：沿因果链读取状态码，绝不读消息文本，否则服务商换个措辞就会悄悄使其失效。推论：耗尽自身重试的一层必须**串接其成因**，否则上层读到的只是一个沉默的外壳，并把本可恢复的失败判为永久。

**而没有度量的东西，就看不见。** 一旦引入重试，服务商调用计数就成了错误的分母：被重试救回的失败会抬高错误率，尽管什么都没丢失。因此「平台健康」现在统计**结果**——每个逻辑操作一行，重试折叠其中——另有一个计数器说明限流对每次尝试做了什么，因为「预算太小」和「Redis 宕机」需要的是相反的处置。

## 38. 会议纪要：这一行就是任务，模板就是契约

**一场会议是数小时的音频，产生自一台可能休眠、断网或被误关的设备——因此采集是为故障而设计的，而不是为理想情况。**浏览器以短片段录制，每个片段一旦生成就以原始请求体上传；片段在服务器上是一个文件，先写入临时名再原子重命名，因此四个 API 工作进程可以乱序接收而绝不会交错字节，重复上传只是无害的覆盖，检测缺口只需列出目录。两种音源共用一个接口：在引擎可信的地方用 `MediaRecorder` 输出 Opus，在其他地方用语音输入早已使用的 AudioWorklet 输出 16 kHz 原始 PCM——这是 Apple 设备上唯一的路径，因为其录音器生成后端拒绝的容器，且在主屏幕应用中表现异常。格式是环境的纯函数，在第一个字节之前一次决定。上传按顺序发出，以在录制期间永不放弃的退避重试，浏览器离线时等待而不是失败；停止时声明上传队列自己的计数，服务器从未收到的序列会被点名拒绝。

**会议这一行就是持久任务。**它的状态承载整个生命周期——录制中、已中断、已停止、处理中、就绪或失败——每一次转换都是一条带条件的 `UPDATE`。处理的认领获取一个租约，心跳在续约的同时发布页面显示的阶段（规范化、转写、生成、索引），心跳丢失会中止之后的一切效果，回收器在有限的尝试预算内重新驱动被搁置的孤儿和过期的租约。不属于流水线故障的拒绝——用量上限、没有可用引擎——会释放任务而不消耗一次尝试。由于批量更新会让 SQLAlchemy 的身份映射过期，其后的每一次读取都先让会话失效：运行时验证曾测得一次停止回答了 `status: recording`，那是在这条规则存在之前。

**引擎构成一条链，而这条链会被走两遍。**第一遍在第一秒之前，仅依据提供商密钥缓存：管理员的语音插槽，然后是 ElevenLabs Scribe 或 OpenAI 的 `gpt-4o-transcribe-diarize`——整个文件、区分发言人——然后是本地 Whisper。本地路径过去只解码一个三十秒窗口，对更长的内容默默返回前三十秒；语音指令从未遇到这个缺陷，而会议一定会遇到。长音频现在由 Silero VAD 切成至多二十秒的语音窗口，各自解码，模型缺失时使用固定窗口——绝不截断。这条链在处理时再走一遍：某个提供商的永久性故障（密钥被拒、文件无法接受）会在用户偏好范围内交给下一个引擎；只有静默（它说明的是音频本身）或属于重试预算的临时故障才会停止这一过程。开发实例曾把密钥 ID 存放在密钥的位置；若没有第二遍，每场会议都会失败，而一把 OpenAI 密钥就在下一级。

**模板就是契约，而语言模型不会被逐字节地信任。**用户的章节——键、标签、指令、类型——被渲染进一次结构化输出调用；当转写超出模型的窗口时，先按部分压缩，因此 128k 的模型仍能为三小时的会议生成忠实的纪要。模型以宽松的形态回答，修复步骤把它折叠进严格的报告：被跳过的章节以空内容返回，凭空创造的章节被丢弃，形态与类型不符的内容被转换，参与者被限制为真正发言过的标签。生成的报告是不可变的；用户编辑的是一份副本，「恢复」复制回来，「重新生成」用当前模板在保存的转写上重新运行生成。同一个序列化器产出「会议」知识空间索引的 Markdown（按角色查找，每场会议一份文档，就地重写并随会议删除）、PDF 渲染器转成文件的分节内容，以及邮件承载的 HTML——三者因此永远不会互相矛盾。

**每一项付费单位都被记账并显示。**一场会议在转写引擎处消耗音频，在生成模型处消耗 token，包括压缩步骤和重新生成；两者都像任何交互一样进入平台的账本——音频通过远程语音识别统计，token 通过归档聊天消息携带的 `run_id`，因此历史记录与 token 日志的关联方式与任何主动通知完全相同。这一行保留纪要自身的花费，页面因此能给出精确合计及其分项，卡片给出两项单位及其总和，而没有管理定价的模型返回 `null`：未知的价格不是免费的价格。同样的诚实贯穿纪要本身——缺口被指出，绝不弥合；未命名的发言人保持为 S2；悬而未决的提议不是决定。

**纪要格式变成了一个库，而选择只有一个归属地。** 三十种内置模板存在于代码中，它们的文字存放在 i18n 数据模块里；启动时的断言会在六种语言中任何一个名称缺失时拒绝启动：验证器能够拒绝的东西，目录就不能交付。模板由一个引用来命名——`builtin:<键>` 或 `user:<uuid>`——会议、偏好设置和请求之间交换的是这个引用而非数据行，因此内置模板无需数据库行，而被删除的模板留下的引用，其读取方懂得回退到已保存的快照。选择遵循**唯一的优先级**：会议自身携带的引用，然后是偏好中的默认值，然后是语言模型读取转写摘录并在置信度阈值之上作出的选择，最后是默认内置模板；每一种结果都会被计数，并连同陈述的理由写入数据行，因此页面呈现的是事实，而非事后重建。第五种章节类型返回转写本身：它无法装进一次回答——合成槽位最多输出八千 token——因此按部分重写，每部分受有效输出窗口约束，索引缺失会将该部分再切分一次，过短的回答会重试一次。重写已生成的纪要在替换时借用持久化的再生成流程，在生成新纪要时创建一行指向其来源的派生记录——绝非副本：转写内容相同，纪要并不相同。同样的顺序考量支配着知识空间中的文档：由于 `rag_chunks.space_id` 是冗余字段且被检索读取，移动操作先写入数据行及其分块并提交，**然后**才移动文件；重命名失败会将两者回滚，并只为该文档报告失败；批量操作绝不会因某一项而中断——每个标识都会带着自己的状态码返回，或已完成，或被跳过。

## 39. 三本登记簿，以及没有人要求过的那一本

**一个会行动的助手，必须能说出它做了什么、读了什么、发生在哪一个回合。** 三本登记簿分别回答这三件事，且从不混同：第一本每条记录一次**行动**，在行动发生之前即被主张，且只在拿到明确结果后才结案；第二本每条记录一次**查阅**，以能力命名，绝不记录搜索内容；第三本每条记录一个**回合**，是另外两本挂靠其上的脊梁。它们的总数并不相加，这是刻意的：一个回合可以查阅五个来源，却什么也没有改变。

**保证是结构性的，但入口只有一个。** 记录是在能力被声明的那一刻装在能力上的——因此新工具不可能忘记它——只是，经由自身*客户端*而非工具入口抵达的能力仍然不可见。生产环境十四天的实测：对话侧的表面记录为 24/24，而回合之外的工作为 **0/228**。什么都没有坏：登记簿跟随图，而这些表面直接调用模型。于是「LIA 主动做的事」这份列表，无论她做了什么都读起来是空的——这是这个缺口最鲜明的一种形态，因为主动通知是一个人真正体验到的、唯一出于她自身发起的行为。

**答案不是第四个检测器，而是一份声明。** 每一个不经工具就读取的表面——每日简报、关系小结、周期性巡查、兴趣、知识空间——都在同一张表里声明自己的词汇，启动断言如今在只看工具之处检查这三十一项能力。导入连接器客户端的二十二个模块被逐一列举：记录者、非读取者（附书面理由）、或欠账——欠账表为空，且只能变小。登记簿终于是被**递上**而不是被去取：智能体域已经导入关系域，反向导入会闭合一个环，而局部导入只会把它藏起来。

**一欧元去了哪里，是声明出来的，绝非推断。** 这里的记账是环境式的——某个节点通过祖先发布的上下文花钱——因此靠读文件来回答「这笔算进去了吗」，曾在一次会话中给出九个错误结论，两个方向都有。于是一张表点名四十六处调用点及其各自抵达的登记簿，一个用 AST 读取调用的守卫纠正了其中六处归类。而记账只是上限的一半：另一半是**先询问**，那四十六处中有五处没有任何东西加以约束。三种形态造出了它们——登记表点名的是外壳而非那道门；守卫在询问之前就提前返回；同一个拒绝有两种不同的回应形式。拒绝如今遵循它的传输方式而非它的判定：请求会抛出，后台任务则放弃并记为**跳过**，绝不记为*失败*。

**服务同一块屏幕的两个入口，只构成一次读取行为。** 仪表板并行取回卡片与综述，而两者各自构建那九个板块的整批数据：七天内共 151 次构建，其中 44 次重复——占页面加载的 39 %，且 44 次全部并发。每个连接器被打开两次，一次读取行为却归档了两批查阅记录。原因是一个问题，而不是一行代码：综述问的是「这批数据里有没有值得一提的东西？」，而不是「这批数据构建过了吗？」，前者分不清冷缓存与平静的一天。先发问者负责构建，其余的拿到同一个对象——在进程内如此，在生产实例的四个 worker 之间亦然，靠一份只由其持有者释放的声明。

**一次导出要么完整，要么就不算导出。** 五份可下载记录曾带有行数上限，那是实测所得而非随意设定：在目标 Raspberry Pi 上，五个来源各五千行时内存峰值达 33,9 MB。这个约束是真实的——整份文档在内存中拼装——但被施加在了错误的变量上。**稀缺的是内存；被限制的却是真相**：实际 49 195 行，对上每个来源一千行，也就是推理记录的 97,9 % 缺席，而文件头如实写着「已截断」。服务端游标如今限制的是缓冲区；行数是精确的——在正文所遍历的同一条语句上做聚合——并在第一行之前公布，于是一次没有上界的导出获得了一个界限：它生成的那一刻，写在文件头里，而不是被无声地钉住。

## 40. 每段关系一份小结：十个板块说不出的东西

**一张卡片堆着十个板块，而没有人会读十个板块。** 读者首先想知道的——我和这个人进展到哪一步、接下来该谈什么——是任何聚合都产生不了的一份小结。因此它由模型写在卡片顶部：还有什么悬而未决、接下来该做什么、什么值得记住。它的生成是**惰性**的：在卡片被打开时才诞生，按读者的本地日期每天最多一次，绝不由调度器触发——关系的数量没有上界——也绝不在对话回合中进行。只有三种重建是正当的：语言、所请求的范围，以及一次明确的请求。

**两套装配会造出两个「这个人是谁」的权威。** 证据的装配本已存在，就在那个于对话中作答的工具内部；因此它被抽取为独立模块，而该工具成了它的第一个消费者。生产路径上的重构不能靠阅读来验证：一份黄金文件是**在改动之前的代码上**捕获的，十八种范围情形，载荷与消息逐字节比对。供应商那一半不再被整体读取然后丢弃——此前多达十一次外部调用，是对着读者早已作出的选择而计费的——而这种收窄需要第三种状态：「我是故意没看」既不是「我什么也没找到」，也不是「我没能去看」。

**没有任何东西是编造的，失败也不摧毁任何东西。** 没有任何证据能在不调用模型的情况下判定为*空*，而一次失败的刷新会**保留先前的文字**，并在其上说明原因：用空白面板取代一份可用的小结，会把「我没能刷新」变成「什么都没有」。声明是单独一条 SQL 语句，每一种结案只写自己的那几列——就绪、空、失败——因为单一的结案会在第一次失败时覆盖正文。这次写作花了多少，与它一并存下并显示在下方；由于零成本是一种断言，一条无从计价的记录什么也不显示，而不是显示「0,00 €」。

**在对话里，小结与同侪信息块并肩出现——却带着相反的指令。** 同侪信息块之所以能陈述确切事实，是因为它在回合之内当场读取；同样一句话若压在一份带日期的小结之上，就成了制造虚假断言的机器。因此模板会说明它带有日期，并给出它的**年龄**而不只是日期，并把任何数字、任何计数、任何状态都交还给工具去核实。名字匹配含糊时**什么都不注入**：目录里收着每一段曾经打开过的关系，公司名与电话号码也在其中，一次误判会把某个人的档案递给一个关于另一个人的问题。

## 41. 工单板：一行记录、两个视角，以及一个会提问的助手

**一个工作单元需要生命周期、承担者和结果** —— 而此前存在的任何一种界面都无法同时提供这三样。厂商的任务清单没有生命周期；提醒只是某一时刻的一次推送，响过之后什么也不留下；例行任务是一条不断重复、永远不会结束的指令。一张工单则携带标题、描述、优先级、日期、子工单、评论与一段历史，穿过七个栏目；它的承担者可以是所有者、一位已连接的伙伴，也可以是助手本身。

**同一行记录，从两侧读取。** 一块板就是「我拥有它，或者我持有它」，作为仓储层的一个谓词只写一次，所有读取都复用它——两条查询就会变成关于同一张共享工单说了什么的两个权威。调用方看不到的工单，其响应与一张根本不存在的工单完全相同：任何标识符都无法被探测。页面及其各栏计数出自同一条经过筛选的语句：栏头写着七、栏内只显示三，比没有栏头更糟。

**数据库能承担什么，不能承担什么。** 承担者所在的列采用 `SET NULL`，因为有四条路径会硬删除 `users` 行，其中只有一条会执行账号清理——级联删除会在伙伴离开时毁掉所有者自己的工单。而「伙伴只能在已接受的连接下持有工单」这一不变量，被刻意地**不做成约束**：跨越两个可被外键动作触及的列的 `CHECK`，无论级联以何种顺序触发都可能被违反，顺序本身没有保证，而 PostgreSQL 也没有可延迟的 `CHECK`。它因此活在服务层，每次写入都重新校验连接——这本身也是更正确的规则：共享应在执行时复核，而绝不从一行存储数据里读取。

**一次扫描只认领一张工单，并依据结果结算。** 每分钟，一个带抖动的作业先释放已死 worker 仍然持有的认领，再以 `FOR UPDATE SKIP LOCKED` 加一条条件 `UPDATE` 认领唯一一张工单，并在任何工作开始之前提交——一次抓走五张的 worker 被杀死时会丢下五张。结算依据明确的结果，绝不依据「没有抛出异常」，并且会说明自己做了什么：三种拒绝可以中止一张已认领的工单，而没有一种算失败。账号停用是永久性的；额度上限与正在进行的对话都会交还认领、把这次执行还给工单的预算，并记录为「已跳过」。

**在对话之外，它会提问而不是拒绝。** 执行闸门此前在对话之外既拒绝确认也拒绝草稿，因为无人可问。而一次工单执行是有对象的：闸门现在允许能够承载草稿的来源构建出聊天本会展示的那张卡片，工单带着它进入「待确认」，所有者随后的评论**完全不经模型调用**就完成分类——一部在六种语言里都做了折叠处理的词典，其中裸露的「是」表示批准，裸露的「否」表示关闭，其余一切都变成一条新指令。随后的重放在图内运行，并受指纹锁约束：执行会公布自己所展示内容的身份，工具重建出的草稿只有在这一精确身份下才被放行，绝不放宽。

**一次执行写下什么，写到哪里。** 一次执行会把自己的任务简报和提问归档为**属于该执行的行**——它们被挡在聊天之外，由决策登记表指向，并在工单关闭后由保留策略清除——而人应当读到的那条通知则是一条普通消息。这一区分是一个**派生列**，而不是元数据里的一个标记，在唯一构建该行的地方计算得出；而每一份归档的回合元数据都由一个具名构建函数产出，并有守卫拒绝在调用处随手写下的字典——因为一次删除预览正是这样跑进了聊天里。

## 42. 进程的解剖：它加载的内容被声明、测量并加以限制

一个 API 服务器是多个进程：一个不处理任何请求的监督进程，以及处理所有请求的工作进程。每个进程出于不同原因占用内存，在弄清哪个进程占用了什么之前，「容器消耗了五 GB」说明不了任何有用的东西。LIA 在这里只用一条规则：**进程加载的内容被声明、按进程测量，并在会成倍增长的地方加以限制。** 每一项都在目标主机上、在一次性进程中测量之后才被改动——其中三项测量推翻了最初的判断，这正是要测量的原因。

监督进程不加载应用。uvicorn 的命令行会在父进程中导入应用，只为尽早产生一个导入错误，随后便将其丢弃——一个只负责看管工作进程的父进程，因此在容器的整个生命周期里占用了数百 MB。`python -m src.serve` 以相同的选项驱动同一条 `Config → Server → Multiprocess` 链，却不做那次导入；一道 AST 守卫禁止该模块从 `src` 导入任何东西，而那个早期错误改由部署本已读取的健康检查来呈现。同样的思路，重量级库——PDF 渲染、表格计算、音频引擎——只在用到的地方导入，绝不在启动时导入：一份清单声明了每个库测得的开销，一道守卫拒绝 `src/` 下任何模块级导入，包括藏在模块级 `try` 中的。

单例加载的内容会按工作进程数成倍增加。语音转写曾为每一种被请求过的语言保留一个常驻模型，放在每个进程各自的单例中；现在缓存有了上限（`VOICE_STT_MAX_RECOGNIZERS`，默认为 1），按最近最少使用淘汰，构造时不加载任何东西，一次淘汰会把内存归还给系统——经过测量——而正在进行的解码保留自己的引用。每个工作进程还会公布自己持有的内容：`lia_worker_memory_bytes{kind}` 由每个工作进程从自己的 `/proc/self/status` 读取，每个存活的工作进程一条序列，随其消失，绘制在两个面板上，并由会指名进程的 `ApiWorkerMemoryHigh` 告警监视。这回答了一个结构性限制：cAdvisor 只看得到容器，而 prometheus_client 的多进程模式不导出任何进程指标。

连接预算有下限，而不只有上限。审计 F004 限制的是峰值——连接池可以打开多少；而每一条持久连接同时也是一个占用七 MB 的空闲 Postgres 后端，与 `shared_buffers` 并存。`ConnectionBudget.persistent_total` 为这个下限命名，一道守卫把 compose 文件（内存限制、`shared_buffers`）与随发布提供的两份 `.env` 配置——完整版和自托管者的最小版，缺失的键像启动时一样取代码默认值——放在一起读取：下限必须保持在限制的一半以下，让数据库把另一半留给自己的缓存。随发布的配置：每个工作进程五条持久连接、十五条溢出连接，限制两 GiB。这一批工作留下的问题也写了下来：没有转写的工作进程在数日内的增长——如今可以测量了——以及 LangGraph 检查点的保留。

## 43. Live 模式：两个智能、一道接缝——以及按模式的一套规则

语音对语音的会话运行在用户用**自己的密钥**连接的实时模型上——这是一个独立的连接器类别 `live`，可叠加：多个密钥可以同时激活，会话在所选的那个上开启，选模型*即*选服务商。服务商计费的内容平台一概不计数、不存储、不显示。API 铸造一次性凭证并生成配置；浏览器原样重放并自行打开连接，因此**音频从不经过 API**。在冻结设计之前，四个事实在真实服务商上被测量，每一个都成为了规则：凭证的约束不会锁定系统指令；已使用的一次性凭证无法重连（每次重连都为同一记录铸造新凭证）；服务商会静默接受未知的声音名称（声音按内置的、带日期的列表校验）；浏览器的原始 WebSocket 只被唯一一个方法接受。

设计的核心是**两个智能之间的一道接缝**（ADR-299）。语音模型拥有对话——倾听、说话、打断、填补等待——并通过唯一声明的 `NON_BLOCKING` 函数 `send_to_lia` 把每一次数据或操作请求委托给聊天引擎。浏览器把这次调用变成用户自己 cookie 下的普通 `POST /chat/stream`：委托的回合在图中运行（HITL、登记簿、配额、归档、学习），带着会话戳绘制在对话中，桥接把有界、扁平化的回答交给语音——或者 LIA 待回答的问题，它*就是*回答。语音从不持有工具、凭证或自己的数据行。一次会话是一行决策；每个委托回合是自己的运行；结束卡片汇总这些运行各自的摘要行。第二个服务商加入时桥接一行未动（ADR-300）：服务商**声明自己的线路**——`connection`（`token`：浏览器用服务商凭证打开套接字；`offer`：API 铸造自己的 nonce，并用用户的密钥交换浏览器的 SDP）和 `delegation_wire`（`tool`；或 `native`，服务商只发出 id 和偏移量、没有文本，请求在浏览器中从转写组合而成）——接缝从不按其名称分支。第三个服务商 ElevenLabs Agents 把用户的*代理*放在模型的位置上：除了提示词和 LIA 的工具，一切都留在其门户；工具在铸造前按指纹附加到代理；其计费声明为 `vendor`——平台不定价任何内容，计数器只显示时钟，服务商自己的账单在结束时读取一次，在其结算所依赖的关闭握手之后。

**直接会话**是浏览器里的电话线路：语音自己持有 LIA 的只读工具——电话推导出的集合减去用户自己的开关——经由单一工具门、受每次会话预算约束，从不委托。所有者的前提在真实会话上测得：五十五个声明占配置的九成，首个回合的成本是委托模式的九倍——直接会话换来的是低延迟，从来不是省钱。服务商的用量报告在浏览器中折算，并按开始时发布的费率计价，绘制在最后一行字幕下方；需要未声明费率的费用显示为不可用，绝不显示部分数字；只有费率表声明了的实时模型才会被提供。播放器中有两个测得的陷阱：Gemini 的情感对话会在模型必须回答的第一件事上关闭套接字，Chrome 152 会重复计划播放片段的第一个渲染块——PCM 播放器是读取队列的单一 AudioWorklet，绝不是每个片段一个源节点。

同样的两种模式随后抵达电话（ADR-301）。本人来电在通话*结束*时转达对话，而浏览器在通话*过程中*委托每个请求——同一个词的两种真相。语音会话现在有**模式**和**载体**，规则只跟随模式：`delegated`（「Live」），每个请求在说出时就是用户的聊天回合；`direct`（「直接 Live」），语音只读、不执行任何操作，话语在结束时作为用户自己的回合转达。**一次结束流程**服务两条线路，由浏览器的结束路由和电话的通话后 webhook 调用；两条线路共享的部分从两者中抽出到 `domains/voice_sessions/`，电话的 Live 模式就是浏览器委托的服务器端版本——一个异步的服务商工具，沿用浏览器的名称和模式；一个逐条规则镜像 TypeScript 桥接的桥（最新请求通过运行中桥接轮询的 Redis 标记胜出；LIA 提出的问题就是结果，下一个请求恢复提出该问题的运行）。在服务商的真实引擎上无需电话即可测量：语音先告知来电并继续交谈，超过服务商上限的迟到回答会丢失，因此桥接在此之前回答。生效模式是推导并公布的——只有回调主机公开时才有 Live——而运行器在回合期间不持有任何数据库会话：之前 8.9 秒的 `idle in transaction`，之后为零。
...
301 篇 ADR 不仅记录了做出的决策，还记录了被否决的替代方案和接受的权衡。1 852 个文件里的 ~30 855 个测试、完整的 CI/CD 和严格的 MyPy 并非虚荣指标 — 它们是让这种复杂度的系统能够无回归演进的机制。
...
*本文档基于源代码（`apps/api/src/`、`apps/web/src/`）、技术文档（680+ 份文档）、301 篇 ADR 及变更日志（v1.0 至 v1.47.1）的分析编写。文中引用的所有指标、版本和模式均可在代码库中验证。*
