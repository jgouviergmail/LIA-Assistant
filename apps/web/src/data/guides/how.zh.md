...
| 生产可靠性 | 245 篇 ADR、由 pytest 在 1 204 个文件中收集的 ~20 586 个测试、原生可观测性、6 层 HITL |
...
| 指标 | 数值 |
|------|------|
| 测试 | 20,586 个（由 pytest 在 1,204 个测试文件中收集）+ 前端 6,219 个 vitest 测试（覆盖率阈值已锁定，ADR-116） |
| pytest Fixtures | 752 个，其中 32 个通过 conftest 共享 |
| 文档 | 545 篇 |
| ADR（架构决策记录） | 242 篇 |
| Prometheus 指标 | 483 定义 |
| Grafana 仪表板 | 26 |
...
242 篇 MADR 格式的 ADR 记录了主要的架构决策。以下是一些代表性示例：
...
245 篇 ADR 不仅记录了做出的决策，还记录了被否决的替代方案和接受的权衡。1 204 个文件里的 ~20 586 个测试、完整的 CI/CD 和严格的 MyPy 并非虚荣指标 — 它们是让这种复杂度的系统能够无回归演进的机制。
