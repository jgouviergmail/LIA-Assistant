...
| 完全透明 | 589 Prometheus 指标、内嵌调试面板、逐 token 追踪 |
| 生产可靠性 | 317 篇 ADR、由 pytest 在 1 974 个文件中收集的 ~32 725 个测试、原生可观测性、6 层 HITL |
...
| 指标 | 数值 |
|------|------|
| 测试 | 32,725 个（由 pytest 在 1,974 个测试文件中收集）+ 前端 9,160 个 vitest 测试（覆盖率阈值已锁定，ADR-116） |
| pytest Fixtures | 1,082 个，其中 48 个通过 conftest 共享 |
| 文档 | 703 篇 |
| ADR（架构决策记录） | 317 篇 |
| Prometheus 指标 | 589 定义 |
| Grafana 仪表板 | 30 |
...
| 提供商 | 模型 | 特性 |
|--------|------|------|
| OpenAI | GPT-6（astra、sol、luna）、GPT-5.6（terra、sol、luna）、GPT-5.5、GPT-5.4（+ mini）、GPT-5.x（+ mini/nano）、GPT-4.1、GPT-4o、o3/o4-mini | 原生 prompt 缓存、Responses API、reasoning_effort |
| Anthropic | Claude Fable 5.1/5、Claude Opus 5.5/5/4.8/4.7/4.6/4.5、Claude Sonnet 5/4.6/4.5、Claude Haiku 4.5 | Extended thinking、prompt 缓存 |
| Google | Gemini 3.8/3.7/3.6/3.5 Flash、Gemini 3.1 Pro、Gemini 2.5 Pro/Flash | 多模态、双向量嵌入 |
| DeepSeek | deepseek-flash（V4.1）、deepseek-v4-pro（V4）、deepseek-chat（V3）、deepseek-reasoner（R1） | 低成本、原生推理 |
| Perplexity | Sonar、Sonar Pro | 搜索增强生成 |
| Qwen | Qwen 3.8（Max、Flash）、3.7（Max、Plus、Flash）、3.6（Plus、Flash）、3.5（Plus、Flash）、qwen3-max | 思考模式、工具 + 视觉（阿里云） |
| Ollama | 所有本地模型（能力读自服务器） | 原生客户端：可控思考、上下文窗口、受约束 JSON。零 API 成本、自托管 |

**为什么要 7 个提供商？** 这并非为了收藏而收藏，而是一种弹性策略：管道中的每个节点可以分配不同的提供商。如果 OpenAI 提价，路由器切换到 DeepSeek。如果 Anthropic 宕机，响应切换到 Gemini。LLM 抽象层（`src/infrastructure/llm/factory.py`）使用 Factory 模式配合 `init_chat_model()`，并通过特定适配器覆盖（`ResponsesLLM` 用于 OpenAI 的 Responses API，通过正则表达式 `^(gpt-4\.1|gpt-5|gpt-6|o[1-9])` 判断适用性）。
...
**架构**：父级 LangGraph 图中的 5 个自定义节点（非子图）：
...
循环**以结果评判**（ADR-310）。过去只要模型不再调用工具，循环就会停止，于是能否坚持下去取决于所配置的模型如何理解一句话。如今，循环着手去获取的每一条信息——用户的要求以及它发起的每一次交叉核对——最终要么获得，要么在结束草稿的 `<unresolved>` 块中声明，并附上尝试过的阶梯：先修正调用，再换一个标注为 `(fallback)` 的来源，最后才是声明。唯一的谓词 `should_recover` 读取这份声明，并调用决定停止的那个函数（`react_exit_reason`），而不是复制它；它会路由到 `react_recovery`——第五个节点，不调用任何模型：它在补救回合开始的那一刻就把草稿移出对话，本回合此后的每次调用都会临时收到——绝不写入 `messages`——紧随其后的草稿和一条指令。补救回合有上限（`REACT_RECOVERY_PASSES_MAX`），按结果计数；如果某个回合最终没有可用的回答，会交还草稿，而不是用它换来一场空。在上游，工具绝不替换它无法识别的值：天气工具会拒绝无法识别的日期，并给出接受的格式和今天的日期，这份约定由唯一的常量发布，清单和工具模式都读取它——循环绑定的是模式，而不是清单。

工具失败在这里以**结构化方式**读取（ADR-303）：`ToolMessage` 的正文是工具的文字说明，因此唯一诚实的判定是消息自身携带的标记——`status="error"`，经实测可在检查点之后保留。唯一的成功谓词 `core/tool_outcome.explicit_success` 同时服务于循环、查询登记簿和指标：声明的失败与空结果一样，不再换来额外的迭代。当用户在设置中选择频繁交流时（ADR-311；`REACT_CROSS_TURN_CACHE_ENABLED` 只决定尚未作出选择的账户的默认值，ADR-308），循环按注册顺序绑定全部工具，并把本回合的上下文放在问题之后——采用各服务商接受的形态，在启动时声明并校验——让一个回合的前缀成为下一个回合的前缀：在 396 个真实回合上测得，根据缓存机制不同，每回合节省 18% 至 42%；没有缓存的模型会多花钱；当目录超出上限或上下文窗口时，会回退到按相关性选择，并计入统计。此时历史也按块丢弃，并以回合计数器为锚，使每个回合都延续上一个回合：三者一同生效，因为实测表明单独任何一项都不划算；偶尔交流则保留按相关性选择，其成本不取决于两个回合之间的间隔。
...
图片生成遵循同样的「声明能力」规则（ADR-305）。一个**系列**——OpenAI GPT Image、Qwen Image 3.0——声明模型接受什么：它的质量档位、尺寸（固定列表或面积范围）、计费档位，以及修改时的参考图是否按张计费。每个界面都读取这唯一的答案：没有任何系列声明的模型，既不会被定价、提供、选择，也不会被执行。每家服务商有一个客户端，执行所配置的模型——OpenAI 的修改通过该模型上的 `images.edit` 完成，而此前绕道 Responses API 的做法执行的是 SDK 的默认模型，外加一个没人计费的文本模型——以 URL 形式交付的结果会在主机白名单内立即下载，并受大小限制和校验；用户的偏好是一种意图，在执行时解析为模型提供的选项（最便宜的可用质量、方向相同且面积最接近的尺寸），并作为实际生效值公布。所选格式在存储前统一转换，对所有服务商只应用一次。

在用户开启选项时，**新**图片的描述可以在调用之前由一个专用槽位按照各服务商公开的建议改写（ADR-315）。这是一种增强，绝不是一道关卡：引号中的文字必须原样返回，改写结果为空、过长或失败时，发出的是原始描述；修改图片时从不改写；这次调用和其他调用一样计费与审计，图库保留的是用户自己的原话。运营者可以为整个实例撤下这个选项，选项界面会公开这一状态，而不是提供一个不起作用的设置。
...
已生成的内容也可以**直接询问**找回（ADR-318）：一个读取工具按标题关键词、类别和时间段搜索当前图库，并把找到的文件以聊天自身的卡片形式展示，附上确切总数。生成的图片还可以作为副本**分享给连接**（ADR-316）：点击即确认——不经草稿，也不调用模型；两个每日配额在事务级锁下按发送者串行化；文件副本、接收方的图库记录与分享台账在同一事务中写入；随后以尽力而为的方式、在独立的会话中触达接收方的聊天。第三方的评论在那里被**逐字**引用：用数字字符引用加以中和，因为聊天会渲染带原始 HTML 和 `https:` 图片的 Markdown，而反斜杠转义会被渲染成公式。
...
**无需确认的操作。**当没有任何东西能改变一个操作的去向时，它就无需询问。发给自己的邮件（ADR-314）没有收件人参数：地址是所连接邮箱的服务商所声明的地址；没有连接邮箱时，则是账户经 LIA 中继的已验证地址——因此任何第三方文字都无法把它重定向。该工具声明为 `reversible` 并写明理由，所以例程可以在草稿被拒绝的场合发送自己的结果。而手动输入、恰好是本人的地址仍要经过草稿：一个有时跳过确认的收件人，会成为注入攻击的目标。
...
**唯一的检索入口**（ADR-313）。查询在打开会话之前先作为*键*进行嵌入——不套用对话中的琐碎语模式，因为它们会与真实姓名和简短问题冲突——然后返回该用户在相关度下限之上的有效记忆，可按类别收窄。所有调用方都经由它：规划器、主动发起与指代解析的记忆事实，对某个人的 360° 回想，以及电话中的查询——对「我对 X 了解什么」，回答的是 X，而不是最近的记忆。

**也可以主动检索。**一个可路由的领域带有一个读取工具，因此 ReAct 循环和规划器能检索消息里没有点名的话题——刚读到的邮件的发件人、文档里提到的地点。它的边界从其所执行的来源发布（类别词汇*就是*存储的词汇，结果上限*就是*该设置），用户的偏好会像拒绝注入一样拒绝它，敏感记忆返回时带着使用上的提示，失败会被分类，绝不会被说成「没有记忆」。BM25 + 嵌入的混合检索存在于真正用得上的地方：RAG Spaces（第 17 节）。
...
**日志也可以被主动检索**（ADR-318）。在注入之外，一个读取工具——两种模式都可用——会在用户的日志中检索途中遇到的话题，使用同样的向量，采用*配置的*相似度下限而不是学习得来的下限，并且只返回指引、模式和画像侧面：尚未整合的原始 `L0` 观察从不外泄。用户的偏好会像拒绝注入一样拒绝它。
...
计价本身也跟随提供商的时钟：一些提供商按 UTC 时段对文本模型计费，高峰时段的价格是低谷时段的数倍。因此每条价格记录都可以携带可选、互不重叠的 UTC 时段（支持跨越午夜）——时段生效期间覆盖单价，基础列则作为默认费率。时段还可以指定其适用的星期几——以时段开始时所在的 UTC 日期为准，因此跨越午夜的时段归属于其开始的那一天——因为高峰可能只存在于工作日：DeepSeek 的周末全天按低谷费率计费。一个唯一的实现为两个成本汇聚点解析当前时段：每次调用按其自身时刻计价——即提供商开具账单的时刻——历史消息重算时保留其原始时段的费率。时段随时间版本化的价格记录一起流转，在 LLM 价格对话框中管理，参考数据内置 DeepSeek 官方的分时费率（ADR-223）。

价格只有在**与服务商实际收费一致**时才有意义。写入提示词缓存按其价格计费——对报告缓存写入的 Anthropic 和 OpenAI 模型代际，为输入价格的 1.25 倍——并作为唯一用量读取器的第四个量计入；一道 AST 守卫会拒绝任何遗漏它的计价入口（ADR-306）。价格本身按服务商页面重新核对，修正迁移只替换 LIA 自己发布的值，绝不覆盖管理员填写的值；一次 Google Maps 调用按其请求触发的 SKU 归类——带实时路况或收费站的路线不是普通路线，距离矩阵按元素计费。价格缓存是一张**所有 worker 共享**的表：每个 worker 启动时都从数据库重建——绝不使用早于最近一次迁移的缓存副本——每个写入方，无论价格还是汇率，都在提交后发布失效通知，因此修改后的价格会立即到达每个 worker，而不是四个中只有一个（ADR-063）。
...
这项约定已发展为**适用于所有缓存机制的统一布局**（ADR-309）：稳定内容在前、易变内容在后，对边界只有一种读法（`core/prompt_layout.split_at_marker`），也不写入任何没人会再读取的缓存。单次调用——分析器、主动发起、记忆、兴趣和日志的提取——把固定部分作为系统消息发送，让各适配器都能对其标记；使用断点的服务商在静态前缀末尾获得断点，无论系统消息是什么形态（列表形态的系统消息曾把断点放在本回合数据上：读取为零，全部按 1.25 倍重写）；只有显式缓存的服务商会被显式标记；不会被任何工具循环延续的请求只写入其声明的断点；回复则只发送一次对话，以完整消息的形式。每一代模型接受什么——采样、强制工具选择、六种思考形态、隐式深度、与其对话绑定的思考块——都在 API 上实测，并声明在唯一的一张表中（Claude 为 `core/claude_surface.py`），由推理规则、适配器、结构化输出和管理界面共同读取。没有缓存的模型支付的费用与以前完全相同：优化追求的是各机制的平均收益，而不是某一个模型。
...
**网络调用期间不保持任何事务**（ADR-304）。基于调用方会话构建的连接器客户端，会迫使该会话——连同它的事务——在服务商应答期间一直保持打开：在生产环境实测，推送唤醒扫描中最长达 863 秒的 `idle in transaction`，简报、心跳或关系视图中为 11 到 330 秒。因此客户端绝不基于 `ConnectorService(db)` 构建——一道守卫直接点名构造函数参数，没有任何例外清单——；唯一的入口在一个短会话中打开某一类别的当前客户端，并在所有路径上关闭它；保留会话的任务在每次等待之前提交已读取的数据；聊天回合中的每个连接器操作都会结束自己的事务，API 密钥的使用时间戳则是一次独立的原子 JSONB 合并，并设有有限的锁等待时间。后台工作遵循同样的规则：提醒一次只认领一条，提交后在没有事务的情况下发送通知；Drive 推送流通过比较并交换推进，按数量和时间都受限的分页处理；只有主 worker 负责设置 Telegram webhook。

### 13.4. 两条认证路径

并非每个连接器都需要账号。**OAuth 连接器**保存用户的个人凭据：Gmail、日历、联系人、Drive。**平台密钥服务**不保存任何用户数据——用户无需开启任何东西，密钥属于这套部署：路线、地点、天气、环境。`ConnectorType.uses_global_api_key` 承载这一区分，工具基类根据**解析后**的类型选择凭据路径。因此同一个功能类别可以混用两者：天气既接受使用个人密钥的服务商，也接受平台服务，而调用方并不知道是谁作出了回应。
...
这一区分也决定了什么才属于**账户**。五个连接器不需要个人提供任何东西——维基百科、网页浏览、地点、天气、环境——因此它们属于实例：没有按账户存储的记录，设置中也看不到，由唯一的判定（`connectors/keyless.py`）决定它们是否为所有人服务——被管理员关闭的类型、实例缺少平台密钥或网页浏览被禁用，都会对所有人暂停该服务。在天气类别中，个人配置的服务商优先，Google 的服务为默认；一项测试确保这五个不会出现在设置的启用列表中（ADR-307）。
...
**联合身份不授予任何权限。** 关联 Google 身份绝不会改变账户状态：等待审批的注册依然等待，被封禁的账户依然被封禁。只有 Google 为所声明的地址担保时才会采信；如果该地址从未被证明，关联操作会证明它，同时撤销注册者的密码和会话，并像邮箱验证那样通知管理员；已删除的账户绝不会被恢复，拒绝会附带有限范围内的原因。「是谁在调用？」只有一个答案：`resolve_client_ip`，管理审计、会话地址和限流都读取它——一道 AST 守卫会拒绝对访客可自行填写的请求头的任何其他读取方式，Web 服务器则转发 Cloudflare 认证的地址。最后，日志配置是应用程序的第一个导入，工具在记录日志时才绑定日志记录器：更早绑定的日志记录器会沿用 structlog 的默认设置，既没有级别过滤，也没有个人数据过滤。
...
### 19.7. 一行日志只记录事实，绝不记录原话（ADR-317）

在 debug 级别以上，一行日志只携带**事实**——数量、长度、标识符、代码、URL 的主机名——绝不携带某人写下的文字，或模型关于他们写下的文字：查询、兴趣、文件/标签/空间的名称、浏览过的网页。这条规则**按值**把关，而不是按字段名：一项测试会读取代码中的每一个日志调用，拒绝预览（对标识符以外的内容做 `x[:n]` 截取）或任何由内容派生的值，无论它叫什么名字；每个例外都附有书面理由。错误文本所**引用**的内容会被过滤器扣下——PostgreSQL 的 `DETAIL` 与 `CONTEXT`、SQLAlchemy 的参数、Pydantic 的 `input_value`，回溯信息中也一样——数据库错误以其事实描述（SQLSTATE、约束、表），服务商密钥在所有被记录的 URL 中、在所有级别都会被遮蔽。有两个限制照实说明：外部服务以未知格式返回的错误信息仍可能引用某个值；debug 级别（生产环境中关闭）会保留更多内容。
...
| 技术 | 角色 |
|------|------|
| Prometheus | 589 自定义指标（RED 模式） |
| Grafana | 30 个生产就绪仪表板 |
| Loki | JSON 结构化日志聚合 |
| Tempo | 跨服务分布式追踪（OTLP gRPC） |
| Langfuse | LLM 专用追踪（prompt 版本、token 用量） |
| Alertmanager | 29 项关键告警核心，邮件通知（附带处置指南，按环境设置阈值） + 指向 LIA 的 Webhook：每条警报都成为产品内的事件（ADR-247） |
| structlog | 结构化日志，带 PII 过滤 |

**没有任何仪表板能展示的指标，就是没有人会去处理的指标。** 代码发出的内容与操作者实际能看到的内容之间的差距是被测量的，而非假设的：`scripts/audit/measure_metric_coverage.py` 用 AST 解析每一处指标定义（而不是正则——正则会把 `ZoneInfo("UTC")` 当成一个 `Info` 指标），并把每个名称与所有仪表板面板、recording rule 和告警表达式逐一比对。已定义 589 项；其中 44 项明确列入一份**只能缩短**的基线：新出现的盲点指标会让构建变红，而已接入的指标必须从清单中移除——否则下一个盲点会悄悄顶替它的位置。没有这套机制的代价：一个开路失效的心跳来源曾让健康信号在一周内丢失 46.5% 的 tick，却没有任何指标能察觉（ADR-148）。这道守卫从构造上关掉了两个陷阱——带标签且从未触发过的计数器**根本不会暴露任何序列**，因此监视罕见故障的面板必须写 `or vector(0)`，否则会在操作者期待绿色零值的地方显示「No data」；覆盖率只从面板与规则的**表达式**中读取，因为注释里提到的指标并没有接入。
...
- **阅读不变量与成熟的输入框** — 流式回复不再拖拽已向上滚动的读者：跟随决策在决策时刻实时测量几何（增长补偿），显式发送 tick 取代数据 diff 启发式（其中两个在真实引擎上误触发），悬浮按钮带屏外回复计数徽标把读者带回。输入框拥有按用户持久化的草稿（防抖，登出清除）、最近 10 条发送的 ↑/↓ 翻阅、`/` 斜杠命令（原生 textarea 上的 WAI-ARIA 组合框、不区分变音符的本地化筛选）以及每条回复下的行内操作行（复制、保存、分享、下载、反馈、执行轨迹——分享会打开系统分享面板和连接列表，选中连接只会预填输入框而不会直接发送；下载会写出一个带日期的 Markdown 文件）。
...
317 篇 MADR 格式的 ADR 记录了主要的架构决策。以下是一些代表性示例：
...
**一条公告会说明它发给了谁**（ADR-312）。管理员公告的受众在写入记录之前就已解析，收件人与公告一同保存：定向公告只会发给他们——包括他们下次登录时——而未选中任何活跃账户的选择会被拒绝，而不是发给没有人，或发给所有人。历史记录以四次查询提供一页内容及其确切总数、有上限的收件人样本及其确切人数、到期时刻和所选时长，与页面大小无关。
...
**日常计算不必等脚本：它们是工具**（ADR-318）。在两种模式下，计算式先被解析成语法树，再以精确的十进制求值——绝不执行——十进制库不抛错直接返回的无穷值（`0 ** -1`、`ln(0)`）会被拒绝，不得不四舍五入的结果会注明。时长在两个绝对时刻之间计算，因为同一时区的两个时间会按挂钟时间相减，把时钟调整那天的 25 小时算成 24 小时。货币换算读取欧洲央行的参考汇率并给出其日期。每一条被执行的边界都是一个设置，写在规划器清单与 ReAct 模式的 schema 共同读取的措辞里。下面的临时脚本则留给没有工具覆盖的需求。
...
**助手会读取自己的登记册**（ADR-318）。对「这周你为我做了什么？」，一个读取工具会依据两本登记册作答，给出精确总数，并采用与标签页相同的作者筛选——用户要求的、LIA 主动做的——而不是凭对某段对话的记忆。一本登记册因此服务两位读者，却不需要两套实现：绘制它的界面，与概括它的回答。
...
317 篇 ADR 不仅记录了做出的决策，还记录了被否决的替代方案和接受的权衡。1 974 个文件里的 ~32 725 个测试、完整的 CI/CD 和严格的 MyPy 并非虚荣指标 — 它们是让这种复杂度的系统能够无回归演进的机制。
...
*本文档基于源代码（`apps/api/src/`、`apps/web/src/`）、技术文档（680+ 份文档）、317 篇 ADR 及变更日志（v1.0 至 v1.47.4）的分析编写。文中引用的所有指标、版本和模式均可在代码库中验证。*
