...
| 生产可靠性 | 266 篇 ADR、由 pytest 在 1 488 个文件中收集的 ~24 454 个测试、原生可观测性、6 层 HITL |
...
每一次 tick 都以**执行记录**中的一行结束，在得到结果时写入、与状态标记同一事务——五种结局，对应执行器的五个出口，写在 savepoint 中，绝不阻塞例程本身。每条例程的本周由服务端用同一个为运行装弹的 cron 引擎计算：一个格子取「服务时刻**等于**该时段时刻」的最后一次执行，从不使用容差窗口，因此改动时间表会按构造清空网格，浏览器也从不重新解读 cron——它只负责绘制。引擎在六个时区里会整天跳过的、于午夜开启的夏令时空洞，在唯一的读取点得到修复，并通过覆盖全部 IANA 时区的差分测试证明。
...
266 篇 MADR 格式的 ADR 记录了主要的架构决策。以下是一些代表性示例：
...
266 篇 ADR 不仅记录了做出的决策，还记录了被否决的替代方案和接受的权衡。1 488 个文件里的 ~24 454 个测试、完整的 CI/CD 和严格的 MyPy 并非虚荣指标 — 它们是让这种复杂度的系统能够无回归演进的机制。
...
*本文档基于源代码（`apps/api/src/`、`apps/web/src/`）、技术文档（490+ 份文档）、266 篇 ADR 及变更日志（v1.0 至 v1.42.4）的分析编写。文中引用的所有指标、版本和模式均可在代码库中验证。*
