...
| 指标 | 数值 |
|------|------|
| 测试 | ~18 321 个（由 pytest 在 997 个测试文件中收集）+ 前端 5,475 个 vitest 测试（覆盖率阈值已锁定，ADR-116） |
| 可复用 Fixtures | 170+ |
| 文档 | 490+ |
| ADR（架构决策记录） | 220 篇 |
...
*本文档基于源代码（`apps/api/src/`、`apps/web/src/`）、技术文档（490+ 份文档）、221 篇 ADR 及变更日志（v1.0 至 v1.30.2的分析编写。文中引用的所有指标、版本和模式均可在代码库中验证。*
