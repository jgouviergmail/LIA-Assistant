...
│   │   ├── nodes/                # Graph nodes: pipeline, ReAct, HITL, initiative
...
│   │   ├── graphs/               # One agent builder per domain
...
│   ├── sub_agents/               # Ephemeral, delegated, read-only sub-agents
...
└── infrastructure/               # Cross-cutting layer
    ...
    └── observability/            # Prometheus metrics per subsystem, OTel tracing
```
...
A turn's spending obeys a conservation law (ADR-256): reasoning time and tool time are two named budgets, never one — a single delegation (a sub-agent, an iterative MCP task, a browser run) opens its own LLM loop behind one tool call, and at its pipeline bound it would equal 100% of the reasoning budget on its own. Every tool call carries an individual bound drawn from the same timeout families as the Pipeline mode, chosen so it is never stricter than the bound the layer below already applies, and a timeout raised by the tool itself is attributed to the tool, not to the loop's own cut. The stop condition remains one predicate with two readers: the router applies it to decide, the finalize node to explain.
...
A reviewer also has a blind spot worth designing against: a judge asked for a verdict reliably checks what a plan **contains** and misses what it **lacks**. So the inspection opens with a coverage pass — enumerate the demands the request establishes, then check each has a covering step — before any verdict is formed. Two shields keep it from firing on healthy plans: a demand met by ANALYSIS of data the plan already fetches belongs to the response LLM and is covered by construction, and a demand no capability can serve is not a missing step either. A step reported missing routes to the same silent re-planning as every other repairable defect.
...
**Provenance survives the summary.** A summary built from messages carrying third-party text inherits a provenance banner, and the summarising prompt reports third-party claims in a dedicated section attributed to their source — see §19.6.
...
```
PostgreSQL + pgvector
├── memories                                        → Long-term memory: facts, preferences, psychological profile
├── rag_documents + rag_chunks                      → Knowledge spaces, hybrid search
└── AsyncPostgresStore (user_id, "context", domain) → Tool context (Data Registry)
```
...
A tool's behaviour annotations (`readOnlyHint`, `destructiveHint`) are read in **one direction only**. The specification is normative here: a client must treat them as untrusted unless they come from a trusted server. A declared mutation is therefore believed — at worst a lying server buys itself a needless confirmation — while a read-only claim never is, because a declared category outranks the name heuristic and would pull the tool out of the invalid-mutation safety net and out of HITL scope detection. In iterative mode, where every tool of a server shares one confirmation setting, a tool the server itself declares destructive asks for confirmation regardless.
...
Two facts travel with each server as data, never as prompt prose. A user server whose credential is the user's own (OAuth, personal bearer or API key) publishes its account scope everywhere the model reads its capabilities — the delegation tool's description, the planner manifest, the sub-agent's context and the generated domain description — so "my repositories" resolves to the connected account instead of a question; an unauthenticated server publishes nothing, because claiming otherwise would be an invented capability. And the sub-agent returns its data, not a count: its output carries the same message-plus-bounded-data contract as every ReAct tool, with third-party payloads wrapped as external content.
...
Google's push channels were alive and functionally inert: their only consumer invalidated caches, so a notification bought freshness for a card nobody had opened. A processed notification now **queues a wake** — `SET NX` per (user, provider), so a storm is one wake dated by the first — and the webhook still answers 200 without deciding anything. A short leader-elected sweep serves the queue under the **full** eligibility checker: window, quota, cooldowns, the user's source preference. Only the "guaranteed minimum" smoothing is bypassed, because a wake answers an event.
...
Built-in FAQ indexed from `docs/knowledge/`. `is_app_help_query` detection by QueryAnalyzer, Rule 0 override in RoutingDecider, App Identity Prompt (~200 tokens, lazy loading). Staleness is judged on a SHA-256 over the source files **and** on the stored corpus itself (one chunk per parsed entry, exactly one document): a matching hash over the wrong number of rows is a repair, not a no-op. Auto-indexation runs in every uvicorn worker, so the space row is claimed with `FOR UPDATE SKIP LOCKED` — one writer, the others skip without queueing — and every vector is computed **before** the first destructive statement, so a provider rejection deletes nothing and the previous corpus keeps serving (ADR-162).

### 17.3. Mail sources: what the user designates (ADR-262)

A space can follow a **Gmail label**. Only the threads carrying it are
rendered — one thread, one Markdown document: subject as the title, messages
in order, attachment *names* only — and indexed like any other document.
Removing the label in Gmail removes the document at the next pass, so the
deletion gesture is the one the user already knows.

Indexing the whole mailbox was measured and refused: it duplicates a personal
archive into a second store, spends embeddings on a corpus that is mostly
notifications and lists, and dilutes retrieval with text nobody chose to keep.
A label is a designation the user maintains, in a tool they use daily. The
full sync anchors Gmail's history **before** it lists the threads, so a
message arriving mid-listing is replayed by the next incremental pass instead
...
Infra/personal info denylist      Code Hygiene (i18n, docs, cycles, CI parity)
Ruff + Black + MyPy               Secret scan (Gitleaks)
Fast unit tests                   Unit + agents tests, coverage floor
Critical pattern detection        Integration tests (PostgreSQL + Redis)
i18n key sync                     Migration replay (from scratch)
...
One CSS rule governs the design system's spacing: vertical margins on an `inline` element are **computed but never rendered**. A `<label>` is inline by browser default, so the label primitive is declared `block` — without it, every `space-y-*` above a control produces the same three-pixel gap whatever its value, and a recalibration changes the code without moving a pixel. The reflex follows: when a visual setting has no effect, measure the element's `display` and its DOM geometry in a real browser before suspecting the delivery chain. A guard pins it.
...
**The diagnostician reads its evidence at the moment it diagnoses.** A value and its threshold are a verdict, not a diagnosis, and four incidents in a row had shown it: the model answered « insufficient evidence » while the breakdown sat in Prometheus and the failing path in Loki. Each incident now declares a *recipe* — the catalogue queries and the log events worth reading for its correlation key, asserted complete at boot — and the diagnosis pump collects the pack once per incident, after the budget gate: breakdown series, a bounded log excerpt filtered through a closed allowlist and the personal-data sanitizer, the running build and its uptime. Every source degrades on its own: an unreachable log store costs the diagnosis its excerpt, never the diagnosis, and is named as unavailable rather than read as silence. What the model saw is stored with what it wrote and drawn under the verdict for the administrator.
...
## 38. Meeting minutes: the row is the job, the template is the contract

**A meeting is hours of audio produced on a device that may sleep, lose its network or be closed by mistake — so capture is designed for the failure, not the nominal case.** The browser records in short segments and uploads each one as a raw-body request the moment it exists; a segment is a file on the server, written to a temporary name and renamed atomically, so four API workers can receive out of order without ever interleaving bytes, a duplicate upload is a harmless overwrite, and detecting a gap is a directory listing. Two sources sit behind one interface: Opus through `MediaRecorder` where the engine is trustworthy, raw 16 kHz PCM through the AudioWorklet the voice input already uses everywhere else — the only path on Apple devices, whose recorder produced a container the backend refuses and misbehaved in home-screen apps. The format is a pure function of the environment, decided once before the first byte. Uploads leave in order, retry with a backoff that never gives up while the recording lasts, and wait — not fail — while the browser is offline; a stop declares the uploader's own count, and sequences the server never received are refused by name.

**The meeting row is the durable job.** Its status carries the whole lifecycle — recording, interrupted, stopped, processing, ready or failed — and every transition is one conditional `UPDATE`. The processing claim takes a lease, heartbeats renew it while publishing the stage the page shows (normalizing, transcribing, synthesizing, indexing), a lost heartbeat aborts every later effect, and reapers re-drive stopped orphans and expired leases within a bounded attempt budget. Refusals that are not pipeline failures — a usage limit, no engine available — release the job without consuming an attempt. And since a bulk update leaves SQLAlchemy's identity map stale, every read that follows one expires the session first: the runtime proof measured a stop that answered `status: recording` before that rule existed.

**Engines form a chain, and the chain is walked twice.** Once before the first second, from the provider-key cache only: the administrator's speech slot, then ElevenLabs Scribe or OpenAI `gpt-4o-transcribe-diarize` — whole file, speakers separated — then the local Whisper engine. The local path used to decode one thirty-second window and silently return the first thirty seconds of anything longer; a voice command never met the defect, a meeting always would. Long audio is now cut by Silero VAD into speech windows of at most twenty seconds, each decoded on its own, with fixed windows when the model is absent — never truncation. The chain is walked again at processing time: a permanent fault of one provider (a refused key, a file it cannot take) hands over to the next engine within the user's preference; only silence, which says something about the audio, or a transient fault, which belongs to the retry budget, stops the walk. The dev instance had stored a key ID where a key belongs; without the second walk every meeting would have dead-lettered with an OpenAI key one step down.
...
## 42. The anatomy of a process: what it loads is declared, measured and bounded

An API server is several processes: a supervisor that serves no request, and workers that serve everything. Each holds memory for different reasons, and "the container uses five gigabytes" says nothing useful until you know which process holds what. LIA applies one rule here: **what a process loads is declared, measured per process, and bounded where it multiplies**. Every item was measured on the target host, in a throwaway process, before being touched — and three measurements contradicted the first reading, which is exactly why one measures.

The supervisor does not load the application. uvicorn's command line imports the application in the parent process to produce an early import error, then discards it — a parent that only watches its workers thus held several hundred megabytes for the life of the container. `python -m src.serve` drives the same `Config → Server → Multiprocess` chain with the same flags, without that import; an AST guard refuses the module any import from `src`, and the early error is rendered by the health check the deployment already reads. In the same spirit, a heavy library — PDF rendering, tabular computation, audio engines — is imported where it is used and never at boot: a list declares each one's measured cost, and a guard refuses any module-level import under `src/`, including one hidden in a module-level `try`.

What a singleton loads is multiplied by the worker count. Speech transcription kept one resident model per language ever asked for, in a singleton per process; the cache is now bounded (`VOICE_STT_MAX_RECOGNIZERS`, one by default), least-recently-used out, nothing loaded at construction, an eviction returns memory to the system — measured — and a decode in flight keeps its own reference. And every worker publishes what it holds: `lia_worker_memory_bytes{kind}` is read by each worker from its own `/proc/self/status`, one series per live worker that disappears with it, drawn on two panels and watched by the `ApiWorkerMemoryHigh` alert, which names the process. It answers a structural limit: cAdvisor sees the container, and prometheus_client's multiprocess mode exports no process metric.
