...
30. [Trabajo visible, aprendizaje gobernado](#30-trabajo-visible-aprendizaje-gobernado)
...
| Google | Gemini 3.8/3.7/3.6/3.5 Flash, Gemini 3.1 Pro, Gemini 2.5 Pro/Flash | Multimodal, embeddings duales |
| DeepSeek | deepseek-flash (V4.1), deepseek-v4-pro (V4), deepseek-chat (V3), deepseek-reasoner (R1) | Coste reducido, reasoning nativo |
...
| Ollama | Cualquier modelo local (capacidades leídas del servidor) | Cliente nativo: razonamiento controlado, ventana de contexto, JSON restringido. Coste API cero, auto-hospedado |

**¿Por qué 7 providers?** La elección no es la colección por sí misma. Es una estrategia de resiliencia: cada nodo del pipeline puede asignarse a un provider diferente. Si OpenAI aumenta sus tarifas, el router pasa a DeepSeek. Si Anthropic tiene una caída, la respuesta se redirige a Gemini. La abstracción LLM (`src/infrastructure/llm/factory.py`) utiliza el pattern Factory con `init_chat_model()`, sobrecargado por adaptadores específicos (`ResponsesLLM` para la API Responses de OpenAI, elegibilidad por regex `^(gpt-4\.1|gpt-5|gpt-6|o[1-9])`).
...
│   ├── config/                   # Módulos Pydantic BaseSettings compuestos via MRO
│   │   ├── __init__.py           # Clase Settings (MRO final)
│   │   ├── agents.py, database.py, llm.py, mcp.py, voice.py, usage_limits.py, ...
│   ├── constants.py              # 2 000+ constantes centralizadas
...
│   │   ├── nodes/                # Nodos del grafo: pipeline, ReAct, HITL, iniciativa
...
│   │   ├── graphs/               # Un builder de agente por dominio
...
│   ├── sub_agents/               # Subagentes efímeros, delegados y de solo lectura
...
└── infrastructure/               # Capa transversal
    ...
    └── observability/            # Métricas Prometheus por subsistema, tracing OTel
```
...
**Arquitectura**: 5 nodos propios en el grafo LangGraph principal (no un subgrafo):
...
```
PostgreSQL + pgvector
├── memories                                        → Memoria a largo plazo: hechos, preferencias, perfil psicológico
├── rag_documents + rag_chunks                      → Espacios de conocimiento, búsqueda híbrida
└── AsyncPostgresStore (user_id, "context", domain) → Contexto herramientas (Data Registry)
```
...
La convención se ha convertido en **una disposición para todos los mecanismos de caché** (ADR-309): lo estable antes que lo volátil, una única lectura de la frontera (`core/prompt_layout.split_at_marker`) y ninguna escritura de caché que nadie vuelva a leer. Una llamada única — el analizador, la iniciativa, las extracciones de memoria, de intereses y del diario — envía su parte fija como mensaje de sistema, donde cada adaptador puede marcarla; los proveedores con puntos de corte reciben el suyo al final del prefijo estático sea cual sea la forma del sistema (un sistema en lista colocaba el punto de corte sobre los datos del turno: nada leído, todo reescrito a 1,25); los que solo tienen caché explícita se marcan explícitamente; una solicitud que ningún bucle de herramientas prolongará solo escribe sus puntos de corte declarados; y la respuesta envía la conversación una sola vez, en mensajes completos. Lo que acepta cada generación de modelos — muestreo, elección de herramienta forzada, seis formas de razonamiento, profundidad implícita, bloques de razonamiento ligados a su conversación — se mide en la API y se declara en una única tabla (`core/claude_surface.py` para Claude) que leen las reglas de razonamiento, el adaptador, la salida estructurada y la administración. Un modelo sin caché paga exactamente lo que pagaba: la optimización apunta a la media de los mecanismos, nunca a un modelo.
...
**Ninguna transacción queda abierta durante una llamada de red** (ADR-304). Un cliente de conector construido sobre la sesión de su llamante obligaba a esa sesión — y a su transacción — a seguir abierta mientras el proveedor respondía: medido en producción, hasta 863 segundos `idle in transaction` en el barrido de despertares push, de 11 a 330 segundos en el briefing, el heartbeat o la vista de relaciones. Por eso un cliente nunca se construye sobre `ConnectorService(db)` — una guarda nombra el argumento del constructor, sin lista de excepciones —; una única puerta abre el cliente activo de una categoría en una sesión corta y lo cierra en todos los caminos; un job que conserva su sesión confirma sus lecturas antes de cada espera; cada operación de conector del turno de chat termina su transacción, y el sello de uso de una clave API es una fusión JSONB atómica aparte, con un tiempo de espera de bloqueo acotado. Los trabajos de fondo siguen la misma regla: un recordatorio se reclama solo, se confirma y luego se notifica sin transacción abierta; el flujo push de Drive avanza por comparar-e-intercambiar, en páginas acotadas en número y en tiempo; y solo el worker líder fija el webhook de Telegram.

### 13.4. Dos caminos de autenticación

No todos los conectores piden una cuenta. Un **conector OAuth** guarda las credenciales personales del usuario: Gmail, Calendario, Contactos, Drive. Un **servicio con clave de plataforma** no guarda ningún dato por usuario — no hay nada que activar, y la clave pertenece a la instalación: Rutas, Lugares, Meteorología, Entorno. `ConnectorType.uses_global_api_key` lleva esa distinción, y la base de herramientas elige el camino de credenciales según el tipo **resuelto**. Una misma categoría funcional puede así mezclar ambos: la meteorología acepta un proveedor con clave personal igual que un servicio de plataforma, sin que quien llama sepa cuál respondió.
...
La distinción decide también qué pertenece a una **cuenta**. Cinco conectores no piden nada a la persona — Wikipedia, la navegación de páginas, Lugares, Tiempo, Entorno — así que pertenecen a la instancia: ninguna fila por cuenta, nada en los ajustes, y un solo predicado (`connectors/keyless.py`) decide si sirven a todos — un tipo apagado por el administrador, una clave de plataforma que la instancia no tiene o la navegación desactivada lo retiene para todos. En la categoría del tiempo gana el proveedor de la persona y el de Google es el predeterminado; un test mantiene los cinco fuera de la lista de activación de los ajustes (ADR-307).
...
Los canales push de Google estaban vivos y funcionalmente inertes: su único consumidor invalidaba cachés, de modo que una notificación compraba frescura para una tarjeta que nadie había abierto. Una notificación procesada **pone ahora al usuario en cola** —`SET NX` por (usuario, proveedor), así que una avalancha es un solo despertar fechado por el primero— y el webhook sigue respondiendo 200 sin decidir nada. Una pasada corta, con elección de líder, atiende la cola bajo el verificador de elegibilidad **completo**: franja, tope, pausas, preferencia de fuentes. Solo se omite el suavizado del «mínimo garantizado», porque un despertar responde a un evento.
...
Un espacio puede seguir una **etiqueta de Gmail**. Solo se renderizan las
conversaciones que la llevan — una conversación, un documento Markdown: el
asunto como título, los mensajes en orden, únicamente los *nombres* de los
adjuntos — y se indexan como cualquier otro documento. Quitar la etiqueta en
Gmail elimina el documento en el siguiente pase: el gesto de borrado es el que
ya se conoce.

Indexar todo el buzón se midió y se descartó: duplica un archivo personal en
un segundo almacén, gasta embeddings en un corpus hecho sobre todo de
notificaciones y listas, y diluye la búsqueda con texto que nadie eligió
conservar. Una etiqueta es una designación que se mantiene en una herramienta
de uso diario. La sincronización completa ancla el historial de Gmail **antes**
de listar las conversaciones, de modo que un mensaje llegado durante el listado
se recupera en el pase incremental siguiente; ese pase viaja sobre el despertar
...
**Una identidad federada no otorga ningún derecho.** Vincular una identidad de Google nunca cambia el estado de una cuenta: un registro pendiente de aprobación sigue pendiente, una cuenta bloqueada sigue bloqueada. La dirección anunciada solo se cree si Google la avala; cuando nunca se había demostrado, la vinculación la demuestra, revoca la contraseña y las sesiones de quien la registró y avisa a los administradores como lo haría una verificación de correo; una cuenta eliminada nunca se reactiva, y un rechazo lleva un motivo acotado. «¿Quién llama?» tiene una sola respuesta, `resolve_client_ip`, que leen las auditorías de administración, la dirección de las sesiones y los límites de frecuencia — una guarda AST rechaza cualquier otra lectura de la cabecera que escribe el visitante, y el servidor web transmite la dirección que Cloudflare certifica. Por último, la configuración de los registros es la primera importación de la aplicación, y una herramienta vincula su registrador en el momento en que registra: un registrador vinculado antes conservaba los valores por defecto de structlog, sin filtro de nivel ni de datos personales.
...
Datos infra/personales            Code Hygiene (i18n, docs, ciclos, paridad CI)
Ruff + Black + MyPy               Secret scan (Gitleaks)
Unit tests rápidos                Tests unitarios + agentes, suelo de cobertura
Detección patterns críticos       Tests de integración (PostgreSQL + Redis)
Sync claves i18n                  Replay de migraciones (desde cero)
