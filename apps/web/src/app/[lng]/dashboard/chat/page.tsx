...
export default function ChatPage() {
  ...
  const { offset: parallaxOffset, isSupported: parallaxSupported, hasPermission: parallaxPermission, requestPermission: requestParallaxPermission } = useDeviceParallax({
    maxOffset: 15, // pixels
    smoothing: 0.12,
  });
  ...
  return (
    ...
    <div className="flex h-[calc(100vh-5.25rem)] gap-4">
      {/* Main Chat Area */}
      <div className={`flex flex-col flex-1 bg-background rounded-xl border border-border/50 shadow-lg overflow-hidden ${showDebugPanel ? 'max-w-[calc(100%-420px)]' : ''}`}>
      ...
      {user?.tokens_display_enabled && (combinedTotals.tokensIn > 0 || combinedTotals.tokensOut > 0) && (
        <div className="hidden mobile:flex bg-muted/50 border-b border-border px-4 py-3 items-center justify-center text-xs">
          <div className="flex items-center gap-4">
            ...
            <span className="text-purple-600">
              🔢{' '}
              {formatNumber(
                combinedTotals.tokensIn + combinedTotals.tokensOut + combinedTotals.tokensCache
              )}{' '}
              TOTAL
            </span>
            <span className="text-orange-500">🟠 {formatNumber(combinedTotals.tokensIn)} IN</span>
            <span className="text-green-600">🟢 {formatNumber(combinedTotals.tokensOut)} OUT</span>
            ...
            <span className="text-muted-foreground">|</span>
            <span className="text-primary font-bold">{formatEuro(combinedTotals.costEur)}</span>
          </div>
        </div>
      )}
