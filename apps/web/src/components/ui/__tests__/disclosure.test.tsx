...
describe('Disclosure', () => {
  it('is closed on arrival and shows nothing of its content', () => {
    renderWithProviders(
      <Disclosure icon={Bell} title="Historique">
        <p>contenu</p>
      </Disclosure>
    );
  ...
  it('tells the caller when it opens, so a fetch can wait for it', async () => {
    ...
    const { user } = renderWithProviders(
      <Disclosure icon={Bell} title="Historique" onOpenChange={onOpenChange}>
        <p>contenu</p>
      </Disclosure>
    );
  ...
  it('shows a badge next to the title when one is given', () => {
    renderWithProviders(
      <Disclosure icon={Bell} title="Historique" badge="57">
        <p>contenu</p>
      </Disclosure>
    );
  ...
  it('can be asked to start open', () => {
    renderWithProviders(
      <Disclosure icon={Bell} title="Historique" defaultOpen>
        <p>contenu</p>
      </Disclosure>
    );
...
describe('a parent re-render must not shut it under the reader', () => {
  ...
  it('stays open when a sibling prop changes', async () => {
    const { user, rerender } = renderWithProviders(
      <Disclosure icon={Bell} title="Historique" badge="1">
        <p>contenu</p>
      </Disclosure>
    );
    ...
    rerender(
      <Disclosure icon={Bell} title="Historique" badge="2">
        <p>contenu</p>
      </Disclosure>
    );
