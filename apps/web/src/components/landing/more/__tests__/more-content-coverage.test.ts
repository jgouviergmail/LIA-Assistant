 ...
 *
 * 1. Structural contract: exactly 51 unique cards across 6 sections, every
...
describe('more-data structural contract', () => {
  it('has 51 unique cards across 6 sections', () => {
    expect(MORE_SECTIONS).toHaveLength(6);
    expect(MORE_CARD_KEYS).toHaveLength(51);
    expect(new Set(MORE_CARD_KEYS).size).toBe(51);
    expect(MORE_SECTIONS.map(s => s.cards.length)).toEqual([4, 9, 7, 7, 8, 16]);
  });
