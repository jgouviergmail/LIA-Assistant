 ...
 *
 * 1. Structural contract: exactly 41 unique cards across 6 sections, every
...
describe('more-data structural contract', () => {
  it('has 41 unique cards across 6 sections', () => {
    expect(MORE_SECTIONS).toHaveLength(6);
    expect(MORE_CARD_KEYS).toHaveLength(41);
    expect(new Set(MORE_CARD_KEYS).size).toBe(41);
    expect(MORE_SECTIONS.map(s => s.cards.length)).toEqual([4, 8, 7, 6, 7, 9]);
  });
