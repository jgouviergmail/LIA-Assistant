 ...
 *
 * 1. Structural contract: exactly 65 unique cards across 6 sections, every
...
describe('more-data structural contract', () => {
  it('has 65 unique cards across 6 sections', () => {
    expect(MORE_SECTIONS).toHaveLength(6);
    expect(MORE_CARD_KEYS).toHaveLength(65);
    expect(new Set(MORE_CARD_KEYS).size).toBe(65);
    expect(MORE_SECTIONS.map(s => s.cards.length)).toEqual([4, 10, 9, 12, 9, 21]);
  });
