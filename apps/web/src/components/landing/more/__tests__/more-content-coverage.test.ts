 ...
 *
 * 1. Structural contract: exactly 42 unique cards across 6 sections, every
...
describe('more-data structural contract', () => {
  it('has 42 unique cards across 6 sections', () => {
    expect(MORE_SECTIONS).toHaveLength(6);
    expect(MORE_CARD_KEYS).toHaveLength(42);
    expect(new Set(MORE_CARD_KEYS).size).toBe(42);
    expect(MORE_SECTIONS.map(s => s.cards.length)).toEqual([4, 8, 7, 6, 7, 10]);
  });
