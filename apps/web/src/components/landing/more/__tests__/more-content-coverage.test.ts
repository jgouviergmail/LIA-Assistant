 ...
 *
 * 1. Structural contract: exactly 59 unique cards across 6 sections, every
...
describe('more-data structural contract', () => {
  it('has 59 unique cards across 6 sections', () => {
    expect(MORE_SECTIONS).toHaveLength(6);
    expect(MORE_CARD_KEYS).toHaveLength(59);
    expect(new Set(MORE_CARD_KEYS).size).toBe(59);
    expect(MORE_SECTIONS.map(s => s.cards.length)).toEqual([4, 10, 9, 10, 9, 17]);
  });
