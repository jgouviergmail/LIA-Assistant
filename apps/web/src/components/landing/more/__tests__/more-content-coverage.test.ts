 ...
 *
 * 1. Structural contract: exactly 46 unique cards across 6 sections, every
...
describe('more-data structural contract', () => {
  it('has 46 unique cards across 6 sections', () => {
    expect(MORE_SECTIONS).toHaveLength(6);
    expect(MORE_CARD_KEYS).toHaveLength(46);
    expect(new Set(MORE_CARD_KEYS).size).toBe(46);
    expect(MORE_SECTIONS.map(s => s.cards.length)).toEqual([4, 8, 7, 7, 7, 13]);
  });
