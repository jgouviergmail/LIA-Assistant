 ...
 *
 * 1. Structural contract: exactly 54 unique cards across 6 sections, every
...
describe('more-data structural contract', () => {
  it('has 54 unique cards across 6 sections', () => {
    expect(MORE_SECTIONS).toHaveLength(6);
    expect(MORE_CARD_KEYS).toHaveLength(54);
    expect(new Set(MORE_CARD_KEYS).size).toBe(54);
    expect(MORE_SECTIONS.map(s => s.cards.length)).toEqual([4, 9, 8, 9, 8, 16]);
  });
