 ...
 *
 * 1. Structural contract: exactly 57 unique cards across 6 sections, every
...
describe('more-data structural contract', () => {
  it('has 57 unique cards across 6 sections', () => {
    expect(MORE_SECTIONS).toHaveLength(6);
    expect(MORE_CARD_KEYS).toHaveLength(57);
    expect(new Set(MORE_CARD_KEYS).size).toBe(57);
    expect(MORE_SECTIONS.map(s => s.cards.length)).toEqual([4, 10, 8, 10, 8, 17]);
  });
