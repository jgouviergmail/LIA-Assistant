 ...
 *
 * 1. Structural contract: exactly 56 unique cards across 6 sections, every
...
describe('more-data structural contract', () => {
  it('has 56 unique cards across 6 sections', () => {
    expect(MORE_SECTIONS).toHaveLength(6);
    expect(MORE_CARD_KEYS).toHaveLength(56);
    expect(new Set(MORE_CARD_KEYS).size).toBe(56);
    expect(MORE_SECTIONS.map(s => s.cards.length)).toEqual([4, 9, 8, 10, 8, 17]);
  });
