...
const eslintConfig = defineConfig([
  ...
  {
    // NOTE — a `settings['jsx-a11y'].components` map for `<Input>`/`<Label>`/…
    // was measured and REJECTED (2026-08-05): jsx-a11y analyses each element in
    // isolation, so it cannot follow `htmlFor` -> `id` between sibling
    // components, which is exactly this design system's pattern. The map
    // produced 96 findings, of which the sampled ones were all correct code
    // (`<Label htmlFor="x">` + `<Input id="x">`). Accessible names of design
    // system controls are guarded on the RENDERED DOM instead — see
    // `src/components/ui/__tests__/form-control-names.guard.test.tsx`.
