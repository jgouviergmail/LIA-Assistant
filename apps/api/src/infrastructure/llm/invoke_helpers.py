...
async def invoke_with_instrumentation(
    ...
    **invoke_kwargs: Any,
    ...
    Example:
        ...
        ... )
    """
    # Layer 2 enforcement, shared with the structured-output door: one rule,
    # one implementation. Until 2026-09-07 this block was that rule's ONLY
    # copy, and the other door had none.
    await enforce_usage_limit(user_id, layer="llm_invoke", state=state, config=config)
