...
QUERY_CATALOGUE: dict[str, NamedQuery] = {
    ...
    for query in (
        ...
        NamedQuery(
            query_id="api_worker_heap_mb",
            title="API worker heap (anonymous memory) by process",
            promql_template='max by (pid) (lia_worker_memory_bytes{kind="anon"}) / 1048576',
            params=(),
            unit="megabytes",
            lia_metrics=("lia_worker_memory_bytes",),
            external_metrics=(),
        ),
        NamedQuery(
            query_id="stt_recognizers_by_worker",
            title="Resident STT recognizers by worker process",
            promql_template="max by (pid) (voice_stt_recognizers_loaded)",
            params=(),
            unit="count",
            lia_metrics=("voice_stt_recognizers_loaded",),
            external_metrics=(),
        ),
        NamedQuery(
