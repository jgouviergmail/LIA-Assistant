...
async def _execute_wave_parallel(
    ...
    # `_execute_single_step_async` via `compute_step_timeout`, so each task
...
# Per-tool-family timeout policy: extracted to `step_timeouts.py` on
# 2026-08-20 (this file is frozen at its size cap; the policy is a
# cohesive, executor-state-free unit). See that module for the full
# floor/ceiling rationale per family.
...
async def _execute_single_step_async(
    ...
    # See `step_timeouts.compute_step_timeout` for the full policy.
    timeout_seconds = compute_step_timeout(step.tool_name, step.timeout_seconds)
