...
class FailedStep(BaseModel):
    """One step of a plan that did not succeed, exactly as the executor recorded it.

    Carried by every ``AgentResult``, successful or not: an aggregate status is
    binary, so it cannot say « three of five steps failed ». This field can, and
    it is what the honesty directive reads (ADR-303).

    Attributes:
        step_index: Position of the step in the plan.
        tool_name: Tool the step called.
        error: Error message the tool returned, when it gave one.
        error_code: ``ToolErrorCode`` value when the payload carried a known
            one, else None — a free-form code stays out of the taxonomy.
    """

    step_index: int = Field(description="Position of the step in the plan")
    tool_name: str = Field(description="Tool the step called")
    error: str | None = Field(default=None, description="Error message head the tool returned")
    error_code: str | None = Field(
        default=None, description="ToolErrorCode value when the tool gave a known one"
    )

    @field_validator("error", mode="before")
    @classmethod
    def _bound_error_head(cls, value: object) -> object:
        """Keep the HEAD of the message — this entry is persisted in the state.

        ``StepResult.error`` is built at the source as ``str(e)``, and one
        branch as ``f"{type(result).__name__}: {result}"`` — a whole payload.
        ``agent_results`` is capped « for memory management » and checkpointed
        to PostgreSQL on every turn, so the bound belongs to the MODEL: no
        producer can put a document there, wherever it builds one. Measured
        before: five failed steps, 200 350 bytes (ADR-303 cold review).

        Args:
            value: Whatever the producer passed.

        Returns:
            The value, its head when it is an oversized string.
        """
        if isinstance(value, str) and len(value) > TOOL_ERROR_HEAD_CHARS:
            return value[:TOOL_ERROR_HEAD_CHARS]
        return value

    @field_validator("error_code", mode="before")
    @classmethod
    def _bound_code(cls, value: object) -> object:
        """A code is an identifier, and it travels to labels and logs."""
        if isinstance(value, str) and len(value) > TOOL_ERROR_CODE_MAX_CHARS:
            return value[:TOOL_ERROR_CODE_MAX_CHARS]
        return value
...
class AgentResult(BaseModel):
    ...
    Attributes:
        ...
        status: Execution status — a value of ``AgentResultStatus`` (ADR-303).
        data: Agent-specific result data (ContactsResultData, dict, or None).
        error: Error message when the status is ``error``.
        failed_steps: Every step that failed, whatever the aggregate status —
            this is what states a PARTIAL failure, never the status.
    ...
    status: Literal["success", "error"] = Field(
        description="Execution status — the values of AgentResultStatus"
    )
    data: (
        ContactsResultData
        | EmailsResultData
    ...
    failed_steps: list[FailedStep] = Field(
        default_factory=list,
        description="Every step that failed, whatever the aggregate status (ADR-303)",
    )
