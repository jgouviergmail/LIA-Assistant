        ...
        async with get_db_context() as db:
            ...
            try:
                ...
                schedule_outcome_recording(
                    ...
                    # Seam closed (habits program Lot 0): the primary domain the
                    # CURRENT run resolved, captured by the streaming service as
                    # the query_intelligence delta flowed by (the local `state`
                    # dict is the PRE-run checkpoint — reading it here would
                    # record the previous turn's domain). Validated downstream
                    # against DOMAIN_REGISTRY.
                    domain=streaming_service.primary_domain,
                )
