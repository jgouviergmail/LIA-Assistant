...
class MessagesState(TypedDict):
    ...
    pending_drafts_queue: list[dict[str, Any]]  # The drafts still to be reviewed after this one
    # ADR-288: True only for a pre-approved FOR_EACH lot (shown whole, confirmed
    # whole); any other queue is walked ONE draft per interrupt, the decisions
    # banked in confirmed_drafts and executed together after the last one.
    pending_drafts_grouped: bool
    confirmed_drafts: list[dict[str, Any]]
...
def create_initial_state(
    ...
    return MessagesState(
        ...
        pending_drafts_queue=[],  # The drafts still to be reviewed after this one
        pending_drafts_grouped=False,  # ADR-288: a pre-approved FOR_EACH lot only
        confirmed_drafts=[],  # ADR-288: decisions banked along a sequence
