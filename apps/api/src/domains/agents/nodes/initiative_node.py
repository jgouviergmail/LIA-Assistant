...
# InitiativeAction is re-exported on purpose: initiative_plan and the test
# suite import it from HERE (its historical home). The `as` idiom marks the
# intentional re-export for Ruff.
from src.domains.agents.nodes.initiative_schemas import (
    InitiativeAction as InitiativeAction,
)
from src.domains.agents.nodes.initiative_schemas import (
    InitiativeDecision,
)
...
def _handoff_motivation(decision: InitiativeDecision, followups: list[str], run_id: str) -> None:
    """Push the provenance line (Lot 1-A3) — only next to an actual proposal."""
    motivation = sanitize_motivation(decision.motivation)
    if motivation and (followups or decision.suggestion):
        push_motivation(run_id, motivation)
...
async def _initiative_core(
    ...
    _handoff_motivation(decision, followups, run_id)
    if followups:
