...
Defines the ``User`` aggregate (profile, preferences, relationships) and
``AdminAuditLog``. The User model moved here from the auth domain (ADR-126:
auth = identity/session, users = account lifecycle & profile), which is why
the ``users`` table is the hub the per-user domain tables hang off.
"""
...
class User(BaseModel):
    ...
    last_known_location_updated_at: Mapped[datetime | None] = mapped_column(
