...
async def test_get_rate_cache_hit():
    ...
    service._cache["USD_EUR"] = (Decimal("0.95"), datetime.now(UTC))
...
async def test_get_rate_cache_expiry():
    ...
    service._cache["USD_EUR"] = (Decimal("0.90"), expired_time)
    ...
    assert service._cache["USD_EUR"][0] == Decimal("0.96")


@pytest.mark.asyncio
async def test_get_rate_api_http_error():
    """Test API HTTP error returns None."""
    service = CurrencyRateService()
