...
def _make_script_only_skill(name: str, scripts: list[str] | None = None) -> dict:
    """Create a skill that has scripts but no deterministic plan_template.

    This matches the Visualizer/Generator archetypes where the ReactSubAgentRunner
    handles end-to-end execution (e.g., ``interactive-map``).
    """
    return {
        "name": name,
        "priority": 50,
        "scope": "admin",
        "owner_id": None,
        "plan_template": None,  # no deterministic template
        "scripts": scripts or ["render.py"],
    }


SCRIPT_ONLY_SKILL = _make_script_only_skill("interactive-map", ["render_map.py"])

NON_DETERMINISTIC_NO_SCRIPTS_SKILL = {
...
class TestPlan:
    ...
    @pytest.mark.unit
    async def test_non_deterministic_skill_without_scripts_returns_failure(self):
        """Non-deterministic skills WITHOUT scripts are left to the LLM planner
        (prompt-expert / advisory archetypes)."""
        strategy = SkillBypassStrategy()
        # NON_DETERMINISTIC_SKILL (research-assistant) has a plan_template
        # with deterministic=False but no 'scripts' field → falls back to LLM.
        # To be explicit, use the prompt-only skill that has no scripts at all.
        intel = _make_intelligence(detected_skill_name="prompt-only-skill")
        config = _make_config()
    ...
    @pytest.mark.unit
    async def test_script_only_skill_yields_to_the_llm_planner_when_cumulating(self):
        """Script-only skills must NOT emit an empty plan when cumulation is on.

        The empty plan was a deliberate choice — it avoided the "spurious" tool
        calls an LLM planner derives from the primary domain. In production
        (2026-07-27) that cost the user the feature they asked for: an image
        request matched ``skill-generator``, the empty plan dropped
        ``generate_image`` (semantic score 1.0, already selected by the router)
        and the sub-agent got only the four skill tools. No image, four times
        out of six.

        Yielding here lets the LLM planner emit the domain's native steps;
        ``response_node`` step 3 still activates the detected skill from
        ``query_intelligence``, independently of the plan. Both run — which is
        what the domain tools were selected for in the first place.
        """
        strategy = SkillBypassStrategy()
        intel = _make_intelligence(
            detected_skill_name="interactive-map",
            domains=["place"],
        ...
        assert result.success is False
        assert result.plan is None
        assert "cumulat" in (result.error or "").lower()

    @pytest.mark.unit
    async def test_script_only_skill_builds_empty_noop_plan_when_cumulation_disabled(self):
        """The historical behaviour stays reachable through configuration.

        Reverting is a one-line env change if the native steps turn out to be
        noise for a given deployment.
        """
        strategy = SkillBypassStrategy()
        intel = _make_intelligence(
            detected_skill_name="interactive-map",
        ...
        with patch(
            "src.domains.agents.services.planner.strategies.skill_bypass.get_settings"
        ) as mock_settings:
            mock_settings.return_value.skill_script_only_cumulates_native_plan = False
            result = await strategy.plan(intelligence=intel, config=config)

        assert result.success is True
        assert result.plan is not None
        assert len(result.plan.steps) == 0  # empty noop plan
        assert result.plan.metadata["skill_name"] == "interactive-map"
        assert result.plan.metadata["skill_bypass"] is True
        assert result.plan.metadata.get("skill_bypass_noop") is True

    @pytest.mark.unit
    async def test_deterministic_skill_is_unaffected_by_cumulation(self):
        """Cumulation only concerns the script-only branch.

        A deterministic ``plan_template`` already declares its own steps, so it
        keeps bypassing the LLM planner exactly as before.
        """
        strategy = SkillBypassStrategy()
        intel = _make_intelligence(
    ...
    @pytest.mark.unit
    async def test_script_only_skill_not_active_returns_failure(self):
        """Script-only skill that the user hasn't enabled must NOT bypass."""
        strategy = SkillBypassStrategy()
        intel = _make_intelligence(detected_skill_name="interactive-map")
        config = _make_config()

        with patch("src.core.context.active_skills_ctx") as ctx:
            ctx.get.return_value = {"other-skill"}  # interactive-map inactive
            result = await strategy.plan(intelligence=intel, config=config)
