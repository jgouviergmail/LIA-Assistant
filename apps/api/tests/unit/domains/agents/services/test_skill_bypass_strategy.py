...
class TestPlan:
    ...
    @pytest.mark.unit
    async def test_script_only_skill_builds_empty_noop_plan(self):
        """Script-only skills (scripts + no deterministic plan_template) must
        bypass the LLM planner with an empty plan so the ReactSubAgentRunner
        can execute the script end-to-end without triggering spurious tool
        calls (e.g., Google Places) generated by the LLM planner."""
        strategy = SkillBypassStrategy()
