...
class TestProfilingBaseline:
    ...
    async def test_context_store_cleanup_profiling(self):
        """
        Trigger cleanup_session_contexts() to collect profiling data.
        ...
        Exercises the profiled path on a populated in-memory store.
        """
        ...
        store = InMemoryStore()
        user_id, session_id = "profiling_user", "profiling_session"
        for domain in range(5):
            namespace = (user_id, session_id, "context", f"domain_{domain}")
        ...
        assert result["domains_cleaned"] == 5
