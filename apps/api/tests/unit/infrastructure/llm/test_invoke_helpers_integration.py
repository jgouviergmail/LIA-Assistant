...
# F006: these are component-integration tests that use only in-process mocks (no
# Redis/DB/network), so the `integration` marker was wrong — it excluded them from
# BOTH the unit job (which drops `integration`) and the integration job (which
# collects tests/integration/ only), leaving them run by NO job. They are plain
# unit tests and now run in the unit job.
