...
def _isolate_http_log_env(monkeypatch):
    """Neutralise any inherited HTTP_LOG_* env var for the whole module (F043).

    These tests are order/worker/environment sensitive under pytest-xdist: a
    developer shell or CI that exports HTTP_LOG_LEVEL / HTTP_LOG_EXCLUDE_PATHS
    (both live in the committed .env) would make the default-asserting tests read
    that value instead of the code default. Deleting them here restores a clean
    baseline before each test; tests that need a specific value set it explicitly.
    """
    monkeypatch.delenv("HTTP_LOG_LEVEL", raising=False)
    monkeypatch.delenv("HTTP_LOG_EXCLUDE_PATHS", raising=False)


def _default_settings() -> Settings:
    """Build a Settings that reflects code defaults only — no env var, no .env file.

    ``_env_file=None`` disables the ``.env`` lookup (which ships HTTP_LOG_*), and
    the autouse fixture has already cleared the matching os.environ keys, so the
    HTTP_LOG_* fields resolve to their in-code defaults deterministically.
    """
...
def test_excluded_path_not_logged():
    ...
    # The middleware reads the module-level `settings` at REQUEST time (not at
    # add_middleware time), so the patch must stay active around the request —
    # otherwise it reads the real global settings and the test depends on the
    # ambient HTTP_LOG_* environment (audit F043).
...
def test_constants_used_correctly():
    ...
    # Verify Settings uses these constants (defaults only — env/.env neutralised)
    settings = _default_settings()
    assert settings.http_log_level == HTTP_LOG_LEVEL_DEFAULT
    assert settings.http_log_exclude_paths == HTTP_LOG_EXCLUDE_PATHS_DEFAULT
