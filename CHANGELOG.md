...
- **Facts corrected on every public surface (×6 locales + README + guides + llms.txt):** 7 LLM providers (title, names, meta descriptions, FAQ, admin-screenshot captions); 76 tools; 394 Prometheus metrics; 22 Grafana dashboards; 54 LLM config types (was "34"); 14 personalities; 100+ ADRs (README said 81); ~10,140 backend tests across 555 files; 169 vitest tests (was "41"); FAQ 218 Q/A; embeddings documented as Gemini `gemini-embedding-001` (ADR-069) instead of OpenAI; "100% local voice mode" reworded to local wake-word/STT (TTS is cloud); "zero API cost" hybrid-search claim removed; README stack table refreshed (12 stale versions); performance tables reframed honestly (TTFT row → first SSE event + LLM-cascade note in README and the six how guides).
