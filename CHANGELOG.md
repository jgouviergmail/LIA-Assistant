...
- **`PSYCHE_DOMINANCE_CENTER`** (default `0.0` = inert): post-damping translation of the dominance resting point, threaded through all 6 baseline call sites. The activation candidate `0.20` is *derived* from the measured catalogue mean (+0.216), not guessed; at that value 7/14 personalities rest D < 0 with the personality ordering exactly preserved and P/A untouched.
...
- **Actionable briefing cards (QW-9)** — mail, agenda, birthday and reminder items become real, labelled `<button>`s (the accessible name IS the intent) that open the chat prefilled through the onboarding `?draft=` deep-link, generalized (`chatDraftHref`): *"Résume-moi le mail « {sujet} » de {expéditeur} et propose une réponse"*, *"Prépare-moi pour « {titre} » de {heure}"*, *"Prépare un message d'anniversaire pour {nom}"* — reminders open the chat plainly. The consumed `?draft=` is stripped from the URL (`history.replaceState`), which also fixes the onboarding's latent re-prefill on F5.
...
- **Host-owned widget sentinels (ADR-137)** — `display/sentinel_filter.py` strips LLM-authored sentinel copies with `html.parser` (never a regex: sentinels nest `<div>`s) at three chokepoints: before canonical injection, in the ReAct history window (replaced by a placeholder — the imitation *prompt* disappears and the tokens come back), and on the ReAct passthrough. `widget_sentinels_stripped_total{source}` tells whether the model still imitates instead of assuming it. Invariant pinned by test: N interactive widgets ⇔ N sentinels, proven non-vacuous by re-injecting the production duplicate.
...
- **`frame.link_url` in the `SkillScriptOutput` contract** — the *user-facing* URL for the fallback card, https-enforced like `url`. The Google Maps embed endpoint answers *"must be used in an iframe"* when opened top-level; `render_map.py` now ships `google.com/maps?q=…` alongside it, and both fallbacks prefer it.
- **QUERY location phrases in both mirrors (6 languages)** — the frontend detector had no "where am I" category at all (backend had it, minus the imperative forms), so *"montre-moi où je suis"* never triggered the just-in-time geolocation permission prompt. Both tables gain the imperative forms (`"où je suis"`, `"ma position"`, `"where i am"`, `"wo ich bin"`), and `GeolocationPrompt` reacts to the new category.
- **MCP-domain guard** (`services/analysis/skill_suppression.py`) — the analyzer LLM fills `domains` and `skill_name` in one output with no coherence guarantee, and the routing decider gave the skill absolute priority. The guard suppresses the detection when the *primary* domain is an MCP domain, reusing the exact ADR-118 dialogue exemption the chat override applies (a mid-dialogue conversational answer keeps its dialogue skill; a fresh imperative request does not). Every suppression is logged and counted (`skill_detection_suppressed_total{reason}`).

### Fixed

- **Interactive map blank on every iOS browser** — `require-corp` + no `credentialless` in WebKit; verbatim engine message: *"Cancelled load … because it violates the resource's Cross-Origin-Resource-Policy response header"*. Root-caused on a Playwright bench with production headers replicated, engine × header matrix measured.
...
- **`resolve_location` silently dropped `LocationType.QUERY`** — the match had no branch for it, so "où suis-je" fell through to the "unreachable" `(None, None)`: no location *and* no user-facing hint.
...
- **"Route to my brother" intermittently geocoding the person's name** — the exact failure chain (memory resolved the person but the analyzer omitted the person typing → no contact expansion → `get_route(destination="<person name>")` → arbitrary geocode → wrong route **cached 300s**) is closed at all three levels: deterministic evidence (above), the runtime guard (above), and `get_route` now returning a recoverable `destination_unresolved` error when a non-address destination that Places cannot find would otherwise be passed raw to the Routes API — nothing is cached on that path, and the error message steers the model to fetch the real address first. A Places *API failure* still passes through (an outage is not evidence the destination is bad).
...
- **S1 — path traversal at skill import (critical)** — the frontmatter `name` was concatenated into the destination path without validation (`name: ../../system/x` overwrote a *system* skill's SKILL.md for all users). Strict agentskills.io name validation now runs before any filesystem write, on every import path.
...
- **SSE endpoint demoted to subscriber (ADR-117, flag on)** — `POST /agents/chat/stream` no longer executes the graph in the response generator: it spawns the detached producer, then serves the Redis Stream. Client disconnect stops the *subscription*, never the *generation*. Flag off = strictly the legacy path.
...
- **Dependency refresh** (already merged on `main`, released here): redis-py 7.4.0 → 8.0.1 (server stays 7.4; typing fallout fixed at the source, three obsolete `cast(Awaitable)` workarounds removed), vite 7.3.5 → 8.1.3 + @vitejs/plugin-react 6 (vitest engine only; 200/200 vitest green), 40 frontend minor/patch bumps (Radix UI line, Dependabot #178), and five backend transitives via a surgical ADR-112 lock recompile (attrs 26.1, packaging 26.2, icalendar 7.2, rpds-py 2026.6.3, google-genai 2.10). Stack tables across README/guides/GUIDE_* aligned with the actual manifests (Node 24 sweep).
