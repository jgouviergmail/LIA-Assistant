...
- **L'image de production se construit à nouveau** — `deb.nodesource.com` répond désormais **403**. `curl -f` échouait bien, mais `| bash -` lisait une entrée vide et **sortait en 0** : la couche réussissait, apt résolvait alors `nodejs` depuis Debian (18.x, qui n'embarque pas npm), et la construction mourait quatre couches plus loin sur `npm: not found`. Node est désormais installé depuis l'archive **officielle** de nodejs.org, en tenant compte de l'architecture pour que la construction multi-plateformes de la release continue de fonctionner — et `node -v && npm -v` s'exécute dans la **même** couche, si bien qu'une prochaine rupture amont fera échouer la construction au bon endroit au lieu de dégrader silencieusement en un Node sans npm.
