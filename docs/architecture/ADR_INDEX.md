...
- Historique : la révocation par connecteur décrite dans cet ADR a été amendée par [ADR-302](ADR-302-OAuth-Grant-Par-Compte-Et-Consentement-Groupe.md) ; une déconnexion unitaire ne révoque plus l'autorisation fournisseur.
...
**Amendement 2026-09-17** : un échec d'indexation porte un **code** (`rag_documents.error_code`, vocabulaire clos `RAGDocumentErrorCode`) traduit à l'affichage sous la ligne du document ; un PDF scanné sans couche texte est nommé comme tel (`scanned_pdf_no_text_layer`, aucune reconnaissance de caractères dans le pipeline) avec son remède (Google Drive → Google Docs → source Drive de l'espace), au lieu du « No text content extracted » brut en info-bulle (ADR-184 appliquée).
---
...
**Status**: ✅ IMPLEMENTED (2026-03-24) — amendé le 2026-09-23 (les écrivains de tarifs LLM publient enfin l'invalidation ; un worker démarre depuis la base, jamais depuis le blob)
 ...
 **Amendement 2026-09-20** : troisième forme de refus manquée par le filet — un appel dont les ARGUMENTS ne sont pas du JSON valide (`invalid_tool_calls`, `tool_calls` vide) était lu « no tool call, empty answer » ; mesuré sur `deepseek-flash` (`"primary_domain": mcp`, mot nu), **4 analyses de requête sur 26 perdues en 48 h de prod**, chacune un tour dégradé en conversation. `quote_bare_values` cite le mot nu en position de valeur (ADR-184, après le verdict ADR-275), la raison nomme le verdict du parseur, `raw_output` porte les arguments refusés, et `llm_structured_output_outcomes_total` (dashboard 05) compte chaque issue. Rejeu : 24/24 réglées, 14 réparées.
---
...
**Décision**: La directive du mode `html` n'exploitait qu'une fraction des capacités en place (callouts success/error stylés mais non documentés ; `details`/`dl`/`kbd` autorisés mais jamais proposés ; aucune classe `language-*` donc jamais de Prism). **Retenu — enrichissement purement déclaratif (prompt + CSS + allowlist), aucun nouveau chemin de code** : la directive documente 7 composants stylés `.lia-response` (callouts ×4 + titre, chips + icônes Material Symbols, `details.lia-collapsible`, `dl.lia-kv`, `div.lia-columns`, `ol.lia-steps`, `div.lia-stats`, code `language-*` → CodeBlock) + accents `mark`/`kbd`/`abbr`, avec règle de sobriété (2-3 composants max) et budget ≤96 lignes — **règle de sobriété RETIRÉE le 2026-09-17 (amendement b)** : le mode HTML demande une page COMPOSÉE (accroche, une section `<h2>` par facette dans son composant, callout de clôture), la forme suit les données de la réponse et jamais celle des réponses précédentes ; seule une salutation ou une réponse d'une phrase reste un `<p>`. Schéma de sanitisation étendu de **6 tags inertes** (`mark`, `caption`, `abbr`, `time`, `figure`, `figcaption`) — rien de scriptable, ordre des plugins et interdits (`script`/`iframe`/`form`/handlers) inchangés, pinné dans les deux sens. **Garde de sync directive↔CSS** (`test_html_directive_css_sync.py`) : une classe `lia-*` citée mais non stylée fait échouer CI (doctrine ADR-085 appliquée au couple prompt/CSS). Overrides `ol`/`ul` de MarkdownContent préservent désormais les classes `lia-*` (contrat `p`/`a`). Aplatissement client partagé `html-plain-text.ts` (miroir de `html_to_text` backend) → copie double-flavor, partage natif, export `.md` ; ligatures d'icônes exclues du surlignage de recherche (bug latent). Gate TTS **amendée le 2026-09-17** : la directive suit l'écoute, pas le type de tour — `route_to == "planner" or not voice_enabled`, la préférence vocale (`users.voice_enabled`, celle que lit le coordinateur vocal) portée par `LiaRuntimeContext.voice_enabled` ; un compte `html` sans voix reçoit la mise en page HTML sur ses tours conversationnels aussi. **Limite connue mesurée** : sur tour skill, le LLM de synthèse ignore parfois la directive (pré-existant, non aggravé). **Alternatives écartées** : composants React interceptés (complexité streaming/a11y — différé) ; ne pas étendre le schéma (dégradation en texte nu pour un coût d'extension quasi nul) ; `<progress>`/`<meter>` natifs (stylage cross-browser pénible, YAGNI).
...
**Amendement (2026-09-05, ADR-267)** : Ollama quitte `excluded` pour `native` — un appel diffusé sans usage déclenchait `LLMCallsWithoutUsage` à chaque tour pour une dépense qui existe à 0 € ; le client natif porte l'usage sur chaque réponse. Perplexity reste `excluded` (clé de l'utilisateur final).
...
**Amendement (2026-09-05)** : la couture couvrait cinq branches sur sept — Ollama et Perplexity passaient l'objet `ReasoningIntent` brut à `ChatOpenAI` (prod : le slot `response` sur Ollama mourait à l'instanciation à chaque tour). Les deux branches appellent `kwargs_for`, une garde parcourt chaque membre de `ProviderType` × chaque niveau (`test_reasoning_seam_guard.py`), `extra_body` est fusionné, jamais assigné, et la famille `ollama` naît avec une échelle déclarée par le serveur à la découverte (ADR-267).
...
**Amendement (2026-09-11)** : le compteur ne repartait jamais de zéro. `react_tool_seconds` est débité en `précédent + dépensé` et restitué par le point de reprise ; la remise à zéro de début de tour, une liste de clés tenue à la main dans `router_node_v3` et écrite avant cet ADR, ne le nommait pas. Mesuré en production : 77 s → 913,8 s en six jours d'usage ordinaire, puis chaque tour ReAct du fil arrêté à l'itération 1 (`tool_budget`), `create_reminder_tool` et `create_event_tool` abandonnés — trois « ko », aucune erreur. `react_productive_iterations` (ADR-248) portait le même oubli et gonflait le budget adaptatif au plafond. Ce qu'un tour commence avec est désormais UNE déclaration, `react_turn_reset()` dans `utils/react_budget.py`, dépliée par le routeur et par `create_initial_state` ; un garde AST refuse toute clé lue par le prédicat d'arrêt ou l'arête de routage qu'elle ne nomme pas. Aucune migration : le premier tour après déploiement repart de zéro dans chaque fil, routines et tickets compris.
---
...
### ADR-258 : enregistrement de réunions et comptes rendus structurés

**Statut**: ✅ ACTÉ (2026-09-02)
**Fichier**: `docs/architecture/ADR-258-Meeting-Recording-And-Structured-Minutes.md`

**Décision** : depuis le trombone du composeur, l'utilisateur enregistre une réunion (téléphone ou ordinateur) et LIA rend le compte rendu — date, heures, lieu, participants, titre, résumé, sujets, actions et échéances, risques — par notification, e-mail et PDF, indexé dans l'espace « Réunions ». Chaque hypothèse a été **mesurée** avant conception : le Whisper local tronquait tout au-delà de ~30 s (corrigé par des fenêtres VAD Silero ≤ 20 s, jamais une troncature, qui répare aussi la saisie vocale) ; ElevenLabs Scribe et OpenAI acceptent un fichier entier et séparent les interlocuteurs (7/7 chacun) ; Safari produisait du MP4 et les PWA iOS avaient un défaut `MediaRecorder` — d'où **deux sources** derrière une interface (Opus par `MediaRecorder`, PCM par le worklet partagé avec la saisie vocale, PCM imposé sur Apple). Le client envoie des **segments** ordonnés, rejoués avec repli et **jamais abandonnés** tant que l'enregistrement dure (hors-ligne = attente) ; un fichier par séquence (quatre workers), écriture atomique, lacunes = listage. **La ligne `meetings` est le job durable** (ADR-129) : claim atomique, bail, heartbeat publiant l'étape, moissonneurs bornés ; un refus qui n'est pas une panne (limite d'usage, aucun moteur) ne consomme pas de tentative. **La chaîne de moteurs est décidée avant la première seconde** (slot admin → repli ElevenLabs/OpenAI → local sans plafond), les codes permanents (clé invalide — l'instance dev stockait un ID de clé) mettent en échec sans réessai. **Le compte rendu est une donnée structurée dont le modèle de l'utilisateur est le contrat** : un appel structuré sur le slot dédié `meeting_synthesis` (condensation par parties si la fenêtre déborde), `repair_report` replie la réponse permissive dans le schéma strict, `report_generated` immuable / `report_current` édité, transcription chiffrée ; UN sérialiseur produit le Markdown indexé, le PDF (ADR-226) et l'e-mail ; l'espace « Réunions » est trouvé par **rôle** (`rag_spaces.kind`), un document RAG par réunion réécrit en place. Bornes publiées, comptes exacts (ADR-185), lacunes dites (`audio_gaps`), capacité plateforme `MEETINGS` (ADR-229). Écartés : transcription en direct, second bac à sable, choix de modèle par réunion (bibliothèque annoncée — `template_snapshot` et `is_default` la préparent).

**Amendement (2026-09-05)** : une réunion n'est jamais perdue et son propriétaire jamais bloqué. Une transition écrite par `case()` avec un membre d'enum nu était liée en `NullType` (la VALEUR partait en base où la colonne stocke le NOM, le `RETURNING` échouait, la transaction était annulée : une réunion `processing` re-drivée toutes les 15 minutes en production) — les littéraux portent le type de la colonne, une garde AST refuse tout `case()` nu et chaque transition du repository est prouvée sur PostgreSQL réel. Le job écrit un **point de contrôle** par étape acquise (audio normalisé, puis transcription chiffrée) dans l'instruction de renouvellement du bail et reprend depuis là ; la rétention ne purge que l'audio des réunions `ready` ; un bail expiré est remis en file ou classé `worker_lost` par le reaper, et la réunion est supprimable entre-temps ; le détail publie `attempts`, `max_attempts`, `worker_stale`. Côté modèle, un `null` sur une liste à défaut est « aucun » (formes modèle tolérantes, sauvetage générique des appels d'outil dans `llm/tool_call_rescue.py`, `parsing_error` lu et nommé).

### ADR-259 : bibliothèque de modèles de compte rendu, sélection automatique et changement de format

**Statut**: ✅ ACTÉ (2026-09-03)
**Fichier**: `docs/architecture/ADR-259-Meeting-Template-Library-And-Reformatting.md`

**Décision** : le modèle unique d'ADR-258 devient une **bibliothèque** — 30 modèles intégrés en sept catégories (code + mots en donnée i18n, complétude assertée au démarrage) et les modèles de l'utilisateur, chacun désigné par un **`TemplateRef`** (`builtin:<clé>` / `user:<uuid>`) que réunions, préférences et requêtes échangent à la place d'une ligne. **Une cinquième sorte de section, `transcript`**, réécrit l'échange par parties sous le budget de sortie effectif du slot (lu en base : `max_tokens=8000` par défaut, surchargeable) — jamais choisie automatiquement, car longue et facturée comme une réunion entière. **Le format est choisi une fois, à un seul endroit** (ref de la réunion → défaut de la préférence → choix du modèle sur un extrait, au-dessus d'un seuil → intégré par défaut), compté et **écrit sur la réunion** avec sa raison ; la régénération lit le ref DE LA RÉUNION, plus le défaut courant de l'utilisateur. **Changer le format** réécrit depuis la transcription : en place, ou en **nouveau compte rendu** pointant sa source et indexé comme son propre document — jamais une « copie ». Le courriel part de `APPLICATION_SMTP_FROM` (échange SMTP hors boucle d'événements), le connecteur et son refus disparaissent. L'enregistrement quitte le « + » du composeur : bascule dans l'en-tête, entrée dynamique du menu du logo qui pulse en rouge, bannière collante qui publie sa hauteur (la coque du chat l'ignorait, mesuré). Espaces de connaissances : **un seul constructeur de chemin** et **une seule vérification de propriété** ; téléchargement, archive zip de la sélection, déplacement qui met à jour la ligne ET ses morceaux (`rag_chunks.space_id` est dénormalisé) puis déplace le fichier, suppression groupée — chaque lot rapporte chaque id, fait ou ignoré avec sa raison. Quatre défauts d'ADR-258 fermés en passant : la régénération qui restait bloquée après une mort brutale, la hauteur du bandeau, le SMTP synchrone, l'absence de sorte pour une transcription.

### ADR-260 : les familles de clés Redis déclarent leur portée ; le reset purge par famille

**Statut**: ✅ ACTÉ (2026-09-03)
**Fichier**: `docs/architecture/ADR-260-Redis-Key-Families-Scope-And-Reset-Purge.md`

**Décision** : l'audit de trois boucles silencieuses (push Google, habitudes, propositions) a trouvé une cause de plomberie commune : `reset_conversation` supprimait toute clé Redis `*:{user_id}*` — écrit quand Redis ne contenait que des caches, jamais relu après qu'ADR-140, ADR-214, ADR-232 et l'ancre Gmail y ont rangé de l'**apprentissage**. Mesuré sur le compte principal : 161 resets en 56 jours, chacun effaçant le ledger de récurrence (20/20 clés), l'ancre history.list, le seuil adaptatif et les dernières bonnes valeurs du briefing, et supprimant au passage une clé de limitation de débit et le registre SSE ; Loki montre le seed du ledger — qui ne tourne que sur ledger vide — 39 secondes après trois resets, tandis que le ledger d'un compte qui ne resète jamais survivait 13 jours dans le même Redis. Un verrou exige 14 jours distincts : aucune proposition ne pouvait exister. **(1) Chaque famille de clés déclare sa portée** dans `infrastructure/cache/key_families.py` — `CONVERSATION`, `USER_CACHE`, `USER_LEARNING`, `USER_RUNTIME`, `GLOBAL` — par plus long préfixe (`briefing:v2:lastgood` bat `briefing:v2`). **(2) Le reset purge par famille, jamais par motif seul** : les six motifs de SCAN historiques sont conservés (aucune clé n'échappe au balayage), mais une clé n'est supprimée que si sa famille est conversation ou cache ; apprentissage et runtime sont conservés et comptés, et **une famille non déclarée n'est jamais purgée** — elle est comptée (`reset_undeclared_family_total`), parce que la suppression silencieuse est exactement ce qui a tué l'apprentissage. **(3) Deux gardes rendent le registre complet par construction** : l'assert de démarrage sur les constantes `core.constants`, et le balayage du code source pour les clés construites par f-string — au premier passage ils ont révélé trois familles de plus non déclarées (`hitl_rate_limit`, `bm25`, `health_metrics_ingest`). **(4) Les surfaces d'oubli possèdent les clés d'apprentissage** : la suppression de compte efface toute famille déclarée utilisateur par le même balayage, et « Tout oublier » efface le ledger qu'il laissait derrière lui. Rejeté : une deny-list (une nouvelle famille d'apprentissage y serait purgée par défaut), renommer les clés pour échapper aux motifs (une convention qu'aucune garde ne vérifie), retirer la purge du reset (HITL et caches appartiennent bien à la conversation oubliée).

### ADR-261 : réveil du heartbeat par le push et synchronisation Drive incrémentale

**Statut**: ✅ ACTÉ (2026-09-03)
**Fichier**: `docs/architecture/ADR-261-Push-Driven-Heartbeat-Wake-And-Incremental-Drive-Sync.md`

**Décision** : les canaux push Google du lot H étaient vivants (802 notifications Gmail traitées en 15 jours, 0 jeton refusé) et fonctionnellement inertes — leur seul consommateur invalidait des caches. **(1)** Une notification traitée **met l'utilisateur en file** (`SET NX` par couple utilisateur/fournisseur : une tempête est UN réveil, daté du premier) et le webhook répond 200 sans décider. **(2)** Un **balayage court, élu leader**, sert la file **sous l'EligibilityChecker complet** — péremption, cooldown de réveil, préférence de source de l'utilisateur, delta frais, pré-filtre déterministe — puis exécute la tâche heartbeat pour cet utilisateur seul, via le runner existant ; le runner gagne `user_ids` et `skip_probabilistic_gate` : un réveil répond à un événement, seul le lissage « minimum garanti » est contourné (mesuré : sans cela, un réveil légitime était refusé au hasard). **(3)** Le delta Gmail est **prévisualisé, jamais consommé** tant que le réveil n'est pas servi ; les deux ancres restent distinctes (dernier événement vu ≠ dernier courrier consommé), et l'ancre du heartbeat est une clé d'apprentissage (ADR-260). **(4)** Le pré-filtre est déterministe et publié (`PUSH_WAKE_*`) : libellé `IMPORTANT`, catégories exclues, courrier de liste exclu ; invitation d'un tiers ou réponse en attente dans l'horizon ; pas de règle « expéditeur favori » faute d'adresse dans les relations (mesuré). **(5)** La décision sait pourquoi elle est réveillée (ligne FRESH) et la ligne d'audit persiste `trigger = push | tick`. **(6)** Un push Drive n'est pas une décision : il **réindexe exactement ce qui a changé** dans les dossiers liés, sous le verrou de la synchro manuelle, par l'ingestion par fichier extraite de `drive_sync.py` en un seul module partagé. **(7)** Un agenda modifié invalide le conseil de départ. Rejetés : décider dans le webhook, un chemin « urgent » hors budget, fusionner les ancres, un pré-filtre LLM.

### ADR-262 : une source RAG « e-mail » choisie par libellé Gmail

**Date** : 2026-09-03 · **Statut** : Accepté
**Fichier**: `docs/architecture/ADR-262-Opt-In-Mail-Label-RAG-Source.md`

**Décision** : historiser toute la boîte aux lettres a été écarté (une seconde archive personnelle à protéger, un coût d'embedding proportionnel à du bruit, une recherche dégradée par un corpus non choisi). **(1)** L'opt-in EST le libellé : un espace suit un libellé Gmail, seuls les fils qui le portent sont indexés, et retirer le libellé dans Gmail retire le document — le geste de l'utilisateur est la suppression. Drapeau `RAG_SPACES_MAIL_SYNC_ENABLED`, **OFF par défaut**. **(2)** Un fil = un document Markdown : rendu pur, messages dans l'ordre, texte brut préféré (HTML converti par l'extracteur du client Gmail, jamais un second analyseur), **noms de pièces jointes seulement**, taille bornée ; le nom affiché est le SUJET, jamais un participant. **(3)** Deux entrées, une seule ingestion (`ingest_thread`), et l'étape « octets → document PENDING » est celle que partagent toutes les sources synchronisées (`drive_ingest`). **(4)** La synchro complète **ancre l'historique AVANT de lister** : un message arrivé pendant le listage est rejoué au passage suivant (où il est gratuit) au lieu de tomber dans le trou. **(5)** Le chemin incrémental monte sur le réveil d'ADR-261 et **ne répond à aucune porte** : indexer n'est pas décider — pas de cooldown, pas de quota, une métrique à soi, et un échec ne coûte jamais le réveil ; une ancre expirée retombe sur une synchro complète, jamais sur le silence. **(6)** Une source synchronisée est un job durable, quelle qu'en soit l'espèce : les trois jobs du dépôt prennent la TABLE en paramètre, validée contre une liste blanche de deux noms. **(7)** Un compteur affiché est exact (`COUNT(*)` des documents READY). Rejetés : la boîte entière, une recherche enregistrée au lieu d'un libellé, un cache Redis des fils, le texte intégral en base.

### ADR-263 : chaîne d'autorité d'exécution et registre des effets

**Date** : 2026-09-04 · **Statut** : Accepté
**Fichier**: `docs/architecture/ADR-263-Execution-Authority-Chain-And-Effect-Register.md`

**Décision** : LIA décidait bien et agissait bien, mais personne ne pouvait vérifier APRÈS COUP ce qu'elle avait fait sans croire celui qui l'avait exécuté. Trois mesures : 13 outils mutateurs sans aucune porte de confirmation dans les deux modes, un brouillon confirmé exécuté DEUX fois, un sous-agent « lecture seule » protégé par une liste manuelle de 17 noms qu'il manquait quatre mutateurs. **(1)** Une capacité **déclare** ce qu'elle doit à l'utilisateur (`mutation_policy` : `read|draft|confirm|reversible|artefact|sandboxed`), assert au boot ; seule la catégorie `search` est exemptée, parce que `readonly` est le REPLI de l'inférence et que c'est là que se trouvent les outils dangereux. `hitl_required` répond à une AUTRE question (le coût, pas l'effet). **(2)** Un effet est **réclamé avant d'arriver** et **clos depuis un résultat explicite**, dans sa propre transaction — un e-mail n'est pas transactionnel ; clé unique `(thread_id, idempotency_key)`, résultat conservé CHIFFRÉ pour servir un rejeu. **(3)** La porte est installée sur la CAPACITÉ à l'enregistrement, jamais demandée aux appelants ; les adaptateurs MCP se gardent EUX-MÊMES dans `_arun`, où se rejoignent leurs **trois** portes d'appel. **(4)** Un effet non confirmé **demande au lieu d'échouer** (`DraftType.TOOL_CALL`, toute la machinerie de brouillon réutilisée) ; un tour non surveillé est refusé — un brouillon que personne ne peut confirmer est une promesse que personne ne tient. **(5)** Le registre consigne ce qu'une CAPACITÉ a fait dans un tour, pas toute écriture en base : la frontière est écrite. **(6)** Toute la surface de preuve **LIT** le registre (carte sous la bulle, panneau debug, journal des actions, export de compte, surfaces admin) ; rien n'écrit — un exécuteur qui peut corriger son propre dossier vide le registre de son sens. **(7)** Deux registres : lisible (celui de l'utilisateur, rendu dans SA langue, masqué côté admin avec dévoilement journalisé) et technique (pseudonymisé PAR CONSTRUCTION, liste blanche de colonnes, identifiants en HMAC). Coût mesuré : **0,64 µs et zéro session** sur une lecture. Rejetés : l'approbation au niveau du plan, un graphe de provenance hachant tout, la suppression de l'interaction `tool_confirmation` (vivante : c'est la carte de ReAct). **(8, lot 5)** Un registre que personne ne peut vérifier est un registre qu'il faut croire : chaque compte reçoit sa **propre chaîne de hachage** (`ledger_chain`). Per COMPTE et non globale — c'est la décision dont tout dépend : supprimer un compte retire une chaîne ENTIÈRE au lieu de percer un trou définitif dans celle des autres, et inaltérabilité et effacement cessent d'être un arbitrage. **Asynchrone sur une mesure** (6,0 ms contre 0,21 ms, ×28 sur le chemin critique) : la fenêtre qui en découle est publiée, mesurée et alertée, jamais tue. **Deux étapes pour une action, une pour une consultation** — une seule empreinte prise à la réclamation ferait passer chaque clôture légitime pour une altération. Ce que la chaîne NE prouve PAS est écrit noir sur blanc : ni la vérité d'une ligne au moment de son écriture, ni la fenêtre, ni un réécrivain qui refait la ligne ET son entrée ET toutes les suivantes — contre celui-là, la seule défense est l'empreinte finale montrée à l'utilisateur et emportée dans son archive. **Rien ne répare une chaîne** : un outil de réparation servirait l'attaquant aussi bien que l'exploitant. **(9, lot 6)** Un TROISIÈME registre, `agent_decisions` : une ligne par TOUR, la colonne vertébrale à laquelle les deux autres se rattachent. Il **pointe** vers la demande et la réponse et n'en copie rien — les mots restent là où ils sont déjà purgés avec le compte, et une suppression de conversation laisse une pierre tombale datée. Une reprise HITL est le MÊME tour : l'écriture fusionne (début le plus ancien, durée CUMULÉE, `segments` qui compte les reprises), parce qu'écraser en silence rendrait un tour interrompu identique à un tour direct. L'issue est DÉDUITE par le gestionnaire de contexte — `interrupted` tant que rien n'a dit mieux, jamais demandée aux appelants : il y a toujours une sortie de plus que quelqu'un oublie. Les trois registres ne s'additionnent toujours pas. Amendement du 2026-09-23 (§23) : ADR-231 avait déplacé l'id du tour dans `metadata`, et la portée des effets le lisait encore dans `configurable` — chaque action directe était classée sous le FIL, la carte « actions menées » manquait, et la clé d'étape pipeline `{fil}:step:step_1` entrait en collision d'un tour à l'autre (production, 2026-09-22 : un rappel servi depuis le registre au lieu d'être créé). Un seul lecteur, `core/run_config.run_id_of`, gardé par AST ; et en mode ReAct les effets réussis du registre sont énoncés comme les actes du modèle lui-même, dans une directive à part (`response_directive_performed_actions`) — mesuré : 6 dénis d'image sur 6 sans rien ; écrits en ligne de données, 0 déni mais 3 « deux images » inventées sur 100 ; en directive, 0 et 0 sur 100, et 0 et 0 sur 100 avec une simple légende. Un libellé d'action se coupe au mot, ellipse comprise (`core/text_clip`).

### ADR-264 : des sourcils et une bouche vivants — une respiration, des organes couplés, un budget en pixels

**Date** : 2026-09-05 · **Statut** : Accepté
**Fichier**: `docs/architecture/ADR-264-Living-Brows-And-Mouth.md`

**Décision** : mesuré avec le rig lui-même avant tout changement (30 px, 60 s de repos), la bouche bougeait de 0,47 px et les sourcils de 0 px ; le sourcil était une BARRE (hauteur, inclinaison, présence) absente au repos sur 10 humeurs sur 14, et le geste nommé `brow` levait l'ŒIL. **(1)** Le sourcil a un ARC (`browArc`, canal `pose`, sans unité), dessiné comme la bouche : un élément, une bordure, borné dans la feuille. **(2)** Il est PRÉSENT au repos, faible (0,5), et se relâche endormi sans disparaître — renverse « invisible on a neutral face » d'ADR-252 (arbitrage propriétaire). **(3)** UNE respiration traverse tout le visage : les sourcils et la largeur de bouche montent sur la respiration existante, même période que la masse, une fraction de tour derrière ; aucune dérive propre aux sourcils, un sourcil qui erre est un tremblement. **(4)** Les couplages secondaires (le regard vers le haut lève les sourcils, un clignement entraîne chaque sourcil avec SA paupière) vivent dans le RIG, pas dans la feuille : un couplage est du mouvement, et une constante dans un `calc()` est illisible par les tests et par la garde de la bulle. Le piège ouvert est fermé par construction : contributions ABSOLUES depuis le ressort et les boucles, et un test compare 20 000 pas de 16 ms à un seul pas sur trois états de repos. **(5)** Les beats atteignent la bouche : `brow` lève enfin le SOURCIL, trois gestes propres (`lip-press`, `corner-tug` avec le sourcil du même côté, `brow-twitch`), le soupir resserre la bouche, le bond l'entraîne en retard, `thinking` mâchonne, un dormeur respire par la bouche ; et les beats des YEUX portent le visage (retour propriétaire sur le widget en marche : les yeux jouaient toutes les quelques secondes, bouche et sourcils ne faisaient que respirer) — `perk` lève et arque les sourcils, `squint` les fronce et resserre la bouche, `tilt` sourit d'un coin ; environ un tiers des tirages atteint le visage (30 % calme, 40 % vif, épinglé), chaque beat joué à une taille tirée au sort dans [0,85 ; 1,15] sur les bandes RELATIVES seulement (une fermeture de paupière est un fait). **(5bis)** Le visage a une VIE PROPRE (`rig/life.ts`) : neuf mini-scènes CARTOON où tout le visage s'engage (grand sourire qui plisse les yeux en arcs, « oh » bouche en O et sourcils au ciel, moue boudeuse, « hmm » un œil plissé et tête penchée, sourire en coin l'autre œil mi-clos, bisou, petit rire à deux rebonds, wiggle, claquement), écrites en ATTAQUE-MAINTIEN-RELÂCHÉ (forme atteinte en moins de 130 ms, tenue 350-700 ms, relâchée plus lentement — une forme non tenue est un bruit, c'est toute la différence entre une mimique et un tic), grandes scènes plus rares que les petites, cadence aléatoire **6-14 s** (une scène toutes les ~9 s, trois à neuf par minute, épinglé sur cinq minutes simulées — à 2,2-6,5 s plus les gestes des yeux, le visage bougeait toutes les 2 s et lisait comme des tics nerveux), rafale 10 % à 1,8 s, taille et côté aléatoires, sur les expressions qui respirent seulement, redémarrée à chaque changement d'expression ; le RELÂCHÉ n'est pas une clé : la bande se termine à son relâché et le canal revient sur la dynamique propre de l'expression (~650 ms, slow-out continu par construction), toujours plus lent que l'attaque (mesuré sur un grin : 270 ms à l'aller, 250-1 100 ms au retour), ressorts d'attaque amortis de 0,55 à 0,72 ; **(5ter)** DIX SAYNÈTES (`rig/sketches.ts`) de 3 à 5 s — la mouche, le double take, l'éternuement, le bâillement-étirement, le hoquet, le cache-cache, le tournis, l'endormissement-sursaut, la danse des sourcils, le regard suspicieux — jouées par le rig toutes les 45 à 120 s sur un visage au repos seulement, une liste à part classée entre les beats ponctuels (un clignement gagne) et les motifs d'état ; quatre règles testées : une pièce et non un beat (3-5 s, jamais les paupières ni la silhouette, retour EXACT à la pose contre un rig jumeau), rare et au repos, une chose à la fois (la vie de la bouche s'efface, une seconde scène remplace la première), jamais plus longtemps que son état (abandonnée au changement d'expression) ; deux planchers de sortie en sont sortis (largeur de bouche ≥ 0,2, écrasement de l'œil ≥ 0,1 : un grin sur un dôme de joie fermait l'œil au-delà d'un trait) ; le vocabulaire de chorégraphie écrit quatre fois est un module (`rig/choreo.ts`) ; une première version discrète (bouche seule, ressorts doux) « bougeait un peu » et a été jugée ni Pixar ni expressive — à juste titre ; elle tire sur un FLUX D'ENTROPIE SÉPARÉ (`lifeRandom`, xorshift semé par le hook) parce qu'un tirage sur `Math.random` à la construction décalait d'un cran les séquences épinglées des tests du widget — quatre rouges mesurés ; sans source, rien ne joue et le budget en pixels reste celui du maintien. **(6)** La parole a des PHRASES (enveloppe lente à travers la fermeture, ouverture bornée à [0, 1], fermée 9,6 % du temps) et des sourcils qui ponctuent (motif bouclé irrégulier) ; le plein régime qu'un motif impose est MESURÉ : 0,11 ms par image en moyenne. **(7)** Entrées étendues : dépassement de l'arc en surprise, sourcils qui coulent en dernier dans la tristesse, un bâillement pour `tired`. **(8)** Un budget en pixels dans les tests : la bouche bouge entre 0,6 et 2 px au repos, les sourcils sous 2 px à chaque taille, une pensée jusqu'à 3 px, une concentration exactement 0 ; la garde de la bulle mesure au plus FORT (amplitude × famille, arc compris) : 6,2 px de dégagement. Rejetés : les sourcils absents au repos, les couplages en CSS, des dérives indépendantes, une dérive en rotation, l'exagération de l'ouverture.


### ADR-265 : la semaine des routines — un historique des exécutions écrit au résultat, une grille qui ne fait que peindre

**Date** : 2026-09-05 · **Statut** : Accepté
**Fichier**: `docs/architecture/ADR-265-Routine-Week-Timeline-And-Run-History.md`

**Décision** : le propriétaire demandait une chronologie verticale (heures × jours), un tri chronologique numéroté, un indicateur de pause, puis une couleur PAR CRÉNEAU de la semaine (blanc, vert, rouge, gris) réinitialisée chaque semaine et à chaque changement. Une couleur est une affirmation sur UN créneau et rien ne la portait : `scheduled_actions` ne garde qu'un `last_executed_at` et un `last_error` sans horodatage ; le registre des tours (ADR-263) classe un tour planifié sous l'id de la CONVERSATION, ne sait pas dire « condition non remplie », « proposée » ni « ignorée, validation en attente », et ignore le créneau servi. **(1)** Le tri est celui de l'affichage (heure, minute, titre en tri linguistique numérique, id) ; l'ordre de l'API (`next_trigger_at`) reste, trois autres lecteurs en dépendent (hub, briefing, outil d'automatisation) ; une routine en pause garde son rang. **(2)** Une table `scheduled_action_runs` écrite AU RÉSULTAT par l'exécuteur, dans la même transaction que le marquage, jamais avant : cinq issues, une par sortie (`success`, `failure`, `skipped_condition`, `proposed`, `skipped_hitl`), insertion dans un SAVEPOINT et échec journalisé, jamais levé ; CHECK réel sur `outcome` (`create_constraint=True` explicite — mesuré sur la base de dev, les trois registres qui se disent « CHECK-backed » n'en portent aucun) ; la ligne porte le CRÉNEAU SERVI (un run dû sert son instant dû, un « Tester » ne sert le créneau du jour qu'une fois celui-ci passé, calculé par la même fonction que la semaine) ; rétention par réglage (90 j), purgée à l'étape 0 du tick de l'exécuteur, avant le retour anticipé du lot vide, dans sa propre session, jamais fatale. **(3)** `GET /scheduled-actions/week` calcule côté serveur, avec le moteur cron qui arme les runs, les sept instants de la semaine de chaque routine depuis le lundi local de SON fuseau ; une cellule prend le DERNIER run dont le `slot_at` est ÉGAL à l'instant — égalité, jamais fenêtre — d'où la réinitialisation au changement (les instants bougent) et chaque semaine (la fenêtre glisse) par construction ; route déclarée avant `/{action_id}`, ordre épinglé. La grille positionne par `trigger_hour` et `days_of_week`, heure murale du fuseau nommé à côté, ne convertit aucun instant, se dessine toujours et DIT quand les états sont indisponibles ; ambre = proposée en attente d'accord. **(4)** Une vraie `<table>` (sept en-têtes de colonne, vingt-quatre de ligne, légende nommant le fuseau), des puces `<button>` nommées « rang, titre, heure, état » qui amènent et FOCALISENT la carte, le titre jamais en texte visible sur la grille ; une seule `RoutineNumberChip` pour la carte et la grille, cinq tons en jetons du thème couverts par la garde de contraste, anneau pour une routine conditionnelle, pulsation immobile sous mouvement réduit ; colonne du jour depuis la lecture serveur, `Intl` en repli sur un fuseau unique, rien sur plusieurs. **(5)** Trois défauts préexistants fermés : chaque sondage démontait les cartes (`initialLoading` monotone + `aria-busy`) ; une routine en pause gardait l'ancien fuseau après un déménagement ; APScheduler 3.11.2 SAUTE la journée d'une routine dans un trou d'heure d'été ouvert à minuit (Santiago, La Havane, Le Caire, Beyrouth — 72 créneaux sur 2 184 mesurés) : un seul point de lecture du cron (`_next_fire`) rend le premier instant existant du jour sauté, différentiel sur toutes les zones IANA : 15 684 identiques, 144 corrigés, 0 anomalie. Rejetés : lire le registre, une fenêtre de tolérance, un placement à la minute, trier l'API, relire le cron dans le navigateur, un job de purge dédié.

### ADR-266 : le diagnosticien lit ses preuves au moment du diagnostic, et un SDK fournisseur reçoit un `str` exact

**Date** : 2026-09-05 · **Statut** : Accepté
**Fichier**: `docs/architecture/ADR-266-Diagnosis-Evidence-At-Diagnosis-Time-And-Exact-Str-Embedding-Inputs.md`

**Décision** : une alerte `embedding_failure_rate` à 25 % et, une fois de plus, un diagnostic automatique concluant « preuves insuffisantes ». Les deux moitiés ont été mesurées. **(1)** Cinq tours sur cinq perdaient leur contexte RAG : `HumanMessage.text` est un `TextAccessor`, SOUS-CLASSE de `str`, et le modèle de requête de google-genai le valide via une union où `Content` (`from_attributes=True`) précède `str` — la sous-classe devient un `Content()` vide, `"content": {}` sur le fil, `500 INTERNAL` en retour, identique sur les SDK 1.67.0 et 2.10.0 sous pydantic 2.13.4 ; le chemin mémoire ne survivait que parce qu'il TRANCHE le texte. L'entonnoir `GeminiRetrievalEmbeddings` coerce ses entrées en `str` exact (même objet quand il l'est déjà), le point d'extraction du message aussi ; rejetés : corriger l'appelant RAG seul (28 appelants), reclasser le `500` en permanent. **(2)** La NATURE d'un refus fournisseur devient une métrique (`embedding_provider_errors_total{reason}`) classée par le même code que le réessai. **(3)** Le dossier de preuves est collecté AU MOMENT DU DIAGNOSTIC, par recette déclarée par clé de corrélation (assert au boot, test CI sur chaque alerte chargée et chaque événement nommé), en échec ouvert source par source : ventilations Prometheus (quatre requêtes de catalogue), extrait Loki borné, filtré côté client, en liste blanche et pseudonymisé, bloc runtime (version, commit, âge du processus), collecté une fois par incident après la porte budgétaire ; le prompt lit le dossier et distingue une source indisponible du silence ; ce que le modèle a vu est stocké avec ce qu'il a dit et rendu à l'administrateur. La contrainte d'ADR-254 (« une tique ne dépend pas de Loki ») tient : l'hôte est la pompe, pas le contrôle. Rejetés : collecter au tick, LogQL libre, un plancher d'échantillons (le contrôle avait raison sur huit opérations). **(4)** `prepare-prod.ps1` stage enfin `docs/runbooks` (jamais livrés : `had_runbook=false` sur tous les diagnostics), test Pester, compte `runbooks_available` publié, avertissement au démarrage. **(5)** `detail` vide omis, fenêtre de mesure dans l'évidence, test fantôme `recent_errors` remplacé, cinq métriques diagnostics aveugles câblées (baseline 56 → 51).

### ADR-267 : Ollama est un fournisseur natif, et son serveur déclare ce que ses modèles savent faire

**Date** : 2026-09-05 · **Statut** : Accepté
**Fichier**: `docs/architecture/ADR-267-Ollama-Native-Provider-And-Discovered-Capabilities.md`

**Décision** : le slot `response` passé sur `ollama / qwen3.8:27b` mourait à l'instanciation (`ChatOpenAI` recevait l'objet `ReasoningIntent`), et le premier correctif — couture ADR-245 sur les branches Ollama et Perplexity, URL normalisée, plafond porté dans `extra_body`, usage demandé — a été réfuté par sa propre preuve runtime : douze jetons demandés, douze jetons de PENSÉE, réponse vide, parce que le pont OpenAI-compatible ne permet pas de dire à Ollama de ne pas penser, ni de fixer la fenêtre de contexte (le seuil de compaction lisait 51 200 = 128 000 × 0,4, le défaut d'un modèle inconnu). **(1)** Ollama est servi par le client natif `langchain-ollama` (`_create_ollama_llm`) : `think`, `num_ctx`, `num_predict`, `format` JSON-schema, `reasoning_content`, usage sur chaque réponse ; le suffixe `/v1` disparaît (racine du serveur, tolérée et retirée par `ollama_urls.py`). **(2)** Une couche DÉCOUVERTE dans `ModelCapabilitiesCache` : les profils lus sur `/api/show` (capacités `tools`/`vision`/`thinking`, longueur de contexte), provenance `discovered`, en mémoire seulement, qui survit aux rechargements du catalogue et GAGNE sur une ligne de seed du même nom ; rafraîchie au boot, à chaque rechargement des clés fournisseur et à la liste admin. **(3)** La famille de raisonnement `ollama` a une échelle déclarée PAR MODÈLE : complète pour un modèle `thinking`, `("none",)` pour les autres — la coercition ramène toute profondeur à l'interrupteur que le serveur accepte partout, le chemin d'écriture refuse en 422, l'UI n'offre que ce que l'API accepte ; un tag non découvert reste inconnu. Quand rien n'est demandé sur un modèle pensant, le défaut du serveur est rendu explicite (`reasoning=True`) pour que la trace soit SÉPARÉE et non perdue. **(4)** Ce que LIA compte est ce que LIA demande : `max_input_tokens` du profil découvert = `OLLAMA_NUM_CTX` sinon le maximum du modèle plafonné par `OLLAMA_NUM_CTX_DEFAULT_CAP`, envoyé comme `num_ctx` à chaque appel et lu par `get_effective_context_window` — mesuré : demandé 32 768, alloué 32 768 (`/api/ps`), compté 32 768. **(5)** Usage `native` (ADR-220 amendé), sortie structurée par le champ `format` (`native_structured_method`, un choix pour trois sites). Garde : `test_reasoning_seam_guard.py` sur chaque `ProviderType` × chaque niveau, mutation-checkée. Amende ADR-245, ADR-220, ADR-244.

---

### ADR-268 : Un moteur de récurrence, deux consommateurs, et un rappel qui peut se répéter

**Date** : 2026-09-06 · **Statut** : Accepté
**Fichier**: `docs/architecture/ADR-268-Generic-Recurrence-And-Reminder-Management.md`

**Décision** : une routine tenait dans trois colonnes cron (`days_of_week`, `trigger_hour`, `trigger_minute`) — elle ne savait dire ni une occurrence unique, ni « un mardi sur deux », ni « le 2e lundi du mois », ni surtout **deux fois dans la même journée** ; un rappel tenait dans un instant et mourait après notification. Le moteur n'est pas un choix de goût : APScheduler 3.11 **SAUTE la journée entière** quand le trou d'heure d'été s'ouvre à minuit — mesuré sur toutes les zones IANA et toutes les transitions 2026, 2 112 créneaux décalés et **72 sautés**, soit 142 exécutions perdues par an sur 73 zones, `Europe/Paris` comprise. **(1)** `src/core/recurrence` porte le vocabulaire, le moteur (`dateutil.rrule` énumère des JOURS puis localise en `fold=0`, il ne demande jamais à un trigger quel jour vient ensuite), la phrase en 6 langues et les trois questions d'une chose planifiée ; le modèle est un PRODUIT — quels jours × quels moments — et `anchor_date` est OBLIGATOIRE, sans quoi « tous les 3 jours » répondait « dans 3 jours » depuis n'importe quel instant. **(2)** Les plafonds sont INJECTÉS (`RecurrenceLimits`) : 12 déclenchements/jour pour une routine, 48 pour un rappel, un seul moteur. **(3)** Le post-it devient un CAS de la règle générale : `rearm_after` rend `None` → `DELETE`, sinon on réarme ; une occurrence unique consommée rend `None`, donc le comportement historique et le réamorçage sont la même ligne, sans branche « ça se répète ? ». D'où `trigger_at` NOT NULL côté rappels (un rappel sans avenir n'existe pas), `retry_count` remis à zéro au succès (il ne l'était jamais — la ligne mourait là, et un rappel quotidien se serait supprimé après trois échecs étalés sur des mois), et un échec répété qui abandonne l'OCCURRENCE, pas la série. **(4)** UNE seule autorité pour l'heure : `POST` refuse `trigger_at` ET `recurrence` ensemble — mesuré avant la garde, l'instant de l'appelant gagnait et la carte annonçait 08:00 pendant que la notification arrivait à 23:00 ; `PATCH` refuse un `trigger_at` seul sur un rappel récurrent, que le réamorçage suivant jetterait. **(5)** Une heure murale suit la personne : un changement de fuseau recalcule routines ET rappels, occurrence unique comprise (changement de comportement assumé), composé dans `infrastructure/` parce que l'appel direct fermait un cycle `reminders<->users` et que le ratchet est shrink-only ; un rappel DÛ mais pas encore ramassé est laissé entièrement tranquille, fuseau compris. **(6)** Le prompt ordonnait « Mention when the request was made » sans condition — un rappel quotidien créé il y a trois mois faisait dire « tu m'as demandé il y a 3 mois » CHAQUE matin ; deux fragments versionnés le remplacent. **(7)** Cinq textes fr/en en dur passent aux 6 langues, dont le **corps de notification de repli** : quatre lecteurs sur six recevaient une notification anglaise, précisément quand quelque chose avait déjà échoué. **(8)** Le domaine des rappels refusait toute surface de gestion, par décision écrite dans son propre routeur ; le propriétaire l'a renversée. Ce qui ne change pas : la liste n'est JAMAIS un historique, ni snooze ni accusé de réception. Rejetés : un second vocabulaire de récurrence pour l'outil de chat (sans corpus de formulations), garder un rappel épuisé « terminé », élargir la ligne de base des cycles. Amende ADR-051, ADR-198, ADR-265.

---

### ADR-269 : Un débrief par relation, écrit une fois par jour, et lu par le chat

**Date** : 2026-09-07 · **Statut** : Accepté
**Fichier**: `docs/architecture/ADR-269-Relationship-Debrief.md`

**Décision** : la fiche d'une relation empile dix sections et personne n'en lit dix ; ce que le lecteur cherche — où j'en suis, qu'est-ce qu'il faut aborder — est une synthèse qu'aucun agrégat ne produit. La découverte qui décide de la forme : l'assemblage des preuves dont un débrief a besoin EXISTAIT, dans `get_person_overview_tool`. **(1)** Il est EXTRAIT vers `domains/relations/overview` et l'outil en devient le premier consommateur, le refactor épinglé par un fichier doré capturé AVANT extraction (18 cas de périmètre, charge utile et message comparés à l'octet). **(2)** Un défaut de coût tombe au passage : l'assemblage lisait les fournisseurs INCONDITIONNELLEMENT puis jetait les blocs que le périmètre excluait — jusqu'à **onze appels externes** payés par un lecteur qui avait tout décoché, soit ADR-184 pointé vers le coût ; le rétrécissement demande un troisième statut, `NOT_REQUESTED`, parce que « je n'ai pas regardé, exprès » n'est ni « rien trouvé » ni « je n'ai pas pu ». **(3)** Le débrief obéit au périmètre 360° — une seule autorité, qui borne aussi le coût et la taille du prompt. **(4)** Construit à l'ouverture de la fiche, jamais par un ordonnanceur : `relations_total` n'est borné par rien. **(5)** Au plus une fois par jour LOCAL, avec trois reconstructions légitimes (langue, périmètre, demande) — une frontière UTC reconstruirait à 2 h du matin pour la moitié de l'Europe. **(6)** L'empreinte des preuves n'est comparée qu'avec les preuves EN MAIN : un drapeau passif « les données ont bougé » énoncerait un négatif que personne n'a vérifié, alors qu'à la construction une reconstruction forcée sur des preuves identiques n'appelle AUCUN modèle et reporte le jour sans toucher à la date d'écriture ; le raccourci n'est pas pris quand la langue a changé, sans quoi le débrief resterait dans l'ancienne langue pour toujours. **(7)** Rien n'est inventé : aucune preuve → `empty` sans appel, et un échec CONSERVE le texte précédent — remplacer une synthèse encore utile par un panneau vide transforme « je n'ai pas pu rafraîchir » en « il n'y a rien ». **(8)** La revendication est une seule instruction (`ON CONFLICT … WHERE … RETURNING`), `held_until` portant deux sens qui se lisent pareil (bail d'un constructeur mort, refroidissement d'un échec) et chaque clôture citant son `claim_owner`. **(9)** Dans le chat le débrief REJOINT le bloc pair sans le remplacer, avec la directive INVERSE : le bloc pair dit ses faits EXACTS parce qu'il les lit dans le tour même, la même phrase sur une synthèse datée serait une machine à affirmations fausses ; une correspondance ambiguë n'injecte rien. **(10)** L'injection réutilise le créneau existant — `fetch_response_context` porte une CC de 67 que la porte gèle, et son `gather` s'arrête à six awaitables. **(11)** L'utilisateur coupe depuis la page Relations, là où le résultat s'affiche. Supprimé au passage : une duplication de `open_category_client` qui rendait un client lié à une session fermée, **trois copies** du lecteur de prompt par chemin (une quatrième allait naître), et le détecteur de mention descendu dans `shared` pour ne pas refermer un cycle `agents<->relations`. Rejetés : le balayage nocturne, la construction pendant un tour, l'injection dans le planificateur et ReAct, l'historique des débriefs. Amende ADR-176, ADR-184, ADR-190.

---

### ADR-270 : Les routes de la dépense, le payeur déclaré, et l'auteur d'une ligne de registre

**Date** : 2026-09-07 · **Statut** : Accepté
**Fichier**: `docs/architecture/ADR-270-Spend-Roads-And-Register-Authorship.md`

**Décision** : deux questions — chaque euro est-il compté et imputé au bon titulaire ; le registre dit-il ce que LIA a fait sans qu'on le lui demande — n'avaient de réponse lisible nulle part, et y répondre en lisant les fichiers a produit **neuf conclusions fausses en une session**. La cause est structurelle : la comptabilité est AMBIANTE (un nœud dépense via un `TrackingContext` posé par un ancêtre), donc le fichier ne mentionne rien. La production a tranché : 24/24, 22/22, 10/10 pour les tours ; **0 sur 228** hors tour ; **0 sur 84** traductions de personnalités, écrites pendant que le registre enregistrait 5 976 autres lignes. **(1)** `spend_roads.py` déclare, pour chaque module appelant `get_llm`, le registre que ses euros atteignent — et la garde, par AST jamais par sous-chaîne, a pris **6 des 46 classifications de l'auteur** en défaut. **(2)** `instance_spend.py` porte la dépense SANS titulaire au seul registre journalier de l'instance, et lui demande d'abord la permission — `token_usage_logs.user_id` étant `NOT NULL`, ce devait être un chemin distinct. **(3)** `cost_bearers.py` déclare qui paie, et la garde compare la déclaration à la somme réellement appliquée **dans les deux sens** : aucune famille d'instance n'échappe au plafond, aucune famille payée par la clé de l'utilisateur n'est facturée à son quota. **(4)** `EffectSource.PROACTIVE` rejoint le vocabulaire, mais l'origine est DÉCLARÉE par chaque site, jamais par défaut : rien ne planifie le briefing, donc tout classer `proactive` aurait crédité LIA d'ouvertures de page qu'elle n'a pas choisies ; un rappel est l'instruction différée de la personne (`scheduled`), seul un balayage est une initiative. La dérivation, écrite **quatre fois**, devient un prédicat unique avant l'ajout de la troisième valeur. **(5)** Toute surface qui ouvre vos sources le dit — et c'est un REGISTRE, pas une liste de correctifs : le briefing (9 sources), le **débrief de relation** (7 ; son auteur ne le trouvait pas et a pris pour lui une ligne de briefing voisine — il n'était filé sous AUCUN nom) et le **balayage heartbeat** (15 sources, **824 runs en 30 jours sans une seule ligne**, le cas où personne ne peut reconstituer ce qui a été lu). Trois trouvés un par un n'est pas une méthode : `user_data_readers.py` fait déclarer à CHAQUE surface hors tour son module d'enregistrement ou la raison écrite qu'elle n'ouvre rien, sur une liste COMPLÈTE. Un cache servi n'est pas une consultation — la boîte mail n'a pas été ouverte — une section masquée non plus, et une source qui refuse se lit `failed`. Le registre est OFFERT et non pris : `agents` importe `relations`, donc l'inversion passe par `domains/shared/consultation_sink` (le ratchet de couplage compte aussi les imports locaux). **(6)** Une garde, deux portes : `get_structured_output_with_retry` ne prenait même pas de `user_id`, et le débrief, la synthèse téléphonique et l'auto-diagnostic passaient par elle ; deux refus qu'elle ne doit PAS faire — une dépense sans titulaire n'est jamais bloquée, `"system"` n'est pas un compte. **(7)** Un quatrième onglet qui est une LECTURE et non un registre : une initiative n'est ni une action ni une consultation, c'est une origine, et `mine` est défini par exclusion pour qu'une cinquième origine soit visible plutôt qu'invisible. Défauts fermés au passage : les notifications de rappel n'écrivaient que l'agrégat par run — ni trace article 12, ni décompte de quota, ni plafond d'instance — et **huit copies** de la lecture des métadonnées d'usage n'étaient pas d'accord entre elles, sept d'entre elles facturant le cache Anthropic au prix plein. Aucune migration : mesuré en production, les trois tables de registre ne portent **aucune contrainte CHECK**. Amende ADR-263, ADR-248, ADR-085.

---

### ADR-271 : Un chargement de page, un seul acte de lecture

**Date** : 2026-09-07 · **Statut** : Accepté
**Fichier**: `docs/architecture/ADR-271-One-Page-Load-One-Act-Of-Reading.md`

**Décision** : le tableau de bord s'affiche par deux points d'entrée lancés en parallèle (`/briefing/cards`, `/briefing/synthesis`) — un découpage délibéré, qui reste — mais chacun obtenait le lot des neuf sections POUR SON COMPTE, si bien que les sources de la personne étaient ouvertes deux fois dans la même seconde. Ce n'est pas déduit du code, c'est MESURÉ sur l'instance : deux constructions à 23 ms d'écart, `duration_ms=1108` chacune, et dans la même fenêtre `weather_current_retrieved`, `calendar_events_listed`, `drive_search_completed`, `tasks_list_tasks_completed`, `treatments_recorded` — tous exactement 2× ; sur sept jours, **151 constructions dont 44 doublons, 39 % des chargements payant double**, et **44 doublons sur 44 concurrents** d'une construction `/cards`. C'est cette dernière ligne qui décide de la forme : la coalescence, et elle seule, les supprime tous. **(1)** La cause racine est une QUESTION : `/synthesis` demandait « ce lot contient-il quelque chose d'intéressant ? » là où la seule question utile est « ce lot a-t-il été construit ? » — les deux se lisent pareil, et la première ne distingue pas un cache froid d'une journée légitimement calme ; elle était de surcroît une SECONDE implémentation d'un seuil que `generate_synthesis` possédait déjà (le service comptait six sections, l'assistant LLM neuf : 0 contre 2 sur le même lot). **(2)** `infrastructure/utils/single_flight.py` pose la couture — miroir de `retry_async`, une FABRIQUE et non un attendable, puisqu'une coroutine ne s'attend qu'une fois. Ce n'est délibérément PAS un cache : une exécution terminée n'est jamais resservie. Quatre propriétés, chacune contre une façon de se tromper : le registre EST la référence forte (le piège `async_utils`), les attendeurs sont `shield`és (sinon une déconnexion tue le travail des autres), une entrée appartient à une boucle d'événements — vérifié AVANT l'achèvement, une tâche étrangère étant inutilisable dans les deux cas —, et un échec est partagé puis OUBLIÉ. **(3)** La clé de vol est l'identité entière (compte, langue, fuseau, sections masquées, sections forcées) : un rafraîchissement forcé ne rejoint JAMAIS une construction non forcée, sinon il rendrait le cache qu'on lui demandait de contourner. **(4)** `/synthesis` décide désormais sur la COUVERTURE (une section visible absente du cache), jamais sur la richesse ; `_count_sections_with_data` disparaît. **(5)** `SECTION_REMINDERS_TTL_SECONDS` passe de 0 à 60 : la carte reste toujours en direct (le plan la force), mais la section est enfin ÉCRITE — à 0 elle était invisible aux lecteurs de cache, et la branche `reminders` de `_summarize_cards_for_llm`, écrite depuis toujours, n'était atteignable que par « tout rafraîchir » : le même tableau de bord produisait deux textes selon la façon dont on le demandait. **(6)** La clé de cache porte la LANGUE : agenda, mails, rappels et documents sont préformatés côté serveur, et le cache servait la langue précédente jusqu'à expiration. Le registre se répare de lui-même — la tâche partagée tourne dans le contexte de celui qui l'a lancée, donc un rejoignant ne collecte rien et ne dépose aucune décision : un acte, un jeu de lignes, sans une ligne de code de registre. Grafana 25 affichera des nombres PLUS BAS (fin d'un double comptage, pas une régression) et deux panneaux le rendent lisible. **(7)** Deux défauts que ce changement a lui-même déplacés, trouvés en revue adversariale à froid. La clé de cache avait **quatre** constructeurs — trois f-strings dans le service, un quatrième dans `push_channels/cache_invalidation`, un autre domaine : y ajouter la langue laissait le quatrième supprimer une clé qui n'existait plus, et `redis.delete` sur une clé absente réussit en renvoyant 0. **Une notification Gmail ou Calendar n'invalidait donc plus rien, en silence**, avec une péremption bornée par la seule TTL que le push existe pour court-circuiter. Un constructeur unique (`briefing/cache_keys.py`), l'invalidation demande toutes les variantes de langue, et le module refuse de se charger sur une liste vide — `SUPPORTED_LANGUAGES=` dans l'env en produit une, et `redis.delete()` sans argument est une erreur que l'appelant avale par doctrine : le même no-op muet. **(8)** Rendre la section cacheable l'a fait atteindre `_summarize_cards_for_llm`, dont la branche non verbeuse tendait à la salutation un **compte NU** de rappels — or la règle écrite juste au-dessus, pour l'agenda, consigne ce que cela produit : « deux rendez-vous cet après-midi » pour des événements répartis sur deux jours. La synthèse reçoit la paire `{content, trigger}` datée que son propre prompt documente ; la salutation ne reçoit rien, suivant le classement que `chat/suggestions` énonce déjà (« un rappel sonnera de lui-même »). Constante morte supprimée au passage : `SECTION_BIRTHDAYS_TTL_SECONDS = 604800` disait « 7 jours » et n'était appliquée nulle part — la vraie règle est minuit local plafonné à 24 h, et la documentation citait la constante comme si c'était la règle. **(9)** AMENDÉ après déploiement : la coalescence in-process ne suffit PAS, parce que la production n'est **pas** mono-processus — uvicorn honore `WEB_CONCURRENCY`, que `.env.prod.example` fixe à **4**. J'avais vérifié le `CMD` du Dockerfile et les `replicas` du compose, **jamais l'environnement**. Mesuré sur l'instance déployée : trois paires de constructions qui se chevauchent, **aucune rejointe**, et `briefing:mails` ouvert **cinq fois** pour deux chargements (une ligne de consultation ne s'écrit que sur un fetch LIVE). D'où `infrastructure/utils/shared_flight.py` : un worker REVENDIQUE la construction dans Redis, les autres attendent ce qu'il publie — le cache de sections qu'il remplit de toute façon, donc aucune charge utile propre à la couture ne transite. Les deux coutures se composent. Trois règles non négociables : la revendication est relâchée par son PROPRIÉTAIRE (comparaison-et-suppression sur un jeton, jamais un `DELETE` inconditionnel après un `SET NX`), l'attente est BORNÉE et celui qui expire CONSTRUIT, et tout échec retombe sur la construction. Un rafraîchissement FORCÉ n'attend jamais : ce qu'un détenteur publie EST le cache qu'il demandait à contourner. Ce que la décision NE résout PAS est écrit : la coalescence in-process seule ne suffisait pas, un connecteur en panne coûte encore une construction supplémentaire hors concurrence (cas mesuré inexistant : 0 section ERROR sur sept jours), et un changement de langue coûte une construction. Rejetés : séquencer les deux appels côté navigateur, mettre les ERREURS en cache, un marqueur « construit à », la coalescence par section. Amende ADR-184, ADR-185, ADR-248, ADR-254, ADR-263.

---

### ADR-272 : Tout jeton payé par la plateforme répond aux deux plafonds

**Date** : 2026-09-07 · **Statut** : Accepté
**Fichier**: `docs/architecture/ADR-272-Every-Platform-Paid-Token-Answers-To-Both-Ceilings.md`

**Décision** : ADR-270 avait réglé OÙ chaque dépense est enregistrée, pas SI quelque chose la borne — et les deux questions sont distinctes : un euro peut être parfaitement compté et dépensé au-delà d'un plafond que personne n'a interrogé. La règle appliquée ici est produit : **tout coût jeton porté par la plateforme est soumis aux quotas — celui du compte ET celui de l'instance ; seul ce que la personne paie avec SA propre clé de connecteur en est exclu**. `cost_bearers` tranche déjà : la famille `llm` porte `CostBearer.INSTANCE` (les modèles tournent sur `provider_api_keys`, table sans `user_id`), donc **tout appel modèle est dans le périmètre** ; Perplexity, Brave, la météo et la téléphonie n'y sont pas. Mesuré sur les 46 sites déclarés par `LLM_SPEND_ROADS` : 17 passent une porte gardée, 21 sont bornés à l'entrée d'un tour, 7 par un comptable nommé — et **5 ne l'étaient par rien** (`evaluation_pipeline` pour l'opérateur, puis `open_loop_extractor`, `briefing/llm`, `user_mcp/description_generation` et `reminder_notification` contre un compte réel). Trois défauts structurels les produisaient. **(1)** Le registre nommait une ENVELOPPE, pas la porte : `get_structured_output_with_retry` délègue à `get_structured_output`, publique, non gardée et appelée directement par neuf modules — une des deux fonctions sœurs du même fichier était gardée, et le test de complétude ne pouvait pas le voir puisqu'il ne vérifie que les portes DÉCLARÉES. Descendre la déclaration a fermé deux trous à elle seule. **(2)** Le garde partagé SORTAIT avant de demander : sur `usage_limits_enabled` faux — ce qui coupait aussi le plafond d'INSTANCE, alors que `check_user_allowed` le maintient délibérément hors de ce drapeau (« borner ce qu'UN compte consomme et borner ce que l'INSTANCE dépense sont deux protections différentes ») ; et sur l'absence de propriétaire — vrai pour le plafond du compte, hors sujet pour la clé du déploiement qui a payé. **(3)** Un même refus, deux réponses : le routeur nommait la pause d'instance avec un code stable et un `Retry-After` calculable, le garde partagé avec ni l'un ni l'autre. Remèdes : la porte est désormais l'INTERNE, le garde demande les deux plafonds dans tous les cas, `usage_limits/enforcement.py` porte l'unique lève-exception (routeur et garde l'appellent), et **deux formes pour RECEVOIR un refus, jamais deux façons de le CALCULER** — un chemin de requête lève, un chemin d'arrière-plan renonce (`spend_blocked`, jumelle non levante de `is_instance_spend_blocked`) : le rappel sonne avec sa phrase écrite, le tableau de bord s'affiche avec sa salutation écrite, le serveur MCP s'enregistre avec sa description déterministe. Et il dit « sauté », pas « échoué » : journaliser un refus de quota comme un échec de génération décrit ce que le code n'a pas fait. Enfin `test_every_spend_site_is_bounded` parcourt les 46 sites et exige que chacun atteigne un plafond — porte déclarée, demande directe, ou entrée de tour NOMMÉE et vérifiée. Ce que la décision ne résout pas est écrit : un plafond est demandé avant l'appel et non pendant, la route `TURN` est bornée une fois par tour à l'entrée, et rien ici ne borne un connecteur que la personne paie elle-même. Amende ADR-270, ADR-248, ADR-085.


**Amendement 2026-09-20 — la règle n'est pas propre au modèle** : le principe du propriétaire, énoncé en entier le 2026-09-19 (« quelles que soient les modalités et chemins, tout ce qui a un coût pour la plateforme pour un utilisateur doit être tracé, affiché, affecté et comptabilisé pour cet utilisateur »), appliqué à la famille Google Maps Platform, qui avait des trous dans toutes les directions. **Mesuré** : les deux portes de persistance de `TrackingContext` ne comptaient que les enregistrements du modèle — un lookup Places/Routes d'un appel téléphonique ou d'une session Live directe (sans appel LLM propre) n'écrivait RIEN dans aucun des quatre grands livres ; le compteur `track_google_api_call` échouait ouvert sans tracker ambiant — 3 020 lignes sur dev, aucune hors d'un tour de chat, alors que le heartbeat (Routes), le briefing (Google Weather : 8 appels facturés par rafraîchissement, 0,0139 €), les réunions (Geocoding) et les proxys d'images (cartes statiques, jamais comptées) payaient ; les photos et la carte d'itinéraire étaient PRÉ-comptées à la construction de l'URL (un fetch qui peut ne jamais avoir lieu), la recherche Places brute de `_resolve_destination` jamais ; la porte directe du géocodage de profil n'alimentait ni la ligne de run ni le registre d'instance ; la facture d'un appel et la carte de clôture Live ne lisaient que la colonne LLM. **Décision** : (1) UN prédicat de persistance, `pending_families()`, lu par les deux portes, avec une garde qui énumère chaque seau `_*_records` du constructeur ; (2) le compteur échoue FERMÉ — `google_api_calls_unaccounted_total{api_name}`, alerte `GoogleApiCallsUnaccounted` à zéro, pendant de `LLMCallsWithoutUsage` ; (3) chaque surface hors tour ouvre sa propre comptabilité autour de l'acte ENTIER (runner proactif, briefing, enrichissement de réunion sous le run id des minutes, géocodage de profil sur sa propre session) ; (4) une image facturée est comptée AU PROXY, à l'instant où Google facture, par UN helper (`serve_billed_image`), et rattachée au tour qui a construit l'URL par un run id SIGNÉ HMAC sur (run id, compte) — un run id nu laisserait un appelant créer la ligne d'un autre compte ; (5) la route est DÉCLARÉE (`google_api/spend_roads.py`, garde sur le graphe d'imports, portes vérifiées par AST) ; (6) UNE figure pour ce qu'un run a coûté (`MessageTokenSummary.billed_cost_eur`), lue par la facture téléphonique, la carte Live, l'archive et les agrégats. Prouvé sur Docker dev : un lookup téléphonique file 0,034904 € de Places dans les quatre grands livres.
---

### ADR-273 : Une extraction de registre est complète, ou ce n'est pas une extraction

**Date** : 2026-09-07 · **Statut** : Accepté
**Fichier**: `docs/architecture/ADR-273-Complete-Register-Extractions.md`

**Décision** : les cinq extractions téléchargeables (trois côté administrateur, deux côté titulaire du compte) portaient un plafond de lignes — 5 000 par export technique, 1 000 par source pour l'extraction unifiée. Le plafond était MESURÉ et non arbitraire : cinq sources à 5 000 lignes rendaient un fichier de 10,8 Mo avec un **pic mémoire de 33,9 Mo** et 939 ms de sérialisation pure, sur le Raspberry Pi 5 que ce projet déploie. Mais il était appliqué à la MAUVAISE variable : chaque rendu assemblait le document entier en mémoire (`"\n".join(lines)`), si bien que le seul levier disponible était de lire moins de lignes. **Ce qui était rare, c'était la mémoire ; ce qui était borné, c'était la vérité.** Une personne exerçant son droit à la portabilité recevait un échantillon de son propre dossier, et un opérateur demandant « qu'a fait ce système ? » recevait un échantillon de l'instance — aucun des deux fichiers ne mentait (tous deux disaient `truncated`), mais un registre qui répond par un sous-ensemble inexpliqué est un registre sur lequel personne ne peut s'appuyer, et aucune formulation d'en-tête ne répare cela. **Mesuré sur l'instance de développement le 2026-09-07**, pour un seul compte et quelques semaines d'historique : l'extraction unifiée porte **49 195 lignes** (111 tours, 11 actions, 363 consultations, **48 710 appels de modèle**, 0 lacune) là où le plafond en laissait passer 5 000 au maximum — **97,9 % du dossier d'inférence était absent de chaque extraction**, c'est-à-dire précisément l'enregistrement qui dit quel modèle a répondu avec quels paramètres, celui pour lequel le lot 7 de l'article 12 existe. **(1)** Un curseur côté serveur remplace le plafond (`infrastructure/database/export_stream.py`) : les lignes sont lues par partitions et remises au rendu une par une, donc une requête tient une partition et non un registre ; `EFFECT_EXPORT_BATCH_ROWS` (défaut 1 000) remplace les deux réglages de plafond et n'arbitre que la mémoire contre les allers-retours. La leçon pour laquelle `export_window` avait été écrit survit à sa cause : l'ordre chronologique est appliqué à UN seul endroit. **(2)** Les lignes sont détachées UNE PAR UNE : `expunge_all()` REMPLACE la table d'identité de la session et TUE l'ancienne, dans laquelle le curseur ouvert charge encore — la deuxième partition mourait sur « this identity map is no longer valid ». Défaut trouvé par les tests d'intégration sur PostgreSQL réel et **invisible pour les tests unitaires**, un flux bouchonné n'ayant aucune table d'identité. **(3)** Les rendus deviennent des générateurs ; le regroupement Markdown par jour survit verbatim, n'ayant jamais eu besoin que du jour de la ligne précédente. **(4)** Le compte est EXACT, par agrégat sur l'énoncé même que le corps parcourt (ADR-185), et publié avant la première ligne — un fichier en flux ne peut pas réviser sa propre première ligne. Pour que les deux décrivent le même ensemble, une extraction sans borne haute s'en voit donner une : **l'instant de génération**. La fenêtre est fermée, le fichier reproductible, et la borne est NOMMÉE dans les `filters` de l'en-tête plutôt que posée en silence. **(5)** La complétude est AFFIRMÉE, jamais déduite du silence : `truncated` reste (toujours `false`), `X-Register-Truncated` reste, `complete: true` reste avec un compte exact par source ; `row_cap` et `cap_per_source` disparaissent, car un plafond qui n'existe plus ne doit pas être annoncé. Supprimer la clé aurait été plus propre et moins bon : un lecteur distinguerait alors un fichier complet d'un fichier partiel par l'ABSENCE d'un avertissement. **(6)** Le téléchargement est compressé en gzip incrémental quand le client l'accepte (`core/streaming_download.py`) — LIA s'auto-héberge, typiquement derrière un lien montant domestique, et ces fichiers sont du texte pur ; le compresseur tient une fenêtre de 32 Kio, jamais le document. Le NOM du fichier ne change pas : `Content-Encoding` est du transport, et un artefact `.gz` serait un autre contrat. **(7)** Le générateur ouvre sa PROPRE session : vérifié dans le source de FastAPI 0.136.3, les dépendances `yield` sont refermées après l'envoi du corps, donc la session de requête survivrait — mais les lignes sont détachées au fil du rendu, ce qu'aucun autre usager d'une session partagée n'attend, et FastAPI 0.106 a déjà livré l'ordre inverse une fois. **(8)** Un ÉCRAN garde sa taille de page : `/admin/effects/readable` conserve une lecture bornée et, avec elle, la règle de la fenêtre la plus récente. Les tests qui affirmaient un plafond ont été RÉÉCRITS et non supprimés — un test qui ne peut plus échouer se lit comme de la couverture — chacun remplacé par la moitié falsifiable de la même affirmation, et le nouvel oracle d'intégration a été délibérément cassé (7 rouges sur 8) avant d'être cru. Amende ADR-263, ADR-185, ADR-184.

---

### ADR-275 : une sortie structurée tronquée est un refus, jamais un sauvetage

**Fichier**: `docs/architecture/ADR-275-Truncated-Structured-Output-Is-A-Refusal.md`

**Décision** : `json_recovery` referme mécaniquement une structure JSON ouverte — il avait été écrit pour les clôtures de bloc, la prose et les virgules finales — et le sauvetage validait ensuite ce qui restait ; **rien ne lisait `finish_reason`**. Mesuré le 2026-09-08 en coupant une charge utile valide à 100 positions : **12 coupures de rapport et 14 de présentation ont validé comme des documents PLUS COURTS**, rendus, stockés et annoncés « generated successfully » — l'exact contraire de ce qu'ADR-226 affirme. Et quand la réparation ne validait PAS, `get_structured_output_with_retry` rejouait le même prompt trois fois : une troncature est déterministe, chaque rejeu achetait le même refus au prix fort. Le défaut avait **trois portes**, dont une absente de la conception et trouvée en revue adversariale : le repli JSON-mode (Ollama, Perplexity), où `json_recovery` est appelé DÉLIBÉRÉMENT, son propre commentaire nommant la troncature comme un cas qu'il « gère » — l'objet raccourci revenait sans la moindre erreur d'analyse. **(1)** UN prédicat lit le verdict du fournisseur lui-même (`infrastructure/llm/output_truncation.py`), cinq formes lues dans les adaptateurs installés et épinglées par fixture ; le doute n'invente jamais un refus. **(2)** Il s'exécute aux TROIS portes, avant tout sauvetage, et lève `StructuredOutputTruncatedError`. **(3)** **Une réponse complète n'est jamais refusée** : le verdict n'est consulté qu'une fois qu'aucune réponse complète n'existe — une charge utile que le fournisseur a analysée sans réparation est syntaxiquement close, donc le budget a arrêté le flux APRÈS la fin de l'objet et rien n'a été perdu ; la refuser coûterait un document valide et une seconde facture. Les deux sens sont épinglés. **(4)** **Rien ne la rejoue** : le wrapper la relance sans seconde tentative, et les deux rejeux applicatifs du rayon d'action sont fermés de la même façon — le validateur sémantique (dont le rejeu existe pour un taux résiduel de réponse VIDE, qu'une seconde tentative met au carré, et qui ne peut pas compléter ce qu'un budget a coupé) tombe désormais tout de suite en fail-open ; et `meetings/processing`, qui marquait toute `StructuredOutputError` en `transient=True`, si bien qu'une synthèse coupée était re-drivée par le moissonneur jusqu'à épuisement du budget de tentatives, chacune payée — une troncature est PERMANENTE : code `synthesis_too_long`, `transient=False`, six langues, la doctrine des codes permanents d'ADR-258. **(5)** Le refus est ACTIONNABLE : l'outil document nomme le budget et propose de raccourcir ou de scinder ; aucune carte, aucun faux succès. **(6)** Le service document passe `user_id` à la porte structurée — `resolve_owner` lit `config.metadata.user_id`, que LangGraph ne renseigne jamais (seul `thread_id` y est fusionné, prouvé par sonde), si bien qu'un « appelant borné » accepté par son NOM ne portait aucun propriétaire. **Limite affirmée** : le chemin de raisonnement en flux n'a pas de message brut en main et ne peut pas consulter le verdict — il ne peut pas non plus raccourcir en silence, un appel d'outil coupé échouant à l'analyse, ce qui le renvoie au chemin bufferisé où le verdict EST lu. Amende ADR-220, ADR-226, ADR-258, ADR-272.

---

### ADR-274 : la facture appartient au renderer, le sens appartient au modèle

**Fichier**: `docs/architecture/ADR-274-Document-Craft-Renderer-Owned-Model-Semantic.md`

**Décision** : constat propriétaire du 2026-09-08 — « la génération fonctionne, mais les mises en page sont basiques, peu travaillées et peu esthétiques ». Le prompt était suspecté ; il ne l'était pas : il demandait déjà plus que le système ne savait rendre. **Deux plafonds, tous deux dans le code, tous deux MESURÉS** : le vocabulaire (`Slide` = titre + puces + notes, `SectionBlock.kind` à quatre valeurs — douze diapositives, douze fois la même mise en page quoi qu'on demande) et le rendu (`pptx.Presentation()` = le gabarit **4:3** d'origine, **2 layouts utilisés sur 11** ; `docx.Document()` = **4 styles nommés sur 164** ; le PDF sans **aucune** feuille de style ; xlsx en gras et largeur de colonne). Six autres mesures ont façonné la conception : **une diapositive dense débordait de 1 087 pt** (le drapeau autofit ne change rien, PowerPoint ne calcule rien à l'ouverture, et `fit_text` de python-pptx choisissait 21 pt là où PowerPoint mesurait encore 364 pt de débordement) ; **Excel REFUSE d'ouvrir** un classeur dont un tableau a un en-tête dupliqué ou vide, ou un nom avec un espace — openpyxl écrit les trois sans broncher ; **`TOC Heading` de Word hérite de `Heading 1`**, donc « Sommaire » devient le chapitre 1 et figure dans le sommaire qu'il introduit ; **MuPDF repeint un rectangle d'en-tête fantôme** en haut de chaque page de continuation quand un `th` porte un fond sous `border-collapse` ; **PyMuPDF embarque ses polices** (CJK compris), sous Windows comme dans le conteneur Linux ; et **`SectionedContent` était hors du mode strict OpenAI depuis ADR-226** (profondeur 6 > 5, un tableau dans un bloc répété coûtant deux niveaux) sans que personne ne l'ait mesuré. **(1)** Le modèle dit ce qu'une chose EST — une séquence ordonnée, une citation, un avertissement, un séparateur, une mise en regard, des données — et le renderer décide comment elle est dessinée : aucun catalogue de gabarits, aucune identité visuelle, aucun `.dotx`/`.potx` vendorisé. **(2)** Mécanismes NATIFS, jamais d'imitation : styles nommés et champs `PAGE`/`NUMPAGES` et définition de numérotation côté Word, layouts et placeholders du gabarit côté PowerPoint, cellules typées et tableau nommé et volet figé côté Excel, feuille de style et signets et numéros exacts côté PDF. **(3)** **Rien ne déborde, par construction** : `fit.py` mesure le texte avant de le poser, CALIBRÉ contre PowerPoint 16 sur 54 combinaisons — toute largeur moyenne de glyphe dans **[0,426 ; 0,493] em compte les lignes EXACTEMENT** (0,46 retenu près du milieu, la chaîne de calibration étant plus étroite que de la prose à majuscules), et `lignes × corps × 1,2 + paragraphes × corps × 0,2` fois **1,05** transforme 18 sous-estimations en **zéro** ; ce qui ne tient pas est réduit jusqu'à 14 pt puis SCINDÉ en « Titre (2/3) », une puce trop longue étant coupée aux phrases, jamais rognée. **(4)** Toute incohérence est RÉPARÉE, aucune n'est refusée (ADR-184) : niveaux bornés, blocs vides retirés, markdown égaré converti, numéros de titres écrits par le modèle retirés quand le renderer numérote, lignes inégales ÉLARGIES (tronquer perdrait ce que le modèle a écrit), en-têtes rendus uniques ; la borne sur `level` est RETIRÉE du schéma — une borne fait échouer tout le document, trois fois payé, pour un titre (leçon d'ADR-269). **(5)** UN prédicat décide l'appareil du document long — sommaire, numérotation, saut de page avant chaque partie basculent ENSEMBLE — et un appelant qui met déjà son document en forme épingle `structure="plain"` : **les comptes rendus de réunion le font** (décision propriétaire), gagnant l'en-tête courant, la pagination et la facture des tableaux tout en gardant leur forme. **(6)** Le sommaire Word est PRÉ-RENDU sans numéros de page (commutateur `
`, liens actifs) — le champ est vide tant que Word ne l'a pas recalculé, si bien qu'un document ouvert et imprimé aussitôt afficherait sinon une page de sommaire blanche ; F9 le régénère à l'identique. **Le sommaire du PDF porte des numéros EXACTS**, parce que nous paginons nous-mêmes : le corps est mis en page UNE fois — il ne contient jamais le sommaire, donc rien de ce qu'il fait ne peut bouger — puis le liminaire est remis en page avec les numéros qui en découlent, et les deux sont concaténés, ce qui donne au passage sa propre page au sommaire. **(7)** Ce que le renderer APPLIQUE, le prompt PUBLIE (ADR-184) : densité des diapositives, longueur des puces, seuil du sommaire et budget de longueur dérivé du `max_tokens` du slot voyagent en placeholders lus des réglages, chacun avec la conséquence de son dépassement. **(8)** A4 par défaut ; les diapositives sont en **16:9 PAYSAGE**, obtenu en mettant à l'échelle les transformations PROPRES du gabarit — écrire une position héritée fige un placeholder à une hauteur nulle. **(9)** Neutre par conception (encre sur papier, niveaux de gris, une bande, un filet) : `typography.py` est le seul endroit où un look se décide, donc un thème futur serait la paramétrisation d'un module et non une passe dans cinq renderers. **(10)** Le vocabulaire coûte au format diapositives son mode strict, et c'est assumé : l'alternative — hisser les tableaux à la racine et les référencer par index — achèterait le mode strict en confiant au modèle un index qu'il peut se tromper, ce qu'ADR-184 refuse ; le verdict est MESURÉ et ÉPINGLÉ par famille pour qu'il ne bouge plus en silence. **Un glyphe pleine chasse vaut un em PAR DÉFINITION** — toutes les mesures de calibration étaient latines, et compter un idéogramme à 0,46 em mesurait une diapositive chinoise dense à 238 pt là où elle en prend 442 dans un cadre de 356, l'oracle de débordement étant d'accord avec le renderer puisque tous deux lisent ce module ; PowerPoint tranche (**2 débordements dont un de 124,1 pt sous la largeur latine, 0 sous la corrigée**), la règle est une généralisation stricte (une chaîne latine mesure toujours `len × 0,46`) et UNE notion de largeur sert désormais les trois endroits qui en mesurent une. **Le budget de longueur est PAR FAMILLE** : mesurés, prose et diapositives convergent à 0,69 mot par jeton de sortie et un classeur à 0,25, si bien qu'un facteur unique publiait un nombre **2,2× trop généreux pour un tableur** — le piège d'ADR-184 retourné contre son auteur. **Mesuré après implémentation** (`task documents:corpus:measure`, Office 16 sur le corpus de 13 documents) : **0 débordement sur 69 diapositives**, toutes en 960 × 540 pt ; Word calcule ses champs (`Page 1 / 10`, et `第 1 页，共 7 页` sur le document chinois), le sommaire existe sur les documents longs et pas sur les courts, les titres sont numérotés `[1] [2] [3]` ; Excel ouvre chaque classeur sans réparation. **Coût** (tiktoken) : le prompt passe de **855 à 1 234 tokens** et le schéma de 193–431 à 259–597 ; l'estimation publiée du manifeste est relevée de 2 000 à 2 500 plutôt que laissée dériver. Amende ADR-226, ADR-184, ADR-085.

---

### ADR-276 : le workboard — un ticket a un cycle de vie, un porteur et un résultat

**Fichier**: `docs/architecture/ADR-276-Workboard.md`

**Décision** : trois surfaces hébergeaient jusqu'ici les demandes qui ne tiennent pas dans un tour de conversation, et chacune avait été construite pour autre chose — les tâches d'un fournisseur (ni cycle de vie, ni porteur, ni résultat), les rappels (une poussée à un instant, rien ne subsiste après la sonnerie) et les routines (une instruction répétée, rien qui se termine). Le workboard tient ce qu'aucune ne tient : **une unité de travail avec un cycle de vie, un porteur et un résultat**. **(1)** Sept colonnes, dont l'ordre d'affichage est DÉRIVÉ de l'énumération et jamais re-listé ; quatre priorités ; des sous-tickets qui sont de vrais tickets enfants sur UN niveau, donc exécutables par LIA, la suppression du parent les emportant mais pas sa clôture — une cascade de clôture « terminerait » en silence un travail que personne n'a fait. **(2)** UNE ligne par ticket, partagée : le tableau de U est « propriétaire = U OU assigné = U », donc les deux côtés ne peuvent pas se contredire sur son état. **(3)** **Un assigné NUL signifie « le propriétaire le détient », et la clé étrangère est `SET NULL`** : quatre chemins suppriment durement une ligne `users` (purge du démonstrateur, nettoyage des comptes non vérifiés, suppression administrateur, suppression RGPD) et **un seul passe par la purge de compte** ; `CASCADE` détruirait le ticket du PROPRIÉTAIRE au départ de son pair — son propre travail, sur son propre tableau — et `RESTRICT` bloquerait les trois chemins qui ne libèrent pas, transformant un filet de sécurité en panne d'exploitation. La purge libère EN PLUS explicitement, la suppression de compte SCRUBBANT la ligne `users` au lieu de la supprimer : deux mécanismes, chacun couvrant ce que l'autre ne peut pas. **(4)** **Aucune contrainte `CHECK` ne croise deux colonnes qu'une clé étrangère peut modifier.** La première conception stockait `peer_connection_id` avec un `CHECK` le liant à l'assigné ; **prouvé sur PostgreSQL réel le 2026-09-09**, supprimer le compte du pair déclenche DEUX actions de clé étrangère indépendantes sur la même ligne, appliquées séquentiellement, et le `CHECK` rejette l'état intermédiaire — quel que soit le sens retenu, l'ordre des cascades n'étant pas garanti — tandis que `CHECK` ne peut pas être `DEFERRABLE`. La colonne a disparu, pas l'invariant : la PAIRE (propriétaire, assigné) EST la connexion, `peer_connections` ne tenant qu'une ligne par paire à vie, et le service la revérifie à chaque écriture (règle D8 des pairs). Défaut **invisible pour 20 tests unitaires verts et pour le rejeu de migration** — trouvé à la première insertion réelle suivie d'un `DELETE FROM users`. **(5)** La page et ses compteurs sortent du MÊME énoncé filtré (ADR-185), toute colonne déclarée est présente à zéro si vide, et toute lecture paginée se termine sur la clé primaire — sans ordre total, deux tickets à clé de tri égale se répètent ou disparaissent à la frontière d'une page. **(6)** Un ticket affecté à LIA est exécuté hors tour par le moteur des routines (lot 2), et **le run archive ses deux lignes comme tout tour, marquées cachées, la LECTURE de l'historique les excluant** : ne rien archiver aurait laissé la ligne de décision d'ADR-263 avec ses deux pointeurs nuls, indistincte d'une conversation supprimée. **(7)** **La volumétrie cachée est BORNÉE et VISIBLE** (2,4 Ko par ligne mesurés, ≈ 5 Ko par run) : durée de vie liée au ticket, plafond de runs par ticket, rétention après clôture, deux jauges et une phrase dans les réglages — une croissance que personne ne voit est une croissance que personne n'arrête. **(8)** Un run qui a besoin de la personne S'ARRÊTE, le refus étant lu depuis un signal STRUCTURÉ et jamais depuis la prose, et une notification porte un lien `?intent=` pour terminer dans le chat. **(9)** **La délégation inter-comptes est refusée des deux côtés** : l'instruction appartient au propriétaire, et l'exécuter sans surveillance avec les outils de l'autre compte mettrait les données de ce compte dans un commentaire que le propriétaire lit. **(10)** Un ticket se nomme par identifiant ou par titre UNIQUE plié, l'ambiguïté étant refusée et jamais devinée ; et **aucun filtre SQL ne précède le pliage** — mesuré, un `ILIKE` sur le besoin brut est PLUS STRICT que le pliage qu'il alimente et écarte exactement les lignes que celui-ci existe pour attraper (« reserver la salle » ne trouvait rien quand le tableau portait « Réserver la salle ») : **un filtre de rétrécissement est plus permissif que le décideur, jamais moins**. **(11)** Pas d'entrée dans l'en-tête, déjà à sept destinations : le tableau s'atteint depuis les réglages, depuis chaque notification de ticket, et le hub porte un badge « a besoin de vous » — un ensemble à décider, jamais un décompte de tout ce que le compte possède. **(12)** `WORKBOARD_ENABLED` vaut `true` par défaut, contrairement aux autres drapeaux de programme : le tableau est une surface centrale, pas un sous-système optionnel. Amende ADR-263, ADR-185, ADR-184, ADR-180, ADR-117.

---

### ADR-277 : mes raccourcis — des sections de réglages épinglées, sur tous les écrans

**Fichier**: `docs/architecture/ADR-277-Settings-Shortcuts-Dock.md`

**Décision** : une personne épingle jusqu'à cinq sections des réglages et un dock flottant les présente sur tous les écrans du tableau de bord, à un clic. **(1)** Ce qui est épinglé appartient au COMPTE (`users.settings_shortcuts`, JSONB nullable, remplacement complet) ; où se trouve le dock et s'il est replié appartiennent à l'APPAREIL (`localStorage`, hors du registre de purge comme les yeux). **(2)** Le backend garde la FORME des jetons — un slug borné, unique, plafonné — jamais leur liste : le vocabulaire est celui du frontend, qui écarte à la lecture une section renommée depuis. **(3)** Le plafond est un réglage PUBLIÉ dans la réponse (ADR-184) ; la sixième case est `aria-disabled`, jamais `disabled`. **(4)** La liste voyage avec le profil (`/auth/me`), lue par un validateur tolérant : le dock ne fait aucune requête. **(5)** UNE mécanique flottante, `useEyesDrag` généralisé en `useFloatingDrag` — une pression sur un contrôle n'est jamais un glisser, les flèches ne déplacent que la surface qui porte le focus. **(6)** Le sélecteur est le modèle du shell sous `useSettingsAvailability`, extrait de la page des réglages : il ne peut offrir une section que le shell n'affiche pas. Amende ADR-227, ADR-172, ADR-184, ADR-252.

---

### ADR-278 : la fenêtre de contexte appartient au poste, pas à l'instance

**Fichier**: `docs/architecture/ADR-278-Per-Slot-Context-Window.md`

**Décision** : `OLLAMA_NUM_CTX` était UNE variable d'instance servie à tous les tags Ollama quelle que soit leur taille — mesuré en production le 2026-09-10, un déploiement à 128 000 demandait la même fenêtre à un modèle de 4 B et à un de 27 B. La fenêtre devient une colonne du POSTE configuré (`llm_config_overrides.context_window`, nullable), pré-remplie par ce que le serveur dit du modèle : deux postes sur le même tag peuvent légitimement différer, et l'opérateur règle le nombre là où il VOIT de quel modèle il parle. **(1)** La découverte lit `GET /api/tags` d'abord — mesuré sur Ollama 0.33.2, il porte capacités ET `context_length` pour les treize tags, cloud compris — et n'appelle `POST /api/show` que pour un tag décrit incomplètement, ce qui retire treize appels du démarrage. **(2)** Un tag que NI l'un NI l'autre n'a décrit ne reçoit AUCUN profil et est nommé : `/api/show` répond vide pour quatre tags cloud sur huit, et ce silence était publié comme « sans outils, sans pensée, plafonné au palier local » — un profil `discovered` qui GAGNE ensuite sur toute ligne de catalogue éditable. **(3)** Un tag cloud se reconnaît à son HÔTE (`remote_host`), jamais à un suffixe `-cloud` : le plafond protège la VRAM de CETTE machine, donc un tag local est plafonné et un tag cloud garde toute sa fenêtre. **(4)** `get_effective_context_window_for_slot` est le point d'entrée unique des quatre lecteurs (compaction, budget ReAct, middleware de résumé, synthèse de réunion), tous partis d'un poste ; le lecteur par modèle reste pour les surfaces de catalogue. **(5)** L'invariant d'ADR-267 tient partout — ne rien demander n'est pas neutre : le serveur choisit alors un palier VRAM et tronque un prompt plus long en silence, donc une fenêtre inconnue demande le plafond. **(6)** La migration reporte la variable sur les postes Ollama UNIQUEMENT si elle est encore présente à la montée. Amende ADR-267, ADR-244.

---

### ADR-279 : un fichier produit par LIA appartient à la personne, pas à la conversation

**Fichier**: `docs/architecture/ADR-279-Generated-Assets-Gallery.md`

**Décision** : les images, les documents et les captures de navigateur que LIA produit étaient écrits dans `attachments` **strictement indistinguables d'un téléversement** — rien ne les listait, le seul chemin vers un rapport était la conversation qui l'avait produit, et `ConversationService.reset` appelait `delete_all_for_user(user_id)` sans filtre : « effacer cette conversation » effaçait aussi les douze images générées la semaine précédente, ailleurs. **(1)** `origin` nomme le producteur sur un vocabulaire CLOS — `upload` et `GENERATED_ORIGINS` partitionnent l'énumération, ce qu'un test vérifie — et une valeur inconnue n'est **jamais** lue comme « générée » : le côté prudent est celui qui n'accumule pas d'inconnus dans une galerie. Deux colonnes l'accompagnent : `title` (le nom que le producteur connaît, jamais l'UUID sur le disque) et `conversation_id` en `SET NULL` (supprimer une conversation ne détruit pas ce qu'elle a produit). **(2)** Le report lit la FORME que les producteurs ont laissée — préfixe `generated_` / `browser_` en TÊTE de nom pour les images et les captures — et **ne devine pas au-delà** : un document est nommé d'après la demande de la personne, donc son nom ne prouve rien et il est reconnu depuis les métadonnées du message qui pointe dessus ; ce qu'aucune passe ne prouve reste `upload`, ce que la ligne était déjà. **(3)** **La réinitialisation d'une conversation CONSERVE les fichiers générés** (arbitrage propriétaire) : `delete_all_for_user` prend `origins`, la réinitialisation passe `{upload}` — la doctrine d'ADR-260 appliquée aux fichiers, une purge retire ce que sa famille déclare et jamais ce qui lui ressemble. Le TTL, lui, expire toujours tout : **la rétention reste à 24 h**, la galerie rend l'échéance visible sans la repousser. **(4)** Trois galeries, les captures ayant la LEUR (arbitrage propriétaire) : elles ne se cherchent pas comme un visuel qu'on a demandé. `upload` n'est pas listable — `GalleryFilters` le refuse à la construction plutôt que de renvoyer une page vide. **(5)** La page et son total sortent du MÊME `WHERE` (ADR-185), tout tri se termine sur la clé primaire, et le besoin de recherche est une DONNÉE passée par `escape_like` — un `_` non échappé matche toutes les lignes. Le plafond de page et la taille par défaut sont PUBLIÉS dans la réponse (ADR-184). **(6)** **La garde de capacité descend du routeur vers `POST /attachments/upload` seule** : téléverser et consulter sont deux capacités qui partagent une table, et couper la première fermait la porte sur des fichiers que la personne garde légitimement et ne pouvait plus supprimer ; le garde de câblage apprend à lire une garde posée au niveau de la ROUTE. **(7)** « Supprimé » veut dire parti : un identifiant qu'on ne possède pas, un téléversement, ou une ligne que le nettoyage a retirée entre le listing et le clic sont ÉCARTÉS et jamais comptés (ADR-185). **(8)** Pourquoi un fichier est parti s'enregistre sur l'étiquette que le tableau de bord groupe DÉJÀ — `reason` devient un vocabulaire clos `expired | conversation_reset | user_deleted`, **aucune métrique nouvelle** : avant, une personne qui vidait sa galerie était indistinguable d'une réinitialisation. Amende ADR-226, ADR-185, ADR-184, ADR-260.

---

### ADR-280 : un commutateur par capacité, et une carte qui ne ment pas

**Fichier**: `docs/architecture/ADR-280-Complete-Capability-Control.md`

**Décision** : le panneau d'administration offrait DOUZE capacités pendant que le produit livrait, en plus, un tableau de travail, des journaux, des habitudes, des notifications proactives, des connexions entre comptes, un profil psychologique, des canaux externes, des fils ouverts, la mémoire à long terme, le suivi des intérêts, des débriefs de relation, la délégation à des sous-agents et un Python éphémère — **treize fonctionnalités qu'un exploitant ne pouvait ni voir ni couper sans redéployer**, la dérive même contre laquelle ADR-085 a posé ses assertions, pointée sur le registre des commutateurs. **(1)** `PlatformCapability` passe à vingt-cinq membres et la partition est vérifiée **dans les deux sens** contre une liste tenue par une PERSONNE : une fonctionnalité livrée sans commutateur échoue, un commutateur que personne n'a décidé échoue aussi ; chaque spécification est vérifiée pièce par pièce (le plafond existe vraiment sur `settings` — un attribut mal orthographié se lirait `False` et couperait la fonctionnalité pour toujours —, la clé porte le nom de SA capacité, le libellé suit la convention). **(2)** **Un commutateur retire la CAPACITÉ, jamais l'ARCHIVE** — la leçon d'ADR-279 généralisée : quand la route EST la capacité, la garde est sur le routeur (huit cas) ; quand la capacité est un acte de FOND qui remplit une archive qu'on continue de lire, la garde est sur l'ACTE et le routeur de l'archive reste ouvert (mémoire, intérêts, débrief, plus les deux sans routeur). Couper la mémoire arrête l'apprentissage ; chaque mémoire apprise reste lisible et supprimable. **(3)** Les cinq points d'étranglement lisaient `settings.<flag>` en direct, donc le commutateur de l'exploitant n'y changeait rien : ils lisent `is_capability_enabled` **à l'appel**, parce qu'un commutateur basculé après le démarrage doit prendre effet sans redémarrage (deux sont devenus asynchrones pour cela). **(4)** **Le nœud dit ce qu'il compte** : mesuré le 2026-09-10, l'étoile clé `relations` et libellée « Relations » comptait des lignes `OpenLoop` et pointait vers la section « fils ouverts » — une personne lisait « Relations : 4 » pour quatre fils non terminés. Le nœud devient `open_loops` ; une vraie entrée « Relations » revient comme nœud-COMMUTATEUR, sans compteur, la page Relations étant une LENTILLE sur les contacts et les messages qui ne possède aucune ligne — inventer un nombre est ce qu'ADR-185 interdit. **(5)** Cinq nœuds de plus (`open_loops`, `workboard`, `reminders`, `generated_files`, `relations`), deux capacités hors carte avec une raison écrite, et deux compteurs qui passent par le module qui POSSÈDE leur règle (`visible_predicate` du tableau, `GENERATED_ORIGINS` des fichiers) plutôt que de la réexprimer. **(6)** `enforced_on_routes` valait `route_enforced or service_enforced` : le champ nommait les routes et portait les deux, donc le panneau annonçait « appliqué sur les routes » d'une garde de service. Les deux sont séparés, et le panneau groupe ses vingt-cinq lignes en six familles **déclarées côté backend** — deux tables finiraient par ne plus être d'accord — dans l'ordre de la déclaration et jamais celui de la charge utile, une famille inconnue étant dessinée EN DERNIER plutôt que perdue : un commutateur invisible est pire qu'un commutateur mal placé. Amende ADR-216, ADR-085, ADR-185, ADR-279.

---

### ADR-281 : revenir à l'instant qui compte, et servir une veille à la minute

**Fichier**: `docs/architecture/ADR-281-Anticipated-Moments-And-Mail-Watches.md`

**Décision** : le battement autonome passe toutes les trente minutes sur un lot tiré au sort — il est PÉRIODIQUE, donc incapable de servir un INSTANT : une réunion importante finit à 15 h 00, la passe suivante tombe à 15 h 22 sur un lot dont la personne ne fait peut-être pas partie, et deux faits vérifiés dans le code l'enferment (la fenêtre calendrier part de `now` et regarde DEVANT, donc une réunion terminée est invisible ; le tick tire un lot et ne cible pas un compte). **(1)** Un « moment anticipé » est une ligne de `proactive_moments` — compte, genre, référence de source, échéance — d'identité unique `(user_id, kind, source_ref)` pour qu'une réunion ne produise jamais deux moments, revendiquée par `FOR UPDATE SKIP LOCKED` plus un `UPDATE` conditionnel dans la MÊME transaction avec jeton de propriétaire, et REVALIDÉE au moment d'être servie (le monde a pu changer : réunion annulée, personne qui a déjà écrit). Le registre des genres porte une assertion de complétude au démarrage (ADR-085). **(2)** Un moment est servi sous le contrôle d'éligibilité COMPLET — fenêtre, quota, cooldowns global, croisé et d'activité — et ne contourne que le lissage probabiliste et le rythme appris, comme un réveil ADR-261 et pour la même raison : **un instant ne se reporte pas**, reporter « comment s'est passée ta réunion ? » de deux heures c'est ne pas la poser. **(3)** La question est POSÉE, jamais l'évaluation : une question ouverte, au plus deux faits, jamais de jugement sur la personne, jamais deux moments empilés, jamais deux fois la même question. **(4)** La garde « en réunion » lit le calendrier avec un cache Redis et **ne compte pas une consultation sur un cache servi** — Redis a répondu, la boîte n'a pas été ouverte (ADR-263) — une lecture en échec étant déclarée `failed` et jamais lue comme un silence. **(5)** Contrôle AVANT exploitation (ADR-214) : chaque genre se coupe seul, la capacité entière a son commutateur (ADR-280), la lecture est tolérante et l'écriture stricte. **(6)** **Un faux négatif corrigé avant d'écrire du code** : la boucle de réponse existait déjà (`_inject_proactive_messages`), le lot prévu a été supprimé et remplacé par treize tests de caractérisation. **(7)** Une routine finie se FERME (`is_enabled = false`, `status = completed`) : `SeriesEnd` fermait déjà la série de trois façons qui aboutissent au même `next_trigger_at` NULL, mais rien ne fermait la LIGNE, restée « active » pour toujours et indiscernable d'une pause. Une colonne `expires_at` a été écrite puis RETIRÉE avant livraison — elle aurait été une seconde autorité sur la fin d'une routine à côté d'un `SeriesEnd` déjà stocké, validé, éditable et raconté en six langues. **(8)** Le réveil ADR-261 sert les veilles `mail_match` sur le delta Gmail qu'il tient déjà, AVANT le verdict du pré-filtre (qui répond « est-ce que ça vaut de réveiller le battement » quand une veille répond « est-ce que c'est ce que j'attends ») : il avance `next_trigger_at` et **ne fait pas tourner la routine** — l'exécuteur reste seul juge, un second exécutant serait une seconde autorité —, il ne déduplique pas une seconde fois (l'empreinte `condition_state` décide déjà), il ne peut que RAPPROCHER un passage et jamais ressusciter une série éteinte, et il arme AU-DELÀ du TTL publié du cache de recherche Gmail parce qu'un cache rempli avant l'arrivée du courriel répondrait « non remplie » et **ce verdict consomme l'armement**. **(9)** Les deux lectures de « ce courriel correspond-il » sont inévitables (le réveil tient une ressource brute, l'exécuteur une projection d'affichage) donc ÉPINGLÉES par un test de concordance sur onze courriels, la seule divergence légitime étant écrite plutôt que découverte. **(10)** La puce « Surveiller » du briefing ÉCRIT là où ses voisines ouvrent le chat — le chat ne peut pas composer une veille — sur l'EXPÉDITEUR et jamais le sujet, avec une fin portée par le `SeriesEnd`, et elle **lit ce que le compte détient avant d'écrire** : deux courriels du même expéditeur sont ordinaires, et deux veilles identiques annonceraient une seule réponse attendue DEUX FOIS. Amende ADR-261, ADR-268, ADR-184, ADR-214, ADR-263, ADR-280.

---

### ADR-282 : une réponse conservée appartient à la personne, pas à la conversation

**Fichier**: `docs/architecture/ADR-282-Message-Bookmarks.md`

**Décision** : une réponse de l'assistant n'existait que dans la conversation qui l'avait produite — UNE par compte, réinitialisée souvent — et ne se gardait qu'en copiant ailleurs ou en téléchargeant un `.md`. Un **bookmark** la conserve DANS LIA : c'est une **COPIE, pas un pointeur** — la réponse (markdown ou document HTML `lia-response`, tel quel), la requête qui l'a produite et la date de la réponse sont copiées au clic dans `message_bookmarks`, les deux références gardées (`message_id`, `conversation_id`) étant en `SET NULL` pour ne servir qu'à la bascule de la bulle tant que la conversation vit. **(1)** La requête est résolue CÔTÉ SERVEUR sous `visible_only` (dépôt déclaré `VISIBLE_ONLY` dans `message_readers`) : le dernier message `user` visible avant la réponse, jamais la question synthétique d'une exécution hors tour (ADR-276) ; une notification envoyée de la propre initiative de LIA (`type` préfixé `proactive_`, préfixe déclaré une fois et lu par ses cinq écrivains) ne garde aucune requête plutôt qu'une fausse. **(2)** La bascule est idempotente par construction — index unique PARTIEL sur `(user_id, message_id)`, conserver deux fois répond 200 avec l'existant — et le plafond `BOOKMARKS_MAX_PER_USER` est publié parce qu'imposé (ADR-184), refusé par une classe à part dont la phrase est traduite en six langues. **(3)** Page et total exact viennent d'un même `WHERE`, l'ordre finit sur la clé primaire et date par la RÉPONSE, l'aiguille passe par `escape_like` (ADR-185). **(4)** `PlatformCapability.BOOKMARKS` garde le `POST` seul : lister, l'état, supprimer, exporter restent ouverts — un commutateur retire la capacité, jamais l'archive (ADR-279/280) ; le nœud `bookmarks` de la carte est compté et pointe sur l'ONGLET de la section des fichiers par une table dédiée, `CAPABILITY_SECTION` étant une bijection. **(5)** Cycle de vie déclaré (`USER_PURGED`, export `FULL`) ; une réinitialisation de conversation ne touche jamais un bookmark. **(6)** Le chat lit l'état UNE fois par montage (`/bookmarks/state`) via un contexte inerte hors fournisseur ; la forme est restituée par le composant de la bulle, et le `.md` suit le chemin du chat avec la requête citée en tête. Amende ADR-279, ADR-280, ADR-185, ADR-184, ADR-276.

---

### ADR-283 : un worker API a une anatomie déclarée et mesurée

**Fichier**: `docs/architecture/ADR-283-Worker-Memory-Anatomy.md`

**Décision** : la production tenait 6 Go pour ses deux API et Postgres cyclait à 95-99 % de son plafond cinq fois par nuit — rien n'était une fuite, tout était mesuré poste par poste sur l'hôte avant d'être touché. **(1)** Le superviseur uvicorn importait l'application (`uvicorn.main.run` → `config.load_app()`, 437 Mo pour aucune requête) : `python -m src.serve` pilote la même chaîne `Config → Server → Multiprocess` sans cet appel et n'importe rien de `src` (garde AST) — 34 Mo mesurés. **(2)** Une bibliothèque lourde est importée où elle sert : `LAZY_ONLY` déclare chaque coût mesuré (PyMuPDF 43 Mo par processus sur arm64) et un garde refuse tout import de premier niveau. **(3)** Le STT gardait un recognizer whisper PAR LANGUE demandée dans un singleton PAR WORKER (1,6 Go croissant à l'usage ; deux workers ont porté le conteneur de 4,2 à 7,0 Go sur 8 Gio) : cache borné `VOICE_STT_MAX_RECOGNIZERS` (1), LRU, rien chargé à la construction, une éviction rend la mémoire (−1 166 Mo mesurés). **(4)** Chaque worker publie ce qu'il tient — `lia_worker_memory_bytes{kind}` lue dans son `/proc/self/status`, une série par worker vivant — avec deux panneaux et l'alerte `ApiWorkerMemoryHigh` qui nomme le `pid` : cAdvisor voyait le conteneur passer de 2,5 à 5,3 Go en deux jours sans pouvoir désigner un processus. **(5)** Le budget de connexions F004 bornait la POINTE ; le PLANCHER (pool persistant × workers, 7 Mo de backend chacun, + `shared_buffers`) faisait 110 % du plafond Postgres : `persistent_total` entre au budget, un garde croise compose et profil (`plancher ≤ limite / 2`), le plafond passe à 2 Gio et le pool à 5 / 15 par worker. Amende ADR-119, ADR-123, ADR-148, ADR-184, F004.

---

### ADR-284 : un prompt énonce ce que le code impose, et rien d'autre

**Fichier**: `docs/architecture/ADR-284-Prompt-States-What-The-Code-Enforces.md`

**Décision** : l'audit des prompts du 2026-09-12 a trouvé toutes les gardes vertes et, derrière elles, six endroits où le modèle lisait quelque chose de FAUX, chacun prouvé par assemblage réel puis mesuré sur Docker dev. **(1)** Tout `{placeholder}` d'un fichier a un producteur et un fichier contenant `{{` est rendu par `.format()` — garde AST par fichier (`test_prompt_placeholders_are_produced.py`), qui a isolé exactement les trois placeholders du générateur HITL jamais remplis depuis la v1.0.0. **(2)** Une consigne par contexte, dans le fichier, émise seulement quand le contenu existe (`response_context_sections.txt`) : plus de `<RAGDocuments>` dans `<UserDocuments>` à consignes contraires, plus 246 jetons d'enveloppes vides par tour nu, plus de double échappement qui livrait `r={{x}}`. **(3)** La borne publiée est la borne imposée, depuis UNE constante : Brave publiait 20/50 au planner et rognait à 10 — l'ADR-184 pointé à l'envers. **(4)** Un nombre vient d'un réglage, un fait du code qui l'applique : bloc de délégation versionné (fini « 2-pass », « Wikipedia », « timeout 120 » et le NOM d'une variable d'environnement envoyé au modèle), `<Computation>` rendu seulement si le sandbox est lié ET allumé, fenêtres du briefing publiées par l'API et interpolées par les six locales. **(5)** La prose d'un prompt n'habite jamais un `.py` : douze sites migrés, un parseur `clé|gabarit` unique (`parse_prompt_sections`) pour trois copies, `load_prompt_with_fallback` supprimé avec ses deux replis. Corollaires : `get_language_name` normalise avant de nommer, quatre réglages `*_prompt_version` sans lecteur supprimés sous un garde, cinq lecteurs manuels de `usage_metadata` ramenés au lecteur unique (le heartbeat facturait le cache DEUX fois). Amende ADR-184, ADR-025, ADR-245, ADR-248, ADR-263, ADR-272.

---

### ADR-285 : une réponse courte se demande sans raisonnement, et une famille de modèles se déclare une fois

**Fichier**: `docs/architecture/ADR-285-Short-Answer-Without-Reasoning-And-One-Family-Declaration.md`

**Décision** : le slot `response` de production est passé sur `deepseek-flash` — le nom courant de DeepSeek-V4.1-Flash — et chaque rappel est arrivé sous la forme « 🔔 » et rien d'autre : le générateur demandait 150 tokens, le modèle réfléchit par défaut et compte la réflexion DANS `max_tokens`, donc 150 tokens de chaîne de pensée et une réponse vide (mesuré trois fois sur trois par le vrai chemin de code, en production). **(1)** Une réponse courte se demande sans raisonnement : `short_answer_config` déclare `none` par la couture ADR-245 là où le modèle peut s'arrêter, et n'applique le budget de réponse que là — un plafond qui inclut la réflexion n'est pas un plafond sur la réponse. **(2)** Une réponse vide ou coupée est un refus (ADR-275) : la phrase de repli part, l'avertissement nomme le modèle et le compte de tokens de réflexion, la dépense est conservée. **(3)** La famille DeepSeek à bascule se déclare UNE fois (`DEEPSEEK_THINKING_PREFIXES`), lue par la règle de profil, l'adaptateur et le détour `tool_choice` — trois copies privées d'un préfixe avaient laissé passer le nom courant : aucune échelle publiée, aucun interrupteur envoyé, le plafond V3 de 8 192 appliqué à un modèle qui sort 384 K ; l'échelle devient celle que le vendeur documente, `none/low/high/max`. **(4)** Le plafond de sortie est la ligne de catalogue (ADR-244). **(5)** Le nom courant voyage par migration ET par graine, tarifé par plages UTC, provenance `verified` comme la ligne de production ; un garde tient les deux sources égales. **(6)** Une seule lecture des métadonnées de réponse (`model_name_of_response`) — la neuvième copie facturait chaque rappel à un modèle sans nom. Amende ADR-245, ADR-267, ADR-244, ADR-275, ADR-272, ADR-223.

---

### ADR-286 : un résultat d'outil est projeté item par item sous un budget de tokens, et la coupe est annoncée

**Fichier**: `docs/architecture/ADR-286-Tool-Result-Projected-Per-Item-Under-A-Token-Budget.md`

**Décision** : en ReAct, le bloc de données d'un résultat d'outil était un dump JSON coupé à 8 000 CARACTÈRES (littéral, v1.16.0) ; un message Gmail `format=full` porte son arbre natif (en-têtes SMTP, MIME, base64 — 85 à 98 % de l'item, tokenisé à 1,45 caractère par token) AVANT ses champs lisibles, si bien que le modèle ne lisait ni sujet, ni id, ni date d'aucun e-mail (mesuré 2026-09-15 sur le checkpoint : 0 champ, 11 itérations, ~74 k tokens, réponse fausse sur 2 entrées sur 5). **(1)** Un résultat est projeté ITEM PAR ITEM (`render_data_block`) : la liste la plus lourde est paginée à la frontière des items, les scalaires voyagent entiers, un item admis est complet et le bloc reste du JSON valide ; au moins un item passe. **(2)** Le budget est en TOKENS, `min(react_tool_result_max_tokens, fenêtre du slot × react_tool_result_window_fraction)` (ADR-278), calculé une fois par passage du nœud. **(3)** La coupe est ANNONCÉE après la balise `</external_content>` (combien sur combien, le budget, la voie vers le reste) et COMPTÉE (`react_tool_result_truncated_total{tool_name}`, dashboard 20). **(4)** L'arbre natif ne quitte pas le builder : `build_emails_output` retire `payload` après ses deux lecteurs, et le manifeste cesse de publier `emails[].headers`, un chemin qu'aucun item n'a jamais porté. **(5)** `local_query_engine_tool` est réservé au pipeline (`execution_modes`), miroir de la règle ADR-249 : offert au ReAct sans son registre, il répondait « No registry data available » à chaque appel. **(6)** `resolve_reference` répond — `action_success(data=…)` levait `TypeError` à chaque résolution réussie depuis la v1.0.0. Amende ADR-070, ADR-248, ADR-256, ADR-184, ADR-249.
...
**Fichier**: `docs/architecture/ADR-287-Email-Content-First-Detail-Levels-Digests-And-Neutral-Message.md`

**Décision** : l'assistant raisonne sur le CONTENU de dix ou vingt e-mails (« résume mes non lus », « synthèse des newsletters de la semaine », les routines) — un modèle « liste puis clique » ne sert pas ces cas — et ce qu'il lisait était le format Gmail imité par deux autres fournisseurs (un `payload.headers` fabriqué par Graph et IMAP, un corps Graph en HTML brut, un corps IMAP aplati par une regex), chaque réponse citant tout son fil, un seul niveau de détail borné à 1 500 caractères, deux outils hérités morts. **(1)** `EmailMessage` est le vocabulaire neutre que les trois normaliseurs produisent (corps texte, jamais HTML ni base64 ; une seule conversion HTML → texte ; parité de signature des clients). **(2)** La citation et la signature quittent le corps au bord du client (`trim_quoted_reply`, marqueurs en six langues, gardiens : moins de 20 caractères restants ou réponses entrelacées ; `EMAILS_TRIM_QUOTED_REPLIES`). **(3)** `get_emails_tool(detail=metadata|summary|full, part, page_token)` : lister sans corps, lire un corps paginé au paragraphe (`body_part`/`body_parts`), ou raisonner sur des condensés ; pagination opaque des trois clients ; `result_size_estimate` publié comme ESTIMATION (ADR-185) ; manifeste, prompt d'agent et planificateur disent le coût de chaque niveau (ADR-184). **(4)** Un condensé par message (`EmailDigest` : gist, points clés, actions, catégorie, importance), calculé UNE fois par le slot `email_digest` sans raisonnement, mis en cache Redis 30 jours (famille `email:digest`, `USER_CACHE`, clé versionnée par langue et schéma), refusé par `spend_blocked` (ADR-272), un vol par clé, un échec laisse le corps (ADR-275), compté par `email_digest_cache_total{result}`. **(5)** La carte dessine « L'essentiel », « Points clés », « À faire » en six langues. **(6)** Les outils hérités, `EMAIL_TRUNCATION_RATIO` et `_extract_body_truncated` sont supprimés. **(7)** `mail-parser-reply` 1.36 mesurée sur le corpus (22/48, 1 gardien sur 2 ; elle coupe une formule de politesse comme une signature) contre 48/48 pour le trimmer maison — elle reste au dehors, la règle d'adoption est écrite. Mesuré en conditions réelles : « résumé de mes 6 derniers emails » en 2 itérations, 6 condensés, 3 934 tokens de résultat ; et la mesure a trouvé que la dépense des condensés n'atteignait AUCUN registre (`callbacks_before_filter: 0`) — le `RunnableConfig` du tour est désormais tendu à la porte structurée. Amende ADR-286, ADR-184, ADR-185, ADR-260, ADR-272, ADR-275, ADR-256.
---
...
**Fichier**: `docs/architecture/ADR-288-Independent-Drafts-Reviewed-One-At-A-Time.md`

**Décision** : mesuré sur Docker dev le 2026-09-16, « envoie un email à X … et un email à Y … » a produit deux brouillons, UNE question sur le premier, et « Valider » a envoyé les deux — le second jamais montré sur une carte, jamais modifiable ; « Modifier » ne réécrivait que le premier. La file `pending_drafts_queue` portait une sémantique de LOT écrite pour FOR_EACH (« ALL queued items are auto-confirmed too ») et appliquée à tout tour à plusieurs brouillons, quel que soit le type, en pipeline comme en ReAct ; un lot mélangé rendait un événement en « 📧 Email » sans libellé. **(1)** Le producteur déclare le mode : `pending_drafts_grouped` n'est vrai que pour un lot pré-approuvé (`is_pre_approved_lot` : tous les brouillons sont des items `<étape>_item_<n>` d'un FOR_EACH approuvé dans CE tour, d'un seul type). **(2)** Une suite se consomme un brouillon par interruption : la décision rejoint `confirmed_drafts`, le suivant devient `pending_draft_critique`, le compteur d'édition repart, aucun `draft_action_result` avant la dernière réponse — le routage existant boucle (doctrine replay-safe). **(3)** La dernière réponse règle tout (`settle`) : un `confirm_batch` ordonné où chaque entrée porte sa décision et son type ; une sortie terminale annule le brouillon à l'écran et RAPPORTE les brouillons en file comme annulés. **(4)** Le lot pré-approuvé est montré entier et confirmé entier, comme avant. **(5)** L'exécuteur saute une entrée annulée et la compte (`cancelled_count`) ; un lot sans type unique se déclare `batch` et chaque ligne du résultat porte son propre type. **(6)** La position est dite par le serveur en six langues (« Brouillon 2 sur 3 »). **(7)** Un ticket suit la même identité : l'approbation du premier le laisse passer, le second est redemandé. `hitl_dispatch_node.py` rétréci (756 → 698), les règles dans `nodes/draft_sequence.py`. Amende ADR-153, ADR-276, ADR-263, ADR-184, ADR-274.
---
...
**Fichier**: `docs/architecture/ADR-289-Draft-Surfaces-Drawn-From-One-Description.md`

**Décision** : trois retours du propriétaire sur la première suite de brouillons (2026-09-16) — l'aperçu de ce que le tour a préparé avait disparu, la carte de brouillon était du Markdown plat là où le chat dessine chaque donnée en `lia-card`, et le compte rendu d'un lot ne disait que des libellés (« ✅ Tout va bien ») sans dire à qui ni quoi. Les 27 renderers par type produisaient des lignes Markdown : la forme était décidée là où le contenu l'était. **(1)** Les renderers DÉCRIVENT la carte (`card_spec.py` : `Row`, `Note`, `Block`, `CardSpec`) ; `to_markdown_lines` la dessine dans la grammaire du lot 13 au caractère près (le filet golden n'a pas été régénéré), `card_html.to_html_card` en `lia-card` avec les classes des cartes de données, chaque valeur échappée, une seule ligne. **(2)** La surface est décidée par le run (`card_surface()` : origine hors-tour = ticket = Markdown aplati ; sinon le chat), jamais devinée. **(3)** La première interruption d'une suite porte `sequence_drafts` et la question s'ouvre sur « N brouillons à relire » plus une ligne par brouillon dans le vocabulaire de son type — sans clic de plus. **(4)** Le résultat est décrit (`describe_execution_result` → `ResultSpec`) et dessiné par surface ; chaque entrée d'un lot porte les champs clés du registre (`detail_fields`) moins ce que la ligne dit déjà, le premier champ texte devenant un extrait d'une ligne borné par `DRAFT_RESULT_EXCERPT_MAX_CHARS`. Amende ADR-276 lots 13 et 14, ADR-274, ADR-288, ADR-185.
---

### ADR-290 : le téléphone est un canal — LIA appelle la personne sur un numéro VÉRIFIÉ, sans carte, et ce qu'elle dit devient son propre tour

**Fichier**: `docs/architecture/ADR-290-Phone-As-A-Channel-Owner-Calls.md`

**Décision** : la carte de confirmation d'ADR-127 protège un tiers qui n'a rien demandé ; quand LIA appelle la personne ELLE-MÊME, celle qui confirmerait est celle qui décroche — à condition que le numéro soit PROUVÉ le sien. **(1)** L'identité est un numéro déclaré dans les réglages, chiffré, normalisé en E.164 par un seul module, puis vérifié par un appel qui lit un code (Redis, essais bornés, comparaison en temps constant) ; jamais une reconnaissance par nom, et l'outil tiers refuse désormais de composer le numéro vérifié du titulaire. **(2)** Un seul agent vocal, trois mandats (`THIRD_PARTY` cuit, `SELF` et `VERIFICATION` par override PAR APPEL — prompt, premier message, langue, plafond), rendus côté serveur par `str.format` avec la syntaxe `{{…}}` du fournisseur neutralisée dans les valeurs ; la permission d'override entre dans l'empreinte de l'agent et sa synchronisation est OBLIGATOIRE pour un override. **(3)** `call_me` est `reversible` avec sa raison — aucune carte par construction, et une routine peut le planifier — offert à un numéro vérifié seulement, avec le contexte du chat sous budget (mémoires, agenda, rappels, boucles ouvertes, derniers échanges), chaque lecture enregistrée sur la surface `phone_call`. **(4)** Le transcript est relayé comme le TOUR de la personne : synthèse par le chokepoint (`owner_confirmed`, `relay_message`), `stream_instruction` en mode `spoken_by_person` (source non automatisée, extractions selon les préférences, origine `phone_call` VISIBLE), outbox `RELAYING` réclamée avant le tour et réglée par mise à jour conditionnelle, lignes échues rendues au faucheur, huit verdicts comptés et dessinés, brouillons confirmés dans le chat. **(5)** Des outils en direct pendant l'appel, derrière `TELEPHONY_LIVE_TOOLS_ENABLED` : allowlist d'outils qui ne font que lire (vérifiée au boot), webhook tools créés une fois par connecteur et rattachés par appel via `tool_ids`, rappel sans session qui n'ouvre que pour un appel `SELF` actif avec un jeton DÉRIVÉ du secret de webhook, arguments validés par le schéma de l'outil, exécution bornée sous le délai du fournisseur, résultat projeté comme en ReAct. Non mesuré : aucun appel réel du titulaire n'a été placé. Amende ADR-127, ADR-263, ADR-276, ADR-179, ADR-184, ADR-284.
---

### ADR-291 : une réponse conservée est indexée comme un document d'un espace de connaissances

**Fichier**: `docs/architecture/ADR-291-Kept-Answers-Knowledge-Space.md`

**Décision** : un bookmark (ADR-282) restait une copie dans un onglet que LIA ne relisait jamais ; il est désormais PROJETÉ dans un espace de connaissances par compte géré par RÔLE (`rag_spaces.kind = 'bookmarks'`, créé à la première conservation dans la langue de la personne, exempt des deux plafonds — celui des bookmarks est la borne), jamais un espace système (sans propriétaire). La forme est celle des comptes rendus de réunion (ADR-258), pas celle d'une source synchronisée : le bookmark est l'ENREGISTREMENT, son document Markdown (titre daté, demande citée ou ligne de notification, date, réponse — un `lia-response` HTML converti par l'extracteur du pipeline) la projection, `rag_document_id` en `SET NULL`, un `index_state` qui ne dit que pourquoi il n'y a pas de projection, l'état exposé étant DÉRIVÉ du document tant qu'il existe. Une projection est RÉCLAMÉE avant tout effet (un `UPDATE` conditionnel, un `pending` plus vieux que la grâce du faucheur est repris), chaque porte laisse un état honnête (`disabled` capacités `RAG_SPACES`/`BOOKMARKS` lues à l'acte, `deferred` plafond de dépense par `spend_blocked`, `error` mort-lettré jamais rejoué par le balayage), le backfill roule sur le tick du faucheur (`infrastructure/scheduler/rag_maintenance.py`, faucheur d'abord), le document ne se déplace ni ne se supprime depuis l'espace (409) et un espace géré par rôle ne se supprime pas à la main (403, réunions comprises), `kind` rejoint la forme de fil. Deux défauts trouvés en chemin : `markdownify(strip=["script","style"])` GARDAIT le contenu des balises, et le document d'une étiquette Gmail était déplaçable. Mesuré sur Docker dev : rappel d'une réponse conservée à 0,762, coût dans le grand livre, backfill `selected=1 indexed=1`. Amende ADR-282, ADR-258, ADR-262, ADR-279/280, ADR-272.

---

### ADR-292 : le portrait de l'utilisateur lit quatre sources, et dit lesquelles

**Fichier**: `docs/architecture/ADR-292-Portrait-Reads-Four-Sources.md`

**Décision** : le portrait compilé par la consolidation du journal — injecté dans treize flux — ne lisait ni les souvenirs, ni les centres d'intérêt, ni les habitudes apprises, ni les débriefs de relation, alors que le schéma d'ADR-079 et une docstring l'affirmaient. Les quatre sources sont OFFERTES à `journals` par un seam (`domains/shared/portrait_sources.py`, vocabulaire fermé, précédent `consultation_sink`) parce que `interests` et `relations` importent déjà `journals` ; chaque domaine installe son lecteur, le boot INSTALLE explicitement et refuse un registre incomplet (`StartupCompletenessError`). Un lecteur répond `used | empty | disabled | unavailable` avec un total EXACT et une page bornée, sous ses trois portes lues à l'acte (plafond, commutateur, préférence) — les souvenirs épinglés puis les plus importants sur TOUT l'ensemble, les intérêts par solde de signaux, les habitudes par `load_consumable_profile` (fenêtres en pause ou bloquées exclues), les débriefs READY sous l'âge publié et DATÉS. L'assembleur lit UNE source à la fois sous des budgets réglés, écarte une section ENTIÈRE au-delà du plafond global, compte chaque réponse (`journal_portrait_sources_total`). Le prompt reçoit quatre sections rendues seulement quand elles existent et un STEP 7 réécrit (matériau d'une synthèse, jamais une liste de faits, jamais mot pour mot, jamais mentionné, « une source absente est INCONNUE, pas vide ») ; les budgets « ~150-220 » / « ~50-70 jetons » écrits en prose depuis ADR-079 deviennent deux réglages (ADR-184). La provenance est persistée AVEC le portrait (`users.journal_portrait_sources`, lue par une porte tolérante) et dessinée sous lui ; la fraîcheur d'une source rouvre l'éligibilité (`EXISTS` par sonde, entrées en jointure externe). Mesuré : une consolidation réelle, quatre sections aux totaux exacts, un portrait qui synthétise sans recopier, 0,0016 € au grand livre — et un dépassement du budget énoncé d'environ un tiers (386 jetons pour 300). Amende ADR-079, ADR-088, ADR-184, ADR-269, ADR-280, ADR-270.

---

### ADR-293 : la boucle ReAct lie ses outils par pertinence, le planner garde son catalogue

**Fichier**: `docs/architecture/ADR-293-React-Tools-Bound-By-Relevance.md`

**Décision** : les deux modes choisissaient leurs outils à l'opposé l'un de l'autre. Le pipeline rétrécit fort (scores sémantiques des domaines détectés, catalogue du planner à quatre outils en médiane, couverture de deux par domaine) ; la boucle ReAct prenait par doctrine TOUS les outils disponibles puis un plafond qui tranchait dans l'ORDRE D'ENREGISTREMENT — sur tout compte comptant plus d'outils que le plafond, les mêmes familles tombaient à chaque tour quelle que soit la question (workboard, pairs, téléphone, skills, bac à sable, et la totalité des outils MCP de la personne), et quatre-vingts schémas pesaient 26 155 jetons, 95 % du premier appel, portés à chaque itération sans qu'aucune métrique ne les compte. Rien ne classait les outils de la boucle alors que l'embedding de la requête était déjà payé par le routeur. Désormais UN embedding, DEUX lecteurs : les scores de domaines du planner sont inchangés à l'octet, et `rank_tools` ordonne du même vecteur TOUS les manifestes disponibles (MCP compris, par leurs vecteurs par requête), ordre porté dans `tool_selection_result["global_ranking"]`. La boucle lie par COMPOSITION, sans aucun nom d'outil écrit : les outils des domaines détectés, puis les `CATALOGUE_DOMAIN_COVERAGE_TOP_N` meilleurs outils de CHAQUE autre famille (la constante de couverture du catalogue, partagée — une famille que le routeur n'a pas nommée reste atteignable), puis les `REACT_TOOL_SEMANTIC_TOP_K` premiers du classement ; le reste est écarté par pertinence, compté ; le plafond reste un filet, par tri stable sur des rangs (priorité, couverture, reste). Sans classement ou à K = 0, le comportement antérieur à l'octet. Une liste déclarée d'outils « de base » a été écrite puis REFUSÉE : elle encodait l'usage d'un compte dans un produit multi-utilisateurs ; la couverture par famille dit la même chose structurellement. Le catalogue du planner ne change pas : son exposition est la sous-détection des domaines, que le classement global ne corrige pas (mesuré : un cas sur vingt). Mesuré sur 58 tours réels d'une instance dev : 68 outils liés en médiane au lieu de tous, 22 488 jetons de schémas au lieu de 35 471, et dans les trois tours où un outil appelé n'aurait pas été lié, sa famille l'était par ses meilleures portes. Deux histogrammes (`react_tools_bound`, `react_bound_tool_tokens`), un harnais `task react:selection:measure` qui ne contient les tours de personne. Amende ADR-070, ADR-248, ADR-256, ADR-184. **Amendement 2026-09-20 — une unité de liaison se lie entière ou pas du tout** : rejeu sur dev, `activate_skill_tool` lié par le classement et `run_skill_script` écarté (3 tours sur 3), la boucle activait un skill qu'elle ne pouvait pas lancer et répondait en prose (le motif prod sur les deux skills) ; `ToolManifest.binding_unit` (les quatre outils skills, `SKILLS_BINDING_UNIT`, gardé), l'unité prend le rang de son meilleur membre, une seule place de couverture, la queue sémantique entière, et le plafond ne coupe jamais dedans ; rejeu : 0 tour sur 3, 62 outils liés au lieu de 60.
---

### ADR-294 : un visage qui ne joue jamais deux fois la même chose — sourcils sur l'œil, parole générée, battements déformés, saynètes comptées en temps de repos

**Fichier**: `docs/architecture/ADR-294-A-Face-That-Never-Plays-The-Same-Twice.md`

**Décision** : le propriétaire a regardé l'avatar en marche : les yeux vivent, les sourcils et la bouche sont le point faible, les transitions accrochent, l'ensemble est PRÉVISIBLE — et « les séquences aléatoires, je ne les ai jamais vues ». Tout a été MESURÉ avant d'être touché (vrai Chromium sur le serveur dev, un enregistreur autour de `CSSStyleDeclaration.setProperty` qui journalise chaque écriture `--rig-*`, cinq minutes sur la page chat d'un compte jetable ; le rig sous horloge simulée ; l'historique psyché du propriétaire). Les saynètes JOUAIENT (six en dix minutes simulées, trois en cinq minutes réelles) et personne ne les voyait : l'attente de 45 à 120 s était tirée UNE fois à la construction du rig et comptée sur son horloge quel que soit l'état du visage — chaque navigation démonte le widget et remettait l'attente à zéro, chaque tirage tombé pendant une réflexion ou une réponse était perdu — et les scènes portées par le regard (0,14 em par unité, six pixels en grand) se lisaient comme un coup d'œil de plus. Désormais une `SketchClock` que l'HÔTE garde pour le document et remet à chaque visage vivant compte le SEUL temps de repos (première scène à 20-40 s de repos, les suivantes à 45-120 s, jamais l'une des trois dernières), et toute scène qui regarde de côté tourne la TÊTE après les yeux. Sourcils et bouche : 27 flashs d'« encre » 0,5 → 1 en cinq minutes (un élément d'interface qui s'allume, jamais un visage) → présence PLEINE, canaux `browA`/`mouthA` supprimés ; le sourcil épinglé au haut de la BOÎTE flottait un tiers d'œil au-dessus d'un dôme de joie → ancré au bord visible de la forme (`top: calc(oy·(1−sy) + lidTop·sy)`), d'un POIDS dérivé de son propre mouvement (`browS` : levé il s'effile, pressé il s'épaissit, à encre constante), qui se RAPPROCHE (`browX` : colère, peur, souci, chagrin) ou s'écarte (surprise), dans un groupe dynamique `brow` qui DEVANCE l'œil sur tout préréglage, couplé au sourire ; un geste unilatéral est écrit une fois et BASCULÉ d'un côté ou de l'autre (`flipTapes`). Bouche : dalle pleine conservée (arbitrage propriétaire), un glissement latéral `mouthX`, et une PAROLE GÉNÉRÉE (`rig/speech.ts` : syllabes, mots, pauses de mots et de phrases, une forme par mot, un accent sur un mot sur trois marqué par les sourcils — les deux, ou un seul — et un hochement) qui remplace la somme de sinus ; un motif qui atteint son cycle est RÉSOLU À NOUVEAU au lieu d'être rembobiné, donc une longue réponse ne boucle jamais. Timing : chaque attaque atteignait 90 % à ~100 ms sur le même ressort et rentrait sur la même dynamique de 650 ms → chaque performance est DÉFORMÉE (un pas global 0,85-1,15, puis par CANAL un jitter de temps ± 6 % et de taille ± 12 %, un facteur par canal pour qu'une tenue et son relâché restent joints), chaque mimique porte son PROPRE relâché (`withRelease`, une boudeuse s'attarde à 0,9 Hz, un smack claque à 3,5 Hz — et un défaut latent trouvé au passage : `tapeSpringIn` imposait le ressort d'une bande AVANT sa première clé), la cadence devient des PHRASES (silence 8-16 s, suite à 1,2-2,5 s une fois sur trois, jamais la mimique qui vient d'être jouée) et le visage RÉPOND aux yeux (`answerIn` sur 30 % des gestes). Transitions : la traction de la pose MONTE progressivement quand une bande rend un canal (fréquence de 10 % à 100 % sur 120 ms — le saut de cible faisait sauter l'accélération, une cassure en haut du mouvement ; jamais à l'attaque, jamais entre deux bandes, jamais sur un changement d'expression, sinon un sursaut sorti d'un visage triste est un sursaut émoussé), les organes partent avec les yeux (bouche 45 → 30 ms), et le clignement de masquage TIENT fermé au-delà de l'instant où l'hôte échange le visage (`maskBlinkTapes`, `blinkMask`) au lieu de rouvrir à 130 ms sur un échange à 140. Refusés : un bouton « rejouer », un croissant effilé, le fondu de tout changement de ressort, une cadence bimodale des gestes des yeux (un silence des YEUX se lit comme un gel). Amende ADR-264, ADR-252, ADR-240.
---

### ADR-295 : un document d'un espace de connaissances se joint à un message comme une copie, espace en pause ou non

**Fichier**: `docs/architecture/ADR-295-Knowledge-Documents-As-Message-Attachments.md`

**Décision** : la recherche du chat ne lit que les espaces ACTIFS, si bien qu'un document d'un espace en pause était inatteignable depuis une question. Le « + » du composeur devient un menu dès qu'il a une seconde entrée et propose les documents `ready` de TOUS les espaces de la personne (un espace en pause est nommé et badgé, jamais caché) ; un document choisi est COPIÉ dans le magasin des pièces jointes (`attachments/knowledge_copy.py`), son texte extrait par le pipeline des espaces (quinze formats, pas l'allowlist image/PDF de l'upload) sous le cap existant et la coupe ÉNONCÉE au modèle, la ligne créée en `origin = upload` — la réinitialisation la retire, le TTL l'expire, l'espace n'est jamais touché. Le listing transverse (`GET /rag-spaces/documents`) est une page et un total EXACT ; la route de copie porte les deux gardes (pièces jointes ET espaces). Deux dettes corrigées en passant : les libellés de l'indice étaient inlinés en Python et clés `zh` (le chinois recevait l'anglais). Mesuré sur dev : un `.docx` d'un espace EN PAUSE cité exactement dans la réponse, en pipeline et en ReAct ; la réinitialisation retire la copie et garde le document.

---

### ADR-296 : une pièce jointe de mail est lue — son texte quand elle en a, le slot de vision sinon

**Fichier**: `docs/architecture/ADR-296-Mail-Attachments-Read-By-Text-Or-Vision.md`

**Décision** : `get_emails` listait les pièces jointes et s'arrêtait là. UNE couture de téléchargement sur les trois clients (`download_attachment`, parité contractuelle ; Gmail en mixin extrait du client gelé, Graph par `contentBytes`, IMAP par l'INDEX de la partie que le normaliseur publie désormais), UNE sélection partagée (par handle, sinon par nom, l'ambiguïté refusée avec ses candidats) ; un handle Gmail PÉRIMÉ — mesuré : les `attachmentId` changent entre deux lectures d'un message — se résout par le nom, puis par l'unicité, sinon publie les handles COURANTS. `agents/emails/attachment_content.py` lit des OCTETS : MIME lu dans les octets magiques, texte extrait par le pipeline des espaces, image ou PDF sans couche de texte rendu en pages bornées et réduites pour le slot `vision_analysis` — dépense du TOUR (`invoke_with_instrumentation` avec la config du runtime), refus honnête sous plafond (`skipped_quota`) et sur sortie coupée (ADR-275). L'outil `get_email_attachment_tool` sert le texte en PARTS sous le budget des corps, enveloppé en contenu externe, chaque refus nommé ; `email_attachment_reads_total{route,outcome}` dessiné. Le slot `vision_analysis` rejoint le `Literal` `LLMType`. Mesuré sur dev : un PDF de 632 Ko extrait à 55 625 caractères ; un PNG rendu et remis au slot, refusé par le fournisseur sans clé — `failed`, jamais une lecture inventée.

---

### ADR-302 : un grant OAuth par compte vérifié, un consentement par fournisseur

**Fichier**: `docs/architecture/ADR-302-OAuth-Grant-Par-Compte-Et-Consentement-Groupe.md`

**Décision** : Google et Microsoft partagent chacun un consentement pour les services d'un même compte signé, en connexion comme en reconnexion. `oauth_grants` détient les tokens par utilisateur, client, fournisseur et subject ; les connecteurs restent indépendants. Le serveur choisit les scopes, valide le state/PKCE, l'ID token et son `at_hash` éventuel contre l'access token échangé, puis n'active que les services réellement autorisés sous verrou et transaction. Les comptes distincts restent séparés, les tokens historiques ne sont pas fusionnés par e-mail, le refresh est unique par grant et la déconnexion unitaire ne révoque pas le token commun. Amende ADR-021 ; les autorités OAuth MCP restent séparées. Le statut Google Testing peut encore imposer une expiration du refresh token.

---

### ADR-301 : sessions vocales — une politique par mode, quelle que soit la ligne

**Fichier**: `docs/architecture/ADR-301-Voice-Sessions-One-Policy-Per-Mode.md`

**Décision** : le téléphone (ADR-290) et le Live du navigateur (ADR-299/300) faisaient l'inverse l'un de l'autre — la voix au téléphone lisait les données et relayait la conversation à la FIN, la voix du navigateur déléguait chaque demande au chat PENDANT — et l'écrivaient trois fois. Une session vocale a désormais un MODE et un CARRIER, et la politique ne suit que le mode : `delegated` (« Live ») — chaque demande devient un tour de chat de la personne pendant qu'elle parle, avec HITL, registres, quotas, archive — ou `direct` (« Live direct ») — la voix lit pour la personne, ne fait rien, et ses mots sont relayés à la fin comme un message de la personne. UNE clôture (`infrastructure/scheduler/voice_session_closing`), appelée par `POST /live/sessions/{id}/end` ET par le webhook de fin d'appel, archive les échanges vocaux, dessine la carte aux chiffres exacts, enregistre la décision et apprend (délégué), ou synthétise et relaie dans une tâche qu'elle possède puis RÉÉCRIT la carte au règlement avec la dépense relue (direct). Ce que les deux lignes partagent est sorti des deux : `domains/voice_sessions/` (session, transcript, projection pinnée au navigateur par UN corpus, requêtes de résumé, bloc de délégation des mandats) et les runners de `infrastructure/scheduler/` (`voice_relay`, l'ancien `self_call_relay` prouvé octet pour octet ; `voice_delegation`, le pont serveur du téléphone, miroir règle pour règle de `lib/live/delegation.ts` : requête vide, le plus récent gagne par un marqueur Redis lu par le pont qui tourne, une question de LIA EST le résultat et la demande suivante reprend le run qui l'a posée, tour de la PERSONNE sans origine hors tour, attente bornée sous le timeout vendeur, jamais une exception vers la voix). Le choix appartient à la personne (`users.phone_call_mode`, Live par défaut : la trace dans la conversation est tout l'intérêt du canal), le mode COURU est une colonne de l'appel (`phone_calls.call_mode`, écrit au dial, lu à la clôture), Live n'est offert que si le fournisseur peut rappeler cette API (`telephony/callback.py` : hôte public, `TELEPHONY_CALLBACK_BASE_URL` pour un tunnel de dev) et l'UI publie le mode EFFECTIF plutôt que le choix nu (ADR-184) ; un outil de délégation refusé par le vendeur dégrade l'appel en direct en le disant. Mesuré sur le moteur RÉEL du vendeur (sans téléphone : le vendeur joué en HTTP contre l'API dev, puis une conversation texte sur le mandat et l'outil de production, rappel par tunnel) : `execution_mode: async` + `pre_tool_speech: force` laissent la voix annoncer l'appel et poursuivre ; passé `response_timeout_secs` la réponse tardive est PERDUE, donc le pont répond `timed_out` avant ; le transcript écrit un appel async comme une entrée agent VIDE avec `tool_calls`, son accusé, puis une seconde paire et la restitution — un échange qui contient une délégation est celui du graphe EN ENTIER, comme le tampon du navigateur. Une session directe navigateur n'est plus « rien d'enregistré » : ses tours restent dans son record (une liste Redis bornée) et deviennent à la fin le tour de la personne, la carte disant le sort (`scheduled` → `answered`…) et le fil rechargé sur un avis SSE ; le 409 `direct_not_archived` disparaît. Trouvé en route : `html_to_text` collait l'étiquette d'une liste de définitions à sa valeur là où le navigateur lisait déjà « Intitulé : … » (le corpus pinne les deux) ; `ConversationRepository.get_by_id` est celui de la CONVERSATION, la carte n'était jamais relue (`get_message`, test PostgreSQL).

---

### ADR-300 : un second fournisseur live — une couture, deux fils

**Fichier**: `docs/architecture/ADR-300-A-Second-Live-Provider-One-Seam-Two-Wires.md`

**Décision** : GPT-Live (OpenAI) rejoint Gemini Live sans qu'une ligne du pont, du bandeau, des registres ou de la carte de clôture ne bouge — la couture d'ADR-299 tient, le fil est celui du fournisseur. Chaque fournisseur DÉCLARE deux choses que la couture lit sans jamais brancher sur son nom : `connection` (`token` : le navigateur ouvre lui-même le socket avec le jeton du fournisseur ; `offer` : l'API frappe son PROPRE nonce à usage unique, gardé sur l'enregistrement de session, que le navigateur lui rend avec son offre SDP sur `POST /live/sessions/{id}/offer`, échangée sur la clé de la personne — « une clé, une connexion » tient sur les deux fils, un second échange sur le même nonce est 409 `credential_invalid`, une prolongation ne refrappe que sur `token`) et `delegation_wire` (`tool` : le modèle appelle `send_to_lia` ; `native` : le modèle délègue de lui-même, `session.delegation.created` porte un id et un `offset_ms` mais AUCUN texte — la demande est COMPOSÉE dans le navigateur depuis la transcription d'entrée, après une courte grâce). Le mandat est rendu avec UN verbe par fil et un bloc de délégation par fil et capacité. Le transport GPT-Live dérive ce que la couture attend des signaux du fournisseur : la transcription de l'assistant EST sa parole (son silence clôt l'énoncé et le tour), les mots de la personne pendant sont une interruption, un résultat est découpé sous la borne de 500 jetons par `commentary.append`, le micro se dit par `mute`/`unmute`, la session est prête au `session.started` MESURÉ sur le canal, `close()` attend `session.closed` borné, `session.usage.updated` n'est lu par personne. Les douze voix n'étant servies par aucun point de synthèse, l'échantillon EST une courte session live côté serveur sur la clé de la personne (silence streamé, phrase demandée par un append, sortie continue rognée de son silence) — le même socket est la sonde d'activation, qui juge donc modèle ET voix. La catégorie `live` est ADDITIVE : deux clés actives, les sessions ouvrent sur le choix du compte (`users.live_preferences.provider`), les réglages listent l'UNION des modèles par marque, choisir un modèle c'est choisir son fournisseur, les réflexes qu'un fournisseur décide seul ne sont pas offerts. Tout mesuré avant d'être cru : session WebSocket depuis le conteneur (voix inconnue refusée AVANT le démarrage, `delegation_id: null` et `content` exigés, une salutation ne se rend que si l'audio d'entrée coule), puis un vrai Chromium contre dev avec un micro jouant une question parlée — connecté en 480 ms, `session.started` sur le canal, délégation du modèle 6 ms après le dernier fragment, notre commentaire prononcé et acquitté, `session.closed` en 700 ms. Un fil WebRTC n'a besoin d'aucun hôte CSP. **Amendements du 2026-09-19** (cinq ajustements du propriétaire, puis vagues 2 et 3) : durées et voix PAR MODÈLE (0 = sans limite, plafond qui roule), entrée « Session Live (marque) », notes de ton ADR-253 vers la voix, `enableAffectiveDialog` mesuré hors jeu ; le lecteur PCM est UN AudioWorklet (le « biiiip » de Chrome 152), les deux voix sont exclusives, une fermeture du fournisseur nomme son code ; puis **le coût de la session est AFFICHÉ, jamais enregistré** — les relevés d'usage du fournisseur (`usageMetadata` par tour chez Gemini, `session.usage.updated` chez GPT-Live) sont cumulés dans le navigateur et tarifés par le tarif que le démarrage publie (`LiveRates`, depuis le cache de tarification), dessinés au milieu du bandeau dans le vocabulaire du compteur du chat avec la taille du contexte ; un coût qui exige un tarif non déclaré est indisponible, jamais partiel ; **un modèle live n'est proposé et démarré que s'il est déclaré dans la tarification LLM** (`domains/live/pricing.py`, `model_unpriced`, les non déclarés NOMMÉS) — la table gagne la paire audio (`audio_input_unit_price` / `audio_output_unit_price`, migration `f1a3c5e7b9d2` + seed + classeur v3, garde d'égalité), les sous-titres ne sont pas une option (facturés dans la minute ou au tarif texte, nécessaires aux deux fils) ; un plafond de dépense par session, optionnel, est celui du CONNECTEUR (`session_budget_eur`, issue `budget_reached`). Mesuré dans la coque Android : une offre WebRTC avec canal de données et un AudioWorklet depuis un `blob:` sous la CSP de production ; iOS pas encore.

---

### ADR-299 : le mode Live vocal — deux intelligences, une seule couture

**Fichier**: `docs/architecture/ADR-299-Live-Voice-Mode-Two-Intelligences-One-Seam.md`

**Décision** : une seconde voix, de parole à parole (Gemini Live aujourd'hui, un autre fournisseur est un transport de plus), sur un connecteur `live` que la personne configure avec SA clé — rien du fournisseur n'est compté, stocké ni montré. Trois arbitrages du propriétaire : la dépense de LIA pendant la session est comptée et AFFICHÉE en fin de mode (jamais celle du fournisseur) ; le mode n'est PAS bridé (la règle lecture seule du téléphone ADR-290 concerne la ligne d'un tiers, pas la personne devant son écran) ; le fil rapporte tout en temps réel sans perdre l'intelligence vive du modèle vocal. Les deux derniers tirent en sens contraire, et UNE forme les réconcilie : le modèle vocal possède la conversation (écoute, parole, interruption, comblement) et DÉLÈGUE toute demande de donnée ou d'action au moteur du chat par UNE fonction `send_to_lia` (`NON_BLOCKING`) que le navigateur transforme en `POST /chat/stream` avec le cookie de la personne — le tour délégué tourne dans le graphe (pipeline ou ReAct, HITL, registres, quotas, archive), se dessine dans le fil, et le pont remet à la voix une réponse aplatie et bornée, ou la question en attente de LIA, qui EST la réponse. Le lot 0 a mesuré le fournisseur avant de figer le dessin, et quatre mesures l'ont changé : la contrainte d'un jeton ne verrouille PAS l'instruction (le serveur rend le `setup`, le navigateur le rejoue) ; un jeton à usage unique ne reconnecte PAS (chaque reconnexion refrappe un jeton pour le MÊME enregistrement et reprend sur le handle) ; un nom de voix inconnu est accepté en silence (la voix est validée contre la liste vendue, datée et sourcée) ; un WebSocket de navigateur n'est accepté que par la méthode `…Constrained` avec le jeton en `access_token`. Une session par compte (revendication Redis à jeton de propriétaire, plafond d'instance, limite de frappe), l'enregistrement survivant au plafond d'une grâce pour fermer ses livres ; le fil montre le tour délégué (tampon de session + paroles prononcées), l'échange purement vocal archivé à `turnComplete` en deux lignes visibles (injectées au tour suivant comme les notifications), et UNE carte de clôture dont la dépense est la SOMME des résumés des tours délégués ; une session est UNE ligne de décision, chaque tour délégué son propre run ; capacité `live` à interrupteur, nœud de carte, `LIVE_ENABLED` ; connecteur vérifié par la liste même qui remplit le choix du modèle, niveau de réflexion offert seulement sur l'échelle ADR-245 du modèle et refusé hors d'elle ; quatre réflexes de conversation dans `users.live_preferences` ; bandeau au-dessus du fil sur le modèle de l'appel actif, composeur fermé en le disant, bouton d'entrée `aria-disabled` jamais `disabled`, yeux lisant l'état vocal de la session, mot d'éveil à l'écart (ADR-258 étendu) ; les échanges vocaux nourrissent mémoire et intérêts en fin de session. CSP : un hôte exact. Sonde `task live:probe`, parcours Playwright hermétique sur `routeWebSocket`.
...
**Décision** : l'API Drive liste UN niveau par appel ; une source n'indexait que les fichiers directement sous son dossier. UN parcours (`rag_spaces/drive_walk.py`, largeur d'abord, borné en fichiers et en dossiers, `truncated` énoncé, raccourcis ignorés, cycles coupés, sous-dossier illisible compté) partagé par la synchronisation ET le comptage ; l'ensemble des dossiers parcourus voyage avec la source (`folder_ids`, migration `b8d4f6c0e2a3`) et le push route dessus, en suivant le flux (un dossier créé sous l'arbre le rejoint, un dossier mis à la corbeille le quitte) ; un lien IMBRIQUÉ est refusé (409 `drive_folder_nested`, deux lectures d'ancêtres bornées) parce que deux sources se seraient volé les documents. Le compte est calculé par le code qui indexe (`GET …/preflight` : `new`, `modified`, `unchanged`, `unsupported`, `over_capacity`, `to_index` sous le cap de l'espace, seuil et bornes publiés) et demandé AU CLIC Synchroniser : au-delà de `rag_drive_sync_confirm_threshold` (10), un dialogue énonce les chiffres exacts — « au moins » si le parcours a été coupé — et ne poste la synchronisation que sur « Les indexer ». La synchronisation en arrière-plan enregistre désormais SA consultation (elle ne le faisait pas). Mesuré sur dev contre un vrai Drive : six dossiers descendus sous une borne de six, la coupe rapportée.

---

### ADR-303 : un échec d'outil est DIT, jamais deviné

**Fichier**: `docs/architecture/ADR-303-Tool-Failure-Restitution.md`

**Décision** : audité sur la v1.44.0, re-mesuré intact sur la v1.47.1 vingt ADR plus tard, pour un volume multiplié par six (378 échecs d'outils au registre, 164 sur sept jours). Quatre ruptures entre un producteur et un lecteur qui ne parlaient pas la même langue : le mapper publiait `failed`, un mot que le formateur ne connaissait pas, donc « ❓ plan_executor : Statut inconnu (failed) » et le champ `error` JETÉ — les deux seules branches qui le restituaient étant mortes, `error` et `connector_disabled` n'ayant AUCUN producteur ; le canal d'honnêteté lisait `step["status"] == "error"`, une clé qu'aucun écrivain n'écrit, et son test figeait la forme fictive ; côté ReAct il exigeait un JSON que `compose_tool_message` n'émet jamais (il écrit la PROSE), et un test affirmait que c'était voulu ; `track_tool_metrics` comptait succès tout retour sans exception, y compris la façon DOCUMENTÉE dont un outil échoue, si bien que le taux de succès valait 1,0 par construction et que ses deux alertes — dont une PagerDuty — ne pouvaient pas se déclencher. Sans consigne, le modèle INVENTAIT une cause : mesuré en production, 7 réponses sur 21 des tours en échec portent un faux diagnostic de configuration contre **0 sur 142** dans le témoin, et un briefing quotidien annonçait un matin sur deux que trois connecteurs `ACTIVE` n'étaient « pas configurés », marche à suivre à l'appui ; le 17/09, un outil ayant répondu « No connected user matches that exact name. Connected users: … » est devenu « aucun service de messagerie n'est connecté ». La règle interdisant cette phrase existait déjà mot pour mot — mais n'était injectée que sur un refus du VALIDATEUR, jamais sur un échec d'exécution. Décision : UN vocabulaire de deux valeurs (`AgentResultStatus`) dont chaque membre est produit ET lu, sous garde AST ; un agrégat BINAIRE où `failed_steps` porte le partiel, calculé dans le mapper pour corriger les DEUX reconstructeurs sans les toucher ; UN fait, UN canal — la directive d'honnêteté lit la forme réellement écrite, nomme l'outil, publie le TOTAL EXACT et n'est plus conditionnée à `DIAGNOSTICS_ENABLED` (dire qu'un appel a échoué n'est pas une fonctionnalité de diagnostic) ; en ReAct le verdict est STRUCTUREL (`ToolMessage.status`, mesuré survivant au checkpoint), un refus de la personne et une boucle qui se répète n'étant délibérément pas marqués ; un échec déclaré n'achète plus d'itération ; UN prédicat de succès (`core/tool_outcome`) pour le registre, les métriques et la boucle, si bien qu'une même fonction ne dit plus `failed` au registre et `ok` à Prometheus ; ce que les outils ont DIT survit (une confirmation d'action et une analyse de sous-agent atteignaient le prompt VIDE) ; et la classification par sous-chaîne de message devient un ratchet shrink-only, qui a nommé une branche morte (« Max concurrent » contre « Maximum concurrent ») rapportant une saturation de pool comme une erreur de configuration. Amende ADR-184, ADR-182, ADR-185, ADR-248, ADR-254, ADR-263, ADR-286.

---

### ADR-304 : aucune transaction ouverte pendant un appel réseau

**Fichier**: `docs/architecture/ADR-304-No-Transaction-Across-A-Network-Call.md`

**Décision** : le 2026-09-22 le balayage des réveils poussés (ADR-261) était bloqué depuis 14 minutes, 0 réveil servi depuis le démarrage contre ~206 par 24 h, une session `idle in transaction` depuis 863 s : le flux Drive lu par pages de 25, un drainage sans borne, un réveil portant le jeton du drainage PRÉCÉDENT (chaque drainage rejouait le précédent : 26 522 changements lus deux fois), la session tenue pendant les appels Google et `max_instances=1` qui sautait tous les ticks. Le recensement a trouvé la classe entière — sessions de 11 à 330 s dans le briefing, le heartbeat, les moments, le contexte téléphonique, la vue relations — toutes sur la même forme : un client connecteur construit sur la session de l'appelant, qui devait donc rester ouverte pendant que le fournisseur répondait. Décision : un client n'est JAMAIS construit sur `ConnectorService(db)` (garde exacte sur l'argument du constructeur, sans liste d'exceptions) ; UNE porte ouvre le client actif d'une catégorie dans une session courte et le ferme sur tous les chemins ; un job qui garde sa session committe ses lectures avant chaque attente ; les opérations de connecteur du tour de chat terminent chacune leur transaction ; le tampon `last_used_at` d'une clé API — un verrou de ligne tenu tout un tour, et un dict JSONB recalculé qui perdait les clés concurrentes — devient une fusion atomique à part sous `lock_timeout` borné ; le drainage Drive est borné, reprend sa place, avance son jeton par comparer-et-échanger et se rebase après N coupes ; le balayage sert un réveil à la fois sous un plafond ; les rappels, dont la « réclamation » ne faisait qu'un `flush` et tenait 100 verrous et une transaction pendant tout un lot de générations LLM, sont réclamés un par un, committés, réglés sous condition, et récupérés sur `updated_at`. Aussi : le webhook Telegram (global au bot) posé par le seul leader et plus supprimé par un worker qui s'arrête ; WebSockets sur l'implémentation sans E/S d'uvicorn (plus de `keepalive ping timeout` en ERROR à la fermeture normale d'une session Live) ; tas V8 du frontend borné sous sa limite (Node le dérivait du cgroup : 259 Mo dans 256 Mo) ; `immutable` sur `/models/` rejeté (URL non versionnées). Nouvelle alerte `PushWakeSweepStalled` (réveils en file, aucun servi) sur un nouveau compteur de mise en file. Amende ADR-261, ADR-301, ADR-263, ADR-119, ADR-283.

---

### ADR-305 : un modèle d'images déclare son offre, un client de fournisseur la sert

**Fichier**: `docs/architecture/ADR-305-Image-Model-Declares-Its-Offer.md`

**Décision** : la génération d'images avait été écrite pour un seul fournisseur et gardait son vocabulaire comme celui de l'application : `IMAGE_GENERATION_VALID_QUALITIES = ("low", "medium", "high")` et trois tailles validaient l'outil, les préférences et le redimensionnement quel que soit le modèle configuré ; l'édition OpenAI passait par l'API Responses, dont l'outil `image_generation` exécutait SON modèle par défaut (`gpt-image-1`, retiré le 2026-10-23) pendant que le coût était imputé au modèle configuré, et un modèle TEXTE (`gpt-4.1-mini`) dépensait des jetons que personne n'enregistrait ; la liste du slot image proposait des modèles du catalogue qu'aucun client ne sert ; les images de référence d'une retouche étaient gratuites ; et le service de tarifs d'images convertissait avec SON taux USD→EUR, lu d'une API au chargement. Décision : une FAMILLE (`image_generation/families.py`, règles ordonnées fournisseur + préfixes comme les profils de raisonnement) déclare ce qu'un modèle accepte et comment il est facturé — OpenAI GPT Image : `low/medium/high`, trois tailles, pas de palier, image de référence facturée en jetons ; Qwen Image 3.0 : `standard`, toute taille d'aire 512²–2048² et d'aspect ≤ 8:1, palier 1K ≤ 2 250 000 px < 2K, image de référence facturée à l'image au palier de la sortie. Un modèle qu'aucune famille ne déclare ne peut être ni tarifé, ni proposé, ni choisi, ni exécuté ; la liste du slot image vient du domaine image SEUL et l'écriture refuse le reste (`image_model_not_served`). UN client par fournisseur (`providers/`), registre vérifié à l'import et au démarrage ; l'édition OpenAI passe par `images.edit` sur le modèle configuré ; Qwen par l'endpoint compatible OpenAI de l'espace de travail, son URL résultat (valable 24 h) téléchargée aussitôt sous liste blanche d'hôtes, sans redirection, bornée, signature PNG vérifiée, le palier facturé par le fournisseur comparé au nôtre. Le coût compte les images de référence (`cost_per_input_image_usd`, EXIGÉ ou REFUSÉ selon la famille) au taux unique du cache de tarifs. Une préférence est une INTENTION validée pour sa forme et mappée à l'exécution par UN résolveur (qualité non offerte → la moins chère ; taille → même orientation, aire la plus proche ; retouche → proportions de la source dans le palier préféré), publié par `GET /image-generation/options`. La source d'une retouche est lue dans l'orientation EXIF. Les 12 tarifs Qwen (grille Allemagne/Global) et les deux lignes de catalogue voyagent par seed ET par migration `a9d3f1c7e5b2` (la production ne rejoue pas les seeds), gardée égale au seed ; sa descente désactive ce que l'ancienne révision ne sait pas servir, ramène un slot image posé sur Qwen à son défaut et les préférences dans son vocabulaire — le test PostgreSQL a trouvé l'INSERT qui liait un même paramètre à deux types sous asyncpg. Revue à froid : un échec sort d'un client comme l'erreur du contrat, avec ses FAITS (statut HTTP, code du fournisseur, délai dépassé, clé absente), et l'outil le classe par la taxonomie partagée (`http_status_to_error_code`, ADR-303) — un 429 dit « réessayer plus tard », un refus de modération « requête refusée », là où tout échec lisait `TOOL_ERROR` ; le client SDK des deux fournisseurs est construit en UN endroit (`providers/openai_sdk.py`) ; le fournisseur du slot image doit être celui du modèle (`image_model_provider_mismatch`) ; mesuré : la limite de 10 Mo de Qwen porte sur le FICHIER (une source PNG de 9,01 Mio, 12,02 Mio en base64, est acceptée). Le FORMAT choisi par la personne (PNG, JPEG, WebP), enregistré mais lu par rien jusqu'ici, est appliqué : le PNG du fournisseur est converti une fois, pour tous les fournisseurs (`image_generation/encoding.py`), JPEG aplati sur blanc, WebP gardant la transparence, qualité `IMAGE_GENERATION_ENCODING_QUALITY` ; une conversion qui échoue livre le PNG, déjà facturé (mesuré : 826 Kio en PNG, 58 en JPEG, 19 en WebP). Limite énoncée : l'image de référence et le prompt d'une retouche OpenAI restent non tarifés (jetons). Amende ADR-184, ADR-272, ADR-279, ADR-063.

---

### ADR-306 : la surface de requête Claude est déclarée une fois ; le cache de prompt est façonné, et facturé, comme l'éditeur le mesure

**Fichier**: `docs/architecture/ADR-306-Claude-Request-Surface-And-Billed-Prompt-Cache.md`

**Décision** : huit modèles Claude servis par l'API manquaient au catalogue (`claude-fable-5-1`, `claude-fable-5`, `claude-opus-5-5`, `claude-opus-5`, `claude-sonnet-5`, `claude-opus-4-8`, `claude-opus-4-7`, `claude-sonnet-4-5`), et chaque génération accepte une requête différente — tout MESURÉ sur l'API avant d'être écrit : une `temperature` non par défaut est un 400 dès Opus 4.7, un `tool_choice` forcé est refusé par Fable 5.1 et Opus 5.5, la réflexion prend six formes (aucune, budget, adaptative, adaptative à la demande avec `display`, active sauf désactivation, toujours active), une requête sans profondeur tourne à `high` (`medium` sur Opus 5.5), et Fable 5.1/Opus 5.5 LIENT un bloc de réflexion à sa conversation (400 pour un compte créé depuis le 2026-08-31 dès que l'historique avant le bloc change — le prompt système que LIA reconstruit à chaque tour suffit). UNE déclaration (`core/claude_surface.py`) que lisent les profils de raisonnement, l'adaptateur (échantillonnage), la sortie structurée (porte auto-outil là où le forçage est refusé) et le verrou de température de l'écriture admin ; une famille `anthropic_adaptive_display` (échelle de l'API Models, pas de `none` sans interrupteur, `display` suivant `exclude_from_output`, profondeur implicite déclarée) ; l'effort voyage dans `output_config`, ce qui le fait enfin atteindre le registre Article 12 ; en-tête bêta + `drop_block` en filet sur les générations qui lient. Le cache : le préfixe statique est marqué au marqueur `DYNAMIC CONTEXT` quelle que soit la forme du système (mesuré : un système en liste relisait 0 et RÉÉCRIVAIT 5 222 jetons à 1,25x, contre 5 074 lus), le point d'arrêt glissant n'est posé que dans une boucle d'outils, la réflexion des tours précédents n'est plus rejouée, TTL 5 minutes seulement. La facturation : une écriture de cache coûte 1,25x l'entrée chez Anthropic et était facturée 1x — `UsageTokens.cache_write` (lue sur la clé générique, seule présente sur le chemin que LIA emprunte, mesuré, ET sur la ventilation TTL) passe par toutes les portes de coût sous garde AST, et c'est le TARIF qui dit le supplément (`cache_write_multiplier`, 1,25 pour Anthropic, 1,0 ailleurs). Trouvé au passage : `TokenCaptureHandler`, neuvième copie du calcul d'usage, facturait deux fois les jetons lus en cache sur quatre surfaces, tous fournisseurs ; le message pair relayé était tarifé à zéro ; la table des fenêtres donnait 200K à Opus 4.7/4.8 et à la paire 4.6. Mesuré sur dev par la chaîne réelle : 5 076 jetons écrits facturés 0,012884 USD, l'arithmétique exacte de l'éditeur (0,010346 avant). Catalogue et tarifs par seed ET migration `c3e7a1f5d9b2`, provenance `verified` ; le garde de budget de réflexion lit la profondeur implicite. Volet OpenAI (même jour) : GPT-5.6 et GPT-6 cachent par point d'arrêt et facturent l'écriture 1,25x, et le point d'arrêt implicite couvrait le contexte du tour — chaque appel RÉÉCRIVAIT tout son prompt (mesuré : 0 lu, 2 873 réécrits) ; `providers/openai_payload.py` pose un point d'arrêt à la fin du préfixe statique (2 832 relus à 0,1x, 41 écrits ; mode implicite conservé pour les boucles d'outils), réservé aux familles déclarées puisque dix modèles antérieurs refusent le champ (400) ; la `prompt_cache_key` s'arrête elle aussi au PREMIER marqueur (un message système sans marqueur placé après — blocs de contexte ReAct, résultats d'agents — était haché en entier : une clé, donc un cache, par tour ; 0 relu avant, 2 831 après) ; la facturation est UNE ligne (`openai` à 1,25) parce que seules ces deux générations signalent une écriture. Amende ADR-087, ADR-244, ADR-245, ADR-263 (lot 7), ADR-272, ADR-284.

---

### ADR-307 : un connecteur qui ne demande rien à la personne appartient à l'instance

**Fichier**: `docs/architecture/ADR-307-Keyless-Connectors-Belong-To-The-Instance.md`

**Décision** : Wikipédia, la navigation de pages, Google Places, Google Météo et Google Environnement ne demandent ni consentement OAuth ni clé personnelle ; depuis le 2026-09-12 chaque nouveau compte les recevait en lignes `ACTIVE` et « Mes connecteurs » les proposait avec un interrupteur. Décision du propriétaire (2026-09-23) : ils ne s'affichent plus, personne ne peut les couper, et les comptes qui les avaient coupés en production les retrouvent. Mesuré avant de décider : seuls Places, Météo et Environnement lisaient leur ligne (l'interrupteur de Wikipédia et du navigateur n'avait jamais rien fait) ; masquer l'écran seul laissait cinq trous — comptes antérieurs au 2026-09-12 sans ligne, lignes `REVOKED` après qu'un administrateur a rallumé un type, `DELETE`/`PATCH` ouverts, démonstrateur qui refuse toute activation, et météo où Google Météo et OpenWeatherMap étaient actifs ensemble, départagés par la ligne modifiée la plus récemment (une réactivation en masse aurait retiré OpenWeatherMap à ceux qui l'avaient choisi) ; `connector_type` et `status` stockent les NOMS de l'enum. Décision : l'INSTANCE décide, en un seul endroit (`connectors/keyless.py` : interrupteur global de l'administrateur, clé de plateforme, `BROWSER_ENABLED`), et `is_connector_active` lui répond sans lire les lignes du compte ; plus aucune ligne par compte — la migration `b2e6d0f4a8c1` les supprime quel que soit leur statut, l'étape d'inscription, `POST /connectors/google-places/activate` et `activate_places_connector` disparaissent, `APIKeyActivationRequest` refuse un type sans clé (activation et rotation) ; dans la catégorie météo le fournisseur que la personne a configuré gagne et Google Météo est le DÉFAUT de l'instance, jamais un rival ; les réglages ne les listent ni ne les mentionnent, l'onboarding ne demande plus de les activer, et un test refuse un type sans clé dans les listes du frontend ; un service retenu par l'instance répond `CONFIGURATION_ERROR` « indisponible sur cette instance » (jamais « allez dans Paramètres ») et un 401/403 sur la clé de plateforme n'affiche plus « Reconnecter » ; la sonde météo du heartbeat lit la catégorie. Conséquences : toujours actifs par construction, y compris quand un type est rallumé ou une clé de plateforme ajoutée ; plus d'e-mail par compte quand un administrateur coupe un de ces types (accepté) ; la carte des capacités, la liste de démarrage, la liste admin et la jauge Prometheus ne comptent plus cinq lignes que personne n'a choisies. Rejeté : garder les lignes et les réparer (cinq réparations qui pouvaient chacune diverger). Amende ADR-126, ADR-134, ADR-184.

---

### ADR-308 : un seul flag fait du préfixe d'un tour ReAct celui du tour précédent — tous les outils, et le contexte du tour après la question

**Fichier**: `docs/architecture/ADR-308-ReAct-Cross-Turn-Prompt-Cache.md`

**Décision** : le premier appel d'un tour ReAct ne relisait presque rien du cache du fournisseur (mesuré en production sur DeepSeek : 4,9 % de ~40K jetons, dont 28K de schémas d'outils), pour trois raisons mesurées : la sélection par pertinence (ADR-293) change la liste d'outils à chaque tour, alors qu'Anthropic et OpenAI cachent les outils EN TÊTE ; le prompt système se termine par les données du tour (date, liens de types, mémoires, savoirs, skills), et DeepSeek place le système AVANT les outils ; et la fenêtre d'historique glisse d'un tour à chaque tour (pas un levier). Un banc réel (396 tours, 14,80 USD, sélecteur et prompt réels, fenêtre glissante, régimes à chaud et à froid pondérés par les écarts de production : 57 % des tours ReAct sous 5 min, 74 % sous 30 min, 78 % sous 1 h) a mesuré par tour : tous les outils + contexte après la question = −18 % sur Claude Sonnet 5, −34 % sur gpt-5.6-luna, −42 % sur deepseek-flash (−66 % si le cache vit un jour), −18 % sur qwen3.7-plus ; tous les outils SANS déplacer le contexte = +44 % sur DeepSeek ; +18 % sur qwen3.5-plus, qui n'a AUCUN cache de prompt à Francfort ; aucun fournisseur ne refuse 215 outils ; qualité du premier outil inchangée sur Claude, GPT-5.6 et DeepSeek (contrôle sur 12 questions), dégradée sur Qwen sans raisonnement. Décision (arbitrage du propriétaire : un seul flag, même comportement pour tous les modèles) : `REACT_CROSS_TURN_CACHE_ENABLED`, désactivé par défaut — sans lui rien ne change ; avec lui `ReactToolSelector.select` lie tous les outils disponibles dans l'ordre d'enregistrement et `react_call_model_node` place le contexte du tour après la question (`nodes/react_turn_layout.py`) : le système de tête est le prompt statique terminé par sa ligne de marqueur (là où coupent les façonneurs Anthropic/OpenAI et où s'arrête la clé de cache OpenAI), le contexte (ligne de marqueur, partie dynamique, blocs) suit la question et précède les messages de la boucle. La forme est DÉCLARÉE par fournisseur (`CONTEXT_PLACEMENT`, vérifiée au démarrage) : message système après la question pour OpenAI, DeepSeek, Qwen (mesuré) ; contexte ajouté au texte du message de la question pour Anthropic (refuse un système non initial), Gemini (`langchain-google-genai` le refond dans l'instruction système), Ollama (gabarit du modèle) et Perplexity (alternance stricte). Quand tous les outils ne peuvent pas être liés, le tour garde la sélection par pertinence, compté par raison (`react_cross_turn_cache_fallback_total`, tableau 20) : `cap` (plus d'outils que `REACT_AGENT_MAX_TOOLS`, dont la borne passe de 200 à 400 : le compte principal de production a déjà 212 outils et le propriétaire veut de la marge pour ses serveurs MCP — un tel catalogue, environ 140K jetons de schémas, vise un modèle à fenêtre d'un million) et `window` (schémas au-delà de `REACT_CROSS_TURN_CACHE_MAX_WINDOW_FRACTION` de la fenêtre du slot, 0,5). Rien n'entre dans le checkpoint. Conséquences : en production environ −41 % par tour ReAct (1 à 2 USD par semaine au volume actuel) ; un tour à froid coûte plus ; Qwen 3.5/3.6 paient plus sous le flag (des marqueurs de cache explicites seraient un levier distinct) ; non mesurés : Gemini, Ollama, Perplexity, OpenAI avant 5.6. Rejeté : un mode `auto` par modèle, le déplacement réservé à DeepSeek, lier au-delà du plafond ou couper dans l'ordre d'enregistrement, un second message système de tête, le TTL d'une heure. Amende ADR-293, ADR-169, ADR-306.

---

### ADR-309 : une seule mise en page de prompt pour tous les mécanismes de cache — le stable avant le volatil, une seule frontière, aucune écriture que personne ne relit

**Fichier**: `docs/architecture/ADR-309-One-Prompt-Layout-For-Every-Cache-Mechanism.md`

**Décision** : règle du propriétaire (2026-09-23) — tous les fournisseurs et modèles sont paramétrables, donc aucune décision ne raisonne sur l'un d'eux, et l'on vise la meilleure optimisation MOYENNE. Hors de la boucle ReAct (ADR-308), un tour coûtait à peu près pareil quel que soit le modèle de la boucle (0,005 à 0,006 USD : 6 % d'un tour à boucle chère, 58 % d'un tour à boucle bon marché), pour cinq causes mesurées : la queue dynamique du prompt de réponse change à chaque tour (un cache automatique ne relisait que la partie fixe, 24,5 % de 12 020 jetons) et portait la conversation DEUX fois (un texte `<History>` coupé à 500 caractères par message, puis les messages eux-mêmes, entiers) ; un appel unique envoyait tout son prompt en UN message utilisateur, que les façonneurs de l'ADR-306 ne marquent pas (sur gpt-6-luna, chaque extraction relisait 0 % et écrivait tout à ×1,25, vérifié au centime) ; une requête sans outils écrivait sa queue non relue à ×1,25 ; la fenêtre d'historique glissait d'un tour à chaque tour ; Qwen 3.5/3.6 n'ont AUCUN cache implicite, seulement un explicite, et gemini-3.7-flash ne relisait rien sous une taille de requête bien au-dessus de son minimum documenté de 4 096 jetons (rien jusqu'à 10,3K sur 11 essais, ~12,3K relus à 17,8K ; LIA lit correctement le cache). Décision : une seule lecture de la frontière (`core/prompt_layout.split_at_marker`) ; un appel unique envoie sa partie fixe en message système (`single_call_messages`, garde AST sur l'appel) — analyseur, initiative, extractions de mémoire, d'intérêts et du journal, dont le prompt sans frontière (les données du tour entre ses règles et sa persona d'analyste) met désormais règles, contrôle final, format de sortie et persona avant le marqueur (2 545 jetons) ; une requête qu'aucune boucle d'outils ne peut prolonger n'écrit que ses points d'arrêt déclarés (mode explicite GPT-5.6/6 sans outils) ; le nœud de réponse envoie la conversation une seule fois, en messages entiers (le texte `<History>` disparaît, et le filtre des plans refusés couvre désormais tout), et ses sections gardent leur ordre d'attention, puisque chacune change à chaque tour (le « profil » est une recherche de souvenirs pour la requête) ; ses deux phrases écrites en Python passent dans `response_prompt_lines.txt` ; le catalogue et les intérêts ouvrent la partie dynamique de l'initiative ; sous `REACT_CROSS_TURN_CACHE_ENABLED`, l'historique de la boucle ReAct tombe par blocs (`REACT_CROSS_TURN_HISTORY_BLOCK_FRACTION`, 0,5 : entre N et N + bloc − 1 tours), la réponse continuant de glisser ; le cache explicite de Qwen est marqué sur les familles déclarées sans cache implicite (`ChatQwenCached` : préfixe système fixe, marqueur roulant en boucle d'outils, écriture DashScope lue et facturée ×1,25 ; tarif de lecture de qwen3.6-flash corrigé à 10 % à sa source). Mesuré sur la chaîne réelle en dev : qwen3.6-plus écrit 5 191 puis relit 5 191, qwen3.5-flash 5 198/5 198 (rien avant) ; gpt-6-luna en mode explicite écrit 2 721 (le seul préfixe fixe) puis relit 2 721 sans rien écrire. Mesuré sur les tours du propriétaire (2026-09-23, sans erreur) : dès leur second appel, l'analyseur relit 53-55 %, la mémoire 40-52 % et les intérêts 73 % sur gpt-6-luna, sans surcoût d'écriture ; la boucle ReAct sur deepseek-flash 94-95 % ; la réponse 22-31 % (sa partie fixe) et le journal 0 %, d'où la révision du 2026-09-24. Rejetés : un catalogue d'initiative stable de tous les outils éligibles (39 outils / 6 043 jetons contre 10-20 / 1 501-3 872, et réouverture des domaines exécutés que le nœud exclut exprès), les `cachedContents` Gemini, le marquage des Qwen à cache implicite, l'ordre « stable d'abord » et les blocs d'historique dans la réponse (écrits puis retirés : rien de sa queue ne restait stable d'un tour à l'autre). Amende ADR-306, ADR-308, ADR-284, ADR-064, ADR-147.

**Amendement (2026-09-24)** : les blocs de l'historique de la boucle bougeaient en plein tour. Leur frontière se calculait sur la LONGUEUR du fil, que le réducteur de messages rogne par la tête à chaque résultat d'outil d'un long tour. Mesuré en production au premier matin sous le drapeau : une routine du fil principal, 10 appels de boucle, un historique passé de 121 à 85 messages. Trois appels n'ont relu que le prompt et les outils (77 312 jetons) et ont refacturé 112 381, 94 403 et 152 869 jetons, alors qu'une fenêtre glissante n'aurait pas bougé. Au total, 425 000 jetons non relus sur ce fil (0 par jour du 20 au 23/09), soit 0,089 € au tarif horaire exact : presque tout le surcoût de la matinée. Le même calcul comptait deux messages par tour et pouvait ouvrir la fenêtre sur une réponse privée de sa question. Correction : la boucle garde les `K(T) = N + ((T − 1 − N) mod bloc)` derniers tours, `T` étant le compteur de tours de la conversation (`current_turn_id`), comptés depuis la FIN et entiers depuis leur question (`get_block_windowed_messages`). Un rognage de tête qui laisse les tours gardés en place ne change plus rien. Un fil qui garde tous ses tours reçoit exactement la même vue qu'avant. Si le réducteur a rogné dans le bloc, la boucle garde la fenêtre propre (N tours) ; un historique plus court que la fenêtre reste entier. Sans le drapeau, rien ne change. La preuve passe par le vrai nœud et le vrai réducteur appel par appel, par une propriété sur 400 fils aléatoires et par six mutations toutes détectées. Limites énoncées : tant que le réducteur garde moins de tours que le bloc n'en veut, la boucle garde la fenêtre propre et glisse d'un tour par tour (jamais moins de N tours), et les champs `turns_wanted`, `turns_available` et `turns_kept` du journal le montrent. Le résumé de compaction, lu en premier là où la compaction l'a ajouté, déplace la vue une fois quand un rognage l'emporte. Un message humain entré sans nouveau tour (paroles d'une session live, réponse HITL du pipeline) coûte une relecture. Rejeté : réduire le bloc à un tour par configuration (le propriétaire refuse de retirer de l'historique à la boucle).

---

### ADR-310 : un tour ReAct est jugé sur son résultat — ce que la boucle a entrepris d'obtenir finit obtenu ou déclaré, et une lacune déclarée achète une passe de reprise bornée

**Fichier**: `docs/architecture/ADR-310-ReAct-Turn-Judged-On-Its-Result.md`

**Décision** : mesuré en dev le 2026-09-23 — « je veux aller chez mon frère demain en voiture… » : la prévision est revenue pour le mauvais jour (l'outil météo ne lisait que l'anglais et ramenait toute référence inconnue à aujourd'hui, `demain` → 24/09, sans rien dire), la boucle l'a vu et a répondu « je n'ai pas pu » après 4 itérations sur les 70 de son budget, sans essayer la recherche web. Quatre causes : l'outil répondait à une autre question en silence ; les règles de reprise ne visaient qu'une ERREUR ou un résultat VIDE ; le prompt disait d'abandonner les enrichissements ; rien ne nommait une autre source ni ne vérifiait la fin du tour. Décision : un tour ReAct est jugé sur son résultat, par un mécanisme de la boucle et non par une phrase de prompt. Tout fait que la boucle a entrepris d'obtenir — la demande, et chaque vérification croisée lancée — finit obtenu ou déclaré dans un bloc `<unresolved>` (une ligne par fait, avec les échelons tentés), jamais abandonné en silence ; un détail que personne n'a demandé ni cherché n'est pas une lacune, et un fait obtenu d'une autre source porte cette source (`(fallback)`). Un seul lecteur (`declared_unresolved`, qui lit un bloc depuis sa DERNIÈRE ouverture : sur la preuve runtime, le modèle nommait la balise dans son raisonnement) et un seul prédicat (`should_recover`, qui APPELLE `react_exit_reason`) routent une lacune déclarée vers `react_recovery` tant qu'une passe reste (`REACT_RECOVERY_PASSES_MAX`, 1 par défaut, 0 = désactivé) ; le nœud n'appelle aucun modèle, retire le brouillon DÈS la passe (une seule réponse dans le fil, remise HITL comprise) et l'enregistre (`react_recovery_passes`, remis à zéro par `react_turn_reset()`) ; chaque appel suivant du tour VOIT le brouillon et une directive après son ancre, jamais écrits dans `messages` ; la réponse d'après passe remplace le brouillon, donc reprend chaque résultat, et une passe qui finit sans réponse utilisable (réponse vide, coupure de budget) rend le brouillon au lieu de l'échanger contre rien ; l'issue est comptée une fois (`react_recovery_turns_total{outcome}`, tableau 20) et affichée dans le panneau de debug. Doctrine : VERIFY (un résultat qui répond à une autre question est un obstacle, comme une erreur), une ÉCHELLE DE REPRISE (son propre appel corrigé, puis une autre source, puis la déclaration ; aucun nombre d'essais promis ; l'échelon script seulement là où le bac à sable est lié), des dates absolues ISO 8601 dans les prompts ReAct, planificateur et initiative. Un outil ne remplace jamais une valeur qu'il ne sait pas lire : la météo refuse une date illisible (`INVALID_INPUT`, format accepté et date du jour), sous un contrat publié par UNE constante lue par le manifeste ET par le schéma `@tool` (la boucle lit le schéma, jamais le manifeste). Mesuré sur un banc (huit scénarios à outils déterministes, trois familles de modèles, trois répétitions, contre la doctrine d'avant ; 1,26 USD, aucun appel Anthropic) : 58/63 faits obtenus contre 50/63 ; sur la forme de l'incident, le bon jour obtenu 7/9 contre 3/9, et 0/9 abandon contre 5/9 ; aucun appel ni passe de plus sur un tour sans obstacle (+388 jetons de prompt fixe) ; trois formulations et un analyseur corrigés par la mesure avant acceptation. Rejetés : la doctrine seule, un vérificateur indépendant, un lecteur multilingue de dates relatives, un compteur d'essais par fait, une directive persistée, une seconde recherche pour confirmer un fait de repli, la reprise en mode pipeline. Amende ADR-248, ADR-303, ADR-298, ADR-284, ADR-184.

---

---

### ADR-311 : le rythme d'échanges appartient à la personne — une seule valeur, lue une fois par tour, décide des outils, de la place du contexte et des blocs d'historique

**Fichier**: `docs/architecture/ADR-311-Exchange-Rhythm-Is-The-Persons-Choice.md`

**Décision** : décision du propriétaire (2026-09-24) — un menu « Préférences » dans les réglages, dont le premier réglage est le rythme d'échanges. Jusque-là, un seul drapeau d'instance (`REACT_CROSS_TURN_CACHE_ENABLED`) décidait pour tout le monde de lier tous les outils, de placer le contexte du tour après la question et de faire tomber l'historique par blocs. Or le gain dépend de la façon dont une PERSONNE utilise LIA : des tours rapprochés relisent le précédent, des tours espacés de plusieurs heures repaient chaque fois le premier appel plus lourd. Mesuré (banc ADR-308, 396 tours réels), le déplacement du contexte ne rapporte que si les outils qui le précèdent restent les mêmes : avec tous les outils, DeepSeek −42 % au lieu de +44 %, Claude −18 % dans les deux cas, GPT-5.6 −34 % au lieu de −37 %, qwen3.7-plus −18 % au lieu de −25 % ; sous la sélection par pertinence, deux tours successifs ne partagent qu'environ deux tiers de leurs outils et le préfixe casse avant le contexte. Les blocs, eux, ne paient que si l'historique est relu. Les trois effets voyagent donc ensemble. **Retenu** : `users.exchange_rhythm` (`frequent` | `occasional`, NULL = jamais choisi), migration `402dd18bbf2a` sans remplissage ; un compte qui n'a pas choisi suit `REACT_CROSS_TURN_CACHE_ENABLED`, devenu le défaut de l'opérateur, si bien qu'aucun déploiement ne change de comportement à la mise à jour. La valeur est lue une seule fois par tour : le service de flux lit la ligne du compte, le constructeur unique du contexte d'exécution résout le rythme effectif (`core/exchange_rhythm`, seul lecteur du réglage, garde à l'appui), le routeur le publie dans l'état du tour (`exchange_rhythm`), le setup lie les outils avec `every_tool` et chaque appel lit `frequent_exchanges(state)` pour la mise en page et les blocs. Une reprise HITL rentre dans le nœud interrompu, pas dans le routeur : un tour garde le rythme avec lequel il a commencé. Écriture par la mise à jour générique du profil (`PATCH /users/{id}`, comme le thème et la police), stricte à l'écriture, tolérante à la lecture ; le profil publie le rythme EFFECTIF. Écran : Réglages › Préférences › Personnalisation › « Préférences d'usage » (groupe radio natif, repli par pertinence énoncé quand le modèle ne peut pas contenir tous les outils, recherche et raccourcis, six langues). Les actions planifiées, les tickets du Workboard et les tours vocaux relayés suivent le rythme du compte. **Rejetés** : garder toujours le déplacement du contexte et les blocs, le choix ne portant que sur les outils (mesuré sans intérêt seul) ; un choix automatique par modèle ou par compte (règle du propriétaire) ; une route `/auth/me` dédiée (routeur au plafond de taille) ; supprimer le réglage d'opérateur (cela aurait changé le comportement de chaque déploiement). Amende ADR-308, ADR-309, ADR-293.

---

### ADR-312 : une diffusion dit à qui elle s'adressait, et l'administrateur relit ce qui est parti

**Fichier**: `docs/architecture/ADR-312-A-Broadcast-Says-Who-It-Was-Addressed-To.md`

**Décision** : demande du propriétaire (2026-09-24) — sous le formulaire d'envoi, l'historique paginé des diffusions (date, destinataires, délai d'expiration). L'écrire a révélé trois défauts mesurés : une diffusion ciblée était stockée SANS ses destinataires, si bien que la liste des non-lus la servait ensuite à TOUS les comptes à leur connexion suivante ; une sélection vide valait « tout le monde » ; les jetons FCM se lisaient compte par compte (N+1). **Retenu** : `admin_broadcasts.audience` (`all` | `selected`) et `admin_broadcast_recipients` (une ligne par compte adressé), l'audience RÉSOLUE avant l'écriture de la ligne (Archive-First conservé) et une sélection qui n'adresse aucun compte actif refusée (400) plutôt qu'élargie ; la liste des non-lus filtre sur l'audience ; migration `a19e985e4ce5` idempotente qui relit le journal d'audit (`admin_broadcast_sent`) pour rendre leur audience aux diffusions ciblées passées, un audit mal formé laissant la ligne à `all` sans interrompre la mise à jour ; `GET /notifications/admin/broadcasts` (superutilisateur) sert une page et son total EXACT (ADR-185), l'audience avec un échantillon borné et le nombre exact de destinataires, l'instant et le délai d'expiration, les accusés de lecture, en quatre requêtes quelle que soit la taille de page, bornes publiées (ADR-184), message tel qu'écrit ; jetons FCM lus une fois par groupe de langue ; l'historique se rafraîchit après un envoi. Prouvé sur PostgreSQL réel. Amende la fonction de diffusion, ADR-185, ADR-184.

---

### ADR-313 : la mémoire à long terme devient une recherche active, par une seule porte

**Fichier**: `docs/architecture/ADR-313-Long-Term-Memory-As-An-Active-Lookup.md`

**Décision** : question du propriétaire (2026-09-24) — un domaine de recherche et lecture de la mémoire aiderait-il le mode ReAct ? Le profil injecté à chaque tour est classé sur le MESSAGE de la personne : rien sur un sujet que le tour découvre en chemin (l'expéditeur d'un e-mail lu, un lieu nommé dans un document, « les règles que je t'ai données »). En l'écrivant, un défaut : la recherche du téléphone (`recall_memories`) et le contexte d'un appel au propriétaire passaient leur question SANS vecteur au constructeur de profil, qui servait alors les dix souvenirs les plus RÉCENTS quelle que soit la question. **Retenu** : une seule porte, `domains/memories/search.py` (`embed_lookup` vectorise une question comme une CLÉ, `search_memories` sert les souvenirs vivants au-dessus du seuil de pertinence, catégories en option ; vecteur calculé avant l'ouverture de session, ADR-304) ; tous les appelants y passent (faits mémoire du planificateur, de l'initiative et de la résolution de références, rappel 360° d'une personne, recherches du téléphone — qui cherche désormais au lieu de lister) ; un domaine routable `memory` et un outil en lecture seule, `search_memories_tool` (`memory_agent`), sans OAuth ni HITL, bornes publiées depuis leurs sources (catégories = `MemoryCategoryType`, plafond = `MEMORY_MAX_RESULTS`), préférence `memory_enabled` respectée, souvenir sensible signalé avec sa nuance d'usage comme obligation, échec classé et jamais lu comme « rien en mémoire » (ADR-303) ; le téléphone reste hors du domaine (`PHONE_DOMAINS`) ; le générateur de skills le connaît. Prouvé sur PostgreSQL réel. Amende ADR-070, ADR-293, ADR-304, ADR-303.

---

### ADR-314 : un e-mail à soi-même ne demande aucune confirmation, et une routine peut l'envoyer

**Fichier**: `docs/architecture/ADR-314-An-E-Mail-To-Oneself-Needs-No-Confirmation.md`

**Décision** : demande du propriétaire (2026-09-24) — pas de HITL ni de brouillon pour les e-mails envoyés à sa propre adresse, comme l'appel téléphonique personnel, pour que les actions planifiées puissent écrire à la personne. Arbitrages : l'adresse de la boîte connectée, et sans boîte connectée l'adresse du compte via la messagerie de LIA (Q1) ; une adresse propre tapée en clair n'est PAS détectée (Q2 : un destinataire qui saute parfois la confirmation serait une cible pour une injection). **Retenu** : un outil à part, `send_email_to_me_tool`, dont le destinataire n'est PAS un paramètre, `reversible` avec sa raison écrite, donc consigné par la porte des effets en conversation comme en routine (précédent `call_me`, ADR-290) ; une boîte ACTIVE envoie à SA propre adresse telle que le fournisseur l'énonce (`get_own_address` sur les trois clients, contrat de parité ; une boîte qui n'en énonce aucune refuse plutôt que de deviner) ; sans boîte, le relais de LIA écrit à l'adresse du compte seulement si elle est VÉRIFIÉE ; une boîte connectée mais en erreur laisse le relais livrer et l'avis « Reconnecter » dit pourquoi ; une seule résolution du contenu pour les deux outils d'envoi (`emails/content_generation.py`, cas « note à soi », config du tour transmise, route de dépense déplacée) ; un corps en texte brut échappé dans la partie HTML du relais ; rien n'est « envoyé » sans l'acceptation d'un fournisseur ou du relais (ADR-303). En chemin : les journaux INFO ne portent plus l'instruction ni le sujet (outil d'envoi) ni les destinataires (Outlook) ; classes mortes et mot-clé fantôme retirés (plafond du module 1 253 → 1 033 lignes logiques). Amende ADR-263, ADR-276, ADR-290, ADR-303, ADR-304.

---

### ADR-315 : un prompt d'image peut être réécrit avec des techniques reconnues, jamais dénaturé

**Fichier**: `docs/architecture/ADR-315-Image-Prompt-Enhancement.md`

**Décision** : demande du propriétaire (2026-09-24) — un LLM dédié qui comprend la demande et l'améliore par des techniques reconnues (« image réaliste d'un chat » → « photo 85 mm, f/1.8… ») sans la dénaturer, optionnel et activé explicitement dans « Génération d'images IA » ; arbitrage Q4 : l'image seule, la réécriture n'est pas montrée. **Retenu** : un créneau `image_prompt_enhancement` (registre, défauts, `LLMType`, description admin en six langues ; appel court structuré, sans raisonnement là où le profil le permet) ; un prompt versionné fondé sur les guides des deux fournisseurs servis (guide de prompting d'image d'OpenAI, réécrivain livré par Qwen avec Qwen-Image) qui fige ce qui ne doit pas changer (sujets, nombre, actions, décor, style nommé, contraintes, texte à rendre mot pour mot entre guillemets, aucun texte ajouté) et énonce les techniques (ordre, termes photographiques, médium et technique, style le plus simple si aucun n'est nommé, fond discret, phrases et non listes de mots-clés, jamais de taille d'image), un seul exemple en anglais, la demande traitée comme donnée ; des contrôles déterministes (réponse vide, au-delà de la borne publiée `IMAGE_PROMPT_ENHANCEMENT_MAX_CHARS`, ou texte cité perdu → écartée) ; jamais une barrière : plafond, échec, troncature ou rejet envoient le prompt ORIGINAL ; la génération seule est améliorée, jamais une retouche ; opt-in de la personne (`users.image_generation_prompt_enhancement`, migration `79db1056a28e`) et retrait possible par l'opérateur (`IMAGE_PROMPT_ENHANCEMENT_ENABLED`), UNE lecture de l'interrupteur partagée par l'outil et la route des options (`prompt_enhancement_available`) ; ce que le fournisseur a reçu est ce qui est facturé et audité, la galerie garde les mots de la personne ; `image_prompt_enhancement_total{outcome}` sur le tableau 05. Qualité non encore mesurée sur modèles réels (mesure payante à budgéter). Amende ADR-305, ADR-244, ADR-285, ADR-275, ADR-309, ADR-184, ADR-272.

---

### ADR-316 : une image générée se partage avec une connexion sous forme de copie, commentaire facultatif cité littéralement

**Fichier**: `docs/architecture/ADR-316-Sharing-A-Generated-Image-With-A-Connection.md`

**Décision** : demande du propriétaire (2026-09-24) — partager une image générée avec un pair comme un message, idéalement l'image seule, qui arrive dans ses fichiers générés comme s'il l'avait générée à la réception ; arbitrages : l'image s'affiche dans le chat du destinataire (Q3), un commentaire facultatif l'accompagne (Q4). **Retenu** : le clic explicite est la confirmation (« Partager avec une connexion » sur chaque carte d'image du chat et de la galerie, boîte de dialogue : connexions acceptées en groupe radio, commentaire facultatif) — ni assistant, ni brouillon, ni appel de modèle ; `POST /peers/connections/{id}/images` vérifie, copie, enregistre et VALIDE, puis atteint le chat du destinataire sur une session à part, au mieux (ADR-304) ; seules les images GÉNÉRÉES, vivantes, du propriétaire se partagent, sur une connexion acceptée sans blocage (404 masquant l'existence, codes `peers_*` traduits) ; quotas journaliers par expéditeur et par paire comptés sur le registre `peer_image_shares` et sérialisés par un verrou consultatif transactionnel (deux partages en course pour le dernier créneau ne passent pas tous deux, prouvé à deux connexions sur PostgreSQL réel et prouvé en échec sans le verrou ; 429 avec `Retry-After`) ; copie rangée comme une image générée à la réception (origine `generated_image`, durée de vie propre, titre de l'expéditeur, `attachments.shared_by_name` affiché « Partagée par … ») ; registre bilatéral sans aucun commentaire, exporté aux deux et purgé avec l'un ou l'autre compte ; bulle de pair `proactive_peer_image` (teinte, répondre, bloquer) portant la même carte qu'une image générée, en direct comme au rechargement, ligne dans la langue du destinataire et commentaire en citation LITTÉRALE (`shared/markdown_literal` : ni image-traceur, ni lien déguisé, ni balisage ; par références numériques et non par antislashs, que l'étape mathématique du chat rendait en formule — mesuré au navigateur), borné à 500 caractères publiés à la boîte de dialogue ; `peers_image_shares_total{outcome}` sur le tableau 09, dans une rangée qui câble aussi deux métriques peers jusque-là invisibles ; le scan axe du dialogue a révélé un défaut antérieur (texte atténué à 4,19:1 sur le verre dépoli des dialogues), corrigé pour les descriptions des primitives (`text-foreground/80`). Amende le programme peers (ADR-180, ADR-186), ADR-279, ADR-304, ADR-185, ADR-184.

---

### ADR-317 : une ligne de journal porte des faits, jamais les mots de la personne

**Fichier**: `docs/architecture/ADR-317-A-Log-Line-Carries-Facts-Never-The-Words.md`

**Décision** : demande du propriétaire (2026-09-24) — traiter les deux sujets pour la conformité RGPD : les paramètres SQL que `BaseRepository` écrivait en ERROR, et tous les journaux au-dessus de DEBUG qui portaient du contenu personnel. **Mesuré avant tout changement** : 438 valeurs au-dessus de DEBUG portaient les mots de la personne ou un texte coupé dedans (requêtes de recherche, thèmes d'intérêt dont la santé, extrait de conversation de 500 caractères, consignes de brouillon, réponses de clarification, raisonnement du modèle sur la personne, sorties de scripts, noms de contacts, libellés, fichiers, dossiers et espaces, URL visitées, coordonnées GPS, réponses CardDAV de 2 000 caractères) ; sur la base de dev, `hide_parameters` retire la ligne `[parameters: …]` et rien d'autre — le `DETAIL` de PostgreSQL cite la clé ou la ligne entière, asyncpg et une conversion refusée citent la saisie, psycopg ajoute `CONTEXT` — et chaque traceback porte tout cela ; le libellé de métrique était décidé par les mots du message ; `filename` contournait le filtre et les clés de fournisseur en query string (`key=`, `appid=`) n'étaient pas masquées. **Retenu** : à la source, tout moteur `hide_parameters=True` (garde) et une erreur de base décrite par ses FAITS lus sur les attributs du pilote (`infrastructure/database/errors.py` : SQLSTATE, contrainte, table, colonne ; libellé de métrique par SQLSTATE ; `BaseRepository` factorisé) ; au filtre, au-dessus de DEBUG, la citation retirée des textes d'erreur reconnus par leur format fixe (`observability/quoted_content.py` : DETAIL, CONTEXT, paramètres, argument asyncpg, messages PostgreSQL citant l'entrée, `input_value=` de Pydantic jusqu'au DERNIER marqueur), traceback compris, paramètres de recherche d'URL retenus, clés de fournisseur masquées à tous les niveaux, noms et suffixes de contenu étendus (un suffixe ne rédige que du texte), contournement réduit à ce que la chaîne produit ; aux sites, les 438 valeurs devenues longueur, nombre, identifiant ou hôte, ou passées en DEBUG, `log_unreadable_text` pour une sortie de modèle illisible (longueur au niveau, mots en DEBUG), cinq messages d'exception qui ne citent plus la valeur ; un garde qui lit la VALEUR et non son nom (`test_log_content_guard.py` : aperçu tronqué, feuille de contenu, dernier mot décisif, `name=` nu refusé), 26 exceptions motivées par écrit et qui échouent si elles ne correspondent plus à une ligne. Limites énoncées : les messages de nos exceptions en général, les textes d'erreur de fournisseurs hors formats reconnus, le niveau DEBUG en production, et les lignes déjà dans Loki jusqu'à l'expiration de la rétention. Amende ADR-027, ADR-095, SEC-012, ADR-303, ADR-266.

---

### ADR-318 : des réponses exactes, et les registres de LIA, sous forme d'outils

**Fichier**: `docs/architecture/ADR-318-Assistant-Tools-Exact-Answers-And-Own-Records.md`

**Décision** : demande du propriétaire (2026-09-24) — six outils pour l'assistant : une calculatrice, l'arithmétique des dates, la conversion de devises, une recherche dans le journal, les registres d'activité, les fichiers générés. **Mesuré avant tout changement** : un calcul, une date ou une conversion revenait plausible et non vérifié (la boucle ReAct avait `run_python_tool`, derrière le bac à sable ; le pipeline, rien) ; la source des taux de référence que la facturation lit publie 30 devises, date chaque cotation (`"date": "2026-09-24"`), répond 404 à un code inconnu et 422 au même code deux fois ; le journal n'était injecté que par pertinence au MESSAGE, les registres (ADR-263) et la galerie (ADR-279) étaient illisibles pour le modèle. **Retenu** : un domaine `calculation` en lecture seule, sans donnée personnelle, dans les deux modes — `calculate_tool` évalue une expression EXACTEMENT (arbre `ast` parcouru nœud par nœud, jamais exécuté ; décimal à la précision publiée, `Inexact` devenu `approximate` ; parcours itératif ; `0 ** -1` et `ln(0)` refusés parce que le décimal rend des infinis sans lever, mesuré ; `9 ** 9 ** 9` déborde en 0,26 ms ; `^ × ÷` réparés, virgule décimale et pourcentage refusés avec la grammaire), `date_time_tool` répond à six opérations (écart en jours, semaines, calendrier et temps ÉCOULÉ sur des instants absolus — Python soustrait deux dates partageant un `tzinfo` sur l'horloge murale et lisait 24 h pour les 25 d'un changement d'heure, mesuré ; ajout avec jour manquant ramené à la fin du mois et dit ; jours ouvrés du lundi au vendredi, jours fériés NON exclus et dits ; conversion de fuseau), `convert_currency_tool` convertit au taux de référence de la BCE et nomme son jour (`CurrencyRateService.get_quote` ; une paire non publiée lève `UnsupportedCurrencyError`, jamais retentée ni mise en cache comme une panne, le refus liste les codes publiés ; `get_rate` garde son contrat de facturation) ; le contrat de date d'ADR-310 quitte le module météo pour `core/date_contract.py` (`read_moment`, `period_bounds` : un jour est un jour ENTIER dans le fuseau de la personne) ; `search_journal_tool` (domaine `journal`, enregistré sous `JOURNALS_ENABLED`) : une porte (`journals/search.py`), le sujet plongé par le modèle d'embedding du JOURNAL avant toute session, niveau L0 exclu, entrées rendues comptées comme injections, plancher CONFIGURÉ (`JOURNAL_CONTEXT_MIN_SCORE`) et non celui que le contrôleur adaptatif apprend pour l'injection — mesuré sur 17 entrées réelles, le 0,70 appris ne gardait qu'une des six entrées notées 0,68-0,705 pour « style des réponses » —, gardé à l'appel par la capacité (l'interrupteur cache aussi `journal_agent` au planificateur) et par la préférence LUE SUR LA LIGNE de la personne (un runtime vocal n'en porte aucune, et son défaut aurait lu « désactivé » pour tous) ; `get_my_activity_tool` (domaine `activity`) : une session, quatre requêtes (`effects/activity.py`) — les actions les plus récentes sous un plafond publié, leur total EXACT, le compte exact par issue et par domaine consulté, sous le filtre d'auteur des onglets, chaque action formulée dans la langue de la personne à partir du libellé que le registre a retenu ; `find_generated_files_tool` (domaine `generated_file`) : toutes les familles fusionnées sous un plafond avec le total exact, seulement les fichiers dont l'échéance n'est pas passée (`GalleryFilters.expires_after`), chaque fichier trouvé MONTRÉ comme la carte du chat par les files des producteurs, et la livraison des cartes ne dépend plus des drapeaux de génération (une carte en file derrière une porte fermée n'était jamais montrée ni libérée) ; coutures partagées (`tools/lookup_parameters.py`, `attachments/urls.py`, la table `_FLAG_GATED_TOOL_MODULES`) ; chaque borne est un réglage énoncé dans la formulation que le manifeste du planificateur ET le schéma de la boucle ReAct lisent (épinglé par un test) ; le téléphone gagne `calculation`, `journal` et `activity`, pas `generated_file` (des cartes qu'aucune surface vocale ne dessine). Trouvé en chemin et corrigé : les magasins sémantiques ne déclaraient aucun `text_search_mode` (mémoire `semantic`, documents `hybrid`), `FLAG_GATED_DOMAINS` oubliait `ticket` (son test lit désormais les deux sens), `search_memories` n'avait pas de libellé de progression. Limites énoncées : jours fériés, taux d'une banque ou d'une carte, trigonométrie aussi exacte qu'un flottant (signalée), un appel d'embedding par recherche dans le journal, la chronologie proactive de `domains/activity` non lue, quatre domaines sous drapeau (`image_generation`, `document_generation`, `health`, `devops`) restant au menu du routeur drapeau éteint — proposé, hors de ce changement. Amende ADR-310, ADR-184, ADR-185, ADR-263, ADR-279, ADR-226, ADR-290, ADR-301, ADR-313, ADR-280.
