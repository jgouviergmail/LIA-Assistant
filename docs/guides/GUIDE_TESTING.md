...
# pyproject.toml
[tool.pytest.ini_options]
addopts = "-ra -q --strict-markers --cov=src --cov-report=term-missing --cov-report=html --cov-fail-under=72"
```
...
```bash
# Coverage complète (applique le gate --cov-fail-under=72)
cd apps/api
...
`--cov-fail-under=72` porté par `task test:backend:unit:coverage`, que ce job
