...
# pyproject.toml
[tool.pytest.ini_options]
addopts = "-ra -q --strict-markers --cov=src --cov-report=term-missing --cov-report=html --cov-fail-under=70"
```
...
```bash
# Coverage complète (applique le gate --cov-fail-under=70)
cd apps/api
...
`--cov-fail-under=70` porté par `task test:backend:unit:coverage`, que ce job
