...
And every call is valued at the price the provider really bills: a write to a model's cache at its own rate, a time window with its days (off-peak weekends when the provider bills them that way), a route at the tier its request triggers (a drive route with traffic or tolls is not billed like a walking route), a route matrix per element returned.
