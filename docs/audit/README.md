...
| Worksite | Status at v1.25.21 (2026-07-25) |
|---|---|
| AC-001 | **Remediated, pending re-audit.** Document search now stays available throughout a same-dimension reindex. The two embedding generations live side by side (no uniqueness constraint on the chunk table), a durable per-space `serving_embedding_model` pointer keeps every query answered by the served generation until the new one is fully built, and each space flips to the new model atomically once its rebuild completes — a crash mid-reindex is resumed by the RAG job reaper. The global "reindex in progress → skip retrieval" block is gone; retrieval groups spaces by served generation and embeds each query with that generation's model. The one case still handled as an explicit, documented maintenance window is a change of embedding **dimension**, which requires a new pgvector column. [ADR-150](../architecture/ADR-150-RAG-Generational-Continuity.md), 8 real-PostgreSQL integration tests. |
