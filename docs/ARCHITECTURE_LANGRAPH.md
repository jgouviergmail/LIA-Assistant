...
- **react_setup**: Binds the tools the turn needs — the detected domains' tools, every other family's best-ranked tools, then the head of the router's global ranking (ADR-293; the cap is a stable-sort safety net), or every available tool in registration order when the person chose frequent exchanges (ADR-311: the rhythm the router published in the turn's state, so the prefix stays the previous turn's) — builds system prompt, injects the **same memory context as the pipeline** (ADR-248: profile block, resolved references, portrait, journal, degradations — built by `react_context.py`; the two modes previously read different subsets, so a stored directive worked in one and silently did nothing in the other) + skills catalogue — published to the dedicated `react_system_blocks` state key rather than appended to the message history (ADR-169: appending re-sent every past copy on every call, destroyed the provider prefix cache and made Anthropic reject the sequence from the second turn)
- **react_call_model**: Recomposes the system blocks as a stable leading prefix, calls the LLM with bound tools, applies message windowing (for frequent exchanges, by blocks of whole turns anchored on the turn counter, so a head trimmed by the state reducer never moves the window mid-turn — ADR-309 amendment, ADR-311), and charges its own duration to the loop's **compute** budget (ADR-170: the deadline used to run on wall clock, so human approval time was billed to the loop and cut the resumed turn)
...
Safety: max iterations (`REACT_AGENT_MAX_ITERATIONS`) + hard timeout (`REACT_AGENT_TIMEOUT_SECONDS`). The ceiling starts from the router's domain span (ADR-238) and is extended only by **productive** iterations (ADR-248): a refusal, an empty result or an error does not buy the next one.

> **Sandboxed Python (ADR-249)**: `run_python_tool` lets the loop — and only this loop — write a short script and run it in the existing skills sandbox (SEC-001, `--network none`, throwaway container), for arithmetic over many rows, joins, timezone durations. The manifest declares `execution_modes={"react"}` and every catalogue reader applies the filter, so the planner never sees a tool whose execution it would be refused; the tool re-checks the mode from the typed runtime context at call time. Turn data travels on stdin, the per-turn run budget lives in graph state (a `ContextVar.set()` does not cross an asyncio task boundary), output is marked untrusted, and the source is surfaced to administrators only. Since [ADR-298](architecture/ADR-298-Sandbox-Egress-Toolbox.md) a run that declares hosts reaches the web through ONE egress proxy — the only routed member of the `lia-sandbox` internal network — with the person's own connector keys swapped in by the proxy for a per-run token the script reads; a host nobody permitted is ASKED with three answers (with the turn's data, without, refuse) and the question is settled INSIDE the loop (`nodes/react_egress_question.py` raises the interrupt and re-invokes the same call under the answer), because a dispatched draft answers from its result and drops every later step. A network run is claimed in the effect register before the container starts, under the `python_sandbox_egress` capability.
...
**Protected registry items** (v1.15.1): The intelligent filtering protection in `response_node` now extends beyond initiative items. Items of type `MCP_APP` (interactive widgets like Excalidraw diagrams) and `DRAFT` (HITL confirmation flows) are unconditionally preserved after `filter_registry_by_relevant_ids()`, as they are structural items that should never be filtered by relevance. The protection handles both dict (serialized via `model_dump`) and Pydantic `RegistryItem` object formats.
