...
>
> **Correction ([ADR-153](../architecture/ADR-153-HITL-Action-Taxonomy.md))**: this section previously claimed the *entire* resume path was language-clean. It was not — the **AMBIGUOUS branch** was missed. When the classifier returned no usable question (demoted EDIT, unclear answer), the resume mapper substituted a hardcoded French sentence, and `draft_critique.py` streams that text **verbatim** to the user. Those notices now go through `HitlResumeMessage` / `HitlMessages.get_resume_message()` in all 6 languages, and the classifier no longer invents a question at all — it does not know the user's language, so localization belongs to the mapper, which receives it from the checkpointed state.
>
