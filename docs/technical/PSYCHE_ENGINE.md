...
**Animated avatar.** On the **latest assistant message only**, the mood smiley is an animated, self-hosted Noto Animated Emoji WebP (`apps/web/public/animated-emoji/{codepoint}.webp`, CC BY 4.0 — see the LICENSE file in that directory), rendered through the shared `AnimatedEmoji` component (`components/ui/animated-emoji.tsx`). Older messages keep the static Unicode glyph: history rows are mood *snapshots*, and this guarantees at most one looping animation on screen. `ChatMessageList` computes the last assistant message id (`getLastAssistantMessageId`) and threads it down as `animateEmoji`; `AnimatedEmoji` falls back to the static glyph when the asset fails to load (per-codepoint error memory) or under `prefers-reduced-motion` (the WebP is then not even fetched). The mood→codepoint mapping lives in `MOOD_COLORS` (`psyche-colors.ts`, `codepoint` field, guarded by an asset-completeness test) and the assets are regenerated by `scripts/assets/fetch_noto_animated_emoji.py`. The personality selector in the header reuses the same component and assets (current personality always animated, menu items on hover, codepoints derived at runtime from the DB-managed emoji — best-effort, static glyph when Noto has no animated version).

**Typing indicator variants.** While LIA is composing, the typing indicator (`TypingIndicator.tsx`) picks one of six CSS animation variants at random per response (wave, orbit, equalizer, sparkle, breathe, typewriter), keeping the classic gray tint. Under reduced motion it swaps to the static three dots.

**Avatar micro-interactions.** History avatars (static mood snapshots) wake up while hovered — the animated WebP plays only during the hover, so at most one transient loop joins the single permanent one. When the live avatar's mood changes between two messages, the ring plays a one-shot `mood-ping` (scale + brightness). Relationship-stage milestones (ORIENTATION → EXPLORATORY → AFFECTIVE → STABLE, one-way) fire a one-shot toast via the headless `PsycheMilestoneWatcher` mounted on the chat page (`psyche.milestone.*` i18n keys, hydration-guarded so a page load never toasts; the toast icon is the shared animated ✨). The settings state card (`PsycheStateSummary`) animates its mood emoji on hover through the same `AnimatedEmoji` component.

**Mood glow & emotion particles.** The bubble being streamed carries a thin border + soft halo in the current mood color (`.mood-glow` + `--mood-color` CSS variable, resolved from the same psyche snapshot as the avatar; double-class specificity so the glow survives the bubble's hover styles). A whitelisted strong emotion (intensity ≥ 0.8 — joy ✨, wonder/pride 🌟, tenderness ❤️, gratitude 💖, enthusiasm ⚡, frustration 💢) fires a one-shot 3-particle burst from the live avatar: change-only, never on mount or history rows, skipped under `prefers-reduced-motion`, particles `opacity: 0` at rest so a disabled animation leaves nothing frozen.
