...
> **Scope de ce système**: Ce système Admin UI gère les **55 types LLM** (registre `LLM_TYPES_REGISTRY`) (router, planner, contacts_agent, etc.). Les configurations LLM d'infrastructure (Excalidraw, MCP description generation) restent dans `.env` via `MCPSettings`. Les clés API provider (OPENAI_API_KEY, etc.) sont dans `.env` comme fallback.
...
> La liste exhaustive et à jour est `LLM_TYPES_REGISTRY` (`src/domains/llm_config/constants.py`) — 55 types au 2026-07. Le tableau ci-dessous donne les catégories principales.
