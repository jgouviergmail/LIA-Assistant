...
**Health Metrics — iPhone Shortcuts ingestion (v1.17.0, [ADR-076](architecture/ADR-076-Health-Metrics-Ingestion.md))** : Nouveau domaine DDD `health_metrics` exposant `POST /api/v1/ingest/health` avec authentification par token Bearer (préfixe `hm_`, hash SHA-256 stocké, valeur brute révélée une seule fois). Body `{"data": {"c": <bpm>, "p": <steps_increment>, "o": <source>}}` — `p` est le nombre de pas depuis le précédent envoi (PAS un compteur cumulatif). Horodatage server-side à réception, rate-limit Redis 5 req/h/token. Validation mixte par champ : valeurs hors plages physiologiques ([20, 250] bpm pour FC, [0, 15 000] pas/sample) → NULL + log warn (sans valeur brute, RGPD), les autres champs valides du payload sont préservés. Migration Alembic `health_metrics_001` (création tables `health_metrics` + `health_metric_tokens`) puis `health_metrics_002` (rename `steps_cumulative` → `steps`). Aggregator Python (`SUM` par bucket horaire/jour/semaine/mois/année + `AVG/MIN/MAX` HR), gaps préservés (`has_data=False`). UI Settings 4 sous-sections (API + tokens, graphiques recharts, statistiques, gestion). 8 métriques Prometheus + dashboard Grafana 21. Feature flag `HEALTH_METRICS_ENABLED`.
